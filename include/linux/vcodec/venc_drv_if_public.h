 ...
 */
	typedef enum __VENC_DRV_QUERY_TYPE_T {
		...
		VENC_DRV_QUERY_TYPE_NORMAL_PRIO,	/* /< Query if recorder scenario adjust to normal
							   priority, for 6571. */
 ...
 */
	typedef enum __VENC_DRV_H264_VIDEO_PROFILE_T {
		...
		/* /< High 444 Predictive (Amendment 2) */
		...
		/* /< Scalable High Intra (Amendment 3) */
...
VENC_DRV_VIDEO_LEVEL_6,
VENC_DRV_VIDEO_LEVEL_7,
 ...
 */
	typedef enum __VENC_DRV_SET_TYPE_T {
		...
		VENC_DRV_SET_TYPE_SLOW_MOTION_POST_PROC,	/* /< Set to Slow Motion Video Recording
								for encoded bs with post processing */
		VENC_DRV_SET_TYPE_SLOW_MOTION_LOCK_HW,	/* /< Set to Slow Motion Video Recording for Lock HW */
		VENC_DRV_SET_TYPE_SLOW_MOTION_UNLOCK_HW,	/* /< Set to Slow Motion Video
								   Recording for UnLock HW */
 ...
 */
	typedef enum __VENC_DRV_GET_TYPE_T {
		...
		VENC_DRV_GET_TYPE_GET_BUF_INFO_FROM_META_HANDLE,	/* /< Get buffer virtual address from meta
									   buffer handle */
		VENC_DRV_GET_TYPE_FREE_META_HANDLE_LIST,	/* /< free a handle allocated from
 ...
 */
	typedef struct __VENC_DRV_QUERY_VIDEO_FORMAT_T {
		...
		VAL_UINT32_T u4Profile;	/* /< [OUT] video profile capability VENC_DRV_H264_VIDEO_PROFILE_T */
 ...
 */
	typedef struct __VENC_DRV_PARAM_BS_BUF_T {
		...
		union {
			...
			VAL_UINT64_T u8BSStartVADummy;	/* /< [IN] Bitstream fill start address */
		};
		union {
			VAL_ULONG_T u4BSSize;	/* /< [IN] Bitstream size (filled bitstream in bytes) */
			VAL_UINT64_T u8BSSizeDummy;	/* /< [IN] Bitstream size (filled bitstream in bytes) */
		};
 ...
 */
	typedef struct __VENC_DRV_DONE_RESULT_T {
		...
		union {
			...
			VAL_UINT64_T prBSBufDummy;
		};
		union {
			P_VENC_DRV_PARAM_FRM_BUF_T prFrmBuf;	/* /< [OUT] Input frame buffer
								   information. if address is null,
								   don't use this buffer, else reuse */
			VAL_UINT64_T prFrmBufDummy;
		};
 ...
 */
	typedef struct __VENC_DRV_PROPERTY_T {
		...
		VAL_UINT32_T u4BufUnitSize;	/* /< [OUT] Buffer unit size is N bytes (e.g.,
						   8, 16, or 64 bytes per unit.) */
 ...
 */
	VENC_DRV_MRESULT_T eVEncDrvQueryCapability(VENC_DRV_QUERY_TYPE_T a_eType,
 ...
 *   VENC_DRV_MRESULT_T     [OUT] VENC_DRV_MRESULT_OK for success, VENC_DRV_MRESULT_FAIL for fail
 */
	VENC_DRV_MRESULT_T eVEncDrvRelease(VAL_HANDLE_T a_hHandle, VENC_DRV_VIDEO_FORMAT_T a_eVideoFormat);
 ...
 *   VENC_DRV_MRESULT_T     [OUT] VENC_DRV_MRESULT_OK for success, VENC_DRV_MRESULT_FAIL for fail
 */
	VENC_DRV_MRESULT_T eVEncDrvInit(VAL_HANDLE_T a_hHandle);
 ...
 *   VENC_DRV_MRESULT_T     [OUT] VENC_DRV_MRESULT_OK for success, VENC_DRV_MRESULT_FAIL for fail
 */
	VENC_DRV_MRESULT_T eVEncDrvDeInit(VAL_HANDLE_T a_hHandle);
 ...
 *   VENC_DRV_MRESULT_T     [OUT] VENC_DRV_MRESULT_OK for success, VENC_DRV_MRESULT_FAIL for fail
 */
	VENC_DRV_MRESULT_T eVEncDrvSetParam(VAL_HANDLE_T a_hHandle,
 ...
 *   VENC_DRV_MRESULT_T     [OUT] VENC_DRV_MRESULT_OK for success, VENC_DRV_MRESULT_FAIL for fail
 */
	VENC_DRV_MRESULT_T eVEncDrvGetParam(VAL_HANDLE_T a_hHandle,
 ...
 *   VENC_DRV_MRESULT_T     [OUT] VENC_DRV_MRESULT_OK for success, VENC_DRV_MRESULT_FAIL for fail
 */
	VENC_DRV_MRESULT_T eVEncDrvEncode(VAL_HANDLE_T a_hHandle,
					  VENC_DRV_START_OPT_T a_eOpt,
					  VENC_DRV_PARAM_FRM_BUF_T *a_prFrmBuf,
					  VENC_DRV_PARAM_BS_BUF_T *a_prBSBuf, VENC_DRV_DONE_RESULT_T *a_prResult);
