 ...
 */
	typedef enum _VAL_CHIP_VERSION_T {
		VAL_CHIP_VERSION_HW_CODE = 0,	/* /< The data will be "6595" for 6595 series;
						"6795" for 6795 series, ... */
 ...
 */
	typedef enum _VAL_DRIVER_TYPE_T {
		...
		VAL_DRIVER_TYPE_H264_DEC_MAIN_CABAC,	/* /< H.264 main profile decoder for CABAC type but
							   packet is the same, just for reload. */
 ...
 */
	typedef struct _VAL_MEM_ADDR_T {
		union {
			VAL_ULONG_T u4VA;	/* /< [IN/OUT] virtual addres */
			VAL_UINT64_T u8VADummy;
		};
		union {
			VAL_ULONG_T u4PA;	/* /< [IN/OUT] physical address */
			VAL_UINT64_T u8PADummy;
		};
		union {
			VAL_ULONG_T u4Size;	/* /< [IN/OUT] size */
			VAL_UINT64_T u8SizeDummy;
		};
	} VAL_MEM_ADDR_T;
 ...
 */
	typedef struct _VAL_MEMORY_T {
		...
		union {
			...
			VAL_UINT64_T u4MemSizeDummy;	/* /< [IN]     The size of memory allocation */
		};
		union {
			VAL_VOID_T *pvMemVa;	/* /< [IN/OUT] The memory virtual address */
			VAL_UINT64_T pvMemVaDummy;	/* /< [IN/OUT] The memory virtual address */
		};
		union {
			VAL_VOID_T *pvMemPa;	/* /< [IN/OUT] The memory physical address */
			VAL_UINT64_T pvMemPaDummy;	/* /< [IN/OUT] The memory physical address */
		};
 ...
 */
	typedef struct _VAL_ATOI_T {
		...
		VAL_INT32_T i4Result;	/* /< [Out]    returns the int value produced by interpreting the input
 ...
 */
	typedef struct _VAL_STRSTR_T {
		...
		VAL_VOID_T *pvStrResult;	/* /< [Out] Returns a pointer to the first occurrence of strSearch
						   in str, or NULL if strSearch does not appear in str. */
		VAL_VOID_T *pvReserved;	/* /< [IN/OUT] The reserved parameter */
		VAL_UINT32_T u4ReservedSize;	/* /< [IN]     The size of reserved parameter structure */
	} VAL_STRSTR_T;
 ...
 */
	typedef struct _VAL_ISR_T {
		...
		VAL_UINT32_T u4IrqStatus[IRQ_STATUS_MAX_NUM];	/* /< [IN/OUT] The value of return
		registers when HW done */
	} VAL_ISR_T;
 ...
 */
	typedef struct _VAL_HW_LOCK_T {
		...
		VAL_UINT32_T u4ReservedSize;	/* /< [IN]     The size of reserved parameter structure */
		VAL_DRIVER_TYPE_T eDriverType;	/* /< [IN] The driver type */
		VAL_BOOL_T bSecureInst;	/* /< [IN] True if this is a secure instance //
