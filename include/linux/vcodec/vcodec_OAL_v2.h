...
void VCodecQueryMemType(IN void *pBuffer_VA,
			IN unsigned int u4Size, OUT VCODEC_MEMORY_TYPE_T * peMemType);

void VCodecQueryPhysicalAddr(IN void *pBuffer_VA, OUT void **pBufferOut_PA);
...
void VCodecFlushCachedBuffer(IN void *pBuffer_VA, IN unsigned int u4Size);

/* VCodecInvalidateCachedBuffer - u4Size is in byte */
void VCodecInvalidateCachedBuffer(IN void *pBuffer_VA, IN unsigned int u4Size);
...
void VCodecMemSet(IN void *pBuffer_VA, IN char cValue, IN unsigned int u4Length);

void VCodecMemCopy(IN void *pvDest, IN const void *pvSrc, IN unsigned int u4Length);
typedef struct {
...
int VCodecPthread_attr_getdetachstate(IN const VCODEC_PTHREAD_ATTR_T * attr, OUT int *detachstate);
int VCodecPthread_attr_setdetachstate(IN VCODEC_PTHREAD_ATTR_T * attr, IN int detachstate);
...
VCODEC_OAL_ERROR_T VCodecBindingCore(IN VCODEC_PTHREAD_T tid, IN unsigned int u4Mask);
...
VCODEC_OAL_ERROR_T VCodecGetAffinity(IN VCODEC_PTHREAD_T tid,
				     OUT unsigned int *pu4Mask, OUT unsigned int *pu4SetMask);
VCODEC_OAL_ERROR_T VCodecCoreLoading(IN int s4CPUid, OUT int *ps4Loading);
VCODEC_OAL_ERROR_T VCodecCoreNumber(OUT int *ps4CPUNums);
void VCodecSleep(IN unsigned int u4Tick);
int VCodec_sem_init(IN VCODEC_OAL_SEM_T * sem, IN int pshared, IN unsigned int value);
...
typedef struct {
	...
	void (*VCodecAssertFail) (IN char *ptr, IN int i4Line, IN int i4Arg);
