...
static int find_group_level(char *buf, struct task_struct *p)
{
		int depth = 0;
		struct sched_entity *se = &p->se;
		u64 w[MTK_FAIR_DBG_DEP];
		int i = 0;
		int loc = 0;
		int level = 0;
	 	for (; se && (depth < MTK_FAIR_DBG_DEP); se = se->parent) {
...
static void get_tracking_id(char *buf, int prev, int next)
{
	memset(buf, 0, MTK_FAIR_DBG_SZ);
	if(prev < 0)
		snprintf(buf, 128, "%d", next);
	else
		snprintf(buf, 128, "%d-%d", prev, next);
}
#endif /* CONFIG_SCHED_VRUNTIME_INFO */
...
#ifdef CONFIG_SCHED_VRUNTIME_INFO
TRACE_EVENT(sched_debug_runtime,

	TP_PROTO(struct task_struct *tsk, int cpu, int prev, int next),

	TP_ARGS(tsk, cpu, prev, next),
	...
	TP_STRUCT__entry(
		...
		__field( int,	cpu)
		__field( int,	depth			)
		__array( char,	group_runtime_debug,	MTK_FAIR_DBG_SZ)
		__array( char,	tracking_id,	MTK_FAIR_DBG_SZ)
	),
	...
	TP_fast_assign(
		...
		__entry->pid		= tsk->pid;
		__entry->cpu		= cpu;
		__entry->depth		= find_group_level(__entry->group_runtime_debug, tsk);
		get_tracking_id(__entry->tracking_id, prev, next);
	),

	TP_printk("cpu=%d comm=%s pid=%d Level=%d%s id=%s",
			__entry->cpu, __entry->comm, __entry->pid,
			__entry->depth, __entry->group_runtime_debug,
			__entry->tracking_id)
);
#endif
