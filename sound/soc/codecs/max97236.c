...
#else
	struct max97236_pdata  *max_pdata;
	u32 irq_gpio = 13;
	int err = 0;
	max_pdata = devm_kzalloc(&i2c->dev, sizeof(struct max97236_pdata),
				GFP_KERNEL);
	if (unlikely(max_pdata == NULL)) {
	    dev_err(&i2c->dev,
		"Failed to allocate platform data for device\n");
	    return -ENOMEM;
	}

	dev_dbg(&i2c->dev,"irq_gpio=%d err =%d name=%s of-name=%s\n",
		irq_gpio, err, id->name, i2c->dev.of_node->name);

	err = of_property_read_u32(i2c->dev.of_node,
		"irq_gpio", &irq_gpio);
	max_pdata->irq_gpio = irq_gpio;
                  max_pdata->clk_enable = NULL;
	dev_dbg(&i2c->dev,"tfa irq_gpio=%d err =%d\n", irq_gpio, err);
	i2c->dev.platform_data = max_pdata;
#endif
...
#else
static struct of_device_id max97236_match_table[] = {
	{ .compatible = "maxim,max97236",},
	{},
};
#endif

MODULE_DEVICE_TABLE(of, max97236_match_table);
...
static struct i2c_driver max97236_i2c_driver = {
	.driver = {
		   ...
		   .owner = THIS_MODULE,
#ifdef CONFIG_OF
		   .of_match_table = max97236_match_table,
#endif
		   },
