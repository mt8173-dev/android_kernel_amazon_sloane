...
static struct snd_soc_dai_link sloane_dais[] = {
	/* FrontEnd DAI Links */
	{
	 .name = "MultiMedia1",
...
static int sloane_channel_cap_set(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int sloane_channel_cap_get(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
...
static const char *const sloane_channel_cap[] = {
	"Stereo", "MonoLeft", "MonoRight"
};

static const struct soc_enum sloane_control_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(sloane_channel_cap), sloane_channel_cap),
};

static const struct snd_kcontrol_new sloane_controls[] = {
	SOC_ENUM_EXT("Board Channel Config", sloane_control_enum[0],
		     sloane_channel_cap_get, sloane_channel_cap_set),
};

static struct snd_soc_card sloane_card = {
	.name = "mt-snd-card",
	.dai_link = sloane_dais,
	.num_links = ARRAY_SIZE(sloane_dais),
	.controls = sloane_controls,
	.num_controls = ARRAY_SIZE(sloane_controls),
};

static int sloane_dev_probe(struct platform_device *pdev)
{
	struct snd_soc_card *card = &sloane_card;
...
static int sloane_dev_remove(struct platform_device *pdev)
{
...
static const struct of_device_id sloane_machine_dt_match[] = {
	{.compatible = "mediatek," MT_SOC_MACHINE_NAME,},
	{}
};

MODULE_DEVICE_TABLE(of, sloane_machine_dt_match);

static struct platform_driver sloane_machine_driver = {
	.driver = {
		   .name = MT_SOC_MACHINE_NAME,
		   .owner = THIS_MODULE,
		   .of_match_table = sloane_machine_dt_match,
#ifdef CONFIG_PM
		   .pm = &snd_soc_pm_ops,
#endif
		   },
	.probe = sloane_dev_probe,
	.remove = sloane_dev_remove,
};

module_platform_driver(sloane_machine_driver);

/* Module information */
MODULE_DESCRIPTION("ASoC driver for Sloane");
MODULE_LICENSE("GPL");
