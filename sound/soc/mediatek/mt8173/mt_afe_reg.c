...
int mt_afe_reg_remap(void *dev)
{
	...
	if (!node) {
		pr_warn("%s of_find_compatible_node(mediatek,TOPCKGEN) fail\n", __func__);
	...
	if (!node) {
		pr_warn("%s of_find_compatible_node(mediatek,SLEEP) fail\n", __func__);
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,AP_CCIF0");
	if (!node) {
		pr_warn("%s of_find_compatible_node(mediatek,AP_CCIF0) fail\n", __func__);
...
void mt_afe_set_reg(uint32_t offset, uint32_t value, uint32_t mask)
{
	...
	if (offset < AFE_ADDR_START || offset > AFE_ADDR_END)
		return;
	/* pr_notice("mt_afe_set_reg offset=%x, value=%x, mask=%x\n",offset,value,mask); */
...
{
	mt_afe_main_clk_on();
	pr_notice("+AudDrv mt_afe_log_print\n");
	...
	pr_notice("AFE_ADDA2_TOP_CON0  = 0x%x\n", mt_afe_get_reg(AFE_ADDA2_TOP_CON0));
	...
	pr_notice("AFE_MOD_DAI_BASE  = 0x%x\n", mt_afe_get_reg(AFE_MOD_DAI_BASE));
	pr_notice("AFE_MOD_DAI_END  = 0x%x\n", mt_afe_get_reg(AFE_MOD_DAI_END));
	pr_notice("AFE_MOD_DAI_CUR  = 0x%x\n", mt_afe_get_reg(AFE_MOD_DAI_CUR));
	...
	pr_notice("AFE_IRQ_MCU_STATUS  = 0x%x\n", mt_afe_get_reg(AFE_IRQ_MCU_STATUS));
	pr_notice("AFE_IRQ_MCU_CLR  = 0x%x\n", mt_afe_get_reg(AFE_IRQ_MCU_CLR));
	...
	pr_notice("AFE_IRQ5_MCU_CNT_MON  = 0x%x\n", mt_afe_get_reg(AFE_IRQ5_MCU_CNT_MON));
