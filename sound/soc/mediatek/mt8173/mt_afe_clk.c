...
static struct audio_clock_attr aud_clks[CLOCK_NUM] = {
	[CLOCK_AFE] = {"aud_afe_clk", true, true, false, NULL},
	[CLOCK_I2S] = {"aud_i2s_clk", true, true, false, NULL},
	[CLOCK_DAC] = {"aud_dac_clk", true, true, false, NULL},
	[CLOCK_DAC_PREDIS] = {"aud_dac_predis_clk", true, true, false, NULL},
	[CLOCK_ADC] = {"aud_adc_clk", true, true, false, NULL},
	[CLOCK_TML] = {"aud_tml_clk", true, true, false, NULL},
	[CLOCK_HDMI] = {"aud_hdmi_clk", true, false, false, NULL},
	[CLOCK_SPDIF] = {"aud_spdif_clk", true, false, false, NULL},
	[CLOCK_SPDIF2] = {"aud_spdif2_clk", true, true, false, NULL},
	[CLOCK_APLL22M] = {"aud_apll22m_clk", false, true, false, NULL},
	[CLOCK_APLL24M] = {"aud_apll24m_clk", false, true, false, NULL},
	[CLOCK_APLL1_TUNER] = {"aud_apll1_tuner_clk", false, false, false, NULL},
	[CLOCK_APLL2_TUNER] = {"aud_apll2_tuner_clk", false, false, false, NULL},
	[CLOCK_INFRA_SYS_AUDIO] = {"infra_sys_audio_clk", true, false, false, NULL},
	[CLOCK_TOP_PDN_AUDIO] = {"top_pdn_audio", true, false, false, NULL},
	[CLOCK_TOP_PDN_AUD_INTBUS] = {"top_pdn_aud_intbus", true, false, false, NULL},
	[CLOCK_TOP_AUD_1_SEL] = {"top_audio_1_sel", false, false, false, NULL},
	[CLOCK_TOP_AUD_2_SEL] = {"top_audio_2_sel", false, false, false, NULL},
	[CLOCK_TOP_APLL1_CK] = {"top_apll1_ck", false, false, false, NULL},
	[CLOCK_TOP_APLL2_CK] = {"top_apll2_ck", false, false, false, NULL},
	[CLOCK_SCP_SYS_AUDIO] = {"scp_sys_audio", false, false, false, NULL},
	[CLOCK_CLK26M] = {"clk26m", false, false, false, NULL}
};
...
void turn_on_afe_clock(void)
{
	...
	if (ret)
		pr_err("%s clk_enable %s fail %d\n", __func__, aud_clks[CLOCK_AFE].name, ret);
#else
...
void turn_on_dac_clock(void)
{
	...
	if (ret)
		pr_err("%s clk_enable %s fail %d\n", __func__, aud_clks[CLOCK_DAC].name, ret);

	ret = clk_enable(aud_clks[CLOCK_DAC_PREDIS].clock);
	if (ret)
		pr_err("%s clk_enable %s fail %d\n", __func__,
			aud_clks[CLOCK_DAC_PREDIS].name, ret);
#else
...
void turn_on_adc_clock(void)
{
	...
	if (ret)
		pr_err("%s clk_enable %s fail %d\n", __func__, aud_clks[CLOCK_ADC].name, ret);
#else
...
void turn_on_i2s_clock(void)
{
	...
	if (ret)
		pr_err("%s clk_enable %s fail %d\n", __func__, aud_clks[CLOCK_I2S].name, ret);
#else
...
void turn_on_hdmi_clock(void)
{
	...
	ret = clk_enable(aud_clks[CLOCK_HDMI].clock);
	if (ret)
		pr_err("%s clk_enable %s fail %d\n", __func__, aud_clks[CLOCK_HDMI].name, ret);
#else
...
{
#if defined(COMMON_CLOCK_FRAMEWORK_API)
	clk_disable(aud_clks[CLOCK_HDMI].clock);
	clk_disable(aud_clks[CLOCK_INFRA_SYS_AUDIO].clock);
#else
...
void turn_on_spdif_clock(void)
{
	...
	ret = clk_enable(aud_clks[CLOCK_SPDIF].clock);
	if (ret)
		pr_err("%s clk_enable %s fail %d\n", __func__, aud_clks[CLOCK_SPDIF].name, ret);
#else
...
{
#if defined(COMMON_CLOCK_FRAMEWORK_API)
	clk_disable(aud_clks[CLOCK_SPDIF].clock);
	clk_disable(aud_clks[CLOCK_INFRA_SYS_AUDIO].clock);
#else
...
void turn_on_spdif2_clock(void)
{
	...
	ret = clk_enable(aud_clks[CLOCK_SPDIF2].clock);
	if (ret)
		pr_err("%s clk_enable %s fail %d\n", __func__, aud_clks[CLOCK_SPDIF2].name, ret);
#else
...
{
#if defined(COMMON_CLOCK_FRAMEWORK_API)
	clk_disable(aud_clks[CLOCK_SPDIF2].clock);
	clk_disable(aud_clks[CLOCK_INFRA_SYS_AUDIO].clock);
#else
...
void turn_on_apll22m_clock(void)
{
	...
	ret = clk_prepare_enable(aud_clks[CLOCK_APLL22M].clock);
	if (ret)
		pr_err("%s clk_prepare_enable %s fail %d\n",
			__func__, aud_clks[CLOCK_APLL22M].name, ret);
#else
...
void turn_off_apll22m_clock(void)
{
	...
	clk_disable_unprepare(aud_clks[CLOCK_APLL22M].clock);
	clk_disable(aud_clks[CLOCK_INFRA_SYS_AUDIO].clock);
...
void turn_on_apll24m_clock(void)
{
	...
	ret = clk_prepare_enable(aud_clks[CLOCK_APLL24M].clock);
	if (ret)
		pr_err("%s clk_prepare_enable %s fail %d\n",
			__func__, aud_clks[CLOCK_APLL24M].name, ret);
#else
...
void turn_off_apll24m_clock(void)
{
	...
	clk_disable_unprepare(aud_clks[CLOCK_APLL24M].clock);
	clk_disable(aud_clks[CLOCK_INFRA_SYS_AUDIO].clock);
...
void turn_on_apll1_tuner_clock(void)
{
	...
	ret = clk_prepare_enable(aud_clks[CLOCK_APLL1_TUNER].clock);
	if (ret)
		pr_err("%s clk_prepare_enable %s fail %d\n",
		       __func__, aud_clks[CLOCK_APLL1_TUNER].name, ret);
...
{
#if defined(COMMON_CLOCK_FRAMEWORK_API)
	clk_disable_unprepare(aud_clks[CLOCK_APLL1_TUNER].clock);
	clk_disable(aud_clks[CLOCK_INFRA_SYS_AUDIO].clock);
...
void turn_on_apll2_tuner_clock(void)
{
	...
	ret = clk_prepare_enable(aud_clks[CLOCK_APLL2_TUNER].clock);
	if (ret)
		pr_err("%s clk_prepare_enable %s fail %d\n",
		       __func__, aud_clks[CLOCK_APLL2_TUNER].name, ret);
...
{
#if defined(COMMON_CLOCK_FRAMEWORK_API)
	clk_disable_unprepare(aud_clks[CLOCK_APLL2_TUNER].clock);
	clk_disable(aud_clks[CLOCK_INFRA_SYS_AUDIO].clock);
...
{
#if defined(COMMON_CLOCK_FRAMEWORK_API)
	int ret = clk_prepare_enable(aud_clks[CLOCK_SCP_SYS_AUDIO].clock);
	if (ret)
		pr_err("%s clk_prepare_enable %s fail %d\n",
		       __func__, aud_clks[CLOCK_SCP_SYS_AUDIO].name, ret);
#else
...
{
#if defined(COMMON_CLOCK_FRAMEWORK_API)
	clk_disable_unprepare(aud_clks[CLOCK_SCP_SYS_AUDIO].clock);
#endif
}
...
void mt_afe_power_off_default_clock(void)
{
	...
	for (i = 0; i < CLOCK_INFRA_SYS_AUDIO; i++) {
		if (!aud_clks[i].default_on)
			continue;

		if (aud_clks[i].prepare_once) {
			ret = clk_enable(aud_clks[i].clock);
			if (ret)
				pr_warn("%s clk_enable %s fail %d\n", __func__,
					aud_clks[i].name, ret);
			else
				clk_disable(aud_clks[i].clock);
		} else {
			ret = clk_prepare_enable(aud_clks[i].clock);
			if (ret)
				pr_warn("%s clk_prepare_enable %s fail %d\n", __func__,
					aud_clks[i].name, ret);
			else
				clk_disable_unprepare(aud_clks[i].clock);
		}
	}
	clk_disable(aud_clks[CLOCK_INFRA_SYS_AUDIO].clock);
#endif
