...
{
	uint32_t reg_vaule = 0;

	reg_vaule |= (MT_AFE_LR_SWAP_NO_SWAP << 31);
	reg_vaule |= (clock_mode << 12);
	reg_vaule |= (mt_afe_rate_to_idx(sample_rate) << 8);
	reg_vaule |= (MT_AFE_INV_LRCK_NO_INVERSE << 5);
	reg_vaule |= (MT_AFE_I2S_FORMAT_I2S << 3);
	reg_vaule |= (MT_AFE_I2S_WLEN_16BITS << 1);
	mt_afe_set_reg(AFE_I2S_CON3, reg_vaule, 0xFFFFFFFE);
}
			...
			uint32_t bck_inv, uint32_t clock_mode)
{
	...
	reg_vaule |= (1 << 31);	/* enable phase_shift_fix for better quality */
	reg_vaule |= (bck_inv << 29);
	reg_vaule |= (MT_AFE_I2S_IN_FROM_IO_MUX << 28);
	reg_vaule |= (clock_mode << 12);
	reg_vaule |= (MT_AFE_INV_LRCK_NO_INVERSE << 5);
	reg_vaule |= (MT_AFE_I2S_FORMAT_I2S << 3);
	reg_vaule |= (src_mode << 2);
	reg_vaule |= (wlen << 1);
	mt_afe_set_reg(AFE_I2S_CON, reg_vaule, 0xFFFFFFFE);
}
...
static irqreturn_t mt_afe_irq_handler(int irq, void *dev_id)
{
	...
	/* here is error handle , for interrupt is trigger but not status , clear all interrupt */
	if (unlikely(reg_value == 0)) {
		int i;
		pr_warn("%s reg_value == 0\n", __func__);
		mt_afe_main_clk_on();

		for (i = 0; i < MT_AFE_IRQ_MCU_STATUS_COUNT; i++)
			mt_afe_set_reg(AFE_IRQ_MCU_CLR, 1 << i, 1 << i);
