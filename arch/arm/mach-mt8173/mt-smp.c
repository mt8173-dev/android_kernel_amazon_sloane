...
{
	/* FIXME: use efuse api to get core numbers? */
...
int __cpuinit mt_smp_boot_secondary(unsigned int cpu, struct task_struct *idle)
{
	...
	if (pen_release == -1) {
		...
		if (is_cpu_type(CA7_TYPEID)) {
			...
			printk(KERN_EMERG "CPU%u, DBG_MON_CTL: 0x%08x, DBG_MON_DATA: 0x%08x\n", cpu,
			       *(volatile u32 *)(DBG_MON_CTL), *(volatile u32 *)(DBG_MON_DATA));
		} else {
			/* decode statge pc on ca15l */
			mt_reg_sync_writel(3 + (cpu - 4) * 4, CA15L_MON_SEL);
			printk(KERN_EMERG "CPU%u, CA15L_MON_SEL: 0x%08x, CA15L_MON: 0x%08x\n", cpu,
			       *(volatile u32 *)(CA15L_MON_SEL), *(volatile u32 *)(CA15L_MON));
		}
...
{
	__asm__ __volatile__("mrc p15, 0, r0, c0, c0, 5	@ pmidr\n\t"
			     "ubfx r1, r0, #8, #4		@ tid\n\t"
			     "and r0, r0, #0xf		@ cid\n\t"
...
void __init mt_smp_init_cpus(void)
{
 ...
 :  : "cc");
	...
	REG_WRITE(CCI400_SI4_SNOOP_CONTROL,
		  REG_READ(CCI400_SI4_SNOOP_CONTROL) | (SNOOP_REQ | DVM_MSG_REQ));
