...
static void __iomem *EINT_BASE = APIRQ_BASE;
...
static void mt_eint_set_timer_event(unsigned int eint_num)
{
	...
	dbgmsg("EINT Module - expires:%llu, jiffies:%llu, deb_in_jiffies:%llu, ",
	       eint_timer->expires, jiffies, msecs_to_jiffies(EINT_FUNC.deb_time[eint_num]));
				  ...
				  void (DEINT_FUNC_PTR) (void))
{
	...
	if (deint_num < DEINT_MAX_CHANNEL) {
		...
		if (request_irq
		    (MT_EINT_DIRECT0_IRQ_ID + deint_num, mt_deint_isr, IRQF_TRIGGER_HIGH, "DEINT",
		     NULL)) {
...
static int mt_eint_non_autounmask_test(void)
{
	...
	request_threaded_irq(EINT_IRQ(eint_num), mt_eint_top_isr, mt_eint_bottom_isr,
			     flag | IRQF_ONESHOT, "EINT-21", NULL);
	printk("EINT %d reauest_irq done\n", eint_num);
	mt_eint_soft_set(eint_num);
...
int gpio_set_debounce(unsigned gpio, unsigned debounce)
{
	mt_eint_set_hw_debounce(gpio, debounce >> 10);
	return 0;
}
...
unsigned int mt_eint_get_raw_status(unsigned int eint_num)
{
...
void mt_eint_set_polarity_no_ack(unsigned int eint_num, unsigned int pol)
{
	unsigned int count;
	unsigned long flags;
	unsigned int base;
...
void mt_eint_store_all_config(void)
{
	...
	for (i = 0; i < EINT_MAX_CHANNEL; i++) {
		...
		mt_eint_registration(i, EINTF_TRIGGER_HIGH, EINT_CONF.eint_func[i],
				     EINT_CONF.eint_auto_umask[i]);
...
void mt_eint_dump_status(unsigned int eint)
{
	if (eint >= EINT_MAX_CHANNEL)
		return;
	printk("[EINT] eint:%d,mask:%x,pol:%x,deb:%x,sens:%x\n", eint, mt_eint_get_mask(eint),
	       mt_eint_get_polarity(eint), mt_eint_get_debounce_cnt(eint), mt_eint_get_sens(eint));
}
#endif
