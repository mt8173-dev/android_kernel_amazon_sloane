...
int mt_devapc_emi_initial(void)
{
	...
	printk(KERN_ERR "[DEVAPC][EMI] DEVAPC0_AO_ADDRESS = %x\n", DEVAPC0_AO_BASE);
	printk(KERN_ERR "[DEVAPC][EMI] DEVAPC0_PD_ADDRESS = %x\n", DEVAPC0_PD_BASE);

	if (DEVAPC0_AO_BASE == 0 || DEVAPC0_PD_BASE == 0) {

		printk(KERN_ERR "[DEVAPC][EMI] DAPC driver does not initial so far...\n");

		node = of_find_compatible_node(NULL, NULL, "mediatek,DEVAPC_AO");

		if (node) {
			DEVAPC0_AO_BASE = (unsigned long)of_iomap(node, 0);
			printk(KERN_ERR
			       "[DEVAPC][EMI] After iomap in EMI, DEVAPC0_AO_ADDRESS = %x\n",
			       DEVAPC0_AO_BASE);
		} else {
			printk(KERN_ERR "[DEVAPC][EMI] Can't find DEVAPC0_AO compatible node...\n");
			return -1;
		}
		...
		if (node) {
			DEVAPC0_PD_BASE = (unsigned long)of_iomap(node, 0);
			printk(KERN_ERR
			       "[DEVAPC][EMI] After iomap in EMI, DEVAPC0_PD_ADDRESS = %x\n",
			       DEVAPC0_PD_BASE);
		} else {
			printk(KERN_ERR "[DEVAPC][EMI] Can't find DEVAPC0_PD compatible node...\n");
			return -1;
		}
	}
	...
	printk(KERN_ERR "[DEVAPC][EMI] DAPC driver finish initialization...\n");
	return 0;
}
...
void mt_devapc_clear_emi_violation(void)
{
  ...
  * no return value.
  */
void mt_devapc_set_permission(unsigned int module, E_MASK_DOM domain_num,
