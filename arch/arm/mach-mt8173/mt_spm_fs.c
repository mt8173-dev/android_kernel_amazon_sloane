...
static ssize_t store_pwr_ctrl(struct pwr_ctrl *pwrctrl, const char *buf, size_t count)
{
	...
	else if (!strcmp(cmd, "apmcu_ctrl"))
		pwrctrl->apmcu_ctrl = val;
	...
	spm_info("[store_pwr_ctrl] cmd = %s, val = 0x%x\n", cmd, val);
	return count;
}
...
static ssize_t golden_dump_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	p += sprintf(p, "[DDRPHY]\n");
	p += sprintf(p, "0x1000f5c8 = 0x%x ([0]=1)\n", spm_read(0xf000f5c8));
	p += sprintf(p, "0x100125c8 = 0x%x ([0]=1)\n", spm_read(0xf00125c8));
	p += sprintf(p, "0x1000f640 = 0x%x ([4]=1)\n", spm_read(0xf000f640));
	p += sprintf(p, "0x10012640 = 0x%x ([4]=1)\n", spm_read(0xf0012640));
	p += sprintf(p, "0x1000f5c8 = 0x%x ([4]=1)\n", spm_read(0xf000f5c8));
	p += sprintf(p, "0x100125c8 = 0x%x ([4]=1)\n", spm_read(0xf00125c8));
	p += sprintf(p, "0x1000f5cc = 0x%x ([8]=0,[12]=1)\n", spm_read(0xf000f5cc));
	p += sprintf(p, "0x100125cc = 0x%x ([8]=0,[12]=1)\n", spm_read(0xf00125cc));
	p += sprintf(p, "0x1000f640 = 0x%x ([2]=1)\n", spm_read(0xf000f640));
...
int spm_fs_init(void)
{
	...
	if (r)
		spm_error("FAILED TO CREATE /sys/power/spm (%d)\n", r);
