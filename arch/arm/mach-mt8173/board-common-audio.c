...
#if defined(CONFIG_SND_SOC_MT8173)
...
int __init mt_audio_init(struct mt_audio_custom_gpio_data *gpio_data,
			 int mt_audio_board_channel_type)
{
	int ret = 0;

#if defined(CONFIG_SND_SOC_MT8173)
	memcpy(&(audio_platform_data.gpio_data), gpio_data,
	       sizeof(struct mt_audio_custom_gpio_data));
