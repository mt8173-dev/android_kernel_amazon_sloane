...
static ssize_t enable_spec_fetch_show(struct device_driver *driver, char *buf)
{
	...
	if (val & (0x1 << 2))
		ptr +=
		    sprintf(ptr,
			    "[CCI] Disable speculative fetches in Control Override Register:0x%x\n",
			    val);
	else
		ptr +=
		    sprintf(ptr,
			    "[CCI] Enable speculative fetches in Control Override Register:0x%x\n",
			    val);

	val = readl(IOMEM(SPEC_CTL));
	ptr +=
	    sprintf(ptr, "[CCI] Speculative fetches info in Speculation Control Register:0x%x\n",
		    val);
 ...
 */
static ssize_t enable_spec_fetch_store(struct device_driver *driver, const char *buf, size_t count)
{
	char *p = (char *)buf;
	unsigned int num;
...
static ssize_t xbar_setting_store(struct device_driver *driver, const char *buf, size_t count)
{
	char *p = (char *)buf;
	unsigned int num;
	num = simple_strtoul(p, &p, 10);
