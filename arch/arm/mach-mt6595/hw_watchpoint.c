...
{
	int test;
	init_wp_event(&wp_event, &my_watch_data2, &my_watch_data2, WP_EVENT_TYPE_ALL,
		      my_wp_handler1);
	...
	for (j = 0; j < NUM_CPU; j++) {
		...
		*(((volatile unsigned int *)(DBGWCR_BASE + 0x2000 * j)) + i) = ctl;
		...
		printk(KERN_ALERT
		       ...
		       (unsigned long)(0x2000 * j), i,
		       *(((volatile unsigned int *)(DBGWVR_BASE + 0x2000 * j)) + i));
		printk(KERN_ALERT
		       "*(((volatile unsigned int *)(DBGWCR_BASE + 0x%4lx)) + %d) = 0x%x\n",
		       (unsigned long)(0x2000 * j), i,
		       *(((volatile unsigned int *)(DBGWCR_BASE + 0x2000 * j)) + i));
	}
	return 0;
}
