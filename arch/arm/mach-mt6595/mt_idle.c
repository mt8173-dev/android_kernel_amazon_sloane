...
#define ENABLED_AFTER_E1MP_PORTING	0
...
#define idle_err(fmt, args...)       \
	do { pr_err(IDLE_TAG); pr_cont(fmt, ##args); } while (0)
#define idle_warn(fmt, args...)      \
	do { pr_warn(IDLE_TAG); pr_cont(fmt, ##args); } while (0)
#define idle_info(fmt, args...)      \
	do { pr_notice(IDLE_TAG); pr_cont(fmt, ##args); } while (0)
#define idle_dbg(fmt, args...)       \
	do { pr_info(IDLE_TAG); pr_cont(fmt, ##args); } while (0)
#define idle_ver(fmt, args...)       \
	do { pr_debug(IDLE_TAG); pr_cont(fmt, ##args); } while (0)
...
static unsigned long slidle_block_cnt[NR_REASONS] = { 0 };
 /*SODI*/ static unsigned int soidle_block_mask[NR_GRPS] = { 0x0 };
...
static unsigned long long dpidle_block_prev_time;
 /*MCDI*/ static unsigned int mcidle_timer_left[NR_CPUS];
...
bool soidle_can_enter(int cpu)
{
	...
	if ((int)soidle_timer_left < soidle_time_critera || ((int)soidle_timer_left) < 0) {
		reason = BY_TMR;
		goto out;
	}
#else
...
static bool dpidle_can_enter(void)
{
	...
	if ((int)dpidle_timer_left < dpidle_time_critera || ((int)dpidle_timer_left) < 0) {
		reason = BY_TMR;
		goto out;
	}
#else
...
/* FIXME: for FPGA early porting */
#if 1 && ENABLED_AFTER_E1MP_PORTING
#if EN_PTP_OD
	if (ptp_data[0]) {
		reason = BY_OTH;
		goto out;
...
static int is_one_cpu(void)
{
#ifdef CONFIG_SMP
	return (atomic_read(&is_in_hotplug) == 0) && (atomic_read(&hotplug_cpu_count) == 1);
#else
	return 1;
#endif
}
...
dpidle_handler, soidle_handler, mcidle_handler, slidle_handler, rgidle_handler,};
...
static ssize_t soidle_state_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	int len = 0;
	char *p = buf;
	...
	for (i = 0; i < NR_GRPS; i++) {
		p += sprintf(p,
			     ...
			     i, grp_get_name(i), soidle_condition_mask[i], grp_get_name(i),
			     soidle_block_mask[i]);
	}
...
static ssize_t slidle_state_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	int len = 0;
	char *p = buf;
	...
	for (i = 0; i < NR_GRPS; i++) {
		p += sprintf(p,
			     ...
			     i, grp_get_name(i), slidle_condition_mask[i], grp_get_name(i),
			     slidle_block_mask[i]);
	}
