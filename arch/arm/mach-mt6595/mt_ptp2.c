...
#define ptp2_err(fmt, args...) xlog_printk(ANDROID_LOG_ERROR, TAG, fmt, ##args)
#define ptp2_warn(fmt, args...) xlog_printk(ANDROID_LOG_WARN, TAG, fmt, ##args)
#define ptp2_info(fmt, args...) xlog_printk(ANDROID_LOG_INFO, TAG, fmt, ##args)
#define ptp2_dbg(fmt, args...) xlog_printk(ANDROID_LOG_DEBUG, TAG, fmt, ##args)
#define ptp2_ver(fmt, args...) xlog_printk(ANDROID_LOG_VERBOSE, TAG, fmt, ##args)

#else				/* USING_XLOG */

#define TAG     "[PTP2] "

#define ptp2_err(fmt, args...) printk(KERN_ERR TAG KERN_CONT fmt, ##args)
#define ptp2_warn(fmt, args...) printk(KERN_WARNING TAG KERN_CONT fmt, ##args)
#define ptp2_info(fmt, args...) printk(KERN_NOTICE TAG KERN_CONT fmt, ##args)
#define ptp2_dbg(fmt, args...) printk(KERN_INFO TAG KERN_CONT fmt, ##args)
#define ptp2_ver(fmt, args...) printk(KERN_DEBUG TAG KERN_CONT fmt, ##args)
...
static unsigned int ptp2_status;	/* PTP2_ENABLE_DCC_CALIN or PTP2_ENABLE_DCC_AUTO_CAL */

static struct ptp2_data ptp2_data;

static int ptp2_lo_enable;
static int ptp2_fbb_enable;
static int ptp2_spark_enable;
static int ptp2_vfbb = 2;	/* default: 300mv */
static int ptp2_spark_count;
static CHIP_SW_VER ver = CHIP_SW_VER_01;
static int ptp2_initialized;
...
void dump_ptp2_ctrl_regs(void)
{
	...
	for (i = 0; i < PTP2_REG_NUM; i++)
		ptp2_info("reg 0x%x = 0x%x", PTP2_CTRL_REG_BASEADDR + (i << 2),
...
{
	int i;
	unsigned int val = BITS(PTP2_DET_RAMPSTART, data->RAMPSTART) |
...
void config_DCC_Auto_Calibrate(void)
{
	...
	ptp2_write(PTP2_DCC_ADDR,
		   ...
		   (BITS(PTP2_DCC_CALRATE, 0x3) |
		    ...
		    BITS(PTP2_DCC_CALIBRATE, 0x1) | BITS(PTP2_DCC_BYPASS, 0x0)
		   ));
