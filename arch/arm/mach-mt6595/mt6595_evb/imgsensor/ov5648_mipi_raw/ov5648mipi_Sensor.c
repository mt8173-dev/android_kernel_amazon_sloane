...
kal_uint16 OV5648MIPI_check_otp_wb(kal_uint16 index)
{
	...
	if (index == 2) {	/* Group 2 */
		...
		rg = (OV5648MIPI_read_cmos_sensor(0x3d07) << 8) +
		    OV5648MIPI_read_cmos_sensor(0x3d08);
		bg = (OV5648MIPI_read_cmos_sensor(0x3d09) << 8) +
		    OV5648MIPI_read_cmos_sensor(0x3d0a);
		...
		rg = (OV5648MIPI_read_cmos_sensor(0x3d0a) << 8) +
		    OV5648MIPI_read_cmos_sensor(0x3d0b);
		bg = (OV5648MIPI_read_cmos_sensor(0x3d0c) << 8) +
		    OV5648MIPI_read_cmos_sensor(0x3d0d);
	} else {
...
kal_uint16 OV5648MIPI_read_otp_wb(kal_uint16 index, struct OV5648MIPI_otp_struct *otp_ptr)
{
	...
	if (index == 2) {	/* Group 2 */
		...
		(*otp_ptr).rg_ratio =
		    (OV5648MIPI_read_cmos_sensor(0x3d07) << 8) +
		    OV5648MIPI_read_cmos_sensor(0x3d08);
		(*otp_ptr).bg_ratio =
		    (OV5648MIPI_read_cmos_sensor(0x3d09) << 8) +
		    OV5648MIPI_read_cmos_sensor(0x3d0a);
		(*otp_ptr).gb_gr_ratio =
		    (OV5648MIPI_read_cmos_sensor(0x3d0b) << 8) +
		    OV5648MIPI_read_cmos_sensor(0x3d0c);
	} else if (index == 1) {	/* Group 1 */
		/* read otp bank 0 */
		OV5648MIPI_write_cmos_sensor(0x3d84, 0xc0);
		OV5648MIPI_write_cmos_sensor(0x3d85, 0x00);	/* OTP start address, bank 0 */
		...
		(*otp_ptr).rg_ratio =
		    (OV5648MIPI_read_cmos_sensor(0x3d0a) << 8) +
		    OV5648MIPI_read_cmos_sensor(0x3d0b);
		(*otp_ptr).bg_ratio =
		    (OV5648MIPI_read_cmos_sensor(0x3d0c) << 8) +
		    OV5648MIPI_read_cmos_sensor(0x3d0d);
		(*otp_ptr).gb_gr_ratio =
		    (OV5648MIPI_read_cmos_sensor(0x3d0e) << 8) +
		    OV5648MIPI_read_cmos_sensor(0x3d0f);
	} else {
		SENSORDB("invalid index!");
		return FALSE;
	}
	...
	SENSORDB("rg_ratio = %d, bg_ratio = %d, gb_gr_ratio = %d", (*otp_ptr).rg_ratio,
		 (*otp_ptr).bg_ratio, (*otp_ptr).gb_gr_ratio);
...
void OV5648MIPI_Set_Mirror_Flip(kal_uint8 image_mirror)
{
	...
	switch (image_mirror) {
	case IMAGE_NORMAL:
		OV5648MIPI_write_cmos_sensor(0x3820,
					     ((OV5648MIPI_read_cmos_sensor(0x3820) & 0xF9) | 0x00));
		OV5648MIPI_write_cmos_sensor(0x3821,
					     ((OV5648MIPI_read_cmos_sensor(0x3821) & 0xF9) | 0x06));
		break;
	case IMAGE_H_MIRROR:
		OV5648MIPI_write_cmos_sensor(0x3820,
					     ((OV5648MIPI_read_cmos_sensor(0x3820) & 0xF9) | 0x00));
		OV5648MIPI_write_cmos_sensor(0x3821,
					     ((OV5648MIPI_read_cmos_sensor(0x3821) & 0xF9) | 0x00));
		break;
	case IMAGE_V_MIRROR:
		OV5648MIPI_write_cmos_sensor(0x3820,
					     ((OV5648MIPI_read_cmos_sensor(0x3820) & 0xF9) | 0x06));
		OV5648MIPI_write_cmos_sensor(0x3821,
					     ((OV5648MIPI_read_cmos_sensor(0x3821) & 0xF9) | 0x06));
		break;
...
static void OV5648MIPI_camera_para_to_sensor(void)
{
	...
	for (i = 0; 0xFFFFFFFF != OV5648MIPI_sensor.eng.reg[i].Addr; i++) {
		OV5648MIPI_write_cmos_sensor(OV5648MIPI_sensor.eng.reg[i].Addr,
					     OV5648MIPI_sensor.eng.reg[i].Para);
	}
	for (i = OV5648MIPI_FACTORY_START_ADDR; 0xFFFFFFFF != OV5648MIPI_sensor.eng.reg[i].Addr;
	     i++) {
		OV5648MIPI_write_cmos_sensor(OV5648MIPI_sensor.eng.reg[i].Addr,
					     OV5648MIPI_sensor.eng.reg[i].Para);
	}
...
inline static void OV5648MIPI_get_sensor_item_info(MSDK_SENSOR_ITEM_INFO_STRUCT *para)
{
	...
	const static kal_char * cct_item_name[] =
	...
	case OV5648MIPI_PRE_GAIN:
		...
		sprintf(para->ItemNamePtr,
			cct_item_name[para->ItemIdx - OV5648MIPI_SENSOR_BASEGAIN]);
...
inline static kal_bool OV5648MIPI_set_sensor_item_info(MSDK_SENSOR_ITEM_INFO_STRUCT *para)
{
	...
	case OV5648MIPI_PRE_GAIN:
		...
		case OV5648MIPI_PRE_GAIN_B_INDEX:
			spin_lock(&ov5648mipi_drv_lock);
			OV5648MIPI_sensor.eng.cct[para->ItemIdx].Para =
...
UINT32 OV5648MIPIOpen(void)
{
	...
	sensor_id =
	    ((OV5648MIPI_read_cmos_sensor(0x300A) << 8) | OV5648MIPI_read_cmos_sensor(0x300B));
...
UINT32 OV5648MIPIControl(MSDK_SCENARIO_ID_ENUM ScenarioId,
...
UINT32 OV5648MIPIGetDefaultFramerateByScenario(MSDK_SCENARIO_ID_ENUM scenarioId,
					       MUINT32 *pframeRate)
{
				...
				UINT8 *pFeaturePara, UINT32 *pFeatureParaLen)
{
	...
	case SENSOR_FEATURE_GET_CCT_REGISTER:
		if (*pFeatureParaLen >= sizeof(OV5648MIPI_sensor.eng.cct) + sizeof(kal_uint32)) {
			*((kal_uint32 *) pFeaturePara++) = sizeof(OV5648MIPI_sensor.eng.cct);
			memcpy(pFeaturePara, &OV5648MIPI_sensor.eng.cct,
			       sizeof(OV5648MIPI_sensor.eng.cct));
		}
		break;
	...
	case SENSOR_FEATURE_GET_ENG_REGISTER:
		if (*pFeatureParaLen >= sizeof(OV5648MIPI_sensor.eng.reg) + sizeof(kal_uint32)) {
			*((kal_uint32 *) pFeaturePara++) = sizeof(OV5648MIPI_sensor.eng.reg);
			memcpy(pFeaturePara, &OV5648MIPI_sensor.eng.reg,
			       sizeof(OV5648MIPI_sensor.eng.reg));
		}
	case SENSOR_FEATURE_GET_REGISTER_DEFAULT:
		...
		memcpy(((PNVRAM_SENSOR_DATA_STRUCT) pFeaturePara)->SensorEngReg,
		       &OV5648MIPI_sensor.eng.reg, sizeof(OV5648MIPI_sensor.eng.reg));
		memcpy(((PNVRAM_SENSOR_DATA_STRUCT) pFeaturePara)->SensorCCTReg,
		       &OV5648MIPI_sensor.eng.cct, sizeof(OV5648MIPI_sensor.eng.cct));
		*pFeatureParaLen = sizeof(NVRAM_SENSOR_DATA_STRUCT);
		break;
	case SENSOR_FEATURE_GET_CONFIG_PARA:
		memcpy(pFeaturePara, &OV5648MIPI_sensor.cfg_data,
		       sizeof(OV5648MIPI_sensor.cfg_data));
	...
	case SENSOR_FEATURE_GET_ENG_INFO:
		memcpy(pFeaturePara, &OV5648MIPI_sensor.eng_info,
		       sizeof(OV5648MIPI_sensor.eng_info));
	...
	case SENSOR_FEATURE_SET_MAX_FRAME_RATE_BY_SCENARIO:
		OV5648MIPISetMaxFramerateByScenario((MSDK_SCENARIO_ID_ENUM) *pFeatureData32,
						    *(pFeatureData32 + 1));
		break;
	case SENSOR_FEATURE_GET_DEFAULT_FRAME_RATE_BY_SCENARIO:
		OV5648MIPIGetDefaultFramerateByScenario((MSDK_SCENARIO_ID_ENUM) *pFeatureData32,
