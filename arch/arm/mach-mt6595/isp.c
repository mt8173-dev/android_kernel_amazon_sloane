...
/* seanlin                ISP_LOG("[mt_isp_write_reg] Start wait ...\n"); */
			...
			timeout =
			    wait_event_interruptible_timeout(isp_wait_queue,
							     atomic_read(&writing_reg) == 0,
							     ms_to_jiffies(500));
			if (dbgMask & ISP_DBG_TASKLET) {
...
static int mt_isp_wait_irq(u32 wait_irq_status)
{
	...
	timeout =
	    wait_event_interruptible_timeout(isp_wait_queue, (wait_irq_status & irq_status),
					     3 * HZ);
	if (timeout == 0) {
...
*
********************************************************************************/
static long mt_isp_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	int ret = 0;
	/*  */
	if (pcmd_buf == NULL || pread_buf == NULL) {
		ISP_ERR("buf is null\n");
		return -EFAULT;
	}
...
static struct cdev *pcdev_isp;
static struct class *pclass_isp;
...
********************************************************************************/
static int
mt_isp_dump_reg_to_proc(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	char *p = page;
	int len = 0;
	u32 u4Index = 0;
...
*
********************************************************************************/
static int mt_isp_reg_debug(struct file *file, const char *buffer, unsigned long count, void *data)
{
	char regBuf[64];
