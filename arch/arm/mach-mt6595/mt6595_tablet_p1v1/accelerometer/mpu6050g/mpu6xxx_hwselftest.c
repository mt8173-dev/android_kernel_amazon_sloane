...
*/
static int inv_check_6500_gyro_self_test(struct inv_selftest_device *st, int *reg_avg, int *st_avg)
{
	u8 regs[3];
	int ret_val, result;
	...
	pr_debug("%s self_test gyro st_shift_prod - %+d %+d %+d\n",
		 st->name, st_shift_prod[0], st_shift_prod[1], st_shift_prod[2]);

	for (i = 0; i < 3; i++) {
		st_shift_cust[i] = st_avg[i] - reg_avg[i];
		if (!otp_value_zero) {
			/* Self Test Pass/Fail Criteria A */
			if (st_shift_cust[i] < DEF_6500_GYRO_CT_SHIFT_DELTA * st_shift_prod[i]) {
				ret_val = 1;
				pr_debug("%s, Fail, A\n", __func__);
			}
		} else {
			...
			if (st_shift_cust[i] < DEF_GYRO_ST_AL *
			    DEF_SELFTEST_GYRO_SENS * DEF_ST_PRECISION) {
				ret_val = 1;
				pr_debug("%s, Fail, B\n", __func__);
			}
		}
	...
	pr_debug("%s self_test gyro st_shift_cust - %+d %+d %+d\n",
		 st->name, st_shift_cust[0], st_shift_cust[1], st_shift_cust[2]);
...
*  @*st_avg:  average value of self test
*/
static int inv_check_6500_accel_self_test(struct inv_selftest_device *st, int *reg_avg, int *st_avg)
{
	int ret_val, result;
		       ...
		       int *gyro_result, int *accel_result)
{
	...
	result = inv_i2c_single_write(st, REG_SAMPLE_RATE_DIV, DEF_SELFTEST_SAMPLE_RATE);
	if (result)
		return result;
	/* wait for the sampling rate change to stabilize */
	mdelay(INV_MPU_SAMPLE_RATE_CHANGE_STABLE);
	result = inv_i2c_single_write(st, REG_GYRO_CONFIG, self_test_flag | DEF_SELFTEST_GYRO_FS);
	if (result)
		return result;
	if (INV_MPU6500 == st->chip_type)
	...
	while (s < st->samples) {
		...
		result = inv_i2c_read(st, REG_FIFO_COUNT_H, FIFO_COUNT_BYTE, data);
		if (result)
			return result;
		...
		while ((i < packet_count) && (s < st->samples)) {
			short vals[3];
			result = inv_i2c_read(st, REG_FIFO_R_W, packet_size / 2, data);
			if (result)
				return result;
			ind = 0;
...
/* return result; */
	...
	while (test_times > 0) {
		result = inv_do_test(st, 0, gyro_bias_regular, accel_bias_regular);
		if (result == -EAGAIN)
			test_times--;
		else
			test_times = 0;
	...
	pr_debug("%s self_test accel bias_regular - %+d %+d %+d\n",
		 st->name, accel_bias_regular[0], accel_bias_regular[1], accel_bias_regular[2]);
	pr_debug("%s self_test gyro bias_regular - %+d %+d %+d\n",
		 st->name, gyro_bias_regular[0], gyro_bias_regular[1], gyro_bias_regular[2]);
	...
	test_times = DEF_ST_TRY_TIMES;
	while (test_times > 0) {
		result = inv_do_test(st, BITS_SELF_TEST_EN, gyro_bias_st, accel_bias_st);
		if (result == -EAGAIN)
			test_times--;
		else
			break;
	...
	pr_debug("%s self_test accel bias_st - %+d %+d %+d\n",
		 st->name, accel_bias_st[0], accel_bias_st[1], accel_bias_st[2]);
	pr_debug("%s self_test gyro bias_st - %+d %+d %+d\n",
		 st->name, gyro_bias_st[0], gyro_bias_st[1], gyro_bias_st[2]);
...
/* compass_result = !st->slave_compass->self_test(st); */
	...
	if (INV_MPU6050 == st->chip_type) {
		...
		accel_result = !inv_check_6500_accel_self_test(st,
							       accel_bias_regular, accel_bias_st);
		gyro_result = !inv_check_6500_gyro_self_test(st, gyro_bias_regular, gyro_bias_st);
	}
