...
#define AKMDBG(format, ...)	pr_err(KERN_ERR "AKM8963 " format "\n", ## __VA_ARGS__)
#else
#define AKMDBG(format, ...)
#endif

#if AKM8963_DEBUG_FUNC
#define AKMFUNC(func) pr_err(KERN_ERR "AKM8963 " func " is called\n")
#else
#define AKMFUNC(func)
#endif
...
static struct i2c_board_info i2c_akm8963 __initdata = { I2C_BOARD_INFO("akm8963", (0x18 >> 1)) };
	...
	if (hw->power_id != POWER_NONE_MACRO) {
		...
		if (power_on == on) {
			...
			if (!hwPowerOn(hw->power_id, hw->power_vol, "akm8963"))
				pr_err(KERN_ERR "power on fails!!\n");
		} else {
			if (!hwPowerDown(hw->power_id, "akm8963"))
				pr_err(KERN_ERR "power off fail!!\n");
		}
	}
	power_on = on;
}
...
static long AKI2C_RxData(char *rxData, int length)
{
	...
	for (loop_i = 0; loop_i < AKM8963_RETRY_COUNT; loop_i++) {
		this_client->addr = this_client->addr;
		/* this_client->addr = this_client->addr | I2C_WR_FLAG; */
		if (i2c_master_send(this_client, rxData, 0x01)) {
			if (i2c_master_recv(this_client, rxData, length))
				break;
		}
		mdelay(10);
	}
	...
	if (loop_i >= AKM8963_RETRY_COUNT) {
		pr_err(KERN_ERR "%s retry over %d\n", __func__, AKM8963_RETRY_COUNT);
		return -EIO;
	}
#if DEBUG
	if (atomic_read(&data->trace) & AMK_I2C_DEBUG) {
		pr_info(KERN_INFO "RxData: len=%02x, addr=%02x\n  data=", length, addr);
		for (i = 0; i < length; i++)
			pr_info(KERN_INFO " %02x", rxData[i]);
...
#if DEBUG
	if (atomic_read(&data->trace) & AMK_I2C_DEBUG) {
		pr_info(KERN_INFO "TxData: len=%02x, addr=%02x\n  data=", length, txData[0]);
		for (i = 0; i < (length - 1); i++)
			pr_info(KERN_INFO " %02x", txData[i + 1]);
...
static long AKECS_GetData(char *rbuf, int size)
{
	...
	if (size < SENSOR_DATA_SIZE) {
		pr_err(KERN_ERR "buff size is too small %d!\n", size);
		return -1;
	}
	...
	for (loop_i = 0; loop_i < AKM8963_RETRY_COUNT; loop_i++) {
		...
		if (ret) {
			pr_err(KERN_ERR "read ST1 resigster failed!\n");
			return -1;
		}
	...
	if (loop_i >= AKM8963_RETRY_COUNT) {
		pr_err(KERN_ERR "Data read retry larger the max count!\n");
		if (0 == factory_mode)
	...
	if (ret < 0) {
		pr_err(KERN_ERR "AKM8975 akm8975_work_func: I2C failed\n");
		return -1;
	}
	rbuf[0] = temp;
...
static ssize_t store_layout_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if (1 == sscanf(buf, "%d", &layout)) {
		...
		if (!hwmsen_get_convert(layout, &data->cvt)) {
			pr_err(KERN_ERR "HWMSEN_GET_CONVERT function error!\r\n");
		} else if (!hwmsen_get_convert(data->hw->direction, &data->cvt)) {
			pr_err(KERN_ERR "invalid layout: %d, restore to %d\n", layout,
			       data->hw->direction);
		} else {
			pr_err(KERN_ERR "invalid layout: (%d, %d)\n", layout, data->hw->direction);
			hwmsen_get_convert(0, &data->cvt);
		}
	} else {
		pr_err(KERN_ERR "invalid format = '%s'\n", buf);
	}
...
static ssize_t show_trace_value(struct device_driver *ddri, char *buf)
{
	...
	if (NULL == obj) {
		pr_err(KERN_ERR "akm8963_i2c_data is null!!\n");
		return 0;
	}
...
static ssize_t store_trace_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if (1 == sscanf(buf, "0x%x", &trace))
		...
		pr_err(KERN_ERR "invalid content: '%s', length = %d\n", buf, count);
...
static int akm8963_create_attr(struct device_driver *driver)
{
	...
	for (idx = 0; idx < num; idx++) {
		...
		if (err) {
			pr_err(KERN_ERR "driver_create_file (%s) = %d\n",
...
static long akm8963_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	/* pr_err(KERN_ERR"akm8963 cmd:0x%x\n", cmd); */
	switch (cmd) {
	...
	case MSENSOR_IOCTL_READ_CHIPINFO:
		if (argp == NULL) {
			pr_err(KERN_ERR "IO parameter pointer is NULL!\r\n");
			break;
		}
	...
	case MSENSOR_IOCTL_SENSOR_ENABLE:
		...
		if (copy_from_user(&enable, argp, sizeof(enable))) {
			...
			pr_info(KERN_INFO "MSENSOR_IOCTL_SENSOR_ENABLE enable=%d!\r\n", enable);
			factory_mode = 1;
			if (1 == enable) {
				atomic_set(&o_flag, 1);
	...
	default:
		pr_err(KERN_ERR "%s not supported = 0x%04x", __func__, cmd);
		return -ENOIOCTLCMD;
		break;
	}
		    ...
		    void *buff_out, int size_out, int *actualout)
{
	...
	case SENSOR_DELAY:
		if ((buff_in == NULL) || (size_in < sizeof(int))) {
			pr_err(KERN_ERR "Set delay parameter error!\n");
			err = -EINVAL;
		} else {
			value = *(int *)buff_in;
			if (value <= 20)
	...
	case SENSOR_ENABLE:
		if ((buff_in == NULL) || (size_in < sizeof(int))) {
			pr_err(KERN_ERR "Enable sensor parameter error!\n");
			err = -EINVAL;
		} else {
	...
	case SENSOR_GET_DATA:
		if ((buff_out == NULL) || (size_out < sizeof(hwm_sensor_data))) {
			pr_err(KERN_ERR "get sensor data parameter error!\n");
			err = -EINVAL;
		} else {
		...
		break;
	default:
		pr_err(KERN_ERR "msensor operate function no this parameter %d!\n", command);
		err = -1;
		break;
	}
				...
				void *buff_out, int size_out, int *actualout)
{
	...
	case SENSOR_GET_DATA:
		...
		break;
	default:
		pr_err(KERN_ERR "gsensor operate function no this parameter %d!\n", command);
		err = -1;
		break;
	}
...
static int akm8963_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	pr_err(KERN_ERR " AKM8963 akm8963_probe: befor init prob\n");
	/* Check connection */
	err = AKECS_CheckDevice();
	if (err < 0) {
		pr_err(KERN_ERR "AKM8963 akm8963_probe: check device connect error\n");
		goto exit_init_failed;
	}


	/* Register sysfs attribute */
	err = akm8963_create_attr(&akm_sensor_driver.driver);
	if (err) {
		pr_err(KERN_ERR "create attribute err = %d\n", err);
		goto exit_sysfs_create_group_failed;
	}

	err = misc_register(&akm8963_device);
	if (err) {
		pr_err(KERN_ERR "akm8963_device register failed\n");
...
static int akm8963_i2c_remove(struct i2c_client *client)
{
	...
	err = akm8963_delete_attr(&akm_sensor_driver.driver);
	if (err)
		pr_err(KERN_ERR "akm8963_delete_attr fail: %d\n", err);
...
static int akm_gpio_rst_config(void)
{
	...
	pr_info(KERN_INFO "akm8963 reset pin is used for this project\n");
	ret = mt_set_gpio_mode(GPIO_COMPASS_RST_PIN, GPIO_MODE_00);
	if (ret < 0)
		pr_err(KERN_ERR "set gpio mode error\n");

	ret = mt_set_gpio_dir(GPIO_COMPASS_RST_PIN, GPIO_DIR_OUT);
	if (ret < 0)
		pr_err(KERN_ERR "set gpio dir error\n");

	ret = mt_set_gpio_out(GPIO_COMPASS_RST_PIN, GPIO_OUT_ZERO);
	if (ret < 0)
		pr_err(KERN_ERR "set gpio out value error\n");
...
static int akm_probe(struct platform_device *pdev)
{
	...
	if (i2c_add_driver(&akm8963_i2c_driver)) {
		pr_err(KERN_ERR "add driver error\n");
		return -1;
	}
	pr_err(KERN_ERR "akm_probe()\n");
	return 0;
}
