...
{
	return enable ?
	    enable_clock(MT_CG_PERI_AP_DMA, "i2c") : disable_clock(MT_CG_PERI_AP_DMA, "i2c");
}

static int mt_i2c_enable_clk(struct mt_i2c_data *i2c, bool enable)
{
	return enable ? enable_clock(i2c->pdn, "i2c") : disable_clock(i2c->pdn, "i2c");
}

static u32 mt_i2c_get_func_clk(struct mt_i2c_data *i2c)
{
	/* return (i2c->flags & MT_WRAPPER_BUS) ? (u32) (I2C_CLK_RATE) : (mt_get_bus_freq() / 16); */
	return 17062;
}
...
static void mt_i2c_power_ctl(int id, int val)
{
	...
	case 0:
		if (0 == val) {
			...
			pr_debug("mt_i2c_power_ctl err\n");
		}
		break;
	case 1:		/* always on */
		pr_debug("hw always power on\n");
		break;
	case 2:		/* same power domain with i2c0 */
		pr_debug("same power domain with i2c0\n");
		break;
	case 3:		/* always on */
		pr_debug("hw always power on\n");
		break;
...
struct mt_i2c_data mt_i2c_data[] = {
	{
	 ...
	 .i2c_power_ctl = mt_i2c_power_ctl,
	 .flags = MT_I2C_6595,
	 },
	{
	 .pdn = MT_CG_PERI_I2C1,
...
static void parse_dfo_tag(struct tag *t)
{
	...
	printk(KERN_ALERT "tag_header size: %d, tag_dfo_boot size: %d\n"
	       "hdr.size: %d\n", sizeof(struct tag_header), sizeof(tag_dfo_boot), t->hdr.size);
	printk(KERN_ALERT "number of tags: %d\n", nr);
	for (i = 0; i < nr; i++) {
		printk(KERN_ALERT "dfo[%d]: %s, 0x%08lx\n", i, p->info[0].name, p->info[0].value);
		/*
	...
	if (((bl_fb.base == FB_START) && (bl_fb.size == FB_SIZE)) || (use_bl_fb == 2)) {
		...
		printk
		    ("******************************************************************************\n"
		     ...
		     "   WARNING WARNING WARNING WARNING WARNING WARNING WARNING WARNING WARNING\n"
		     "******************************************************************************\n"
		     "\n", bl_fb.base, bl_fb.size, (unsigned long long)FB_START,
		     (unsigned long long)FB_SIZE);
