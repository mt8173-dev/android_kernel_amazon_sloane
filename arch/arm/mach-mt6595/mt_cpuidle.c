...
#define CPU_DORMANT_LOG_PRINT CPU_DORMANT_LOG_NONE
...
__weak bool spm_is_cpu_irq_occur(int core_id)
{
	return 0;
}
...
unsigned *restore_vfp(unsigned int *container)
{
	return container;
}

#else				/* #if !defined (CONFIG_VFP) */

#if 1
extern unsigned *save_vfp(unsigned int *container);
extern unsigned *restore_vfp(unsigned int *container);
#else
/********************/
/* .global save_vfp */
/********************/
unsigned *save_vfp(unsigned int *container)
{
	...
	__asm__ __volatile__("@ FPU state save/restore.					 \n\t"
...
{
	__asm__ __volatile__("mrc    p15,0,r8,c9,c12,0    @ PMon: Control Register\n\t"
			     "bic    r1,r8,#1\n\t"
...
{
	__asm__ __volatile__("@ NOTE: all counters disabled by PMCR<0> == 0 on reset\n\t"
			     "ldr    r8,[%0]                  @ r8 = PMCR\n\t"
...
{
	__asm__ __volatile__(" cmp    %1, #0		  @ non-secure?\n\t"
...
static void gic_dist_save(void)
{
	...
	for (i = 0; i < DIV_ROUND_UP(gic_irqs, 16); i++)
		gic_data[0].saved_spi_conf[i] = readl_relaxed(dist_base + GIC_DIST_CONFIG + i * 4);
...
static void gic_dist_restore(void)
{
	...
	for (i = 0; i < DIV_ROUND_UP(gic_irqs, 16); i++)
		writel_relaxed(gic_data[0].saved_spi_conf[i], dist_base + GIC_DIST_CONFIG + i * 4);
...
void cci_dvm_snoop_disable(void)
{
	int val;
	...
	else if (is_cpu_type(CA15L_TYPEID)) {
		/* able to issue dvm/snoop request from this interface ? */
...
void cci_dvm_snoop_enable(void)
{
	int val;
	if (is_cpu_type(CA7_TYPEID)) {
		/** CA7MCU BUS_CONFIG[4], Disable CA7 snoop function
...
#if 0				/* defined (CONFIG_MTK_FORCE_CLUSTER1) */
		/* able to issue dvm/snoop request from this interface ? */
		val = reg_read(CCI400_CA15L_SNOOP_CTLR);
		val = _or(val, 0x3);
		reg_write(CCI400_CA15L_SNOOP_CTLR, val);
...
static inline unsigned int ca7_l2rstdisable(int l2reset)
{
	unsigned int read_back = reg_read(CA7_CACHE_CONFIG);
...
static inline unsigned int ca15l_l2rstdisable(int l2reset)
{
	unsigned int read_back = reg_read(CA15L_RST_CTL);
	int reg_val = _aor(read_back, ~CA15L_L2RSTDISABLE_F, (!l2reset) ? CA15L_L2RSTDISABLE_F : 0);
	reg_write(CA15L_RST_CTL, reg_val);

	return read_back & CA15L_L2RSTDISABLE_F;
}


/* l2rstdisable imply "not l2reset", vice versa. */
#define ca7_l2rstdisable_restore(f) ca7_l2rstdisable(!(f))
#define ca15l_l2rstdisable_restore(f) ca15l_l2rstdisable(!(f))
...
static void mt_cluster_save(int flags)
{
	...
	if (is_cpu_type(CA7_TYPEID)) {
		GET_CLUSTER_DATA()->l2rstdisable = ca7_l2rstdisable(IS_DORMANT_INNER_OFF(flags));
	} else if (is_cpu_type(CA15L_TYPEID)) {
		GET_CLUSTER_DATA()->l2rstdisable = ca15l_l2rstdisable(IS_DORMANT_INNER_OFF(flags));
	}
}
...
static void mt_cluster_restore(int flags)
{
	...
	if (is_cpu_type(CA7_TYPEID)) {
		ca7_l2rstdisable_restore(GET_CLUSTER_DATA()->l2rstdisable);
	} else if (is_cpu_type(CA15L_TYPEID)) {
		ca15l_l2rstdisable_restore(GET_CLUSTER_DATA()->l2rstdisable);
	}
}
...
void mt_cpu_save(void)
{
	...
	/**************************************************************************************************************/
	/* ret = save_dbg_regs(core->dbg_data,                                                */
	/*                  (unsigned int)(is_cpu_type(CA7_TYPEID) ? CA7_DBGAPB_BASE : CA15L_DBGAPB_BASE));           */
	/* SENTINEL_CHECK(core->dbg_data, ret);                                               */
	/**************************************************************************************************************/
...
void mt_cpu_restore(void)
{
	...
	/**************************************************************************************************************/
	/* restore_dbg_regs(core->dbg_data,                                                          */
	/*                  (unsigned int)(is_cpu_type(CA7_TYPEID) ? CA7_DBGAPB_BASE : CA15L_DBGAPB_BASE));           */
	/**************************************************************************************************************/
 ...
 **/
#define disable_dcache_safe(inner_off)					\
	__asm__ __volatile__ (						\
...
int mt_cpu_dormant_reset(unsigned long flags)
{
	...
	GET_CORE_DATA()->timestamp[1] = read_cntpct();

	if (unlikely(IS_DORMANT_BREAK_CHECK(flags)) &&
	    unlikely(spm_is_cpu_irq_occur(cpuid + clusterid * 4))) {
		ret = 2;	/* dormant break */
		goto _break;
	}
...
/***************************************************************************/
#if !defined(__KERNEL__)
int ctp_cpu_pm_resume(void)
{
	return 0;
}

int ctp_cpu_pm_suspend(void)
{
	return 0;
...
static void (*cpu_resume_phys) (void);
...
#define NOMMU(a) (((a) & ~0xf0000000) | 0x10000000)
	...
	} else if (likely(is_cpu_type(CA15L_TYPEID))) {
		/* ACINACTM & AINACTS */
	...
	/* start_resume: */
	/* jump to cpu_resume() */
	__asm__ __volatile__("adr r0, 1f\n\t"
			     "ldm r0, {r1-r2}\n\t"
			     "sub r0, r1, r0\n\t"
			     "add r3, r2, r0\n\t"
			     "ldr r3, [r3] \n\t"
			     "bx r3\n\t"
			     "1:		\n\t" ".long .\n\t" ".long cpu_resume_phys\n\t");
}
...
int mt_cpu_dormant(unsigned long flags)
{
	...
	GET_CORE_DATA()->timestamp[0] = read_cntpct();
	read_id(&cpuid, &clusterid);
	CPU_DORMANT_INFO("dormant(flags:%x) start (cluster/cpu:%d/%d)!\n", flags, clusterid, cpuid);
	...
	if (unlikely(IS_DORMANT_SNOOP_OFF(flags))) {
		 ...
		 **/
		cpu_resume_phys = (void (*)(void))virt_to_phys(cpu_resume);
	...
	/* for ctp only */
	if (ctp_cpu_pm_suspend()) {
		ret = MT_CPU_DORMANT_BREAK_V(CPU_PM_BREAK);
		goto _back;
	}
	/* dormant break */
	if (IS_DORMANT_BREAK_CHECK(flags) && spm_is_cpu_irq_occur(cpuid + clusterid * 4)) {
	...
	GET_CORE_DATA()->timestamp[2] = read_cntpct();
	...
	default:		/* back from dormant break */
	case 2:
 ...
 _back:
	GET_CORE_DATA()->timestamp[3] = read_cntpct();
	CPU_DORMANT_INFO("dormant(flags:%x) exit(ret:%x) (cluster/cpu:%d/%d)!\n",
			 flags, ret, clusterid, cpuid);

	CPU_DORMANT_INFO("dormant timing: %llu, %llu, %llu, %llu\n",
			 GET_CORE_DATA()->timestamp[0],
			 GET_CORE_DATA()->timestamp[1],
			 GET_CORE_DATA()->timestamp[2], GET_CORE_DATA()->timestamp[3]);
...
volatile int mt_cpu_dormant_test_mode = (CPU_DORMANT_MODE | 0);
