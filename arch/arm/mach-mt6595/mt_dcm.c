...
void dcm_dump_regs(unsigned int type)
{
	...
	if (type & ISP_DCM) {
		if (subsys_is_on(SYS_ISP)) {
			volatile unsigned int cam_ctl_raw_dcm_dis, cam_ctl_rgb_dcm_dis,
			    cam_ctl_yuv_dcm_dis;
			volatile unsigned int cam_ctl_cdp_dcm_dis, cam_ctl_dma_dcm_dis;
			volatile unsigned int venc_cg_ctrl, venc_ce, venc_clk_dcm_ctrl, jpgenc,
			    smi_larb2_con_set;
	...
	if (type & MJC_DCM) {
		if (subsys_is_on(SYS_MJC)) {
			volatile unsigned int mjc_hw_dcm_dis, mjc_hw_dcm_dis_set,
			    mjc_hw_dcm_dis_clr;
...
void dcm_enable(unsigned int type)
{
	...
	if (type & CPU_DCM) {
		...
		dcm_clrl(MCUSYS_CONFIG, 0x0F9C0000);	/* 0xF020001C,95 */

		dcm_setl(CACHE_CONFIG, 0x00000B00);	/* 0xF0200100,set bit8,bit9,bit11=1,95 */
		dcm_clrl(CACHE_CONFIG, 0x00000400);	/* 0xF0200100,clear bit10, 95 */
		dcm_setl(ARMPLL_CTL, 0x00000010);	/* 0xF0200160,set bit4, 95 */
	...
	if (type & IFR_DCM) {
		...
		dcm_clrl(CA7_CKDIV1, 0x0000001F);	/* 0x10001008//5'h0,00xxx: 1/1,95 */

		dcm_setl(INFRA_TOPCKGEN_DCMCTL, 0x00000771);	/* 0x10001010,set0,4,5,6,8,9,10=1, 95 */

/* no use        dcm_setl   (INFRA_TOPCKGEN_DCMDBC , 0x00000001); //0x10001014//set bit0=1, force to 26M,95 */

		dcm_setl(INFRA_GLOBALCON_DCMCTL, 0x00000303);	/* 0x10001050//set bit0,bit1,bit8,bit9=1,DCM debouncing counter=0,95 */

		dcm_setl(INFRA_GLOBALCON_DCMDBC, 0x01000100);	/* 0xF0001054,set bit8,24=1, 95 */
		dcm_clrl(INFRA_GLOBALCON_DCMDBC, 0x007F007F);	/* 0xF0001054,clear bit0~6,16~22, 95 */

		dcm_setl(INFRA_GLOBALCON_DCMFSEL, 0x10100000);	/* 0xF0001058,95 */
		dcm_clrl(INFRA_GLOBALCON_DCMFSEL, 0x0F0F0F07);	/* 0xF0001058,95 */

		dcm_clrl(MM_MMU_DCM_DIS, 0x0000007F);	/* 0xF0205050,95 */

		dcm_clrl(PERISYS_MMU_DCM_DIS, 0x0000007F);	/* 0xF0214050,95 */

		/* DRAMC */
		dcm_setl(channel_A_DRAMC_PD_CTRL, 0xC3000000);	/* 0xF00041DC,95 */
		dcm_clrl(channel_A_DRAMC_PD_CTRL, 0x00000008);	/* 0xF00041DC,95 */

		dcm_setl(channel_B_DRAMC_PD_CTRL, 0xC3000000);	/* 0xF00111DC,95 */
		dcm_clrl(channel_B_DRAMC_PD_CTRL, 0x00000008);	/* 0xF00111DC,95 */
	...
	if (type & PER_DCM) {
		...
		dcm_setl(PERI_GLOBALCON_DCMCTL, 0x000000F3);	/* 0xF0003050,set bit0,1,4~7, 95 */
		dcm_clrl(PERI_GLOBALCON_DCMCTL, 0x00001F00);	/* 0x10003050//clear bit8~12, 95 */

		dcm_clrl(PERI_GLOBALCON_DCMDBC, 0x0000000F);	/* 0x10003054//clear bit0~3 ,95 */
		dcm_setl(PERI_GLOBALCON_DCMDBC, 0x000000F0);	/* 0x10003054//set bit4~7=1 ,95 */

		dcm_clrl(PERI_GLOBALCON_DCMFSEL, 0x001F0F07);	/* 0x10003058//clear bit0~bit2,bit8~bit11,bit16~bit20,95 */

		/* MSDC module */
		dcm_setl(MSDC0_PATCH_BIT1, 0x00200000);	/* 0xF12300B4//set bit21=1,95 */
		dcm_clrl(MSDC0_PATCH_BIT1, 0xFF800000);	/* 0xF12300B4//clear bit23~bit31=0,95 */

		dcm_setl(MSDC1_PATCH_BIT1, 0x00200000);	/* 0xF12400B4//set bit21=1,95 */
		dcm_clrl(MSDC1_PATCH_BIT1, 0xFF800000);	/* 0xF12400B4//clear bit23~bit31=0,95 */

		dcm_setl(MSDC2_PATCH_BIT1, 0x00200000);	/* 0xF12500B4//set bit21=1,95 */
		dcm_clrl(MSDC2_PATCH_BIT1, 0xFF800000);	/* 0xF12500B4//clear bit23~bit31=0,95 */

		dcm_setl(MSDC3_PATCH_BIT1, 0x00200000);	/* 0xF12600B4//set bit21=1,95 */
		dcm_clrl(MSDC3_PATCH_BIT1, 0xFF800000);	/* 0xF12600B4//clear bit23~bit31=0,95 */

		/* USB */
		dcm_clrl(USB0_DCM, 0x00070000);	/* 0x11200700//clear bit16~bit18=0,95 */

		/* PMIC */
		dcm_setl(PMIC_WRAP_DCM_EN, 0x00000001);	/* 0x1000D13C//set bit0=1,95 */

		/* I2C */
		dcm_setl(I2C0_I2CREG_HW_CG_EN, 0x00000001);	/* 0xF1007054//set bit0=1,95 */
		dcm_setl(I2C1_I2CREG_HW_CG_EN, 0x00000001);	/* 0xF1008054//set bit0=1,95 */
		dcm_setl(I2C2_I2CREG_HW_CG_EN, 0x00000001);	/* 0xF1009054//set bit0=1,95 */
		dcm_setl(I2C3_I2CREG_HW_CG_EN, 0x00000001);	/* 0xF1010054//set bit0=1,95 */
		dcm_setl(I2C4_I2CREG_HW_CG_EN, 0x00000001);	/* 0xF1011054//set bit0=1,95 */
	...
	if (type & SMI_DCM) {
		...
		dcm_writel(SMI_COMMON_SMI_DCM, 0x00000001);	/* 0xF4022300//set bit 0=1, 95 */
	...
	if (type & EMI_DCM) {
		...
		dcm_setl(EMI_CONM, 0x40000000);	/* 0xF0203060,set bit30=1,95 */
		dcm_clrl(EMI_CONM, 0xBF000000);	/* 0xF0203060,clear bit31,bit29,bit28,bit27~bit27,95 */
	...
	if (type & DIS_DCM) {
		...
		if (subsys_is_on(SYS_DIS)) {
			...
			dcm_writel(MMSYS_HW_DCM_DIS0, 0x00000000);	/* 0x14000120,95 */
			dcm_writel(MMSYS_HW_DCM_DIS_SET0, 0x00000000);	/* 0x14000124,95 */
			dcm_writel(MMSYS_HW_DCM_DIS_CLR0, 0xFFFFFFFF);	/* 0x14000128,95 */

			dcm_writel(MMSYS_HW_DCM_DIS1, 0x00000000);	/* 0xF4000130,95 */
			dcm_writel(MMSYS_HW_DCM_DIS_SET1, 0x00000000);	/* 0x14000130,95 */
			dcm_writel(MMSYS_HW_DCM_DIS_CLR1, 0xFFFFFFFF);	/* 0x14000134,95 */

			dcm_setl(SMI_LARB0_CON_SET, 0x00000010);	/* 0x14210014//set bit4=1,95 */
	...
	if (type & ISP_DCM) {	/* video encoder : sensor=>ISP=>VENC */
		...
		if (subsys_is_on(SYS_ISP) && subsys_is_on(SYS_VEN)) {
			...
			dcm_setl(VENC_CLK_CG_CTRL, 0xFFFFFFFF);	/* 0xF80020FC   ,95 */
			dcm_setl(VENC_CLK_DCM_CTRL, 0x00000001);	/* 0xF80020F4//set bit0=1,95 */
			dcm_clrl(JPGENC_DCM_CTRL, 0x00000001);	/* 0xF8003300//clear bit0=0,95 */
			dcm_clrl(JPGDEC_DCM_CTRL, 0x00000001);	/* 0xF8004300//clear bit0=0,95 */

			dcm_setl(SMI_LARB2_CON_SET, 0x00000010);	/* 0x15001014//set bit0=1,95 */
			dcm_setl(SMI_LARB3_CON_SET, 0x00000010);	/* 0x18001014//set bit0=1,95 */
	...
	if (type & VDE_DCM) {
		...
		if (subsys_is_on(SYS_VDE)) {
			...
			dcm_clrl(VDEC_DCM_CON, 0x00000001);	/* 0xF6000018,95 */

			dcm_setl(SMI_LARB1_CON_SET, 0x00000010);	/* 0xF6010014,set bit4=1,95 */
	...
	if (type & MJC_DCM) {	/* improve video record resloution */
		if (subsys_is_on(SYS_MJC)) {
			dcm_writel(MJC_HW_DCM_DIS, 0x00000000);	/* 0x17000010,95 */
			dcm_writel(MJC_HW_DCM_DIS_SET, 0x00000000);	/* 0x17000014,95 */
			dcm_writel(MJC_HW_DCM_DIS_CLR, 0x00000000);	/* 0x17000018,95 */

			dcm_setl(SMI_LARB4_CON_SET, 0x00000010);	/* 0x17002014//set bit0=1,95 */
...
/* volatile unsigned int temp; */
	...
	if (type & CPU_DCM) {
		...
		dcm_setl(MCUSYS_CONFIG, 0x0F9C0000);	/* 0xF020001C,95 */
		dcm_setl(CACHE_CONFIG, 0x00000300);	/* 0xF0200100,set bit8~9 =1,95 */
		dcm_clrl(CACHE_CONFIG, 0x00000C00);	/* 0xF0200100,clear bit10,bit11,95 */
		dcm_clrl(ARMPLL_CTL, 0x00000010);	/* 0xF0200160,clear bit4, 95 */
	...
	if (type & PER_DCM) {
		...
		dcm_clrl(PERI_GLOBALCON_DCMCTL, 0x00001FF3);	/* 0x10003050,clear bit0,1,4~7,8~12 ,95 */

		dcm_clrl(PERI_GLOBALCON_DCMDBC, 0x0000000F);	/* 0x10003054//clear bit0~3,8~12 ,95 */
		dcm_setl(PERI_GLOBALCON_DCMDBC, 0x000000F0);	/* 0x10003054//set bit4~7=1 ,95 */

		dcm_clrl(PERI_GLOBALCON_DCMFSEL, 0x001F0F07);	/* 0xF0003058, 95 */

		/* MSDC module */
		dcm_clrl(MSDC0_PATCH_BIT1, 0x00200000);	/* 0xF12300B4//clear bit21,95 */
		dcm_setl(MSDC0_PATCH_BIT1, 0xFF800000);	/* 0xF12300B4//set bit23~bit31=0,95 */

		dcm_clrl(MSDC1_PATCH_BIT1, 0x00200000);	/* 0xF12400B4//clear bit21,95 */
		dcm_setl(MSDC1_PATCH_BIT1, 0xFF800000);	/* 0xF12400B4//set bit23~bit31=0,95 */

		dcm_clrl(MSDC2_PATCH_BIT1, 0x00200000);	/* 0xF12500B4//clear bit21,95 */
		dcm_setl(MSDC2_PATCH_BIT1, 0xFF800000);	/* 0xF12500B4//set bit23~bit31=0,95 */

		dcm_clrl(MSDC3_PATCH_BIT1, 0x00200000);	/* 0xF12600B4//clear bit21,95 */
		dcm_setl(MSDC3_PATCH_BIT1, 0xFF800000);	/* 0xF12600B4//set bit23~bit31=0,95 */

		/* USB */
		dcm_setl(USB0_DCM, 0x00070000);	/* 0xF1200700,95 */

		/* PMIC */
		dcm_clrl(PMIC_WRAP_DCM_EN, 0x00000001);	/* 0x1000D13C,95 */

		/* I2C */
		dcm_clrl(I2C0_I2CREG_HW_CG_EN, 0x00000001);	/* 0xF1007054//clear bit0=1,95 */
		dcm_clrl(I2C1_I2CREG_HW_CG_EN, 0x00000001);	/* 0xF1008054//clear bit0=1,95 */
		dcm_clrl(I2C2_I2CREG_HW_CG_EN, 0x00000001);	/* 0xF1009054//clear bit0=1,95 */
		dcm_clrl(I2C3_I2CREG_HW_CG_EN, 0x00000001);	/* 0xF1010054//clear bit0=1,95 */
		dcm_clrl(I2C4_I2CREG_HW_CG_EN, 0x00000001);	/* 0xF1011054//clear bit0=1,95 */
	...
	if (type & IFR_DCM) {
		...
		/* DRAMC */
		dcm_setl(channel_A_DRAMC_PD_CTRL, 0x01000000);	/* 0xF00041DC,set bit24=1, 95 */
		dcm_clrl(channel_A_DRAMC_PD_CTRL, 0xC2000008);	/* 0xF00041DC,clear bit30,31,25,3 ,95 */

		dcm_setl(channel_B_DRAMC_PD_CTRL, 0x01000000);	/* 0xF00111DC,set bit24=1, 95 */
		dcm_clrl(channel_B_DRAMC_PD_CTRL, 0xC2000008);	/* 0xF00111DC,clear bit30,31,25,3 ,95 */


		dcm_clrl(INFRA_TOPCKGEN_DCMCTL, 0x00000771);	/* 0x10001010,clear bit0,bit4,5,6,bit8,9,10, 95 */
		dcm_clrl(INFRA_TOPCKGEN_DCMDBC, 0x00000001);	/* 0x10001014,clear bit0,95 */
		dcm_clrl(INFRA_GLOBALCON_DCMCTL, 0x00000303);	/* 0x10001050,clear bit0,1,bit8,9 ,95 */

		dcm_setl(MM_MMU_DCM_DIS, 0x0000007F);	/* 0xF0205050,95 */

		dcm_setl(PERISYS_MMU_DCM_DIS, 0x0000007F);	/* 0xF0214050,95 */
	...
	if (type & SMI_DCM) {
		...
		dcm_clrl(SMI_COMMON_SMI_DCM, 0x00000001);	/* 0x14022300,clear bit0, 95 */
	...
	if (type & DIS_DCM) {
		...
		dcm_writel(MMSYS_HW_DCM_DIS0, 0xFFFFFFFF);	/* 0x14000120,95 */
		dcm_writel(MMSYS_HW_DCM_DIS_SET0, 0xFFFFFFFF);	/* 0x14000124,95 */
		dcm_writel(MMSYS_HW_DCM_DIS_CLR0, 0x00000000);	/* 0x14000128,95 */


		dcm_writel(MMSYS_HW_DCM_DIS1, 0xFFFFFFFF);	/* 0x1400012C,95 */
		dcm_writel(MMSYS_HW_DCM_DIS_SET1, 0xFFFFFFFF);	/* 0x14000130,95 */
		dcm_writel(MMSYS_HW_DCM_DIS_CLR1, 0x00000000);	/* 0x14000134,95 */

		/* SMI_LARB0: DISP/MDP(MMSYS) */
		dcm_setl(SMI_LARB0_CON_CLR, 0x00000010);	/* 0x14010018,set bit4=1,95 */
	...
	if (type & ISP_DCM) {
		...
		/* dcm_setl   (CTL_RAW_DCM_DIS         , 0x03FFFFFF); //0xF5004188//set bit0~25=1,95 */
		dcm_setl(CTL_RAW_D_DCM_DIS, 0x024EAFE8);	/* 0xF500418C//set bit0~25=1,95 */
		dcm_setl(CTL_DMA_DCM_DIS, 0x07FFFFFF);	/* 0xF5004190//set bit0~26=1,95 */
		dcm_setl(CTL_RGB_DCM_DIS, 0x0000007F);	/* 0xF5004194//set bit0~6=1,95 */
		dcm_setl(CTL_YUV_DCM_DIS, 0x000FFFFF);	/* 0xF5004198//set bit0~19=1,95 */
		dcm_setl(CTL_TOP_DCM_DIS, 0x0000000F);	/* 0xF500419C//set bit0~3=1,95 */

		dcm_setl(FDVT_CTRL, 0x0000001F);	/* 0xF500B19C//set bit25~28=1,95 */

		dcm_setl(JPGENC_DCM_CTRL, 0x00000001);	/* 0xF8003300//set bit0=1,95 */
		dcm_setl(JPGDEC_DCM_CTRL, 0x00000001);	/* 0xF8004300//set bit0=1,95 */
		dcm_writel(VENC_CLK_CG_CTRL, 0x00000000);	/* 0xF80020FC,95 */
		dcm_clrl(VENC_CLK_DCM_CTRL, 0x00000001);	/* 0xF80020F4,95 */

		/* SMI_LARB2: ISP */
		dcm_setl(SMI_LARB2_CON_CLR, 0x00000010);	/* 0xF5001018,set bit4=1,95 */
		/* SMI_LARB3: VENC */
		dcm_setl(SMI_LARB3_CON_CLR, 0x00000010);	/* 0xF8001018,set bit4=1,95 */
	...
	if (type & VDE_DCM) {
		...
		dcm_setl(VDEC_DCM_CON, 0x00000001);	/* 0xF6000018,95 */

		/* SMI_LARB1: VDEC */
		dcm_setl(SMI_LARB1_CON_CLR, 0x00000010);	/* 0xF6010018,set bit15=1,95 */
	...
	if (type & MJC_DCM) {
		dcm_writel(MJC_HW_DCM_DIS, 0x0000000F);	/* 0xF7000010,95 */
		/* SMI_LARB4: MJC */
		dcm_setl(SMI_LARB4_CON_CLR, 0x00000010);	/* 0xF7002018,set bit15=1,95 */
	...
	if (type & EMI_DCM) {
		...
		dcm_setl(EMI_CONM, 0xFF000000);	/* 0xF0203060,set bit31~24=1,95 */
