...
#define print_func() dbgmsg("in %s\n", __func__);
#else
#define dbgmsg pr_crit
#define print_func() dbgmsg(CIRQ_LOG_LEVEL "in %s\n", __func__);
#endif
#else
#define dbgmsg(...)
...
do { \
	if (x < 0 || x >= CIRQ_IRQ_NUM) { \
		pr_crit(KERN_ERR "[CIRQ] %s: wrong cirq num %d which is less than 0 or larger than %d\n",\
...
static void mt_cirq_dump_reg(void);
...
void mt_cirq_ack_all(void)
{
	...
	for (i = 0; i < CIRQ_CTRL_REG_NUM; i++)
		writel_relaxed(0xFFFFFFFF, (void __iomem *)(CIRQ_ACK_BASE + (i * 4)));
...
void mt_cirq_mask_all(void)
{
	...
	for (i = 0; i < CIRQ_CTRL_REG_NUM; i++)
		writel_relaxed(0xFFFFFFFF, (void __iomem *)(CIRQ_MASK_SET_BASE + (i * 4)));
...
void mt_cirq_unmask_all(void)
{
	...
	for (i = 0; i < CIRQ_CTRL_REG_NUM; i++)
		writel_relaxed(0xFFFFFFFF, (void __iomem *)(CIRQ_MASK_CLR_BASE + (i * 4)));
...
static int mt_cirq_set_sens(unsigned int cirq_num, unsigned int sens)
{
	...
	if (sens == MT_EDGE_SENSITIVE) {
		...
		pr_crit(KERN_ERR "[CIRQ] set_sens invalid sensitivity value %d\n", sens);
		return -1;
	}
...
static int mt_cirq_set_pol(unsigned int cirq_num, unsigned int pol)
{
	...
	if (pol == MT_CIRQ_POL_NEG) {
		...
		pr_crit(KERN_ERR "[CIRQ] set_pol invalid polarity value %d\n", pol);
		return -1;
	}
...
static void mt_cirq_dump_reg(void)
{
	...
	pr_crit(KERN_NOTICE "IRQ:\tPOL\tSENS\tMASK\n");
	for (cirq_num = 0; cirq_num < CIRQ_IRQ_NUM; cirq_num++) {
			...
			for (irq_iter = 0; __check_irq_type[irq_iter].num >= 0; irq_iter++) {
				if (__check_irq_type[irq_iter].num == CIRQ_TO_IRQ_NUM(cirq_num)) {
					if (__check_irq_type[irq_iter].sensitivity != sens) {
						pr_crit(KERN_NOTICE "[CIRQ] Error sens in irq:%d\n",
...
static int mt_cirq_test(void)
{
	...
	if (mt_cirq_get_pol(cirq_num) != MT_CIRQ_POL_NEG)
		pr_crit(KERN_NOTICE "mt_cirq_set_pol test failed!!\n");
	mt_cirq_set_pol(cirq_num, MT_CIRQ_POL_POS);
	...
	if (mt_cirq_get_sens(cirq_num) != MT_EDGE_SENSITIVE)
		pr_crit(KERN_NOTICE "mt_cirq_set_sens test failed!!\n");
	mt_cirq_set_sens(cirq_num, MT_LEVEL_SENSITIVE);
	...
	if (mt_cirq_get_mask(cirq_num) != 1)
		pr_crit(KERN_NOTICE "mt_cirq_mask test failed!!\n");
	mt_cirq_unmask(cirq_num);
	if (mt_cirq_get_mask(cirq_num) != 0)
		pr_crit(KERN_NOTICE "mt_cirq_unmask test failed!!\n");
...
{
	pr_crit(CIRQ_LOG_LEVEL "CIRQ_Handler\n");
...
static int __init mt_cirq_init(void)
{
	...
	pr_crit(CIRQ_LOG_LEVEL "CIRQ init...\n");

	if (request_irq(SYS_CIRQ_IRQ_BIT_ID, cirq_irq_handler, IRQF_TRIGGER_LOW, "CIRQ", NULL))
		pr_crit(KERN_ERR "CIRQ IRQ LINE NOT AVAILABLE!!\n");
	else
		pr_crit(CIRQ_LOG_LEVEL "CIRQ handler init success.\n");

	ret = driver_register(&mt_cirq_drv.driver);
	if (ret == 0)
		pr_crit(CIRQ_LOG_LEVEL "CIRQ init done...\n");

#ifdef LDVT
	ret = driver_create_file(&mt_cirq_drv.driver, &driver_attr_cirq_dvt);
	if (ret == 0)
		pr_crit(CIRQ_LOG_LEVEL "CIRQ create sysfs file done...\n");
#endif
