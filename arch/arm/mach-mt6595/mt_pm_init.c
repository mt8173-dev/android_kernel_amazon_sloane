...
/* mt6331_upmu_set_rg_vsram_dvfs1_en, mt6332_upmu_set_rg_vsram_dvfs2_en */
...
static void _pmic_late_init(void)
{
	...
	printk("mt_get_chip_sw_ver: %d\n", ver);
	if (ver == CHIP_SW_VER_01) {
		enable_pll(ARMCA15PLL, "pll");
		disable_pll(ARMCA15PLL, "pll");

		/* #if defined (MT_LP_HQA) */
		/* enable ca15l power isolation */
		pminit_write(SPM_SLEEP_DUAL_VCORE_PWR_CON,
			     pminit_read(SPM_SLEEP_DUAL_VCORE_PWR_CON) | VCA15_PWR_ISO);
