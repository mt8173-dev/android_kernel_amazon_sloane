...
void BM_Enable(const unsigned int enable)
{
	...
	mt65xx_reg_sync_writel((value & ~(BUS_MON_PAUSE | BUS_MON_EN)) | (enable ? BUS_MON_EN : 0),
			       EMI_BMEN);
}
...
int BM_GetTransTypeCount(const unsigned int counter_num)
{
	return (counter_num < 1
		|| counter_num >
		BM_COUNTER_MAX) ? BM_ERR_WRONG_REQ : readl(IOMEM(EMI_TTYPE1 +
								 (counter_num - 1) * 8));
}
...
{
	unsigned int value;
	value = readl(IOMEM(EMI_BMEN2)) & ~(0 b11 << 24);
