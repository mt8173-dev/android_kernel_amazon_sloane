...
static int abc123_gpio_to_irq(struct lab_i2c_board_info *info);

#ifdef CONFIG_SND_SOC_MAX97236
static struct max97236_pdata max97236_platform_data __initdata = {
	.irq_gpio = -1,
};
static int abc123_max97236_init(struct lab_i2c_board_info *info)
{
...
static void cyttsp4_config_ldo_abc123(void)
{
...
static int cyttsp4_xres_abc123(struct cyttsp4_core_platform_data *pdata, struct device *dev)
{
...
static int cyttsp4_init_abc123(struct cyttsp4_core_platform_data *pdata, int on, struct device *dev)
{
	const unsigned long rst_gpio = (unsigned long)pdata->rst_gpio;
...
#if 0
static int abc123_gpio_to_eint(struct lab_i2c_board_info *info)
{
...
static void abc123_gpio_init(int gpio, u32 flags, int pull_mode)
{
	if (gpio > 0) {
		int err;
		mt_pin_set_mode_gpio(gpio);
		mt_pin_set_pull(gpio, pull_mode);
		err = gpio_request_one(gpio, flags, "abc123");
...
static int abc123_touch_init(struct lab_i2c_board_info *info)
{
	int err;
	u16 gpio_irq = info->bi.irq;
...
static struct lp855x_rom_data abc123_lp8557_eeprom_arr[] = {
	{0x14, 0xCF}, /* 4V OV, 4 LED string enabled */
};

static struct lp855x_platform_data abc123_lp8557_pdata = {
	.mode = REGISTER_BASED,
	.device_control = (LP8557_I2C_CONFIG | LP8557_DISABLE_LEDS),
	.initial_brightness = 0x64,
	.max_brightness = 0xFF,
	.load_new_rom_data = 1,
	.size_program = ARRAY_SIZE(abc123_lp8557_eeprom_arr),
	.rom_data = abc123_lp8557_eeprom_arr,
...
static struct lab_i2c_board_info abc123_i2c_bus2_backlight[] = {
	{
		.bi = {
			I2C_BOARD_INFO("lp8557_led", 0x2C),
			.platform_data = &abc123_lp8557_pdata,
		},
	},
};
...
static int abc123_mpu_init(struct lab_i2c_board_info *info)
{
	unsigned gpio_irq = info->bi.irq;
	int err;
 ...
 */
static void abc123_slimport_clk_enable(bool enable)
{
	if (enable)
...
static struct anx3618_platform_data anx3618_data = {
	...
	.anx3618_clk_enable = abc123_slimport_clk_enable,
};
...
static int anx_init(struct lab_i2c_board_info *info)
{
	...
	abc123_gpio_init(pdata->gpio_1_0_v_on,
		GPIOF_DIR_OUT,	MT_PIN_PULL_DISABLE);
	abc123_gpio_init(pdata->gpio_rst,
		GPIOF_DIR_OUT,	MT_PIN_PULL_DISABLE);
	abc123_gpio_init(pdata->gpio_hdmi,
		GPIOF_DIR_OUT,	MT_PIN_PULL_DISABLE);
	abc123_gpio_init(pdata->gpio_vmch,
		GPIOF_DIR_OUT,	MT_PIN_PULL_DISABLE);
	abc123_gpio_init(pdata->gpio_usb_plug,
		GPIOF_DIR_OUT,	MT_PIN_PULL_DISABLE);
	abc123_gpio_init(pdata->gpio_pd,
		GPIOF_DIR_OUT,	MT_PIN_PULL_DISABLE);
	abc123_gpio_init(pdata->gpio_sel_uart,
		GPIOF_DIR_IN,	MT_PIN_PULL_ENABLE_DOWN);
	abc123_gpio_init(pdata->gpio_cable_det,
		GPIOF_DIR_IN,	MT_PIN_PULL_ENABLE_UP);
	abc123_gpio_init(pdata->gpio_intp,
		GPIOF_DIR_IN,	MT_PIN_PULL_ENABLE_UP);
	abc123_gpio_init(pdata->gpio_usb_det,
		GPIOF_DIR_IN,	MT_PIN_PULL_DISABLE);
	abc123_gpio_to_irq(info);
...
static struct i2c_board_cfg abc123_i2c_config[] = {
	{
		...
		.max_rev =	BOARD_REV_abc123_PROTO_2_0,
	},
	{
		LAB_I2C_BUS_CFG(0, i2c_bus0_evt1, 100),
		.min_rev =	BOARD_REV_abc123_PROTO_2_1,
		.max_rev =	(u16)-1,
	},
	{
	...
	{
		LAB_I2C_BUS_CFG(2, abc123_i2c_bus2_backlight, 100),
		.max_rev =      (u16)-1,
	},
	{
	...
	},
	{
		...
		.min_rev =	BOARD_REV_abc123_EVT_1_1,
		.max_rev =	(u16)-1,
	},
	{
		LAB_I2C_BUS_CFG(5, i2c_bus5_evt1, 300),
		.max_rev =	BOARD_REV_abc123_EVT_1_0,
	}
};

static int __init abc123_config_i2c(struct i2c_board_cfg *bus_cfg, int n_cfg, u16 brd_rev)
{
	int i, j, err;
...
struct mt_pinctrl_cfg abc123_pin_cfg[] = {
...
static void __init mt_abc123_serial_init(void)
{
...
static void __init stingray_init(void)
{
	...
	mt_pinctrl_init(abc123_pin_cfg, NULL);

	pm_power_off = mt_power_off;
	mt_charger_init();

	mtk_wifi_sdio_set_data(&stingray_wifi_sdio_data);
	...
	abc123_config_i2c(
		abc123_i2c_config, ARRAY_SIZE(abc123_i2c_config), BOARD_REV_STRINGRAY_1_0);
