...
int mt65xx_start_gdma(int channel)
{
	...
	writel(DMA_INT_FLAG_CLR_BIT, (void *)(DMA_INT_FLAG(DMA_BASE_CH(channel))));
...
int mt65xx_stop_gdma(int channel)
{
	...
	writel(DMA_FLUSH_BIT, (void *)(DMA_FLUSH(DMA_BASE_CH(channel))));
	while (readl((void *)(DMA_START(DMA_BASE_CH(channel)))));
	writel(DMA_FLUSH_CLR_BIT, (void *)(DMA_FLUSH(DMA_BASE_CH(channel))));
...
/* pr_info("GDMA fixed address mode requires 8-bytes aligned address\n"); */
	if (!config->sinc) {
		pr_info("GMDA fixed adress mode doesn't support\n");
		return -DMA_ERR_INV_CONFIG;
	}
...
/* pr_info("GDMA fixed address mode requires 8-bytes aligned address\n"); */
	...
	switch (flag) {
	case ALL:
		writel(config->src, (void *)(DMA_SRC(DMA_BASE_CH(channel))));
		writel(config->dst, (void *)(DMA_DST(DMA_BASE_CH(channel))));
		writel((config->wplen) & DMA_GDMA_LEN_MAX_MASK,
		       (void *)(DMA_LEN2(DMA_BASE_CH(channel))));
		writel(config->wpto, (void *)(DMA_JUMP_ADDR(DMA_BASE_CH(channel))));
		writel((config->count) & DMA_GDMA_LEN_MAX_MASK,
		       (void *)(DMA_LEN1(DMA_BASE_CH(channel))));

		/*setup coherence bus */
		if (config->cohen) {
			writel((DMA_READ_COHER_BIT |
				readl((void *)(DMA_AXIATTR(DMA_BASE_CH(channel))))),
			       (void *)(DMA_AXIATTR(DMA_BASE_CH(channel))));
			writel((DMA_WRITE_COHER_BIT |
				readl((void *)(DMA_AXIATTR(DMA_BASE_CH(channel))))),
			       (void *)(DMA_AXIATTR(DMA_BASE_CH(channel))));
		}
		...
		if (config->sec) {
			pr_info("1:GMDA GSEC:%x, ChSEC:%x\n", readl((void *)(DMA_GLOBAL_GSEC_EN)),
				readl((void *)(DMA_GDMA_SEC_EN(channel))));
			writel((DMA_GSEC_EN_BIT | readl((void *)(DMA_GLOBAL_GSEC_EN))),
			       (void *)(DMA_GLOBAL_GSEC_EN));
			writel((DMA_SEC_EN_BIT | readl((void *)(DMA_GDMA_SEC_EN(channel)))),
			       (void *)(DMA_GDMA_SEC_EN(channel)));
			pr_info("2:GMDA GSEC:%x, ChSEC:%x\n", readl((void *)(DMA_GLOBAL_GSEC_EN)),
				readl((void *)(DMA_GDMA_SEC_EN(channel))));
		} else {
			pr_info("1:GMDA GSEC:%x, ChSEC:%x\n", readl((void *)(DMA_GLOBAL_GSEC_EN)),
				readl((void *)(DMA_GDMA_SEC_EN(channel))));
		...
		if (config->iten) {
			writel(DMA_INT_EN_BIT, (void *)(DMA_INT_EN(DMA_BASE_CH(channel))));
		} else {
			writel(DMA_INT_EN_CLR_BIT, (void *)(DMA_INT_EN(DMA_BASE_CH(channel))));
		}
		...
		writel(dma_con, (void *)(DMA_CON(DMA_BASE_CH(channel))));
		break;
...
{
	const unsigned glbsta = readl((void *)(DMA_GLOBAL_INT_FLAG));
...
#if (DMA_DEBUG == 1)
		glbsta = readl((void *)(DMA_GLOBAL_INT_FLAG));
		pr_debug("DMA Module - GLBSTA after ack = 0x%x\n", glbsta);
#endif
	}
...
{
	pr_crit("GDMA1 checks EMI MPU violation.\n");
	pr_crit("addr = 0x%x, %s violation.\n", addr, wr_vio ? "Write" : "Read");
	pr_crit("DMA SRC = 0x%x.\n", readl((void *)(DMA_SRC(DMA_BASE_CH(0)))));
	pr_crit("DMA DST = 0x%x.\n", readl((void *)(DMA_DST(DMA_BASE_CH(0)))));
	pr_crit("DMA COUNT = 0x%x.\n", readl((void *)(DMA_LEN1(DMA_BASE_CH(0)))));
	pr_crit("DMA CON = 0x%x.\n", readl((void *)(DMA_CON(DMA_BASE_CH(0)))));
}
...
/* mt6577_irq_set_sens(MT6577_GDMA1_IRQ_ID, MT_LEVEL_SENSITIVE); */
/* mt6577_irq_set_polarity(MT6577_GDMA1_IRQ_ID, MT_POLARITY_LOW); */

	if (request_irq(MT_GDMA1_IRQ_ID, gdma1_irq_handler, IRQF_TRIGGER_LOW, "GDMA1", NULL)) {
		pr_err("GDMA1 IRQ LINE NOT AVAILABLE!!\n");
	}
/* enable_irq(MT6577_GDMA1_IRQ_ID); */

/* mt6577_irq_set_sens(MT6577_GDMA2_IRQ_ID, MT_LEVEL_SENSITIVE); */
/* mt6577_irq_set_polarity(MT6577_GDMA2_IRQ_ID, MT_POLARITY_LOW); */

	if (request_irq(MT_GDMA2_IRQ_ID, gdma2_irq_handler, IRQF_TRIGGER_LOW, "GDMA2", NULL)) {
		pr_err("GDMA2 IRQ LINE NOT AVAILABLE!!\n");
	}

	pr_info("[APDMA] Init APDMA OK\n");
...
void mt65xx_dma_running_status(void)
{
	...
	dma_running_status = readl((void *)(DMA_GLOBAL_RUNNING_STATUS));
	for (i = 0; i < 15; i++) {
		if (((dma_running_status >> i) & 0x01) == 1) {
			pr_info("DMA %s is running\n", DMA_name[i]);
		}
	}
}
