...
#define DVFS_CA15_F3    (975500)	/* KHz */
#define DVFS_CA15_F4    (806750)	/* KHz */
...
unsigned int mt_cpufreq_max_frequency_by_DVS(unsigned int num, unsigned int cur_cluster)
{
	...
	if (num == voltage_change_num) {
		xlog_printk(ANDROID_LOG_INFO, "Power/DVFS",
				...
				int num)
{
	...
	if (mt_cpu_freqs_table[cur_cluster] == NULL) {
		...
		xlog_printk(ANDROID_LOG_INFO, "Power/DVFS", "Create cpufreq table\n");
	}
			   ...
			   unsigned int cur_cluster)
{
	...
	if (freq_new > freq_old) {
		...
		if (cur_cluster == CA7_CLUSTER) {	/* CA7 */
			...
			mt65xx_reg_sync_writel(armpll, ARMPLL2_CON1);	/* CA7 */
			mb();
			...
			mt_cpu_clock_switch(TOP_CKMUXSEL_ARMPLL2, cur_cluster);
		} else {
			mt_cpu_clock_switch(g_top_ckmuxsel, cur_cluster);
			mt65xx_reg_sync_writel(armpll, ARMPLL_CON1);	/* CA15 */
			mb();
			udelay(PLL_SETTLE_TIME);
			...
			mt_cpu_clock_switch(TOP_CKMUXSEL_ARMPLL, cur_cluster);
		}
			     ...
			     unsigned int relation)
{
    ...
    ***************************************/
	...
	for (i = 0; i < mt_cpu_freqs_num[cur_cluster]; i++) {
		dprintk("cpu = %d, freqs.new = %d, mt_cpu_freqs[%d].cpufreq_khz = %d\n", cpu,
			freqs.new, i, mt_cpu_freqs[cur_cluster][i].cpufreq_khz);
...
static int mt_cpufreq_init(struct cpufreq_policy *policy)
{
    ...
    **********************************************/
	...
	xlog_printk(ANDROID_LOG_INFO, "Power/DVFS", "CPU %d initialized\n", policy->cpu);
...
{
	xlog_printk(ANDROID_LOG_INFO, "Power/DVFS", "%s: Exited, cpu: %d\n", __func__, policy->cpu);
...
no_policy:
	xlog_printk(ANDROID_LOG_INFO, "Power/DVFS", "Cannot get the policy\n");
	return -EINVAL;
...
void __ref mt_cpufreq_thermal_protect(unsigned int limited_power)
{
	...
	if (g_thermal_type == 1) {	/* throttle according to power budget table to find the best performance */
		...
		xlog_printk(ANDROID_LOG_INFO, "Power/DVFS", "No thermal throttle\n");
	}
...
static int mt_cpufreq_pdrv_probe(struct platform_device *pdev)
{
    ...
    *************************************************/
	...
	xlog_printk(ANDROID_LOG_INFO, "Power/DVFS", "Current VCore: 0x%x\n", g_vcore);
