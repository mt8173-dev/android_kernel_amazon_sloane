...
int raw_spin_trylock_n_irqsave(raw_spinlock_t *lock, unsigned long *flags, struct seq_file *m, char *msg){
	...
	if (!locked){
		...
		if (lock->owner && lock->owner != SPINLOCK_OWNER_INIT )
			owner = lock->owner;
		SEQ_printf(m, " lock: %p, .magic: %08x, .owner: %s/%d, .owner_cpu: %d, value: %u\n",
			lock, lock->magic,
			owner ? owner->comm : "<<none>>",
			owner ? task_pid_nr(owner) : -1,
			lock->owner_cpu, __ARCH_SPIN_LOCK_RAW_VALUE(lock));
#endif
	}
...
int spin_trylock_n_irqsave(spinlock_t *lock, unsigned long *flags, struct seq_file *m, char *msg){
	...
	if (!locked){
		...
		if (rlock.owner && rlock.owner != SPINLOCK_OWNER_INIT )
			owner = rlock.owner;
		SEQ_printf(m, " lock: %p, .magic: %08x, .owner: %s/%d, .owner_cpu: %d, value: %d\n",
			lock, rlock.magic,
			owner ? owner->comm : "<<none>>",
			owner ? task_pid_nr(owner) : -1,
			rlock.owner_cpu, __ARCH_SPIN_LOCK_RAW_VALUE(&rlock));
#endif
	}
