...
/* #define PHYMEM_MAP_CHUNK    (16*COMPRESS_1MB) */
static ssize_t mdump_compdump_binary_read(struct file *filp,
		struct kobject *kobj,
		struct bin_attribute *attr,
		char *buf, loff_t off, size_t count)
{
	unsigned long expbase;
	...
	if (expbase > (unsigned long)(mdcattr->paddr)) {
		...
		if (mdcattr->vaddr == NULL) {
			pr_err("compress dump function can not map physicall addr: 0x%lx\n", expbase);
			return 0;
		}
	...
	if ((off % PHYMEM_MAP_CHUNK) == 0) {
		pr_err("total %u MBytes had been sent!\n", (unsigned int)(((off/PHYMEM_MAP_CHUNK)*PHYMEM_MAP_CHUNK)/COMPRESS_1MB));
	}
...
static void check_compress_dump(void)
{
	...
	*/
	...
	pr_info("%s: compression dump size is: 0x%x\n", __func__, (unsigned int) s_mdump_compdump.binattr.size);
...
void mdump_mark_reboot_reason(int reason)
{
	...
	if (s_mdump_buffer->signature != MDUMP_BUFFER_SIG || (s_mdump_buffer->reboot_reason != (u8)reason)) {
		printk(KERN_EMERG "mdump_mark_reboot_reason: buffer addr: 0x%p ## bad sig: 0x%x\n", s_mdump_buffer, s_mdump_buffer->signature);
		...
		/* printk(KERN_EMERG "MDump buffer corrupted!!!!, have to recover.\n"); */
	} else {
		/* printk(KERN_EMERG "*********************MDump reboot reason marked!*************************.\n"); */
		pr_err("reboot reason: %d\n", reason);
	}
}
