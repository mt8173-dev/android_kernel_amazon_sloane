				...
				char *buf)
{
	...
	oz_polling_lock_bh();
	s = sprintf(buf, "MAC\tState\tApps\tDevNum\tBattery\n");
	if (s > 0) {
		buf += s;
		count = s;

		list_for_each(e, oz_get_pdlist()) {
			pd = container_of(e, struct oz_pd, link);

			s = sprintf(buf, "%pm\t", pd->mac_addr);
			if (s < 0)
				break;
			count += s;
			buf += s;
			...
			/* Service */
			s = sprintf(buf, "%04X\t", pd->total_apps);
			if (s < 0)
				break;
			count += s;
			buf += s;
			...
			/* DevNum */
			s = sprintf(buf, "%d\t", oz_usb_get_bus_addr(pd));
			if (s < 0)
				break;
			count += s;
			buf += s;
