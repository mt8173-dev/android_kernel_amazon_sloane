...
#define MEMPOOL_BUFFER_SIZE  (sizeof(struct oz_endpoint) + OZ_EP_BUFFER_SIZE_ISOC)
#define MIN_NUM_BUFFER  (2)
...
static void *oz_ep_mempool_alloc(gfp_t gfp_mask, void *pool_data)
{
	return kzalloc(MEMPOOL_BUFFER_SIZE, gfp_mask);
}

static void oz_ep_mempool_free(void *element, void *pool_data)
{
	kfree(element);
}
...
static struct oz_endpoint *oz_ep_alloc(gfp_t mem_flags, int buffer_size)
{
	...
	if (buffer_size != OZ_EP_BUFFER_SIZE_ISOC) {
		oz_trace("kzalloc alloc buffer_size %d\n", buffer_size);
		ep = kzalloc(sizeof(struct oz_endpoint)+buffer_size, mem_flags);
	} else {
		oz_trace("mempool_alloc buffer_size %d\n", buffer_size);
		ep = mempool_alloc(g_mempool, mem_flags);
		oz_trace("mempool_alloc g_mempool->curr_nr %d\n", g_mempool->curr_nr);
	}
	if (ep) {
...
static void oz_ep_free(struct oz_port *port, struct oz_endpoint *ep)
{
	if (port) {
		...
		if (ep->flags & OZ_F_EP_HAVE_REF_CLK_STREAM)
			oz_usb_ref_clock_stream_delete(port->hpd, ep->ep_num);
	...
	if (ep->buffer_size != OZ_EP_BUFFER_SIZE_ISOC) {
		kfree(ep);
	} else {
		oz_trace("mempool_free.\n");
		mempool_free(ep, g_mempool);
	}
}
/*------------------------------------------------------------------------------
 * Context: softirq
...
void oz_complete_buffered_urb(struct oz_port *port, struct oz_endpoint *ep,
			struct urb *urb)
{
	int ep_data_len, copy_len, this_copy_len;
	int status = 0;
	int out_ix;
	struct oz_hcd *ozhcd = port->ozhcd;
	...
	spin_lock_bh(&ozhcd->hcd_lock);
	out_ix = ep->out_ix;
	ep_data_len = ep->buffer[out_ix];

	if (ep_data_len <= urb->transfer_buffer_length) {
		copy_len = ep_data_len;
	} else {
	...
	/* Length field was on the edge of FIFO top. */
	if (++out_ix == ep->buffer_size)
		out_ix = 0;

	/* Copy data to FIFO top */
	this_copy_len = copy_len;
	if (this_copy_len >= (ep->buffer_size - out_ix))
		this_copy_len = ep->buffer_size - out_ix;
	memcpy(urb->transfer_buffer, &ep->buffer[out_ix], this_copy_len);
	copy_len -= this_copy_len;

	/* Copy date from FIFO bottom */
	if (copy_len) {
		memcpy((urb->transfer_buffer + this_copy_len), ep->buffer,
						copy_len);
		out_ix += copy_len;
	}

	/* Update ep parameters. */
	ep->out_ix += ep_data_len+1;
	if (ep->out_ix >= ep->buffer_size) {
		ep->out_ix -= ep->buffer_size;
	}
	ep->buffered_units--;

	spin_unlock_bh(&ozhcd->hcd_lock);

	oz_complete_urb(port->ozhcd->hcd, urb, status);
}
/*------------------------------------------------------------------------------
 * Context: softirq
 */
				...
				u8 frame, const u8 *data, int data_len)
{
	...
	if (ep->buffered_units >= ep->max_buffer_units*2) {
		int len;
		oz_trace_msg(M,
			"BUFFER OVER_RUN ep->buffered_units = %d\n", ep->buffered_units);
...
/*------------------------------------------------------------------------------
 */
int oz_hcd_copy_isoc_frame(struct oz_endpoint *ep, u8 * buffer, int update)
{
	int len = 0;
 ...
 * Context: unknown
 */
void oz_hcd_isoc_frame(void *hport, u8 endpoint, u8 frame, const u8 *data, int data_len)
{
	struct oz_port *port = (struct oz_port *)hport;
	struct oz_endpoint *ep;
	struct oz_hcd *ozhcd = port->ozhcd;
 ...
 * Context: unknown
 */
int oz_hcd_get_port_num(void *hport)
{
	int port_num = -1;
	struct oz_port *port = (struct oz_port *)hport;
	struct oz_hcd *ozhcd;
	if (port) {
		ozhcd = port->ozhcd;
		spin_lock_bh(&ozhcd->hcd_lock);
			port_num = oz_get_port_from_addr(
					ozhcd, port->bus_addr);
		spin_unlock_bh(&ozhcd->hcd_lock);
	}
	return port_num;
...
int oz_hcd_heartbeat(void *hport)
{
	...
	list_for_each(e, &port->isoc_out_ep) {
		...
		credit = div64_u64(timespec_to_ns(&delta), NSEC_PER_MSEC);
		ep->credit += credit;
		if (ep->credit > ep->credit_ceiling) {
			...
			ep->timestamp =
				timespec_add(
					ep->timestamp,
					ns_to_timespec(credit*NSEC_PER_MSEC));
		}
	...
	list_for_each(e, &port->isoc_in_ep) {
		...
		if (ep->flags & OZ_F_EP_BUFFERING) {
			if (ep->buffered_units >= ep->max_buffer_units) {
				oz_trace_msg(M,
					"Silent EP_BUFFERING done ep->buffered_units = %d and ep->silent_frames = %d\n",
					ep->buffered_units, ep->silent_frames);
				ep->flags &= ~OZ_F_EP_BUFFERING;
				ep->credit = 0;
				ep->credit2 = 0;
				ep->timestamp = ts;
			...
			for (i = 0; i < urb->number_of_packets; i++) {
				...
				do {
					...
					if (ep->buffered_units == 0) {
						oz_trace_msg(M,
							"EP:%02X Buffer under run ep->next_frame #%02X\n",
							ep->ep_num | USB_DIR_IN,
							ep->next_frame);
						proc = STREAM_PROC_INSERT_SILENCE_FRAME;
						ep->flags |= OZ_F_EP_BUFFERING;
						/* ep->next_frame++; */
						ep->lost_frames = 1;
						ep->buffer_underrun = 1;
						if (ep->max_buffer_units >= 100) {
							ep->max_buffer_units = 100;
						}
						break;
					}
					...
					/*This is the first frame after the buffer underrun.
					 *For ths frame not to be dropped make this frame
					 *as the expected frame.
					 */
					if (ep->buffer_underrun == 1) {
						oz_trace_msg(M,
							"First Frame after under run :#%02X #%02X\n",
							 ep->next_frame, frame);
						ep->next_frame = frame;
						ep->buffer_underrun = 0;
					}
					/* This flag is set during the initialization.
					 * This is to make sure that the first frame received by host from remote is not dropped.
					 * If the frames are dropped by host this might lead to buffer over run.
					 */
					if (ep->flags & OZ_F_EP_ISOC_FRAME_NUM_VALID) {
						ep->next_frame = frame;
						ep->flags &=
				...
				case STREAM_PROC_INSERT_SILENCE_FRAME:
					...
					ep->silent_frames++;
					oz_trace_msg(I, "STREAM_PROC_INSERT_SILENCE_FRAME len = %d\n", len);
	...
	oz_usb_ref_clock_send(port->hpd, 0x83);
	return rc;
}
/*------------------------------------------------------------------------------
 * Context: softirq
	...
	for (i = 0; i < intf->desc.bNumEndpoints; i++) {
		...
		if ((ep->attrib & USB_ENDPOINT_XFERTYPE_MASK)
			...
			if (ep_addr & USB_ENDPOINT_DIR_MASK) {
				...
				/*
				oz_usb_ref_clock_stream_create(
					port->hpd, ep_addr);
				ep->flags |= OZ_F_EP_HAVE_REF_CLK_STREAM;
				*/
			} else {
...
int oz_hcd_init(void)
{
	...
	if (err)
		goto error2;
	oz_trace("oz_hcd_init: mempool_create.\n");
	g_mempool = mempool_create(MIN_NUM_BUFFER, oz_ep_mempool_alloc, oz_ep_mempool_free, NULL);
	if (!g_mempool) {
		oz_trace("Failed to create memory pool for endpiont\n");
		goto error2;
	}
	return 0;
error2:
