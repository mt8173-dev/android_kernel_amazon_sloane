			    ...
			    void *__cmdq)
{
	...
	DISP_REG_SET(cmdq, DISP_COLOR_G_PIC_ADJ_MAIN_1,
		     (g_Color_Index.
		      BRIGHTNESS[pq_param_p->
				 u4Brightness] << 16) | g_Color_Index.CONTRAST[pq_param_p->
									       u4Contrast]);
	...
	for (index = 0; index < 8; index++) {
		DISP_REG_SET(cmdq, DISP_COLOR_Y_SLOPE_1_0_MAIN + 4 * index,
			     (g_Color_Index.
			      PARTIAL_Y[2 * index] | g_Color_Index.PARTIAL_Y[2 * index + 1] << 16));
	}
	...
	case DISP_IOCTL_READ_REG:
		{
			...
			if (rParams.reg > DISPSYS_REG_ADDR_MAX
			    || rParams.reg < DISPSYS_REG_ADDR_MIN) {
				COLOR_ERR
	...
	case DISP_IOCTL_WRITE_REG:
		{
			...
			if (wParams.reg > DISPSYS_REG_ADDR_MAX
			    || wParams.reg < DISPSYS_REG_ADDR_MIN) {
				COLOR_ERR
				    ("reg write, addr invalid, addr min=0x%x, max=0x%x, addr=0x%x\n",
	...
	if ((module == DISP_MODULE_COLOR0) && (state == CMDQ_AFTER_STREAM_EOF)) {
		ret =
		    cmdqRecReadToDataRegister(cmdq_trigger_handle,
					      IO_VIRT_TO_PHYS(DISP_COLOR_TWO_D_W1_RESULT),
