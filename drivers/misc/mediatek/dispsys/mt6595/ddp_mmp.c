	...
	if (pLayer->layer_en) {
		...
		if (!raw) {
			...
			if (m4u_mva_map_kernel
			    (pLayer->addr, Bitmap.data_size, (unsigned int *)&Bitmap.pData,
			     &Bitmap.data_size) == 0) {
				if (session == 1)
			...
			if (m4u_mva_map_kernel
			    (pLayer->addr, meta.size, (unsigned int *)&meta.pData,
			     &meta.size) == 0) {
				if (session == 1)
			...
			unsigned int down_sample_x, unsigned int down_sample_y)
{
	...
	if (!raw) {
		...
		Bitmap.down_sample_y = down_sample_y;
		if (m4u_mva_map_kernel
		    (wdma_layer->dstAddress, Bitmap.data_size, (unsigned int *)&Bitmap.pData,
		     &Bitmap.data_size) == 0) {
		...
		if (m4u_mva_map_kernel
		    (wdma_layer->dstAddress, meta.size, (unsigned int *)&meta.pData,
		     &meta.size) == 0) {
