...
static PDSI_PHY_REGS const DSI_PHY_REG[2] =
    { (PDSI_PHY_REGS) (MIPI_TX0_BASE), (PDSI_PHY_REGS) (MIPI_TX1_BASE) };
static PDSI_CMDQ_REGS const DSI_CMDQ_REG[2] =
    { (PDSI_CMDQ_REGS) (DSI0_BASE + 0x200), (PDSI_CMDQ_REGS) (DSI1_BASE + 0x200) };
static PDSI_VM_CMDQ_REGS const DSI_VM_CMD_REG[2] =
    { (PDSI_VM_CMDQ_REGS) (DSI0_BASE + 0x134), (PDSI_VM_CMDQ_REGS) (DSI1_BASE + 0x134) };
...
void DSI_clk_HS_mode(bool enter)
{
	...
	if (enter /* && !DSI_clk_HS_state(i, cmdq) */) {
		DSI_OUTREGBIT(NULL, DSI_PHY_LCCON_REG, DSI_REG[i]->DSI_PHY_LCCON, LC_HS_TX_EN, 1);
	} else if (!enter /* && DSI_clk_HS_state(i, cmdq) */) {
		DSI_OUTREGBIT(NULL, DSI_PHY_LCCON_REG, DSI_REG[i]->DSI_PHY_LCCON, LC_HS_TX_EN, 0);
	}
}
...
void dsi_analysis(DISP_MODULE_ENUM module)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		DISPMSG
		    ("DSI%d IRQ,RD_RDY:%d, CMD_DONE:%d, EXT_TE:%d, SLEEPOUT_DONE:%d, TE_RDY:%d, VM_CMD_DONE:%d, VM_DONE:%d\n",
		     ...
		     DSI_REG[i]->DSI_INTSTA.TE_RDY, DSI_REG[i]->DSI_INTSTA.VM_CMD_DONE,
