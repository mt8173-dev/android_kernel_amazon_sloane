...
{				/* ourput setting */
	...
	unsigned int idx = rdma_index(module);
	DDPDBG
	    ("RDMAConfig idx %d, mode %d, address 0x%x, inputformat %s, pitch %u, width %u, height %u\n",
	     idx, mode, (unsigned int)address, rdma_intput_format_name(inputFormat, input_swap),
	     pitch, width, height);
	ASSERT(idx <= 2);
