...
int DDPK_Bitblt_Config(int channel, DdpkBitbltConfig *pParam)
{
	...
	spin_lock_irqsave(&g_ddpk_bitblt_chnls.lock, irq_state);
	{
		if ((g_ddpk_bitblt_chnls.request_flag & (0x1 << channel)) != 0) {
			DDPK_ERROR
			    ("DDPK DDPK_Bitblt_Config Bitblt Ch:%d (0x%08X) is busy. Config fail\n",
...
int DDPK_Bitblt(int channel)
{
	...
	spin_lock_irqsave(&g_ddpk_bitblt_chnls.lock, irq_state);
	{
		...
		DDPK_BBINFO("DDPK Bitblt Request Ch:%d (0x%08X)\n", channel,
			    (unsigned int)g_ddpk_bitblt_chnls.request_flag);
	}
	spin_unlock_irqrestore(&g_ddpk_bitblt_chnls.lock, irq_state);
	/*~Critical Section */
	...
	ret_of_wait = wait_event_interruptible_timeout(g_ddpk_bitblt_chnls.done_wait_queue,
						       (g_ddpk_bitblt_chnls.
							request_flag & (0x1 << channel)) == 0,
						       DDPK_Util_MsToJiffies(kTIMEOUT));
	if (ret_of_wait == 0) {
		DDPK_ERROR("DDPK Timeout. Ch:%d (0x%08X) timeout = %dms\n", channel,
			   (unsigned int)g_ddpk_bitblt_chnls.request_flag, (int)kTIMEOUT);
		return -1;
	}
...
unsigned int ddp_bitblt_ioctl_wait_reequest(unsigned long ioctl_user_param)
{
	...
	spin_lock_irqsave(&g_ddpk_bitblt_chnls.lock, irq_state);
	{
		...
		memcpy(&param.out_config, &(g_ddpk_bitblt_chnls.config[ch]),
		       sizeof(DdpkBitbltConfig));
	...
	if (copy_to_user
	    ((void __user *)ioctl_user_param, (void *)&param, sizeof(DDPIOCTL_DdpkBitbltConfig))) {
...
unsigned int ddp_bitblt_ioctl_inform_done(unsigned long ioctl_user_param)
{
	...
	if (copy_from_user
	    (&param, (const void __user *)ioctl_user_param,
	     sizeof(DDPIOCTL_DdpkBitbltInformDone))) {
	...
	spin_lock_irqsave(&g_ddpk_bitblt_chnls.lock, irq_state);
	{
		if ((g_ddpk_bitblt_chnls.request_flag & (0x1 << channel)) == 0) {
			DDPK_ERROR
			    ("DDPK Bitblt Ch:%d (0x%08X) is free, but a done signal received.\n",
			     channel, (unsigned int)g_ddpk_bitblt_chnls.request_flag);
			spin_unlock_irqrestore(&g_ddpk_bitblt_chnls.lock, irq_state);
			return -1;
		}
...
*/
	...
	dump_pixel(0, (char *)src_buffer[channel], TEST_OUT_WIDTH, TEST_OUT_WIDTH / 2 - 1,
		   TEST_OUT_HEIGHT / 4 - 1);
	dump_pixel(0, (char *)src_buffer[channel], TEST_OUT_WIDTH, TEST_OUT_WIDTH - 1,
		   TEST_OUT_HEIGHT / 2 - 1);
	dump_pixel(0, (char *)src_buffer[channel], TEST_OUT_WIDTH, 0, TEST_OUT_HEIGHT / 2);
	dump_pixel(0, (char *)src_buffer[channel], TEST_OUT_WIDTH, TEST_OUT_WIDTH / 2 - 1,
		   TEST_OUT_HEIGHT * 3 / 4 - 1);
	dump_pixel(0, (char *)src_buffer[channel], TEST_OUT_WIDTH, TEST_OUT_WIDTH - 1,
		   TEST_OUT_HEIGHT - 1);

	dump_pixel(1, (char *)dst_buffer[channel], TEST_OUT_WIDTH, 0, 0);
	dump_pixel(1, (char *)dst_buffer[channel], TEST_OUT_WIDTH, TEST_OUT_WIDTH / 2 - 1,
		   TEST_OUT_HEIGHT / 4 - 1);
	dump_pixel(1, (char *)dst_buffer[channel], TEST_OUT_WIDTH, TEST_OUT_WIDTH - 1,
		   TEST_OUT_HEIGHT / 2 - 1);
...
void ddpk_testfunc_2(unsigned long channel)
{
	...
	DDPK_DEBUG("src_buffer[%lu] = 0x%08X(PA) ==> 0x%08X\n", channel, (unsigned int)src_pa,
		   (unsigned int)src_buffer[channel]);
	...
	DDPK_DEBUG("dst_buffer[%lu] = 0x%08X(PA) ==> 0x%08X\n", channel, (unsigned int)dst_pa,
		   (unsigned int)dst_buffer[channel]);
