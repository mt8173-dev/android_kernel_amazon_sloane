...
#define DDP_ROT_LOG(fmt, arg...) {pr_info("[ROT]"); pr_info(fmt, ##arg); }
#else
#define DDP_ROT_LOG()
#endif
...
int ROTConfig(int rotateDegree,
	      ...
	      int srcWidth,
	      int srcHeight, int srcPitch, struct DISP_REGION srcROI, DISP_COLOR_FORMAT * outFormat)
{
	...
	disp_rot_reg_src(disp_rot_format(inFormat), memAddr,	/* src addr[3] */
			 srcWidth,	/* src widht */
			 srcHeight,	/* src height */
			 srcPitch, &srcROI, src_vdo_mode, src_vdo_field);	/* ROI(x,y,width, height) */

	*outFormat = DISP_COLOR_FORMAT_YUV_444_1P;

	if (interlace != DISP_INTERLACE_FORMAT_NONE
	    && inFormat == DISP_COLOR_FORMAT_YUV_420_2P_VDO_BLK) {
...
*/
unsigned disp_rot_get_bkgd(unsigned src_format, unsigned src_pitch, unsigned idx)	/* main frame or sub frame */
{
...
int disp_rot_src_offset(int pln, int boff_w, int boff_h)
{
	...
	/* address offset */
	tmp_0 = ((flip == 0) && (rotation == 1)) || ((flip == 0) && (rotation == 2)) || ((flip == 1)
											 &&
											 (rotation
											  == 2))
	    || ((flip == 1) && (rotation == 3));
	tmp_1 = ((flip == 0) && (rotation == 2)) || ((flip == 0) && (rotation == 3)) || ((flip == 1)
											 &&
											 (rotation
											  == 0))
	    || ((flip == 1) && (rotation == 3));
	...
	mf_offset =
	    (mf_bkgd_w * mf_src_h * mf_bpp - mf_bkgd_wb) * tmp_0 +
	    (mf_src_w * mf_bpp * mf_pad_h) * tmp_1;
	sf_offset =
	    (sf_bkgd_w * sf_src_h * sf_bpp - sf_bkgd_wb) * tmp_0 +
	    (sf_src_w * sf_bpp * sf_pad_h) * tmp_1;
...
void disp_rot_reg_src(ROT_SRC_FORMAT src_format,
		      ...
		      ROT_VDO_MODE src_vdo_mode, ROT_VDO_FIELD src_vdo_field)
{
...
/*    if (src_format == ROT_SRC_FORMAT_YCBCR_420_2P_HW_VDO_BLOCK)
	...
	DISP_REG_SET_FIELD(MF_BKGD_SIZE_IN_BYTE_FLD_MF_BKGD_WB, DISP_REG_ROT_MF_BKGD_SIZE_IN_BYTE,
			   g_src_mf_pitch);
	DISP_REG_SET_FIELD(SF_BKGD_SIZE_IN_BYTE_FLD_SF_BKGD_WB, DISP_REG_ROT_SF_BKGD_SIZE_IN_BYTE,
			   g_src_sf_pitch);
	...
	DISP_REG_SET_FIELD(MF_CLIP_SIZE_FLD_MF_CLIP_H, DISP_REG_ROT_MF_CLIP_SIZE,
			   g_dst_clip_height);
	...
	if (src_format == ROT_SRC_FORMAT_YCBCR_420_2P_HW_ISP_BLOCK ||
		...
		DISP_REG_SET_FIELD(MF_OFFSET_1_FLD_MF_OFFSET_H_1, DISP_REG_ROT_MF_OFFSET_1,
				   offset_h);
		DISP_REG_SET_FIELD(MF_OFFSET_1_FLD_MF_OFFSET_W_1, DISP_REG_ROT_MF_OFFSET_1,
				   offset_w);
...
void disp_rot_set_mfsf_parameter(void)
{
	...
	case ROT_SRC_FORMAT_YCBCR_420_3P_SW_SCAN:	/* 0 */
		...
		if ((g_rotation == ROT_DEGREE_0) || (g_rotation == ROT_DEGREE_180)) {
			...
			mb_pps =
			    ((lb_4b_mode ? (int)((mb_pps_ori + 1) / 2) * 2 : mb_pps_ori) +
			     1) & 0xFFFFFFFE;
			sb_pps =
			    ((lb_4b_mode ? (int)((sb_pps_ori + 1) / 2) * 2 : sb_pps_ori) +
			     1) & 0xFFFFFFFE;
	...
	case ROT_SRC_FORMAT_YCBCR_444_3P_SW_SCAN:	/* 8 */
		if ((g_rotation == ROT_DEGREE_0) || (g_rotation == ROT_DEGREE_180)) {
			...
			sb_pps_ori = (int)((sb_w_p + sb_depth - 1) / sb_depth);
			mb_pps =
			    ((lb_4b_mode ? (int)((mb_pps_ori + 3) / 4) * 4 : mb_pps_ori) +
			     1) & 0xFFFFFFFE;
			sb_pps =
			    ((lb_4b_mode ? (int)((sb_pps_ori + 3) / 4) * 4 : sb_pps_ori) +
			     1) & 0xFFFFFFFE;
		} else		/* horizontal rotation */
		{
			mb_depth = 64;
...
void disp_rot_reg_enable(BOOL enable)
{
	...
	if (enable) {
		...
		*(volatile kal_uint32 * )(DISPSYS_ROT_BASE + 0xe8) = 0x02580078;
		...
		DISP_REG_SET_FIELD(INTERRUPT_ENABLE_FLD_UNDERRUN_INT_EN,
				   DISP_REG_ROT_INTERRUPT_ENABLE, 0);
		DISP_REG_SET_FIELD(INTERRUPT_ENABLE_FLD_REG_UPDATE_INT_EN,
				   DISP_REG_ROT_INTERRUPT_ENABLE, 0);
		DISP_REG_SET_FIELD(INTERRUPT_ENABLE_FLD_FRAME_COMPLETE_INT_EN,
				   DISP_REG_ROT_INTERRUPT_ENABLE, 0);
...
void disp_rot_reg_reset(void)
{
	...
	while ((DISP_REG_GET(DISP_REG_ROT_MON_STA_1) & 0xfff00) != 0x100) {
		delay_cnt++;
		if (delay_cnt > 10000) {
			pr_info("[DDP] error, disp_rot_reg_reset() timeout!\n");
