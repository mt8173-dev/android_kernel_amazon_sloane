...
unsigned int ovl_fmt_convert(unsigned int fmt);
int OVLStart(void)
{
...
int OVLReset(void)
{
	...
	while ((DISP_REG_GET(DISP_REG_OVL_INTSTA) & 0x1) != 0) {
		delay_cnt++;
		if (delay_cnt > 10000) {
			pr_info("[DDP] error, OVLReset() timeout!\n");
			break;
		}
	}
...
int OVLROI(unsigned int bgW, unsigned int bgH, unsigned int bgColor)
{
	if ((bgW > OVL_MAX_WIDTH) || (bgH > OVL_MAX_HEIGHT)) {
		pr_info("error: OVLROI(), exceed OVL max size, w=%d, h=%d\n", bgW, bgH);
		ASSERT(0);
	}
		...
		unsigned int en_3d, unsigned int landscape, unsigned int r_first)
{
	...
	case 3:
		...
		break;
	default:
		pr_info("error: OVL3DConfig(), invalid layer=%d\n", layer_id);	/* invalid layer */
		ASSERT(0);
	}
...
int OVLLayerConfig(unsigned int layer, unsigned int source, unsigned int format, unsigned int addr, unsigned int src_x,	/* ROI x offset */
		   ...
		   bool keyEn, unsigned int key,	/* color key */
		   ...
		   unsigned char alpha)
{
	...
	if ((source == OVL_LAYER_SOURCE_SCL || source == OVL_LAYER_SOURCE_PQ) &&
		...
		pr_info("error: direct link to OVL only support YUV444!\n");
		ASSERT(0);	/* direct link support YUV444 only */
	}

	if ((source == OVL_LAYER_SOURCE_MEM && addr == 0)) {
		pr_info("error: source from memory, but addr is 0!\n");
		ASSERT(0);	/* direct link support YUV444 only */
	}
	...
	if ((src_x & 1)) {
		switch (fmt) {
		case OVL_INPUT_FORMAT_YUYV:
			fmt = OVL_INPUT_FORMAT_YVYU;
			break;
		case OVL_INPUT_FORMAT_UYVY:
			fmt = OVL_INPUT_FORMAT_VYUY;
			break;
		case OVL_INPUT_FORMAT_YVYU:
			fmt = OVL_INPUT_FORMAT_YUYV;
			break;
		case OVL_INPUT_FORMAT_VYUY:
			fmt = OVL_INPUT_FORMAT_UYVY;
			break;
		}
	}
...
unsigned int ovl_fmt_convert(unsigned int fmt)
{
	unsigned int ovl_fmt = OVL_INPUT_FORMAT_UNKNOWN;

	/* pr_info("BOOT ovl_fmt_convert 0 fmt=%d, ovl_fmt=%d\n", fmt, ovl_fmt); */
	switch (fmt) {
	case eYUY2:
	...
	default:
		pr_info("error: DDP, unknown ovl input format = %d\n", fmt);
	}

	/* pr_info("BOOT ovl_fmt_convert 1 fmt=%d, ovl_fmt=%d\n", fmt, ovl_fmt); */
...
void OVLLayerTdshpEn(unsigned layer, bool en)
{
	...
	if (en) {
		...
		DISP_REG_SET_FIELD(L0_CON_FLD_LAYER_SRC, DISP_REG_OVL_L0_CON + layer * 0x20,
				   OVL_LAYER_SOURCE_PQ);
