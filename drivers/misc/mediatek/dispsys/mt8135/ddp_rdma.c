...
#ifndef ASSERT
/* #define ASSERT(expr) do {pr_info("ASSERT error func=%s, line=%d\n", __FUNC__, __LINE__);} while (!(expr)) */
#define ASSERT(expr) do {pr_info("ASSERT error \n"); } while (!(expr))
#endif
...
int RDMAReset(unsigned idx)
{
	...
	pr_info("[DDP] RDMAReset called %d\n", cnt++);
...
int RDMAConfig(unsigned idx, enum RDMA_MODE mode, enum RDMA_INPUT_FORMAT inputFormat, unsigned address, enum RDMA_OUTPUT_FORMAT outputFormat, unsigned pitch, unsigned width, unsigned height, bool isByteSwap,	/* input setting */
	       bool isRGBSwap)	/* ourput setting */
{
	...
	pr_info("RDMA: w=%d, h=%d, addr=%x, pitch=%d, mode=%d\n", width, height, address,
		width * bpp, mode);
