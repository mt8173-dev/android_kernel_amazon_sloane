...
int dprec_logger_get_result_string(DPREC_LOGGER_ENUM source, char *stringbuf, int strlen)
{
	...
	len +=
	    scnprintf(stringbuf + len, strlen - len,
		      "|%-24s|%8llu |%8lld.%02ld |%8llu.%02ld |%8llu.%02ld |%8llu.%02ld |%8llu.%02ld |\n",
		      dprec_logger_spy(source), l->count, fps_high, fps_low, SPLIT_NS(avg),
		      SPLIT_NS(l->period_max_frame), SPLIT_NS(l->period_min_frame), SPLIT_NS(l->period_frame));
	...
	if (cmdq) {
		if (mask) {
			DISPPR_HWOP("%s/0x%p/0x%08x=0x%08x&0x%08x\n",
				    _find_module_by_reg_addr(reg), cmdq, reg, val,
				    mask);
		} else {
