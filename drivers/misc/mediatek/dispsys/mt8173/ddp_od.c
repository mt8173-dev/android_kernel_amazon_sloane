...
#define OD_REG_SET_FIELD(reg32, val, field)  \
       do {  \
	      mt_reg_sync_writel((*(volatile unsigned int*)((unsigned long)reg32) & ~REG_FLD_MASK(field))|REG_FLD_VAL((field), (val)), (unsigned long)reg32);  \
	  } while (0)
...
#define OD_REG_SET(reg32, val) \
	do { if (g_od_cmdq_p) DISP_REG_SET(*g_od_cmdq_p, reg32, val); else DISP_REG_SET(NULL, reg32, val); } while (0)
...
#define OD_REG_BEGIN(cmdq) \
	do { if (cmdq) g_od_cmdq_p = (cmdqRecHandle*)(&cmdq); else od_create_cmdq(); } while (0)
...
static void _od_reg_init(const void *od_init_table)
{
	...
	for (index = 0; index < tbl_size + 1; index++, init_table++) {
		if (REGTBL_END == init_table->reg_addr) {
			...
			OD_REG_SET(init_table->reg_addr,
