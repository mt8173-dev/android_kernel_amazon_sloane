...
static int g_disp_fence_log_level;
int disp_get_fence_log_level(void)
{
	return g_disp_fence_log_level;
}
...
static void process_dbg_opt(const char *opt)
{
	...
	if (0 == strncmp(opt, "regr:", 5)) {
...
#endif
	} else if (0 == strncmp(opt, "dbg_level:", 10)) {
		char *p = (char *)opt + 10;
		unsigned int level;
		unsigned int enable;
		...
		STR_CONVERT(&p, &level, uint, goto Error);
		STR_CONVERT(&p, &enable, uint, goto Error);
		set_ddp_log_level(level, enable);
	} else if (0 == strncmp(opt, "fence_log:", 10)) {
		char *p = (char *)opt + 10;
		STR_CONVERT(&p, &g_disp_fence_log_level, uint, goto Error);
	} else {
		dbg_buf[0] = '\0';
		goto Error;
	}
