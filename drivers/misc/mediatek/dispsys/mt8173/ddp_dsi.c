...
#define DSI_OUTREGBIT(cmdq, TYPE, REG, bit, value)  \
	do {\
		TYPE r;\
		TYPE v;\
		if (cmdq)		{*(unsigned int *)(&r) = ((unsigned int)0x00000000); r.bit = ~(r.bit);  *(unsigned int *)(&v) = ((unsigned int)0x00000000); v.bit = value; DISP_REG_MASK(cmdq, (unsigned long)&REG, AS_UINT32(&v), AS_UINT32(&r)); }		\
		else			{r = *((TYPE *)&INREG32(&REG)); r.bit = (value);		DISP_REG_SET(cmdq, (unsigned long)&REG, AS_UINT32(&r)); }				\
	} while (0);
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		if (0 != data_Rate) {
			...
			if (1 != dsi_params->ssc_disable) {
				...
				DSI_OUTREGBIT(cmdq, MIPITX_DSI_PLL_CON1_REG,
					DSI_PHY_REG[i]->MIPITX_DSI_PLL_CON1, RG_DSI0_MPPLL_SDM_SSC_PRD, 0x1B1);
