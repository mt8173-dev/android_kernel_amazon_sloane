...
const char *ddp_get_clk_name(MM_CLK_ENUM clk_enum)
{
	...
	case MM_CLK_DPI1_ENGINE:
		...
		/*case MM_CLK_HDMI_PIXEL:
		   return "MMSYS_CLK_HDMI_PIXEL";
		   case MM_CLK_HDMI_PLLCK:
		   return "MMSYS_CLK_HDMI_PLLCK";
		   case MM_CLK_HDMI_AUDIO:
		   return "MMSYS_CLK_HDMI_AUDIO";
		   case MM_CLK_HDMI_SPDIF:
		   return "MMSYS_CLK_HDMI_SPDIF"; */
	...
	case MM_CLK_MUX_DPI0_SEL:
		...
		/*case MM_CLK_HDMI_HDCP:
		   return "MMSYS_CLK_HDMI_HDCP";
		   case MM_CLK_HDMI_HDCP_24M:
		   return "MMSYS_CLK_HDMI_HDCP_24M"; */
	case APMIXED_TVDPLL:
...
int ddp_module_clock_enable(MM_CLK_ENUM module, bool enable)
{
	...
	if (DISP_REG_CONFIG_MMSYS_CG_CON0 != 0x100) {
		DDPMSG("MMSYS CLOCK before config %s->%s: 0x14000100:0x%x 0x14000110:0x%x\n",
...
int ddp_path_top_clock_on(void)
{
	...
	DDPMSG("ddp path vcore notify on\n");
	#endif
