			      ...
			      DPI_POLARITY polarity)
{
	...
	DISP_LOG_PRINT(ANDROID_LOG_WARN, "DPI", "ddp_dpi_ConfigPclk clk_req = %d\n", clk_req);
	...
	DISP_LOG_PRINT(ANDROID_LOG_WARN, "DPI", "u4Feq = 0x%x, bPix = 0x%x\n", u4Feq, bPix);

#ifndef CONFIG_FPGA_EARLY_PORTING
	DISP_LOG_PRINT(ANDROID_LOG_WARN, "DPI", "set dpi clk 1 0x%x\n", clksrc);
	...
	DISP_LOG_PRINT(ANDROID_LOG_WARN, "DPI", "get dpi0 sel clk = 0x%x\n",
	...
	DISP_LOG_PRINT(ANDROID_LOG_WARN, "DPI", "get dpi clk 0x270=0x%x ,0x274=0x%x\n",
				     ...
				     UINT8 HDMI_Res)
{
	...
	printk("[DPI1] ddp_dpi_config_colorspace: ColorSpace[%d] / HDMI Resolution[%d]\n",
	       ColorSpace, HDMI_Res);
...
DPI_STATUS ddp_dpi_ConfigMatrix(DISP_MODULE_ENUM module, cmdqRecHandle cmdq, BOOL enable_matrix,
	BOOL enable_internal_matrix, UINT32 internal_matrix_sel, BOOL enable_external_matrix)
{
	DPI_REG_CNTL CNTL = DPI_REG[DPI_IDX(module)]->CNTL;
	DPI_REG_MATRIX_SET MATRIX_SET = DPI_REG[DPI_IDX(module)]->MATRIX_SET; /* 00B4 */

	CNTL.MATRIX_EN = enable_matrix;
	MATRIX_SET.EXT_MATRIX_EN = enable_external_matrix;
	MATRIX_SET.INT_MATRIX_SEL = enable_internal_matrix;

	DISPMSG("enable_matrix %d enable_external_matrix %d enable_internal_matrix %d==df==\n",
	enable_matrix, enable_external_matrix, enable_internal_matrix);

	if (enable_external_matrix) {
		DPI_REG_MATRIX_COEFF_SET00 MATRIX_COEFF_SET00 = DPI_REG[DPI_IDX(module)]->MATRIX_COEFF_SET00; /* 00B8 */
		DPI_REG_MATRIX_COEFF_SET02 MATRIX_COEFF_SET02 = DPI_REG[DPI_IDX(module)]->MATRIX_COEFF_SET02; /* 00BC */
		DPI_REG_MATRIX_COEFF_SET11 MATRIX_COEFF_SET11 = DPI_REG[DPI_IDX(module)]->MATRIX_COEFF_SET11; /* 00C0 */
		DPI_REG_MATRIX_COEFF_SET20 MATRIX_COEFF_SET20 = DPI_REG[DPI_IDX(module)]->MATRIX_COEFF_SET20; /* 00C4 */
		DPI_REG_MATRIX_COEFF_SET22 MATRIX_COEFF_SET22 = DPI_REG[DPI_IDX(module)]->MATRIX_COEFF_SET22; /* 00C8 */
		DPI_REG_MATRIX_IN_OFFSET0 MATRIX_IN_OFFSET0 = DPI_REG[DPI_IDX(module)]->MATRIX_IN_OFFSET0; /* 00CC */

		DPI_REG_MATRIX_IN_OFFSET2 MATRIX_IN_OFFSET2 = DPI_REG[DPI_IDX(module)]->MATRIX_IN_OFFSET2; /* 00D0 */
		DPI_REG_MATRIX_OUT_OFFSET0 MATRIX_OUT_OFFSET0 = DPI_REG[DPI_IDX(module)]->MATRIX_OUT_OFFSET0; /* 00D4 */
		DPI_REG_MATRIX_OUT_OFFSET2 MATRIX_OUT_OFFSET2 = DPI_REG[DPI_IDX(module)]->MATRIX_OUT_OFFSET2; /* 00D8 */

		MATRIX_COEFF_SET00.MATRIX_COEFF_C00 = 0x0;
		MATRIX_COEFF_SET00.MATRIX_COEFF_C01 = 0x370;
		MATRIX_COEFF_SET02.MATRIX_COEFF_C02 = 0x0;

		MATRIX_COEFF_SET02.MATRIX_COEFF_C10 = 0x0;
		MATRIX_COEFF_SET11.MATRIX_COEFF_C11 = 0x0;
		MATRIX_COEFF_SET11.MATRIX_COEFF_C12 = 0x370;

		MATRIX_COEFF_SET20.MATRIX_COEFF_C20 = 0x370;
		MATRIX_COEFF_SET20.MATRIX_COEFF_C21 = 0x0;
		MATRIX_COEFF_SET22.MATRIX_COEFF_C22 = 0x0;

		MATRIX_IN_OFFSET0.MATRIX_IN_OFFSET_0 = 0x0;
		MATRIX_IN_OFFSET0.MATRIX_IN_OFFSET_1 = 0x0;
		MATRIX_IN_OFFSET2.MATRIX_IN_OFFSET_2 = 0x0;


		MATRIX_OUT_OFFSET0.MATRIX_OUT_OFFSET_0 = 0x10;
		MATRIX_OUT_OFFSET0.MATRIX_OUT_OFFSET_1 = 0x10;
		MATRIX_OUT_OFFSET2.MATRIX_OUT_OFFSET_2 = 0x10;

		DPI_OUTREG32(cmdq, &DPI_REG[DPI_IDX(module)]->MATRIX_COEFF_SET00, AS_UINT32(&MATRIX_COEFF_SET00));
		DPI_OUTREG32(cmdq, &DPI_REG[DPI_IDX(module)]->MATRIX_COEFF_SET02, AS_UINT32(&MATRIX_COEFF_SET02));
		DPI_OUTREG32(cmdq, &DPI_REG[DPI_IDX(module)]->MATRIX_COEFF_SET11, AS_UINT32(&MATRIX_COEFF_SET11));
		DPI_OUTREG32(cmdq, &DPI_REG[DPI_IDX(module)]->MATRIX_COEFF_SET20, AS_UINT32(&MATRIX_COEFF_SET20));
		DPI_OUTREG32(cmdq, &DPI_REG[DPI_IDX(module)]->MATRIX_COEFF_SET22, AS_UINT32(&MATRIX_COEFF_SET22));
		DPI_OUTREG32(cmdq, &DPI_REG[DPI_IDX(module)]->MATRIX_IN_OFFSET0, AS_UINT32(&MATRIX_IN_OFFSET0));
		DPI_OUTREG32(cmdq, &DPI_REG[DPI_IDX(module)]->MATRIX_IN_OFFSET2, AS_UINT32(&MATRIX_IN_OFFSET2));
		DPI_OUTREG32(cmdq, &DPI_REG[DPI_IDX(module)]->MATRIX_OUT_OFFSET0, AS_UINT32(&MATRIX_OUT_OFFSET0));
		DPI_OUTREG32(cmdq, &DPI_REG[DPI_IDX(module)]->MATRIX_OUT_OFFSET2, AS_UINT32(&MATRIX_OUT_OFFSET2));
	}

	DPI_OUTREG32(cmdq, &DPI_REG[DPI_IDX(module)]->MATRIX_SET, AS_UINT32(&MATRIX_SET));
	DPI_OUTREG32(cmdq, &DPI_REG[DPI_IDX(module)]->CNTL, AS_UINT32(&CNTL));
...
int ddp_dpi_config(DISP_MODULE_ENUM module, disp_ddp_path_config *config, void *cmdq_handle)
{
	if (s_isDpiConfig == FALSE /*|| (module == DISP_MODULE_DPI0) */) {
		...
		DISP_LOG_PRINT(ANDROID_LOG_WARN, "DPI", "ddp_dpi%x_config DPI status:%x, cmdq:%x\n",
		...
		case DISP_MODULE_DPI0:
			{
				...
				ddp_dpi_ConfigSize(module, cmdq_handle, dpi_config->width,
						   Is_interlace_resolution(dpi_config->
									   dpi_clock) ? dpi_config->
						   height / 2 : dpi_config->height);
		...
		ddp_dpi_ConfigMatrix(module, cmdq_handle, true, false, 0x0, true);
...
int ddp_dpi_init(DISP_MODULE_ENUM module, void *cmdq)
{
	...
	DISP_LOG_PRINT(ANDROID_LOG_WARN, "DPI",
		       "APMIXEDSYS=0x%lx => APMIXEDSYS_VA=0x%lx, TOP_CLOCK=0x%lx => TOP_CLOCK_VA=0x%lx\n",
	...
	if (module == DISP_MODULE_DPI0) {
		DPI_OUTREGBIT(NULL, DPI_REG_INTERRUPT,
					DPI_REG[0]->INT_ENABLE, VSYNC, 1);
	} else {
		DPI_OUTREGBIT(NULL, DPI_REG_INTERRUPT,
					DPI_REG[1]->INT_ENABLE, VSYNC, 1);
	}
#endif
...
int ddp_dpi_deinit(DISP_MODULE_ENUM module, void *cmdq_handle)
{
	...
	if (module == DISP_MODULE_DPI0) {
		DPI_OUTREGBIT(NULL, DPI_REG_INTERRUPT,
					DPI_REG[0]->INT_ENABLE, VSYNC, 0);
	} else {
		DPI_OUTREGBIT(NULL, DPI_REG_INTERRUPT,
					DPI_REG[1]->INT_ENABLE, VSYNC, 0);
	}
...
void LVDS_PLL_Init(DISP_MODULE_ENUM module, unsigned int pixel_clock, cmdqRecHandle cmdq)
{
	...
	printk("postdiv = 0x%x, pcw = 0x%x, ck_div = 0x%x\n", postdiv, pcw, ck_div);
...
void ddp_dpi_lvds_config(DISP_MODULE_ENUM module, LCM_DPI_PARAMS *dpi_config, void *cmdq_handle)
{
	...
	LVDS_ANA_Init(module, dpi_config->is_lvds_dual_tx, cmdq_handle);
	LVDS_Clk_Path_Config();
	LVDS_PLL_Init(module, dpi_config->PLL_CLOCK, cmdq_handle);
	LVDS_DIG_Init(module, dpi_config->is_lvds_dual_tx, cmdq_handle);
}
