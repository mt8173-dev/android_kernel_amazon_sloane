...
#define DISP_REG_GET_FIELD(field, reg32) ((*(volatile unsigned int*)((unsigned long)reg32) & REG_FLD_MASK(field)) >> REG_FLD_SHIFT(field))
...
#define DISP_CPU_REG_SET_FIELD(field, reg32, val)  \
    do {                                \
	   if (0)dprec_reg_op(NULL, (unsigned int)(reg32), val<<REG_FLD_SHIFT(field), REG_FLD_MASK(field));\
...
static inline unsigned int ddp_addr_convert_va2pa(unsigned long va)
{
	...
	for (i = 0; i < DISP_MODULE_NUM; i++) {
		if (ddp_regbase_va[i] == (va & (~0xfffl))) {
			/* printk("ddp_addr_convert_va2pa success: va=0x%lx --> pa=0x%x!\n", va,  (ddp_regbase_pa[i]+(va&0xfffl))); */
...
#define MMSYS_DISPLAY_FLD_DCM_OVL1_SET						REG_FLD(1, 12)
