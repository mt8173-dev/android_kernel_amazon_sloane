...
int mt_led_set_pwm(int pwm_num, struct nled_setting *led)
{
	...
	/* pwm_set_spec_config(&pwm_setting); //-------------->temp mark */
...
int generate_led_pattern(enum mt65xx_led_pattern led_pattern)
{
	LEDS_DEBUG("[LED]led_breathe_pmic: led_pattern=%d\n", led_pattern);

	switch (led_pattern) {
	case MT65XX_LED_PMIC_OFF:
		upmu_set_isinks_ch1_en(0x0);
		upmu_set_isinks_ch2_en(0x0);
		break;
	case MT65XX_LED_PMIC_AMBER_BLINK:
		upmu_set_rg_bst_drv_1m_ck_pdn(0x0);
		upmu_set_isinks_ch1_mode(PMIC_PWM_0);   /*PWM MODE */
		upmu_set_isinks_ch1_step(0x7);  /* 24mA */
		upmu_set_isink_rsv2_isink1(0x1);
		upmu_set_isink_dim1_duty(15); /* 50/50 duty cycle */
		upmu_set_isink_dim1_fsel(23); /*1s period*/
		upmu_set_isinks_ch2_en(0x0);
		upmu_set_isinks_ch1_en(0x1);
		break;
	case MT65XX_LED_PMIC_WHITE_BLINK:
		upmu_set_rg_bst_drv_1m_ck_pdn(0x0);
		upmu_set_isinks_ch2_mode(PMIC_PWM_0);
		upmu_set_isinks_ch2_step(0x7);
		upmu_set_isink_rsv2_isink2(0x1);
		upmu_set_isink_dim2_duty(15);
		upmu_set_isink_dim2_fsel(23);
		upmu_set_isinks_ch1_en(0x0);
		upmu_set_isinks_ch2_en(0x1);
		break;
	case MT65XX_LED_PMIC_AMBER_ON:
		upmu_set_rg_bst_drv_1m_ck_pdn(0x0);
		upmu_set_isinks_ch1_mode(PMIC_PWM_0);
		upmu_set_isinks_ch1_step(0x7);
		upmu_set_isink_rsv2_isink1(0x1);
		upmu_set_isink_dim1_duty(31);
		upmu_set_isinks_ch2_en(0x0);
		upmu_set_isinks_ch1_en(0x1);
		break;
	case MT65XX_LED_PMIC_WHITE_ON:
		upmu_set_rg_bst_drv_1m_ck_pdn(0x0);
		upmu_set_isinks_ch2_mode(PMIC_PWM_0);
		upmu_set_isinks_ch2_step(0x7);
		upmu_set_isink_rsv2_isink2(0x1);
		...
		upmu_set_isinks_ch2_en(0x1);
		break;
	case MT65XX_LED_PMIC_AMBER_BREATHE:
		upmu_set_rg_bst_drv_1m_ck_pdn(0x0);
		upmu_set_isinks_ch1_mode(PMIC_PWM_1);  /* BREATHE MODE */
		upmu_set_isinks_ch1_step(0x7);
		upmu_set_isinks_breath1_trf_sel(0x2);  /* .523S */
		upmu_set_isinks_breath1_ton_sel(0x0);  /* 0.123S */
		upmu_set_isinks_breath1_toff_sel(0x0); /* 0.246S */
		upmu_set_isink_rsv2_isink1(0x1);
		upmu_set_isinks_ch2_en(0x0);
		upmu_set_isinks_ch1_en(0x1);
		break;
	case MT65XX_LED_PMIC_WHITE_BREATHE:
		upmu_set_rg_bst_drv_1m_ck_pdn(0x0);
		upmu_set_isinks_ch2_mode(PMIC_PWM_1);
		upmu_set_isinks_ch2_step(0x7);
		upmu_set_isinks_breath2_trf_sel(0x9);/* 1.876s */
		upmu_set_isinks_breath2_ton_sel(0x0);
		upmu_set_isinks_breath2_toff_sel(0x0);
		upmu_set_isink_rsv2_isink2(0x1);
		upmu_set_isinks_ch1_en(0x0);
		upmu_set_isinks_ch2_en(0x1);
		break;
	default:
		pr_err("[LED] Invalid led_pattern=%d\n", led_pattern);
		return -1;
	}
	return 0;
...
int mt_led_set_breath_mode(U32 isink_channel, U32 isink_mode)
{
	LEDS_DEBUG("[LEDS]mt_led_set_breath_mode: ch=%d,mode=0x%x\n", isink_channel, isink_mode);
	switch (isink_channel) {
	case 0x0:
		upmu_set_rg_bst_drv_1m_ck_pdn(0x0);
		upmu_set_isinks_ch0_mode(isink_mode);
		upmu_set_isink_rsv2_isink1(0x1);
		upmu_set_isinks_ch0_en(0x1);
		break;
	case 0x1:
		upmu_set_rg_bst_drv_1m_ck_pdn(0x0);
		upmu_set_isinks_ch1_mode(isink_mode);
		upmu_set_isink_rsv2_isink1(0x1);
		upmu_set_isinks_ch1_en(0x1);
		break;
	case 0x2:
		upmu_set_rg_bst_drv_1m_ck_pdn(0x0);
		upmu_set_isinks_ch2_mode(isink_mode);
		upmu_set_isink_rsv2_isink1(0x1);
		upmu_set_isinks_ch2_en(0x1);
		break;
	default:
	...
	if (!isink_mode) {
		LEDS_DEBUG("disable isink[%d] enable/disable=0x%x !\n", isink_channel, isink_mode);
		switch (isink_channel) {
		case 0x0:
			upmu_set_rg_bst_drv_1m_ck_pdn(0x1);
			upmu_set_isink_rsv2_isink1(0x0);
...
int mt_led_set_breath_time(U32 isink_channel, U32 breath_time)
{
	int ret = 0;
	LEDS_DEBUG("[LEDS]mt_led_set_breath_time: ch=%d,time=0x%x\n", isink_channel, breath_time);
	switch (isink_channel) {
	case 0x0:
		ret = pmic_config_interface(0x057C, breath_time, 0xFFFF, 0x0);
		break;
	case 0x1:
		ret = pmic_config_interface(0x057E, breath_time, 0xFFFF, 0x0);
		break;
	case 0x2:
		ret = pmic_config_interface(0x0580, breath_time, 0xFFFF, 0x0);
		break;
	default:
		break;
	}
...
int mt_led_set_step_current(U32 isink_channel, U32 step_current)
{
	LEDS_DEBUG("[LEDS]mt_led_set_step_current: isink=%d, step_current=0x%x\n", isink_channel, step_current);
	switch (isink_channel) {
	case 0x0:
		upmu_set_isinks_ch0_step(step_current);
		break;
	case 0x1:
		upmu_set_isinks_ch1_step(step_current);
		break;
	case 0x2:
		upmu_set_isinks_ch2_step(step_current);
		break;
	default:
		break;
	}
...
int mt_backlight_set_pwm(int pwm_num, u32 level, u32 div, struct PWM_config *config_data)
{
	...
	if (BacklightLevelSupport == BACKLIGHT_LEVEL_PWM_256_SUPPORT) {
		...
		if (level > 0 && level < 256) {
			...
			/* mt_pwm_disable(pwm_setting.pwm_no, config_data->pmic_pad);//-------------->temp mark */
		}
		return 0;
...
int mt_brightness_set_pmic(enum mt65xx_led_pmic pmic_type, u32 level, u32 div)
{
	...
	if (pmic_type == MT65XX_LED_PMIC_LCD_ISINK) {
		...
		if (level) {
			...
			/* upmu_set_isink_rsv2(0x4); */
			upmu_set_isink_rsv2_isink2(0x1);
			upmu_set_isinks_ch2_en(0x01);
		} else {
			upmu_set_isinks_ch2_en(0x00);
			upmu_set_isink_rsv2_isink2(0x00);
			/* upmu_set_isink_rsv2(0x00); */
			/* upmu_set_rg_bst_drv_1m_ck_pdn(0x1); */
			/* upmu_top2_bst_drv_ck_pdn(0x1); */
		}
		return 0;
	...
	case MT65XX_LED_MODE_PWM:
		if (strcmp(cust->name, "lcd-backlight") == 0) {
			...
			if (level == 0) {
				...
				/* mt_pwm_disable(cust->data, cust->config_data.pmic_pad);//-------------->temp mark */
...
#if 0 /*def CONTROL_BL_TEMPERATURE*/
		mutex_lock(&bl_level_limit_mutex);
		current_level = level;
		/* printk("brightness_set_cust:current_level=%d\n", current_level); */
		if (0 == limit_flag) {
		...
		if (strcmp(led_data->cust.name, "lcd-backlight")) {
			...
			if (MT65XX_LED_MODE_CUST_BLS_PWM == led_data->cust.mode) {
				mt_mt65xx_led_set_cust(&led_data->cust,
					((((1 << MT_LED_INTERNAL_LEVEL_BIT_CNT) - 1)*level + 127)/255));
			} else {
				mt_mt65xx_led_set_cust(&led_data->cust, led_data->level);
			}
		}
