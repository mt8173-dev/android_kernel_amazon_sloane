...
/*if want to spi interrupt action, cancle the comment*/
				   ...
				   sprintf(tmp,"%s,%5lld:%8lld;",str,sched_clock() - spi_rec_t0,sched_clock()
				    - rec_time);
...
{
	enable_clock(MT_CG_PERI_SPI1_PDN, "spi");
	return;
}

static void disable_clk(void)
{
	disable_clock(MT_CG_PERI_SPI1_PDN, "spi");
	return;
}
...
static void spi_check_mpu_violation(u32 addr, int wr_vio)
{
	pr_crit("SPI checks EMI MPU violation.\n");
	pr_crit("addr = 0x%x, %s violation.\n", addr, wr_vio ? "Write" : "Read");
	pr_crit("DMA SRC = 0x%x.\n",
		__raw_readl((void const volatile *)(SPI1_BASE + SPI_TX_SRC_REG)));
	pr_crit("DMA DST = 0x%x.\n",
		__raw_readl((void const volatile *)(SPI1_BASE + SPI_RX_DST_REG)));
	pr_crit("DMA COUNT = 0x%x.\n",
		__raw_readl((void const volatile *)(SPI1_BASE + SPI_CFG1_REG)));
	pr_crit("DMA CON = 0x%x.\n", __raw_readl((void const volatile *)(SPI1_BASE + SPI_CMD_REG)));
}
...
static int __init mt_spi_init(void)
{
	...
	pr_info("%s(%d)\n", __func__, __LINE__);
	pr_info("hank:0xf0005d20=0x%x\n", __raw_readl((void const volatile *)0xf0005d20));
	emi_mpu_notifier_register(MST_ID_MMPERI_1, spi_check_mpu_violation);
	/*fix build warning */
	/*
	   ret = platform_driver_register ( &mt_spi_driver );
	 */
	ret = platform_driver_probe(&mt_spi_driver, mt_spi_probe);
	return ret;
}
