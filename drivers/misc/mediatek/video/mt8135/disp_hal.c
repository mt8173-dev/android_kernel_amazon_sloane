...
extern void DSCWrite(UINT8 cmd, UINT8 bVal);
extern char DSCRead(UINT8 cmd);
...
int disphal_process_dbg_opt(char *buf, const char *opt)
{
	if (0 == strncmp(opt, "dbi:", 4)) {
		...
		if (0 == strncmp(opt + 4, "rd:", 3)) {
			char *p = (char *)opt + 7;
			UINT8 dsc_cmd = (UINT8) simple_strtoul(p, &p, 16);
			UINT8 Val = 0;
			Val = DSCRead(dsc_cmd);
			sprintf(buf, "dsc_cmd: Read: cmd = 0x%02X  Val = 0x%02X\n", dsc_cmd, Val);

		} else if (0 == strncmp(opt + 4, "wt:", 3)) {
			char *p = (char *)opt + 7;
			UINT8 dsc_cmd = (UINT8) simple_strtoul(p, &p, 16);
			UINT8 bVal = (UINT8) simple_strtoul(p + 1, &p, 16);

			DSCWrite(dsc_cmd, bVal);
			sprintf(buf, "dsc_cmd: write cmd = 0x%02X  bVal = 0x%02X\n", dsc_cmd, bVal);

		} else if (0 == strncmp(opt + 4, "rw:", 3)) {
			char *p = (char *)opt + 7;
			int i = 0;
			UINT8 dsc_cmd = (UINT8) simple_strtoul(p, &p, 16);
			UINT8 bVal = (UINT8) simple_strtoul(p + 1, &p, 16);
			UINT8 u2loop = (UINT16) simple_strtoul(p + 1, &p, 16);
			char RdVal = 0;
			for (i = 0; i < u2loop; i++) {
				DSCWrite(dsc_cmd, bVal);
				RdVal = DSCRead(dsc_cmd);
				if (RdVal != bVal) {
					break;
				}
			}
			i = i + 1;
			sprintf(buf,
				"dsc_cmd:rw: addr = 0x%02X ; bVal = 0x%02X ; loop times = 0x%04X ; run times = 0x%04X\n ",
				dsc_cmd, bVal, u2loop, i);
...
static const LCM_UTIL_FUNCS lcm_utils = {
	...
	.dsi_cabc_send = (void (*)(unsigned int *, unsigned int, unsigned char))DSI_cabc_send,
};
...
int disphal_update_screen(const LCM_DRIVER *lcm_drv, struct mutex *pLcmCmdMutex, UINT32 x,
...
int disphal_get_pwm(const LCM_DRIVER *lcm_drv, struct mutex *pLcmCmdMutex, UINT32 divider,
		    unsigned int *freq)
{
	mutex_lock(pLcmCmdMutex);
...
const LCM_DRIVER *disphal_get_lcm_driver(const char *lcm_name, unsigned long *lcm_index)
{
	LCM_DRIVER *lcm = NULL;
...
{
	/* DPI0 */
	mt_irq_set_sens(MT8135_DISP_DPI0_IRQ_ID, MT_LEVEL_SENSITIVE);
	mt_irq_set_polarity(MT8135_DISP_DPI0_IRQ_ID, MT_POLARITY_LOW);

	/* DSI */
	mt_irq_set_sens(MT8135_DISP_DSI_IRQ_ID, MT_LEVEL_SENSITIVE);
	mt_irq_set_polarity(MT8135_DISP_DSI_IRQ_ID, MT_POLARITY_LOW);
...
{
	int ret;
	ret = m4u_alloc_mva(M4U_CLNTMOD_WDMA, va, size, 0, 0, dma_pa);
	if (ret != 0) {
		return ret;
	}
	m4u_dma_cache_maint(M4U_CLNTMOD_WDMA, (const void *)va, size, DMA_BIDIRECTIONAL);
	return 0;
}

int disphal_unmap_overlay_out_buffer(unsigned int va, unsigned int size, unsigned int dma_pa)
{
	m4u_dealloc_mva(M4U_CLNTMOD_WDMA, va, size, dma_pa);
	return 0;
}
...
int disphal_init_overlay_to_memory(void)
{
	...
	disp_module_clock_on(DISP_MODULE_WDMA1, "disphal overlay2mem");
...
int disphal_deinit_overlay_to_memory(void)
{
	...
	portStruct.Direction = 0;
	m4u_config_port(&portStruct);
	disp_module_clock_off(DISP_MODULE_WDMA1, "disphal overlay2mem");
	return 0;
}
