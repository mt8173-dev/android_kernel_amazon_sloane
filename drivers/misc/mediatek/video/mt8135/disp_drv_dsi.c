...
inline DPI_FB_FORMAT get_dsi_tmp_buffer_format(void)
{
...
static void init_intermediate_buffers(UINT32 fbPhysAddr)
{
	...
	UINT32 tmpFbPitchInBytes = lcm_params->width * dsiTmpBufBpp;
	UINT32 tmpFbSizeInBytes = tmpFbPitchInBytes * lcm_params->height;
...
static void init_lcd(void)
{
	...
	if (lcm_params->dsi.mode == CMD_MODE) {
		LCD_CHECK_RET(LCD_ConfigDSIIfFormat(lcm_params->dsi.data_format.color_order,
						    ...
						    lcm_params->dsi.packet_size, TRUE));
...
static void init_dpi(BOOL isDpiPoweredOn)
{
	...
	DPI_CHECK_RET(DPI_FBSetSize(lcm_params->width, lcm_params->height));
...
void init_dsi(BOOL isDsiPoweredOn)
{
	...
	DSI_CHECK_RET(DSI_TXRX_Control(TRUE,	/* cksm_en */
				       ...
				       FALSE,	/* dis_eotp_en */
				       (BOOL) (1 - lcm_params->dsi.cont_clock), 0));	/* max_return_size */
...
#ifndef MT65XX_NEW_DISP
	DSI_PHY_clk_setting(lcm_params->dsi.pll_div1, lcm_params->dsi.pll_div2,
			    lcm_params->dsi.LANE_NUM);
#else
	if (lcm_params->dsi.mode == CMD_MODE) {
		DSI_PHY_clk_setting(lcm_params);
	}
	DSI_CHECK_RET(DSI_PS_Control
		      (lcm_params->dsi.PS, lcm_params->height, lcm_params->width * dsiTmpBufBpp));
#endif
...
static DISP_STATUS dsi_init(UINT32 fbVA, UINT32 fbPA, BOOL isLcmInited)
{
...

	{
		...
		config.ovl_config.src_pitch = ALIGN_TO(lcm_params->width, 32) * 2;	/* pixel number */
		...
		LCD_LayerSetPitch(FB_LAYER, ALIGN_TO(lcm_params->width, 32) * 2);
...
static DISP_STATUS dsi_update_screen(BOOL isMuextLocked)
{
	...
	if (DDMS_capturing)
		DISP_LOG_PRINT(ANDROID_LOG_INFO, "DSI",
			       "[DISP] kernel - dsi_update_screen. DDMS is capturing. Skip one frame.\n");
...
static UINT32 dsi_get_working_buffer_size(void)
{
	...
	if (lcm_params->dsi.mode != CMD_MODE) {
		...
		return
		    lcm_params->width *
		    lcm_params->height * dsiTmpBufBpp * lcm_params->dsi.intermediat_buffer_num;
	}
