...
static int mtkfb_probe(struct device *dev)
{
	...
	{
		extern LCM_PARAMS *lcm_params;
		struct fb_overlay_layer OvllayerInfo = { 0 };
		struct disp_path_config_struct info = { 0 };
		...
		OvllayerInfo.layer_id = 0;
		OvllayerInfo.layer_enable = TRUE;
		OvllayerInfo.src_base_addr = (void *)((DWORD) fbdev->fb_va_base);
		OvllayerInfo.src_phy_addr = (void *)fb_pa;
		OvllayerInfo.src_direct_link = 0;
		OvllayerInfo.src_fmt = MTK_FB_FORMAT_RGB565;
		OvllayerInfo.src_use_color_key = 0;
		OvllayerInfo.src_color_key = 0xFF;
		OvllayerInfo.src_pitch = ALIGN_TO(lcm_params->width, MTK_FB_ALIGNMENT);
		OvllayerInfo.src_offset_x = 0;
		OvllayerInfo.src_offset_y = 0;
		OvllayerInfo.src_width = lcm_params->width;
		OvllayerInfo.src_height = lcm_params->height;
		OvllayerInfo.tgt_offset_x = 0;
		OvllayerInfo.tgt_offset_y = 0;
		OvllayerInfo.tgt_width = lcm_params->width;
		OvllayerInfo.tgt_height = lcm_params->height;
		OvllayerInfo.layer_rotation = MTK_FB_ORIENTATION_0;
		OvllayerInfo.layer_type = LAYER_2D;
		OvllayerInfo.video_rotation = MTK_FB_ORIENTATION_0;
		OvllayerInfo.isTdshp = TRUE;	/* set to 1, will go through tdshp first, then layer blending, then to color */
		OvllayerInfo.next_buff_idx = 0;
		OvllayerInfo.identity = 0;
		OvllayerInfo.connected_type = 0;
		OvllayerInfo.security = 0;

		pr_info("[mtkfb] mtkfb_probe set va=0x%x, pa=0x%x\n",
			(unsigned int)fbdev->fb_va_base, fb_pa);
		Disp_Ovl_Engine_Set_layer_info(mtkfb_instance, &OvllayerInfo);

		OvllayerInfo.layer_id = 1;
		OvllayerInfo.layer_enable = FALSE;
		Disp_Ovl_Engine_Set_layer_info(mtkfb_instance, &OvllayerInfo);

		OvllayerInfo.layer_id = 2;
		OvllayerInfo.layer_enable = FALSE;
		Disp_Ovl_Engine_Set_layer_info(mtkfb_instance, &OvllayerInfo);

		OvllayerInfo.layer_id = 3;
		OvllayerInfo.layer_enable = FALSE;
		Disp_Ovl_Engine_Set_layer_info(mtkfb_instance, &OvllayerInfo);
		...
		info.bgROI.x = 0;
		info.bgROI.y = 0;
		info.bgROI.width = lcm_params->width;
		info.bgROI.height = lcm_params->height;

		if (LCM_TYPE_DSI == lcm_params->type) {
			if (lcm_params->dsi.mode == CMD_MODE)
				info.dstModule = DISP_MODULE_DSI_CMD;	/* DISP_MODULE_WDMA1 */
			else
				info.dstModule = DISP_MODULE_DSI_VDO;	/* DISP_MODULE_WDMA1 */
		} else if (LCM_TYPE_DPI == lcm_params->type) {
			info.dstModule = DISP_MODULE_DPI0;
		} else {
			info.dstModule = DISP_MODULE_DBI;
		}

		info.outFormat = RDMA_OUTPUT_FORMAT_ARGB;
		info.pitch = lcm_params->width * 2;
		info.srcModule = DISP_MODULE_OVL;

		info.srcROI.x = 0;
		info.srcROI.y = 0;
		info.srcROI.width = lcm_params->width;
		info.srcROI.height = lcm_params->height;
		/* memcpy(&(info.ovl_config), &OvllayerInfo, sizeof(OVL_CONFIG_STRUCT)); */

		/* ovl_config */
		info.addr = fb_pa;
		info.ovl_config.layer = 0;
		info.ovl_config.layer_en = 1;
		info.ovl_config.fmt = eRGB565;
		info.ovl_config.addr = fb_pa;
		info.ovl_config.source = OVL_LAYER_SOURCE_MEM;
		info.ovl_config.src_x = 0;
		info.ovl_config.src_y = 0;
		info.ovl_config.dst_x = 0;	/* ROI */
		info.ovl_config.dst_y = 0;
		info.ovl_config.dst_w = DISP_GetScreenWidth();
		info.ovl_config.dst_h = DISP_GetScreenHeight();
		info.ovl_config.src_pitch = ALIGN_TO(DISP_GetScreenWidth(), 32) * 2;	/* pixel number */
		info.ovl_config.keyEn = 0;
		info.ovl_config.key = 0xFF;	/* color key */
		info.ovl_config.aen = 0;	/* alpha enable */
		info.ovl_config.alpha = 0;




		Disp_Ovl_Engine_Set_Path_Info(mtkfb_instance, &info);
		Disp_Ovl_Engine_Trigger_Overlay(mtkfb_instance);
	}
#endif
