...
static char STR_HELP[] =
    ...
    "             power on/off lcm\n"
    "\n"
    "        cabc:[ui|mov|still|off]\n"
    "             cabc mode, UI/Moving picture/Still picture, and disable\n"
    "\n"
    "        lcd:[on|off]\n"
...
static void inline reset_fps_logger(void)
{
	memset(&fps, 0, sizeof(fps));
}

static void inline reset_hdmi_fps_logger(void)
{
...
void DBG_OnLcdDone(void)
{
	...
	if (fps.trigger_lcd_count >= dbg_opt.log_fps_wnd_size) {
		...
		DISP_LOG_PRINT(ANDROID_LOG_INFO, "DBG",
			       "MTKFB FPS: %ld.%02ld, Avg. update time: %ld.%02ld ms "
...
/* hdmi_force_init(); */
		...
		if (0 == strncmp(opt + 5, "off", 3)) {
			mtkfb_set_backlight_mode(0);
		} else if (0 == strncmp(opt + 5, "ui", 2)) {
	...
	} else if (0 == strncmp(opt, "ovl_log_level=", 14)) {
		char *p = (char *)opt + 14;
		unsigned long value;
		if (kstrtoul(p, 10, &value) != 0)
			pr_err("kstrtoul fail.\n");
		disp_ovl_engine_log_level = (int)value;
		pr_err("disp_ovl_engine_log_level = %d\n", disp_ovl_engine_log_level);
	} else {
		if (disphal_process_dbg_opt(buf, opt))
			goto Error;
	}
