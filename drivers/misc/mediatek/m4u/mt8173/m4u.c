	...
	if (va >= VMALLOC_START && va <= VMALLOC_END) {
		if (VMALLOC_END - va_align > page_num * PAGE_SIZE) {
			for_each_sg(table->sgl, sg, table->nents, i) {
				...
				#ifdef CONFIG_ARM64
				if (page < virt_to_page(PAGE_OFFSET) ||
				    page > virt_to_page(high_memory)) {
					M4UMSG(
						"vmalloc_to_page fail, va=0x%lx, page=0x%p, vmemmap:0x%p~0x%p\n",
						va_align + i * PAGE_SIZE, page,
		...
		if (PAGE_OFFSET - va_align > page_num * PAGE_SIZE &&
		    access_ok(VERIFY_READ, (void __user *)va, 1) &&
		    access_ok(VERIFY_READ, (void __user *)(va + size - 1), 1)) {
			...
			M4UMSG("%s fail, va=0x%lx, page num=%d\n",
				__func__, va_align, page_num);
			goto err;
		}
		 ...
		 if (va <= (unsigned long)high_memory &&
			...
			M4UMSG(
				"%s fail, va=0x%lx, page num=%d, lowmem:0x%lx~0x%lx, vmalloc:0x%lx~0x%lx\n",
...
static long MTK_M4U_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case MTK_M4U_T_DEALLOC_MVA:
		{
			...
			if (m4u_module.port >= M4U_PORT_UNKNOWN || m4u_module.port < 0)
				return -EINVAL;
	...
	case MTK_M4U_T_CONFIG_PORT:
		...
		if (m4u_port.ePortID >= M4U_PORT_UNKNOWN || m4u_port.ePortID < 0)
			return -EINVAL;
...
static int m4u_probe(struct platform_device *pdev)
{
	...
	if (0 == pdev->id) {
		...
		if (!m4u_tee_en) {
			...
			/* Add 5M for gap */
			securitymemorysize += 5*1024*1024;
			securitymemorysize = ALIGN(securitymemorysize,  SZ_1M);
