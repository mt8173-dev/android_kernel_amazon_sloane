...
#define M4U_ASSERT(x) {if (!(x)) \
	{xlog_printk(ANDROID_LOG_ERROR, "M4U", "assert fail, file:%s, line:%d",\
		__FILE__, __LINE__); } }
...
#define M4ULOG(string, args...) do { if (gM4uLogFlag)\
	xlog_printk(ANDROID_LOG_INFO, "M4U", "[pid=%d] "string, current->tgid, ##args);\
} while (0)

#endif

#define M4UMSG(string, args...)	xlog_printk(ANDROID_LOG_WARN, "M4U", string, ##args)
/* #define M4UINFO(string, args...) xlog_printk(ANDROID_LOG_DEBUG, "M4U", string,##args) */
#define M4UINFO(string, args...) xlog_printk(ANDROID_LOG_WARN, "M4U", string, ##args)
/* #define KERN_INFO */
...
#if M4U_TEE_WHOLE_ENABLE

#include "tz_cross/ta_mem.h"
#include "trustzone/kree/mem.h"
#include <linux/kthread.h>

static int m4u_invalid_tlb(M4U_ID_ENUM m4u_id, int L2_en, int isInvAll,
...
static uint32_t m4u_insert_seq_range_SQ(uint32_t m4u_index,
					uint32_t RangeReg_ID,
					uint32_t m4u_index_offset,
					uint32_t MVAStart, uint32_t MVAEnd);
#endif
...
#define M4U_MVA_DBUG(string, args...) pr_info("[M4U_K][MVA]"string, ##args)
...
static int m4u_dma_cache_flush_all(void);
...
static unsigned int m4u_index_of_larb[SMI_LARB_NR] = { 1, 1, 0, 1, 1 };
static unsigned int smi_port0_in_larbx[SMI_LARB_NR + 2] = { 0, 9, 16, 28, 43, 55, 56 };
static unsigned int m4u_port0_in_larbx[SMI_LARB_NR + 2] = { 0, 9, 16, 28, 42, 52, 53 };
...
typedef enum {
	...
	M4U_TEST_LEVEL_STRESS = 2	/* stricker verification ,
					may use M4UERR instead M4UMSG sometimes,
					used for our own internal test */
} M4U_TEST_LEVEL_ENUM;
					 ...
					 int L2_en, unsigned int mva_start, unsigned int mva_end)
{
	...
	while (!(m4uHw_get_field_by_mask
		...
		;
		m4uHw_set_field_by_mask(0, REG_L2_GPE_STATUS_SEC, F_L2_GPE_ST_RANGE_INV_DONE_SEC,
					0);
	}
	...
	while (pListHead != &(pNode->mvaList)) {
		...
		if (pList->mvaStart != 0) {
			...
			if (!(pList->flags & MVA_REGION_REGISTER))
				M4UERR
				    ("warning: in garbage reclaim: mva==0, but MVA_REGION_REGISTER is not set!! flag=0x%x\n",
				     pList->flags);
		}
	}
...
static long MTK_M4U_ioctl(struct file *a_pstFile, unsigned int a_Command, unsigned long a_Param)
{
	...
	case MTK_M4U_T_DEALLOC_MVA:
		{
			...
			if (pMvaInfo == NULL) {
				M4UMSG("error to dealloc mva: id=%s,va=0x%x,size=%d,mva=0x%x\n",
				       ...
				       m4u_module.BufAddr, m4u_module.BufSize, m4u_module.MVAStart);
	...
	case MTK_M4U_T_CACHE_SYNC:
		...
		M4UDBG
		    ("ioctl cachesync, moduleID=%d, eCacheSync=%d, buf_addr=0x%x, buf_length=0x%x\n",
	...
	case MTK_M4U_T_DUMP_PAGETABLE:
		do {
			...
			for (i = 0; i < page_num; i++) {
				pr_info("0x%08x, ", m4u_user_v2p(va + i * M4U_PAGE_SIZE));
				if ((i + 1) % 8 == 0)
					pr_info("\n 0x%08x: ", (va + ((i + 1) << 12)));
			}
			pr_info("\n");
...
static int MTK_M4U_mmap(struct file *a_pstFile, struct vm_area_struct *a_pstVMArea)
{
	...
	M4UMSG
	    ("MTK_M4U_Mmap, a_pstVMArea=0x%x, vm_start=0x%x, vm_pgoff=0x%x, size=0x%x, vm_page_prot=0x%x\n",
	     (unsigned int)a_pstVMArea, (unsigned int)a_pstVMArea->vm_start,
...
/*volatile static int gM4u_L2_invalid_range_done;*/
/*volatile static int gM4u_L2_invalid_all_done;*/
...
static irqreturn_t MTK_M4U_isr(int irq, void *dev_id)
{
	...
	if (IntrSrc & F_INT_TRANSLATION_FAULT) {
		...
		else if (larbID == 5)	/* audio larbID 5 */
			m4u_port = M4U_PORT_AUDIO;
		else		/* gcpu  larbID 6 */
			m4u_port = M4U_PORT_GCPU;
		...
		/* search invalid main tlb */
		{
			...
			for (i = 0; i < M4U_MAIN_TLB_NR; i++) {
				...
				if ((mva & (F_MAIN_TLB_VALID_BIT | F_MAIN_TLB_INV_DES_BIT))
					...
					pr_info("%d:0x%x ", i, mva & F_MAIN_TLB_VA_MSK);
				}
			}
			/* pr_info("\n"); */
		}
	...
	if (IntrSrc & F_INT_ENTRY_REPLACEMENT_FAULT) {
		...
		for (i = 0; i < M4U_CLNTMOD_MAX; i++) {
			if (0 != lock_cnt[i])
				pr_info("%s(lock=%d), ", m4u_get_module_name(i), lock_cnt[i]);
		}
		pr_info("\n");
	}
...
/*static struct class *pM4uClass;
static struct device *m4uDevice; */
static int m4u_probe(struct platform_device *pdev)
{
	struct proc_dir_entry *m4u_entry = NULL;
	MMP_Event M4U_Event;
	...
	*/
	...
	if (request_irq
	    (MT6589_MMU_L2_SEC_IRQ_ID, MTK_M4U_L2_isr, IRQF_TRIGGER_LOW, M4U_DEVNAME, NULL)) {
		M4UERR("request M4U2 IRQ line failed\n");
		return -ENODEV;
	}
...
int m4u_dump_reg(int m4u_index)
{
	...
	for (i = 0; i < M4U_REG_SIZE / 8; i += 4) {
		M4UDBG("+0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n", 8 * i,
		       M4U_ReadReg32(m4u_base, 8 * i + 4 * 0), M4U_ReadReg32(m4u_base,
									     8 * i + 4 * 1),
		       M4U_ReadReg32(m4u_base, 8 * i + 4 * 2), M4U_ReadReg32(m4u_base,
									     8 * i + 4 * 3),
		       M4U_ReadReg32(m4u_base, 8 * i + 4 * 4), M4U_ReadReg32(m4u_base,
									     8 * i + 4 * 5),
		       M4U_ReadReg32(m4u_base, 8 * i + 4 * 6), M4U_ReadReg32(m4u_base,
									     8 * i + 4 * 7));
	}
	...
	for (i = 0; i < g4M4UTagCount[m4u_index]; i++) {
		...
		if (regval & F_PFH_TAG_VALID_MSK) {	/* /> a valid Prefetch TLB entry */
			...
			if (mva >= sa_align && mva <= ea_align) {
				if (gTestLevel == M4U_TEST_LEVEL_STRESS) {
					M4UERR
					    ("prefetch: i=%d, idx=0x%x, MVAStart=0x%x, MVAEnd=0x%x, RegValue=0x%x\n",
					     i, m4u_index, MVAStart, MVAEnd, regval);
					m4u_dump_reg(m4u_index);
				} else if (gTestLevel == M4U_TEST_LEVEL_ENG) {
					M4UMSG
...
static int m4u_dump_pfh_tlb_tags(int m4u_id)
{
	...
	for (i = 0; i < M4U_PRE_TLB_NR; i++) {
		pr_info("0x%x ", M4U_ReadReg32(m4u_base, REG_MMU_PFH_TAG(i)));
		if ((i + 1) % 8 == 0)
			pr_info("\n");
	}
	pr_info("\n");
...
static int m4u_dump_pfh_tlb_des(int m4u_id)
{
	...
	M4UINFO("dump main tlb=======>\n");
	for (i = 0; i < M4U_PRE_TLB_NR; i++) {
		pr_info("%d:0x%x:0x%x,0x%x,0x%x,0x%x  ", i,
...
static void m4u_dump_pagetable_range(unsigned int mvaStart, unsigned int nr)
{
	...
	/* pr_info("m4u dump pagetable by range: start=0x%x, nr=%d ==============>\n", vaStart, nr); */
	/* pr_info("index : mva : PTE\n"); */
	pr_info("\n 0x%08x: ", mvaStart);
	for (i = 0; i < nr; i++) {
		pr_info("0x%08x, ", pteStart[i]);
		if ((i + 1) % 8 == 0) {
			pr_info("\n 0x%08x: ", (mvaStart + ((i + 1) << 12)));
		}
	}
	pr_info("\n");
	/* pr_info("m4u dump pagetable done : start=0x%x, nr=%d ==============<\n", vaStart, nr); */
}
...
static int m4u_dump_pagetable(M4U_MODULE_ID_ENUM eModuleID)
{
	...
	pr_info("[M4U_K] dump pagetable by module: %s, page_num=%d ========>\n",
...
{
	enable_clock(MT_CG_INFRA_M4U_PDN, "m4u");
	enable_clock(MT_CG_INFRA_SMI_PDN, "m4u");
	return 0;
}
...
static int m4u_clock_off(void)
{
	...
	disable_clock(MT_CG_INFRA_M4U_PDN, "m4u");
	disable_clock(MT_CG_INFRA_SMI_PDN, "m4u");
	return 0;
}
...
static void m4u_mvaGraph_dump(void)
{
	...
	pr_info("start      size     blocknum    owner\n");
				     ...
				     const unsigned int BufSize, mva_info_t *pMvaInfo)
{
	...
	for (s = 1;
		(s < (MVA_MAX_BLOCK_NR + 1)) && (mvaGraph[s] < nr);
	...
	/*unmap kernel va for debug*/
	{
		if (pMvaInfo->mapped_kernel_va_for_debug) {
			M4UMSG("[kernel_va_debug] unmap va: mva=0x%x, kernel_va=0x%x, size=0x%x\n",
			       ...
			       pMvaInfo->size);
			m4u_mva_unmap_kernel(pMvaInfo->mvaStart, pMvaInfo->size,
		...
		if ((pList->eModuleId == eModuleID) && (query_start >= s && query_end <= e)) {
			if (pList->mvaStart > 0) {	/* here we have allocated mva for this buffer */
				*pRetMVABuf = pList->mvaStart + (query_start - s);
				...
				/* here we have not allocated mva
				(this buffer is registered, and query for first time) */
				mva_info_t *pMvaInfo;
				   ...
				   unsigned int mvaRegionAddr, struct sg_table *sg_table)
{
	...
	/* fill pagetable with 0 */
	{
		...
		/* this 2 module is in both larb3&4 */
		if ((eModuleID != M4U_CLNTMOD_IMG) && (eModuleID != M4U_CLNTMOD_GDMA)) {
			m4u_invalidate_and_check(m4u_module_2_m4u_id(eModuleID), mvaRegionAddr,
						 mvaRegionAddr + BufSize + align_page_num * 0x1000 -
						 1);
		} else {
			m4u_invalid_tlb_by_range(M4U_ID_ALL, gM4U_L2_enable, mvaRegionAddr,
						 mvaRegionAddr + BufSize + align_page_num * 0x1000 -
						 1);
		}
			      ...
			      struct sg_table *sg_table)
{
	...
	/* this 2 module is in both larb3&4 */
	if ((eModuleID != M4U_CLNTMOD_IMG) && (eModuleID != M4U_CLNTMOD_GDMA)) {
		m4u_invalidate_and_check(m4u_module_2_m4u_id(eModuleID), mvaStart,
					 mvaStart + BufSize - 1);
	} else {
		m4u_invalid_tlb_by_range(M4U_ID_ALL, gM4U_L2_enable, mvaStart,
					 mvaStart + BufSize - 1);
	}
	spin_unlock(&gM4u_reg_lock);
...
static unsigned int m4u_do_mva_alloc_fix(unsigned int mva, unsigned int size, void *priv)
{
	...
	M4UMSG("m4u_do_mva_alloc_fix mva = 0x%x, size = 0x%x, priv = 0x%x.\n", mva, size, priv);
	/*-----------------------------------------------------*/
	/*calculate mva block number*/
	...
	M4UMSG("m4u_do_mva_alloc_fix region_start = 0x%x, nr = 0x%x, priv = 0x%x.\n", region_start, nr, priv);
...
static int __m4u_alloc_mva(mva_info_t *pMvaInfo, struct sg_table *sg_table)
{
	...
	/* check alignment */
	if (((BufAddr & (L1_CACHE_BYTES - 1)) != 0) || ((BufSize & (L1_CACHE_BYTES - 1)) != 0)) {
		M4ULOG
		    ("warning: alloc_mva with buffer not aligned: va=0x%x, size=0x%x, align=0x%x, module=%s\n",
		     BufAddr, BufSize, L1_CACHE_BYTES, m4u_get_module_name(eModuleID));
	}
...
#if __M4U_MAP_MVA_TO_KERNEL_FOR_DEBUG__
	{
		...
		int ret;
		ret = m4u_mva_map_kernel(mvaStart, BufSize, security, &kernel_va, &kernel_size);
		if (ret) {
		  ...
		  int security, int cache_coherent, unsigned int *pRetMVABuf)
{
	...
	int ret;
	pMvaInfo =
	    m4u_alloc_garbage_list(0, BufSize, eModuleID, BufAddr, 0, security, cache_coherent);
	ret = __m4u_alloc_mva(pMvaInfo, NULL);
...
static int __m4u_dealloc_mva(M4U_MODULE_ID_ENUM eModuleID,
			     const unsigned int BufAddr,
			     const unsigned int BufSize,
			     const unsigned int MVA, struct sg_table *sg_table)
{
	...
	{
		if (m4u_invalid_seq_range_by_mva
		    (m4u_module_2_m4u_id(eModuleID), MVA, MVA + BufSize - 1) == 0) {
			M4UMSG
			    ("warning: dealloc mva without invalid tlb range!! id=%s,add=0x%x,size=0x%x,mva=0x%x\n",
...
static int m4u_do_insert_seq_range(M4U_PORT_ID_ENUM eModuleID,
				   unsigned int MVAStart,
				   unsigned int MVAEnd, unsigned int entryCount)
{
	...
	for (i = m4u_index_offset; i < m4u_index_offset + SEQ_RANGE_NUM; i++) {
		if (1 == pRangeDes[i].Enabled) {
			if (MVAEnd < pRangeDes[i].MVAStart || MVAStart > pRangeDes[i].MVAEnd) {	/* no overlap */
                ...
                if((MVAEnd != pRangeDes[i].MVAEnd) ||(MVAStart != pRangeDes[i].MVAStart)) {
                    M4UMSG("insert range overlap!: larb=%d,module=%s\n",
                           m4u_module_2_larb(eModuleID),
                           m4u_get_module_name(eModuleID));

                    M4UMSG
                        ("error: insert tlb range is overlapped with previous ranges, current process=%s,!\n",
                         current->comm);
...
/* ====================================================== */
	...
	{
		M4U_POW_ON_TRY(eModuleID);
		m4u_insert_seq_range_SQ(m4u_index, RangeReg_ID, m4u_index_offset, MVAStart, MVAEnd);
		M4U_POW_OFF_TRY(eModuleID);
	}
...
int m4u_insert_seq_range(M4U_MODULE_ID_ENUM eModuleID, unsigned int MVAStart, unsigned int MVAEnd,
...
{				/* 0:disable multi-entry, 1,2,4,8,16: enable multi-entry */
	...
	mutex_unlock(&gM4uMutex);
	MMProfileLogEx(M4U_MMP_Events[PROFILE_INSERT_TLB], MMProfileFlagEnd, eModuleID,
		       MVAEnd - MVAStart + 1);
	...
	if (FreeSEQRegs[m4u_index] < SEQ_RANGE_NUM) {
		for (i = m4u_index_offset; i < m4u_index_offset + SEQ_RANGE_NUM; i++) {
			if (pRangeDes[i].Enabled == 1 &&
				...
				pRangeDes[i].Enabled = 0;
				m4u_insert_seq_range_SQ(m4u_index, i, m4u_index_offset, 0, 0);
				mb();
				FreeSEQRegs[m4u_index]++;
				break;
			}
...
int m4u_insert_wrapped_range(M4U_MODULE_ID_ENUM eModuleID,
			     M4U_PORT_ID_ENUM portID, unsigned int MVAStart, unsigned int MVAEnd)
{
	unsigned int i;
	...
	if (mva2module(MVAStart) != eModuleID || mva2module(MVAEnd) != eModuleID) {
		m4u_aee_print("insert wrap fail!: larb=%d,module=%s\n",
			      m4u_module_2_larb(eModuleID), m4u_get_module_name(eModuleID));
		M4UMSG
		    ("m4u_insert_wrapped_range module=%s, MVAStart=0x%x is %s, MVAEnd=0x%x is %s\n",
		     m4u_get_module_name(eModuleID), MVAStart,
		     m4u_get_module_name(mva2module(MVAStart)), MVAEnd,
		     m4u_get_module_name(mva2module(MVAEnd)));
		m4u_mvaGraph_dump();
	...
	for (i = m4u_index_offset; i < m4u_index_offset + g4M4UWrapCount[m4u_index]; i++) {
		if (1 == pWrapDes[i].Enabled) {
			if (MVAEnd < pWrapDes[i].MVAStart || MVAStart > pWrapDes[i].MVAEnd) {	/* no overlap */
                ...
                if((MVAEnd != pRangeDes[i].MVAEnd) ||(MVAStart != pRangeDes[i].MVAStart)) {
                    M4UMSG
                        ("error: insert wrap range is overlapped with previous ranges, current!\n");
                    M4UMSG("module=%s, mva_start=0x%x, mva_end=0x%x\n",
                           m4u_get_module_name(eModuleID), MVAStart, MVAEnd);
                    M4UMSG
                        ("overlapped range id=%d, module=%s, mva_start=0x%x, mva_end=0x%x\n",
                         i, m4u_get_module_name(pWrapDes[i].eModuleID),
	...
	M4U_POW_ON_TRY(eModuleID);
	/* write registers */
	M4U_WriteReg32(m4u_base, REG_MMU_WRAP_SA(WrapRangeID - m4u_index_offset),
		       MVAStart & (~0xfff));
	M4U_WriteReg32(m4u_base, REG_MMU_WRAP_EA(WrapRangeID - m4u_index_offset),
		       MVAEnd & (~0xfff));
			      ...
			      M4U_PORT_ID_ENUM portID, unsigned int MVAStart, unsigned int MVAEnd)
{
	...
	if (!mva_owner_match(eModuleID, mva2module(MVAStart))) {
		m4u_aee_print("invalid wrap fail: larb=%d,module=%s\n",
			      m4u_module_2_larb(eModuleID), m4u_get_module_name(eModuleID));
		M4UMSG
		    ("m4u_invalid_wrapped_range module=%s, MVAStart=0x%x is %s, MVAEnd=0x%x is %s\n",
		     m4u_get_module_name(eModuleID), MVAStart,
		     m4u_get_module_name(mva2module(MVAStart)), MVAEnd,
		     m4u_get_module_name(mva2module(MVAEnd)));
		m4u_mvaGraph_dump();
	...
	if (FreeWrapRegs[m4u_index] < g4M4UWrapCount[m4u_index]) {
		for (i = m4u_index_offset; i < m4u_index_offset + g4M4UWrapCount[m4u_index]; i++) {
			if (pWrapDes[i].Enabled == 1 &&
			    pWrapDes[i].MVAStart >= MVAStart && pWrapDes[i].MVAEnd <= MVAEnd) {
...
static int smi_reg_backup_sec(void)
{
	...
	if (ret != TZ_RESULT_SUCCESS) {
		pr_info("m4u reg backup sec ServiceCall error %d\n", ret);
		return -1;
	}
...
static int smi_reg_restore_sec(void)
{
	...
	if (ret != TZ_RESULT_SUCCESS) {
		pr_info("m4u reg backup ServiceCall error %d\n", ret);
		return -1;
	}

	return 0;
}
#else				/* #if M4U_TEE_WHOLE_ENABLE */

typedef struct _M4U_ISR_MNG_ {
	KREE_SESSION_HANDLE mem_session;
	uint32_t u4ShareMemVa;
	KREE_SHAREDMEM_HANDLE hShareMemHandle;
} M4U_ISR_MNG;

static int m4u_dump_main_tlb_des(int m4u_id, M4U_ISR_INFO *pIsrInfo)
{
	/* M4U related */
	unsigned int i = 0;
/* unsigned int m4u_base = gM4UBaseAddr[m4u_id]; */
	M4UINFO("dump main tlb=======>\n");
	for (i = 0; i < M4U_MAIN_TLB_NR; i++) {
		pr_info("%d:0x%x:0x%x  ", i, pIsrInfo->main_tags[i], pIsrInfo->main_des[i]);
		/* M4U_ReadReg32(m4u_base, REG_MMU_MAIN_TAG(i)), */
		/* m4u_get_main_descriptor(m4u_base,i)); */
...
static int m4u_dump_pfh_tlb_des(int m4u_id, M4U_ISR_INFO *pIsrInfo)
{
	/* M4U related */
	unsigned int i = 0;
/* unsigned int m4u_base = gM4UBaseAddr[m4u_id]; */
	M4UINFO("dump main tlb=======>\n");
	for (i = 0; i < M4U_PRE_TLB_NR * 4; i += 4) {
		pr_info("%d:0x%x:0x%x,0x%x,0x%x,0x%x  ", i,
			pIsrInfo->pfh_tags[i / 4],
			pIsrInfo->pfn_des[i],
			pIsrInfo->pfn_des[i + 1], pIsrInfo->pfn_des[i + 2], pIsrInfo->pfn_des[i + 3]
			/* M4U_ReadReg32(m4u_base, REG_MMU_PFH_TAG(i)),
			   m4u_get_pfh_descriptor(m4u_base,i, 0),
			   m4u_get_pfh_descriptor(m4u_base,i, 1),
			   m4u_get_pfh_descriptor(m4u_base,i, 2),
...
static int MTK_M4U_isr(M4U_ISR_INFO *pIsrInfo)
{
	...
	if (!pIsrInfo) {
		M4UMSG("M4U isr data error\n");
		return -1;
	}

	if (pIsrInfo->u4Check != M4U_CHECKSELF_VALUE) {
		M4UMSG("M4U isr data check error\n");
		return -1;
	}

	IntrSrc = pIsrInfo->IntrSrc;
	faultMva = pIsrInfo->faultMva;
	port_regval = pIsrInfo->port_regval;
	m4u_index = pIsrInfo->u4IrqM4uIndex;

	M4UDBG("MTK_M4U_isr IntrSrc 0x%x faultmva 0x%x 0x%x 0x%x\n",
	       IntrSrc, faultMva, port_regval, m4u_index);

	if (0 == IntrSrc) {
		M4UMSG("warning: MTK_M4U_isr, larbID=%d, but REG_MMU_FAULT_ST=0x0\n", m4u_index);
		/* m4u_clear_intr(m4u_base); // in tee */
		return 0;
	}
	...
	if (IntrSrc & F_INT_TRANSLATION_FAULT) {
		...
		if (g_debug_dump_rs_in_isr)
			__m4u_dump_rs_info(pIsrInfo->rs_va, pIsrInfo->rs_pa, pIsrInfo->rs_st);

	}

	if (IntrSrc & F_INT_TLB_MULTI_HIT_FAULT) {
		m4u_dump_main_tlb_des(m4u_index, pIsrInfo);
		m4u_dump_pfh_tlb_des(m4u_index, pIsrInfo);
		M4UERR("multi-hit error!\n");
	}
	if (IntrSrc & F_INT_INVALID_PHYSICAL_ADDRESS_FAULT) {
		if (!(IntrSrc & F_INT_TRANSLATION_FAULT)) {
	...
	if (IntrSrc & F_INT_ENTRY_REPLACEMENT_FAULT) {
		...
		for (i = 0; i < g4M4UTagCount[m4u_index]; i++) {
			...
			lock_cnt[mva2module(pIsrInfo->main_tags[i] & (~0xfff))]++;
		}
		for (i = 0; i < M4U_CLNTMOD_MAX; i++) {
			if (0 != lock_cnt[i]) {
				pr_info("%s(lock=%d), ", m4u_get_module_name(i), lock_cnt[i]);
	...
	/* m4u_invalid_tlb_all(m4u_index, gM4U_L2_enable); //in tee
...
static int m4u_tee_init_sharemem(M4U_ISR_MNG *pIsrMng)
{
	uint32_t i4ret = 0;
	M4U_ISR_INFO *pM4uIsrData = NULL;

	KREE_SHAREDMEM_HANDLE shm_handle;
	KREE_SHAREDMEM_PARAM shm_param;

	pM4uIsrData = (M4U_ISR_INFO *) kmalloc(sizeof(M4U_ISR_INFO), GFP_KERNEL | __GFP_ZERO);
	if (!pM4uIsrData) {
		return 0;
	}
	memset(pM4uIsrData, 0x0, sizeof(M4U_ISR_INFO));
	shm_param.buffer = pM4uIsrData;
	shm_param.size = sizeof(M4U_ISR_INFO);
	i4ret = KREE_RegisterSharedmem(pIsrMng->mem_session, &shm_handle, &shm_param);
	if (i4ret != TZ_RESULT_SUCCESS) {
		M4UMSG("m4u RegShMem Error: %s\n", TZ_GetErrorString(i4ret));
		return 0;
	}

	pIsrMng->u4ShareMemVa = (uint32_t) pM4uIsrData;
	pIsrMng->hShareMemHandle = shm_handle;

	M4ULOG("shm alloc va 0x%x handle = 0x%x\n", (uint32_t) pM4uIsrData, shm_handle);
...
static int m4u_tee_wait_isr(KREE_SHAREDMEM_HANDLE hShareMem)
{
	MTEEC_PARAM param[4];
	uint32_t paramTypes;
	TZ_RESULT ret;

	param[0].memref.handle = (uint32_t) hShareMem;
	param[0].memref.offset = 0;
	param[0].memref.size = sizeof(M4U_ISR_INFO);

	paramTypes = TZ_ParamTypes1(TZPT_MEMREF_INPUT);

	ret = KREE_TeeServiceCall(m4u_session, M4U_TZCMD_WAIT_ISR, paramTypes, param);
	if (ret != TZ_RESULT_SUCCESS) {
		M4UMSG("m4u_tee_wait_isr ServiceCall error 0x%x\n", ret);
		return -1;
	}
	return 0;
}

static int m4u_tee_isr_handle(void *pv_arg)
{
	uint32_t i4Ret = 0;
	KREE_SESSION_HANDLE mem_session;
	M4U_ISR_MNG tIsrMng = { 0 };

	i4Ret = KREE_CreateSession(TZ_TA_MEM_UUID, &mem_session);
	if (i4Ret != TZ_RESULT_SUCCESS) {
		M4UMSG("Create memory session error %d\n", i4Ret);
		return -1;
	}
	tIsrMng.mem_session = mem_session;

	M4ULOG("m4u_tee_isr_thread enter memhandle 0x%x\n", mem_session);

	i4Ret = m4u_tee_init_sharemem(&tIsrMng);
	if (0 == i4Ret) {
		i4Ret = KREE_CloseSession(mem_session);
		if (i4Ret != TZ_RESULT_SUCCESS)
			M4UMSG("Close memory session error %d\n", i4Ret);
		return 0;
	}

	while (1) {
		memset((void *)tIsrMng.u4ShareMemVa, 0x0, sizeof(M4U_ISR_INFO));

		m4u_tee_wait_isr(tIsrMng.hShareMemHandle);

		MTK_M4U_isr((M4U_ISR_INFO *) tIsrMng.u4ShareMemVa);
	}

	i4Ret = KREE_UnregisterSharedmem(tIsrMng.mem_session, tIsrMng.hShareMemHandle);
	kfree((void *)tIsrMng.u4ShareMemVa);

	if (i4Ret != TZ_RESULT_SUCCESS)
		M4UMSG("m4u UnReg Error: %s\n", TZ_GetErrorString(i4Ret));

	i4Ret = KREE_CloseSession(mem_session);
	if (i4Ret != TZ_RESULT_SUCCESS)
		M4UMSG("Close memory session error %d\n", i4Ret);

	M4UMSG("m4u_tee_isr_thread exit\n");

	return 0;
}

static struct task_struct *m4u_tee_irq_task;
static int m4u_tee_isr_thread_init(void)
{
	m4u_tee_irq_task = kthread_create(m4u_tee_isr_handle, NULL, "M4uTeeIsr");
	if (!m4u_tee_irq_task) {
		M4UMSG("create m4u thread error\n");
	}
	wake_up_process(m4u_tee_irq_task);

	return 0;
}


static int m4u_hw_init_sec(void)
{
	MTEEC_PARAM param[4];
	uint32_t paramTypes;
	TZ_RESULT ret;

	param[0].value.a = pt_pa_nonsec;	/* unsecurity pa */
	param[0].value.b = gM4U_L2_enable;	/* L2 enable */

	paramTypes = TZ_ParamTypes1(TZPT_VALUE_INPUT);

	ret = KREE_TeeServiceCall(m4u_session, M4U_TZCMD_HW_INIT, paramTypes, param);
	if (ret != TZ_RESULT_SUCCESS) {
		M4UMSG("m4u_hw_init_sec ServiceCall error 0x%x\n", ret);
		return -1;
	}
	return 0;
}

/**below function don't called in system, only for debug, don't move it right now***/
/* static void m4u_L2_prefetch(unsigned int start, unsigned int end, int lock) */
/* m4u_invalid_seq_range_by_mva */
/* m4u_L2_monitor_start */
/* m4u_L2_monitor_stop */
/* m4u_L2_enable_lock_alert */
/* m4u_L2_disable_lock_alert */
/**end function don't called in system, only for debug, don't move it right now***/
			   ...
			   unsigned int mva_start, unsigned int mva_end)
{
	...
	param[0].value.a = m4u_id;
	param[0].value.b = L2_en;
	param[1].value.a = isInvAll;
	param[1].value.b = 0;
	param[2].value.a = mva_start;
	param[2].value.b = mva_end;

	paramTypes = TZ_ParamTypes3(TZPT_VALUE_INPUT, TZPT_VALUE_INPUT, TZPT_VALUE_INPUT);

	ret = KREE_TeeServiceCall(m4u_session, M4U_TZCMD_INVALID_TLB, paramTypes, param);
	if (ret != TZ_RESULT_SUCCESS) {
		M4UMSG("m4u_invalid_tlb ServiceCall error 0x%x\n", ret);
		return -1;
	}
...
static int m4u_invalidate_and_check(unsigned int m4u_index, unsigned int start, unsigned int end)
{
	...
	param[0].value.b = 0;
	param[1].value.a = start;
	param[1].value.b = end;

	paramTypes = TZ_ParamTypes2(TZPT_VALUE_INPUT, TZPT_VALUE_INPUT);

	ret = KREE_TeeServiceCall(m4u_session, M4U_TZCMD_INVALID_CHECK, paramTypes, param);
	if (ret != TZ_RESULT_SUCCESS) {
		M4UMSG("m4u_invalid_and_check ServiceCall error 0x%x\n", ret);
		return -1;
	}
					...
					uint32_t MVAStart, uint32_t MVAEnd)
{
	...
	param[0].value.a = m4u_index;
	param[0].value.b = 0;
	param[1].value.a = RangeReg_ID;
	param[1].value.b = m4u_index_offset;
	param[2].value.a = MVAStart;
	param[2].value.b = MVAEnd;

	paramTypes = TZ_ParamTypes3(TZPT_VALUE_INPUT, TZPT_VALUE_INPUT, TZPT_VALUE_INPUT);

	ret = KREE_TeeServiceCall(m4u_session, M4U_TZCMD_INSERT_SEQ, paramTypes, param);
	if (ret != TZ_RESULT_SUCCESS) {
		M4UMSG("m4u_insert_seq_SQ ServiceCall error 0x%x\n", ret);
		return -1;
	}
...
int m4u_enable_error_hang_tee(int m4u_index, bool fgEnable)
{
	MTEEC_PARAM param[4];
	uint32_t paramTypes;
	TZ_RESULT ret;
	...
	param[0].value.a = m4u_index;
	param[0].value.b = (int)fgEnable;
	paramTypes = TZ_ParamTypes1(TZPT_VALUE_INPUT);

	ret = KREE_TeeServiceCall(m4u_session, M4U_TZCMD_ERRHANGE_EN, paramTypes, param);
	if (ret != TZ_RESULT_SUCCESS) {
		pr_info("m4u_errorhange_en ServiceCall error %d\n", ret);
		return -1;
	}
	return 0;
}
...
int m4u_dump_reg(int m4u_index)
{
	...
	ret = KREE_TeeServiceCall(m4u_session, M4U_TZCMD_DUMP_REG, paramTypes, param);
	if (ret != TZ_RESULT_SUCCESS) {
		pr_info("m4u_dump_reg ServiceCall error %d\n", ret);
		return -1;
	}
	return 0;
}
...
static int smi_reg_restore(void)
{
	...
	ret = KREE_TeeServiceCall(m4u_session, M4U_TZCMD_REG_RESTORE, paramTypes, param);
	if (ret != TZ_RESULT_SUCCESS) {
		pr_info("m4u reg restore ServiceCall error %d\n", ret);
		return -1;
	}
...
#else/* #ifndef M4U_TEE_SERVICE_ENABLE */
...
{				/* native */
	...
	M4ULOG
	    ("m4u_config_port(), port=%s (%d), Virtuality=%d, Security=%d, Distance=%d, Direction=%d\n",
	     m4u_get_port_name(pM4uPort->ePortID), pM4uPort->ePortID, pM4uPort->Virtuality,
	...
	if (pM4uPort->ePortID != M4U_PORT_GCPU)
	...
	if (pM4uPort->Distance > 16) {
		M4ULOG
		    ("m4u_config_port() error, port=%s, Virtuality=%d, Security=%d, Distance=%d, Direction=%d\n",
	...
	if (pM4uPort->ePortID != M4U_PORT_GCPU)
		...
		COM_WriteReg32(REG_SMI_SECUR_CON_G3D, sec_con_val);
...
int m4u_config_port_rotator(M4U_PORT_STRUCT_ROTATOR *pM4uPort)
{
	...
	M4ULOG
	    ("m4u_config_port_rotator(), module=%s, port=%s, Virtuality=%d, Security=%d, MVAStart=0x%x, BufAddr=0x%x, BufSize=0x%x, angle=%d\n",
	     m4u_get_module_name(eModuleID), m4u_get_port_name(pM4uPort->ePortID),
	     pM4uPort->Virtuality, pM4uPort->Security, pM4uPort->MVAStart, pM4uPort->BufAddr,
	     pM4uPort->BufSize, pM4uPort->angle);
	...
	if (pM4uPort->ePortID != M4U_PORT_VDO_ROT0_OUT0 &&
	    ...
	    pM4uPort->ePortID != M4U_PORT_TV_ROT_OUT0 && pM4uPort->ePortID != M4U_PORT_G2D_W) {
	...
	if (M4U_PORT_G2D_W == pM4uPort->ePortID) {
		if (pM4uPort->angle == ROTATE_0 ||
		    ...
		    pM4uPort->angle == ROTATE_HFLIP_90 || pM4uPort->angle == ROTATE_HFLIP_0) {
			direction = 0;
		} else if (pM4uPort->angle == ROTATE_HFLIP_270 ||
			   pM4uPort->angle == ROTATE_270 ||
			   pM4uPort->angle == ROTATE_HFLIP_180 || pM4uPort->angle == ROTATE_180) {
			direction = 1;
		} else {
	...
	if (1 == portStruct.Virtuality) {
		if (page_num % 16 == 0 || page_num % 16 > 8) {
			m4u_insert_wrapped_range(eModuleID, pM4uPort->ePortID, pM4uPort->MVAStart,
						 pM4uPort->MVAStart + pM4uPort->BufSize - 1);
		} else {
			/* lock first (page_num%16) pages */
			for (i = 0; i < page_num % 16; i++) {
				m4u_manual_insert_entry(eModuleID,
							pM4uPort->MVAStart + DEFAULT_PAGE_SIZE * i,
							true);
			}
			m4u_insert_wrapped_range(eModuleID, pM4uPort->ePortID,
						 pM4uPort->MVAStart +
						 (page_num % 16) * DEFAULT_PAGE_SIZE,
						 pM4uPort->MVAStart + pM4uPort->BufSize - 1);
		}
		...
		m4u_insert_seq_range(eModuleID, pM4uPort->MVAStart,
				     pM4uPort->MVAStart + pM4uPort->BufSize - 1,
				     RT_RANGE_HIGH_PRIORITY, entryCount);
	} else {
		if (page_num % 16 == 0 || page_num % 16 > 8) {
			m4u_invalid_wrapped_range(eModuleID, pM4uPort->ePortID, pM4uPort->MVAStart,
						  pM4uPort->MVAStart + pM4uPort->BufSize - 1);
		} else {
			m4u_invalid_wrapped_range(eModuleID, pM4uPort->ePortID,
						  pM4uPort->MVAStart +
						  (page_num % 16) * DEFAULT_PAGE_SIZE,
						  pM4uPort->MVAStart + pM4uPort->BufSize - 1);
		}
		m4u_invalid_seq_range(eModuleID, pM4uPort->MVAStart,
				      pM4uPort->MVAStart + pM4uPort->BufSize - 1);
	}
#endif
...
int m4u_monitor_stop(int m4u_id)
{
	...
	MMProfileLogEx(M4U_MMP_Events[PROFILE_MAIN_TLB_MON], MMProfileFlagEnd, (unsigned int)m4u_id,
		       cnt.main_tlb_miss_cnt);
	MMProfileLogEx(M4U_MMP_Events[PROFILE_PREF_TLB_MON], MMProfileFlagEnd, (unsigned int)m4u_id,
...
static int m4u_get_pages(M4U_MODULE_ID_ENUM eModuleID, unsigned int BufAddr, unsigned int BufSize,
			 unsigned int *pPhys)
{
	int ret, i;
	int page_num;
			...
			if ((vma->vm_flags) & VM_PFNMAP) {
				...
				if (bufEnd > vma->vm_end) {
					M4UMSG
					    ("error: page_num=%d,module=%s, va=0x%x, size=0x%x, vm_flag=0x%x\n",
					     page_num, m4u_get_module_name(eModuleID), BufAddr,
					     BufSize, vma->vm_flags);
				...
				if (ret < page_num) {
					...
					if (unlikely(fatal_signal_pending(current))) {
						M4UMSG
						    ("error:sigkill get_user_pages,Page=%d,ret=%d,module=%s,curproc:%s\n",
					...
					if (ret > 0) {
						M4UMSG
						    ("error: page_num=%d,get_user_pages return=%d,module=%s,process:%s\n",
						     page_num, ret,
						     m4u_get_module_name(eModuleID),
						     current->comm);
						M4UMSG
						    ("error hint: maybe the allocated VA size is smaller than the size configured to m4u_alloc_mva()!");
					} else {
						/* return vaule is smaller than 0,
						maybe the buffer is not exist,
						just return error to up-layer*/
						M4UMSG
						    ("error: page_num=%d, get_user_pages return=%d, module=%s, process:%s\n",
						     page_num, ret,
						     m4u_get_module_name(eModuleID),
						     current->comm);
						M4UMSG
						    ("error hint: maybe the VA is deallocated before call m4u_alloc_mva(), or no VA has be ever allocated!");
					}
					m4u_dump_maps(BufAddr);
					}
			     ...
			     unsigned int BufSize, unsigned int MVA, struct sg_table *sg_table)
{
	...
	if (BufAddr < PAGE_OFFSET && sg_table == NULL) {	/* from user space */
		...
		for (i = 0; i < page_num; i++) {
			...
			if ((start_pa & 0x02) == 0) {
				...
				if (page_count(page) > 0) {
					...
					if (m4u_put_unlock_page(page) && (page_num < 3)) {
						M4UMSG
						    ("warning: put_unlock_page fail module=%s, va=0x%x, size=0x%x,mva=0x%x (page is unlocked before put page)\n",
		...
		if (put_page_err == M4U_ERR_PAGE_UNLOCKED) {
			M4UMSG
			    ("warning: in m4u_release_page: module=%s, va=0x%x, size=0x%x,mva=0x%x (page is unlocked before put page)\n",
			     m4u_get_module_name(eModuleID), BufAddr, BufSize, MVA);
		}
		...
		if (1) {	/* screen red in debug mode */
			...
			M4UMSG
			    ("error: addr un-align, module=%s, addr=0x%x, size=0x%x, process=%s, align=0x%x\n",
			     ...
			     (unsigned int)start, size, current->comm, L1_CACHE_BYTES);
		} else {
			M4UMSG
			    ("error: addr un-align, module=%s, addr=0x%x, size=0x%x, process=%s, align=0x%x\n",
		...
		if ((unsigned int)start < PAGE_OFFSET) {	/* from user space */
			for (i = 0; i < page_num; i++, page_start += DEFAULT_PAGE_SIZE) {
				...
				if ((pPhy[i] == 0)) {	/* || (!PageMlocked(page))) */
					ret = -1;
					M4UMSG
					    ("error: cache_maint() fail, module=%s, start=0x%x, page_start=0x%x, size=%d, pPhy[i]=0x%x\n",
					     m4u_get_module_name(eModuleID), (unsigned int)start,
					     (unsigned int)page_start, size, pPhy[i]);
					/* dump_page(page); */
			...
			for (i = 0; i < page_num; i++, page_start += DEFAULT_PAGE_SIZE) {
				...
				if (ppage == NULL) {
					ret = -1;
					M4UMSG
					    ("error: ppage is 0 in cache_maint of vmalloc!, module=%s, start=0x%x, pagestart=0x%x\n",
					     m4u_get_module_name(eModuleID), (unsigned int)start,
					     page_start);
					goto out;
				}
...
static bool m4u_struct_init(void)
{
	...
	/* ======= alloc pagetable======================= */
	pPT_nonsec =
	    dma_alloc_coherent(NULL, PT_TOTAL_ENTRY_NUM * sizeof(unsigned int), &pt_pa_nonsec,
			       GFP_KERNEL);
	if (!pPT_nonsec) {
	...
	if ((pt_pa_nonsec & M4U_PAGE_TABLE_ALIGN) != 0) {
		...
		M4UMSG("dma_alloc_coherent memory not align. PageTablePA=0x%x we will try again\n",
		       pt_pa_nonsec);
		dma_free_coherent(NULL, PT_TOTAL_ENTRY_NUM * sizeof(unsigned int), pPT_nonsec,
				  pt_pa_nonsec);
		tmp =
		    (unsigned int)dma_alloc_coherent(NULL,
						     PT_TOTAL_ENTRY_NUM * sizeof(unsigned int) +
	...
	M4ULOG("dma_alloc_coherent success! pagetable_va=0x%x, pagetable_pa=0x%x.\n",
	...
	pProtectVA =
	    (unsigned int *)kmalloc(TF_PROTECT_BUFFER_SIZE * TOTAL_M4U_NUM + M4U_PROTECT_BUF_OFFSET,
...
/* gM4U_align_page_va = (unsigned int)page_address(tmp_page); */
	...
	M4ULOG("gM4U_align_page_pa is 0x%x\n", gM4U_align_page_pa);
...
/* ============================================= */
	...
	M4ULOG("bus = 0x%x whole tee %d\n", COM_ReadReg32(REG_SMI_BUS_SEL), M4U_TEE_WHOLE_ENABLE);
	...
	m4uHw_set_field_by_mask(0, REG_SMI_L1LEN, F_SMI_L1LEN_AXROUTE_G3D_EMI(1),
				F_SMI_L1LEN_AXROUTE_G3D_EMI(1));
	/* config AUDIO route to EMI (MCI in default) */
	m4uHw_set_field_by_mask(0, REG_SMI_L1LEN, F_SMI_L1LEN_AXROUTE_AUDIO_EMI(1),
...
static char *m4u_get_module_name(M4U_MODULE_ID_ENUM moduleID)
{
	...
	case M4U_CLNTMOD_AUDIO:
		return "AUDIO";
	case M4U_CLNTMOD_UNKNOWN:
		return "UNKNOWN";
	case M4U_CLNTMOD_LCDC_UI:
		return "LCDC_UI";
	case M4U_CLNTMOD_GCPU:
		return "GCPU";
	default:
...
unsigned int m4u_get_pa_by_mva(unsigned int mva)
{
	...
	pPageTableAddr =
	    (unsigned int *)((unsigned int)pPageTableVA_nonCache + M4U_GET_PTE_OFST_TO_PT_SA(mva));
...
static void m4u_print_active_port(unsigned int m4u_index)
{
	...
	M4UINFO("active ports: ");
	{
		for (i = 0; i < M4U_PORT_NR; i++) {
			...
			if (regval)
				pr_info("%s(%d), ", m4u_get_port_name(i), i);
		}
		pr_info("\n");
	}
#endif
		...
		if (pa & 0x2) {
			...
			if ((pa >= start_addr) && (pa <= end_addr)) {
				M4UMSG("pa found in pagetable: pa=0x%x, module=%s\n", pa,
				       m4u_get_module_name(mva2module(mva)));
				found = 1;
			}
		}
	}
...
static int m4u_dump_user_addr_register(M4U_PORT_ID_ENUM port)
{
	...
	case M4U_PORT_HW_VDEC_MC_EXT:
		...
		M4UINFO("0xf60223e0: 0x%x,0x%x,0x%x\n",
			COM_ReadReg32(0xf60223e0), COM_ReadReg32(0xf60223e4),
			COM_ReadReg32(0xf60223e8));
		...
		break;
	case M4U_PORT_HW_VDEC_PP_EXT:
		M4UINFO("0xf6025224: 0x%x,0x%x\n", COM_ReadReg32(0xf6025224),
			COM_ReadReg32(0xf6025228));
...
int m4u_debug_command(unsigned int command)
{
	...
	case 4:
	case 5:
		g_debug_enable_error_hang = (command == 4) ? true : false;
		M4UMSG(" g_debug_enable_error_hang %d\n", g_debug_enable_error_hang);

#if !M4U_TEE_WHOLE_ENABLE
		if (g_debug_enable_error_hang == true)
			m4u_enable_error_hang(0)
			    else
			m4u_disable_error_hang(0);
#else
		m4u_enable_error_hang_tee(0, (bool) g_debug_enable_error_hang);
#endif
		break;
	...
	case 18:
		{
			...
			orignal_va = (unsigned int)vmalloc(size + 0x104);
			va = orignal_va + 0x104;
			...
			for (i = 0; i < page_num; i++) {
				struct page *p1, *p2;
				p1 = vmalloc_to_page((void *)(va + i * 0x1000));
				p2 = vmalloc_to_page((void *)(map_va + i * 0x1000));
				if (p1 != p2)
...
int __dump_memory_range(unsigned int start, unsigned int end)
{
	...
	for (i = 0; i <= end - start; i += 4) {
		pr_info("0x%x,", *(unsigned int *)(start + i));/*violate*/
		if ((i + 4) % 32 == 0)
			pr_info("\n");
	}
	if (i % 32 != 0)
