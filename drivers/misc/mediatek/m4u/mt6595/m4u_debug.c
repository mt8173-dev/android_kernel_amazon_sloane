...
int m4u_test_reclaim(unsigned int size)
{
	...
	for (i = 0; i < 10; i++) {
		...
		ret =
		    m4u_alloc_mva(client, M4U_PORT_DISP_OVL0, va[i], NULL, buf_size,
				  M4U_PROT_READ | M4U_PROT_CACHE, 0, &mva);
		if (ret) {
...
__attribute__ ((weak))
extern int ddp_mem_test(void);
__attribute__ ((weak))
extern int __ddp_mem_test(unsigned int *pSrc, unsigned char *pSrcPa,
			  unsigned int *pDst, unsigned char *pDstPa, int need_sync);
...
int m4u_test_ddp(unsigned int prot)
{
	...
	m4u_alloc_mva(client, M4U_PORT_DISP_OVL0, (unsigned int)pSrc, NULL, size, prot, 0, &src_pa);

	m4u_alloc_mva(client, M4U_PORT_DISP_OVL0, (unsigned int)pDst, NULL, size, prot, 0, &dst_pa);
