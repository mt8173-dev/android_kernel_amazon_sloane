...
static unsigned int gLarbBaseAddr[] =
	...
	for (i = 0; i < SEQ_NR_PER_M4U_SLAVE; i++) {
		if (1 == pSeq[i].Enabled) {
			if (MVAEnd < pSeq[i].MVAStart || MVAStart > pSeq[i].MVAEnd) {	/*no overlap */
				...
				M4ULOG_MID
				    ("warning: insert tlb range overlapped with prev ranges, cur process=%s!\n",
				     current->comm);
	...
	for (port = 0; port < M4U_PORT_NR; port++) {
		if (port_array->ports[port] & M4U_PORT_ATTR_EN) {
			...
			ret |= _m4u_config_port(port,
						!!(port_array->
						    ports[port] & M4U_PORT_ATTR_VIRTUAL),
...
void m4u_print_port_status(struct seq_file *seq, int only_print_active)
{
	...
	for (port = 0; port < gM4u_port_num; port++) {
		...
		if (m4u_index == 0) {
			...
			mmu_en =
			    m4uHw_get_field_by_mask(0, REG_PERIAXI_BUS_CTL3,
...
int m4u_enable_prefetch(M4U_PORT_ID PortID)
{
	...
	m4uHw_set_field_by_mask(m4u_base, REG_MMU_CTRL_REG, F_MMU_CTRL_PFH_DIS(1),
...
{
	unsigned int m4u_base = gM4UBaseAddr[m4u_id];
	m4uHw_set_field_by_mask(m4u_base, REG_MMU_CTRL_REG, F_MMU_CTRL_INT_HANG_en(1),
...
//=============================================*/
	...
	for (i = 0; i < TOTAL_M4U_NUM; i++) {
		...
		M4U_WriteReg32(gM4UBaseAddr[i], REG_MMUg_PT_BASE_SEC,
		...
		if (!gM4U_4G_DRAM_Mode)
			M4U_WriteReg32(gM4UBaseAddr[i], REG_MMU_IVRP_PADDR,
				       F_MMU_IVRP_PA_SET((unsigned int)ProtectPA));
		else
			M4U_WriteReg32(gM4UBaseAddr[i], REG_MMU_IVRP_PADDR,
				       F_MMU_IVRP_4G_DRAM_PA_SET((unsigned int)ProtectPA));
