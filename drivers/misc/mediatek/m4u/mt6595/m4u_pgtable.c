...
{
	COM_WriteReg32((unsigned int)&(imu_pgd_val(*pgd)), val);
}
	...
	if (info->valid) {
		if (info->size == SZ_4K) {
			M4U_PRINT_LOG_OR_SEQ(data,
					     "va(0x%x)-->pgd(0x%x)-->pte(0x%x)-->pa(0x%x) small\n",
					     info->mva, imu_pgd_val(*info->pgd),
					     imu_pte_val(*info->pte), info->pa);
		} else if (info->size == SZ_64K) {
			M4U_PRINT_LOG_OR_SEQ(data,
					     "va(0x%x)-->pgd(0x%x)-->pte(0x%x)-->pa(0x%x) large\n",
					     info->mva, imu_pgd_val(*info->pgd),
					     imu_pte_val(*info->pte), info->pa);
		} else if (info->size == SZ_1M) {
			M4U_PRINT_LOG_OR_SEQ(data, "va(0x%x)-->pgd(0x%x)-->pa(0x%x) section\n",
					     info->mva, imu_pgd_val(*info->pgd), info->pa);
		} else if (info->size == SZ_16M) {
			M4U_PRINT_LOG_OR_SEQ(data, "va(0x%x)-->pgd(0x%x)-->pa(0x%x) super\n",
					     info->mva, imu_pgd_val(*info->pgd), info->pa);
		}
	} else {
...
int m4u_alloc_pte(m4u_domain_t *domain, imu_pgd_t *pgd, unsigned int pgprot)
{
	...
	if (unlikely(pte_new & (IMU_BYTES_PER_PTE - 1))) {
		m4u_aee_print("%s: fail, not algin pa=0x%x, va=0x%x\n", __func__, pte_new,
			      pte_new_va);
		/*kfree(pte_new_va); */
	...
	if (likely(!imu_pgd_val(*pgd))) {
		...
		/*kfree(__va(pte_new)); */
		M4ULOG_LOW("m4u pte alocated by others: pgd=0x%p\n", pgd);
		kmem_cache_free(gM4u_pte_kmem, (void *)pte_new_va);
		return 1;
	}
}
...
************************************************************/
int m4u_map_16M(m4u_domain_t *m4u_domain, unsigned int mva, unsigned int pa, unsigned int prot)
{
	int i;
	imu_pgd_t *pgd;
	unsigned int pgprot;
	...
	if ((mva & (~F_PGD_PA_SUPERSECTION_MSK)) != (pa & (~F_PGD_PA_SUPERSECTION_MSK))) {
		m4u_aee_print("error to mk_pte: mva=0x%x, pa=0x%x, type=%s\n", mva, pa,
			      "supersection");
		return -EINVAL;
	}
	...
	for (i = 0; i < 16; i++) {
		if (unlikely(imu_pgd_val(*pgd))) {
			m4u_aee_print("%s: pgd=0x%x, i=%d\n", __func__, imu_pgd_val(*pgd), i);
			goto err_out;
		}
		m4u_set_pgd_val(pgd, pa | pgprot);
		pgd++;
	}
...
int m4u_map_1M(m4u_domain_t *m4u_domain, unsigned int mva, unsigned int pa, unsigned int prot)
{
	imu_pgd_t *pgd;
	unsigned int pgprot;
	...
	pa &= F_PGD_PA_SECTION_MSK;

	pgprot = __m4u_get_pgd_attr_1M(prot);
	pgd = imu_pgd_offset(m4u_domain, mva);

	if (unlikely(imu_pgd_val(*pgd))) {
		m4u_aee_print("%s: pgd=0x%x\n", __func__, imu_pgd_val(*pgd));
		return -1;
	}
...
int m4u_map_64K(m4u_domain_t *m4u_domain, unsigned int mva, unsigned int pa, unsigned int prot)
{
	int ret, i;
	imu_pgd_t *pgd;
	imu_pte_t *pte;
	...
	if (!imu_pgd_val(*pgd)) {
		...
		if (unlikely((imu_pgd_val(*pgd) & (~F_PGD_PA_PAGETABLE_MSK)) != pgprot)) {
			m4u_aee_print("%s: fail pgd=0x%x, pgprot=0x%x\n", __func__,
...
int m4u_map_4K(m4u_domain_t *m4u_domain, unsigned int mva, unsigned int pa, unsigned int prot)
{
	int ret, pte_new;
	imu_pgd_t *pgd;
	imu_pte_t *pte;
...
static inline int m4u_map_phys_align(m4u_domain_t *m4u_domain, unsigned int iova,
				     unsigned int paddr, unsigned int size, unsigned int prot)
{
	int ret;
		       ...
		       unsigned int paddr, unsigned int size, unsigned int prot)
{
	...
	if (!IS_ALIGNED(iova | paddr | size, min_pagesz)) {
		M4UMSG("unaligned: iova 0x%lx pa 0x%lx size 0x%lx min_pagesz "
		       "0x%x\n", iova, (unsigned long)paddr, (unsigned long)size, min_pagesz);
		return -EINVAL;
	}
	...
	while (size) {
		...
		pr_debug("mapping: iova 0x%x pa 0x%x pgsize %lu\n", iova, paddr, pgsize);
		    ...
		    struct sg_table *sg_table, unsigned int size, unsigned int prot)
{
	...
	for_each_sg(sg_table->sgl, sg, sg_table->nents, i) {
		...
		if (map_mva + len > map_end) {
			M4UMSG("%s: map_mva(%d)+len(%d)>end(%d)\n", __func__, map_mva, len,
			       map_end);
			break;
		}
