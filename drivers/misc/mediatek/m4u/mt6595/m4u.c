...
static int m4u_fill_sgtable_user(struct vm_area_struct *vma, unsigned int va, int page_num,
				 struct scatterlist **pSg, int has_page)
{
	...
	for (i = 0; i < page_num; i++) {
		...
		if (!pa || !sg) {
			M4UMSG
			    ("%s: fail va=0x%x,page_num=0x%x,fail_va=0x%x,pa=0x%llx,sg=0x%x,i=%d\n",
		  ...
		  unsigned int size, unsigned int prot, unsigned int flags, unsigned int *pMva)
{
	...
	if (va) {
		...
		if (IS_ERR_OR_NULL(sg_table)) {
			M4UMSG
			    ("m4u_alloc_mva fail, cannot create sg: larb=%d,module=%s,va=0x%x,sg=0x%x,size=%d,prot=0x%x,flags=0x%x\n",
			     m4u_port_2_larb_id(port), m4u_get_port_name(port), va,
			     (unsigned int)sg_table, size, prot, flags);
			return (int)sg_table;
		}
	}
	...
	*pMva = mva;
	...
	M4ULOG_HIGH
	    ("m4u_alloc_mva: larb=%d,module=%s,va=0x%x,sg=0x%x,size=%d,prot=0x%x,flags=0x%x,mva=0x%x\n",
	     m4u_port_2_larb_id(port), m4u_get_port_name(port), va, (unsigned int)sg_table, size,
	     prot, flags, mva);
...
int m4u_dealloc_mva(m4u_client_t *client, M4U_PORT_ID port, unsigned int mva)
{
	...
	if (unlikely(!pMvaInfo)) {
		...
		m4u_dump_buf_info(NULL);
		MMProfileLogEx(M4U_MMP_Events[M4U_MMP_DEALLOC_MVA], MMProfileFlagEnd, 0x5a5a5a5a,
			       mva);
		return -EINVAL;
	}
	...
	if (is_err) {
		m4u_aee_print("%s fail: port=%s, mva=0x%x, size=0x%x, sg=0x%x\n", __func__,
			      ...
			      (unsigned int)pMvaInfo->sg_table);
		ret = -EINVAL;
	} else
		ret = 0;
...
static int __m4u_cache_sync_user(unsigned int start, size_t size, M4U_CACHE_SYNC_ENUM sync_type)
{
	...
	map_start = start;
	while (map_start < end) {
		map_end = min((unsigned int)((map_start & (~M4U_PAGE_MASK)) + PAGE_SIZE), end);
		   ...
		   M4U_CACHE_SYNC_ENUM sync_type)
{
	...
	if (sync_type < M4U_CACHE_CLEAN_ALL) {
		...
		/*some user may sync mva from other client
		   (eg. ovl may not know who allocated this buffer, but he need to sync cache).
		...
		if (!pMvaInfo) {
			M4UMSG("cache sync fail, cannot find buf: mva=0x%x, client=0x%x\n", mva,
			       (unsigned int)client);
			m4u_dump_buf_info(NULL);
			MMProfileLogEx(M4U_MMP_Events[M4U_MMP_CACHE_SYNC], MMProfileFlagStart, 0,
				       0);
			return -1;
		}
		...
		if ((pMvaInfo->size != size) || (pMvaInfo->va != va)) {
			M4UMSG
			    ("cache_sync fail: expect mva=0x%x,size=0x%x,va=0x%x, but mva=0x%x,size=0x%x,va=0x%x\n",
			     pMvaInfo->mva, pMvaInfo->size, pMvaInfo->va, mva, size, va);
			MMProfileLogEx(M4U_MMP_Events[M4U_MMP_CACHE_SYNC], MMProfileFlagStart,
...
************************************************************/
int m4u_mva_map_kernel(unsigned int mva, unsigned int size, unsigned int *map_va,
