...
static int _mjc_WaitEvent(MJC_EVENT_T *a_prParam)
{
	...
	if (0 ==
	    wait_event_interruptible_timeout(*prWaitQueue,
					     *((unsigned char *)a_prParam->pvFlag)
	...
	case MJC_LOCKHW:
		{
			...
			if (sizeof(MJC_IOCTL_LOCK_HW_T) != rLockHW.u4StructSize) {
				MJCMSG
				    ("[ERROR] mjc_ioctl() MJC_LOCKHW context size mismatch (user:%d, kernel:%d)\n",
	...
	case MJC_WAITISR:
		{
			...
			if (sizeof(MJC_IOCTL_ISR_T) != rIsr.u4StructSize) {
				MJCMSG
				    ("[ERROR] mjc_ioctl() MJC_WAITISR context size mismatch (user:%d, kernel:%d)\n",
	...
	case MJC_READ_REG:
		{
			...
			if (rReadReg.reg > (MJC_REG_PA + MJC_REG_REGION)
			    || rReadReg.reg < MJC_REG_PA) {
				MJCMSG
				    ("[ERROR] mjc_ioctl() MJC_READ_REG, addr invalid, addr min=0x%x, max=0x%x, addr=0x%x\n",
				     MJC_REG_PA, (MJC_REG_PA + MJC_REG_REGION), rReadReg.reg);
				return -1;
			}
	...
	case MJC_WRITE_REG:
		{
			...
			if (rWriteReg.reg > (MJC_REG_PA + MJC_REG_REGION)
			    || rWriteReg.reg < MJC_REG_PA) {
				MJCMSG
				    ("[ERROR] mjc_ioctl() MJC_WRITE_REG, addr invalid, addr min=0x%x, max=0x%x, addr=0x%x\n",
				     MJC_REG_PA, (MJC_REG_PA + MJC_REG_REGION), rWriteReg.reg);
				return -1;
			}
			...
			*(volatile unsigned int *)i4Add =
			    (*(volatile unsigned int *)i4Add & ~rWriteReg.
			     mask) | (rWriteReg.val & rWriteReg.mask);
		}
		break;
	...
	case MJC_WRITE_REG_TBL:
		{
			...
			for (cnt = 0; cnt < MJC_FORCE_REG_NUM; cnt++) {
				...
				*(volatile unsigned int *)i4Add =
				    (*(volatile unsigned int *)i4Add & ~gfWriteReg[cnt].mask) |
...
static int mjc_mmap(struct file *pFile, struct vm_area_struct *pVma)
{
	...
	MJCMSG("start = 0x%x, pgoff= 0x%x vm_end= 0x%x, vm_page_prot= 0x%x\n",
	       ...
	       (unsigned int)pVma->vm_end, (unsigned int)pVma->vm_page_prot);
	...
	if ((u4Length > MJC_REG_REGION) || (u4Addr < MJC_REG_PA)
	    || (u4Addr > MJC_REG_PA + MJC_REG_REGION)) {
...
static int mjc_probe(struct platform_device *pDev)
{
	...
	/* Gary todo: */
	if (request_irq
	    (MJC_TOP_IRQ_BIT_ID, (irq_handler_t) mjc_intr_dlr, IRQF_TRIGGER_LOW, MJC_DEVNAME,
	     NULL) < 0) {
