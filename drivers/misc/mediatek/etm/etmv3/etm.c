 ...
 * @return value written to the register
 */
unsigned int etm_writel(const struct etm_trace_context_t *ctx, int n, unsigned int v, int x)
{
...
void trace_start_by_cpus(const struct cpumask *mask, int init_etb)
{
	...
	if (init_etb) {
		...
		mt65xx_reg_sync_writel(AHB_EN, (u32 *)AHBAP_EN);
	...
	if (init_etb) {
		/* Avoid DBG_sys being reset */
		mt65xx_reg_sync_writel(DEM_UNLOCK_MAGIC, (u32 *)DEM_UNLOCK);
		mt65xx_reg_sync_writel(POWER_ON_RESET, (u32 *)DBGRST_ALL);
		mt65xx_reg_sync_writel(BUSCLK_EN, (u32 *)DBGBUSCLK_EN);
		mt65xx_reg_sync_writel(SYSCLK_EN, (u32 *)DBGSYSCLK_EN);

		etb_lock(&tracer);
	}
}

static inline ssize_t run_show(struct device *kobj, struct device_attribute *attr, char *buf)
{
...
static ssize_t etm_cr_show(struct device *kobj, struct device_attribute *attr, char *buf)
{
...
static ssize_t etm_ccer_show(struct device *kobj, struct device_attribute *attr, char *buf)
{
...
static int etm_probe(struct platform_device *pdev)
{
	...
	new_regs = krealloc(tracer.etm_regs, sizeof(tracer.etm_regs[0]) * new_count, GFP_KERNEL);
	if (!new_regs) {
	...
	new_info = krealloc(tracer.etm_info, sizeof(tracer.etm_info[0]) * new_count, GFP_KERNEL);
	if (!new_info) {
