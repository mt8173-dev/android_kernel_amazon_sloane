...
{
	const kal_uint16 dummy_pixel =
	    OV2722MIPI_sensor.line_length - OV2722MIPI_PV_PERIOD_PIXEL_NUMS;
	const kal_uint16 pv_min_fps =
	    enable ? OV2722MIPI_sensor.night_fps : OV2722MIPI_sensor.normal_fps;
	...
	if (!OV2722MIPI_sensor.video_mode)
		return;
	max_exposure_lines =
	    OV2722MIPI_sensor.pclk * OV2722MIPI_FPS(1) / (pv_min_fps *
							  OV2722MIPI_sensor.line_length);
...
static void OV2722MIPI_camera_para_to_sensor(void)
{
	...
	for (i = 0; 0xFFFFFFFF != OV2722MIPI_sensor.eng.reg[i].Addr; i++) {
		OV2722MIPI_write_cmos_sensor(OV2722MIPI_sensor.eng.reg[i].Addr,
					     OV2722MIPI_sensor.eng.reg[i].Para);
	}
	for (i = OV2722MIPI_FACTORY_START_ADDR; 0xFFFFFFFF != OV2722MIPI_sensor.eng.reg[i].Addr;
	     i++) {
		OV2722MIPI_write_cmos_sensor(OV2722MIPI_sensor.eng.reg[i].Addr,
					     OV2722MIPI_sensor.eng.reg[i].Para);
	}
...
/* ------------------------ Engineer mode ------------------------ */
static void OV2722MIPI_get_sensor_group_count(kal_int32 *sensor_count_ptr)
{
...
static void OV2722MIPI_get_sensor_group_info(MSDK_SENSOR_GROUP_INFO_STRUCT *para)
{
...
static void OV2722MIPI_get_sensor_item_info(MSDK_SENSOR_ITEM_INFO_STRUCT *para)
{

	static kal_char * cct_item_name[] = { "SENSOR_BASEGAIN", "Pregain-R", "Pregain-Gr", "Pregain-Gb", "Pregain-B" };
	static kal_char * editer_item_name[] = { "REG addr", "REG value" };
	...
	case OV2722MIPI_PRE_GAIN:
		...
		sprintf(para->ItemNamePtr,
			cct_item_name[para->ItemIdx - OV2722MIPI_SENSOR_BASEGAIN]);
...
static kal_bool OV2722MIPI_set_sensor_item_info(MSDK_SENSOR_ITEM_INFO_STRUCT *para)
{
	...
	case OV2722MIPI_PRE_GAIN:
		...
		case OV2722MIPI_PRE_GAIN_B_INDEX:
			spin_lock(&ov2722mipiraw_drv_lock);
			OV2722MIPI_sensor.eng.cct[para->ItemIdx].Para =
...
static void OV2722MIPI_init(void)
{
...
UINT32 OV2722MIPIOpen(void)
{
	...
	if (OV2722MIPI_SENSOR_ID != OV2722MIPI_sensor_id) {
		id = ((OV2722MIPI_read_cmos_sensor(0x300A) << 8) |
		      OV2722MIPI_read_cmos_sensor(0x300B));
		spin_lock(&ov2722mipiraw_drv_lock);
...
{
	int retry = 3;
	pr_debug("OV2722 Get sensor id start!\n");

	/* check if sensor ID correct */
	do {
		*sensorID =
		    ((OV2722MIPI_read_cmos_sensor(0x300A) << 8) |
		     OV2722MIPI_read_cmos_sensor(0x300B));
		spin_lock(&ov2722mipiraw_drv_lock);
			 ...
			 MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	kal_uint16 dummy_pixel = 0, cap_fps;
#endif
	kal_uint16 startx = 0, starty = 0, dummy_line = 0;
	...
	dummy_pixel =
	    OV2722MIPI_sensor.pclk * OV2722MIPI_FPS(1) /
	    ((OV2722MIPI_FULL_PERIOD_LINE_NUMS + OV2722MIPI_DEFAULT_DUMMY_LINES) * cap_fps);
	dummy_pixel =
	    dummy_pixel <
	    (OV2722MIPI_FULL_PERIOD_PIXEL_NUMS +
	     OV2722MIPI_DEFAULT_DUMMY_PIXELS) ? 0 : dummy_pixel -
	    (OV2722MIPI_FULL_PERIOD_PIXEL_NUMS + OV2722MIPI_DEFAULT_DUMMY_PIXELS);
#endif
			 ...
			 MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
	...
	pr_debug("OV2722MIPIGetInfo£¬FeatureId:%d\n", ScenarioId);
...
UINT32 OV2722MIPIControl(MSDK_SCENARIO_ID_ENUM ScenarioId,
...
UINT32 OV2722MIPISetVideoMode(UINT16 u2FrameRate)
{
	...
	if (u2FrameRate == 0)
		return TRUE;
	dummy_line =
	    OV2722MIPI_sensor.pclk / u2FrameRate / (OV2722MIPI_PV_PERIOD_PIXEL_NUMS +
						    OV2722MIPI_DEFAULT_DUMMY_PIXELS) -
	    (OV2722MIPI_PV_PERIOD_LINE_NUMS + OV2722MIPI_DEFAULT_DUMMY_LINES);
	if (dummy_line < 0)
		dummy_line = 0;
				...
				UINT8 *pFeaturePara, UINT32 *pFeatureParaLen)
{
	...
	case SENSOR_FEATURE_GET_CCT_REGISTER:	/* 12 */
		if (*pFeatureParaLen >= sizeof(OV2722MIPI_sensor.eng.cct) + sizeof(kal_uint32)) {
			*((kal_uint32 *) pFeaturePara++) = sizeof(OV2722MIPI_sensor.eng.cct);
			memcpy(pFeaturePara, &OV2722MIPI_sensor.eng.cct,
			       sizeof(OV2722MIPI_sensor.eng.cct));
		}
		break;
	...
	case SENSOR_FEATURE_GET_ENG_REGISTER:	/* 14 */
		if (*pFeatureParaLen >= sizeof(OV2722MIPI_sensor.eng.reg) + sizeof(kal_uint32)) {
			*((kal_uint32 *) pFeaturePara++) = sizeof(OV2722MIPI_sensor.eng.reg);
			memcpy(pFeaturePara, &OV2722MIPI_sensor.eng.reg,
			       sizeof(OV2722MIPI_sensor.eng.reg));
		}
	case SENSOR_FEATURE_GET_REGISTER_DEFAULT:
		...
		memcpy(((PNVRAM_SENSOR_DATA_STRUCT) pFeaturePara)->SensorEngReg,
		       &OV2722MIPI_sensor.eng.reg, sizeof(OV2722MIPI_sensor.eng.reg));
		memcpy(((PNVRAM_SENSOR_DATA_STRUCT) pFeaturePara)->SensorCCTReg,
		       &OV2722MIPI_sensor.eng.cct, sizeof(OV2722MIPI_sensor.eng.cct));
		*pFeatureParaLen = sizeof(NVRAM_SENSOR_DATA_STRUCT);
		break;
	case SENSOR_FEATURE_GET_CONFIG_PARA:
		memcpy(pFeaturePara, &OV2722MIPI_sensor.cfg_data,
		       sizeof(OV2722MIPI_sensor.cfg_data));
	...
	case SENSOR_FEATURE_GET_ENG_INFO:
		memcpy(pFeaturePara, &OV2722MIPI_sensor.eng_info,
		       sizeof(OV2722MIPI_sensor.eng_info));
