...
#if 0
static void S5K4ECGX_write_cmos_sensor_wID(kal_uint32 addr, kal_uint32 para, kal_uint32 id)
{
...
static void S5K4ECGX_write_cmos_sensor(kal_uint32 addr, kal_uint32 para)
{
	char puSendCmd[4] = { (char)(addr >> 8), (char)(addr & 0xFF),
		(char)(para >> 8), (char)(para & 0xFF)
	};
...
static void S5K4ECGX_table_write_cmos_sensor(const kal_uint16 *para, kal_uint32 len)
{
...
static void S5K4ECGX_MIPI_AWB_UnLock(void)
{
	...
	S5K4ECGX_write_cmos_sensor(0x002A, 0x2C66);
	S5K4ECGX_write_cmos_sensor(0x0F12, 0x0001);
	if (!((SCENE_MODE_OFF == S5K4ECGX_Driver.sceneMode)
	      || (SCENE_MODE_NORMAL == S5K4ECGX_Driver.sceneMode)
	      || (SCENE_MODE_HDR == S5K4ECGX_Driver.sceneMode))) {
...
S5K4ECGX_AAA_STATUS_ENUM S5K4ECGX_MIPI_AF_Stop(void)
{
	...
	/* //Lock AE */
	if ((!S5K4ECGX_Driver.userAskAeLock) && (S5K4ECGX_AF_MODE_SINGLE == S5K4ECGX_Driver.afMode)) {
		spin_unlock(&s5k4ecgx_mipi_drv_lock);
...
static void S5K4ECGX_MIPI_FlashTriggerCheck(uintptr_t pFeatureReturnPara32)
{
	unsigned int NormBr;
...
unsigned int jpegParserParseImage(unsigned char *srcBuf, unsigned int bufSize,
				  unsigned int *eoiOffset)
{
	...
	while (rdPtr < endPtr) {
		if (JPEG_MARKER_START_CODE == rdPtr[0]) {
			if (JPEG_MARKER_EOI == rdPtr[1]) {
				rdPtr += 2;
				SENSORDB
				    ("[4EC] jpegParserParseImage B:Encounter FFD9: rdPtr=0x%x\n",
				     rdPtr);
				break;
			}
			rdPtr += 1;
			...
			if ((offsetOfEncounter100Zeros == 0)
				...
				if (!(memcmp(array100Zeros, rdPtr, 100))) {
					SENSORDB
					    ("[4EC] jpegParserParseImage:C offsetOfEncounter100Zeros = 1: rdPtr=0x%x\n",
					     rdPtr);
...
#define OUTPUT_FMT  5		/* YUV */
	...
	/* ================================================================================== */
	{
		...
		S5K4ECGX_table_write_cmos_sensor(addr_data_pair, sizeof(addr_data_pair) / sizeof(addr_data_pair[0]));
	}
...
static void S5K4ECGX_Init_Setting(void)
{
	...
	S5K4ECGX_write_cmos_sensor(0x002A, 0x0722);
	/* #skl_OTP_usWaitTime  // This register should be positioned in fornt of D0001000. // Check */
	S5K4ECGX_write_cmos_sensor(0x0F12, 0x0100);
	S5K4ECGX_write_cmos_sensor(0x002A, 0x0726);
	/* skl_bUseOTPfunc OTP shading is used,this reg should be 1  // */
	S5K4ECGX_write_cmos_sensor(0x0F12, 0x0000);
	S5K4ECGX_write_cmos_sensor(0x002A, 0x08D6);
	/* #ash_bUseOTPData // If OTP for shading is used, this register should be enable. ( default : disable) */
	S5K4ECGX_write_cmos_sensor(0x0F12, 0x0000);
	S5K4ECGX_write_cmos_sensor(0x002A, 0x146E);
...
void S5K4ECGX_MIPI_Capture(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
			     MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
 ...
 */
void S5K4ECGX_MIPI_AF_Infinity(void)
{
...
static void S5K4ECGX_MIPI_AE_Get_Status(UINT32 *pFeatureReturnPara32)
{
	...
	/* AE disable */
	if ((kal_bool)false == aeEnable) {
		*pFeatureReturnPara32 = SENSOR_AE_IDLE;
	} else if ((kal_bool)true == S5K4ECGX_Driver.userAskAeLock) {
...
static void S5K4ECGX_MIPI_AWB_Get_Status(UINT32 *pFeatureReturnPara32)
{
	...
	/* AWB disable */
	if ((kal_bool)false == awbEnable) {
		*pFeatureReturnPara32 = SENSOR_AWB_IDLE;
	} else if ((kal_bool)true == S5K4ECGX_Driver.userAskAwbLock) {
...
BOOL S5K4ECGX_MIPI_set_param_exposure_for_HDR(UINT16 para)
{
	...
	exposureTime =
	    (exposureTime >
	     S5K4ECGX_MIPI_MAX_EXPOSURE_TIME) ? S5K4ECGX_MIPI_MAX_EXPOSURE_TIME : exposureTime;
...
void S5K4ECGX_MIPI_get_AEAWB_lock(uintptr_t pAElockRet32, uintptr_t pAWBlockRet32)
{
	pAElockRet32 = 1;
	pAWBlockRet32 = 1;
	SENSORDB("[4EC]GetAEAWBLock,AE=%ld ,AWB=%ld\n,", pAElockRet32, pAWBlockRet32);
}
...
void S5K4ECGX_MIPI_set_scene_mode(UINT16 para)
{
	...
	if (((SCENE_MODE_PORTRAIT == prevSceneMode) &&
	    ...
	    ((SCENE_MODE_LANDSCAPE == prevSceneMode) && (SCENE_MODE_LANDSCAPE != para))) {
	...
	if (((SCENE_MODE_NIGHTSCENE == prevSceneMode) && (SCENE_MODE_NIGHTSCENE != para))) {
		S5K4ECGX_write_cmos_sensor(0x002A, 0x06B8);
		S5K4ECGX_write_cmos_sensor(0x0F12, 0x452C);
	...
	if (((SCENE_MODE_NIGHTSCENE == prevSceneMode) &&
	    ...
	    ((SCENE_MODE_SPORTS == prevSceneMode) && (SCENE_MODE_SPORTS != para))) {
		S5K4ECGX_MIPI_AE_ExpCurveGain_Config(para);
	} else if ((SCENE_MODE_NIGHTSCENE == para) || (SCENE_MODE_SPORTS == para)) {
		S5K4ECGX_MIPI_AE_ExpCurveGain_Config(para);
	}
...
UINT32 S5K4ECGX_MIPI_FeatureControl(MSDK_SENSOR_FEATURE_ENUM FeatureId, UINT8 *pFeaturePara,
				    UINT32 *pFeatureParaLen)
{
	...
	case SENSOR_FEATURE_GET_TRIGGER_FLASHLIGHT_INFO:
		S5K4ECGX_MIPI_FlashTriggerCheck((uintptr_t)pFeatureData);
	...
	case SENSOR_FEATURE_GET_DEFAULT_FRAME_RATE_BY_SCENARIO:
		...
		S5K4ECGX_MIPI_GetDefaultFramerateByScenario((MSDK_SCENARIO_ID_ENUM)*pFeatureData,
							    (MUINT32 *) (uintptr_t) (*(pFeatureData + 1)));
		break;
	...
	case SENSOR_FEATURE_GET_AE_AWB_LOCK_INFO:
		...
		S5K4ECGX_MIPI_get_AEAWB_lock((uintptr_t) (*pFeatureData),
