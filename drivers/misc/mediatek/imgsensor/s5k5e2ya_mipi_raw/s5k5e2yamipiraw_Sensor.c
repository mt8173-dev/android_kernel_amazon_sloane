...
#define LOG_2 \
	LOG_INF("preview 1280*960@30fps,864Mbps/lane; video 1280*960@30fps; capture 5M@30fps\n")
...
static imgsensor_info_struct imgsensor_info = {
	...
	.pre = {
		...
		.framelength = 2025,	/* record different mode's framelength */
...
{2576, 1936, 8, 8, 2560, 1920, 2560, 1920, 0000, 0000, 2560, 1920, 0, 0, 2560, 1920},	/* Preview */
...
static void write_shutter(kal_uint16 shutter)
{
	...
	if (main_sensor_init_setting_switch == 0) {
		...
		write_cmos_sensor(0x0340, 0x08); /* frame_length_lines : def. 1962d */
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x0307, 0xC3); /* PLLM (195 * (24/5) = 936) */
	write_cmos_sensor(0x3C1F, 0x00); /* PLLS  */

	write_cmos_sensor(0x0820, 0x03); /* requested link bit rate mbps : (936Mbps) */
	write_cmos_sensor(0x0821, 0xA8);
	write_cmos_sensor(0x3C1C, 0x59); /* dbr_div */

	/* MIPI setting for MCLK 24MHz */
	write_cmos_sensor(0x30BC, 0x38);
	write_cmos_sensor(0x30BD, 0x40);
	write_cmos_sensor(0x3110, 0x70);
	write_cmos_sensor(0x3111, 0x80);
	write_cmos_sensor(0x3112, 0x7B);
	write_cmos_sensor(0x3113, 0xC0);
	write_cmos_sensor(0x30C7, 0x1A);

	/*  Size Setting */
	write_cmos_sensor(0x0340, 0x08); /* frame_length_lines : def. 2112d (30fps) */
	write_cmos_sensor(0x0341, 0x40);
	write_cmos_sensor(0x0342, 0x0B); /* line_length_pck : def. 2950d */
	...
	/* Integration time */
	write_cmos_sensor(0x0202, 0x07); /* coarse integration (EIT=32ms) */
	...
	/* Analog Timing Tuning S5K5E2_HVT STD_Rev1.0 */
	...
	write_cmos_sensor(0x300E, 0x79);	/*  ct_write_cmos_sensor(0x3_width */
	...
	/* CDS */
	write_cmos_sensor(0x3099, 0x0E);  /*  cds_option ([3]:crs switch disable */
	...
	write_cmos_sensor(0x3063, 0x27); /*  ADC_SAT  */
	...
	write_cmos_sensor(0x3300, 0x0D); /* BPC Off,  0C for DPC ON, 0D for DPC OFF */
	...
	/* OB Level */
	write_cmos_sensor(0x3232, 0x10);
	write_cmos_sensor(0x3410, 0x10);
	write_cmos_sensor(0x3C11, 0x10);
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	UINT32 **pp_feature_data_32 = (UINT32 **)feature_para;
	...
	case SENSOR_FEATURE_GET_CROP_INFO:
		...
		wininfo = (SENSOR_WINSIZE_INFO_STRUCT *) (*(pp_feature_data_32 + 1));
	...
	case SENSOR_FEATURE_GET_FACTORY_MODE:
		*feature_return_para_32 = main_sensor_init_setting_switch;
		*feature_para_len = 4;
		break;
	default:
		break;
