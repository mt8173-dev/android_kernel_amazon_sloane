...
kal_uint32 OV2659_shutter_check(kal_uint32 shutter)
{
	if (OV2659Sensor.IsPVmode)
		shutter =
		    (shutter > OV2659_MAX_SHUTTER_PREVIEW) ? OV2659_MAX_SHUTTER_PREVIEW : shutter;
	else
		shutter =
		    (shutter > OV2659_MAX_SHUTTER_CAPTIRE) ? OV2659_MAX_SHUTTER_CAPTIRE : shutter;
...
static void OV2659InitialSetting(void)
{
	...
	OV2659_write_cmos_sensor(0x0100, 0x00);	/* OV2659_write_cmos_sensor(0x0100,0x01);        // 95: Houston 20140227 Should NOT streaming out before calling Control */
	...
	OV2659Sensor.PreviewPclk = OV2659_PREVIEW_PCLK;
	OV2659Sensor.CapturePclk = OV2659_CAPTURE_PCLK;
...
UINT32 OV2659Open(void)
{
	...
	OV2659_GetSensorID((kal_uint32 *) &sensor_id);

	if (sensor_id != OV2659_SENSOR_ID) {
		OV2659SENSORDB("[OV2659Open] Get ID Failed ... sensor id = 0x%x\n", sensor_id);
		return ERROR_SENSOR_CONNECT_FAIL;
	}
...
UINT32 OV2659GetResolution(MSDK_SENSOR_RESOLUTION_INFO_STRUCT *pSensorResolution)
{
	...
	OV2659SENSORDB("[OV2659GetResolution] Prv (%d x %d), Full (%d x %d), Vid (%d x %d)\n",
		       pSensorResolution->SensorPreviewWidth,
		       pSensorResolution->SensorPreviewHeight, pSensorResolution->SensorFullWidth,
		       pSensorResolution->SensorFullHeight, pSensorResolution->SensorVideoWidth,
...
UINT32 OV2659SetMaxFramerateByScenario(MSDK_SCENARIO_ID_ENUM scenarioId, MUINT32 frameRate)
{
	...
	case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
		...
		dummyLine = frameHeight - OV2659_PV_PERIOD_LINE_NUMS;
		OV2659SENSORDB
		    ("[OV2659SetMaxFramerateByScenario][video]framerate=%d, dummy_line=%d\n",
		     frameRate, dummyLine);
		OV2659SetDummy(0, dummyLine);
		break;
	case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
	case MSDK_SCENARIO_ID_CAMERA_ZSD:
		pclk = OV2659_CAPTURE_PCLK / 10;
		lineLength = OV2659_FULL_PERIOD_PIXEL_NUMS;
		frameHeight = (10 * pclk) / frameRate / lineLength;
		dummyLine = frameHeight - OV2659_FULL_PERIOD_LINE_NUMS;
		OV2659SENSORDB
		    ("[OV2659SetMaxFramerateByScenario][capture/zsd]framerate=%d, dummy_line=%d\n",
		     frameRate, dummyLine);
		OV2659SetDummy(0, dummyLine);
		break;
	default:
			    ...
			    UINT8 *pFeaturePara, UINT32 *pFeatureParaLen)
{
	...
	case SENSOR_FEATURE_GET_PERIOD:
		*pFeatureReturnPara16++ =
		    OV2659_PV_PERIOD_PIXEL_NUMS + OV2659Sensor.PreviewDummyPixels;
	...
	case SENSOR_FEATURE_GET_CONFIG_PARA:
		memcpy(pSensorConfigData, &OV2659SensorConfigData,
	...
	case SENSOR_FEATURE_SET_MAX_FRAME_RATE_BY_SCENARIO:
		OV2659SetMaxFramerateByScenario((MSDK_SCENARIO_ID_ENUM) *pFeatureData32,
						*(pFeatureData32 + 1));
		break;
	case SENSOR_FEATURE_GET_DEFAULT_FRAME_RATE_BY_SCENARIO:
		OV2659GetDefaultFramerateByScenario((MSDK_SCENARIO_ID_ENUM) *pFeatureData32,
