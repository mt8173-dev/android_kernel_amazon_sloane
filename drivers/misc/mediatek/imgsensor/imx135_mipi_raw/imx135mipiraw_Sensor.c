...
#define LOG_WRN(format, args...)    pr_err(PFX "[%s]" format, __func__, ##args)
#define LOG_ERR(format, args...)    pr_err(PFX "[%s] ERROR" format, __func__, ##args)
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if (gain < BASEGAIN || gain > 32 * BASEGAIN) {
		LOG_WRN("Error gain setting: gain: %d\n", gain);
...
static void capture_setting(kal_uint16 currefps)	/* IMX135MIPI_set_13M */
{
	...
	if (currefps == 300)	/* default 30.0 fps */
	{
		...
		mdelay(35);	/* After SW standby to data streaming on: 1.5ms + exposure time (33ms) */
	} else {
		write_cmos_sensor(0x011E, 0x18);
		write_cmos_sensor(0x011F, 0x00);
		write_cmos_sensor(0x0301, 0x05);
		...
		mdelay(43);	/* After SW standby to data streaming on: 1.5ms + exposure time (41ms) */
	}
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			...
			LOG_WRN("Read sensor id fail, id: 0x%x\n", imgsensor.i2c_write_id);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
...
static kal_uint32 open(void)
{
	...
	if (imgsensor_info.sensor_id != sensor_id) {
		LOG_ERR("Read sensor ID fail: sensor read: 0x%x , expected: 0x%x\n", sensor_id,
						...
						MUINT32 framerate)
{
	...
	default:		/* coding with  preview scenario by default */
		...
		spin_unlock(&imgsensor_drv_lock);
		/*set_dummy();*/
		LOG_WRN("error scenario_id = %d, we use preview scenario\n", scenario_id);
		break;
	}
	return ERROR_NONE;
}
