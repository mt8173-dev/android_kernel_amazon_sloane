...
#define SENSOR_WR32(addr, data)    mt65xx_reg_sync_writel(data, (void *)(addr))
#define SENSOR_RD32(addr)          ioread32((void *)(addr))
/******************************************************************************
 * Debug configuration
******************************************************************************/
...
#else
static void KD_IMGSENSOR_PROFILE_INIT(void)
{
}

static void KD_IMGSENSOR_PROFILE(char *tag)
{
}
#endif
		...
		char sensorNameStr[KDIMGSENSOR_MAX_INVOKE_DRIVERS][32], BOOL On, char *mode_name)
{
	...
	for (i = KDIMGSENSOR_INVOKE_DRIVER_0; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS; i++) {
		if (g_bEnableDriver[i]) {
			PK_XLOG_INFO("[%s][%d][%d][%s][%s]\r\n", __func__, g_bEnableDriver[i],
				     socketIdx[i], sensorNameStr[i], mode_name);
...
********************************************************************************/
static int adopt_CAMERA_HW_Open(void)
{
	UINT32 err = 0;
	...
	if (g_pSensorFunc) {
		...
		if (ERROR_NONE != err) {
			PK_DBG
			    (" ERROR:SensorOpen(), turn off power, Mclk1(0x%x), ISPClk(0x%x), MclkPll(0x%x)\n",
...
********************************************************************************/
static int adopt_CAMERA_HW_CheckIsAlive(void)
{
	UINT32 err = 0;
	UINT32 err1 = 0;
	UINT32 i = 0;
...
********************************************************************************/
static int adopt_CAMERA_HW_GetResolution(void *pBuf)
{
	/* ToDo: remove print */
...
static int adopt_CAMERA_HW_GetInfo2(void *pBuf)
{
	...
	PK_DBG("[CAMERA_HW][Resolution] %x\n", (unsigned int)pSensorGetInfo->pSensorResolution);
...
********************************************************************************/
static int adopt_CAMERA_HW_Control(void *pBuf)
{
	int ret = 0;
...
********************************************************************************/
static int adopt_CAMERA_HW_Close(void)
{
	/* if (atomic_read(&g_CamHWOpend) == 0) { */
	/* return 0; */
	/* } */
...
static int kdSetSensorMclk(int *pBuf)
{
	int ret = 0;
...
static int kdSetSensorGpio(int *pBuf)
{
	int ret = 0;
	unsigned int temp = 0;
			    ...
			    unsigned int a_u4Command, unsigned long a_u4Param)
{
	...
	default:
		PK_DBG("No such command %x %x\n", a_u4Command, KDIMGSENSORIOC_X_SET_GPIO);
		i4RetValue = -EPERM;
		break;
...
* RegisterCAMERA_HWCharDrv
********************************************************************************/
static int RegisterCAMERA_HWCharDrv(void)
{
...
********************************************************************************/
/*static void UnregisterCAMERA_HWCharDrv(void)
{*/
	/* Release char driver */
...
* RegisterCAMERA_HWCharDrv
********************************************************************************/
static int RegisterCAMERA_HWCharDrv2(void)
{
	struct device *sensor_device = NULL;
	UINT32 major;
...
/*static void UnregisterCAMERA_HWCharDrv2(void)
{*/
	/* Release char driver */
...
/*******************************************************************************
  ...
  * Used to dump some critical sensor register
  ********************************************************************************/
static int CAMERA_HW_DumpReg_To_Proc(struct file *file, char *buffer, size_t count, loff_t *offp)
{
	return count;
}

static int CAMERA_HW_DumpReg_To_Proc2(struct file *file, char *buffer, size_t count, loff_t *offp)
{
	return count;
}

/*static int CAMERA_HW_DumpReg_To_Proc3(struct file *file, char *buffer, size_t count,
					 loff_t *offp)
{
	return count;
}*/
