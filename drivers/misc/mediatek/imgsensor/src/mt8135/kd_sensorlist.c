...
#else
#define PK_DBG pr_debug
/* #define PK_ERR(a...) */
#define PK_XLOG_INFO  pr_debug

#endif

#define PK_ERR(fmt, arg...)         printk(PFX fmt, ##arg)
...
int iBurstWriteReg(u8 *pData, u32 bytes, u16 i2cId)
{
	...
	if (gI2CBusNum == SUPPORT_I2C_BUS_NUM1) {
		...
		spin_lock(&kdsensor_drv_lock);
		g_pstI2Cclient->addr = g_pstI2Cclient->addr >> 1;
		spin_unlock(&kdsensor_drv_lock);

		ret = 0;
		retry = 3;
		do {
			ret = i2c_master_send(g_pstI2Cclient, pData, bytes);
			retry--;
			if (ret != bytes) {
		...
		spin_lock(&kdsensor_drv_lock);
		g_pstI2Cclient2->addr =
			g_pstI2Cclient2->addr >> 1;
		spin_unlock(&kdsensor_drv_lock);

		ret = 0;
		retry = 3;
		do {
			ret = i2c_master_send(g_pstI2Cclient2, pData, bytes);
			retry--;
			if (ret != bytes) {
				PK_DBG("Error sent I2C ret = %d\n", ret);
			}
...
MUINT32 kd_MultiSensorOpen(void)
{
	...
	for (i = (KDIMGSENSOR_MAX_INVOKE_DRIVERS - 1); i >= KDIMGSENSOR_INVOKE_DRIVER_0; i--) {
		if (g_bEnableDriver[i] && g_pInvokeSensorFunc[i]) {
			...
			pr_debug("kd_MultiSensorOpen - Power On\n");
		...
		if (g_bEnableDriver[i] && g_pInvokeSensorFunc[i]) {
			...
			if (InvokeCamera == g_invokeSocketIdx[i]) {
				if (DUAL_CAMERA_MAIN_2_SENSOR == g_invokeSocketIdx[i]) {
					...
					spin_unlock(&kdsensor_drv_lock);
					PK_XLOG_INFO
					    ("kd_MultiSensorFeatureControl: switch I2C BUS2\n");
				} else {
					spin_lock(&kdsensor_drv_lock);
					gI2CBusNum = SUPPORT_I2C_BUS_NUM1;
					spin_unlock(&kdsensor_drv_lock);
					PK_XLOG_INFO
					    ("kd_MultiSensorFeatureControl: switch I2C BUS1\n");
				}
...
/*  */
MUINT32
kd_MultiSensorControl(MSDK_SCENARIO_ID_ENUM ScenarioId,
	...
	if ((atomic_read(&g_CamHWOpend) == 0) && (atomic_read(&g_CamHWOpend2) == 0)) {
		...
		if (g_pSensorFunc) {
			...
			if (ERROR_NONE != err) {
				...
				pr_debug("adopt_CAMERA_HW_Open - turn off power\n");

				kdModulePowerOn((CAMERA_DUAL_CAMERA_SENSOR_ENUM *)
						g_invokeSocketIdx, g_invokeSensorNameStr, false,
						CAMERA_HW_DRVNAME1);
			}
		} else {
...
static inline int adopt_CAMERA_HW_CheckIsAlive(void)
{
	...
	pr_debug("adopt_CAMERA_HW_CheckIsAlive\n");


	KD_IMGSENSOR_PROFILE_INIT();
	/* power on sensor */
	err = kdModulePowerOn((CAMERA_DUAL_CAMERA_SENSOR_ENUM *) g_invokeSocketIdx, g_invokeSensorNameStr,
			true, CAMERA_HW_DRVNAME1);
	/* Bypass redundant search operation of getting sensor ID, if power on failed */
	if (ERROR_NONE != err) {
		PK_DBG("%s\n", err == -ENODEV ? "No device in this socket position" : "kdModulePowerOn failed");
		return err;
	}
	/* wait for power stable */
	mDELAY(10);
	...
	if (g_pSensorFunc) {
		for (i = KDIMGSENSOR_INVOKE_DRIVER_0; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS; i++) {
			if (DUAL_CAMERA_NONE_SENSOR != g_invokeSocketIdx[i]) {
				...
				PK_XLOG_INFO("adopt_CAMERA_HW_CheckIsAlive sensorId = 0x%x",
					     sensorID);
				if (sensorID == 0) {	/* not implement this feature ID */
					...
					/* ToDo: remove print log */
					PK_XLOG_INFO(" Sensor found ID = 0x%x\n", sensorID);
					PK_DBG(" Sensor found ID = 0x%x\n", sensorID);
					err = ERROR_NONE;
				}
				if (ERROR_NONE != err) {
					PK_DBG
					    ("ERROR:adopt_CAMERA_HW_CheckIsAlive(), No imgsensor alive %d\n",
					     err);
				}
			}
		}
	...
	pr_debug("adopt_CAMERA_HW_CheckIsAlive - Power off\n");
	...
	err1 = g_pSensorFunc ? g_pSensorFunc->SensorClose() : 0;
	if (ERROR_NONE != err1) {
		PK_DBG("SensorClose\n");
	}
	/*  */
...
static inline int adopt_CAMERA_HW_Control(void *pBuf)
{
	...
	if (g_pSensorFunc) {
		ret =
		    g_pSensorFunc->SensorControl(pSensorCtrl->ScenarioId, &imageWindow,
						 &sensorConfigData);
	} else {
		PK_DBG("[CAMERA_HW]ERROR:NULL g_pSensorFunc\n");
	}
...
static inline int adopt_CAMERA_HW_FeatureControl(void *pBuf)
{
	...
	if (SENSOR_FEATURE_SINGLE_FOCUS_MODE == pFeatureCtrl->FeatureId || SENSOR_FEATURE_CANCEL_AF == pFeatureCtrl->FeatureId || SENSOR_FEATURE_CONSTANT_AF == pFeatureCtrl->FeatureId) {	/* YUV AF_init and AF_constent and AF_single has no params */
	} else {
	...
	case SENSOR_FEATURE_GET_FACTORY_MODE:
		/* do nothing */
	...
	if (g_pSensorFunc) {
		...
		PK_DBG("[CAMERA_HW]INFO SensorFeatureControl ret=%d\n", ret);
	} else {
		PK_DBG("[CAMERA_HW]ERROR:NULL g_pSensorFunc\n");
	}
...
{
	pr_debug("adopt_CAMERA_HW_Close\n");
	...
	if ((atomic_read(&g_CamHWOpend) == 0) && (atomic_read(&g_CamHWOpend2) == 0)) {
		...
		/* power off sensor */
		pr_debug("adopt_CAMERA_HW_Close - Power Off\n");
...
static ssize_t CAMERA_Temperature_Read(struct file *file, char __user *pPage, size_t count, loff_t *ppos)
{
	MUINT32 sensortemp = 0;
	char buffer[10] = {0};
	ssize_t bytes = count < (10-(*ppos)) ? count : (10-(*ppos));

	if (g_pSensorFunc != NULL) {
		g_pSensorFunc->SensorFeatureControl(DUAL_CAMERA_MAIN_SENSOR,
							SENSOR_FEATURE_GET_SENSOR_CURRENT_TEMPERATURE,
							(MUINT8 *) &sensortemp,
							(MUINT32 *)sizeof(MUINT32));

		PK_DBG("[camera_temp_show] sensor temperature : %d degrees\n", sensortemp);
	}

	sprintf(buffer, "%d\n", sensortemp);

	if (copy_to_user(pPage, buffer , sizeof(char)*10))
		return -EFAULT;
...
static const struct file_operations g_stCAMERA_camsensor2_fops = {
	.owner = THIS_MODULE,
	.read = CAMERA_HW_DumpReg_To_Proc,
	.write = CAMERA_HW_Reg_Debug2
};

static const struct file_operations g_stCAMERA_camsensor3_fops = {
	.owner = THIS_MODULE,
	.read = CAMERA_HW_DumpReg_To_Proc,
	.write = CAMERA_HW_Reg_Debug3
};

static const struct file_operations g_stCAMERA_temperature_fops = {
	.owner      = THIS_MODULE,
	.read       = CAMERA_Temperature_Read
};
...
static int __init CAMERA_HW_i2C_init(void)
{
	...
	prEntry = proc_create("driver/camsensor2", 0, NULL, &g_stCAMERA_camsensor2_fops);
	if (!prEntry) {
		PK_ERR("add /proc/driver/camsensor2 entry fail\n");
	}
	/* Register proc file for sub sensor register debug */
	prEntry = proc_create("driver/camsensor3", 0, NULL, &g_stCAMERA_camsensor3_fops);
	if (!prEntry) {
		PK_ERR("add /proc/driver/camsensor entry fail\n");
	}
	/* Register proc file for Main sensor temperature query */
	prEntry = proc_create("driver/camsensor_temperature", 0, NULL, &g_stCAMERA_temperature_fops);
	if (!prEntry)
		PK_ERR("add /proc/driver/camsensor_temperature entry fail\n");
