...
int md32_dump_regs(char *buf)
{
	...
	for (reg = (volatile unsigned int *)MD32_BASE;
	     (unsigned int)reg < (MD32_BASE + 0x90); reg++) {
		if (!((unsigned int)reg & 0xF)) {
			ptr += sprintf(ptr, "\n");
			ptr += sprintf(ptr, "[0x%08x]   ", (unsigned int)reg);
		}
...
void md32_reboot_from_irq(struct work_struct *ws)
{
	...
	/* Temp mask wait for AEE porting */
...
void md32_irq_init(void)
{
	...
	if (request_irq
	    (MT_IPC_MD2HOST_INTR, md32_irq_handler, IRQF_TRIGGER_HIGH, "MD32 IPC_MD2HOST", NULL))
