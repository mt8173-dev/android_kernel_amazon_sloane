...
UINT32 md32_ocd_read_reg(enum md32_reg_idx r)
{
	...
	if (r == r14) {
		...
		/* DW(INST,0x067000e0 "sw r14,#0(r0)");
		   IW(EXECUTE); DW(ADDR, 0x00000000);
		   IW(DMb_LOAD); DR(DATA);
		   load r14
		 */
		...
		/* DW(INST,0x067000f0 "sw r15,#0(r0)");
		   IW(EXECUTE); DW(ADDR, 0x00000000); IW(DMb_LOAD); DR(DATA); */
		load r15 u32 data, pm_data;
...
int md32_ocd_execute_cmd(MD32_OCD_CMD_CFG *in_cfg)
{
	...
	case CMD_MD32_OCD_WRITE_REG:
		...
		/* ??? need to translate "lw r1,#tmp_addr(r0)" as m32 instrunction */
		md32_ocd_iw(DBG_EXECUTE_INSTR);
		break;
#endif
...
void md32_jtag_dvt_setup_gpio(u32 jtag_setup)
{
	...
	case 1:
		...
		mt_set_gpio_mode(GPIO6, 5);
		mt_set_gpio_pull_enable(GPIO6, 1);
		mt_set_gpio_mode(GPIO7, 5);
		mt_set_gpio_mode(GPIO8, 5);
		mt_set_gpio_dir(GPIO8, 1);
...
static ssize_t md32_jtag_gpio_dvt_show(struct device *kobj, struct device_attribute *attr,
				       char *buf)
{
	char *ptr = buf;
	u32 mode;
	...
	for (i = start; i <= end; i++) {
		...
		ptr +=
		    sprintf(ptr,
			    "gpio%03d mode=%d, dir=%d, pull_en=%d, ies=%d, inv=%d, out=%d in=%d\n",
			    gpio, mode, mt_get_gpio_dir(gpio), mt_get_gpio_pull_enable(gpio),
			    mt_get_gpio_ies(gpio), mt_get_gpio_inversion(gpio),
			    mt_get_gpio_out(gpio), mt_get_gpio_in(gpio));
	}
...
static ssize_t md32_jtag_gpio_dvt_store(struct device *kobj, struct device_attribute *attr,
...
static const char *const jtag_mode_str[] = {
	/* 0x0 */ "io",
	/* 0x1 */ "ap",
	/* 0x2 */ "mfg",
	/* 0x3 */ "tdd",
...
void md32_jtag_setup_gpio(u32 jtag_mode)
{
	if (jtag_mode < JTAG_GPIO_MODE_TOTAL) {
		pr_debug("change the JTAG pins to %s(%d)\n", jtag_mode_str[jtag_mode], jtag_mode);
...
static ssize_t md32_jtag_switch_show(struct device *kobj, struct device_attribute *attr, char *buf)
{
	...
	ptr += sprintf(ptr, "current jtag mode: ");
	...
	/* show usage */
	ptr += sprintf(ptr, "jtag switch usage:\n" "echo [mode] > md32_jtag_switch\n" "[mode]\n");
...
static ssize_t md32_jtag_switch_store(struct device *kobj, struct device_attribute *attr,
				      const char *buf, size_t n)
{
	int mode;
