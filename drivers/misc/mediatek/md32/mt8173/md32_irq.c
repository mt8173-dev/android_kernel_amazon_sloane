...
int md32_dump_regs(char *buf)
{
	...
	/*dump all md32 regs */
	for (reg = (volatile unsigned long *)MD32_BASE;
	     (unsigned long)reg < (unsigned long)(MD32_BASE + 0x90); reg++) {
		if (!((unsigned long)reg & 0xF)) {
			ptr += sprintf(ptr, "\n");
			ptr += sprintf(ptr, "[0x%016lx]   ", (unsigned long)reg);
		}
...
#if 0
	if (request_irq
	    (md32req->irq, md32_irq_handler, IRQF_TRIGGER_HIGH, "MD32 IPC_MD2HOST", NULL))
