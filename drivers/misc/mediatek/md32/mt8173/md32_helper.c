...
unsigned char g_md32_log_buf[512+1];
...
void logger_ipi_handler(int id, void *data, unsigned int len)
{
	...
	pr_crit("[MD32] md32_log_buf_addr = %lx\n", md32_log_buf_addr);
	pr_crit("[MD32] md32_log_start_idx_addr = %lx\n", md32_log_start_idx_addr);
	pr_crit("[MD32] md32_log_end_idx_addr = %lx\n", md32_log_end_idx_addr);
	pr_crit("[MD32] md32_log_lock_addr = %lx\n", md32_log_lock_addr);
	pr_crit("[MD32] md32_log_buf_len_addr = %lx\n", md32_log_buf_len_addr);
	pr_crit("[MD32] enable_md32_mobile_log_addr = %lx\n", enable_md32_mobile_log_addr);
}
...
int md32_tcm_swap(int group_id)
{
	...
	unsigned long app_program_start_addr;
	unsigned long app_program_end_addr;
	unsigned int app_program_sz;
	/* unsigned char *p_buf; */
	/* int p_sz; */

	unsigned long app_data_start_addr;
	unsigned long app_data_end_addr;
	unsigned int app_data_sz;
	/* unsigned char *d_buf; */
	/* int d_sz; */

	unsigned long swap_out_data_start_addr;
	unsigned long swap_out_data_end_addr;
...
void swap_ipi_handler(int id, void *data, unsigned int len)
{
	...
	if (info_type == INFO_GROUP) {
		...
		for (i = 0; i < g_group_info.group_num; i++) {
			...
			pr_debug("[swap_ipi_handler] group %d, app_data_start_addr = 0x%lx\n", i,
				 app_data_start_addr);
			pr_debug("[swap_ipi_handler] group %d, app_data_stop_addr = 0x%lx\n", i,
				 app_data_end_addr);
		}

		pr_debug("info_type = %d\n", g_group_info.info_type);
		pr_debug("group_num = %d\n", g_group_info.group_num);
		pr_debug("app_ptr_d = 0x%lx\n", g_group_info.app_ptr_d);
		pr_debug("app_ptr_p = 0x%lx\n", g_group_info.app_ptr_p);
		pr_debug("swap_lock_addr = 0x%lx\n", g_group_info.swap_lock_addr);
...
int reboot_load_md32(void)
{
	...
	ret = load_md32((const char *)MD32_DATA_IMAGE_PATH, (unsigned long)md32_data_image);
	if (ret < 0) {
	...
	ret = load_md32((const char *)MD32_PROGRAM_IMAGE_PATH, (unsigned long)md32_program_image);
	if (ret < 0) {
			       ...
			       size_t n)
{
	...
	if (sw_rstn == 0x0) {
		pr_info("[MD32] Enable MD32\n");
		if (reboot_load_md32() < 0) {
			pr_err("[MD32] Enable MD32 failed\n");
			return -EINVAL;
		}
	} else {
		pr_info("[MD32] MD32 is enabled\n");
	}
...
int md32_dt_init(void)
{
	...
	if (!node) {
		pr_err("Can't find node: mediatek,SCP_SRAM\n");
		return -1;
	}

	mreg->sram = of_iomap(node, 0);
	pr_info("md32 sram base=0x%016lx\n", (unsigned long)mreg->sram);


	if (!mreg->sram) {
		pr_err("Unable to ioremap mregisters\n");
		return -1;
	}

	node = of_find_compatible_node(NULL, NULL, "mediatek,SCP_CFGREG");
	if (!node)
		pr_err("Can't find node: mediatek,SCP_CFGRE\n");

	mreg->cfg = of_iomap(node, 0);
	mreg->irq = irq_of_parse_and_map(node, 0);

	pr_info("md32 irq=%d, cfgreg=0x%016lx\n", mreg->irq, (unsigned long)mreg->cfg);

	if (!mreg->cfg) {
		pr_err("Unable to ioremap registers\n");
		return -1;
	}
...
static int __init md32_init(void)
{
	...
	if (request_irq(md32reg.irq, md32_irq_handler, IRQF_TRIGGER_HIGH, "MD32 IPC_MD2HOST", NULL))
		pr_err(KERN_WARNING "[%s] MD32 request irq failed %d\n", __func__, ret);
	return ret;
}
