...
void md32_ipi_handler(void)
{
	if (ipi_desc[md32_rcv_obj->id].handler)
		ipi_desc[md32_rcv_obj->id].handler(md32_rcv_obj->id, md32_rcv_obj->share_buf,
						   md32_rcv_obj->len);
	MD32_TO_SPM_REG = 0x0;
}
...
ipi_status md32_ipi_send(ipi_id id, void *buf, unsigned int len, unsigned int wait)
{
	...
	if (sw_rstn == 0x0) {
		pr_info("[MD32] md32_ipi_send: MD32 not enabled\n");
		return ERROR;
	}
	if (id < MD32_NR_IPI) {
