...
static int Accdet_PMIC_IMM_GetOneChannelValue(int deCount)
{
	...
	ACCDET_DEBUG("ACCDET read Voltage: 0x%x mv!!\n\r", vol_val);
	return vol_val;
}
...
static void inline disable_accdet(void)
{
	...
	pmic_pwrap_write(ACCDET_STATE_SWCTRL,
			 pmic_pwrap_read(ACCDET_STATE_SWCTRL) & (~ACCDET_SWCTRL_EN));
	pmic_pwrap_write(ACCDET_CTRL, pmic_pwrap_read(ACCDET_CTRL) & (~(ACCDET_ENABLE)));
#endif

	/* disable clock and Analog control */
#if defined ACCDET_EINT || defined ACCDET_EINT_IRQ
	pmic_pwrap_write(ACCDET_ADC_REG, pmic_pwrap_read(ACCDET_ADC_REG) & (~(7 << 8)));
#endif
...
static inline int accdet_setup_eint(void)
{
	...
	ACCDET_DEBUG
	    ("[Accdet]accdet set EINT finished, accdet_eint_num=%d, accdet_eint_debounce_en=%d, accdet_eint_polarity=%d\n",
...
static inline void accdet_init(void)
{
   ...
   /*********************ACCDET Analog Setting***********************************************************/
	pmic_pwrap_write(ACCDET_ADC_REG, pmic_pwrap_read(ACCDET_ADC_REG) | (ACCDET_MIC_VOL << 8));	/* 0x27F0 */
	...
	pmic_pwrap_write(ACCDET_STATE_SWCTRL,
			 pmic_pwrap_read(ACCDET_STATE_SWCTRL) & (~ACCDET_SWCTRL_EN));
	pmic_pwrap_write(ACCDET_STATE_SWCTRL,
			 pmic_pwrap_read(ACCDET_STATE_SWCTRL) | ACCDET_EINT_PWM_EN);
#else
	/* enable ACCDET unit */
   ...
   /**************************************AUXADC enable auto sample****************************************/
	pmic_pwrap_write(ACCDET_AUXADC_AUTO_SPL,
