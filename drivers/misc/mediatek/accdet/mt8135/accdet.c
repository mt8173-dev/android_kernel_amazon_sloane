...
static inline void disable_accdet(void)
{
	...
	set clear accdet irq bit after disable accdet*/
...
static void disable_micbias_callback(struct work_struct *work)
{
	if (cable_type == HEADSET_NO_MIC) {
			...
			pmic_pwrap_write(ACCDET_STATE_SWCTRL, pmic_pwrap_read(ACCDET_STATE_SWCTRL)
...
static inline int accdet_setup_eint(void)
{
	...
	ACCDET_DEBUG("[Accdet]accdet set EINT finished, accdet_eint_num=%d, accdet_eint_debounce_en=%d,
		accdet_eint_polarity=%d\n", CUST_EINT_ACCDET_NUM, CUST_EINT_ACCDET_DEBOUNCE_EN, CUST_EINT_ACCDET_TYPE);
...
#define PIN_ADC_CHANNEL 5
	...
	case STAND_BY:
			if (current_status == 3) {
		 #ifdef ACCDET_MULTI_KEY_FEATURE
						ACCDET_DEBUG("[Accdet]accdet do not send plug out event in
							stand by!\n");
			 #else
						mutex_lock(&accdet_eint_irq_sync_mutex);
					 if (1 == eint_accdet_sync_flag) {
