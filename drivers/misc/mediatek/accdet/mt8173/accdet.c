...
struct headset_mode_data *cust_headset_data_hal = NULL;
...
static int g_accdet_working_in_suspend;
#endif
...
static irqreturn_t accdet_eint_func(int irq, void *arg)
{
	int ret = 0;
	if (cur_eint_state == EINT_PIN_PLUG_IN) {
		/*
		 ...
		 */
		if (cust_headset_data_hal->cust_eint_accdet_type == CUST_EINTF_TRIGGER_HIGH)
			irq_set_irq_type(irq_num, IRQF_TRIGGER_HIGH);
		else
			irq_set_irq_type(irq_num, IRQF_TRIGGER_LOW);

#ifdef ACCDET_SHORT_PLUGOUT_DEBOUNCE
		mt_eint_set_hw_debounce(cust_headset_data_hal->cust_eint_accdet_num,
						cust_headset_data_hal->cust_eint_accdet_debounce_cn);
#endif
...
/* #endif */
		...
		/* update the eint status */
#ifdef ACCDET_SHORT_PLUGOUT_DEBOUNCE
		mt_eint_set_hw_debounce(cust_headset_data_hal->cust_eint_accdet_num, ACCDET_SHORT_PLUGOUT_DEBOUNCE_CN);
#endif
...
static inline int accdet_setup_eint(void)
{
	...
	ret = request_irq(irq_num, accdet_eint_func, IRQF_ONESHOT, "accdet", NULL);
	if (ret) {
		ACCDET_DEBUG("[Accdet]request irq fail\n");
		return -EINVAL;
	}
...
static int dump_register(void)
{
	...
	ACCDET_DEBUG(" 0x0732 =%x\n", pmic_pwrap_read(0x0732));	/* clock register in 6320 */
	ACCDET_DEBUG(" MULTIKEY_ADC_CHANNEL(%x) is %x\n", MULTIKEY_ADC_CHANNEL,
		     PMIC_IMM_GetOneChannelValue(MULTIKEY_ADC_CHANNEL, 1, 1));
...
int mt_accdet_probe(void)
{
	...
	cust_headset_data_hal = get_cust_headset_data();
	...
	if (g_accdet_first == 1) {
		...
		if (node) {
			...
			ACCDET_DEBUG("Accdet irq_num is %d\n", irq_num);
			if (irq_num <= 0) {
				ACCDET_DEBUG("Accdet request irq number fail\n");
				return -EINVAL;
			 }
		} else {
			ACCDET_DEBUG("Accdet find compatible node fail\n");
			return -EINVAL;
		}
		accdet_setup_eint();
