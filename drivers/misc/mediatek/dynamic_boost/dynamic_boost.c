...
static int dboost_dvfs_hotplug_thread(void *ptr)
{
	...
	while (!kthread_should_stop()) {
		...
		interactiveplus_boost_cpu(max_freq);
		l = snprintf(cmd, 128, "cpu_min_num_big=%d\n cpu_min_num_little=%d", cores_to_set_b, cores_to_set_l);
		if (l < 128)
			mt_send_hotplug_cmd(cmd);
#else
...
fs_initcall(dynamic_boost_init);
