...
void venc_power_off(void)
{
	...
	if (gu4VencPWRCounter == 0) {
		MFV_LOGD("venc_power_off none +\n");
	} else {
		gu4VencPWRCounter--;
		MFV_LOGD("venc_power_off D+\n");
#ifdef CONFIG_OF
	    clk_disable(clk_smi);
		clk_unprepare(clk_smi);
...
void enc_isr(void)
{
	...
	if (grVcodecEncHWLock.eDriverType == VAL_DRIVER_TYPE_H264_ENC) {	/*added by bin.liu  hardwire */
		...
		MFV_LOGE("enc_isr Invalid lock holder driver type = %d\n", grVcodecEncHWLock.eDriverType);
	}

	eValRet = eVideoSetEvent(&EncIsrEvent, sizeof(VAL_EVENT_T));
	if (VAL_RESULT_NO_ERROR != eValRet) {
		MFV_LOGE("[MFV][ERROR] enc_isr ISR set EncIsrEvent error\n");
	}
	MFV_LOGE("[MFV] enc_isr ISR set EncIsrEvent done\n");
}
...
static long vcodec_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case VCODEC_SET_THREAD_ID:
		MFV_LOGE("[8173] VCODEC_SET_THREAD_ID [EMPTY] + tid = %d\n", current->pid);
	...
	case VCODEC_ALLOC_NON_CACHE_BUFFER:
		{
			MFV_LOGE("[8173][M4U]! VCODEC_ALLOC_NON_CACHE_BUFFER + tid = %d\n",
				 current->pid);
	...
	case VCODEC_FREE_NON_CACHE_BUFFER:
		{
			MFV_LOGE("[8173][M4U]! VCODEC_FREE_NON_CACHE_BUFFER + tid = %d\n",
				 current->pid);
	...
	case VCODEC_INC_DEC_EMI_USER:
		MFV_LOGD("[8173] VCODEC_INC_DEC_EMI_USER + tid = %d\n", current->pid);
	...
	case VCODEC_DEC_DEC_EMI_USER:
		MFV_LOGD("[8173] VCODEC_DEC_DEC_EMI_USER + tid = %d\n", current->pid);
	...
	case VCODEC_INC_ENC_EMI_USER:
		MFV_LOGD("[8173] VCODEC_INC_ENC_EMI_USER + tid = %d\n", current->pid);
	...
	case VCODEC_DEC_ENC_EMI_USER:
		{
			MFV_LOGD("[8173] VCODEC_DEC_ENC_EMI_USER + tid = %d\n", current->pid);
	...
	case VCODEC_LOCKHW:
		{
			MFV_LOGD("[8173] VCODEC_LOCKHW + tid = %d\n", current->pid);
			user_data_addr = (VAL_UINT8_T *) arg;
			...
			if (rHWLock.eDriverType == VAL_DRIVER_TYPE_MP4_DEC ||
				...
				while (bLockedHW == VAL_FALSE) {
					...
					if (EncHWLockEvent.u4TimeoutMs == 1) {
						MFV_LOGE("[NOT ERROR][VCODEC_LOCKHW] ENC First Use HW %d!!\n", rHWLock.eDriverType);
						FirstUseEncHW = 1;
					} else {
						FirstUseEncHW = 0;
					}
						...
						if (FirstUseEncHW != 1) {
							...
							if (grVcodecEncHWLock.pvHandle == 0) {
								...
								if (gLockTimeOutCount > 30) {
									MFV_LOGE
									    ("[VCODEC_LOCKHW] ENC - ID %d  fail, someone locked HW time out more than 30 times 0x%lx, %lx, 0x%lx, type:%d\n",
									     current->pid,
									     (VAL_ULONG_T)
									     grVcodecEncHWLock.
									     pvHandle,
				...
				MFV_LOGD("[VCODEC_LOCKHW] ENC get locked - ObjId =%d\n", current->pid);

				MFV_LOGD("[VCODEC_LOCKHW] ENC - tid = %d\n", current->pid);
			} else {
				MFV_LOGE("[VCODEC_LOCKHW] [WARNING] Unknown instance\n");
				return -EFAULT;
			}
	...
	case VCODEC_UNLOCKHW:
		{
			MFV_LOGD("[8173] VCODEC_UNLOCKHW + tid = %d\n", current->pid);
			user_data_addr = (VAL_UINT8_T *) arg;
			ret = copy_from_user(&rHWLock, user_data_addr, sizeof(VAL_HW_LOCK_T));
			if (ret) {
	...
	case VCODEC_INC_PWR_USER:
		{
			MFV_LOGD("[8173] VCODEC_INC_PWR_USER + tid = %d\n", current->pid);
			user_data_addr = (VAL_UINT8_T *) arg;
			...
			if (rPowerParam.eDriverType == VAL_DRIVER_TYPE_H264_ENC ||
				...
				if (1 == gu4L2CCounter) {
					if (config_L2(0)) {
						...
						MFV_LOGE
						    ("[MFV] Switch L2C size to 512K successful\n");
					}
				}
			}
#endif
	...
	case VCODEC_DEC_PWR_USER:
		{
			MFV_LOGD("[8173] VCODEC_DEC_PWR_USER + tid = %d\n", current->pid);
			user_data_addr = (VAL_UINT8_T *) arg;
			ret = copy_from_user(&rPowerParam, user_data_addr, sizeof(VAL_POWER_T));
			if (ret) {
				MFV_LOGE
			...
			if (rPowerParam.eDriverType == VAL_DRIVER_TYPE_H264_ENC ||
				...
				if (0 == gu4L2CCounter) {
					if (config_L2(1)) {
						...
						MFV_LOGE
						    ("[MFV] Switch L2C size to 0K successful\n");
					}
				}
			}
#endif
	...
	case VCODEC_WAITISR:
		{
			MFV_LOGD("[8173] VCODEC_WAITISR + tid = %d\n", current->pid);
			user_data_addr = (VAL_UINT8_T *) arg;
	...
	case VCODEC_INITHWLOCK:
		{
			MFV_LOGE("[8173] VCODEC_INITHWLOCK [EMPTY] + - tid = %d\n", current->pid);
	...
	case VCODEC_DEINITHWLOCK:
		{
			MFV_LOGE("[8173] VCODEC_DEINITHWLOCK [EMPTY] + - tid = %d\n", current->pid);
	...
	case VCODEC_GET_CPU_LOADING_INFO:
		{
			...
			MFV_LOGD("[8173] VCODEC_GET_CPU_LOADING_INFO +\n");
			user_data_addr = (VAL_UINT8_T *) arg;
			/* TODO: */
	...
	case VCODEC_GET_CORE_LOADING:
		{
			MFV_LOGD("[8173] VCODEC_GET_CORE_LOADING + - tid = %d\n", current->pid);
	...
	case VCODEC_GET_CORE_NUMBER:
		{
			MFV_LOGD("[8173] VCODEC_GET_CORE_NUMBER + - tid = %d\n", current->pid);
	...
	case VCODEC_SET_CPU_OPP_LIMIT:
		{
			MFV_LOGE("[8173] VCODEC_SET_CPU_OPP_LIMIT [EMPTY] + - tid = %d\n",
				 current->pid);
			user_data_addr = (VAL_UINT8_T *) arg;
			ret =
...
static int vcodec_open(struct inode *inode, struct file *file)
{
	...
	MFV_LOGE("vcodec_open pid = %d, MT8173Driver_Open_Count %d\n", current->pid,
		 MT8173Driver_Open_Count);
...
static int vcodec_flush(struct file *file, fl_owner_t id)
{
	...
	MFV_LOGE("vcodec_flush pid = %d, MT8173Driver_Open_Count %d\n", current->pid, MT8173Driver_Open_Count);
...
static int vcodec_probe(struct platform_device *pdev)
{
	...
	if (request_irq
	    (VENC_LT_IRQ_ID, (irq_handler_t) video_intr_dlr2, IRQF_TRIGGER_LOW, VCODEC_DEVNAME,
	     NULL) < 0) {
		MFV_LOGD("[VCODEC_DEBUG][ERROR] error to request enc_LT irq\n");
	} else {
		MFV_LOGD("[VCODEC_DEBUG] success to request enc_LT irq: %d\n", VENC_LT_IRQ_ID);
	}
...
static int venc_enableIRQ(VAL_HW_LOCK_T *prHWLock)
{
	...
	venc_power_on();
#ifdef CONFIG_MTK_SEC_VIDEO_PATH_SUPPORT
	MFV_LOGD("[VCODEC_LOCKHW] ENC rHWLock.bSecureInst 0x%x\n", prHWLock->bSecureInst);
	if (prHWLock->bSecureInst == VAL_FALSE) {
		MFV_LOGE("[VCODEC_LOCKHW]  ENC Request IR by type 0x%x\n", prHWLock->eDriverType);
		if (request_irq(
			u4IrqId ,
...
/* KVA_VENC_IRQ_ACK_ADDR  = (VAL_ULONG_T)ioremap(VENC_IRQ_ACK_addr, 4); */
	...
	if (VAL_FALSE == bIsOpened) {
		...
		if ((Ret = platform_driver_register(&VCodecDriver)) < 0) {
			MFV_LOGE("platform_driver_register fail");
			return Ret;
		}
#else
		vcodec_probe(NULL);
