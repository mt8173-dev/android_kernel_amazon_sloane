...
#define HW_BASE         0x80002004
#define HW_REGION       0x2000

int KVA_VENC_IRQ_ACK_ADDR, KVA_VENC_MP4_IRQ_ACK_ADDR;
int KVA_VENC_IRQ_STATUS_ADDR, KVA_VENC_MP4_IRQ_STATUS_ADDR, KVA_VENC_ZERO_COEF_COUNT_ADDR,
...
void vdec_power_on(VAL_DRIVER_TYPE_T drvType)
{
	/* Central power on */
	/* enable_clock(MT_CG_VDEC0_VDE, "VDEC"); */
	/* enable_clock(MT_CG_VDEC1_SMI, "VDEC"); */

	enable_clock(MT_CG_VDEC_CKEN, "VDEC");
	enable_clock(MT_CG_VDEC_LARB_CKEN, "VDEC");
	enable_clock(MT_CG_V_SYSPLL_D3, "VDEC");
	enable_pll(VDECPLL, "VDEC_PLL");

	switch (drvType) {
	case VAL_DRIVER_TYPE_H264_DEC:
		pll_fsel(VDECPLL, 0x800D09D8);
		break;
	default:
		pll_fsel(VDECPLL, 0x800C89D8);
		break;
	}

	clkmux_sel(MT_CLKMUX_VDEC_SEL, MT_CG_V_AD_VDECPLL_CK, "VDEC");
}
...
void vdec_power_off(void)
{
	...
	disable_clock(MT_CG_VDEC_CKEN, "VDEC");
	disable_clock(MT_CG_VDEC_LARB_CKEN, "VDEC");
	disable_clock(MT_CG_V_SYSPLL_D3, "VDEC");
	disable_pll(VDECPLL, "VDEC_PLL");
}
...
void venc_power_on(void)
{
	...
	/* MTCMOS on */
	VDO_HW_WRITE(KVA_VENC_PWR_ADDR,
		     ((VDO_HW_READ(KVA_VENC_PWR_ADDR) & 0xffffffc0) | PWR_ON_SEQ_0));
	VDO_HW_WRITE(KVA_VENC_PWR_ADDR,
		     ((VDO_HW_READ(KVA_VENC_PWR_ADDR) & 0xffffffc0) | PWR_ON_SEQ_1));
	VDO_HW_WRITE(KVA_VENC_PWR_ADDR,
		     ((VDO_HW_READ(KVA_VENC_PWR_ADDR) & 0xffffffc0) | PWR_ON_SEQ_2));
	VDO_HW_WRITE(KVA_VENC_PWR_ADDR,
		     ((VDO_HW_READ(KVA_VENC_PWR_ADDR) & 0xffffffc0) | PWR_ON_SEQ_3));
	...
	enable_clock(MT_CG_VENC_CKE, "VENC");
...
void venc_power_off(void)
{
	...
	disable_clock(MT_CG_VENC_CKE, "VENC");
	MFV_LOGD("venc_power_off -\n");
}
...
void dec_isr(void)
{
	...
	if ((u4Infra & (0x1 << 15)) != 0) {
		MFV_LOGE("[MFV][ERROR] Entering ISR, but Infra is not 0x0 (0x%08x)(0x%08x)",
			 u4Infra & ((0x1 << 15)), u4Infra);
	}
#endif
	...
	if ((u4DecDoneStatus & (0x1 << 16)) != 0x10000) {
		MFV_LOGE("[MFV][ERROR] DEC ISR, Decode done status is not 0x1 (0x%08x)",
			 u4DecDoneStatus);
		return;
	}
	...
	u4CgStatus = VDO_HW_READ(0xF6000000);
	if (u4CgStatus != 0x1) {
		MFV_LOGE("[MFV][ERROR] Entering ISR, but CG status is not 0x1 (0x%08x), mid!...",
			 u4CgStatus);
		/* return; */
	}

	u4Val = VDO_HW_READ(0xF60200C8);
	MFV_LOGD("[MFV][DEBUG] Read MISC_50 0xF60200C8 = 0x%08x", u4Val);
	u4Val = VDO_HW_READ(0xF60200CC);
	MFV_LOGD("[MFV][DEBUG] Read MISC_51 0xF60200CC = 0x%08x", u4Val);

	u4Val = VDO_HW_READ(0xF6024250);
	MFV_LOGD("[MFV][DEBUG] Intra MV count Read 0xF6024250 = 0x%08x", u4Val);
	u4Val = VDO_HW_READ(0xF60213CC);
	MFV_LOGD("[MFV][DEBUG] Error check Read 0xF60213CC = 0x%08x", u4Val);
	u4Val = VDO_HW_READ(0xF602000C);
	MFV_LOGD("[MFV][DEBUG] CRC Read 0xF602000C = 0x%08x", u4Val);
#if 0				/* CRC */
	...
	MFV_LOGD"[MFV][DEBUG] Read 0x10006234 = 0x%08x, is_ok = %d", u4Val,
		 (u4Val & 0xF00) == 0xF00);
	u4Val = VDO_HW_READ(KVA_CHECK_2_ADDR);
	MFV_LOGD("[MFV][DEBUG] Read 0x10006210 = 0x%08x, is_ok = %d", u4Val,
		 (u4Val & 0xF00) == 0xF00);
#endif
	...
	if ((u4Infra & (0x1 << 15)) != 0) {
		MFV_LOGE
		    ("[MFV][ERROR] Leaving ISR, but Infra is not 0x0 (0x%08x)(0x%08x), leaving...",
		     u4Infra & ((0x1 << 15)), u4Infra);
	}

	u4CgStatus = VDO_HW_READ(0xF6000000);
	if (u4CgStatus != 0x1) {
		MFV_LOGE("[MFV][ERROR] Entering ISR, but CG status is not 0x1 (0x%08x), leaving...",
			 u4CgStatus);
		/* return; */
	}
#endif
...
void enc_isr(void)
{
	...
	if (grVcodecEncHWLock.eDriverType == VAL_DRIVER_TYPE_H264_ENC ||
	    grVcodecEncHWLock.eDriverType == VAL_DRIVER_TYPE_VP8_ENC ||
	    grVcodecEncHWLock.eDriverType == VAL_DRIVER_TYPE_MP4_ENC) {
		...
		if (gu4HwVencIrqStatus & VENC_IRQ_STATUS_DRAM_VP8) {
			VDO_HW_WRITE(KVA_VENC_IRQ_ACK_ADDR,
				     (VENC_IRQ_STATUS_DRAM_VP8 | VENC_IRQ_STATUS_DRAM));
		}
...
*((volatile VAL_UINT32_T *)oal_hw_context[index].oalmem_status[i].u4ReadAddr);
			...
			if (maxnum == 3) {
				if (i == 0) {
					...
					if (u4IRQStatus != 2) {
						MFV_LOGE
						    ("[ERROR][ISR] IRQ status error u4IRQStatus = %d\n",
						     u4IRQStatus);
					}
				}
...
static long vcodec_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case VCODEC_ALLOC_NON_CACHE_BUFFER:
		...
		if (ret) {
			MFV_LOGE
			    ("[ERROR] VCODEC_ALLOC_NON_CACHE_BUFFER, copy_from_user failed: %d\n",
			     ret);
			return -EFAULT;
		}

		rTempMem.u4ReservedSize /*kernel va */  =
		    (unsigned int)dma_alloc_coherent(0, rTempMem.u4MemSize,
		...
		MFV_LOGD("kernel va = 0x%x, kernel pa = 0x%x, memory size = %d\n",
			 (unsigned int)rTempMem.u4ReservedSize, (unsigned int)rTempMem.pvMemPa,
		...
		Add_NonCacheMemoryList(rTempMem.u4ReservedSize, (VAL_UINT32_T) rTempMem.pvMemPa,
				       (VAL_UINT32_T) rTempMem.u4MemSize, u4TempVCodecThreadNum,
				       u4TempVCodecThreadID);
	...
	case VCODEC_FREE_NON_CACHE_BUFFER:
		...
		if (ret) {
			MFV_LOGE
			    ("[ERROR] VCODEC_FREE_NON_CACHE_BUFFER, copy_from_user failed: %d\n",
			     ret);
			return -EFAULT;
		}
		...
		if (rHWLock.eDriverType == VAL_DRIVER_TYPE_MP4_DEC ||
			...
			while (bLockedHW == VAL_FALSE) {
				...
				if (DecHWLockEvent.u4TimeoutMs == 1) {
					MFV_LOGD("[NOT ERROR][VCODEC_LOCKHW] First Use Dec HW!!\n");
					FirstUseDecHW = 1;
				} else {
					FirstUseDecHW = 0;
				}
				...
				if (FirstUseDecHW == 1) {
					eValRet =
					    eVideoWaitEvent(&DecHWLockEvent, sizeof(VAL_EVENT_T));
				}
				mutex_lock(&DecHWLockEventTimeoutLock);
				...
				if (grVcodecDecHWLock.pvHandle ==
				    (VAL_VOID_T *) pmem_user_v2p_video((unsigned int)rHWLock.
								       pvHandle)) {
					MFV_LOGW
					    ("[WARNING] one decoder instance try to lock twice, may cause lock HW timeout!! instance = 0x%x, CurrentTID = %d\n",
				...
				if (grVcodecDecHWLock.pvHandle == 0)	/* No one holds dec hw lock now */
				{
					grVcodecDecHWLock.pvHandle =
					    (VAL_VOID_T *) pmem_user_v2p_video((unsigned int)
									       rHWLock.pvHandle);
					...
					MFV_LOGD
					    ("LockInstance = 0x%x CurrentTID = %d, rLockedTime(s, us) = %d, %d\n",
					     grVcodecDecHWLock.pvHandle, current->pid,
					     grVcodecDecHWLock.rLockedTime.u4Sec,
					     grVcodecDecHWLock.rLockedTime.u4uSec);

					bLockedHW = VAL_TRUE;
					vdec_power_on(rHWLock.eDriverType);
					...
					if (rHWLock.bSecureInst == VAL_FALSE) {
						if (request_irq
						    (MT_VDEC_IRQ_ID, (irq_handler_t) video_intr_dlr,
						     IRQF_TRIGGER_RISING, VCODEC_DEVNAME,
						     NULL) < 0) {
							MFV_LOGE
				...
				{
					MFV_LOGD("[NOT ERROR][VCODEC_LOCKHW] E\n");
					...
					MFV_LOGD
					    ("Instance = 0x%x CurrentTID = %d, TimeInterval(ms) = %d, TimeOutValue(ms)) = %d\n",
					     grVcodecDecHWLock.pvHandle, current->pid,
					     u4TimeInterval, rHWLock.u4TimeoutMs);

					MFV_LOGD
					    ("Instance = 0x%x, CurrentTID = %d, rLockedTime(s, us) = %d, %d, rCurTime(s, us) = %d, %d\n",
					     grVcodecDecHWLock.pvHandle, current->pid,
					     grVcodecDecHWLock.rLockedTime.u4Sec,
					     grVcodecDecHWLock.rLockedTime.u4uSec, rCurTime.u4Sec,
					     rCurTime.u4uSec);
			...
			while (bLockedHW == VAL_FALSE) {
				...
				if (EncHWLockEvent.u4TimeoutMs == 1) {
					MFV_LOGD("[NOT ERROR][VCODEC_LOCKHW] First Use Enc HW!!\n");
					FirstUseEncHW = 1;
				} else {
					FirstUseEncHW = 0;
				}
				...
				if (FirstUseEncHW == 1) {
					eValRet =
					    eVideoWaitEvent(&EncHWLockEvent, sizeof(VAL_EVENT_T));
				}
				mutex_lock(&EncHWLockEventTimeoutLock);
				if (EncHWLockEvent.u4TimeoutMs == 1) {
					...
					if (rHWLock.eDriverType == VAL_DRIVER_TYPE_H264_ENC ||
						...
						grVcodecEncHWLock.pvHandle =
						    ...
						    pmem_user_v2p_video((unsigned int)rHWLock.
									pvHandle);
						MFV_LOGD
						    ("[LOG][VCODEC_LOCKHW] No process use HW, so current process can use HW, handle = 0x%x\n",
						     grVcodecEncHWLock.pvHandle);
						grVcodecEncHWLock.eDriverType = rHWLock.eDriverType;
						eVideoGetTimeOfDay(&grVcodecEncHWLock.rLockedTime,
								   sizeof(VAL_TIME_T));
				...
				} else	/* someone use HW, and check timeout value */
				{
					...
					MFV_LOGD
					    ("LockInstance = 0x%x, CurrentInstance = 0x%x, CurrentTID = %d, TimeInterval(ms) = %d, TimeOutValue(ms)) = %d\n",
					     grVcodecEncHWLock.pvHandle,
					     pmem_user_v2p_video((unsigned int)rHWLock.pvHandle),
					     current->pid, u4TimeInterval, rHWLock.u4TimeoutMs);

					MFV_LOGD
					    ("LockInstance = 0x%x, CurrentInstance = 0x%x, CurrentTID = %d, rLockedTime(s, us) = %d, %d, rCurTime(s, us) = %d, %d\n",
					     grVcodecEncHWLock.pvHandle,
					     pmem_user_v2p_video((unsigned int)rHWLock.pvHandle),
					     current->pid, grVcodecEncHWLock.rLockedTime.u4Sec,
					     grVcodecEncHWLock.rLockedTime.u4uSec, rCurTime.u4Sec,
					     rCurTime.u4uSec);
						...
						/* Locked process(A) timeout, so release HW and let currnet process(B) to use HW */
					{
						if (rHWLock.eDriverType == VAL_DRIVER_TYPE_H264_ENC
							...
							grVcodecEncHWLock.pvHandle =
							    (VAL_VOID_T *)
							    pmem_user_v2p_video((unsigned int)
										rHWLock.pvHandle);
			...
			MFV_LOGW("[WARNING] VCODEC_LOCKHW Unknown instance\n");
			return -EFAULT;
		}
		MFV_LOGD("[MT6589] VCODEC_LOCKHW - tid = %d\n", current->pid);
		break;
		...
		if (rHWLock.eDriverType == VAL_DRIVER_TYPE_MP4_DEC ||
			...
			if (rHWLock.eDriverType == VAL_DRIVER_TYPE_H264_ENC ||
				...
				} else	/* Not current owner */
				{
					/* [TODO] error handling */
					MFV_LOGE
					    ("[ERROR] Not owner trying to unlock enc hardware 0x%x\n",
					     pmem_user_v2p_video((unsigned int)rHWLock.pvHandle));
					mutex_unlock(&VencHWLock);
					return -EFAULT;
				}
			...
			MFV_LOGW("[WARNING] VCODEC_UNLOCKHW Unknown instance\n");
			return -EFAULT;
		}
		MFV_LOGD("[MT6589] VCODEC_UNLOCKHW - tid = %d\n", current->pid);
		break;
	...
	case VCODEC_WAITISR:
		...
		if (val_isr.eDriverType == VAL_DRIVER_TYPE_MP4_DEC ||
			...
			mutex_lock(&VdecHWLock);
			if (grVcodecDecHWLock.pvHandle ==
			    (VAL_VOID_T *) pmem_user_v2p_video((unsigned int)val_isr.pvHandle)) {
				bLockedHW = VAL_TRUE;
			} else {
			}
			mutex_unlock(&VdecHWLock);
			...
			if (val_isr.eDriverType == VAL_DRIVER_TYPE_H264_ENC || val_isr.eDriverType == VAL_DRIVER_TYPE_VP8_ENC ||	/* Pure HW */
			    val_isr.eDriverType == VAL_DRIVER_TYPE_MP4_ENC) {
				mutex_lock(&VencHWLock);
				if (grVcodecEncHWLock.pvHandle ==
				    (VAL_VOID_T *) pmem_user_v2p_video((unsigned int)val_isr.
								       pvHandle)) {
					bLockedHW = VAL_TRUE;
				} else {
				}
				...
				if (val_isr.u4IrqStatusNum > 0) {
					...
					if (ret) {
						MFV_LOGE
						    ("[ERROR] VCODEC_WAITISR, copy_to_user failed: %d\n",
						     ret);
						return -EFAULT;
					}
				}
			...
			MFV_LOGW("[WARNING] VCODEC_WAITISR Unknown instance\n");
			return -EFAULT;
		}
		MFV_LOGD("[MT6589] VCODEC_WAITISR - tid = %d\n", current->pid);
		break;
	...
	case VCODEC_INITHWLOCK:
		{
			...
			context->Oal_HW_mem_reg =
			    (VAL_UINT32_T *) (((VAL_VCODEC_OAL_HW_REGISTER_T *) user_data_addr)->
					      pHWStatus);
			if (hwoal_reg.u4NumOfRegister != 0) {
				context->pa_Oal_HW_mem_reg =
				    pmem_user_v2p_video((int)
			...
			MFV_LOGD("user_data_addr->u4HWIsCompleted ua = 0x%x pa= 0x%x\n",
				 (int)&(((VAL_VCODEC_OAL_HW_REGISTER_T *) user_data_addr)->
			...
			if (hwoal_reg.u4NumOfRegister != 0) {
				...
				mutex_lock(&NonCacheMemoryListLock);
				pu4TempKVA =
				    (VAL_UINT32_T *) Search_NonCacheMemoryList_By_KPA(u4TempKPA);
				mutex_unlock(&NonCacheMemoryListLock);
				spin_lock_irqsave(&OalHWContextLock, ulFlags);
				context->kva_Oal_HW_mem_reg = pu4TempKVA;
				MFV_LOGD("context->ua = 0x%x  pa_Oal_HW_mem_reg = 0x%x\n",
					 (int)(((VAL_VCODEC_OAL_HW_REGISTER_T *) user_data_addr)->
					       pHWStatus), context->pa_Oal_HW_mem_reg);
			}
			spin_unlock_irqrestore(&OalHWContextLock, ulFlags);
			mutex_lock(&NonCacheMemoryListLock);
			kva_TempReg =
			...
			context->kva_u4HWIsCompleted =
			    (VAL_UINT32_T) (&(kva_TempReg->u4HWIsCompleted));
			context->kva_u4HWIsTimeout = (VAL_UINT32_T) (&(kva_TempReg->u4HWIsTimeout));
			MFV_LOGD
			    ("kva_TempReg = 0x%x, kva_u4HWIsCompleted = 0x%x, kva_u4HWIsTimeout = 0x%x\n",
			     (VAL_UINT32_T) kva_TempReg, context->kva_u4HWIsCompleted,
			...
			for (i = 0; i < hwoal_reg.u4NumOfRegister; i++) {
				...
				/* TODO: */
				addr_pa =
				    pmem_user_v2p_video((unsigned int)oal_mem_status[i].u4ReadAddr);
				kva = (VAL_UINT32_T) ioremap(addr_pa, 8);	/* need to remap addr + data addr */
				MFV_LOGD("[REG_INFO_2] [%d] pa = 0x%x  kva = 0x%x\n", i, addr_pa,
					 kva);
				context->oalmem_status[i].u4ReadAddr = kva;	/* oal_mem_status[i].u4ReadAddr; */
			}
			spin_unlock_irqrestore(&OalHWContextLock, ulFlags);

			MFV_LOGD(" VCODEC_INITHWLOCK addr1 0x%x addr2 0x%x\n",
				  (unsigned int)arg,
				  (unsigned int)((VAL_VCODEC_OAL_HW_REGISTER_T *) arg)->pHWStatus);
			MFV_LOGD("[MT6589] VCODEC_INITHWLOCK - - tid = %d\n", current->pid);
		}
		break;
	...
	case VCODEC_DEINITHWLOCK:
		{
			...
			addr_pa = pmem_user_v2p_video((unsigned int)user_data_addr);
			 ...
			 MFV_LOGD("VCODEC_DEINITHWLOCK ObjId=%d\n", current->pid);
			spin_lock_irqsave(&OalHWContextLock, ulFlags);
	...
	case VCODEC_GET_CPU_LOADING_INFO:
		{
			...
			if (ret) {
				MFV_LOGE
				    ("[ERROR] VCODEC_GET_CPU_LOADING_INFO, copy_to_user failed: %d\n",
				     ret);
				return -EFAULT;
			}
	...
	case VCODEC_GET_CORE_LOADING:
		{
			...
			if (ret) {
				MFV_LOGE
				    ("[ERROR] VCODEC_GET_CORE_LOADING, copy_from_user failed: %d\n",
				     ret);
				return -EFAULT;
			}
			...
			if (ret) {
				MFV_LOGE
				    ("[ERROR] VCODEC_GET_CORE_LOADING, copy_to_user failed: %d\n",
				     ret);
				return -EFAULT;
			}
	...
	case VCODEC_GET_CORE_NUMBER:
		{
			...
			if (ret) {
				MFV_LOGE
				    ("[ERROR] VCODEC_GET_CORE_NUMBER, copy_to_user failed: %d\n",
				     ret);
				return -EFAULT;
			}
	...
	case VCODEC_SET_CPU_OPP_LIMIT:
		...
		MFV_LOGD("+VCODEC_SET_CPU_OPP_LIMIT (%d, %d, %d), tid = %d\n",
			 rCpuOppLimit.limited_freq, rCpuOppLimit.limited_cpu, rCpuOppLimit.enable,
			 current->pid);
...
static int vcodec_open(struct inode *inode, struct file *file)
{
	...
	MFV_LOGD("vcodec_open pid = %d, MT6589Driver_Open_Count %d\n", current->pid,
		 MT6589Driver_Open_Count);
...
static int vcodec_flush(struct file *file, fl_owner_t id)
{
	...
	mutex_lock(&DriverOpenCountLock);
	MFV_LOGD("vcodec_flush pid = %d, MT6589Driver_Open_Count %d\n", current->pid,
		 MT6589Driver_Open_Count);
...
{
	mutex_lock(&PWRLock);
	MFV_LOGI("vcodec_early_suspend, tid = %d, PWR_USER = %d\n", current->pid, gu4PWRCounter);
	mutex_unlock(&PWRLock);
/*
    if (gu4PWRCounter != 0)
    {
...
{
	mutex_lock(&PWRLock);
	MFV_LOGI("vcodec_late_resume, tid = %d, PWR_USER = %d\n", current->pid, gu4PWRCounter);
	mutex_unlock(&PWRLock);
/*
    if (gu4PWRCounter != 0)
    {
...
static struct class *pVcodecClass;
static struct device *VcodecDevice;
static int vcodec_probe(struct platform_device *dev)
{
	...
	if (ret) {
		MFV_LOGE("[MFV_DEBUG][ERROR] Can't Get Major number for VCodec Device\n");
	}
	...
	/* create /dev/Vcodec automaticly */
	pVcodecClass = class_create(THIS_MODULE, VCODEC_DEVNAME);
	if (IS_ERR(pVcodecClass)) {
		int ret = PTR_ERR(pVcodecClass);
		MFV_LOGE("Unable to create class, err = %d", ret);
		return ret;
	}
	VcodecDevice = device_create(pVcodecClass, NULL, vcodec_devno, NULL, VCODEC_DEVNAME);

#ifdef CONFIG_MTK_SEC_VIDEO_PATH_SUPPORT
#else
	if (request_irq
	    (MT_VDEC_IRQ_ID, (irq_handler_t) video_intr_dlr, IRQF_TRIGGER_RISING, VCODEC_DEVNAME,
	     NULL) < 0) {
		MFV_LOGE("[MFV_DEBUG][ERROR] error to request dec irq\n");
	} else {
	...
	if (request_irq
	    (MT_VENC_IRQ_ID, (irq_handler_t) video_intr_dlr2, IRQF_TRIGGER_LOW, VCODEC_DEVNAME,
	     NULL) < 0) {
		MFV_LOGE("[MFV_DEBUG][ERROR] error to request enc irq\n");
	} else {
