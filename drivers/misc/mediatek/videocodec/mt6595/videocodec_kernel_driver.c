...
static long vcodec_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
		...
		if (rHWLock.eDriverType == VAL_DRIVER_TYPE_MP4_DEC ||
			...
			while (bLockedHW == VAL_FALSE) {
				...
				if (grVcodecDecHWLock.pvHandle == 0) {	/* No one holds dec hw lock now */
					...
					{
						grVcodecDecHWLock.pvHandle = (VAL_VOID_T *)
			...
			while (bLockedHW == VAL_FALSE) {
				...
				if (EncHWLockEvent.u4TimeoutMs == 1) {
					MFV_LOGE
					    ("[NOT ERROR][VCODEC_LOCKHW] First Use Enc HW %d!!\n",
					     rHWLock.eDriverType);
					FirstUseEncHW = 1;
				} else {
					FirstUseEncHW = 0;
				...
				if (grVcodecEncHWLock.pvHandle ==
				    (VAL_VOID_T *) pmem_user_v2p_video((unsigned int)
								       rHWLock.pvHandle)) {
					MFV_LOGE
					    ("[WARNING] one encoder instance try to lock twice, may cause lock HW timeout!! instance = 0x%x, CurrentTID = %d, type:%d\n",
				...
				if (VAL_RESULT_INVALID_ISR == eValRet) {
					...
					if (FirstUseEncHW != 1) {
						...
						if (grVcodecEncHWLock.pvHandle == 0) {
							...
							MFV_LOGE
							    ("[WARNING] someone use HW, and check timeout value!! %d\n",
							     gLockTimeOutCount);
							++gLockTimeOutCount;
							if (gLockTimeOutCount > 30) {
								MFV_LOGE
								    ("VCODEC_LOCKHW - ID %d  fail, someone locked HW time out more than 30 times %x, %x, %x, type:%d\n",
								     current->pid,
								     grVcodecEncHWLock.pvHandle,
								     pmem_user_v2p_video((unsigned
											  int)
											 rHWLock.pvHandle), rHWLock.pvHandle, rHWLock.eDriverType);
								gLockTimeOutCount = 0;
								mutex_unlock(&VencHWLock);
								return -EFAULT;
							}
							...
							if (rHWLock.u4TimeoutMs == 0) {
								MFV_LOGE
								    ("VCODEC_LOCKHW - ID %d  fail, someone locked HW already %x, %x, %x, type:%d\n",
								     current->pid,
								     grVcodecEncHWLock.pvHandle,
								     pmem_user_v2p_video((unsigned
											  int)
				...
				if (grVcodecEncHWLock.pvHandle == 0) {	/* No process use HW, so current process can use HW */
					...
					if (gLockTimeOutCount > 30) {
						MFV_LOGE
						    ("VCODEC_LOCKHW - ID %d  fail, someone locked HW over 30 times without timeout %x, %x, %x, type:%d\n",
						     current->pid, grVcodecEncHWLock.pvHandle,
						     pmem_user_v2p_video((unsigned int)
									 rHWLock.pvHandle),
						     rHWLock.pvHandle, rHWLock.eDriverType);
						gLockTimeOutCount = 0;
						mutex_unlock(&VencHWLock);
						return -EFAULT;
					}
				...
				if (VAL_TRUE == bLockedHW) {
					MFV_LOGE
					    ("Lock ok grVcodecEncHWLock.pvHandle = 0x%x, va:%x, type:%d",
					     grVcodecEncHWLock.pvHandle,
					     (unsigned int)rHWLock.pvHandle, rHWLock.eDriverType);
					gLockTimeOutCount = 0;
				}
				mutex_unlock(&VencHWLock);
			}
	...
	case VCODEC_UNLOCKHW:
		...
		if (rHWLock.eDriverType == VAL_DRIVER_TYPE_MP4_DEC ||
			...
			if (grVcodecEncHWLock.pvHandle == (VAL_VOID_T *) pmem_user_v2p_video((unsigned int)rHWLock.pvHandle)) {	/* Current owner give up hw lock */
				...
				/* [TODO] error handling */
				MFV_LOGE
				    ("[ERROR] Not owner trying to unlock enc hardware 0x%x, pa:%x, va:%x type:%d\n",
				     grVcodecEncHWLock.pvHandle,
				     pmem_user_v2p_video((unsigned int)rHWLock.pvHandle),
				     (unsigned int)rHWLock.pvHandle, rHWLock.eDriverType);
				mutex_unlock(&VencHWLock);
	...
	case VCODEC_WAITISR:
...
#endif
		{
			...
			if (bLockedHW == VAL_FALSE) {
				MFV_LOGE
				    ("[ERROR] DO NOT have enc HWLock, so return fail pa:%x, va:%x\n",
...
static int vcodec_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	if (((length > VENC_REGION) || (pfn < VENC_BASE) || (pfn > VENC_BASE + VENC_REGION)) &&
		...
		for (u4I = 0; u4I < VCODEC_MULTIPLE_INSTANCE_NUM_x_10; u4I++) {
			if ((grNonCacheMemoryList[u4I].u4KVA != 0xffffffff)
			    && (grNonCacheMemoryList[u4I].u4KPA != 0xffffffff)) {
				u4Addr = grNonCacheMemoryList[u4I].u4KPA;
				u4Size =
				    (grNonCacheMemoryList[u4I].u4Size + 0x1000 - 1) & ~(0x1000 - 1);
...
static int vcodec_probe(struct platform_device *dev)
{
	...
	if (request_irq
	    (VDEC_IRQ_BIT_ID, (irq_handler_t) video_intr_dlr, IRQF_TRIGGER_LOW, VCODEC_DEVNAME,
	     NULL) < 0) {
	...
	if (request_irq
	    (VENC_IRQ_BIT_ID, (irq_handler_t) video_intr_dlr2, IRQF_TRIGGER_LOW, VCODEC_DEVNAME,
	     NULL) < 0) {
