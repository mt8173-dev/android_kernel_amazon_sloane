...
{
	return (U32) (GET_WACS0_FSM(x) != WACS_FSM_IDLE);
}

static inline U32 wait_for_fsm_vldclr(U32 x)
{
	return (U32) (GET_WACS0_FSM(x) != WACS_FSM_WFVLDCLR);
}

static inline U32 wait_for_sync(U32 x)
{
	return (U32) (GET_SYNC_IDLE0(x) != WACS_SYNC_IDLE);
}

static inline U32 wait_for_idle_and_sync(U32 x)
{
	return (U32) ((GET_WACS2_FSM(x) != WACS_FSM_IDLE) || (GET_SYNC_IDLE2(x) != WACS_SYNC_IDLE));
}

static inline U32 wait_for_wrap_idle(U32 x)
{
	return (U32) ((GET_WRAP_FSM(x) != 0x0) || (GET_WRAP_CH_DLE_RESTCNT(x) != 0x0));
}

static inline U32 wait_for_wrap_state_idle(U32 x)
{
	return (U32) (GET_WRAP_AG_DLE_RESTCNT(x) != 0);
}

static inline U32 wait_for_man_idle_and_noreq(U32 x)
{
	return (U32) ((GET_MAN_REQ(x) != MAN_FSM_NO_REQ) || (GET_MAN_FSM(x) != MAN_FSM_IDLE));
}

static inline U32 wait_for_man_vldclr(U32 x)
{
	return (U32) (GET_MAN_FSM(x) != MAN_FSM_WFVLDCLR);
}
...
{
	return (U32) (GET_STAUPD_FSM(x) != 0x0);
}
...
static S32 _pwrap_init_sistrobe(void)
{
	...
	for (ind = 0; ind < 24; ind++) {
		...
		if (rdata == MT6331_DEFAULT_VALUE_READ_TEST) {
			PWRAPLOG
			    ("_pwrap_init_sistrobe [Read Test of MT6331] pass,index=%d rdata=%x\n",
			     ind, rdata);
			result[0] |= (0x1 << ind);
		} else {
			PWRAPLOG
			    ("_pwrap_init_sistrobe [Read Test of MT6331] tuning,index=%d rdata=%x\n",
			     ind, rdata);
		}
#endif
#ifdef SLV_6332
...
static void pwrap_int_test(void)
{
	...
	while (1) {
		...
		PWRAPREG
		    ("Pwrap INT status check,PMIC_WRAP_EINT_STA=0x%x,MT6331_INT_STA[0x01B4]=0x%x\n",
		     rdata1, rdata2);
#endif
#ifdef SLV_6332
		rdata1 = WRAP_RD32(PMIC_WRAP_EINT_STA);
