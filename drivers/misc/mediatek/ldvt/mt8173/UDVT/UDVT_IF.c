...
void UDVT_IF_TestProgress(UINT32 u4Percent)
{
  ...
  {
    pPck = UDVT_GetTestCasePacketByThreadHandle(hTestCaseThread,UDVT_GET_TESTCASE_PACKET_OUTPUT);
    if(pPck != 0)
    {
      pPck->lData[0] = u4Percent;
      pPck->sPacketHeader.PacketLength = 12+4;
      pPck->sPacketHeader.lPacketChecksum = 0;
      pPck->sPacketHeader.PacketHeader = UDVT_FrameTheHeader(UDVT_DATA_PACKET,UDVT_ACK,FALSE);;
...
UINT32 UDVT_IF_ReadFile(void *ptr,UINT32 size,UINT32 count,UINT32 stream)
{
  ...
  if(ptr == 0 || BuffSize == 0)
  {
    UDVT_LOG(3,"[UDVT] Read Buff is not prepare, ptr 0x%x BuffSize %d\n", ptr, BuffSize);
    return 0;
  }
