                           ...
                           BOOL                  wait)
{
    ...
    if (pe_events_got == NULL ||
        (rem_op != X_EV_OP_AND && rem_op != X_EV_OP_OR && rem_op != X_EV_OP_AND_CONSUME && rem_op != X_EV_OP_OR_CONSUME))
    {
        return OSR_INV_ARG;
    }
    ...
    if (wait)
    {
        while (TRUE)
        {
            if(e_op & 0x10) //interuptible
            {
			    ...
			    {
			        ret = wait_event_interruptible(pt_ev_grp->wq, (pt_ev_grp->e_events & e_events_req) == e_events_req);
			    }
			    else
			    {
			        ret = wait_event_interruptible(pt_ev_grp->wq, (pt_ev_grp->e_events & e_events_req) != (EV_GRP_EVENT_T)(0));
			    }
            }
			else
            {
    ...
    *pe_events_got = 0;
    ...
    while (TRUE)
    {
        if(e_op & 0x10) //interuptible
        {
            if (rem_op == X_EV_OP_AND || rem_op == X_EV_OP_AND_CONSUME)
            {
                ret = wait_event_interruptible_timeout(pt_ev_grp->wq, (pt_ev_grp->e_events & e_events_req) == e_events_req, ui4_time / quantum_ms);
            }
            else
            {
                ret = wait_event_interruptible_timeout(pt_ev_grp->wq, (pt_ev_grp->e_events & e_events_req) != (EV_GRP_EVENT_T)(0), ui4_time / quantum_ms);
            }
        }
		else
		{
