               ...
               UINT16       ui2_msg_count)
{
    ...
    pt_msg_q = kcalloc(1, sizeof(OS_MSGQ_LIGHT_T) - sizeof(BYTE) + message_size * (ui2_msg_count + 1), GFP_KERNEL);
    if (pt_msg_q == NULL)
    {
                        ...
                        UINT32          ui4_time)
{
    ...
    if (pt_msg_q->read == pt_msg_q->write)
    {
        ret = wait_event_timeout(pt_msg_q->write_wq, pt_msg_q->read != pt_msg_q->write, ui4_time / quantum_ms);
        if (ret == 0)
        {
            return OSR_TIMEOUT;
        }
