...
#if 1 //#ifdef SATA_HDD_FS_SUPPORT
...
//          UTIL_Printf(" i4Fd(%x)\n", i4Fd);
    ...
    if (u4Length >= (u4FileSize - u4Offset))
        ...
        //printk(" pvAddr (0x%lx), u4ReadSize(%d), filesize(%d), i4Fd is 0x%lx\n",pvAddr, u4ReadSize, u4FileSize, i4Fd);
...
UINT32 u4HDDFsGetFileSize(INT32 *pi4FileId)
{
    ...
    DrvFSUSBGetFileSize(pi4FileId, &u4FileSize);

	printk(" ===> fgHDDFsGetFile %d <===\n", *pi4FileId, u4FileSize);
