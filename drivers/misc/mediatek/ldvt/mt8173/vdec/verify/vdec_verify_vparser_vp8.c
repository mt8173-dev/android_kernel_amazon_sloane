...
#if VDEC_VP8_IMAGERESIZE_SUPPORT
...
void vVP8WEBPImageResizeHandle(UINT32 u4InstID);
#endif
...
extern void vVP8WEBPOpenFile(UINT32 u4InstID);
...
//    i4VDEC_HAL_VP8_InitVDecHW(u4InstID, &rVp8VDecInitPrm);
    ...
    prVDecVp8FrmHdr->u4WritePos= (UINT32)_pucVFifo[u4InstID] + V_FIFO_SZ;
...
//  prVDecVp8FrmHdr->u4FifoEnd = VIRTUAL(prVDecVp8FrmHdrPhysical.u4FifoEnd);
...

  {
     ...
     {
        i2QValue= 0; //prVp8DecParam->QIndexInfo[u4Index];
        if(u4VDEC_VP8_VLDReadLiteral(u4InstID, 1))
        {
...
*/
    ...
    prVDecVp8FrmHdr->u4SegIdWrapper = (UINT32)_pucSegIdWrapper[u4InstID];
       ...
       prVDecVp8FrmHdr->u4TmpBuf = (UINT32)_pucWebpReszWorkBuf[u4InstID];
	   prVDecVp8FrmHdr->u4ImgReszY = (UINT32)_pucWebpReszFrameYAddr[u4InstID];
	   prVDecVp8FrmHdr->u4ImgReszC = (UINT32)_pucWebpReszFrameCAddr[u4InstID];
#endif
...
//    VDEC_INFO_DEC_PRM_T *prDecPrm = &_tVerMpvDecPrm[u4InstID];
	...
	#if 1
    {
        ...
        u4InputWindow = u4VDecReadVP8VLD(u4InstID, RO_VP8_VBSR1);
    
        printk("# %d , Input Window 1 is 0x%x, \n", _u4FileCnt[u4InstID], u4InputWindow);

	    u4InputWindow = u4VDecReadVP8VLD(u4InstID, RO_VP8_VBSR2);

	    printk("# %d , Input Window 2 is 0x%x, \n", _u4FileCnt[u4InstID], u4InputWindow);
    }
	#endif
...
//Searh path name
    ...
    {
        {
           //UTIL_Printf("VP8 Test 21\n");
    	}

	    {
           //UTIL_Printf("_pucMergeCRCBuf[u4InstID] pointer is %lx\n", _pucMergeCRCBuf[u4InstID]);
    	}
		UCHAR  *pucMergeCRCBuf = _pucMergeCRCBuf[u4InstID];
		//UTIL_Printf("VP8 Test 22\n");
		UCHAR  *pu4CRCResultBufAddr = (UCHAR *)(pucMergeCRCBuf + 32*_u4FileCnt[u4InstID]);
		//UTIL_Printf("VP8 Test 23\n");
    ...
    #else
    {
		if (_u4PicTotalCnt[u4InstID] < _u4FileCnt[u4InstID])
		{
			fgOpen = FALSE;
		}
		else
...
//#endif   
  //  CHAR _VP8GoldenList[100]= {"\\\\pc-02098\\VP8_bitstream\\"}; 
    ...
    CHAR _bFileAddStrYLen[20] = {"_len_Y.out\0"};
    CHAR _bFileAddStrCLen[20] = {"_len_CbCr.out\0"};
#else
...
/*		   for (j=filename_byte_addr; j>0 ; j--)
    ...
    if(fgCompare)
    {
        ...
        #endif    
        {
         ...
         #else
#if VDEC_VP8_IMAGERESIZE_SUPPORT
		 u4NonSwapYBase = prVDecVp8FrmHdr->u4ImgReszY;
#else
         u4NonSwapYBase = prVDecVp8FrmHdr->u4CurYAddr;
#endif

#ifndef VDEC_UFO_SUPPORT
#if VDEC_VP8_IMAGERESIZE_SUPPORT
            u4NonSwapCBase = prVDecVp8FrmHdr->u4ImgReszC;
#else
            u4NonSwapCBase = u4NonSwapYBase + PIC_Y_SZ;
#endif
#else
            u4NonSwapCBase = u4NonSwapYBase + _tVerMpvDecPrm[u4InstID].u4PIC_SIZE_Y_BS;
#endif
#endif
#ifdef VDEC_UFO_SUPPORT
            u4NonSwapYLengthBase = (UINT32)(u4NonSwapYBase + _tVerMpvDecPrm[u4InstID].u4PIC_SIZE_BS);
            u4NonSwapCLengthBase = (UINT32)(u4NonSwapYLengthBase + _tVerMpvDecPrm[u4InstID].u4UFO_LEN_SIZE_Y);
            printk("TRC-vVP8WrData2PC +2.4.1, u4NonSwapYLengthBase=0x%x, u4NonSwapCLengthBase=0x%x\n", u4NonSwapYLengthBase, u4NonSwapCLengthBase);
            printk("TRC-vVP8WrData2PC +2.4.2, u4PIC_SIZE_BS=%d, u4UFO_LEN_SIZE_Y=%d, u4PIC_SIZE_Y_BS=%d\n",
                   _tVerMpvDecPrm[u4InstID].u4PIC_SIZE_BS,
                   _tVerMpvDecPrm[u4InstID].u4UFO_LEN_SIZE_Y,
                   _tVerMpvDecPrm[u4InstID].u4PIC_SIZE_Y_BS);
#endif
            if (_tVerMpvDecPrm[u4InstID].ucAddrSwapMode != 4)
            {
                UINT32 u4AlignWidth, u4AlignHeight;
...
//    if((_u4FileCnt[u4InstID]%10) == 0 || _fgVP8DumpReg)
	...
	#if VDEC_VP8_WEBP_SUPPORT //For WEBP picure mode
	printk("[Success pic count]\n");
...
void vVP8WEBPPixComp(UINT32 u4InstID)
{
    #if ((!defined(COMP_HW_CHKSUM)) || defined(DOWN_SCALE_SUPPORT))
    UINT32 u4Cnt;
    #ifdef GOLDEN_128BIT_COMP  
        ...
        #endif    
        {
            ...
            u4Ty1 = (u4Y==0)? u4Ty0 : (u4Ty0+1);
                ...
                for (mbw=0; mbw < u4Tx1; mbw++)
                {
                   ...
                   // u4Start = ((_u4MbH%8) *(16*16))  + mbw* (16*32) + u4Offset;
                    if (_u4MbH%2)
                    {
                    ...
                    if (u4DecMbH%2)
                    {
                        u4Start = ((u4DecMbH%2)*(16*16)) + ((u4DecMbH>>1)*u4Tx1 + mbw) * (16*32);
                    }
                    else
                    {
                        u4Start = (u4DecMbH == 0) ? ((u4DecMbH*u4Tx1 + mbw) * (16*32)) : (((u4DecMbH>>1)*u4Tx1 + mbw) * (16*32));
                    }
                    pbGoldenBuf = (UCHAR*) (((UINT32) (_pucDumpYBuf[u4InstID])) + u4StartGld);
                    pbDecBuf = (UCHAR*) (u4YBase + u4Start);
                    ...
                    {             
                        ...
                        {                   
                                ...
                                else
                                {
                                    if ((*(pbDecBuf)) != (*(pbGoldenBuf)))
                                    {
                                        //printk("u4StartGld is %d, u4Start is %d, mbw is %d, u4Tx1 is %d, u4Ty1 is %d, u4Tx0 is %d, u4Ty0 is %d\n",u4StartGld, u4Start, mbw, u4Tx1, u4Ty1, u4Tx0, u4Ty0);
                                        u4Cnt ++;
                                        u4XPix = mbw * 16 + i;
                                        u4YPix = _u4MbH * 16 + j;
					                     vVDecOutputDebugString("Pic count to [%d]\n", _u4FileCnt[u4InstID]);  
									...
									else
									{
									    //u4XPix = mbw * 16 + i;
                                       // u4YPix = _u4MbH * 16 + j;
                                       // printk("Y Data Good at [x= 0x%d, y=0x%d] = 0x%x, Golden = 0x%x !!! \n", u4XPix, u4YPix, (*pbDecBuf), (*pbGoldenBuf));
									}
                                }
            ...
            #endif      
            {
                ...
                u4Ty1 = (u4Y==0)? u4Ty0 : (u4Ty0+1);
                    ...
                    for (mbw=0; mbw < u4Tx1; mbw++)
                    {
                        ...
                        //u4Start = ((_u4MbH%8)*(16*8)) + mbw* (16*16) + u4Offset;
	                    if (_u4MbH%2)
	                    {
                        ...
                        if (u4DecMbH%2)
	                    {
	                        u4Start = ((u4DecMbH%2)*(16*8)) + ((u4DecMbH>>1)*u4Tx1 + mbw) * (16*16);
	                    }
	                    else
	                    {
	                        u4Start = (u4DecMbH == 0) ? ((u4DecMbH*u4Tx1 + mbw) * (16*16)) : (((u4DecMbH>>1)*u4Tx1 + mbw) * (16*16));
	                    }
                        pbGoldenBuf = (UCHAR*) (((UINT32) (_pucDumpCBuf[u4InstID])) + u4StartGld);
                        pbDecBuf = (UCHAR*) (u4CBase + u4Start);
	...
	printk("Compare Error==> MB Row Count to [%d] \n", _u4FileCnt[u4InstID] - 1);
    fgWrMsg2PC(strMessage,strlen(strMessage),8,&_tFileListRecInfo[u4InstID]);
    }
	else
	{
        printk("After imageResize,  MB Row Count[%d] Compare OK!\n", _u4FileCnt[u4InstID] - 1);
	}
    #endif
    if(_u4FileCnt[u4InstID] >= _u4EndCompPicNum[u4InstID])
    {
...
#if 1 //((!VDEC_VER_COMPARE_CRC) && (!VP8_MB_ROW_MODE_SUPPORT))
  VDEC_INFO_VP8_FRM_HDR_T *prVDecVp8FrmHdr = &_rVDecVp8FrmHdr[u4InstID];
#endif
#if (VDEC_VP8_ERR_TEST)
  CHAR *pBsData;
...
BOOL fgIsVP8VDecComplete(UINT32 u4InstID)
{
  ...
  if(_fgVDecComplete[u4InstID])
  {
      ...
      printk("prVDecVp8FrmHdr->u4Width = 0x%x, prVDecVp8FrmHdr->u4Height = 0x%x\n", prVDecVp8FrmHdr->u4Width, prVDecVp8FrmHdr->u4Height);      
        ...
        if ((u4MbX >= (((prVDecVp8FrmHdr->u4Width + 0xF) >> 4) & 0x1FF)) && (u4MbY >= (((prVDecVp8FrmHdr->u4Height + 0xF) >> 4) & 0x1FF)))
        {
            #if VDEC_VP8_IMAGERESIZE_SUPPORT
	        printk("Test TRUE imageresize\n");
            vVP8WEBPImageResizeHandle(u4InstID);

			//vVP8WEBPPixComp(u4InstID);
	        #endif
            //printk("TRC-fgIsVP8VDecComplete +1\n");
            #if 0
            {
                char _FileList_WebpY[100]= {"D:\\ChkFolder\\Row_Webp_Y.bin"};
				char _FileList_WebpC[100]= {"D:\\ChkFolder\\Row_Webp_C.bin"};
				
			    fgWrData2PC(prVDecVp8FrmHdr->u4ImgReszY, prVDecVp8FrmHdr->u4Width * prVDecVp8FrmHdr->u4Height, 7, _FileList_WebpY);
				fgWrData2PC(prVDecVp8FrmHdr->u4ImgReszC, prVDecVp8FrmHdr->u4Width * prVDecVp8FrmHdr->u4Height / 2, 7, _FileList_WebpC);
			...
			UINT32 i;
	   	    UINT8 a,b,c,d,e,f,g,h,j,k,l,m,n,o,p,q;
			UCHAR *ucYCBase= NULL;
			
            printk("After imageResize VP8 Y Data\n");
			ucYCBase = prVDecVp8FrmHdr->u4ImgReszY;
       	    for(i=0; i < 25400 ; i=i+16)
       	    {
				a = *(UINT8 *)(ucYCBase + i);
				b = *(UINT8 *)(ucYCBase + i + 1);
				c = *(UINT8 *)(ucYCBase + i + 2);
				d = *(UINT8 *)(ucYCBase + i + 3);
				e = *(UINT8 *)(ucYCBase + i + 4);
				f = *(UINT8 *)(ucYCBase + i + 5);
				g = *(UINT8 *)(ucYCBase + i + 6);
				h = *(UINT8 *)(ucYCBase + i + 7);
				j = *(UINT8 *)(ucYCBase + i + 8);
				k = *(UINT8 *)(ucYCBase + i + 9);
				l = *(UINT8 *)(ucYCBase + i + 10);
				m = *(UINT8 *)(ucYCBase + i + 11);
				n = *(UINT8 *)(ucYCBase + i + 12);
				o = *(UINT8 *)(ucYCBase + i + 13);
				p = *(UINT8 *)(ucYCBase + i + 14);
				q = *(UINT8 *)(ucYCBase + i + 15);
      ...
      while(0 == ((u4VDecVP8ReadImgResz(0, 4 * IMG_RESZ_DONE)>>4)&0x1))//imgrsz_63[4]
      {
           printk("Wait img_resz on\n");
      }
	   
	  while(0 == (u4VDecVP8ReadImgResz(0, 4 * IMG_RESZ_DONE)&0x1))//imgresz_done
      {
          printk("Wait ImgResz Done\n");
      }
      ...
      if(((u4MbX+1)>=((prVDecVp8FrmHdr->u4Width>>4)&0x1FF))&&((u4MbY+1)>=((prVDecVp8FrmHdr->u4Height>>4)&0x1FF)))
      {
          #if 0
            {
                char _FileList_WebpY[100]= {"D:\\ChkFolder\\Webp_Y.bin"};
				char _FileList_WebpC[100]= {"D:\\ChkFolder\\Webp_C.bin"};
				
			    fgWrData2PC(prVDecVp8FrmHdr->u4ImgReszY, prVDecVp8FrmHdr->u4Width * prVDecVp8FrmHdr->u4Height, 7, _FileList_WebpY);
				fgWrData2PC(prVDecVp8FrmHdr->u4ImgReszC, prVDecVp8FrmHdr->u4Width * prVDecVp8FrmHdr->u4Height / 2, 7, _FileList_WebpC);


				vVDEC_HAL_VP8_VDec_DumpReg(0, FALSE);
...
void vVerVP8DecEndProc_MB_ROW_End(UINT32 u4InstID)
{
    ...
    }
	{
          ...
          u4HWCRC_C3 = u4VDecReadCRC(u4InstID, VDEC_CRC_C_CHKSUM3);
		   ...
		   printk("u4HWCRC_Y0 = 0x%x\n", u4HWCRC_Y0);
	       printk("u4HWCRC_Y1 = 0x%x\n", u4HWCRC_Y1);
	       printk("u4HWCRC_Y2 = 0x%x\n", u4HWCRC_Y2);
	       printk("u4HWCRC_Y3 = 0x%x\n", u4HWCRC_Y3);

	       printk("u4HWCRC_C0 = 0x%x\n", u4HWCRC_C0);
	       printk("u4HWCRC_C1 = 0x%x\n", u4HWCRC_C1);
	       printk("u4HWCRC_C2 = 0x%x\n", u4HWCRC_C2);
	       printk("u4HWCRC_C3 = 0x%x\n", u4HWCRC_C3);
    ...
    {    
      ...
      if((u4Cnt & 0x3f)== 0x3f)
      {
        ...
        if(fgIsVP8VDecComplete(u4InstID))
        {
		   ...
		   u4Value_478 = u4VDecReadMC(u4InstID, 478*4);
		   u4Value_632 = u4VDecReadMC(u4InstID, 632*4);
...
//Print LOG
    ...
    {
       sprintf(strMessage,"======");  
       printk("%s, %d\n", strMessage, _u4FileCnt[u4InstID]);
    }
#if 0//ChunChia_LOG  
  ...
  vVerVP8VDecEnd(u4InstID);
    ...
    vVDecWriteVLDTOP(u4InstID, RW_VLD_TOP_COPY_MEM_DRAM_BASE, PHYSICAL(_rVDecVp8FrmHdrAddr[u4InstID].u4SegIdWrapper));
    vVDecWriteVLDTOP(u4InstID, RW_VLD_TOP_COPY_MEM_SRAM_SETTING, RW_VLD_TOP_COPY_MEM_SRAM_SETTING_VAL_DEFAULT);
    vVDecWriteVLDTOP(u4InstID, RO_VLD_TOP_COPY_MEM_STATUS,
                               (RO_VLD_TOP_COPY_MEM_STATUS_VAL_SRAM_TO_DRAM << RO_VLD_TOP_COPY_MEM_STATUS_FLD_COPY_MODE) |
                               (1 << RO_VLD_TOP_COPY_MEM_STATUS_FLD_COPY_TRIG));
    // Wait for seg_id data copy completion
    while (u4VDecReadVLDTOP(u4InstID, RO_VLD_TOP_COPY_MEM_STATUS) & (1 << RO_VLD_TOP_COPY_MEM_STATUS_FLD_COPY_BUSY));
#endif
...
void vVP8WEBPImageResizeHandle(UINT32 u4InstID)
{
    ...
    UINT32 u4PicPitch = 0;
	UINT32 u4VdoPPOn = 0;
	BOOL fgIsLastRow = FALSE;
	UINT32 u4AlignWidth = ((prVDecVp8FrmHdr->u4Width+15)>>4)<<4;
    UINT32 u4ALignHeight = ((prVDecVp8FrmHdr->u4Height+15)>>4)<<4;
  
	u4MbX_last = u4MbX;
	u4MbY_last = u4MbY;
     
	u4MbY = u4VDecReadVP8VLD(0,RO_VP8_XY)&0xFFFF;//vld_95[15:0]
	u4MbX = (u4VDecReadVP8VLD(0,RO_VP8_XY)>>16)&0xFFFF;//vld_95[31:16]

	UINT32 u4Num = 0;

#if 1//log
	printk("Total Mbx = %d, Total Mby = %d\n", (u4AlignWidth+15)/16, (u4ALignHeight+15)/16);
	printk("Mby = 0x%x, Toal Mby 0x%x\n", u4VDecReadVP8VLD(0,RO_VP8_XY)&0xFFFF, (u4ALignHeight+15)/16);
	printk("@@@@MCU row will decode:MB_Y 0x%x, Total MB_row 0x%x\n", u4MbY, (u4ALignHeight+15)/16);
	printk("brc init: vp8_vld_92: %d\n", u4VDecReadVP8VLD(u4InstID,RO_VP8_VOKR));
	printk("brc init: multi_token_partition: %d\n", (u4VDecReadVP8VLD(u4InstID,RW_VP8_HEADER3)>>12)&0x3);
	printk("Ring_Fifo, brc_int input_window: 0x%x\n", u4VDecReadVP8VLD(u4InstID, RO_VP8_VBSR1));
#endif

    if ( ((u4AlignWidth+15)/16 == 1) || ((u4ALignHeight+15)/16 == 1))
	{
        printk("Total Mbx is 1, Total Mby is 1,  Do not require do imageResize\n");
	}

	printk("Misc_41 Value is 0x%x\n", u4VDecReadDV(0, RW_VP8_CTRL));
    while(0 == ((u4VDecReadDV(0, RW_VP8_CTRL)>>16)&0x1))//misc_41[16]
    {
        printk("Wait vdec_int done\n");
    }
	...
	if (_fgLastMBRow)
    {
        ...
        vVDecVP8WriteImgResz(0, 4 * IMG_RESZ_JPG_MODE, 0x1D100);
        vVDecVP8WriteImgResz(0, 4 * IMG_RESZ_START, 0x83);
        vVDecVP8WriteImgResz(0, 4 * IMG_RESZ_START, 0x82);
        while(0 == ((u4VDecVP8ReadImgResz(0, 4 * IMG_RESZ_DONE)>>4)&0x1))//imgrsz_63[4]
        {
            printk("Wait img_resz on\n");
        }
        #if 0
	    {
		    ...
		    fgWrData2PC(prVDecVp8FrmHdr->u4ImgReszY, 176 * 144, 7, _FileList_WebpY);
			fgWrData2PC(prVDecVp8FrmHdr->u4ImgReszC, 176 * 144 / 2, 7, _FileList_WebpC);
    ...
    else
    {
	    ...
	    vVDecWriteDV(0, RW_VP8_CTRL,0x1011);//clear vdec_init
	    vVDecWriteDV(0, RW_VP8_CTRL,0x1001);
	    while(0 == ((u4VDecVP8ReadImgResz(0, 4 * IMG_RESZ_DONE)>>4)&0x1))//imgrsz_63[4]
	    {
	        printk("Wait img_resz on\n");
	    }
	    if (!_fgFirstMBRow)
	    {
	        while(0 == (u4VDecVP8ReadImgResz(0, 4 * IMG_RESZ_DONE)&0x1))//imgresz_done
	        {
	            ...
	            //UTIL_Printf("Imgeresz on\n");
	    if (_fgFirstMBRow)
	    {
	        printk("First MB Row\n");
	        u4PicPitch = u4AlignWidth>>4;

	        vVDecVP8WriteImgResz(0, 4 * IMG_RESZ_START, 0x8c);
	        vVDecVP8WriteImgResz(0, 4 * RZ_JPEG_INT_SWITCH, 0x0);//jpg_init_switch row mode 0x0
	        u4VdoPPOn = u4VDecReadVP8MC(0, RW_MC_PP_DBLK_MODE);
	        vVDecVP8WriteImgResz(0, 4 * IMG_RESZ_TYPE,((((u4VdoPPOn>>1)&0x1)|(u4VdoPPOn&0x1))<<7)|1<<6);//IMG_RESZ_TYPE: pp-on ->0xc0, pp-off->0x40         
	        vVDecVP8WriteImgResz(0, 4 * IMG_RESZ_JPG_MODE, 0x1E100);//JPG_MODE
	        //tmp DRAM address
	        vVDecVP8WriteImgResz(0, 4 * RZ_TMP_Y_OW_ADDR_BASE,(UINT32)((UINT32)PHYSICAL(prVDecVp8FrmHdr->u4TmpBuf)>>4));
	        //source buffer length
	        vVDecVP8WriteImgResz(0, 4 * RZ_SRC_BUF_LEN, ((u4PicPitch>>1)<<12)|u4PicPitch);
	        //target buffer length
	        vVDecVP8WriteImgResz(0,4 * RZ_TG_BUF_LEN,(1<<31|1<<30|0<<29|0x10<<24|(u4PicPitch<<12)|u4PicPitch));
	        //source base address
	        vVDecVP8WriteImgResz(0, 4 * RZ_SRC_Y_OW_ADDR_BASE1, (UINT32)((UINT32)PHYSICAL(prVDecVp8FrmHdr->u4CurYAddr)>>4));//OUT_BUF_ADDR_Y>>4
	        vVDecVP8WriteImgResz(0, 4 * RZ_SRC_CB_OW_ADDR_BASE1, (UINT32)((UINT32)PHYSICAL(prVDecVp8FrmHdr->u4CurYAddr+PIC_Y_SZ)>>4));//OUT_BUF_ADDR_C>>4
	        vVDecVP8WriteImgResz(0, 4 * RZ_SRC_CR_OW_ADDR_BASE1, (UINT32)((UINT32)PHYSICAL(prVDecVp8FrmHdr->u4CurYAddr+PIC_Y_SZ)>>4));//OUT_BUF_ADDR_C>>4
	        //target base address
	        vVDecVP8WriteImgResz(0, 4 * RZ_TG_Y_OW_ADDR_BASE, (UINT32)((UINT32)PHYSICAL(prVDecVp8FrmHdr->u4ImgReszY)>>4));//RESZ_FRAME_ADDR_Y>>4
	        vVDecVP8WriteImgResz(0, 4 * RZ_TG_C_OW_ADDR_BASE, (UINT32)((UINT32)PHYSICAL(prVDecVp8FrmHdr->u4ImgReszC)>>4));//RESZ_FRAME_ADDR_C>>4
	        //source widhht/height
	        vVDecVP8WriteImgResz(0, 4 * RZ_SRC_SIZE_Y, (u4AlignWidth<<16)|u4ALignHeight);//value" qw{widht, height}
	        vVDecVP8WriteImgResz(0, 4 * RZ_SRC_SIZE_CB, (u4AlignWidth>>1)<<16|(u4ALignHeight>>1));//value" qw{}
	        vVDecVP8WriteImgResz(0, 4 * RZ_SRC_SIZE_CR, (u4AlignWidth>>1)<<16|(u4ALignHeight>>1));//value" qw{}
	        //target wdith/height
	        vVDecVP8WriteImgResz(0, 4 * RZ_TG_SIZE_Y, (u4AlignWidth<<16)|u4ALignHeight);//value" qw{}
	        vVDecVP8WriteImgResz(0, 4 * IMG_RESZ_START, 0x8c);//value" qw{}
	        vVDecVP8WriteImgResz(0, 4 * IMG_RESZ_START, 0x82);
	        vVDecVP8WriteImgResz(0, 4 * IMG_RESZ_START, 0x83);
	        vVDecVP8WriteImgResz(0, 4 * IMG_RESZ_START, 0x82);
	        _fgFirstMBRow = FALSE;
	    ...
	    {
	        // UTIL_Printf("Normal MB Row\n");    
	        vVDecVP8WriteImgResz(0, 4 * IMG_RESZ_JPG_MODE, 0x1C100);               
	        vVDecVP8WriteImgResz(0, 4 * IMG_RESZ_START, 0x83);               
	        vVDecVP8WriteImgResz(0, 4 * IMG_RESZ_START, 0x82);

	    }    
	
	    if ((u4MbY+1)>=(u4ALignHeight>>4))
	    {
	        printk("Last MB Row\n");
	        _fgLastMBRow = TRUE;
	        while(0 == ((u4VDecVP8ReadImgResz(0, 4 * IMG_RESZ_DONE)>>4)&0x1))//imgrsz_63[4]
        ...
        #if 0
		printk("ImgRez 119 value is 0x%x\n", u4VDecVP8ReadImgResz(0, 4 * 119));
		printk("ImgRez 120 value is 0x%x\n", u4VDecVP8ReadImgResz(0, 4 * 120));
		printk("ImgRez 121 value is 0x%x\n", u4VDecVP8ReadImgResz(0, 4 * 121));
		printk("ImgRez 122 value is 0x%x\n", u4VDecVP8ReadImgResz(0, 4 * 122));

		for (u4Num = 0; u4Num < 122; u4Num ++)
		{
            u4VDecVP8ReadImgResz(0, 4 * u4Num);
		}
		#endif
    }
	
   #if 1// print input window
   printk("Ring_Fifo, row_trig input_window: 0x%x\n", u4VDecReadVP8VLD(u4InstID, RO_VP8_VBSR1));
   printk("row_trig_vdec input_window_sec: 0x%x\n", u4VDecReadVP8VLD(u4InstID, RO_VP8_VBSR1));
   #endif
		   ...
		   
		   {
	        ...
	        printk("Before imageResize VP8 Y Data\n");
			ucYCBase = prVDecVp8FrmHdr->u4CurYAddr;
       	    for(i=0; i < 9216 ; i=i+16)
       	    {
				a = *(UINT8 *)(ucYCBase + i);
				b = *(UINT8 *)(ucYCBase + i + 1);
				c = *(UINT8 *)(ucYCBase + i + 2);
            ...
            #if 0
            UTIL_Printf("Before ImageResize VP8 C Data\n");
			ucYCBase = prVDecVp8FrmHdr->u4CurCAddr;;
            ...
            #if 0
            UTIL_Printf("After ImageResize VP8 C Data\n");
			ucYCBase = prVDecVp8FrmHdr->u4ImgReszC;
