...
void * x_alloc_aligned_verify_mem_ex(UINT32 u4Size, UINT32 u4Align, BOOL fgChannelA, const char *szFunction, INT32 i4Line)
{
    ...
    if (g_u4AllocSize + u4Size < 0xB400000)
...
//    printk("CRCBUF_SA = 0x%x\n", CRCBUF_SA);
  ...
  printk("_pucCRCBuf[u4InstID] = 0x%x 0x%x 0x%x\n", _pucCRCBuf[u4InstID], CRCBUF_SA, CRC_SZ);
  ...
  
    {
        ...
        printk("// <buffer> _pucDumpYBuf[u4InstID] = 32'h%x GOLD_Y_SZ 0x%x\n", PHYSICAL(_pucDumpYBuf[u4InstID]), GOLD_Y_SZ);
...

    {
        ...
        printk("// <buffer> _pucDumpCBuf[u4InstID] = 32'h%x GOLD_C_SZ 0x%x\n", PHYSICAL(_pucDumpCBuf[u4InstID]), GOLD_C_SZ);
    ...
    DBG_H264_PRINTF("_pucH265CRCYBuf0[u4InstID] = 0x%x\n", _pucH265CRCYBuf0[u4InstID]);
    DBG_H264_PRINTF("_pucH265CRCCBuf0[u4InstID] = 0x%x\n", _pucH265CRCCBuf0[u4InstID]);   
    DBG_H264_PRINTF("_pucH265CRCYBuf1[u4InstID] = 0x%x\n", _pucH265CRCYBuf1[u4InstID]);
    DBG_H264_PRINTF("_pucH265CRCCBuf1[u4InstID] = 0x%x\n", _pucH265CRCCBuf1[u4InstID]);
#else
...
void vMemoryFree(UINT32 u4InstID)
{
  ...
  if(_ucMVCType[u4InstID] != 1)
	   ...
	   #if VDEC_SUPPORT_MERGE_CRC
       iounmap(_pucMergeCRCBuf[u4InstID]);
       iounmap(_pucPicTolCnt[u4InstID]);
       #endif       
  ...
  #if VDEC_SUPPORT_MERGE_CRC
  vMemoryFreeLoop(_pucMergeCRCBuf[u4InstID], GOLD_CHANEL_A);
  vMemoryFreeLoop(_pucPicTolCnt[u4InstID], 16);
  #endif
...
#if 1 //HUB TEST
        _pucVLDPredBuf[u4InstID] = (UCHAR *)x_alloc_aligned_verify_mem(PRED_SZ, 256, WORKING_AREA_CHANEL_A);
        _pucVLDIPBuf[u4InstID] = (UCHAR *)x_alloc_aligned_verify_mem(IP_SZ, 256, WORKING_AREA_CHANEL_A);
        _pucVLDVBuf[u4InstID] = (UCHAR *)x_alloc_aligned_verify_mem(VBUF_SZ, 256, WORKING_AREA_CHANEL_A);
        _pucPPSAOBuf[u4InstID] = (UCHAR *)x_alloc_aligned_verify_mem(SAO_SZ, 256, WORKING_AREA_CHANEL_A);
        _pucPPDBKBuf[u4InstID] = (UCHAR *)x_alloc_aligned_verify_mem(DBK_SZ, 256, WORKING_AREA_CHANEL_A);
#else
...
#endif
        printk("_pucVLDPredBuf :0x%X\n", _pucVLDPredBuf[u4InstID]);
        printk("_pucVLDIPBuf :0x%X\n", _pucVLDIPBuf[u4InstID]);
        printk("_pucVLDVBuf :0x%X\n", _pucVLDVBuf[u4InstID]);
        printk("_pucPPSAOBuf :0x%X\n", _pucPPSAOBuf[u4InstID]);
        printk("_pucPPDBKBuf :0x%X\n", _pucPPDBKBuf[u4InstID]);
#endif
    }
  ...
  if (_u4CodecVer[u4InstID] == VDEC_VP8)
{
	   ...
	   _pucWebpReszWorkBuf[u4InstID] = (UCHAR *)x_alloc_aligned_verify_mem(WEBP_RESZ_TMP_SIZE, 2048, WORKING_AREA_CHANEL_A);
       memset(_pucWebpReszWorkBuf[u4InstID] ,0,WEBP_RESZ_TMP_SIZE);
	   
       _pucWebpReszFrameYAddr[u4InstID] = (UCHAR *)x_alloc_aligned_verify_mem(WEBP_RESZ_Y_SIZE, 2048, WORKING_AREA_CHANEL_A);
	   memset(_pucWebpReszFrameYAddr[u4InstID] ,0,WEBP_RESZ_Y_SIZE);
	   
       _pucWebpReszFrameCAddr[u4InstID] = (UCHAR *)x_alloc_aligned_verify_mem(WEBP_RESZ_C_SIZE, 2048, WORKING_AREA_CHANEL_A);
	   memset(_pucWebpReszFrameCAddr[u4InstID] ,0,WEBP_RESZ_C_SIZE);
#endif
...
#ifdef HEVC_STILL_IMAGE
        x_free_aligned_verify_mem(_pucVLDPredBuf[u4InstID], WORKING_AREA_CHANEL_A);
        g_u4AllocSize -= PRED_SZ;
        x_free_aligned_verify_mem(_pucVLDIPBuf[u4InstID], WORKING_AREA_CHANEL_A);
        g_u4AllocSize -= IP_SZ;
        x_free_aligned_verify_mem(_pucVLDVBuf[u4InstID], WORKING_AREA_CHANEL_A);
        g_u4AllocSize -= VBUF_SZ;
        x_free_aligned_verify_mem(_pucPPSAOBuf[u4InstID], WORKING_AREA_CHANEL_A);
        g_u4AllocSize -= SAO_SZ;
        x_free_aligned_verify_mem(_pucPPDBKBuf[u4InstID], WORKING_AREA_CHANEL_A);
        g_u4AllocSize -= DBK_SZ;
#endif
