...
INT32 vHEVCCheckAllRefPicsAreAvailable(UINT32 u4InstID, VDEC_INFO_DEC_PRM_T *tVerMpvDecPrm)
{
    ...
    pSPS = tVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prSPS;
...
*/           
            if( 1 == pCurrRPS->bCheckLTMSB[i] ) {
                ...
                if(( _ptH265FBufInfo[u4InstID][j].bUsedAsLongTerm) && (_ptH265FBufInfo[u4InstID][j].i4POC%(1<<(pSPS->u4Log2MaxPicOrderCntLsbMinus4+4))) == pCurrRPS->i4POC[i]%(1<<(pSPS->u4Log2MaxPicOrderCntLsbMinus4+4)) )
                    isAvailable = 1;
            }
        } 
        ...
        if(!isAvailable) {
            for( j=0; j<_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.ucMaxFBufNum; j++) {
                if( 1 == pCurrRPS->bCheckLTMSB[i] ) {
                    ...
                    if ( ((_ptH265FBufInfo[u4InstID][j].i4POC%(1<<(pSPS ->u4Log2MaxPicOrderCntLsbMinus4+4))) == (pCurrRPS->i4POC[i]%(1<<(pSPS ->u4Log2MaxPicOrderCntLsbMinus4+4)))) && _ptH265FBufInfo[u4InstID][j].bFirstSliceReferenced){
                        isAvailable = 1;
                        _ptH265FBufInfo[u4InstID][j].ucFBufRefType = LREF_PIC;
                        _ptH265FBufInfo[u4InstID][j].bUsedAsLongTerm = 1;        
                        //printk("[DEBUG] FB index %d pCurrRPS->i4POC[%d] = %d mark as LT\n", j, i,pCurrRPS->i4POC[i]);
...
UINT32 vHEVCVerifyVDecSetPicInfo(UINT32 u4InstID, VDEC_INFO_DEC_PRM_T *ptVerMpvDecPrm)
{
    ...
    vVDEC_HAL_H265_InitPicInfo(u4InstID);
...
#else
    vVDEC_HAL_H265_SetSHDRHEVLD(u4InstID, ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prSPS,
                                      ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prSliceHdr,
                                      ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prSPS->bUseSAO,
                                      ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prPPS);
#endif
    ...
    ptVerMpvDecPrm->u4PicW = ((ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prSPS->u4PicWidthInLumaSamples + 63) >> 6) << 6;
    ptVerMpvDecPrm->u4PicH = ((ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prSPS->u4PicHeightInLumaSamples + 63) >> 6) << 6;

    if ((ptVerMpvDecPrm->u4PicW > 4160) || (ptVerMpvDecPrm->u4PicH > 2304))
    {
...
void vHEVCVerifyPrepareFBufInfo(UINT32 u4InstID, VDEC_INFO_DEC_PRM_T *tVerMpvDecPrm)
{
    ...
    if ((tVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prSPS->u4PicWidthInLumaSamples != tVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.rLastInfo.u4LastPicWidthInLumaSamples)
        || (tVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prSPS->u4PicHeightInLumaSamples != tVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.rLastInfo.u4LastPicHeightInLumaSamples))
    {
        printk("[INFO] Width (%d) -> (%d) Height (%d) -> (%d)\n", tVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.rLastInfo.u4LastPicWidthInLumaSamples,
            tVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prSPS->u4PicWidthInLumaSamples, tVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.rLastInfo.u4LastPicHeightInLumaSamples,
            tVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prSPS->u4PicHeightInLumaSamples);
#if !HEVC_CRC_COMPARE
...
void vHEVCVDecApplyRefPicList(UINT32 u4InstID, VDEC_INFO_DEC_PRM_T *tVerMpvDecPrm)
{
    ...
    for(j=0; j<_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.ucMaxFBufNum; j++){
        ...
        if(!_ptH265FBufInfo[u4InstID][j].bFirstSliceReferenced )
        {
            continue;
        }
        ...
        for(;i<pCurrRPS->u4NumberOfPictures;i++)
        {
            ...
            {
                if((_ptH265FBufInfo[u4InstID][j].ucFBufRefType==LREF_PIC) && (_ptH265FBufInfo[u4InstID][j].i4POC%(1<<(pSPS->u4Log2MaxPicOrderCntLsbMinus4+4))) == pCurrRPS->i4POC[i]%(1<<(pSPS->u4Log2MaxPicOrderCntLsbMinus4+4)) )
                {
                    isReference = 1;
                    _ptH265FBufInfo[u4InstID][j].bUsedByCurr = pCurrRPS->bUsed[i];
                    _rH265PicInfo[u4InstID].i4DpbLTBuffId[_rH265PicInfo[u4InstID].i4DpbLTBuffCnt] = j;
...
#if DEBUG_LEVEL >= DBG_LEVEL_INFO
            printk ("[INFO] _ptH265FBufInfo index = %d marked as NREF_PIC (cleared) !!\n", j );
#endif
...
UINT32 vHEVCVDecSetRefPicList(UINT32 u4InstID, VDEC_INFO_DEC_PRM_T *tVerMpvDecPrm)
{
    ...
    _ptH265RefPicList[u4InstID].u4RefPicCnt = numPocTotalCurr;
...
#if 1 //HUB MT8137 HEVC
{
    ...
    UINT32 u4DramPicYSize,u4DramPicY_Y_LENSize;
    UINT32 u4DramPicCSize,u4DramPicC_C_LENSize;
    UINT32 u4CMemStartOffset;
    UINT32 u4MVMemStartOffset;
    ...
    u4DramMvSize = (((( u4DramPicSize >> 8) << 4) + 127 ) >> 7 ) << 7;
    ...
    if (0)
    {
        ...
        u4PicSizeYBS = ((u4DramPicSize + 8191) >>13) <<13;
        u4PicSizeBS = ((u4PicSizeYBS + (u4DramPicSize >>1) + 511) >>9) <<9;
        u4DramPicYCSize  = (((u4PicSizeBS + (u4UFOLenYsize << 1)) + 511) >> 9) <<9;     //512 align    
        u4DramPicArea = ((u4DramPicYCSize +  u4DramMvSize + 8191) >> 13) <<13;         //8192 align

        u4UFOLenYsize = ((((u4DramPicSize + 255) >> 8);
        u4UFOLenCsize = (((u4UFOLenYsize >> 1 );
        u4PicSizeBS = ((u4PicSizeYBS + (u4DramPicSize >>1) + 511) >>9) <<9;
        u4DramPicYCSize  = (((u4PicSizeBS + (u4UFOLenYsize << 1)) + 511) >> 9) <<9;     //512 align    
        u4DramPicArea = ((u4DramPicYCSize +  8191) >> 13) <<13;         //8192 align    
#endif
    } 
    else 
    {
        if (_u4Main10Mode == 0) // normal 8-bit
        {
            u4DramPicYSize = ((u4DramPicSize + 1023 ) >> 10 ) << 10;   // 1024 align
            u4DramPicCSize = (((u4DramPicSize >> 1) + 511) >> 9 ) << 9;   // 512 align
        }    
        else if (_u4Main10Mode == HEVC_16BIT_MODE || _u4Main10Mode == HEVC_REORDER_MODE) // 16-bit & reorder mode
        {
#if 0
            u4DramPicYCSize = ((u4DramPicSize* 3 + 511) >> 9 )<< 9;   //512 align     
            u4DramPicSize = u4DramPicSize << 1;        
#endif
            u4DramPicYSize = (((u4DramPicSize* 2) + 1023) >> 10 )<< 10;   //1024 align                        
            u4DramPicCSize = ((u4DramPicSize + 511) >> 9 ) << 9;   // 512 align
        }
        else // HEVC_COMPACT_MODE
        {
#if 0
            u4DramPicYCSize = ((u4DramPicSize* 3/2*5/4 + 511) >> 9 )<< 9;   //512 align        
            u4DramPicSize = u4DramPicSize *5/4;
#endif
            u4DramPicYSize = (((u4DramPicSize * 5/4) + 1023) >> 10 ) << 10;   //1024 align                        
            u4DramPicCSize = ((((u4DramPicSize * 5/4) >> 1) + 511) >> 9 ) << 9;   // 512 align
        }
        
        u4DramPicArea =  (( u4DramPicYSize +  u4DramPicCSize + u4DramMvSize));

        if (bIsUFO)
        {    
            u4UFOLenYsize = ((u4DramPicYSize + 255) >> 8);
            u4UFOLenCsize = (u4UFOLenYsize >> 1 );
            // Y_BS + Y_LES Block 
            u4DramPicY_Y_LENSize  = (((u4DramPicYSize + u4UFOLenYsize) + 8191) >> 13) << 13;     //512 align    
            u4DramPicC_C_LENSize  = (((u4DramPicCSize + u4UFOLenCsize) + 8191) >> 13) << 13;     //512 align

            u4DramPicArea = ((u4DramPicY_Y_LENSize +  u4DramPicC_C_LENSize + u4DramMvSize));
        }
    }

    _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.ucMaxFBufNum = DPB_SZ / u4DramPicArea;

    u4CMemStartOffset = u4DramPicYSize * _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.ucMaxFBufNum ;
    u4MVMemStartOffset = u4DramPicCSize * _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.ucMaxFBufNum ;
    
    u4CStartAddr = ((UINT32)_pucDPB[u4InstID]) + u4CMemStartOffset;
    u4MVStartAddr =  u4CStartAddr + u4MVMemStartOffset;

    if (bIsUFO)
    {
        u4CMemStartOffset = u4DramPicY_Y_LENSize * _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.ucMaxFBufNum ;        
        u4MVMemStartOffset = u4DramPicC_C_LENSize * _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.ucMaxFBufNum ;
    }
    
    printk("// [VDEC_VER] PicW = %d, PicH = %d, u4DramPicArea = %d. MaxFBufNum = %d\n", _tVerMpvDecPrm[u4InstID].u4PicW, _tVerMpvDecPrm[u4InstID].u4PicH,  u4DramPicArea, _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.ucMaxFBufNum);
    printk("// [VDEC_VER] _u4Main10Mode %d \n", _u4Main10Mode);

    printk("// [VDEC_VER] u4CMemStartOffset 0x%x, u4MVMemStartOffset 0x%x\n",( u4CMemStartOffset),( u4MVMemStartOffset));
    printk("// [VDEC_VER] u4CMemStart PHY 0x%x, u4MVMemStart PHY 0x%x\n",PHYSICAL( u4CStartAddr),PHYSICAL( u4MVStartAddr));

    printk("// [VDEC_VER  u4DramPicYSize  0x%x, u4DramPicCSize  0x%x\n",u4DramPicYSize, u4DramPicCSize);
    printk("// [VDEC_VER] u4DramPicY_Y_LENSize  0x%x, u4DramPicC_C_LENSize  0x%x\n", u4DramPicY_Y_LENSize,u4DramPicC_C_LENSize);
    printk("// [VDEC_VER] u4UFOLenYsize  0x%x, u4UFOLenCsize  0x%x\n", u4UFOLenYsize,u4UFOLenCsize);
    ...
    {
        printk("// [VDEC_VER] H265 DPB Size Not Enough!!!!!\n");
        printk("// [VDEC_VER] H265 DPB Size Not Enough!!!!!\n");
        printk("// [VDEC_VER] H265 DPB Size Not Enough!!!!!\n");
        printk("// [VDEC_VER] H265 DPB Size Not Enough!!!!!\n");
        printk("// [VDEC_VER] H265 DPB Size Not Enough!!!!!\n");
    ...
    printk("// [H265] Inst%d, u4DramPicSize:0x%x, u4DramPicArea:0x%x\n", u4InstID, u4DramPicSize, u4DramPicArea);
    
    if (bIsUFO){
        printk("// [H265]    u4UFOLenYsize:0x%x, u4UFOLenCsize:0x%x, u4PicSizeYBS:0x%x, u4PicSizeBS:0x%x\n", u4UFOLenYsize, u4UFOLenCsize, u4PicSizeYBS, u4PicSizeBS );
    }
    ...
    for (i = 0; i < _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.ucMaxFBufNum; i++)
    {
        ...
        _ptH265FBufInfo[u4InstID][i].u4Addr = ((UINT32)_pucDPB[u4InstID]) + (i * u4DramPicYSize);

        _ptH265FBufInfo[u4InstID][i].u4YStartAddr = _ptH265FBufInfo[u4InstID][i].u4Addr;    
        _ptH265FBufInfo[u4InstID][i].u4CStartAddr = ((UINT32)_pucDPB[u4InstID]) + u4CMemStartOffset + (i * u4DramPicCSize);
        _ptH265FBufInfo[u4InstID][i].u4CAddrOffset = u4DramPicSize;
        _ptH265FBufInfo[u4InstID][i].u4DramYSize= u4DramPicYSize;
        _ptH265FBufInfo[u4InstID][i].u4DramCSize= u4DramPicCSize;

#if 0
        printk("[H265] Inst%d, buf %d use frame Buffer 0x%x (0x%x), C Addr 0x%x (0x%x)\n", u4InstID, i, _ptH265FBufInfo[u4InstID][i].u4Addr, PHYSICAL(_ptH265FBufInfo[u4InstID][i].u4Addr),
        _ptH265FBufInfo[u4InstID][i].u4CAddrOffset ,PHYSICAL(_ptH265FBufInfo[u4InstID][i].u4CAddrOffset));
#endif

        if (bIsUFO)
        {
            _ptH265FBufInfo[u4InstID][i].u4Addr = ((UINT32)_pucDPB[u4InstID]) + (i * u4DramPicY_Y_LENSize);
            _ptH265FBufInfo[u4InstID][i].u4YStartAddr = _ptH265FBufInfo[u4InstID][i].u4Addr;            
            _ptH265FBufInfo[u4InstID][i].u4CStartAddr = ((UINT32)_pucDPB[u4InstID]) + u4CMemStartOffset + (i * u4DramPicC_C_LENSize);        
            ...
            _ptH265FBufInfo[u4InstID][i].u4YLenStartAddr = _ptH265FBufInfo[u4InstID][i].u4YStartAddr + u4DramPicYSize;
            _ptH265FBufInfo[u4InstID][i].u4CLenStartAddr = _ptH265FBufInfo[u4InstID][i].u4CStartAddr + u4DramPicCSize;
            _ptH265FBufInfo[u4InstID][i].u4DramPicY_Y_LENSize = u4DramPicY_Y_LENSize;
            _ptH265FBufInfo[u4InstID][i].u4DramPicC_C_LENSize = u4DramPicC_C_LENSize;

            printk("// [H265] Inst%d, buf %d use Y UFO Buffer 0x%x (0x%x), C UFO Buffer 0x%x (0x%x) \n", u4InstID, i, _ptH265FBufInfo[u4InstID][i].u4YLenStartAddr, PHYSICAL(_ptH265FBufInfo[u4InstID][i].u4YLenStartAddr),
            _ptH265FBufInfo[u4InstID][i].u4CLenStartAddr, PHYSICAL(_ptH265FBufInfo[u4InstID][i].u4CLenStartAddr));
        }
        printk("// [H265] Inst%d, buf %d use frame Buffer 0x%x (0x%x), C Addr 0x%x (0x%x)\n", u4InstID, i, _ptH265FBufInfo[u4InstID][i].u4Addr, PHYSICAL(_ptH265FBufInfo[u4InstID][i].u4Addr),
        _ptH265FBufInfo[u4InstID][i].u4CStartAddr  ,PHYSICAL(_ptH265FBufInfo[u4InstID][i].u4CStartAddr));
...
#endif
        _ptH265FBufInfo[u4InstID][i].u4MvStartAddr =  ((UINT32)_pucDPB[u4InstID]) + u4CMemStartOffset+ u4MVMemStartOffset +(u4DramMvSize * i);
        printk("// [H265] Inst%d, buf %d use large B MV Buffer 0x%x (0x%x) \n", u4InstID, i, _ptH265FBufInfo[u4InstID][i].u4MvStartAddr,PHYSICAL(_ptH265FBufInfo[u4InstID][i].u4MvStartAddr));    
    }
...
#else
{
    ...
    printk("[VDEC_VER] PicW = %d, PicH = %d, u4DramPicArea = %d. MaxFBufNum = %d\n", _tVerMpvDecPrm[u4InstID].u4PicW, _tVerMpvDecPrm[u4InstID].u4PicH,  
        u4DramPicArea, _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.ucMaxFBufNum);
    ...
    for(i=0; i<_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.ucMaxFBufNum; i++)
    {
        ...
        printk("[H265] Inst%d, buf %d use frame Buffer 0x%x (0x%x)\n", u4InstID, i, _ptH265FBufInfo[u4InstID][i].u4Addr, PHYSICAL(_ptH265FBufInfo[u4InstID][i].u4Addr));
        ...
        if (bIsUFO){
            ...
            printk("[H265] Inst%d, buf %d use UFO Buffer 0x%x (0x%x)\n", u4InstID, i, _ptH265FBufInfo[u4InstID][i].u4YLenStartAddr, PHYSICAL(_ptH265FBufInfo[u4InstID][i].u4YLenStartAddr));
        }
...
void vHEVCSetCurrFBufIdx(UINT32 u4InstID, UINT32 u4DecFBufIdx)
{
   ...
   if (_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.bIsUFOMode){                     //UFO mode clear Dram for golden compare
        memset(_ptH265CurrFBufInfo[u4InstID]->u4YStartAddr , 0, _ptH265CurrFBufInfo[u4InstID]->u4DramPicY_Y_LENSize);
        memset(_ptH265CurrFBufInfo[u4InstID]->u4CStartAddr , 0, _ptH265CurrFBufInfo[u4InstID]->u4DramPicC_C_LENSize);
   }
}
...
UINT32 vHEVCSearchRealPic(UINT32 u4InstID)
{
    ...
    do
    {
        ...
        i4VDEC_HAL_H265_InitVDecHW(u4InstID); /// < RW_SYS_CLK_SEL
...
UINT32 vHEVCVerifySPS_Rbsp(UINT32 u4InstID)
{
    ...
    vHEVCVerifyPTL_Rbsp(_u4BSID[u4InstID], &(rTmpPTL), rTmpPTL.bProfilePresentFlag, u4TmpMaxTLayersMinus1); ///< profile_tier_level()
...
#endif
#if 1 //HUB MT8127 HEVC
    if (_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.rLastInfo.u4LastPicWidthInLumaSamples != pH265_SPS_DataInst->u4PicWidthInLumaSamples)
        _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.rLastInfo.u4LastPicWidthInLumaSamples = pH265_SPS_DataInst->u4PicWidthInLumaSamples;
#endif
...
#endif
#if 1 //HUB MT8127 HEVC
    if (_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.rLastInfo.u4LastPicHeightInLumaSamples != pH265_SPS_DataInst->u4PicHeightInLumaSamples)
        _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.rLastInfo.u4LastPicHeightInLumaSamples = pH265_SPS_DataInst->u4PicHeightInLumaSamples;
#endif
...
#if 1 //HUB MT8137 HEVC
     // only support 0, 2, 4 (8,10,12 bit)
    if (pH265_SPS_DataInst->u4BitDepthLumaMinus8 & 0x1 || pH265_SPS_DataInst->u4BitDepthChromaMinus8 & 0x1 )
    {         
#if DEBUG_LEVEL >= DBG_LEVEL_BUG1
        DBG_H265_PRINTF(pfLogFile,"[ERROR] bit-depth not support!! \n" );
#endif
        printk("// [ERROR] bit-depth not support!! \n" );
        //return NOT_SUPPORT;
        H265_DRV_PARSE_SET_ERR_RET(-19);
        return SPS_SYNTAX_ERROR; 
    }
    else if(pH265_SPS_DataInst->u4BitDepthLumaMinus8 == 2 && pH265_SPS_DataInst->u4BitDepthChromaMinus8 == 2)
    {
       printk("// bit-depth == 10, Trigger 10-bit Compact mode!!\n");
       _u4Main10Mode = HEVC_COMPACT_MODE;
    }
#else
    ...
    if (pH265_SPS_DataInst->bScalingListFlag)
    {
        ...
        {
            if (vHEVCVerifySL_Rbsp(_u4BSID[u4InstID], &(pH265_SPS_DataInst->rSPS_ScalingList)) != PARSE_OK) //Scaling List
            {
    ...
    {   
        if (NULL == pH265_SPS_DataInst->pShortTermRefPicSets[u4Idx])
        {
            pH265_SPS_DataInst->pShortTermRefPicSets[u4Idx] = (pH265_RPS_Data) vmalloc(sizeof(H265_RPS_Data));
        }
        
        if (vHEVCVerifyRPS_Rbsp(_u4BSID[u4InstID], pH265_SPS_DataInst, pH265_SPS_DataInst->pShortTermRefPicSets[u4Idx], u4Idx) != PARSE_OK)
        {
#if DEBUG_LEVEL >= DBG_LEVEL_BUG1
...
#endif
            return SLICE_SYNTAX_ERROR; ///< Syntax Error
        }
    ...
    if ( 0==pH265_Slice_Hdr_DataInst->bDependentSliceSegmentFlag ){
        ...
        if  ( (u4NalType == NAL_UNIT_CODED_SLICE_IDR) || (u4NalType == NAL_UNIT_CODED_SLICE_IDR_N_LP) ){ // IDR picture
            ...
            if ( 0 == u4NalTemporalID )
            {
                ...
                ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.i4PrePOCLsb = 0;
                ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.i4PrePOCMsb = 0;     
            }
            ...
            iPrevPOClsb = ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.i4PrePOCLsb;
            iPrevPOCmsb = ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.i4PrePOCMsb;
            ...
            if ( 0==u4NalTemporalID )
            {
                ...
                ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.i4PrePOCLsb = iPOClsb;
                ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.i4PrePOCMsb = iPOCmsb;
