...
#include "../verify/vdec_verify_imgresz_rm.h"
#endif
...
void vVDecVP8WriteImgResz(UINT32 u4VDecID, UINT32 u4Addr, UINT32 u4Val)
{
    if (u4VDecID == 0)
    {
        mb();
        vWriteReg(IMG_RESZ_REG_OFFSET0 + u4Addr, u4Val);
        //vVDecSimDumpW(u4VDecID, IMG_RESZ_REG_OFFSET0, u4Addr, u4Val);
        printk("RISCWrite('IMG_RESZ_REG_OFFSET0 + 4*%d, 32'h%x); \n", u4Addr>>2, u4Val);
    }
    else
    {
        vWriteReg(IMG_RESZ_REG_OFFSET0 + u4Addr, u4Val);
...
UINT32 u4VDecVP8ReadImgResz(UINT32 u4VDecID, UINT32 u4Addr)
{
    if (u4VDecID == 0)
    {
       UINT32 u4Val = u4ReadReg(IMG_RESZ_REG_OFFSET0 + u4Addr);
       //vVDecSimDumpR(u4VDecID, IMG_RESZ_REG_OFFSET0, u4Addr, u4Val);
       printk("RISCRead('IMG_RESZ_REG_OFFSET0 + 4*%d, 32'h%x); \n", u4Addr>>2, u4Val);
       return (u4Val);
    }
    else
    {
...
UINT32 i4VDEC_HAL_VP8_DecStart(UINT32 u4VDecID, VDEC_INFO_VP8_FRM_HDR_T *pVp8DecInfo)
{
   ...
   UINT32 u4DramPicArea;
    ...
    printk("TRC-i4VDEC_HAL_VP8_DecStart +0, u4PIC_SIZE_Y[%d]=%d\n", u4VDecID, u4PIC_SIZE_Y[u4VDecID]);
    printk("TRC-i4VDEC_HAL_VP8_DecStart +0, u4PIC_SIZE[%d]=%d\n", u4VDecID, u4PIC_SIZE[u4VDecID]);
    printk("TRC-i4VDEC_HAL_VP8_DecStart +0, u4UFO_LEN_SIZE_Y[%d]=%d\n", u4VDecID, u4UFO_LEN_SIZE_Y[u4VDecID]);
    printk("TRC-i4VDEC_HAL_VP8_DecStart +0, u4UFO_LEN_SIZE_C[%d]=%d\n", u4VDecID, u4UFO_LEN_SIZE_C[u4VDecID]);
    printk("TRC-i4VDEC_HAL_VP8_DecStart +0, u4PIC_SIZE_Y_BS[%d]=%d\n", u4VDecID, u4PIC_SIZE_Y_BS[u4VDecID]);
    printk("TRC-i4VDEC_HAL_VP8_DecStart +0, u4PIC_SIZE_BS[%d]=%d\n", u4VDecID, u4PIC_SIZE_BS[u4VDecID]);
    printk("TRC-i4VDEC_HAL_VP8_DecStart +0, u4PIC_SIZE_REF[%d]=%d\n", u4VDecID, u4PIC_SIZE_REF[u4VDecID]);

    printk("TRC-i4VDEC_HAL_VP8_DecStart +0, u4DramPicSize=%d\n", u4DramPicSize);
    printk("TRC-i4VDEC_HAL_VP8_DecStart +0, u4DramPicArea=%d\n", u4DramPicArea);
    ...
    printk("TRC-i4VDEC_HAL_VP8_DecStart +0, u4PIC_UFO_WIDTH=%d\n", u4PIC_UFO_WIDTH);
    printk("TRC-i4VDEC_HAL_VP8_DecStart +0, u4PIC_UFO_HEIGHT=%d\n", u4PIC_UFO_HEIGHT);
    printk("TRC-i4VDEC_HAL_VP8_DecStart +0, u4PIC_W_H=%d\n", u4PIC_W_H);
...
//   else
   {
    ...
    vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_COPY_MEM_DRAM_BASE, PHYSICAL(pVp8DecInfo->u4SegIdWrapper));
    vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_COPY_MEM_SRAM_SETTING, RW_VLD_TOP_COPY_MEM_SRAM_SETTING_VAL_DEFAULT);
    vVDecWriteVLDTOP(u4VDecID, RO_VLD_TOP_COPY_MEM_STATUS,
                               (RO_VLD_TOP_COPY_MEM_STATUS_VAL_DRAM_TO_SRAM << RO_VLD_TOP_COPY_MEM_STATUS_FLD_COPY_MODE) |
                               (1 << RO_VLD_TOP_COPY_MEM_STATUS_FLD_COPY_TRIG));
    // Wait for seg_id data copy completion
    while (u4VDecReadVLDTOP(u4VDecID, RO_VLD_TOP_COPY_MEM_STATUS) & (1 << RO_VLD_TOP_COPY_MEM_STATUS_FLD_COPY_BUSY));
#endif
...
#if 0
    if (u4VDecReadVP8MC(u4VDecID, RW_MC_PP_DBLK_MODE) == 2)
	{
        vVDecWriteVP8MC(u4VDecID, RW_MC_PP_DBLK_MODE,3);
		vVDecWriteVP8PP(u4VDecID, RW_PP_REG_17, 1);
	}
...
#if VDEC_VP8_IMAGERESIZE_SUPPORT
    vVDecVP8WriteImgResz(u4VDecID, 4 * IMG_RESZ_START, 0x8c);
    vWriteGconReg(0x18, 1);

#if !VP8_MB_ROW_MODE_SUPPORT_ME2_INTEGRATION
            u4PicWidth = ((pVp8DecInfo->u4Width+15)>>4)<<4;
            u4PicHeight = ((pVp8DecInfo->u4Height+15)>>4)<<4;
            u4PicPitch = u4PicWidth>>4;
           // UTIL_Printf("First MB Row\n");
           // UTIL_Printf("pic width-0x%x,pic height-0x%x\n", pVp8DecStartInfo->u4Width, pVp8DecStartInfo->u4Height);
            vVDecVP8WriteImgResz(u4VDecID, 4 * RZ_JPEG_INT_SWITCH, 0x10);//jpg_init_switch pic mode->0x10 
            u4VdoPPOn = u4VDecReadVP8MC(u4VDecID, RW_MC_PP_DBLK_MODE);            
            vVDecVP8WriteImgResz(u4VDecID, 4 * IMG_RESZ_TYPE,((((u4VdoPPOn>>1)&0x1)|(u4VdoPPOn&0x1))<<7)|1<<6);//IMG_RESZ_TYPE: pp-on ->0xc0, pp-off->0x40            
            vVDecVP8WriteImgResz(u4VDecID, 4 * IMG_RESZ_JPG_MODE, 0x1E100);//JPG_MODE
            //tmp DRAM address
            vVDecVP8WriteImgResz(u4VDecID, 4 * RZ_TMP_Y_OW_ADDR_BASE,(UINT32)((UINT32)PHYSICAL(pVp8DecInfo->u4TmpBuf)>>4));
            //source buffer length
            vVDecVP8WriteImgResz(u4VDecID, 4 * RZ_SRC_BUF_LEN, ((u4PicPitch>>1)<<12)|u4PicPitch);
            //target buffer length
            vVDecVP8WriteImgResz(u4VDecID,4 * RZ_TG_BUF_LEN,(1<<31|1<<30|0<<29|0x10<<24|(u4PicPitch<<12)|u4PicPitch));
            //source base address
            vVDecVP8WriteImgResz(u4VDecID, 4 * RZ_SRC_Y_OW_ADDR_BASE1, (UINT32)((UINT32)PHYSICAL(pVp8DecInfo->u4CurYAddr)>>4));//OUT_BUF_ADDR_Y>>4
            vVDecVP8WriteImgResz(u4VDecID, 4 * RZ_SRC_CB_OW_ADDR_BASE1, (UINT32)((UINT32)PHYSICAL(pVp8DecInfo->u4CurCAddr)>>4));//OUT_BUF_ADDR_C>>4
            vVDecVP8WriteImgResz(u4VDecID, 4 * RZ_SRC_CR_OW_ADDR_BASE1, (UINT32)((UINT32)PHYSICAL(pVp8DecInfo->u4CurCAddr)>>4));//OUT_BUF_ADDR_C>>4
            //target base address
            vVDecVP8WriteImgResz(u4VDecID, 4 * RZ_TG_Y_OW_ADDR_BASE, (UINT32)((UINT32)PHYSICAL(pVp8DecInfo->u4ImgReszY)>>4));//RESZ_FRAME_ADDR_Y>>4
            vVDecVP8WriteImgResz(u4VDecID, 4 * RZ_TG_C_OW_ADDR_BASE, (UINT32)((UINT32)PHYSICAL(pVp8DecInfo->u4ImgReszC)>>4));//RESZ_FRAME_ADDR_C>>4
            //source widhht/height
            vVDecVP8WriteImgResz(u4VDecID, 4 * RZ_SRC_SIZE_Y, (u4PicWidth<<16)|u4PicHeight);//value" qw{widht, height}
            vVDecVP8WriteImgResz(u4VDecID, 4 * RZ_SRC_SIZE_CB, (u4PicWidth>>1)<<16|(u4PicHeight>>1));//value" qw{}
            vVDecVP8WriteImgResz(u4VDecID, 4 * RZ_SRC_SIZE_CR, (u4PicWidth>>1)<<16|(u4PicHeight>>1));//value" qw{}
            //target wdith/height
            vVDecVP8WriteImgResz(u4VDecID, 4 * RZ_TG_SIZE_Y, (u4PicWidth<<16)|u4PicHeight);//value" qw{}
            vVDecVP8WriteImgResz(u4VDecID, 4 * IMG_RESZ_START, 0x8c);//value" qw{}
            vVDecVP8WriteImgResz(u4VDecID, 4 * IMG_RESZ_START, 0x82);
            vVDecVP8WriteImgResz(u4VDecID, 4 * IMG_RESZ_START, 0x83);
            vVDecVP8WriteImgResz(u4VDecID, 4 * IMG_RESZ_START, 0x82);
#endif
#endif
...
#if VDEC_VP8_IMAGERESIZE_SUPPORT
    u4HalValue = ((pVp8DecInfo->u4Width+15)/16) * 32;               // frame buffer width in MB * 32
#else
#if VP8_MB_ROW_MODE_SUPPORT_ME2_INTEGRATION
...
#if VDEC_VP8_IMAGERESIZE_SUPPORT
    vVDecWriteVP8PP(u4VDecID, 0x6C, 0x80000000); 
#else
#if VP8_MB_ROW_MODE_SUPPORT_ME2_INTEGRATION
...
//    	printk("RW_PP_SPECIAL_MODE: 0x%08x\n", u4VDecReadVP8PP(u4VDecID, RW_PP_SPECIAL_MODE));
   ...
   //vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_VP8_ENHANCE, 0);  //Hide, Do not set when VP8 or Webp @20140805
...
#ifdef VDEC_UFO_SUPPORT
    // Debug only
    printk("TRC-i4VDEC_HAL_VP8_DecStart +9, u4VDecReadMC(u4VDecID, 700 * 4)=0x%x\n", u4VDecReadMC(u4VDecID, 700 * 4));
    printk("TRC-i4VDEC_HAL_VP8_DecStart +9, u4VDecReadMC(u4VDecID, 664 * 4)=0x%x\n", u4VDecReadMC(u4VDecID, 664 * 4));
    printk("TRC-i4VDEC_HAL_VP8_DecStart +9, u4VDecReadMC(u4VDecID, 698 * 4)=0x%x\n", u4VDecReadMC(u4VDecID, 698 * 4));
    printk("TRC-i4VDEC_HAL_VP8_DecStart +9, u4VDecReadMC(u4VDecID, 699 * 4)=0x%x\n", u4VDecReadMC(u4VDecID, 699 * 4));

    printk("TRC-i4VDEC_HAL_VP8_DecStart +9, u4VDecReadMC(u4VDecID, 663 * 4)=0x%x\n", u4VDecReadMC(u4VDecID, 663 * 4));
    printk("TRC-i4VDEC_HAL_VP8_DecStart +9, u4VDecReadMC(u4VDecID, 701 * 4)=0x%x\n", u4VDecReadMC(u4VDecID, 701 * 4));
    printk("TRC-i4VDEC_HAL_VP8_DecStart +9, u4VDecReadMC(u4VDecID, 343 * 4)=0x%x\n", u4VDecReadMC(u4VDecID, 343 * 4));
#endif
   //Trigger decoder
...
void vVDEC_HAL_VP8_VDec_DumpReg(UINT32 u4VDecID, BOOL fgBefore)
{
  INT32 i; 
	  ...
	  printk("******* VLD VP8 Registers *******\n");
	  for (i=41; i<68; i++)
	  {
		  //u4VDecReadVP8VLD(0, i*4);
	  ...
	  printk("******* VLD TOP Registers *******\n");
	  for (i=0; i<100; i++)
	  {
		  //u4VDecReadVLDTOP(0, i*4);
		  printk("VLD TOP %d (0x%04x): 0x%08x\n", i, i * 4, u4VDecReadVLDTOP(0, i*4));
	  }
	  ...
	  for (i=0; i<600; i++)
	  {
		  //u4VDecReadVP8PP(0, i*4);
		  printk("PP %d (0x%04x): 0x%08x\n", i, i * 4, u4VDecReadVP8PP(0, i*4));
	  }
  
	  printk("******* MISC Registers *******\n");
  
	  for (i=0; i<121; i++)
	  {
		  //u4VDecReadDV(0, i*4);
		  printk("MISC %d (0x%04x): 0x%08x\n", i, i * 4, u4VDecReadDV(0, i*4));
	  }
	  ...
	  {
		  //u4VDecReadVLD(0, i*4);
		  printk("VLD %d (0x%04x): 0x%08x\n", i, i * 4, u4VDecReadVLD(0, i*4));
	  }
  
	  printk("******* End dump Registers *******\n");
