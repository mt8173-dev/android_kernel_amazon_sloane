...
#endif
{
	...
	{
      UINT32 u4Value;
	  u4Value = u4ReadGconReg(6*4);
	  vWriteGconReg(6*4, u4Value & 0xFFFFFFFE);
	  u4Value = u4ReadGconReg(6*4);
	  u4Value = u4VDecReadDV(u4VDecID, 59*4);
	  vVDecWriteDV( u4VDecID, 59*4, u4Value & 0xFFFFFFFE);
	  u4Value = u4VDecReadDV(u4VDecID, 59*4); 
	}
	#else
	vWriteGconReg(6*4, 1);
        ...
        if (DUMP_FOR_SIM)
        {
            ...
            #endif
            {
                ...
                case HEVC_VLD_REG_OFFSET0:
                    DBG_H264_PRINTF(" RISCWrite_HEVC_VLD( %d, 32'h%x);\n", u4Addr >> 2, u4Val);
                    break;
                default:
