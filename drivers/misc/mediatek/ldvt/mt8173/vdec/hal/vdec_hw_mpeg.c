...
void vVLDM4vDecPrmProc(UINT32 u4VDecID, VDEC_INFO_DEC_PRM_T *prDecPrm)
{
    ...
    if (2 == prDecPrm->ucMpegSpecType)
    {
		...
		//printk("Test2 RW_VLD_MP4_FLG value is 0x%x\n", u4RegVal1);
    }
	else if (3 == prDecPrm->ucMpegSpecType)
	{
        u4RegVal1 = u4VDecReadVLD(u4VDecID, RW_VLD_MP4_FLG) & ~(0x1 << 9);
        ...
        vVDecWriteVLD(u4VDecID, RW_VLD_MP4_FLG, u4RegVal1);
		//printk("Test3 RW_VLD_MP4_FLG value is 0x%x\n", u4RegVal1);
	}
#endif
...
//	vVDecWriteVLD(u4VDecID,RW_VLD_MP4_ERR_DET2,0xffffffff);
    ...
    #if H263_USE_SORENSON  // if sorenson263, remove this option
	...
	//turn off MPEG4 header check error detect in Sorenson h263 
	u4Reg |= (0x1 << 6);
  ...
  //  if(fgIsPScanTv())
    ...
    else
    {
      //#if ((VDEC_DDR3_SUPPORT && (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8560))||VDEC_UFO_ENABLE) // DDR3+PPOUT setting mtk40343
        #if 1 //Bypass mode
              //  printk("//<vdec> here ucDecFBufIdx %d\n",prDecPrm->ucDecFBufIdx);        
		...
		vVDecWriteMC(u4VDecID, RW_MC_PP_WB_BY_POST, 1); // wirte PP out only
            ...
            vVDecWriteMC(u4VDecID, RW_MC_PP_MB_WIDTH, (prDecPrm->u4PicW + 15) >> 4); ///Louis check
			

            temp = (((prDecPrm->u4PicW + 63) >> 6)<<2);
            //printk("[Louis] u4PicW_1 %d\n",temp);
...
void VDecDumpMP4RegisterByYiFeng(UINT32 u4VDecID)
{
     UINT32 i;
    /* VLD Registers */
	printk("Start Dump Register!\n");
    ...
    printk("******* AVC_MV Registers *******\n");
    for (i=0; i<256; i++)
    {
		printk("AVCMV %d (0x%04x): 0x%08x\n", i, i * 4, u4VDecReadAVCMV(0, i*4));
    }

	printk("\n******* IS Registers *******\n");
    for (i=0; i<65; i++)
    {
		printk("PP %d (0x%04x): 0x%08x\n", (0x200 + i*4)/4, 0x200 + i*4, u4VDecReadMP4PP(0, 0x200 + i*4));
    }

    printk("\n*******DCAC Registers *******\n");  
    for (i=0; i<65; i++)
    {
		printk("PP %d (0x%04x): 0x%08x\n", (0x700 + i*4)/4, 0x700 + i*4, u4VDecReadMP4PP(0, 0x700 + i*4));
    }

   printk("\n******* IQ Registers *******\n");
    for (i=0; i<65; i++)
    {
		printk("PP %d (0x%04x): 0x%08x\n", (0x500 + i*4)/4, 0x500 + i*4, u4VDecReadMP4PP(0, 0x500 + i*4));
    }

    printk("\n******* IT Registers *******\n");
    for (i=0; i<65; i++)
    {
		printk("PP %d (0x%04x): 0x%08x\n", (0x900 + i*4)/4, 0x900 + i*4, u4VDecReadMP4PP(0, 0x900 + i*4));
    }

    printk("\n******* ORED_WR *******\n");
    for (i=0; i<65; i++)
    {
		printk("PP %d (0x%04x): 0x%08x\n", (0x800 + i*4)/4, 0x800 + i*4, u4VDecReadMP4PP(0, 0x800 + i*4));
    }
