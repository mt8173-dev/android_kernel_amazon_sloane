...
DECLARE_COMPLETION(mt_spi_done_ldvt);
...
#define SPI_CROSS_ALIGN_OFFSET 1008
	...
	if (flag == 0) {
		...
		xfer->tx_dma = dma_map_single(NULL, (void *)xfer->tx_buf, xfer->len, DMA_TO_DEVICE);
		if (dma_mapping_error(NULL, xfer->tx_dma)) {
			SPIDEV_LOG("dma mapping tx_buf error.\n");
			return -ENOMEM;
		}
		xfer->rx_dma = dma_map_single(NULL, (void *)xfer->rx_buf, xfer->len, DMA_TO_DEVICE);
...
static int threadfunc3(void *data)
{
	...
	while (1) {
		...
		wait_for_completion(&mt_spi_done_ldvt);
...
static int spi_test_remove(struct spi_device *spi)
{
	...
	SPIDEV_LOG("spi_ldvt_test_remove.\n");
...
{
	SPIDEV_LOG("spi ldvt test probe  enter\n");
	spi_test = spi;
	spi->mode = SPI_MODE_3;
...
struct spi_device_id spi_id_table_ldvt = { "spidevldvt", 0 };
