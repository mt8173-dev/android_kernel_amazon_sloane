...
typedef struct {
	...
	VAL_REGS_test msdc0_ctrl0;	/*0x0C00 ~ 0x0C0F:  16 bytes */
	VAL_REGS_test msdc0_ctrl1;	/*0x0C10 ~ 0x0C1F:  16 bytes */
	VAL_REGS_test msdc0_ctrl2;	/*0x0C20 ~ 0x0C2F:  16 bytes */
	VAL_REGS_test msdc0_ctrl5;	/*0x0C30 ~ 0x0C3F:  16 bytes */
	VAL_REGS_test msdc1_ctrl0;	/*0x0C40 ~ 0x0C4F:  16 bytes */
	VAL_REGS_test msdc1_ctrl1;	/*0x0C50 ~ 0x0C5F:  16 bytes */
	VAL_REGS_test msdc1_ctrl2;	/*0x0C60 ~ 0x0C6F:  16 bytes */
	VAL_REGS_test msdc1_ctrl5;	/*0x0C70 ~ 0x0C7F:  16 bytes */
	VAL_REGS_test msdc2_ctrl0;	/*0x0C80 ~ 0x0C8F:  16 bytes */
	VAL_REGS_test msdc2_ctrl1;	/*0x0C90 ~ 0x0C9F:  16 bytes */
	VAL_REGS_test msdc2_ctrl2;	/*0x0CA0 ~ 0x0CAF:  16 bytes */
	VAL_REGS_test msdc2_ctrl5;	/*0x0CB0 ~ 0x0CBF:  16 bytes */
	VAL_REGS_test msdc3_ctrl0;	/*0x0CC0 ~ 0x0CCF:  16 bytes */
	VAL_REGS_test msdc3_ctrl1;	/*0x0CD0 ~ 0x0CDF:  16 bytes */
	VAL_REGS_test msdc3_ctrl2;	/*0x0CE0 ~ 0x0CEF:  16 bytes */
	VAL_REGS_test msdc3_ctrl5;	/*0x0CF0 ~ 0x0CFF:  16 bytes */
	VAL_REGS_test msdc0_ctrl3;	/*0x0D00 ~ 0x0D0F:  16 bytes */
	VAL_REGS_test msdc0_ctrl4;	/*0x0D10 ~ 0x0D1F:  16 bytes */
	VAL_REGS_test msdc1_ctrl3;	/*0x0D20 ~ 0x0D2F:  16 bytes */
	VAL_REGS_test msdc1_ctrl4;	/*0x0D30 ~ 0x0D3F:  16 bytes */
	VAL_REGS_test msdc2_ctrl3;	/*0x0D40 ~ 0x0D4F:  16 bytes */
	VAL_REGS_test msdc2_ctrl4;	/*0x0D50 ~ 0x0D5F:  16 bytes */
	VAL_REGS_test msdc3_ctrl3;	/*0x0D60 ~ 0x0D6F:  16 bytes */
	VAL_REGS_test msdc3_ctrl4;	/*0x0D70 ~ 0x0D7F:  16 bytes */
	...
	VAL_REGS_test hsic_ctrl[4];	/*0x0E20 ~ 0x0E5F:  64 bytes */
} GPIO_REGS_test;
...
void mt_reg_test(void)
{
	...
	for (i = 0; i < 1023; i += 4) {
		...
		GPIOMSG("0x%04X, 0x%04X, 0x%04X, 0x%04X\n", test_data->data[i],
			test_data->data[i + 1], test_data->data[i + 2], test_data->data[i + 3]);
	}
...
static void mt_gpio_dump_addr_ext(void)
{
	...
	for (idx = 0; idx < sizeof(reg->pullen) / sizeof(reg->pullen[0]); idx++)
		GPIOMSG("val[%2d] %p\nset[%2d] %p\nrst[%2d] %p\n", idx, &reg->pullen[idx].val, idx,
			&reg->pullen[idx].set, idx, &reg->pullen[idx].rst);
	GPIOMSG("# pull select\n");
	for (idx = 0; idx < sizeof(reg->pullsel) / sizeof(reg->pullsel[0]); idx++)
		GPIOMSG("val[%2d] %p\nset[%2d] %p\nrst[%2d] %p\n", idx, &reg->pullsel[idx].val, idx,
			&reg->pullsel[idx].set, idx, &reg->pullsel[idx].rst);
	GPIOMSG("# data inversion\n");
	for (idx = 0; idx < sizeof(reg->dinv) / sizeof(reg->dinv[0]); idx++)
		GPIOMSG("val[%2d] %p\nset[%2d] %p\nrst[%2d] %p\n", idx, &reg->dinv[idx].val, idx,
			&reg->dinv[idx].set, idx, &reg->dinv[idx].rst);
	GPIOMSG("# data output\n");
...
static ssize_t mt_gpio_dump_addr_base(void)
{
	...
	for (idx = 0; idx < sizeof(reg->ies) / sizeof(reg->ies[0]); idx++)
		GPIOMSG("val[%2d] %p\nset[%2d] %p\nrst[%2d] %p\n", idx, &reg->ies[idx].val, idx,
			&reg->ies[idx].set, idx, &reg->ies[idx].rst);
	GPIOMSG("# pull enable\n");
	for (idx = 0; idx < sizeof(reg->pullen) / sizeof(reg->pullen[0]); idx++)
		GPIOMSG("val[%2d] %p\nset[%2d] %p\nrst[%2d] %p\n", idx, &reg->pullen[idx].val, idx,
			&reg->pullen[idx].set, idx, &reg->pullen[idx].rst);
	...
	if (!strncmp(buf, "-h", 2)) {
		...
		if (!strncmp(buf, "mode", 4) && (2 == sscanf(buf + 4, "%d %d", &pin, &mode))) {
			...
			GPIOMSG("set dir (%3d, %d)=%d\n", pin, dir, ret);
			 ...
			 sscanf(buf, "=%d:%d %d %d %d %d %d", &pin, &mode, &pullsel, &dout, &pullen,
				&dir, &ies)) {
			ret = mt_set_gpio_mode(pin, mode);
			GPIOMSG("set mode(%3d, %d)=%d\n", pin, mode, ret);
			ret = mt_set_gpio_pull_select(pin, pullsel);
		...
		if (!strncmp(buf, "s", 1) && (3 == sscanf(buf + 1, "%d %d %d", &num, &src, &div))) {
			ret = mt_set_clock_output(num, src, div);
