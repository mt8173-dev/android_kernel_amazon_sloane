...
static size_t atf_log_dump(unsigned char *buffer, struct ipanic_atf_log_rec *rec, size_t size)
{
	...
	atf_log_unlock();
	show_data((unsigned long)atf_log_vir_addr, 24*1024, "atf_buf");
	return ret;
}
...
static ssize_t atf_log_write(struct file *file, const char __user *iov, size_t nr_segs, loff_t *ppos)
{
...
start:
	...
	/*pr_notice("atf_log_read: return %d, idx: %lu, readpos: %p, writepos: %p\n", ret,
		read_index, atf_buf_vir_ctl->info.atf_read_pos, atf_buf_vir_ctl->info.atf_write_pos);*/
	return ret;
}
...
int atf_get_from_dt(unsigned long *phy_addr, unsigned int *len)
{
	unsigned long node = 0;
	struct mem_desc_s *mem_desc = NULL;
	if (of_scan_flat_dt(dt_scan_memory, &node)) {
		mem_desc = (struct mem_desc_s *)of_get_flat_dt_prop(node, "tee_reserved_mem", NULL);
...
static void show_data(unsigned long addr, int nbytes, const char *name)
{
	...
	for (i = 0; i < nlines; i++) {
		 ...
		 */
		pr_warn("%04lx ", (unsigned long)p & 0xffff);
		for (j = 0; j < 8; j++) {
			u32	data;
...
static int __init atf_log_init(void)
{
	...
	if (request_irq(281, (irq_handler_t)ATF_log_irq_handler, IRQF_TRIGGER_NONE, "ATF_log_irq", NULL) != 0) {
