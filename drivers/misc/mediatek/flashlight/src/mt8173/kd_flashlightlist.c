...
/* Houston add define for PMIC doesn't support power throttling */
#define PMIC_NO_SUPPORT_THROTTLING
		...
		for (j = 0; j < e_Max_Strobe_Num_Per_Dev; j++) {
			...
			for (k = 0; k < e_Max_Part_Num_Per_Dev; k++) {
				...
				if (g_pFlashInitFunc[i][j][k] != 0) {
					...
					g_pFlashInitFunc[i][j][k]->
					    flashlight_ioctl(FLASH_IOC_SET_ONOFF, 0);
				}
			}
		}
	}
...
static int __init flashlight_init(void)
{
...
#ifndef PMIC_NO_SUPPORT_THROTTLING
	...
	register_battery_oc_notify(&bat_oc_protection_powerlimit,
				   BATTERY_OC_LEVEL_0 /*K2's upmu_sw:BATTERY_OC_PRIO_FLASHLIGHT */
				   );
#endif
