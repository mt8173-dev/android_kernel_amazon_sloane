...
#if 0
	ghLogBuf =
	    ged_log_buf_alloc(320, 64 * 320, GED_LOG_BUF_TYPE_QUEUEBUFFER_AUTO_INCREASE, NULL,
			      "profile_dvfs");
#endif
...
GED_ERROR ged_profile_dvfs_enable(void)
{
	...
	if (0 == ghLogBuf) {
		ghLogBuf =
		    ged_log_buf_alloc(320, 64 * 320, GED_LOG_BUF_TYPE_QUEUEBUFFER_AUTO_INCREASE,
				      NULL, "profile_dvfs");
	}
...
void ged_profile_dvfs_record_freq_volt(unsigned int ui32Frequency, unsigned int ui32Voltage)
{
	...
	if (ghLogBuf && gbAllowRecord) {
		...
		ged_log_buf_print(ghLogBuf, "%5lu.%06lu,freq_volt,%u,%u", (unsigned long)t,
				  nanosec_rem, ui32Frequency, ui32Voltage);
	}
	mutex_unlock(&gsMutex);
}
...
void ged_profile_dvfs_record_temp(int i32Temp)
{
	...
	if (ghLogBuf && gbAllowRecord) {
		...
		ged_log_buf_print(ghLogBuf, "%5lu.%06lu,temp,%d", (unsigned long)t, nanosec_rem,
				  i32Temp);
	}
	mutex_unlock(&gsMutex);
...
void ged_profile_dvfs_record_thermal_limit(unsigned int ui32FreqLimit)
{
	...
	if (ghLogBuf && gbAllowRecord) {
		...
		ged_log_buf_print(ghLogBuf, "%5lu.%06lu,thermal_limit,%u", (unsigned long)t,
				  nanosec_rem, ui32FreqLimit);
	}
	mutex_unlock(&gsMutex);
}
...
void ged_profile_dvfs_record_gpu_loading(unsigned int ui32GpuLoading)
{
	...
	if (ghLogBuf && gbAllowRecord) {
		...
		ged_log_buf_print(ghLogBuf, "%5lu.%06lu,gpu_load,%u", (unsigned long)t, nanosec_rem,
				  ui32GpuLoading);
	}
...
void ged_profile_dvfs_record_clock_on(void)
{
	...
	if (ghLogBuf && gbAllowRecord) {
		...
		ged_log_buf_print(ghLogBuf, "%5lu.%06lu,gpu_clock,1", (unsigned long)t,
				  nanosec_rem);
	}
...
void ged_profile_dvfs_record_clock_off(void)
{
	...
	if (ghLogBuf && gbAllowRecord) {
		...
		ged_log_buf_print(ghLogBuf, "%5lu.%06lu,gpu_clock,0", (unsigned long)t,
				  nanosec_rem);
	}
