...
static long ged_dispatch(GED_BRIDGE_PACKAGE *psBridgePackageKM)
{
	...
	if (psBridgePackageKM->i32InBufferSize >= 0 && psBridgePackageKM->i32OutBufferSize >= 0
		...
		if (psBridgePackageKM->i32InBufferSize > 0) {
			if (0 !=
			    ged_copy_from_user(pvInt, psBridgePackageKM->pvParamIn,
		...
		if (psBridgePackageKM->i32OutBufferSize > 0) {
			if (0 !=
			    ged_copy_to_user(psBridgePackageKM->pvParamOut, pvOut,
...
static long ged_ioctl(struct file *pFile, unsigned int ioctlCmd, unsigned long arg)
{
	...
	if (0 !=
	    ged_copy_from_user(psBridgePackageKM, psBridgePackageUM, sizeof(GED_BRIDGE_PACKAGE))) {
...
static long ged_ioctl_compat(struct file *pFile, unsigned int ioctlCmd, unsigned long arg)
{
	...
	if (0 !=
	    ged_copy_from_user(psBridgePackageKM32, psBridgePackageUM32,
...
#ifdef GED_DEBUG
	ghLogBuf_GLES =
	    ged_log_buf_alloc(160, 128 * 160, GED_LOG_BUF_TYPE_RINGBUFFER, GED_LOG_BUF_COMMON_GLES,
			      NULL);
	ghLogBuf_GED =
	    ged_log_buf_alloc(32, 64 * 32, GED_LOG_BUF_TYPE_RINGBUFFER, "GED internal", NULL);
#endif
