...
/* ----------------------------------------------------------------------------- */
static ssize_t ged_debugFS_write(struct file *psFile,
...
/* ----------------------------------------------------------------------------- */
GED_ERROR ged_debugFS_create_entry(const char *pszName,
				   void *pvDir,
				   struct seq_operations *psReadOps,
				   GED_ENTRY_WRITE_FUNC *pfnWrite,
				   void *pvData, struct dentry **ppsEntry)
{
	...
	psEntry = debugfs_create_file(pszName,
				      ...
				      psPrivData, &gsGEDDebugFSFileOps);
	if (IS_ERR(psEntry)) {
...
/* ----------------------------------------------------------------------------- */
GED_ERROR ged_debugFS_create_entry_dir(const char *pszName,
				       struct dentry *psParentDir, struct dentry **ppsDir)
{
	struct dentry *psDir;
