...
/* ----------------------------------------------------------------------------- */
int ged_bridge_log_buf_get(GED_BRIDGE_IN_LOGBUFGET *psLogBufGetIN,
...
/* ----------------------------------------------------------------------------- */
int ged_bridge_log_buf_write(GED_BRIDGE_IN_LOGBUFWRITE *psLogBufWriteIN,
...
#if 0
	if (ged_log_buf_write(psLogBufWriteIN->hLogBuf,
...
/* ----------------------------------------------------------------------------- */
int ged_bridge_log_buf_reset(GED_BRIDGE_IN_LOGBUFRESET *psLogBufResetIn,
...
/* ----------------------------------------------------------------------------- */
int ged_bridge_boost_gpu_freq(GED_BRIDGE_IN_BOOSTGPUFREQ *psBoostGpuFreqIN,
			      GED_BRIDGE_OUT_BOOSTGPUFREQ *psBoostGpuFreqOUT)
{
	...
	if (mtk_custom_get_gpu_freq_level_count(&ui32Count)) {
		int i32Level =
		    (ui32Count - 1) - GED_BOOST_GPU_FREQ_LEVEL_MAX -
		    psBoostGpuFreqIN->eGPUFreqLevel;
...
/* ----------------------------------------------------------------------------- */
int ged_bridge_monitor_3D_fence(GED_BRIDGE_IN_MONITOR3DFENCE *psMonitor3DFenceINT,
