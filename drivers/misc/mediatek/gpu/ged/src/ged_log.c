...
static GED_ERROR __ged_log_buf_vprint(GED_LOG_BUF *psGEDLogBuf, const char *fmt, va_list args,
				      int attrs)
{
	int buf_n_r;
	int buf_n;
	...
	if (psGEDLogBuf->i32LineCurrent >= psGEDLogBuf->i32LineCount ||
		...
		if (attrs & GED_LOG_ATTR_RINGBUFFER) {
			...
			if (attrs & GED_LOG_ATTR_AUTO_INCREASE) {
				...
				/* TODO */
				/* ged_log_buf_resize(psGEDLogBuf->ui32HashNodeID,
	...
	if (attrs & GED_LOG_ATTR_TIME) {
		...
		buf_n_r =
		    snprintf(buf, buf_n, "[%5lu.%06lu] ", (unsigned long)t, nanosec_rem / 1000);
	...
	if (attrs & GED_LOG_ATTR_RINGBUFFER) {
		...
		for (i = psGEDLogBuf->i32LineCurrent + 1; --check && i < psGEDLogBuf->i32LineCount;
		     ++i) {
...
static int __ged_log_buf_write(GED_LOG_BUF *psGEDLogBuf, const char __user *pszBuffer,
			       int i32Count)
{
	int cnt;
	char buf[256];
...
static ssize_t ged_log_buf_write_entry(const char __user *pszBuffer, size_t uiCount,
	...
	if (psGEDLogBuf != NULL) {
		...
		if (psGEDLogBuf->acName[0] != '\0') {
			seq_printf(psSeqFile, "---------- %s (%d/%d) ----------\n",
				   psGEDLogBuf->acName, psGEDLogBuf->i32BufferCurrent,
...
GED_LOG_BUF_HANDLE ged_log_buf_alloc(int i32MaxLineCount,
				     ...
				     GED_LOG_BUF_TYPE eType,
				     const char *pszName, const char *pszNodeName)
{
	...
	if (pszNodeName) {
		...
		err = ged_debugFS_create_entry(psGEDLogBuf->acNodeName,
					       gpsGEDLogBufsDir,
					       &gsGEDLogBufReadOps,
					       ged_log_buf_write_entry,
					       psGEDLogBuf, &psGEDLogBuf->psEntry);
...
GED_ERROR ged_log_buf_resize(GED_LOG_BUF_HANDLE hLogBuf,
			     int i32NewMaxLineCount, int i32NewMaxBufferSizeByte)
{
	...
	spin_lock_irqsave(&psGEDLogBuf->sSpinLock, psGEDLogBuf->ui32IRQFlags);
	{
		...
		memcpy(pi32NewLine, psGEDLogBuf->pi32Line,
		       sizeof(int) * min(i32NewMaxLineCount, psGEDLogBuf->i32LineCount));
		{
			int i;
			...
			if (n > psGEDLogBuf->i32LineCurrent) {
				...
				for (i = 0; i < buf_n; ++i)
					psGEDLogBuf->pcBuffer[i] =
					    psGEDLogBuf->pcBuffer[buf_start + i];
...
/*  */
/* ----------------------------------------------------------------------------- */
static ssize_t ged_log_write_entry(const char __user *pszBuffer, size_t uiCount, loff_t uiPosition,
				   void *pvData)
{
	...
	if ((0 < uiCount) && (uiCount < GED_LOG_CMD_SIZE)) {
		if (0 == ged_copy_from_user(acBuffer, pszBuffer, uiCount)) {
			...
			if (strcmp(acBuffer, "reset") == 0) {
				...
				list_for_each_safe(psListEntry, psListEntryTemp, psList) {
					GED_LOG_BUF *psGEDLogBuf =
					    (GED_LOG_BUF *) list_entry(psListEntry, GED_LOG_BUF,
								       sList);
...
static void *ged_log_seq_start(struct seq_file *psSeqFile, loff_t *puiPosition)
{
	...
	list_for_each_safe(psListEntry, psListEntryTemp, psList) {
		GED_LOG_BUF *psGEDLogBuf =
		    (GED_LOG_BUF *) list_entry(psListEntry, GED_LOG_BUF, sList);
		if (psGEDLogBuf->acName[0] != '\0') {
...
GED_ERROR ged_log_system_init(void)
{
	...
	err = ged_debugFS_create_entry("gedlog",
				       NULL,
				       &gsGEDLogReadOps,
				       ged_log_write_entry, NULL, &gpsGEDLogEntry);
	...
	err = ged_debugFS_create_entry_dir("logbufs", NULL, &gpsGEDLogBufsDir);
