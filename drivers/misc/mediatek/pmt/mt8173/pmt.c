...
static int pmt_region_read(struct pmt_region *region, void *buf)
{
	...
	err =
	    eMMC_rw_x(region->base_addr, (unsigned int *)buf, 0, 0, region->size, 1,
		      EMMC_PART_USER);
	if (err) {
...
static int pmt_region_write(struct pmt_region *region, void *buf)
{
	...
	err =
	    eMMC_rw_x(region->base_addr, (unsigned int *)buf, 0, 1, region->size, 1,
		      EMMC_PART_USER);
	if (err) {
	...
	for (i = 0; i < PART_NUM; i++) {
		if (new_part[i].size != last_part[i].size
			...
			pmt_info("[SD_UPGRADE]new_pt %d size changed from %llx to %llx\n", i,
				 last_part[i].size, new_part[i].size);
			*changed = 1;
			pt_change_tb[i] = 1;
...
static void update_part_size(pt_resident *new_part, int *pt_change_tb)
{
	...
	for (i = 0; i < PART_NUM; i++) {
		if (pt_change_tb[i] == 1 && new_part[i].size == 0) {
			new_part[i].size =
			    emmc_user_size - new_part[i].offset - s_info.emmc_reserve * 512;
		}
	}
}
...
static int pmt_upgrade_proc_write(struct file *file, const char *buffer, size_t size, loff_t *ppos)
{
...
static long pmt_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
	case PMT_READ:
		err = -EINVAL;	/* read_pmt(argp); */
		break;
	case PMT_WRITE:
		err = -EINVAL;	/* write_pmt(argp); */
		break;
	default:
		err = -EINVAL;
	}
