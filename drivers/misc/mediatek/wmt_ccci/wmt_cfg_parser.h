...
#define WMT_CCCI_LOUD_FUNC(fmt, arg...)    if (wmtCcciLogLvl >= WMT_CCCI_LOG_LOUD) { printk(KERN_DEBUG DFT_TAG "[L]%s:"  fmt, __func__ , ##arg); }
#define WMT_CCCI_INFO_FUNC(fmt, arg...)    if (wmtCcciLogLvl >= WMT_CCCI_LOG_INFO) { printk(KERN_ERR DFT_TAG "[I]%s:"  fmt, __func__ , ##arg); }
#define WMT_CCCI_WARN_FUNC(fmt, arg...)    if (wmtCcciLogLvl >= WMT_CCCI_LOG_WARN) { printk(KERN_WARNING DFT_TAG "[W]%s:"  fmt, __func__ , ##arg); }
#define WMT_CCCI_ERR_FUNC(fmt, arg...)     if (wmtCcciLogLvl >= WMT_CCCI_LOG_ERR)  { printk(KERN_ERR DFT_TAG "[E]%s(%d):"  fmt, __func__ , __LINE__, ##arg); }
#define WMT_CCCI_DBG_FUNC(fmt, arg...)     if (wmtCcciLogLvl >= WMT_CCCI_LOG_DBG)  { printk(KERN_DEBUG DFT_TAG "[D]%s:"  fmt, __func__ , ##arg); }

#define wmt_ccci_assert(condition) if (!(condition)) {printk(KERN_ERR DFT_TAG "%s, %d, (%s)\n", __FILE__, __LINE__, #condition); }
