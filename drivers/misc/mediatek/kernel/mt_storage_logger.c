...
#endif
static ssize_t storage_logger_dump_write_proc(struct file *file, const char __user *buffer,
...
static ssize_t storage_logger_bufsize_write_proc(struct file *file, const char __user *buffer,
					     size_t count, loff_t *data)
{
	char tmpBuf[30];
	unsigned long u4CopySize = 0;
...
static ssize_t storage_logger_filename_write_proc(struct file *file, const char __user *buffer,
					      size_t count, loff_t *data)
{
	...
	if (copy_from_user(tmpBuf, buffer, u4CopySize)) {
		pr_info("Fail to copy buffer from user space, %s, %d\n", __func__, __LINE__);
		return 0;
	}
...
static ssize_t storage_logger_iosched_write_proc(struct file *file, const char __user *buffer,
					     size_t count, loff_t *data)
{
	char tmpBuf[4];
	int u4CopySize = 0;
...
static ssize_t storage_logger_fs_rectime_threshold_write_proc(struct file *file,
							  ...
							  loff_t *data)
{
	...
	/* pr_info(" %s, %d - write file system threshold to fs_rectime with %d ms\n", __FUNCTION__, __LINE__, fs_rectime_threshold); */
	return count;
}
...
static ssize_t storage_logger_write_proc(struct file *file, const char __user *buffer, size_t count,
				     loff_t *data)
{
	char acBuf[32];
	char cmd[32];
...
static int mtk_io_osd_write_proc(struct file *file, const char *buffer, unsigned long count,
				 void *data)
{
	char acBuf[32];
	unsigned long CopySize = 0;
...
static ssize_t storage_logger_write_flag(struct file *file, const char __user *buffer, size_t count,
				     loff_t *data)
{
	unsigned long CopySize = 0;
...
static ssize_t usb_logger_write_flag(struct file *file, const char __user *buffer, size_t count,
				 loff_t *data)
{
	...
	len = min(count, (size_t) (sizeof(kbuf) - 1));
...
ssize_t storage_logger_proc_read(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
	...
	pr_info("Holmes storage_logger_proc_read: %d %Lu %Lu\n", size, *ppos, m->read_pos);
	mutex_lock(&m->lock);
