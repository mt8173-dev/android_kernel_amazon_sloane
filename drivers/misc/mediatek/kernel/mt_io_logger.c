...
static uint *io_logger_mem_pool;
...
/*pr_err("MsgID: %d, PID: %d, Func: %s, Line: %d, len:%d - %s",
	...
	if (likely(enable_inMem)) {
		...
		if (iswrapped) {
			...
			while ((len + bin_len) >= (readIndex - writeIndex)) {
				...
				if (writeIndex >=
				    ((io_logger_bufsize >> 2) - (io_logger_bufsize >> 6))) {
					readIndex = 0;
					iswrapped = false;
					break;
				}
		...
		/* pr_err("eee---W :%d, r:%d is_wrapper: %d len:%d localbuffer:%p\n",writeIndex,readIndex,iswrapped,len,(char *)localbuffer); */
...
static int io_logger_config_show(struct seq_file *m, void *v)
{
	seq_puts(m, "\r\n[io_logger config flag]\r\n");
	seq_puts(m, "=========================================\r\n");
	seq_printf(m, "enable_IOLogger = %d\r\n", enable_IOLogger);
	seq_printf(m, "readIndex = %d\r\n", readIndex);
	seq_printf(m, "writeIndex = %d\r\n", writeIndex);
	seq_printf(m, "enable_inMem = %d\r\n", enable_inMem);
	seq_printf(m, "enable_printk = %d\r\n", enable_printk);
	seq_printf(m, "enable_reverse = %d\r\n", enable_reverse);
	seq_printf(m, "enable_ProcName = %d\r\n", enable_ProcName);
	seq_printf(m, "io_logger_bufsize = %d, %dMB\n", io_logger_bufsize,
		   io_logger_bufsize / (1024 * 1024));
...
static ssize_t io_logger_write_config(struct file *file, const char __user *buffer, size_t count,
				  loff_t *data)
{
	...
	if (sscanf(acBuf, "%d %d", &tmp1, &tmp2) == 2)
		;

	if (tmp1 != ENABLE_IOLOGGER) {
		if (enable_IOLogger) {
			pr_err("please disable io logger first\n");
			return count;
		}
...
static void en_io_logger(int i)
{
	...
	if (atomic_read(&external_read_counter) > 0) {
		pr_err("some process open dump file,it can not enable IO Logger\n");
		return;
	}
...
static void parse_log(char *buf, int count)
{
	...
	while (count > 0) {
		...
		curtime_high = *bufp;
		bufp++;
		curtime_low |= (curtime_high << 32) & (0xFFFFFFFF00000000L);
		threadtime_low = *bufp;
		bufp++;
		threadtime_high = *bufp;
		bufp++;
		threadtime_low |= (threadtime_high << 32) & (0xFFFFFFFF00000000L);
		payload_len = *bufp;
		bufp++;
		...
		pr_debug("MsgID:%d,PID:[%s|%d],%s_%d,[%Lu|%Lu]-",
...
static void parse_log_reverse(char *buf, int count)
{
	...
	if (count == 0)
		return;
	pr_info("[parse_log_reverse]count %d\n", count);
	while ((count > 0) && (printk_count > 0)) {
		...
		sig = (*(bufp - 1)) & 0x0fff;
		if (sig != 0xabc) {
			pr_info("[parse_log_reverse]total_length=%d,sig=%x\n", total_length, sig);
			return;
		}
		count -= total_length;
...
void dump_trace_log(const char *func_name, unsigned long long timeoffset)
{
	...
	int count = 0;
	/* unsigned long flags; */
	pr_info("++++++++++ [%s,%lld] trace timeout ++++dump trace log++++++\n", func_name,
		timeoffset);
	...
	mutex_unlock(&logger_lock);
	/* spin_unlock_irqrestore(&logger_lock, flags); */
	pr_info("++++++++++ [%s] trace timeout ++++dump trace log++++++\n", func_name);
}
...
static int io_logger_help_show(struct seq_file *m, void *v)
{

	seq_puts(m, "=================help========================\r\n");
	seq_puts(m, "[io_logger_display]--- display io logger\n");
	seq_puts(m, "[io_logger_config] --- config io logger, you can read it before config\n\n");
	seq_puts(m, "How to config io logger?? -->\n ");
	seq_puts(m, "\t echo '[option] [value]' > /proc/driver/io_logger/io_logger_config\n");
	seq_puts(m, "[option]:\n");
	seq_printf(m, "%d: enable_IOLogger. 1: enable, 0:disable. \r\n", ENABLE_IOLOGGER);
	seq_printf(m, "%d: enable_inMem. 1: enable, 0:disable.\r\n", ENABLE_INMEM);
	seq_printf(m, "%d: enable_printk. 1: enable, 0:disable.\r\n", ENABLE_PRINTK);
	seq_printf(m, "%d: enable_reverse. 1: enable, 0:disable.\r\n", ENABLE_RESERVER);
	seq_printf(m, "%d: enable_ProcName. 1: enable, 0:disable.\r\n", ENABLE_PRONAME);
	seq_printf(m, "%d: io_logger_bufsize.unit is KB\n", IO_LOGGER_BUFSIZE);

	return 0;
}

static int io_logger_help_open(struct inode *inode, struct file *file)
{
	return single_open(file, io_logger_help_show, inode->i_private);
}

static const struct file_operations io_logger_help_fops = {
	.open = io_logger_help_open,
	.read = seq_read,
};

static int io_logger_config_open(struct inode *inode, struct file *file)
{
	return single_open(file, io_logger_config_show, inode->i_private);
}

static const struct file_operations io_logger_config_fops = {
	.open = io_logger_config_open,
	.read = seq_read,
	.write = io_logger_write_config,
};
