	...
	if (pucPairSuite) {
		...
		for (i = 0; i < (UINT_32) u2PairSuiteCount; i++) {
			...
			DBGLOG(RSN, LOUD,
				...
				(UCHAR) ((prRsnInfo->au4PairwiseKeyCipherSuite[i] >> 8) &
					 0x000000FF),
				(UCHAR) ((prRsnInfo->au4PairwiseKeyCipherSuite[i] >> 16) &
					 0x000000FF),
				(UCHAR) ((prRsnInfo->au4PairwiseKeyCipherSuite[i] >> 24) &
					 0x000000FF)));
		}
	} else {
	...
	if (pucAuthSuite) {
		...
		for (i = 0; i < (UINT_32) u2AuthSuiteCount; i++) {
			...
			DBGLOG(RSN, LOUD, ("RSN: AKM suite [%d]: %02x-%02x-%02x-%02x\n",
					   ...
					   (UCHAR) ((prRsnInfo->au4AuthKeyMgtSuite[i] >> 8) &
						    0x000000FF),
					   (UCHAR) ((prRsnInfo->au4AuthKeyMgtSuite[i] >> 16) &
						    0x000000FF),
					   (UCHAR) ((prRsnInfo->au4AuthKeyMgtSuite[i] >> 24) &
						    0x000000FF)));
		}
	} else {
		/* The information about the authentication and key management suites
		...
		DBGLOG(RSN, LOUD, ("RSN: AKM suite: %02x-%02x-%02x-%02x (default)\n",
				   ...
				   (UCHAR) ((prRsnInfo->au4AuthKeyMgtSuite[0] >> 24) &
					    0x000000FF)));
	}
	...
	if (pucPairSuite) {
		...
		for (i = 0; i < (UINT_32) u2PairSuiteCount; i++) {
			...
			DBGLOG(RSN, LOUD,
				...
				(UCHAR) ((prWpaInfo->au4PairwiseKeyCipherSuite[i] >> 8) &
					 0x000000FF),
				(UCHAR) ((prWpaInfo->au4PairwiseKeyCipherSuite[i] >> 16) &
					 0x000000FF),
				(UCHAR) ((prWpaInfo->au4PairwiseKeyCipherSuite[i] >> 24) &
					 0x000000FF)));
		}
	} else {
		/* The information about the pairwise key cipher suites is not present.
	...
	if (pucAuthSuite) {
		...
		for (i = 0; i < (UINT_32) u2AuthSuiteCount; i++) {
			...
			DBGLOG(RSN, LOUD, ("WPA: AKM suite [%d]: %02x-%02x-%02x-%02x\n",
					   ...
					   (UCHAR) ((prWpaInfo->au4AuthKeyMgtSuite[i] >> 8) &
						    0x000000FF),
					   (UCHAR) ((prWpaInfo->au4AuthKeyMgtSuite[i] >> 16) &
						    0x000000FF),
					   (UCHAR) ((prWpaInfo->au4AuthKeyMgtSuite[i] >> 24) &
						    0x000000FF)));
		}
	} else {
		/* The information about the authentication and key management suites
		...
		DBGLOG(RSN, LOUD, ("WPA: AKM suite: %02x-%02x-%02x-%02x (default)\n",
				   ...
				   (UCHAR) ((prWpaInfo->au4AuthKeyMgtSuite[0] >> 24) &
					    0x000000FF)));
	}
...
BOOLEAN rsnIsSuitableBSS(IN P_ADAPTER_T prAdapter, IN P_RSN_INFO_T prBssRsnInfo)
{
	...
	do {
		...
		for (i = 0; i < prBssRsnInfo->u4PairwiseKeyCipherSuiteCount; i++) {
			if (((prAdapter->rWifiVar.rConnSettings.
			      rRsnInfo.au4PairwiseKeyCipherSuite[0] & 0x000000FF) !=
		...
		for (i = 0; i < prBssRsnInfo->u4AuthKeyMgtSuiteCount; i++) {
			if (((prAdapter->rWifiVar.rConnSettings.
			      rRsnInfo.au4AuthKeyMgtSuite[0] & 0x000000FF) !=
			...
			if (prAdapter->rWifiVar.rAisSpecificBssInfo.
				...
				DBGLOG(RSN, TRACE,
					...
					MAC2STR(prAdapter->rWifiVar.
						...
						rBssidInfo.arPMKID)));
				kalMemCopy(cp,
					   (PVOID) prAdapter->rWifiVar.
					   rAisSpecificBssInfo.arPmkidCache[u4Entry].rBssidInfo.
					   arPMKID, sizeof(PARAM_PMKID_VALUE));
				/* ucExpendedLen = 40; */
			} else {
	...
	for (i = 0; i < prAisSpecBssInfo->u4PmkidCandicateCount; i++) {
		...
		if (j == prAisSpecBssInfo->u4PmkidCacheCount
			...
			kalMemCopy((PVOID)
				   ...
				   MAC_ADDR_LEN);
			prAisSpecBssInfo->arPmkidCache[prAisSpecBssInfo->
						       u4PmkidCacheCount].fgPmkidExist = FALSE;
