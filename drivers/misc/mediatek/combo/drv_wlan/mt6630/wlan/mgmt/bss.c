	...
	if ((!pucDestAddr) &&	/* For Beacon only. */
		...
		if (prBssInfo->eNetworkType == NETWORK_TYPE_P2P) {
			...
			TIM_IE(pucBuffer)->ucLength =
			    (3 + MAX_LEN_TIM_PARTIAL_BMP) /*((u4N2 - u4N1) + 4) */;
			 ...
			 IN P_BSS_INFO_T prBssInfo, IN P_BSS_DESC_T prBssDesc, IN UINT_8 ucRCPI)
{
	...
	if (fgIsCheckCapability) {
		...
		do {
			if (!
			    (prBssDesc->ucPhyTypeSet & (prAdapter->rWifiVar.ucAvailablePhyTypeSet)))
			{
				DBGLOG(BSS, LOUD,
	...
	if (prBssInfo->ucNetTypeIndex == NETWORK_TYPE_P2P_INDEX) {
		...
		if (u2AssocId == 0) {
			...
			pucPartialVirtualBitmap =
			    &prP2pSpecificBssInfo->aucPartialVirtualBitmap[(u2AssocId >> 3)];
