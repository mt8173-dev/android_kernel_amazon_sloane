				 ...
				 IN ENUM_P2P_ROLE_STATE_T eNextState)
{
	...
	do {
		...
		prBssInfo->eBand = prP2pRoleFsmInfo->rConnReqInfo.rChannelInfo.eBand;
		...
		ASSERT_BREAK((prAdapter != NULL) && (prP2pRoleFsmInfo != NULL)
			     && (prP2pRoleFsmInfo->rChnlReqInfo != NULL));
		prP2pRoleFsmInfo->rChnlReqInfo.u8Cookie = 0;
		...
		prP2pRoleFsmInfo->rChnlReqInfo.eChnlSco = prBssInfo->eBssSCO;
		prP2pRoleFsmInfo->rChnlReqInfo.u4MaxInterval = P2P_AP_CHNL_HOLD_TIME_MS;
		prP2pRoleFsmInfo->rChnlReqInfo.eChnlReqType = CH_REQ_TYPE_GO_START_BSS;

		if (prBssInfo->eBand == BAND_5G)
			prP2pRoleFsmInfo->rChnlReqInfo.eChannelWidth = CW_80MHZ;
		else
			prP2pRoleFsmInfo->rChnlReqInfo.eChannelWidth = CW_20_40MHZ;
