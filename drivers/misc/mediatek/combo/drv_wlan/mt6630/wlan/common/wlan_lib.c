...
wlanImageSectionDownloadStage(IN P_ADAPTER_T prAdapter,
			      IN PVOID pvFwImageMapFile, IN UINT_32 index,
			      IN UINT_32 u4FwImageFileLength, IN BOOLEAN fgValidHead,
		 ...
		 IN PVOID pvFwImageMapFile, IN UINT_32 u4FwImageFileLength)
{
	...
	do {
		...
		if (prRegInfo->prNvramSettings->
		    u2FeatureReserved & BIT(MTK_FEATURE_2G_256QAM_DISABLED)) {
			...
			if (wlanImageSectionConfig(prAdapter,
				...
				for (j = 0; j < prFwHead->arSection[index].u4Length;
					...
					if (wlanImageSectionDownload(prAdapter,
								     u4ImgSecSize, (PUINT_8)
								     pvFwImageMapFile
								     +
								     prFwHead->arSection[index].
								     u4Offset + j) !=
					    WLAN_STATUS_SUCCESS) {
						DBGLOG(INIT, ERROR,
	...
	while (prQueueEntry) {
		...
		if (eFrameAction == FRAME_ACTION_DROP_PKT) {
			...
			if (rStatus == WLAN_STATUS_RESOURCES) {
				...
				DBGLOG(INIT, WARN,
				       ("%s: NO Resource for CMD TYPE[%u] ID[0x%02X] SEQ[%u]\n",
...
WLAN_STATUS wlanSendCommand(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo)
{
	...
	do {
		...
		if (!prCmdInfo->fgDriverDomainMCR) {
			...
			rStatus =
			    nicTxAcquireResource(prAdapter, ucTC, nicTxGetCmdPageCount(prCmdInfo));
			if (rStatus == WLAN_STATUS_RESOURCES) {
		   ...
		   IN P_CMD_INFO_T prCmdInfo, IN ENUM_TX_RESULT_CODE_T rTxDoneStatus)
{
	...
	case COMMAND_TYPE_SECURITY_FRAME:
		DBGLOG(INIT, INFO, ("Free SEC Frame: BSS[%u] StaRec[%u] SeqNum[%u]\n",
				    prCmdInfo->ucBssIndex,
				    prCmdInfo->ucStaRecIndex, prCmdInfo->ucCmdSeqNum));
...
wlanFwDwnloadHandler(IN P_ADAPTER_T prAdapter,
		     IN UINT_32 u4FwImgLength, IN PVOID pvFwImageMapFile,
		     OUT WLAN_STATUS *u4Status)
{
	...
	do {
		if (kalIsCardRemoved(prAdapter->prGlueInfo) == TRUE || fgIsBusAccessFailed == TRUE) {
			...
			if (prInitHifRxHeader->rInitWifiEvent.ucEID != INIT_EVENT_ID_PENDING_ERROR) {
				...
				prEventPendingError =
				    (P_INIT_EVENT_PENDING_ERROR) (prInitHifRxHeader->
	...
	do {
		if (kalIsCardRemoved(prAdapter->prGlueInfo) == TRUE || fgIsBusAccessFailed == TRUE) {
			...
			if (prInitHifRxHeader->rInitWifiEvent.ucEID != INIT_EVENT_ID_CMD_RESULT) {
				...
				prEventCmdResult =
				    (P_INIT_EVENT_CMD_RESULT) (prInitHifRxHeader->
							       rInitWifiEvent.aucBuffer);
...
VOID wlanInitFeatureOption(IN P_ADAPTER_T prAdapter)
{
	...
	prWifiVar->ucStaHtBfee =
	    (UINT_8) wlanCfgGetUint32(prAdapter, "StaHTBfee", FEATURE_DISABLED);
	prWifiVar->ucStaVhtBfee =
	    (UINT_8) wlanCfgGetUint32(prAdapter, "StaVHTBfee", FEATURE_ENABLED);
	...
	prWifiVar->u4HifIstLoopCount =
	    (UINT_32) wlanCfgGetUint32(prAdapter, "IstLoop", CFG_IST_LOOP_COUNT);
	...
	prWifiVar->ucRxBaSize = (UINT_8) wlanCfgGetUint32(prAdapter, "RxBaSize", 16);
	...
	prWifiVar->au4TcPageCount[TC0_INDEX] =
	    (UINT_32) wlanCfgGetUint32(prAdapter, "Tc0Page", NIC_TX_PAGE_COUNT_TC0);
	prWifiVar->au4TcPageCount[TC1_INDEX] =
	    (UINT_32) wlanCfgGetUint32(prAdapter, "Tc1Page", NIC_TX_PAGE_COUNT_TC1);
	prWifiVar->au4TcPageCount[TC2_INDEX] =
	    (UINT_32) wlanCfgGetUint32(prAdapter, "Tc2Page", NIC_TX_PAGE_COUNT_TC2);
	prWifiVar->au4TcPageCount[TC3_INDEX] =
	    (UINT_32) wlanCfgGetUint32(prAdapter, "Tc3Page", NIC_TX_PAGE_COUNT_TC3);
	prWifiVar->au4TcPageCount[TC4_INDEX] =
	    (UINT_32) wlanCfgGetUint32(prAdapter, "Tc4Page", NIC_TX_PAGE_COUNT_TC4);
	prWifiVar->au4TcPageCount[TC5_INDEX] =
	    (UINT_32) wlanCfgGetUint32(prAdapter, "Tc5Page", NIC_TX_PAGE_COUNT_TC5);
		...
		if (u4PktPrintPeriod && (prStaRec->u4TotalTxPktsNumber >= u4PktPrintPeriod)) {
			DBGLOG(TX, TRACE, ("[%u]N[%4lu]A[%5lu]M[%4lu]T[%4lu]E[%4lu]\n",
					   ...
					   (prStaRec->u4TotalTxPktsTime /
					    prStaRec->u4TotalTxPktsNumber),
					   prStaRec->u4MaxTxPktsTime, prStaRec->u4ThresholdCounter,
