...
BOOLEAN nicRxIsDuplicateFrame(IN OUT P_SW_RFB_T prSwRfb)
{
	...
	if (RXM_IS_QOS_DATA_FRAME(u2FrameCtrl)) {	/* WLAN header shall exist when doing duplicate detection */
		if (prSwRfb->prStaRec->aprRxReorderParamRefTbl[prSwRfb->ucTid]) {
			...
			if (g_prMqm->
			    arRxBaTable[prSwRfb->prStaRec->aucRxBaTable[prSwRfb->ucTID]].ucStatus ==
			...
			if (prSwRfb->
			    prStaRec->ucDesiredPhyTypeSet & (PHY_TYPE_BIT_HT | PHY_TYPE_BIT_VHT)) {
		...
		if (u2SequenceControl != prSwRfb->prStaRec->au2CachedSeqCtrl[u4SeqCtrlCacheIdx]) {
			...
			if (prSwRfb->prStaRec->afgIsIgnoreAmsduDuplicate[u4SeqCtrlCacheIdx]) {
				if (fgIsAmsduSubframe == RX_PAYLOAD_FORMAT_LAST_SUB_AMSDU) {
					prSwRfb->
					    prStaRec->afgIsIgnoreAmsduDuplicate[u4SeqCtrlCacheIdx] =
					    FALSE;
				}
			} else {
				fgIsDuplicate = TRUE;
	...
	case EVENT_ID_TX_DONE:
		{
			...
			DBGLOG(INIT, INFO, ("EVENT_ID_TX_DONE WIDX:PID[%u:%u] Status[%u] SN[%u]\n",
					    ...
					    prTxDone->ucStatus, prTxDone->u2SequenceNumber));
	...
	case EVENT_ID_UPDATE_NOA_PARAMS:
		...
		if (prAdapter->fgIsP2PRegistered) {
			...
			if (GET_BSS_INFO_BY_INDEX
			    (prAdapter,
			     prEventUpdateNoaParam->ucBssIndex)->eNetworkType == NETWORK_TYPE_P2P) {
...
VOID nicRxSDIOReceiveRFBs(IN P_ADAPTER_T prAdapter)
{
	...
	for (rxNum = 0; rxNum < 2; rxNum++) {
		u2RxPktNum =
		    (rxNum ==
		     0 ? prSDIOCtrl->rRxInfo.u.u2NumValidRx0Len : prSDIOCtrl->rRxInfo.
		     u.u2NumValidRx1Len);
...
VOID nicRxSDIOAggReceiveRFBs(IN P_ADAPTER_T prAdapter)
{
	...
	do {
		...
		for (rxNum = 0; rxNum < 2; rxNum++) {
			u2RxPktNum =
			    (rxNum ==
			     0 ? prEnhDataStr->rRxInfo.u.u2NumValidRx0Len : prEnhDataStr->rRxInfo.
			     u.u2NumValidRx1Len);
			...
			for (i = 0; i < u4RxAggCount; i++) {
				...
				if (ALIGN_4(u2PktLength + HIF_RX_HW_APPENDED_LEN) >
				    CFG_RX_MAX_PKT_SIZE) {
					DBGLOG(RX, ERROR,
					       ("[%s] Request_len(%d) is greater than CFG_RX_MAX_PKT_SIZE(%d)...Drop the unexpected packet...\n",
						__func__,
