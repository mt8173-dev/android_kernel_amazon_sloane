	...
	while (1) {
		...
		if (u4RegValue & WHLPCR_FW_OWN_REQ_SET) {
			...
			if ((prAdapter->u4OwnFailedCount == 0) ||
				...
				DBGLOG(INIT, ERROR,
				       ("LP cannot be own back, Timeout[%u](%ums), BusAccessError[%u], Reseting[%u], CardRemoved[%u] NoAck[%u] Cnt[%u]\n",
					fgTimeout, kalGetTimeTick() - u4CurrTick,
					fgIsBusAccessFailed, kalIsResetting(),
...
BOOLEAN nicpmSetAcpiPowerD0(IN P_ADAPTER_T prAdapter)
{
	...
	do {
		...
		if (pvFwImageMapFile) {
			...
			if (fgValidHead == TRUE) {
				wlanFwDvdDwnloadHandler(prAdapter, prFwHead, pvFwImageMapFile,
							&u4Status);
			} else
#endif
			{
