...
int _ioctl_trigger_session(unsigned long arg)
{
	...
	if (DISP_SESSION_TYPE(session_id) == DISP_SESSION_PRIMARY) {
		...
		/* /if(config.mode != DISP_SESSION_DIRECT_LINK_MIRROR_MODE && config.mode != DISP_SESSION_DECOUPLE_MIRROR_MODE) */
		{
...
int _ioctl_set_input_buffer(unsigned long arg)
{
	...
	if (DISP_SESSION_TYPE(session_id) == DISP_SESSION_PRIMARY) {
		unsigned int mva_offset = 0;
		/* unsigned int ticket = primary_display_get_ticket(); */
		...
		for (i = 0; i < session_input.config_layer_num; i++) {
			dst_mva = 0;
			/* /DISPMSG("setip i:%d, layer_id:%d, layer_en:%d idx 0x%x\n", i, session_input.config[i].layer_id, session_input.config[i].layer_enable, session_input.config[i].next_buff_idx); */
			...
			if (session_input.config[i].layer_enable) {
				if (session_input.config[i].src_phy_addr) {
					dst_mva =
					    (unsigned int)session_input.config[i].src_phy_addr;
				} else {
					disp_sync_query_buf_info(session_id, layer_id,
								 (unsigned int)
								 session_input.config[i].
								 next_buff_idx, &dst_mva,
								 &dst_size);
					/* if(dst_size < session_input.config[i].src_pitch * session_input.config[i].src_height) */
					/* DISPERR(""); */
				}
				...
				DISPPR_FENCE
				    ("S+/L%d/e%d/id%d/%dx%d(%d,%d)(%d,%d)/%s/%d/0x%08x/mva0x%08x\n",
				     ...
				     session_input.config[i].src_phy_addr, dst_mva);
			} else {
		...
		primary_display_config_input_multiple((primary_disp_input_config *) &
						      primary_input);
	} else if (DISP_SESSION_TYPE(session_id) == DISP_SESSION_EXTERNAL) {
#ifdef CONFIG_MTK_HDMI_SUPPORT
		...
		for (i = 0; i < session_input.config_layer_num; ++i) {
			...
			/* /_sync_convert_fb_layer_to_ovl_struct(session_input.session_id, &(session_input.config[i]),&external_cached_layer_config[layer_id], dst_mva); */
			disp_sync_put_cached_layer_info(session_id, layer_id,
							&session_input.config[i], dst_mva);
			_sync_convert_fb_layer_to_disp_input(session_input.session_id,
							     &(session_input.config[i]),
			...
			if (session_info) {
				dprec_submit(&session_info->event_setinput,
					     ...
					     (session_input.
					      config[i].layer_id << 24) | (session_input.
									   config[i].src_fmt << 12)
					     | session_input.config[i].layer_enable);
			}
		}
		...
		for (i = 0; i < session_input.config_layer_num; i++) {
			...
			if (session_input.config[i].layer_enable) {
				...
				DISPPR_FENCE
				    ("S+/L%d/e%d/id%d/%dx%d(%d,%d)(%d,%d)/%s/%d/0x%08x/mva0x%08x/t%d\n",
				     session_input.config[i].layer_id,
				     session_input.config[i].layer_enable,
				     session_input.config[i].next_buff_idx,
				     session_input.config[i].src_width,
			...
			_sync_convert_fb_layer_to_ovl_struct(session_input.session_id,
							     &(session_input.config[i]),
							     &ovl2mem_in_cached_config[layer_id],
							     dst_mva);
			/* /disp_sync_put_cached_layer_info(session_id, layer_id, &session_input.config[i], get_ovl2mem_ticket()); */
...
#if 0
	DISPMSG
	    ("_convert_fb_layer_to_disp_input():id=%u, en=%u, next_idx=%u, vaddr=0x%x, paddr=0x%x, src fmt=%s, dst fmt=%u, pitch=%u, xoff=%u, yoff=%u, w=%u, h=%u, aen=%u\n",
