...
#undef CONFIG_OF		/* diplay driver don't use CONFIG_OF */
...
#if 1
	DISPMSG
	    ("_convert_fb_layer_to_disp_input():id=%u, en=%u, next_idx=%u, vaddr=0x%x, paddr=0x%x, src fmt=%u, dst fmt=%u, pitch=%u, xoff=%u, yoff=%u, w=%u, h=%u\n",
	...
	case MTKFB_SLT_AUTO_CAPTURE:
		{
			...
			if (copy_from_user(&capConfig, (void __user *)arg, sizeof(capConfig))) {
				...
				primary_display_capture_framebuffer_ovl((unsigned int)
...
void _parse_tag_videolfb(void)
{
	...
	if (of_chosen) {
		videolfb_tag =
		    (struct tag_videolfb *)of_get_property(of_chosen, "atag,videolfb", NULL);
		if (videolfb_tag) {
			memset((void *)mtkfb_lcm_name, 0, sizeof(mtkfb_lcm_name));
	...
	if (node) {
		/* frame buffer */
		videolfb_tag =
		    (struct tag_videolfb *)of_get_flat_dt_prop(node, "atag,videolfb", NULL);
		if (videolfb_tag) {
			memset((void *)mtkfb_lcm_name, 0, sizeof(mtkfb_lcm_name));
			strcpy((char *)mtkfb_lcm_name, videolfb_tag->lcmname);
			mtkfb_lcm_name[strlen(videolfb_tag->lcmname)] = '\0';
...
static int mtkfb_probe(struct device *dev)
{
...

	{
		...
		DAL_STATUS ret;
		unsigned int fbVA = (unsigned int)fbdev->fb_va_base;
...
int mtkfb_get_debug_state(char *stringbuf, int buf_len)
{
	...
	len +=
	    scnprintf(stringbuf + len, buf_len - len,
		      "|Framebuffer VA:0x%08x, PA:0x%08x, MVA:0x%08x, Reserved Size:0x%08x|%d\n",
