		...
		if (_disp_fence_context[i].session_id == 0xffffffff) {
			DISPMSG
			    ("not found session info for session_id:0x%08x,insert 0x%08x to array index:%d\n",
			     session_id, (unsigned int)&(_disp_fence_context[i]), i);
			...
			dprec_logger_event_init(&session_info->event_prepare, name,
						...
						(MMP_Event *) ddp_mmp_get_events()->session_Parent);
			...
			for (j = 0;
				...
				layer_info->inited = 1;
				layer_info->timeline =
				    (struct sy_sync_timeline *)timeline_create(name);
...
unsigned int mtkfb_query_buf_mva(unsigned int session_id, unsigned int layer_id, unsigned int idx)
{
	...
	if (mva != 0x0) {
		...
		if (buf->cache_sync) {
			...
			dprec_logger_start(DPREC_LOGGER_DISPMGR_CACHE_SYNC, (unsigned int)buf->hnd,
					   buf->mva);
			mtkfb_ion_cache_flush(ion_client, buf->hnd);
			dprec_logger_done(DPREC_LOGGER_DISPMGR_CACHE_SYNC, (unsigned int)buf->hnd,
					  buf->mva);
		}
				      ...
				      unsigned int idx, unsigned int *mva, unsigned int *size)
{
	...
	if (dst_mva != 0x0) {
		...
		*size = dst_size;
		...
		/* FIXME: non-ion buffer need cache sync here? */
		DISPERR
		    ("cannot find this buf, session:%s%d,layer=%d, idx=%d, fence_idx=%d, timeline_idx=%d, cur_idx=%d!\n",
		     disp_session_mode_spy(session_id), DISP_SESSION_DEV(session_id), timeline_id,
