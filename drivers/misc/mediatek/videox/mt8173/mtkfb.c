...
int mtkfb_set_backlight_level(unsigned int level)
{
	MTKFB_FUNC();
#if HDMI_SUB_PATH
	DISPMSG("error: mtkfb_set_backlight_level box can not set backlight\n");
#else
...
#if 1
	DISPDBG
	    ("_convert_fb_layer_to_disp_input():id=%u, en=%u, next_idx=%u, vaddr=0x%lx, paddr=0x%x, src fmt=%u, dst fmt=%u, pitch=%u, xoff=%u, yoff=%u, w=%u, h=%u\n",
	     dst->layer, dst->layer_en, dst->buff_idx, (unsigned long)(dst->vaddr),
	     (unsigned int)(dst->addr), src->src_fmt, dst->fmt, dst->src_pitch, dst->src_x,
...
#if 1
	DISPMSG
	    ("_convert_fb_layer_to_disp_input():id=%u, en=%u, next_idx=%u, vaddr=0x%x, paddr=0x%x, fmt=%u, pitch=%u, xoff=%u, yoff=%u, w=%u, h=%u\n",
	     dst->layer, dst->layer_en, dst->buff_idx, (unsigned int)(dst->addr),
	     (unsigned int)(dst->vaddr), dst->fmt, dst->src_pitch, dst->src_x, dst->src_y,
	     dst->src_w, dst->src_h);
	DISPMSG
	    ("_convert_fb_layer_to_disp_input():target xoff=%u, target yoff=%u, target w=%u, target h=%u, aen=%u\n",
	     dst->dst_x, dst->dst_y, dst->dst_w, dst->dst_h, dst->aen);
...
static int mtkfb_pan_display_impl(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	DISPDBG
	    ("vaStart0x%x,paStart0x%lx,xoffset=%u, yoffset=%u, xres=%u, yres=%u, xresv=%u, yresv=%u,"
	     ...
	     (unsigned long)vaStart, paStart, var->yoffset, info->var.xres, info->var.yres,
	     info->var.xres_virtual, info->var.yres_virtual, var->blue.offset, var->green.offset,
...
#if HDMI_SUB_PATH
	DISPDBG("%s ext_disp_config_input\n", __func__);
	if (factory_mode) {
...
static int mtkfb_check_var(struct fb_var_screeninfo *var, struct fb_info *fbi)
{
	...
	DISPDBG("fbdev->fb_size_in_byte=0x%lx\n", fbdev->fb_size_in_byte);
...
static int mtkfb_set_par(struct fb_info *fbi)
{
	...
	/*DISPDBG("fb_va=0x%lx\n", fb_layer.src_base_addr); */
	DISPDBG("fb_pa=0x%lx, var->yoffset=0x%08x,fbi->fix.line_length=0x%08x\n", fb_pa,
...
#if HDMI_SUB_PATH
	DISPDBG("%s ext_disp_config_input && ext_disp_trigger factory_mode %d\n", __func__,
		factory_mode);
...
static int mtkfb_ioctl(struct fb_info *info, unsigned int cmd, unsigned long arg)
{
	...
	DISPMSG("mtkfb_ioctl, info=0x%lx, cmd=0x%08x, arg=0x%08x\n", (unsigned long)info,
...
#if HDMI_SUB_PATH
				DISPMSG("%s MTKFB_SET_OVERLAY_LAYER ext_disp_config_input\n",
					__func__);
				if (factory_mode) {
...
#if HDMI_SUB_PATH
					if (factory_mode) {
						DISPMSG("%s  ext_disp_config_input\n", __func__);
	...
	case MTKFB_PREPARE_OVERLAY_BUFFER:
		{
			...
			if (copy_from_user
			    (&overlay_buffer, (void __user *)arg, sizeof(overlay_buffer))) {
				printk("[FB Driver]: copy_from_user failed! line:%d\n", __LINE__);
				r = -EFAULT;
			} else {
				if (overlay_buffer.layer_en) {
					buf = mtkfb_prepare_buf((struct fb_overlay_buffer_t *)
								&overlay_buffer);
					if (buf != NULL) {
				...
				if (copy_to_user
				    ((void __user *)arg, &overlay_buffer, sizeof(overlay_buffer))) {
	...
	case MTKFB_SET_MULTIPLE_LAYERS:
		{
			...
			if (copy_from_user(&layerInfo, (void __user *)arg, sizeof(layerInfo))) {
				MTKFB_LOG("[FB]: copy_from_user failed! line:%d\n", __LINE__);
				MMProfileLogMetaString(MTKFB_MMP_Events.SetMultipleLayers,
						       MMProfileFlagEnd, "Copy_from_user failed!");
				r = -EFAULT;
			} else {
				int32_t i;
				...
				for (i = 0; i < HW_OVERLAY_COUNT; ++i) {
					DISPMSG("mslayer(0x%08x) bidx (%d)\n",
						(layerInfo[i].
						 layer_id << 24 | layerInfo[i].layer_enable),
					...
					if (layerInfo[i].layer_enable
						...
						MTKFB_ERR
						    ("layerId(%d) HWC reset the same buffer(%d)!\n",
					...
					if (layerInfo[i].layer_enable
						...
						DISPERR
						    ("layerId(%d) HWC set the old buffer(%d),timeline_idx(%d)!\n",
						     layerInfo[i].layer_id,
						     layerInfo[i].next_buff_idx,
						     mtkfb_fence_timeline_index(i));
					...
					if (is_early_suspended) {
						DISPERR
						    ("in early suspend layer(0x%x),idx(%d),time_idx(%d)!\n",
						     layerInfo[i].
						     layer_id << 16 | layerInfo[i].layer_enable,
						     layerInfo[i].next_buff_idx,
						     mtkfb_fence_timeline_index(i));
					...
					if (!(isAEEEnabled && i == DISP_DEFAULT_UI_LAYER_ID)) {
						_overlay_info_convert(&layerInfo[i],
								      &cached_layer_config[i]);
					}
#endif
...
#if HDMI_SUB_PATH
#else
static void disp_clear_current_fb_buffer(void)
{
	struct mtkfb_device *fbdev = (struct mtkfb_device *)mtkfb_fbi->par;
			      ...
			      unsigned int h, unsigned int color)
{
	...
	printk("@(%d,%d)addr=0x%lx, MTK_FB_XRESV=%d, draw_block start addr=0x%lx, w=%d, h=%d\n", x,
	       y, addr, MTK_FB_XRESV, start_addr, w, h);
...
static int _mtkfb_internal_test(unsigned long va, unsigned int w, unsigned int h, unsigned int fmt)
{
	...
	printk("UT starts @ va=0x%lx, w=%d, h=%d\n", va, w, h);
...
void _parse_tag_videolfb(void)
{
	...
	if (np == NULL) {
		DISPERR("DT <mediatek,MTKFB> is not config\n");
		return;
	}

	if (of_address_to_resource(mtkfb_fbdev->dev.of_node, 0, &res)) {
		DISPERR("mtkfb of_address_to_resource failed\n");
		return;
	}
	if (lcm_driver_list[0] == NULL)	/* peeking lcm param before primary_display does lcm_probe */
		printk("[MTKFB ERROR] there is no lcm found so peeking lcm param fail");
	else
		islcmconnected = 1;
	...
	DISPCHECK("[DT][videolfb] res.end       = 0x%x\n", res.end);
#else
	...
	DISPCHECK("[DT][videolfb] vram       = 0x%x\n", vramsize);
...
static int mtkfb_probe(struct device *dev)
{
	...
	for (i = 0; i < MM_CLK_NUM; i++) {
		...
		if (IS_ERR(ddp_clk_map[i]))
			bug_idx = i;
		printk("***DT|DISPSYS clock|%s ID %d: 0x%lx\n", ddp_get_clk_name(i), i,
		       (unsigned long)ddp_clk_map[i]);
	}
	if (bug_idx >= 0) {
		printk("DISP get clock error on ddp_clk_map[%d]:%s\n", bug_idx,
...
#if HDMI_SUB_PATH
	ext_disp_common_init();
	ext_disp_config_m4u_port();
#else
...
#if HDMI_SUB_PATH && HDMI_SUB_PATH_PROB
	{
		...
		DISPMSG("%s ext_disp_config_input boot_mode %d factory_mode %d\n", __func__,
	...
	_mtkfb_internal_test((unsigned long)fbdev->fb_va_base, MTK_FB_XRES, MTK_FB_YRES, MTK_FB_FORMAT_BGRA8888);
	/* check RGBA color format */
	mtkfb_fbinfo_modify(fbi, MTK_FB_FORMAT_RGBA8888);
	_mtkfb_internal_test((unsigned long)fbdev->fb_va_base, MTK_FB_XRES, MTK_FB_YRES, MTK_FB_FORMAT_RGBA8888);
