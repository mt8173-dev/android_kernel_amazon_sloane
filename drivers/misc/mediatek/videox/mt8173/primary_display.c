		...
		if (primary_display_is_video_mode() == 1) {	/* only for video mode */
			...
			if (enter) {
				...
				if (pgc->session_mode == DISP_SESSION_DECOUPLE_MODE) {
					DISPDBG("[LP]switch to directlink.\n");
					_disp_primary_low_power_change_ddp_clock(enter);
					primary_display_switch_mode_nolock
...
static int _disp_primary_path_idle_detect_thread(void *data)
{
	...
	while (1) {
		...
		if (gSkipIdleDetect || atomic_read(&isDdp_Idle) == 1) {	/* skip is already in idle mode */
			DISPMSG("[ddp_idle]_disp_primary_path_idle_detect is already Idle\n");
			continue;
		}
		_primary_path_lock(__func__);
		if (pgc->state == DISP_SLEEPED) {
...
static void _cmdq_build_trigger_loop(void)
{
	...
	if (pgc->cmdq_handle_trigger == NULL) {
		...
		DISPMSG("primary path trigger thread cmd handle=0x%p\n", pgc->cmdq_handle_trigger);
	}
...
static void disp_sodi_set_dram_self_refresh(unsigned int enable, void *cmdq_handle)
{
	...
	if (is_drma_reg_map == 0) {
		reg_va_a = (unsigned long)ioremap_nocache(DISP_REG_DRAM_SELF_REFRESH_A, sizeof(unsigned long));
		reg_va_b = (unsigned long)ioremap_nocache(DISP_REG_DRAM_SELF_REFRESH_B, sizeof(unsigned long));
		is_drma_reg_map = 1;
	}
	...
	if (gEnableSODIControl == 1) {
		if (cmdq_handle != NULL) {
			if (enable == 1) {
				cmdqRecWrite(cmdq_handle, DISP_REG_DRAM_SELF_REFRESH_A, 1<<26, 1<<26);
				cmdqRecWrite(cmdq_handle, DISP_REG_DRAM_SELF_REFRESH_B, 1<<26, 1<<26);
			} else {
		...
		if (lcm_fake_width && lcm_fake_height) {
			if (DISP_STATUS_OK !=
			    primary_display_change_lcm_resolution(lcm_fake_width,
								  lcm_fake_height)) {
				DISPMSG("[DISP]WARNING!!! Change LCM Resolution FAILED!!!\n");
			}
		}
	}
#endif
...
int primary_display_switch_mode(int sess_mode, unsigned int session, int force)
{
	...
	DISPDBG("primary_display_switch_mode switch mode from %d(%s) to %d(%s)\n",
		pgc->session_mode, session_mode_spy(pgc->session_mode), sess_mode, session_mode_spy(sess_mode));
	...
	DISPMSG("primary_display_switch_mode from DISP_IOCTL_SET_SESSION_MODE\n");
...
static int primary_display_switch_mode_nolock(int sess_mode, unsigned int session, int force)
{
	...
	/* _primary_path_unlock(__func__); */
	DISPMSG("leave primary_display_switch_mode_nolock\n");
	pgc->session_id = session;
