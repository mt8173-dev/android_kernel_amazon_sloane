...
bool mtkfb_update_secure_buf_info(unsigned int session_id, unsigned int layer_id, unsigned int idx,
			   unsigned int mva_offset, unsigned int secure_handle, unsigned int isSecure)
{
	...
	list_for_each_entry(buf, &layer_info->buf_list, list) {
		if (buf->idx == idx && (!buf->secure_handle)) {
			...
			ret = true;
			if (isSecure) {
				TZ_RESULT res = KREE_ReferenceSecurechunkmem(extd_disp_secure_memory_session_handle(), secure_handle);
				if (res != TZ_RESULT_SUCCESS)
					DISPERR("KREE_ReferenceSecurechunkmem failed (%s), sec_handle 0x%x\n", TZ_GetErrorString(res), secure_handle);
				DISP_PRINTF(DDP_SECURE_LOG, "KREE_ReferenceSecurechunkmem,sec_handle 0x%x\n", secure_handle);
			}
			break;
		}
	}
			...
			if (buf->idx <= fence) {
...
#ifdef CONFIG_MTK_SEC_VIDEO_PATH_SUPPORT
				if (buf->isScure) {
					TZ_RESULT res = KREE_UnreferenceSecurechunkmem(extd_disp_secure_memory_session_handle(), buf->secure_handle);
					if (res != TZ_RESULT_SUCCESS)
						DISPERR("KREE_UnreferenceSecurechunkmem failed (%s), sec_handle 0x%x\n", TZ_GetErrorString(res), buf->secure_handle);
					DISP_PRINTF(DDP_SECURE_LOG, "KREE_UnreferenceSecurechunkmem,sec_handle 0x%x\n", buf->secure_handle);
				}
#endif
				...
				if (DISP_SESSION_TYPE(session_id) == DISP_SESSION_EXTERNAL
					...
					MMProfileLogEx(ddp_mmp_get_events()->
						       Extd_fence_release[layer_id],
						       MMProfileFlagPulse, buf->idx, fence);
				}
			}
		}
