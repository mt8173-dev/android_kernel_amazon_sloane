...
int disp_create_session(disp_session_config *config)
{
	...
	if (create_session_count != 0) {
		DISPMSG("don't destory session last time, so need destory before create\n");
...
int _ioctl_trigger_session(unsigned long arg)
{
	...
	if (DISP_SESSION_TYPE(session_id) == DISP_SESSION_PRIMARY) {
		   ...
		   && config.mode != DISP_SESSION_DECOUPLE_MIRROR_MODE) */
		{
			...
			ret = -1;
			DISPERR
			    ("primary display path is removed in box,session_id = 0x%x return directly in %s\n",
			     session_id, __func__);
#else
...
int _ioctl_prepare_present_fence(unsigned long arg)
{
	...
	/* disable present fence when rdma stops */
	if (is_hdmi_rdma_stop)
		use_present_fence = 0;
	...
	if (use_present_fence == 0) {
		...
		data.fence = MTK_FB_INVALID_FENCE_FD;
		data.value = ++fence_idx;
		sprintf(data.name, "present_fence-0x%x-%d-%d", preset_fence_struct.session_id, disp_sync_get_present_timeline_id(), data.value);
...
static int set_external_buffer(disp_session_input_config session_input)
{
	...
	for (i = 0; i < session_input.config_layer_num; ++i) {
		...
		if (session_input.config[i].layer_enable) {
			...
			mtkfb_update_secure_buf_info(session_input.session_id,
					      session_input.config[i].layer_id,
					      session_input.config[i].next_buff_idx,
					      mva_offset,
					      dst_mva,
					      extd_input[layer_id].security);
			#else
	...
	ret = ext_disp_config_input_multiple((ext_disp_input_config *) &extd_input,
...
int _ioctl_set_input_buffer(unsigned long arg)
{
	...
	if (ret == -2) {
		DISPMSG("_ioctl_set_input_buffer : mtkfb_release_layer_fence\n");
		for (i = 0; i < session_input.config_layer_num; i++)
			mtkfb_release_layer_fence(session_input.session_id, i);
	}
...
extern unsigned int get_frame_drop_count(void);
int _ioctl_get_frame_drop_count(unsigned long arg)
{
	int ret = 0;
	void __user *argp = (void __user *)arg;
	unsigned int frame_drop_count = 0;

	frame_drop_count = get_frame_drop_count();
	if (copy_to_user(argp, &frame_drop_count, sizeof(frame_drop_count))) {
		DISPMSG("[FB]: copy_to_user failed! line:%d\n", __LINE__);
		ret = -EFAULT;
	}
...
int _ioctl_get_rdma_frame_drop_count(unsigned long arg)
{
	int ret = 0;
	void __user *argp = (void __user *)arg;
	unsigned long long rdma_drop_frame_count = 0;

	rdma_drop_frame_count = get_rdma_frame_drop_count();
	if (copy_to_user(argp, &rdma_drop_frame_count, sizeof(rdma_drop_frame_count))) {
		DISPMSG("%s : copy_to_user failed!\n", __func__);
		ret = -EFAULT;
	}
...
long mtk_disp_mgr_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case DISP_IOCTL_GET_FRAME_DROP_COUNT:
		{
			return _ioctl_get_frame_drop_count(arg);
		}
	case DISP_IOCTL_GET_RDMA_FRAME_DROP_COUNT:
		{
			return _ioctl_get_rdma_frame_drop_count(arg);
		}
	...
	case DISP_IOCTL_PQ_SET_DC_PARAM:
		{
#if HDMI_SUB_PATH
			DISPMSG("[session] ioctl not supported in primary path, cmd 0x%08x %s\n",
				cmd, _session_ioctl_spy(cmd));
#else
