...
void mt_power_gs_dump_dpidle(void)
{
	mt_power_gs_compare("DPIdle",
			    ...
			    MT6331_PMIC_REG_gs_early_suspend_deep_idle,
			    MT6331_PMIC_REG_gs_early_suspend_deep_idle_len,
			    MT6332_PMIC_REG_gs_early_suspend_deep_idle,
			    MT6332_PMIC_REG_gs_early_suspend_deep_idle_len);
}
...
static int __init mt_power_gs_dpidle_init(void)
{
	...
	if (!mt_power_gs_dir) {
		...
		mt_entry =
		    proc_create("dump_dpidle", S_IRUGO | S_IWUSR | S_IWGRP, mt_power_gs_dir,
				&proc_fops);
		if (NULL == mt_entry) {
			return -ENOMEM;
		}
