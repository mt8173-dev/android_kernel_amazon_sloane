...
#define ID_NAME(n)	{n, __stringify(n)}
	...
	ID_NAME(ARMCA15PLL),
		    ...
		    ID_NAME(VENCPLL),
		    ID_NAME(TVDPLL),
		    ID_NAME(MPLL), ID_NAME(VCODECPLL), ID_NAME(APLL1), ID_NAME(APLL2),};

	struct {
		const int id;
		const char *name;
	} subsyss[NR_SYSS] = {
		ID_NAME(SYS_MD1), ID_NAME(SYS_DIS),
/* ID_NAME(SYS_MFG_ASYNC), */
/* ID_NAME(SYS_MFG_2D), */
	ID_NAME(SYS_MFG),
		    ID_NAME(SYS_ISP),
		    ID_NAME(SYS_VDE), ID_NAME(SYS_MJC), ID_NAME(SYS_VEN), ID_NAME(SYS_AUD),};
	...
	for (i = 0; i < ap_cg_gs_len; i += 3) {
		...
		if (val1 != val2) {
			printk("%s - AP CG - 0x%x - 0x%x - 0x%x - 0x%x - ",
			       scenario, ap_cg_gs[i], gs_read(ap_cg_gs[i]), ap_cg_gs[i + 1],
			       ap_cg_gs[i + 2]);
