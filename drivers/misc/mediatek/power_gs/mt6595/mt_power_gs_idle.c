...
void mt_power_gs_dump_idle(void)
{
	mt_power_gs_compare("Idle",
			    ...
			    MT6331_PMIC_REG_gs_early_suspend_idle,
			    MT6331_PMIC_REG_gs_early_suspend_idle_len,
			    MT6332_PMIC_REG_gs_early_suspend_idle,
			    MT6332_PMIC_REG_gs_early_suspend_idle_len);
}

static int dump_idle_read(struct seq_file *m, void *v)
{
	seq_puts(m, "mt_power_gs : idle\n");
...
static int __init mt_power_gs_idle_init(void)
{
	...
	if (!mt_power_gs_dir) {
		...
		mt_entry =
		    proc_create("dump_idle", S_IRUGO | S_IWUSR | S_IWGRP, mt_power_gs_dir,
				&proc_fops);
		if (NULL == mt_entry) {
			return -ENOMEM;
		}
