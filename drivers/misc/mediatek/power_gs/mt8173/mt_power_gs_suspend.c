...
void mt_power_gs_dump_suspend(void)
{
	mt_power_gs_compare("Suspend",
			    ...
			    MT6331_PMIC_REG_gs_flightmode_suspend_mode,
			    MT6331_PMIC_REG_gs_flightmode_suspend_mode_len,
			    MT6332_PMIC_REG_gs_flightmode_suspend_mode,
			    MT6332_PMIC_REG_gs_flightmode_suspend_mode_len);
}
...
static int __init mt_power_gs_suspend_init(void)
{
	...
	if (!mt_power_gs_dir) {
		...
		mt_entry =
		    proc_create("dump_suspend", S_IRUGO | S_IWUSR | S_IWGRP, mt_power_gs_dir,
				&proc_fops);
		if (NULL == mt_entry) {
			return -ENOMEM;
		}
