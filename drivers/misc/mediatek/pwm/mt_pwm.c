...
static void mt_pwm_power_on_all(void)
{
	mt_pwm_power_on(PWM1, false);
	mt_pwm_power_on(PWM2, false);
	mt_pwm_power_on(PWM3, false);
	mt_pwm_power_on(PWM4, false);
	mt_pwm_power_on(PWM5, false);
	mt_pwm_power_on(PWM6, false);
	mt_pwm_power_on(PWM7, false);
}

static void mt_pwm_power_off_all(void)
{
	mt_pwm_power_off(PWM1, false);
	mt_pwm_power_off(PWM2, false);
	mt_pwm_power_off(PWM3, false);
	mt_pwm_power_off(PWM4, false);
	mt_pwm_power_off(PWM5, false);
	mt_pwm_power_off(PWM6, false);
	mt_pwm_power_off(PWM7, false);
}
...
static S32 mt_set_pmic_pwm_disable(U32 pwm_no)
{
	U32 read_reg;
	if (pwm_no <= PWM4) { 	
		PWMDBG("pmic did't have this pwm number\n");
		return -EINVALID;
	}
	pwrap_read(PMIC_PWM_EN, &read_reg); 
	switch (pwm_no) { 
	case PWM5:	
			pwrap_write(PMIC_PWM_EN, (0 << (pwm_no)) | (read_reg &(~PWM5_EN_MASK)));		
			break;	
	case PWM6:		
			pwrap_write(PMIC_PWM_EN, (0 << (pwm_no)) | (read_reg &(~PWM6_EN_MASK)));		
			break;	
	case PWM7:		
			pwrap_write(PMIC_PWM_EN, (0 << (pwm_no)) | (read_reg & (~PWM7_EN_MASK))); 	
			break;
			default:		
			break;				
	}	
...
static S32 mt_set_pmic_pwm_power_off(U32 pwm_no)
{
  U32 read_reg;
	if (pwm_no <= PWM4) {
		PWMDBG("pmic did't have this pwm number\n");
	...
	pwrap_read(PMIC_PWM_CLK, &read_reg);	
	pwrap_write(PMIC_PWM_CLK, (1<< PWM_CON_CLKSEL_OFFSET) | (read_reg & (~PWM_CON_CLKSEL_MASK)));
	pwrap_read(PMIC_PWM_CLK, &read_reg);
	pwrap_write(PMIC_PWM_CLK, (1<< PWM_CON_CLKSEL_RTC_OFFSET) | (read_reg & (~PWM_CON_CLKSEL_RTC_MASK)));
	return RSUCCESS;
}
/*modify pmic only for 6397*/
void mt_pwm_disable(U32 pwm_no, BOOL pmic_pad)
{
    if (pmic_pad) {
 		mt_set_pmic_pwm_disable(pwm_no);
		mt_set_pmic_pwm_power_off(pwm_no);
	} else {
...
s32 pwm_set_pmic_config(struct pwm_spec_config *conf)
{
  U32 val;
	int offset;
	U32 read_reg;
	...
	switch (conf->pwm_no) {
	case PWM5:
		pwrap_write(PMIC_PWM_EN, (1 << (conf->pwm_no)) | (read_reg &(~PWM5_EN_MASK)));
		break;
	case PWM6:
		pwrap_write(PMIC_PWM_EN, (1 << (conf->pwm_no)) | (read_reg &(~PWM6_EN_MASK)));
		break;
	case PWM7:
		pwrap_write(PMIC_PWM_EN, (1 << (conf->pwm_no)) | (read_reg & (~PWM7_EN_MASK)));
		break;
	default:
		break;			
	}
	pwrap_read(PMIC_PWM_CLK, &read_reg);
	pwrap_write(PMIC_PWM_CLK, (CLK_BLOCK<< PWM_CON_CLKSEL_OFFSET) | (read_reg & (~PWM_CON_CLKSEL_MASK)));	
	offset = GET_PWD_OFFSET(conf->pwm_no);
	PWMDBG("offset(pwm%d):%#x\n", offset, conf->pwm_no);
	switch (conf->mode) {
	case PWM_MODE_OLD:
		PWMDBG("PWM_MODE_OLD\n");
		val = OLDMODE_ENABLE << PWM_CON_OLD_MODE_OFFSET;
		val |= conf->PWM_MODE_OLD_REGS.IDLE_VALUE << PWM_CON_IDLE_VALUE_OFFSET;
		val |= conf->PWM_MODE_OLD_REGS.GUARD_VALUE << PWM_CON_GUARD_VALUE_OFFSET;
		pwrap_write(PMIC_PWM_CON + offset, val);		
		pwrap_write(PMIC_PWM_GD + offset, conf->PWM_MODE_OLD_REGS.GDURATION);	
		pwrap_write(PMIC_PWM_WAVE_NUM + offset, conf->PWM_MODE_OLD_REGS.WAVE_NUM);	
		if (conf->pwm_no == PWM5)
		{
			pwrap_write(PMIC_PWM5_DATA_WIDTH + offset, conf->PWM_MODE_OLD_REGS.DATA_WIDTH);
		} else {
			pwrap_write(PMIC_PWM_DATA_WIDTH + offset, conf->PWM_MODE_OLD_REGS.DATA_WIDTH);
		}
		
		if (conf->pwm_no == PWM5)
		{
			pwrap_write(PMIC_PWM5_THRESH + offset, conf->PWM_MODE_OLD_REGS.THRESH);
		} else {
			pwrap_write(PMIC_PWM_THRESH + offset, conf->PWM_MODE_OLD_REGS.THRESH);
		}		
		PWMDBG("PWM set old mode finish\n");
		break;
	case PWM_MODE_FIFO:
		PWMDBG("PWM_MODE_FIFO\n");
		val = OLDMODE_DISABLE<< PWM_CON_OLD_MODE_OFFSET;
		val |= PWM_FIFO << PWM_CON_SRCSEL_OFFSET;
		val |= PERIOD << PWM_CON_MODE_OFFSET;
		val |= conf->PWM_MODE_FIFO_REGS.IDLE_VALUE << PWM_CON_IDLE_VALUE_OFFSET;
		val |= conf->PWM_MODE_FIFO_REGS.GUARD_VALUE << PWM_CON_GUARD_VALUE_OFFSET;
		val |= conf->PWM_MODE_FIFO_REGS.STOP_BITPOS_VALUE << PWM_CON_STOP_BITS_OFFSET;
		pwrap_write(PMIC_PWM_CON + offset, val);	
		pwrap_write(PMIC_PWM_HD + offset, conf->PWM_MODE_FIFO_REGS.HDURATION);		
		pwrap_write(PMIC_PWM_LD + offset, conf->PWM_MODE_FIFO_REGS.LDURATION);		
		pwrap_write(PMIC_PWM_GD + offset, conf->PWM_MODE_FIFO_REGS.GDURATION);	
		pwrap_write(PMIC_PWM_SEND_DATA00 + offset, conf->PWM_MODE_FIFO_REGS.SEND_DATA0 & PWM_SEND_DATA_LOW_MASK);
		pwrap_write(PMIC_PWM_SEND_DATA01 + offset, conf->PWM_MODE_FIFO_REGS.SEND_DATA0 >> PWM_SEND_DATA_LOW_OFFSET);	
		pwrap_write(PMIC_PWM_SEND_DATA10 + offset, conf->PWM_MODE_FIFO_REGS.SEND_DATA1 & PWM_SEND_DATA_LOW_MASK);
		pwrap_write(PMIC_PWM_SEND_DATA11 + offset, conf->PWM_MODE_FIFO_REGS.SEND_DATA1 >> PWM_SEND_DATA_LOW_OFFSET);	
		pwrap_write(PMIC_PWM_WAVE_NUM + offset, conf->PWM_MODE_FIFO_REGS.WAVE_NUM);		
		break;
	default:
		break;
	}
	...
	switch (conf->clk_src) {
	case PWM_CLK_OLD_MODE_BLOCK:
		pwrap_read(PMIC_PWM_CON + offset, &read_reg);
		pwrap_write(PMIC_PWM_CON + offset, (read_reg & (~PWM_CON_CLK_MASK)) | conf->clk_div);	
		PWMDBG("Enable oldmode and set clock block\n");
		break;
	case PWM_CLK_OLD_MODE_32K:
		pwrap_read(PMIC_PWM_CLK, &read_reg);
		pwrap_write(PMIC_PWM_CLK, (CLK_BLOCK<< PWM_CON_CLKSEL_RTC_OFFSET) | (read_reg & (~PWM_CON_CLKSEL_RTC_MASK)));	
		pwrap_read(PMIC_PWM_CON + offset, &read_reg);
		pwrap_write(PMIC_PWM_CON + offset, (read_reg & (~PWM_CON_CLK_MASK)) | conf->clk_div | PMIC_PWM_CLK_1625);		
	...
	mb();
	PWMDBG("mt_set_pmic_pwm_enable\n");
	return RSUCCESS;
}
/*modify pmic only for 6397
**FIFO mode: clk 24M
**OLD mode: clk 24M or 32K
**fifo_high_pulse = clk/(Vm * (Vh+1)) Vm: div,Vh: HDuration
**fifo_low_pulse = clk/(Vm * (Vl+1)) Vm: div,Vl: LDuration
**fifo_output = 1/(1/fifo_high_pulse + 1/fifo_low_pulse)
**eg: div:1,Vh:5,Vl:5 output 2M
*/
...
static int mt_pwm_probe(struct platform_device *pdev)
{
	...
	struct clk *clk;
		...
		clk = devm_clk_get(&pdev->dev, "MT_CG_PERI_PWM1");
		if (!IS_ERR(clk))
			mt_clk_pwm[0] = clk;
		clk = devm_clk_get(&pdev->dev, "MT_CG_PERI_PWM2");
		if (!IS_ERR(clk))
			mt_clk_pwm[1] = clk;
		clk = devm_clk_get(&pdev->dev, "MT_CG_PERI_PWM3");
		if (!IS_ERR(clk))
			mt_clk_pwm[2] = clk;
		clk = devm_clk_get(&pdev->dev, "MT_CG_PERI_PWM4");
		if (!IS_ERR(clk))
			mt_clk_pwm[3] = clk;
		clk = devm_clk_get(&pdev->dev, "MT_CG_PERI_PWM5");
		if (!IS_ERR(clk))
			mt_clk_pwm[4] = clk;
		clk = devm_clk_get(&pdev->dev, "MT_CG_PERI_PWM6");
		if (!IS_ERR(clk))
			mt_clk_pwm[5] = clk;
		clk = devm_clk_get(&pdev->dev, "MT_CG_PERI_PWM7");
		if (!IS_ERR(clk))
			mt_clk_pwm[6] = clk;
		clk = devm_clk_get(&pdev->dev, "MT_CG_PERI_PWM");
		if (!IS_ERR(clk))
			mt_clk_pwm[7] = clk;
		mt_pwm_power_on_all();
		mt_pwm_power_off_all();
#if 0
