...
void mt_pwm_power_on_hal(U32 pwm_no, BOOL pmic_pad, unsigned long *power_flag)
{
	if (0 == (*power_flag)) {
		...
		clk_prepare(mt_clk_pwm[PWM_CG]);
		clk_enable(mt_clk_pwm[PWM_CG]);
	...
	if (!test_bit(pwm_no, power_flag)) {
		...
		clk_prepare(mt_clk_pwm[pwm_no]);
		clk_enable(mt_clk_pwm[pwm_no]);
...
void mt_pwm_power_off_hal(U32 pwm_no, BOOL pmic_pad, unsigned long *power_flag)
{
	if (test_bit(pwm_no, power_flag)) {
		...
		clk_disable(mt_clk_pwm[pwm_no]);
		clk_unprepare(mt_clk_pwm[pwm_no]);
	...
	if (BIT(PWM_CG) == (*power_flag)) {
		...
		clk_disable(mt_clk_pwm[PWM_CG]);
		clk_unprepare(mt_clk_pwm[PWM_CG]);
...
void mt_pwm_init_power_flag(unsigned long *power_flag)
{
	...
	PWM_register[PWM4] = (unsigned long)pwm_base + 0x00d0;	/*PWM4 register base,   13 registers */
	PWM_register[PWM5] = (unsigned long)pwm_base + 0x0110;	/*PWM5 register base,   13 registers */
	PWM_register[PWM6] = (unsigned long)pwm_base + 0x0150;	/*PWM6 register base,   13 registers */
	PWM_register[PWM7] = (unsigned long)pwm_base + 0x0190;	/*PWM7 register base,   13 registers */
...
void mt_set_pwm_clk_hal(U32 pwm_no, U32 clksrc, U32 div)
{
	...
	if ((clksrc & 0x80000000) != 0) {	/* old mode 32k clk */
		clksrc &= ~(0x80000000);
...
void mt_pwm_dump_regs_hal(void)
{
	int i;
	long reg_val;
	reg_val = INREG32((long)PWM_ENABLE);
	PWMMSG("\r\n[PWM_ENABLE is:%lx]\n\r ", reg_val);
	reg_val = INREG32((long)PWM_CK_26M_SEL);
	PWMMSG("\r\n[PWM_26M_SEL is:%lx]\n\r ", reg_val);
