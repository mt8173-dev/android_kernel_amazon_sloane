				...
				upmu_adc_user_list_enum user_num)
{
	...
	if (chip_num == MT6331_CHIP) {
		if (channel_num == 8 && user_num == GPS) {
			ret =
			    pmic_read_interface_nolock(MT6331_AUXADC_ADC14, (&int_status_val_0),
						       0x8000, 0x0);
		} else if (channel_num == 7 && user_num == MD) {
			ret =
			    pmic_read_interface_nolock(MT6331_AUXADC_ADC15, (&int_status_val_0),
						       0x8000, 0x0);
		} else if (channel_num == 7 && user_num == AP) {
			ret =
			    pmic_read_interface_nolock(MT6331_AUXADC_ADC16, (&int_status_val_0),
						       0x8000, 0x0);
		} else if (channel_num == 4 && user_num == MD) {
			ret =
			    pmic_read_interface_nolock(MT6331_AUXADC_ADC17, (&int_status_val_0),
						       0x8000, 0x0);
		} else {
			ret =
			    pmic_read_interface_nolock(MT6331_AUXADC_ADC0 + (channel_num * 2),
						       (&int_status_val_0), 0x8000, 0x0);
		}
	} else if (chip_num == MT6332_CHIP) {
		if (channel_num == 4 && user_num == MD) {
			ret =
			    pmic_read_interface_nolock(MT6332_AUXADC_ADC17, (&int_status_val_0),
						       0x8000, 0x0);
		} else {
			ret =
			    pmic_read_interface_nolock(MT6332_AUXADC_ADC0 + (channel_num * 2),
						       (&int_status_val_0), 0x8000, 0x0);
		}
	}
			       ...
			       upmu_adc_user_list_enum user_num)
{
	...
	if (chip_num == MT6331_CHIP) {
		...
		if (channel_num == 4 && user_num == MD) {
			...
			ret =
			    pmic_read_interface_nolock(MT6332_AUXADC_ADC16, (&int_status_val_0),
						       0x7FFF, 0x0);
		} else {
			ret =
			    pmic_read_interface_nolock(MT6332_AUXADC_ADC0 + (channel_num * 2),
...
static kal_uint32 PMIC_IMM_RequestAuxadcChannel(kal_int32 channel_num,
						upmu_adc_chip_list_enum chip_num,
						upmu_adc_user_list_enum user_num, int mode)
{
#if 1
	unsigned long flags;
	kal_uint32 ret = 0;
	...
	if (chip_num == MT6331_CHIP) {
		if (user_num == AP) {
			spin_lock_irqsave(&pmic_adc_lock, flags);
			if (mode == 0) {
				ret = pmic_config_interface_nolock((kal_uint32)
								   (MT6331_AUXADC_RQST0_CLR), 0x1,
								   0x1, channel_num);
				ret = pmic_config_interface_nolock((kal_uint32)
								   (MT6331_AUXADC_RQST0_CLR), 0x0,
								   0x1, channel_num);
			} else if (mode == 1) {
				ret = pmic_config_interface_nolock((kal_uint32)
								   (MT6331_AUXADC_RQST0_SET), 0x1,
								   0x1, channel_num);
			} else if (mode == 2) {
				ret =
				    pmic_config_interface_nolock((kal_uint32) (MT6331_AUXADC_RQST0),
								 0x1, 0x1, channel_num);
			}
			spin_unlock_irqrestore(&pmic_adc_lock, flags);
		} else if ((user_num == MD && (channel_num == 4 || channel_num == 7))
			   || (user_num == GPS && channel_num == 8)) {
			spin_lock_irqsave(&pmic_adc_lock, flags);
			if (mode == 0) {
				ret = pmic_config_interface_nolock((kal_uint32)
								   (MT6331_AUXADC_RQST1_CLR), 0x1,
								   0x1, channel_num);
				ret = pmic_config_interface_nolock((kal_uint32)
								   (MT6331_AUXADC_RQST1_CLR), 0x0,
								   0x1, channel_num);
			} else if (mode == 1) {
				ret = pmic_config_interface_nolock((kal_uint32)
								   (MT6331_AUXADC_RQST1_SET), 0x1,
								   0x1, channel_num);
			} else if (mode == 2) {
				ret =
				    pmic_config_interface_nolock((kal_uint32) (MT6331_AUXADC_RQST1),
								 0x1, 0x1, channel_num);
			}
			spin_unlock_irqrestore(&pmic_adc_lock, flags);
		} else {
		...
		if (user_num == AP) {
			...
			if (mode == 0) {
				ret = pmic_config_interface_nolock((kal_uint32)
								   (MT6332_AUXADC_RQST0_CLR), 0x1,
								   0x1, channel_num);
				ret = pmic_config_interface_nolock((kal_uint32)
								   (MT6332_AUXADC_RQST0_CLR), 0x0,
								   0x1, channel_num);
			} else if (mode == 1) {
				ret = pmic_config_interface_nolock((kal_uint32)
								   (MT6332_AUXADC_RQST0_SET), 0x1,
								   0x1, channel_num);
			} else if (mode == 2) {
				ret =
				    pmic_config_interface_nolock((kal_uint32) (MT6332_AUXADC_RQST0),
								 0x1, 0x1, channel_num);
			}
			spin_unlock_irqrestore(&pmic_adc_lock, flags);
			...
			if (mode == 0) {
				ret = pmic_config_interface_nolock((kal_uint32)
								   (MT6332_AUXADC_RQST1_CLR), 0x1,
								   0x1, channel_num);
				ret = pmic_config_interface_nolock((kal_uint32)
								   (MT6332_AUXADC_RQST1_CLR), 0x0,
								   0x1, channel_num);
			} else if (mode == 1) {
				ret = pmic_config_interface_nolock((kal_uint32)
								   (MT6332_AUXADC_RQST1_SET), 0x1,
								   0x1, channel_num);
			} else if (mode == 2) {
				ret =
				    pmic_config_interface_nolock((kal_uint32) (MT6332_AUXADC_RQST1),
								 0x1, 0x1, channel_num);
			}
			spin_unlock_irqrestore(&pmic_adc_lock, flags);
		} else {
...
int PMIC_IMM_GetOneChannelValue(upmu_adc_chl_list_enum dwChannel, int deCount, int trimd)
{
	...
	do {
		...
		if (channel_num == 7 && chip_num == MT6332_CHIP) {
			pmic_config_interface((kal_uint32) (MT6332_CHR_CON14), 0x1,
					      MT6332_PMIC_RG_AUXADC_USB_DET_MASK,
					      MT6332_PMIC_RG_AUXADC_USB_DET_SHIFT);
			pmic_config_interface((kal_uint32) (MT6332_CHR_CON14), 0x0,
					      MT6332_PMIC_RG_AUXADC_DCIN_DET_MASK,
					      MT6332_PMIC_RG_AUXADC_DCIN_DET_SHIFT);
		...
		if (Enable_BATDRV_LOG == 2) {
			xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
				    "[PMIC_IMM_GetOneChannelValue] channel_num=%d, chip_num = %d, user_num = %d\n",
				    channel_num, chip_num, user_num);
		}
		...
		default:
			xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
				    "[AUXADC] Invalid channel value(%d,%d)\n", dwChannel, trimd);
			if (mt6332_upmu_get_swcid() == PMIC6332_E2_CID_CODE) {
	...
	if (Enable_BATDRV_LOG == 2) {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[AUXADC] adc_result_temp=%d, adc_result=%d, r_val_temp=%d.\n",
