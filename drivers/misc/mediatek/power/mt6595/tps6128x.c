...
void tps6128x_hw_component_detect(void)
{
	...
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
		    "[tps6128x_hw_component_detect] exist=%d, Reg[0x3]=0x%x\n", g_tps6128x_hw_exist,
		    val);
}
...
static int tps6128x_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
		    "[tps6128x_driver_probe] g_tps6128x_hw_exist=%d, g_tps6128x_driver_ready=%d\n",
				     ...
				     const char *buf, size_t size)
{
	...
	if (buf != NULL && size != 0) {
		battery_xlog_printk(BAT_LOG_CRTI,
				    "[store_tps6128x_access] buf is %s and size is %d\n", buf,
				    size);
...
static int __init tps6128x_init(void)
{
	...
	if (i2c_add_driver(&tps6128x_driver) != 0) {
		battery_xlog_printk(BAT_LOG_CRTI,
				    "[tps6128x_init] failed to register tps6128x i2c driver.\n");
	} else {
		battery_xlog_printk(BAT_LOG_CRTI,
				    "[tps6128x_init] Success to register tps6128x i2c driver.\n");
	}
	...
	if (ret) {
		battery_xlog_printk(BAT_LOG_CRTI,
				    "****[tps6128x_init] Unable to device register(%d)\n", ret);
		return ret;
	}
	ret = platform_driver_register(&tps6128x_user_space_driver);
	if (ret) {
		battery_xlog_printk(BAT_LOG_CRTI,
				    "****[tps6128x_init] Unable to register driver (%d)\n", ret);
		return ret;
	}
#endif
