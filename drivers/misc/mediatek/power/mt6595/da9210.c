...
/**********************************************************
  ...
  *
  *********************************************************/
int da9210_read_byte(kal_uint8 cmd, kal_uint8 *returnData)
{
	char cmd_buf[1] = { 0x00 };
	char readData = 0;
	int ret = 0;
...
int da9210_write_byte(kal_uint8 cmd, kal_uint8 writeData)
{
	char write_data[2] = { 0 };
	int ret = 0;
...
void da9210_dump_register(void)
{
	kal_uint8 i = 0;
	/* ---------------------------------------------------------------- */
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[da9210] page 0,1: ");
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[0x%x]=0x%x ", 0x0, da9210_get_reg_value(0x0));
	for (i = 0x50; i <= 0x5D; i++)
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[0x%x]=0x%x ", i,
			    da9210_get_reg_value(i));
	for (i = 0xD0; i <= 0xD9; i++)
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[0x%x]=0x%x ", i,
			    da9210_get_reg_value(i));
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "\n");
	/* ---------------------------------------------------------------- */
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[da9210] page 2,3: ");
	for (i = 0x05; i <= 0x06; i++) {
...
int set_da9210_buck_en(int en_bit)
{
	...
	if (g_da9210_driver_ready == 1) {
		if (g_da9210_hw_exist == 1) {
			...
			xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
				    "[set_da9210_buck_en] en_bit=%d\n", en_bit);
		} else {
			xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
				    "[set_da9210_buck_en] da9210 driver is not exist\n");
		}
	} else {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[set_da9210_buck_en] da9210 driver is not ready\n");
	}
#endif
...
void da9210_hw_init(void)
{
	...
	if (g_vproc_vsel_gpio_number != 0) {
		...
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[da9210_hw_init] ext_buck_vproc_vsel(1)\n");
	}
	...
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
		    "[da9210_hw_init] [0x0]=0x%x, [0x59]=0x%x, [0x5D]=0x%x, [0xD2]=0x%x, [0xD6]=0x%x, [0xD8]=0x%x, [0xD9]=0x%x\n",
		    da9210_get_reg_value(0x0), da9210_get_reg_value(0x59),
		    da9210_get_reg_value(0x5D), da9210_get_reg_value(0xD2),
		    da9210_get_reg_value(0xD6), da9210_get_reg_value(0xD8),
		    da9210_get_reg_value(0xD9));
...
void da9210_hw_component_detect(void)
{
	...
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
		    "[da9210_hw_component_detect] exist=%d, Reg[0x105][7:4]=0x%x\n",
		    g_da9210_hw_exist, val);
}

int is_da9210_sw_ready(void)
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "g_da9210_driver_ready=%d\n",
...
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "g_da9210_hw_exist=%d\n", g_da9210_hw_exist);
...
int da9210_vosel(unsigned long val)
{
	...
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
		    "[da9210_vosel] val=%d, reg_val=%d, Reg[0xD8]=0x%x\n", val, reg_val,
		    da9210_get_reg_value(0xD8));
...
void bigcore_power_init(void)
{
	...
	if (ver == CHIP_SW_VER_01) {
		...
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[bigcore_power_init] Done. ret=%d, ver=%d.\n", ret, ver);
	}
}
...
void bigcore_power_off(void)
{
	...
	if (ver == CHIP_SW_VER_01) {
		...
		if (reg_val == 0x0)
			...
			xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
				    "[bigcore_power_off] wrong reg_val=%d\n", reg_val);

		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[bigcore_power_off] Turn off bigcore vproc and vsram. ver=%d\n", ver);
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[bigcore_power_off] [0x%x]=0x%x, [0x%x]=0x%x, [0x%x]=0x%x, reg_val=%d\n",
			    0x63C, upmu_get_reg_value(0x63C),
			    0x18, upmu_get_reg_value(0x18),
			    0x524, upmu_get_reg_value(0x524), reg_val);
	}
}
...
void bigcore_power_on(void)
{
	...
	if (ver == CHIP_SW_VER_01) {
		...
		if (reg_val == 0x0)
			...
			xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
				    "[bigcore_power_on] wrong reg_val=%d\n", reg_val);

		/* turn on ca15l vproc(ext buck) */
		set_da9210_buck_en(1);
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[bigcore_power_on] Turn on bigcore vproc and vsram. ver=%d\n", ver);
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[bigcore_power_on] [0x%x]=0x%x, [0x%x]=0x%x, [0x%x]=0x%x, reg_val=%d\n",
			    0x63C, upmu_get_reg_value(0x63C),
			    0x18, upmu_get_reg_value(0x18),
			    0x524, upmu_get_reg_value(0x524), reg_val);
	}
...
static int da9210_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
		    "[da9210_driver_probe] g_da9210_hw_exist=%d, g_da9210_driver_ready=%d\n",
...
static ssize_t store_da9210_access(struct device *dev, struct device_attribute *attr,
				   const char *buf, size_t size)
{
	...
	if (buf != NULL && size != 0) {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[store_da9210_access] buf is %s and size is %d\n", buf, size);
		reg_address = kstrtoul(buf, &pvalue, 16);

		if (size > 4) {
			reg_value = kstrtoul((pvalue + 1), NULL, 16);
			xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
				    "[store_da9210_access] write da9210 reg 0x%x with value 0x%x !\n",
				    reg_address, reg_value);

			if (reg_address < 0x100) {
				xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
					    "[store_da9210_access] page 0,1\n");
				da9210_config_interface(0x0, 0x0, 0xF, 0);	/* select to page 0,1 */
			} else {
				xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
					    "[store_da9210_access] page 2,3\n");
				da9210_config_interface(0x0, 0x2, 0xF, 0);	/* select to page 2,3 */
			...
			xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
				    "[store_da9210_access] read da9210 reg 0x%x with value 0x%x !\n",
				    reg_address, g_reg_value_da9210);
			xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
				    "[store_da9210_access] Please use \"cat da9210_access\" to get value\r\n");
		}
	}
	return size;
}
...
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
		    "[show_da9210_vosel_pin] g_da9210_vosel_pin=%d\n", g_da9210_vosel_pin);
	return sprintf(buf, "%u\n", g_da9210_vosel_pin);
}

static ssize_t store_da9210_vosel_pin(struct device *dev, struct device_attribute *attr,
				      const char *buf, size_t size)
{
	int val = 0;
	char *pvalue = NULL;
	...
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
		    "[store_da9210_vosel_pin] ext_buck_vproc_vsel(%d)\n", g_da9210_vosel_pin);
...
static struct i2c_board_info i2c_da9210 __initdata = {
	I2C_BOARD_INFO("da9210", (da9210_SLAVE_ADDR_WRITE >> 1))
};
#endif
...
static int __init da9210_init(void)
{
	...
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
		    "[da9210_init] g_vproc_vsel_gpio_number=0x%x, g_vproc_en_gpio_number=0x%x\n",
	...
	if (g_vproc_vsel_gpio_number != 0) {
		...
		if (i2c_add_driver(&da9210_driver) != 0) {
			xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
				    "[da9210_init] failed to register da9210 i2c driver.\n");
		} else {
			xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
				    "[da9210_init] Success to register da9210 i2c driver.\n");
		}
		...
		if (ret) {
			xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
				    "****[da9210_init] Unable to device register(%d)\n", ret);
			return ret;
		}
		ret = platform_driver_register(&da9210_user_space_driver);
		if (ret) {
			xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
				    "****[da9210_init] Unable to register driver (%d)\n", ret);
			return ret;
		}
	} else {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[da9210_init] DCT no define EXT BUCK\n");
		g_da9210_hw_exist = 0;
		g_da9210_driver_ready = 1;
	}
