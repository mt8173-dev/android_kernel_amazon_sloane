	...
	if (tmp32 == 0) {
		...
		if (mt_usb_is_device()) {
			...
			if (ncp1851_get_otg_en() == 0) {	/* boost mode disabled */
				if (Enable_BATDRV_LOG == 1) {
					xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
						    "[upmu_is_chr_det] USB host mode but boost 5V not enabled\n");
					if (ncp1851_get_faultint() == 1) {
						xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
							    "[upmu_is_chr_det] ncp1851 fault int detected!\n");
						ncp1851_read_register(0x6);
						bst_int = ncp1851_reg[0x6];
						ncp1851_reg[0x6] = 0;
						if (bst_int &
						    (CON6_VBUSILIM_MASK << CON6_VBUSILIM_SHIFT))
							xlog_printk(ANDROID_LOG_INFO,
								    "Power/Battery",
								    "[upmu_is_chr_det] ncp1851 boost mode fail due to vbus over current!\n");
						if (bst_int &
						    (CON6_VBUSOV_MASK << CON6_VBUSOV_SHIFT))
							xlog_printk(ANDROID_LOG_INFO,
								    "Power/Battery",
								    "[upmu_is_chr_det] ncp1851 boost mode fail due to vbus over voltage!\n");
						if (bst_int &
						    (CON6_VBATLO_MASK << CON6_VBATLO_SHIFT))
							xlog_printk(ANDROID_LOG_INFO,
								    "Power/Battery",
								    "[upmu_is_chr_det] ncp1851 boost mode fail due to vbat low voltage!\n");
					}
				}
...
extern kal_uint32 ncp1851_read_interface(kal_uint8 RegNum, kal_uint8 *val, kal_uint8 MASK,
					 kal_uint8 SHIFT);
...
int get_bat_charging_current_level(void)
{
	...
	if (BMT_status.charger_type == STANDARD_HOST) {
		...
		if (ret_val == 0x00)
			...
			return 700;
		else if (ret_val == 0x04)
			return 800;
		else if (ret_val == 0x05)
			return 900;
		else if (ret_val == 0x06)
			return 1000;
		else if (ret_val == 0x07)
			return 1100;
		else if (ret_val == 0x08)
			return 1200;
		else if (ret_val == 0x09)
			return 1300;
		else if (ret_val == 0x0A)
			return 1400;
		else if (ret_val == 0x0B)
			return 1500;
		else if (ret_val == 0x0C)
			return 1600;
		else
			return -1;
	} else {
...
void ncp1851_set_ac_current(void)
{
	if (Enable_BATDRV_LOG == 1) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:ncp1851] ncp1851_set_ac_current \r\n");
	}
	...
	{
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:ncp1851] invalid current selected (%ld), use 500mA \r\n",
			    g_temp_CC_value);
		ncp1851_set_ichg(1);
	} else {
#if defined(MTK_JEITA_STANDARD_SUPPORT)
...
void ncp1851_set_low_chg_current(void)
{
	if (Enable_BATDRV_LOG == 1) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:ncp1851] ncp1851_set_low_chg_current \r\n");
	}
...
void select_charging_curret_ncp1851(void)
{
	if (g_ftm_battery_flag) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:ncp1851] FTM charging : %d\r\n", charging_level_data[0]);
		...
		if (BMT_status.charger_type == STANDARD_HOST) {
			if (g_Support_USBIF == 1) {
				if (g_usb_state == USB_SUSPEND) {
					...
					if (Enable_BATDRV_LOG == 1) {
						xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
							    "[BATTERY:ncp1851] disable charging\r\n");
					}
					...
					if (Enable_BATDRV_LOG == 1) {
						xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
							    "[BATTERY:ncp1851] ncp1851_set_ac_current(), CC value (%ld) \r\n",
							    g_temp_CC_value);
					}
				} else {
					g_temp_CC_value = USB_CHARGER_CURRENT_UNCONFIGURED;
			...
			ncp1851_set_iinset_pin_en(0x0);	/* Input current limit and AICL control by I2C */
#ifdef MTK_NCP1852_SUPPORT
			if ((g_temp_CC_value > Cust_CC_1500MA)
			    && (g_temp_CC_value <= Cust_CC_1800MA))
			...
			if (Enable_BATDRV_LOG == 1) {
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BATTERY:ncp1851] Unknown charger type\n");
			}
...
void ChargerHwInit_ncp1851(void)
{
	...
	if (Enable_BATDRV_LOG == 1) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:ncp1851] ChargerHwInit_ncp1851\n");
	}
...
void pchr_turn_off_charging_ncp1851(void)
{
	if (Enable_BATDRV_LOG == 1) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:ncp1851] pchr_turn_off_charging_ncp1851!\r\n");
	}
...
void pchr_turn_on_charging_ncp1851(void)
{
	if (BMT_status.bat_charging_state == CHR_ERROR) {
		if (Enable_BATDRV_LOG == 1) {
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:ncp1851] Charger Error, turn OFF charging !\r\n");
		}
		...
		if (Enable_BATDRV_LOG == 1) {
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:ncp1851] pchr_turn_on_charging_ncp1851 !\r\n");
		}
		...
		if (g_temp_CC_value == Cust_CC_0MA) {
			...
			if ((gFG_booting_counter_I_FLAG == 2) || (g_ocv_lookup_done == 1)) {
				...
				if (Enable_BATDRV_LOG == 1) {
					xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
						    "[BATTERY:ncp1851] charger enable !\r\n");
				}
			} else {
				pchr_turn_off_charging_ncp1851();
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BATTERY:ncp1851] wait gFG_booting_counter_I_FLAG==2 (%d)\r\n",
...
int do_jeita_state_machine(void)
{
	...
	if (BMT_status.temperature >= TEMP_POS_60_THRESHOLD) {
		xlog_printk(ANDROID_LOG_WARN, "Power/Battery",
			    "[BATTERY:ncp1851] Battery Over high Temperature(%d) !!\n\r",
		...
		if ((g_temp_status == TEMP_ABOVE_POS_60)
			...
			xlog_printk(ANDROID_LOG_WARN, "Power/Battery",
				    "[BATTERY:ncp1851] Battery Temperature between %d and %d,not allow charging yet!!\n\r",
				    TEMP_POS_60_THRES_MINUS_X_DEGREE, TEMP_POS_60_THRESHOLD);

			return PMU_STATUS_FAIL;
		} else {
			xlog_printk(ANDROID_LOG_WARN, "Power/Battery",
				    "[BATTERY:ncp1851] Battery Temperature between %d and %d !!\n\r",
		...
		if (((g_temp_status == TEMP_POS_45_TO_POS_60)
			...
			xlog_printk(ANDROID_LOG_WARN, "Power/Battery",
				    "[BATTERY:ncp1851] Battery Temperature not recovery to normal temperature charging mode yet!!\n\r");
		} else {
			if (Enable_BATDRV_LOG >= 1) {
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BATTERY:ncp1851] Battery Normal Temperature between %d and %d !!\n\r",
		...
		xlog_printk(ANDROID_LOG_WARN, "Power/Battery",
			    "[BATTERY:ncp1851] Battery below low Temperature(%d) !!\n\r",
...
/* g_bat_temperature_pre = 22; // MT6320 E1 workaround */
	...
	if ((g_battery_tt_check_flag == 0) && (BMT_status.temperature < MAX_CHARGE_TEMPERATURE)
		...
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:ncp1851] Enable battery TT (%d)\n\r", BMT_status.temperature);
		g_battery_tt_check_flag = 1;
	}

	if (Enable_BATDRV_LOG == 1) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:ADC:ncp1851] VCHR:%ld BAT_SENSE:%ld I_SENSE:%ld TBAT:%ld TBAT_vol:%ld BatTT:%d\n",
	...
	if (Enable_BATDRV_LOG == 1) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:AVG:ncp1851] BatTemp:%d Vbat:%d VBatSen:%d SOC:%d ChrDet:%d Vchrin:%d Icharging:%d ChrType:%d USBstate:%d\r\n",
	...
	if (Enable_BATDRV_LOG == 1) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:FG:ncp1851] %d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\r\n",
			    BMT_status.temperature, BMT_status.bat_vol, BMT_status.ADC_BAT_SENSE,
			    BMT_status.SOC, upmu_is_chr_det(), BMT_status.charger_vol,
	...
	if (battery_cmd_thermal_test_mode == 1) {
		...
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:ncp1851] In thermal_test_mode, Tbat=%d\n",
			    BMT_status.temperature);
	}
#if defined(MTK_JEITA_STANDARD_SUPPORT)
	if (Enable_BATDRV_LOG == 1) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:ncp1851] support JEITA, Tbat=%d\n", BMT_status.temperature);
	}
	...
	if (upmu_is_chr_det() == KAL_TRUE) {
...
#endif
		{
			if (Enable_BATDRV_LOG == 1) {
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BATTERY:ncp1851] Battery Re-charging !!\n\r");
			}
...
{
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
		    "[BATTERY:ncp1851] Charging over %d hr stop !!\n\r", MAX_CHARGING_TIME);
...
PMU_STATUS BAT_BatteryFullAction(void)
{
	if (Enable_BATDRV_LOG == 1) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:ncp1851] check Battery full !!\n\r");
	}
	...
	if (gFG_can_reset_flag == 1) {
...
#endif
		{
			...
			if (Enable_BATDRV_LOG >= 1) {
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BATTERY:ncp1851] Battery real full. Call FGADC_Reset_SW_Parameter.\n");
			}
			FGADC_Reset_SW_Parameter();
		} else {
			if (Enable_BATDRV_LOG >= 1) {
			...
			if (Enable_BATDRV_LOG >= 1) {
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BATTERY:ncp1851] (%d) VBAT<4150mV. Don't FGADC_Reset_SW_Parameter.\n",
					    BMT_status.bat_vol);
			}
		}
	}
...
void mt_battery_notify_check(void)
{
	...
	if (g_BN_TestMode == 0x0000) {
		if (Enable_BATDRV_LOG == 1) {
			xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
				    "[BATTERY:ncp1851] mt_battery_notify_check\n");
		}
#if defined(BATTERY_NOTIFY_CASE_0000)
		if (Enable_BATDRV_LOG == 1) {
			xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
				    "[BATTERY:ncp1851] BATTERY_NOTIFY_CASE_0000\n");
		}
#endif
			...
			/* if(BMT_status.charger_vol > 3000) //test */
		{
			...
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:ncp1851] BMT_status.charger_vol(%d) > %d mV\n",
		...
		if (Enable_BATDRV_LOG == 1) {
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:ncp1851] BATTERY_NOTIFY_CASE_0001 (%x)\n",
				    g_BatteryNotifyCode);
		}
#endif
			...
			/* if(BMT_status.temperature > 20) //test */
		{
			...
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:ncp1851] bat_temp(%d) > 50'C\n",
				    BMT_status.temperature);
		} else {
			g_BatteryNotifyCode &= ~(0x0002);
		}
		if (Enable_BATDRV_LOG == 1) {
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:ncp1851] BATTERY_NOTIFY_CASE_0002 (%x)\n",
				    g_BatteryNotifyCode);
		}
#endif
			...
			/* if(BMT_status.ICharging > 200) //test */
		{
			...
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:ncp1851] I_charging(%d) > 1000mA\n",
		...
		if (Enable_BATDRV_LOG == 1) {
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:ncp1851] BATTERY_NOTIFY_CASE_0003 (%x)\n",
				    g_BatteryNotifyCode);
		}
#endif
			...
			/* if(BMT_status.bat_vol > 3800) //test */
		{
			...
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:ncp1851] bat_vlot(%d) > 4350mV\n",
				    BMT_status.bat_vol);
		} else {
			g_BatteryNotifyCode &= ~(0x0008);
		}
		if (Enable_BATDRV_LOG == 1) {
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:ncp1851] BATTERY_NOTIFY_CASE_0004 (%x)\n",
				    g_BatteryNotifyCode);
		}
#endif
		...
		if ((g_battery_thermal_throttling_flag == 2)
				...
				/* if(BMT_status.total_charging_time >= 60) //test */
			{
				...
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BATTERY:ncp1851] Charging Over Time\n");
			} else {
				g_BatteryNotifyCode &= ~(0x0010);
			}
		}
		...
		if (Enable_BATDRV_LOG == 1) {
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:ncp1851] BATTERY_NOTIFY_CASE_0005 (%x)\n",
				    g_BatteryNotifyCode);
		}
#endif
		...
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
			    "[BATTERY:ncp1851] Unknown BN_TestMode Code : %x\n", g_BN_TestMode);
	}
}
...
void check_battery_exist(void)
{
	...
	if (baton_count >= 3) {
		if ((g_boot_mode == META_BOOT) || (g_boot_mode == ATE_FACTORY_BOOT)) {
			...
			printk
			    ("[BATTERY] Battery is not exist, power off NCP1851 and system (%d)\n",
			     baton_count);
			pchr_turn_off_charging_ncp1851();
			arch_reset(0, NULL);
		}
...
void BAT_thread_ncp1851(void)
{
...
#if defined(CONFIG_POWER_EXT)
			xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
				    "[BATTERY:ncp1851] CONFIG_POWER_EXT, no update mt6320_battery_update_power_down.\n");
#else
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[Battery:ncp1851] Tbat(%d)>=60, system need power down.\n",
				    BMT_status.temperature);
	...
	if (upmu_is_chr_det() == KAL_TRUE) {
		...
		if (BMT_status.charger_type == CHARGER_UNKNOWN) {
			...
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BAT_thread:ncp1851] CHR_Type_num = %d\r\n", CHR_Type_num);
		...
		/*Use gas gauge */
			...
			if (bat_volt_check_point != 100) {
				...
				if (Enable_BATDRV_LOG == 1) {
					xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
						    "[Battery_Only:ncp1851] Set g_bat_full_user_view=KAL_FALSE\r\n");
				}
			}
		}
	...
	if (battery_cmd_thermal_test_mode == 1) {
		BMT_status.temperature = battery_cmd_thermal_test_mode_value;
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:ncp1851] In thermal_test_mode 3, Tbat=%d\n",
			    BMT_status.temperature);
	}
...
#if defined(CONFIG_POWER_EXT)
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
		    "[BATTERY:ncp1851] CONFIG_POWER_EXT, no update Android.\n");
#else
	if (gFG_booting_counter_I_FLAG == 2) {
		...
		if (gForceADCsolution == 1) {
			...
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:ncp1851] gFG_booting_counter_I_FLAG is 1, SOC=50.\n");
		}
	} else {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:ncp1851] gFG_booting_counter_I_FLAG!=2 (%d)\r\n",
			    gFG_booting_counter_I_FLAG);
	}
#endif
	...
	if (BAT_status == PMU_STATUS_FAIL || g_Battery_Fail) {
		...
		if (Enable_BATDRV_LOG == 1) {
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:ncp1851] PMU_STATUS_FAIL (chr_err_cnt = %d)\n",
				    chr_err_cnt);
		}
	}
	/* Battery Full */
	/* HW charging done, real stop charging */
		...
		if (Enable_BATDRV_LOG == 1) {
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:ncp1851] Battery real full.\n");
		}
		BMT_status.bat_full = KAL_TRUE;
		BMT_status.total_charging_time = 0;
		BMT_status.PRE_charging_time = 0;
	...
	/* Battery Not Full and Charger exist : Do Charging */
		...
		if ((BMT_status.bat_vol > RECHARGING_VOLTAGE) && ((ncp1851_status == 0x6) || (ncp1851_status == 0x7))) {	/* TODO: monitor charge done, check if need to monitor DPP */
			...
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:ncp1851] Battery real full and disable charging (%d)\n",
				    ncp1851_status);
			gSyncPercentage = 1;
			return;
			...
			if (Enable_BATDRV_LOG == 1) {
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BATTERY:ncp1851] Ncp1851_status is (%d), but battery voltage not over recharge voltage\n",
					    ncp1851_status);
			}
		}
		...
		if (Enable_BATDRV_LOG == 1) {
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:ncp1851] Total charging timer=%d, ncp1851_status=%d\n\r",
...
void PrechargeCheckStatus(void)
{
	...
	if (get_charger_detect_status() == KAL_TRUE) {	/* do not check USB device check at precharge state */
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:ncp1851] Boot up during charging!\n");
		g_boot_charging = 1;
		...
		if (vfet_status == 1) {
			...
			bat_vol = get_i_sense_volt(10);
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:ncp1851] Powerpath init battery voltage > 3600, battery voltage during system discharging = %d\n",
				    bat_vol);
			return;
		} else if (bat_vol >= 3700) {	/* Error handling when vfet_status is not reliable */
			g_ocv_lookup_done = 0;
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:ncp1851] Powerpath init, (vfet_status = %d) (ncp1851_status = %d)\n",
		...
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:ncp1851] Powerpath init battery voltage < 3600, battery voltage  = %d\n",
			    bat_vol);
	} else {
		return;
	}
	...
	while (1) {
		if (get_charger_detect_status() == KAL_TRUE) {	/* do not check USB device check at precharge state */
			...
			if (BMT_status.charger_type == CHARGER_UNKNOWN) {
				CHR_Type_num = mt_charger_type_detection();
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BATTERY:ncp1851] CHR_Type_num = %d\r\n",
					    CHR_Type_num);
				BMT_status.charger_type = CHR_Type_num;
			...
			if ((g_battery_tt_check_flag == 0) && (bat_temp < 60) && (bat_temp > (-20))) {
				g_battery_thermal_throttling_flag = 1;
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BATTERY:ncp1851] Powerpath precharge enable battery TT (%d)\n\r",
					    bat_temp);
				g_battery_tt_check_flag = 1;
			}
			...
			if ((bat_temp <= MIN_CHARGE_TEMPERATURE) ||
				...
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BATTERY:ncp1851] Powerpath precharge battery Under Temperature or NTC fail !!\n\r");
				BMT_status.bat_charging_state = CHR_ERROR;
			}
#endif
			if (bat_temp >= MAX_CHARGE_TEMPERATURE) {
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BATTERY:ncp1851] Powerpath precharge battery Over Temperature !!\n\r");
				BMT_status.bat_charging_state = CHR_ERROR;
			}
#if (V_CHARGER_ENABLE == 1)
			if (charger_vol <= V_CHARGER_MIN) {
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BATTERY:ncp1851] Powerpath precharge charger under voltage!!\r\n");
				BMT_status.bat_charging_state = CHR_ERROR;
			}
#endif
			if (charger_vol >= V_CHARGER_MAX) {
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BATTERY:ncp1851] Powerpath precharge charger over voltage !!\r\n");
				BMT_status.bat_charging_state = CHR_ERROR;
			}
			...
			if ((g_temp_CC_value == Cust_CC_0MA) || (chr_err_cnt > 0)) {
				...
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BATTERY:ncp1851] Powerpath precharge chr_err_cnt = %d\r\n",
					    chr_err_cnt);
			} else {
				ncp1851_set_chg_en(0x1);	/* charger enable */
				/* Set SPM = 1, for lenovo platform, GPIO_CHR_SPM_PIN */
				...
				if (Enable_BATDRV_LOG == 1) {
					xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
						    "[BATTERY:ncp1851] Power path precharging !\r\n");
				}
			}

			if ((bat_vol >= SYSTEM_OFF_VOLTAGE + 100) || (vfet_status == 1)) {
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BATTERY:ncp1851] Powerpath precharge finish, battery voltage = %d\n",
					    bat_vol);
				BMT_status.bat_charging_state = CHR_CC;
				break;
			} else {
				...
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BATTERY:ncp1851] Powerpath precharge continue, battery voltage (%d), system voltage (%d), charger volt (%d)\n",
			...
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:ncp1851]VCDT voltage = %d\n", get_charger_volt(10));
			wake_unlock(&battery_suspend_lock);
			pchr_turn_off_charging_ncp1851();
			BMT_status.bat_charging_state = CHR_PRE;
...
#endif
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:ncp1851] Powerpath precharge stopped due to charger plug-out, wait for system shutdown\n");
			msleep(5000);
		}
	}
