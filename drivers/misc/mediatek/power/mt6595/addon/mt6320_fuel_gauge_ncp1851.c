...
kal_uint32 fg_get_data_ready_status(void)
{
	...
	if (Enable_FGADC_LOG == 1) {
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
			    "[fg_get_data_ready_status] Reg[0x%x]=0x%x\r\n", FGADC_CON0, temp_val);
	}
...
kal_uint32 fg_get_sw_clear_status(void)
{
	...
	if (Enable_FGADC_LOG == 1) {
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
			    "[fg_get_sw_clear_status] Reg[0x%x]=0x%x\r\n", FGADC_CON0, temp_val);
	}

	temp_val =
	    (temp_val & (PMIC_FG_SW_CLEAR_MASK << PMIC_FG_SW_CLEAR_SHIFT)) >>
...
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
		    "[dump_nter] upmu_get_fg_nter_29_16 = 0x%x\r\n", upmu_get_fg_nter_29_16());
	xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
		    "[dump_nter] upmu_get_fg_nter_15_00 = 0x%x\r\n", upmu_get_fg_nter_15_00());
}

void dump_car(void)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
		    "[dump_car] upmu_get_fg_car_35_32 = 0x%x\r\n", upmu_get_fg_car_35_32());
	xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
		    "[dump_car] upmu_get_fg_car_31_16 = 0x%x\r\n", upmu_get_fg_car_31_16());
	xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
		    "[dump_car] upmu_get_fg_car_15_00 = 0x%x\r\n", upmu_get_fg_car_15_00());
}
...
/* Read HW Raw Data */
	...
	while (fg_get_data_ready_status() == 0) {
		...
		if (m > 1000) {
			if (Enable_FGADC_LOG == 1) {
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[fgauge_read_columb] fg_get_data_ready_status timeout 1 !\r\n");
			}
			break;
		}
	}
	...
	if (Enable_FGADC_LOG == 1) {
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
			    "[FGADC] fgauge_read_columb : FG_CAR = 0x%x\r\n", uvalue32_CAR);
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
			    "[FGADC] fgauge_read_columb : uvalue32_CAR_MSB = 0x%x\r\n",
			    uvalue32_CAR_MSB);
	}
	/* (5)    (Read other data) */
	/* (6)    Clear status to 0 */
	...
	while (fg_get_data_ready_status() != 0) {
		...
		if (m > 1000) {
			if (Enable_FGADC_LOG == 1) {
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[fgauge_read_columb] fg_get_data_ready_status timeout 2 !\r\n");
			}
			break;
		}
	}
...
/* calculate the real world data */
	...
	if (Enable_FGADC_LOG == 1) {
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
			    "[FGADC] fgauge_read_columb : dvalue_CAR = %d\r\n", dvalue_CAR);
	}
#if (OSR_SELECT_7 == 1)
	dvalue_CAR = dvalue_CAR * 8;
	if (Enable_FGADC_LOG == 1) {
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
			    "[FGADC] fgauge_read_columb : dvalue_CAR update to %d\r\n", dvalue_CAR);
	}
#endif
...
/* Auto adjust value */
	if (R_FG_VALUE != 20) {
		if (Enable_FGADC_LOG == 1) {
			xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
				    "[FGADC] Auto adjust value deu to the Rfg is %d\n Ori CAR=%d, ",
	...
	if (Enable_FGADC_LOG == 1) {
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
			    "[FGADC] fgauge_read_columb : final dvalue_CAR = %d\r\n", dvalue_CAR);
	}
...
/* Read HW Raw Data */
	...
	if (Enable_FGADC_LOG == 1) {
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
			    "[FGADC] fgauge_read_current : FG_CURRENT = %x\r\n", uvalue16);
	}
	/* (5)    (Read other data) */
	/* (6)    Clear status to 0 */
	ret = pmic_config_interface(FGADC_CON0, 0x0800, 0xFF00, 0x0);
...
/* calculate the real world data */
	...
	if (Enable_FGADC_LOG == 1) {
		if (gFG_Is_Charging == KAL_TRUE) {
			xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
				    "[FGADC] fgauge_read_current : current(charging) = %d mA\r\n",
				    dvalue);
		} else {
			xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
				    "[FGADC] fgauge_read_current : current(discharging) = %d mA\r\n",
				    dvalue);
		}
	}
/* Auto adjust value */
...
void fgauge_read_avg_I_V(void)
{
	...
	for (i = 0; i < 14; i++) {
		...
		cur_sum += gFG_current;
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[fgauge_read_avg_I_V] #%d voltage[%d], current[%d]\r\n", i, vol[i],
			    gFG_current);
	}
...
/* if (Enable_FGADC_LOG == 1) */
	{
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[fgauge_read_avg_I_V] AVG voltage[%d], current[%d]\r\n", gFG_voltage,
			    gFG_current);
	}
}
...
/* R-Table (First Time) */
	...
	for (i = 0; i < saddles; i++) {
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "<Voltage,DOD> at %d = <%d,%d>\r\n",
			    gFG_bat_temperature, (profile_p + i)->voltage,
			    (profile_p + i)->percentage);
	}
#endif
...
void fg_qmax_update_for_aging(void)
{
	if (g_HW_Charging_Done == 1) {	/* charging full */
		...
		if (Enable_FGADC_LOG == 1) {
			xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
				    "[fg_qmax_update_for_aging] g_HW_Charging_Done=%d\r\n",
				    g_HW_Charging_Done);
		}
	}
}
...
kal_int32 fgauge_update_dod(void)
{
	...
	if (gFG_DOD0 > 100) {
		...
		if (Enable_FGADC_LOG == 1) {
			xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
				    "[fgauge_update_dod] gFG_DOD0 set to 100, gFG_columb=%d\r\n",
				    gFG_columb);
		}
	} else if (gFG_DOD0 < 0) {
		gFG_DOD0 = 0;
		if (Enable_FGADC_LOG == 1) {
			xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
				    "[fgauge_update_dod] gFG_DOD0 set to 0, gFG_columb=%d\r\n",
				    gFG_columb);
		}
	} else {
	}
	...
	if (FG_dod_1 > 100) {
		...
		if (Enable_FGADC_LOG == 1) {
			xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
				    "[fgauge_update_dod] FG_dod_1 set to 100, gFG_columb=%d\r\n",
				    gFG_columb);
		}
	} else if (FG_dod_1 < 0) {
		FG_dod_1 = 0;
		if (Enable_FGADC_LOG == 1) {
			xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
				    "[fgauge_update_dod] FG_dod_1 set to 0, gFG_columb=%d\r\n",
				    gFG_columb);
		}
	} else {
	}
...
kal_int32 fgauge_read_capacity(kal_int32 type)
{
	...
	if (Enable_FGADC_LOG == 1) {
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
			    ...
			    C_0mA, C_400mA, dvalue, dvalue_new, gFG_DOD1, gFG_DOD0);
	}
	/* ---------------------------------------------------------------------------- */
	...
	if (Enable_FGADC_LOG >= 1) {
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
			    ...
			    dvalue, dvalue_new, gFG_BATT_CAPACITY_init_high_current,
			    gFG_BATT_CAPACITY_aging);
	}
	/* ---------------------------------------------------------------------------- */
#endif
...
kal_int32 fgauge_read_capacity_by_v(void)
{
	...
	for (i = 0; i < saddles - 1; i++) {
		if ((gFG_voltage <= (profile_p + i)->voltage)
		    && (gFG_voltage >= (profile_p + i + 1)->voltage)) {
			ret_percent =
			    (profile_p + i)->percentage +
			    (((((profile_p + i)->voltage) -
			       gFG_voltage) * (((profile_p + i + 1)->percentage) -
					       ((profile_p + i)->percentage))
			     ) / (((profile_p + i)->voltage) - ((profile_p + i + 1)->voltage))
			    );
...
kal_int32 fgauge_read_v_by_capacity(int bat_capacity)
{
	...
	for (i = 0; i < saddles - 1; i++) {
		if ((bat_capacity >= (profile_p + i)->percentage)
			...
			xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
				    "[fgauge_read_v_by_capacity] ret_volt=%d\r\n", ret_volt);
			xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
				    "[fgauge_read_v_by_capacity] (profile_p+i)->percentag=%d\r\n",
				    (profile_p + i)->percentage);
			xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
				    "[fgauge_read_v_by_capacity] ((profile_p+i+1)->percentage)=%d\r\n",
				    ((profile_p + i + 1)->percentage));
			xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
				    "[fgauge_read_v_by_capacity] ((profile_p+i)->voltage)=%d\r\n",
				    ((profile_p + i)->voltage));
			xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
				    "[fgauge_read_v_by_capacity] ((profile_p+i+1)->voltage) =%d\r\n",
				    ((profile_p + i + 1)->voltage));
...
kal_int32 mtk_imp_tracking(kal_int32 ori_voltage, kal_int32 ori_current, kal_int32 recursion_time)
{
	...
	for (i = 0; i < recursion_time; i++) {
		...
		if (Enable_FGADC_LOG == 1) {
			xlog_printk(ANDROID_LOG_INFO, "Power/FGOAM",
...
int get_hw_ocv(void)
{
	...
	adc_result = (adc_result_reg * r_val_temp * 1200) / 1024;
	xlog_printk(ANDROID_LOG_INFO, "Power/FGOAM",
		    "[oam] get_hw_ocv (pchr) : adc_result_reg=%d, adc_result=%d\n", adc_result_reg,
		    adc_result);
#endif
...
/* vol_bat = get_bat_sense_volt(15); */
	...
	if (gFG_Is_Charging == KAL_FALSE) {
		...
		case 1:
			d5_count_time_rate =
			    (((gFG_BATT_CAPACITY_aging * 60 * 60 / 100 / (450 - 50)) * 10) +
			     5) / 10;
			break;
		case 2:
			d5_count_time_rate =
			    (((gFG_BATT_CAPACITY_aging * 60 * 60 / 100 / (650 - 50)) * 10) +
			     5) / 10;
			break;
		case 3:
			d5_count_time_rate =
		...
		default:
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[get_charger_type] Invalid value\n");
			break;
		}
	...
	if (oam_run_i == 0) {
		if (Enable_FGADC_LOG == 1) {
			xlog_printk(ANDROID_LOG_INFO, "Power/FGOAM",
				    "[oam_run] oam_i_1,oam_i_2,oam_car_1,oam_car_2,oam_d_1,oam_d_2,oam_v_ocv_1,oam_d_3,oam_r_1,oam_v_ocv_2,oam_r_2,vol_bat,vol_bat_hw_ocv,d_hw_ocv\n");
		}
		oam_run_i = 1;
	}
	...
	if (Enable_FGADC_LOG == 1) {
		xlog_printk(ANDROID_LOG_INFO, "Power/FGOAM",
			    "[oam_run] %d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n", oam_i_1,
			    oam_i_2, oam_car_1, oam_car_2, oam_d_1, oam_d_2, oam_v_ocv_1, oam_d_3,
			    oam_r_1, oam_v_ocv_2, oam_r_2, vol_bat, vol_bat_hw_ocv, d_hw_ocv);
	}

	if (Enable_FGADC_LOG == 1) {
		xlog_printk(ANDROID_LOG_INFO, "Power/FGOAM",
			    "[oam_total] %d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n", gFG_capacity_by_c,
			    gFG_capacity_by_v, gfg_percent_check_point, oam_d_1, oam_d_2, oam_d_3,
			    oam_d_4, oam_d_5, gFG_capacity_by_c_init, d_hw_ocv);
	}
...
/* 1.1 Average FG_voltage */
	...
	if (gFG_booting_counter_I_FLAG != 0) {
		...
		if (gFG_vbat_offset <= MinErrorOffset) {
			...
			if (Enable_FGADC_LOG == 1) {
				xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
					    "[FG] Over MinErrorOffset:V=%d,Avg_V=%d, ", gFG_voltage,
					    gFG_voltage_AVG);
			}
	...
	if (gFG_booting_counter_I_FLAG == 1) {
		...
		if (g_ocv_lookup_done == 0) {
			...
			if (g_boot_charging == 0) {
				...
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[fgauge_read_avg_I_V] HW OCV[%d], SW OCV[%d]\r\n",
					    gFG_voltage, vol_adc);
			}
			gFG_capacity_by_v = fgauge_read_capacity_by_v();
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[FGADC] get_hw_ocv=%d, SOC=%d\n", gFG_voltage,
				    gFG_capacity_by_v);
			/* ------------------------------------------------------------------------------- */
#endif
			...
			if (g_rtc_fg_soc >= gFG_capacity_by_v) {
				...
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[FGADC] [1] g_rtc_fg_soc=%d, gFG_capacity_by_v=%d\n",
				...
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[FGADC] [2] g_rtc_fg_soc=%d, gFG_capacity_by_v=%d\n",
					    g_rtc_fg_soc, gFG_capacity_by_v);
			}
			/* ------------------------------------------------------------------------------- */
		...
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[FGADC] update DOD0 after booting %d s\r\n",
		...
		if (gFG_current_auto_detect_R_fg_result <= CURRENT_DETECT_R_FG) {
			...
			if (gForceADCsolution == 0) {
				...
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[FGADC] Detect Rfg, but use AUXADC report. due to gForceADCsolution=%d \r\n",
					    gForceADCsolution);
			}
		}
	}
...
kal_int32 fgauge_get_Q_max_high_current(kal_int16 temperature)
{
	...
	kal_int32 low_temperature = 0, high_temperature = 0;
	kal_int32 low_Q_max = 0, high_Q_max = 0;
	int q_max_neg_10_h_current, q_max_pos_0_h_current, q_max_pos_25_h_current,
	    q_max_pos_50_h_current;
...
void fgauge_precharge_init(void)
{
	...
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
		    "******** [fgauge_initialization] reset HW FG!\n");
	...
	if (current_val == 0)
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[fgauge_precharge_init] current value is still 0\r\n");
}
...
kal_int32 fgauge_precharge_compensated_voltage(kal_int32 recursion_time)
{
	...
	for (i = 0; i < recursion_time; i++) {
		...
		if (Enable_FGADC_LOG == 1) {
			xlog_printk(ANDROID_LOG_VERBOSE, "Power/Battery",
				    "[fgauge_precharge_compensated_voltage] %d,%d,%d,%d\r\n",
...
/* 2. SW algorithm initialization */
	...
	if (g_ocv_lookup_done == 0) {
		...
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "g_ocv_loopup_done is 0, do fgauge_read_capacity(0)\n");
	} else {
...
void FGADC_Reset_SW_Parameter(void)
{
...
#if 0
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
		    "[FGADC] FGADC_Reset_SW_Parameter : Todo \r\n");
#else
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
		    "[FGADC] FGADC_Reset_SW_Parameter : Start \r\n");
	...
	gFG_columb = 0;
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
		    "[FGADC] FGADC_Reset_SW_Parameter : Done \r\n");

	if (g_Calibration_FG == 1) {
		gFG_DOD0 = 0;
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[FGADC] FG Calibration DOD0=%d and DOD1=%d \r\n", gFG_DOD0, gFG_DOD1);
	} else {
...
kal_int32 get_dynamic_period(int first_use, int first_wakeup_time, int battery_capacity_level)
{
	...
	if (first_use == 1) {
		...
		if (g_car_sleep > g_car_wakeup) {
			...
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[get_dynamic_period] reset g_car_sleep\r\n");
		}
		...
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[get_dynamic_period] g_car_instant=%d, g_car_wakeup=%d, g_car_sleep=%d, I_sleep=%d, gFG_BATT_CAPACITY=%d, g_last_time=%d, new_time=%d\r\n",
			    g_car_instant, g_car_wakeup, g_car_sleep, I_sleep, gFG_BATT_CAPACITY,
...
void FGADC_thread_kthread(void)
{
	...
	/* { */
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
		    "[FGADC] %d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\r\n",
		    gFG_Is_Charging, gFG_current, gFG_SW_CoulombCounter, gFG_columb, gFG_voltage,
		    gFG_capacity_by_v, gFG_capacity_by_c, gFG_capacity_by_c_init, gFG_BATT_CAPACITY,
		    ...
		    gFG_DOD0, gFG_DOD1, current_get_ori, CAR_TUNE_VALUE, AGING_TUNING_VALUE);
	/* } */
...
ssize_t fgadc_log_write(struct file *filp, const char __user *buff, unsigned long len, void *data)
{
	...
	if (proc_fgadc_data[0] == '1') {
		...
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "enable FGADC driver log system:2\n");
		Enable_FGADC_LOG = 2;
	} else {
...
static int __init mt6320_fgadc_init(void)
{
	...
	if (ret) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "****[mt6320_fgadc_driver] Unable to device register(%d)\n", ret);
		return ret;
	}
	...
	if (ret) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "****[mt6320_fgadc_driver] Unable to register driver (%d)\n", ret);
		return ret;
	}

	xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
		    "****[mt6320_fgadc_driver] Initialization : DONE\n");
