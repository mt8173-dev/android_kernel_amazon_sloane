...
int get_bat_sense_volt(int times)
{
	...
	ret =
	    pmic_config_interface(AUXADC_CON14, 0x0, PMIC_RG_SOURCE_CH0_NORM_SEL_MASK,
				  PMIC_RG_SOURCE_CH0_NORM_SEL_SHIFT);
...
int get_i_sense_volt(int times)
{
	...
	ret =
	    pmic_config_interface(AUXADC_CON14, 0x1, PMIC_RG_SOURCE_CH0_NORM_SEL_MASK,
				  PMIC_RG_SOURCE_CH0_NORM_SEL_SHIFT);
...
int get_tbat_volt(int times)
{
	...
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
		    "[get_tbat_volt] return PMIC_IMM_GetOneChannelValue(3,times,1);\n");
	...
	if (upmu_get_cid() == 0x1020) {
		...
		if (Enable_BATDRV_LOG == 1) {
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[get_tbat_volt] return PMIC_IMM_GetOneChannelValue(4,times,1);\n");
		}
		return PMIC_IMM_GetOneChannelValue(4, times, 1);
#endif
	}
	...
	if (tmp32 == 0) {
		...
		if (mt_usb_is_device()) {
			...
			if (bq24196_get_vbus_stat() != 0x3) {	/* not in OTG boost mode */
				if (Enable_BATDRV_LOG == 1) {
					xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
						    "[upmu_is_chr_det] USB host mode but boost mode fault\n");
				}
...
int bat_thread_control(int start)
{				/* for psensor firmware download in factory mode */
	if (start == 1) {
/* start_gpt(gpt_num); */
...
ssize_t bat_log_write(struct file *filp, const char __user *buff, unsigned long len, void *data)
{
			      ...
			      unsigned long len, void *data)
{
	...
	if (proc_bat_data_ctrl[0] == '1') {
		...
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "not valid value, resume bat_thread_kthread() as default\n");
		bat_thread_control(1);
	}
...
static void mt6320_battery_update(struct mt6320_battery_data *bat_data)
{
		...
		if (BMT_status.bat_exist) {
...
#endif
			{
				...
				/*Use gas gauge */
					...
					if (Enable_BATDRV_LOG == 1) {
						xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
							    "[Battery_SyncRecharge] In recharging state, do not sync FG\r\n");
					}
				}
			}
			/* battery charging */
				...
				if (g_bat_full_user_view) {
					...
					if (Enable_BATDRV_LOG == 1) {
						xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
							    "[Battery_Recharging] Keep UI as 100. bat_volt_check_point=%d, BMT_status.SOC=%d\r\n",
					...
					/*Use gas gauge */
						...
						if (bat_volt_check_point >= 100) {
							...
							/* if (Enable_BATDRV_LOG == 1) { */
							xlog_printk(ANDROID_LOG_INFO,
								    "Power/Battery",
								    "[Battery] Use gas gauge : gas gague get 100 first (%d)\r\n",
								    bat_volt_check_point);
							/* } */
						} else {
							if (bat_volt_check_point == BMT_status.SOC) {
								...
								if (Enable_BATDRV_LOG == 1) {
									xlog_printk
									    (ANDROID_LOG_INFO,
									     "Power/Battery",
									     "[Battery] Can sync due to bat_volt_check_point=%d, BMT_status.SOC=%d\r\n",
									     bat_volt_check_point,
									     BMT_status.SOC);
								}
							} else {
								if (Enable_BATDRV_LOG == 1) {
									xlog_printk
									    (ANDROID_LOG_INFO,
									     "Power/Battery",
									     "[Battery] Keep UI due to bat_volt_check_point=%d, BMT_status.SOC=%d\r\n",
									     bat_volt_check_point,
									     BMT_status.SOC);
								}
							}
	...
	/* Only Battery */
		...
		if (BMT_status.bat_vol <= SYSTEM_OFF_VOLTAGE) {
			...
			if (gForceADCsolution == 1) {
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BAT BATTERY] VBAT < %d mV : Android will Power Off System !!\r\n",
					    SYSTEM_OFF_VOLTAGE);
				bat_data->BAT_CAPACITY = 0;
			}
			/*Use gas gauge */
				...
				bat_data->BAT_CAPACITY = bat_volt_check_point;
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[Battery] VBAT < %d mV (%d, gFG_DOD0=%d)\r\n",
					    SYSTEM_OFF_VOLTAGE, bat_volt_check_point, gFG_DOD0);
			}
		}
#ifndef CONFIG_MTK_BQ27541_SUPPORT
		/* If FG_VBAT <= gFG_15_vlot, then run to 15% */
		else if ((gFG_voltage <= gFG_15_vlot) && (gForceADCsolution == 0)
			 && (bat_volt_check_point >= 15)) {
			/*Use gas gauge */
			gSyncPercentage = 1;
			...
			bat_data->BAT_CAPACITY = bat_volt_check_point;
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[Battery] FG_VBAT <= %d, then SOC run to 15. (SOC=%d,Point=%d,D1=%d,D0=%d)\r\n",
				    gFG_15_vlot, BMT_status.SOC, bat_volt_check_point, gFG_DOD1,
				    gFG_DOD0);
		}
		/* If "FG_VBAT > gFG_15_vlot" and "FG_report=15%" , then keep 15% till FG_VBAT <= gFG_15_vlot */
			...
			bat_data->BAT_CAPACITY = bat_volt_check_point;
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[Battery] FG_VBAT(%d) > gFG_15_vlot(%d) and FG_report=15, then UI(%d) keep 15. (D1=%d,D0=%d)\r\n",
				    gFG_voltage, gFG_15_vlot, bat_volt_check_point, gFG_DOD1,
				    gFG_DOD0);
		}
#endif
		else {
			...
			/*Use gas gauge : gas gague get 0% fist */
				...
				if (Enable_BATDRV_LOG == 1) {
					xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
						    "[Battery_OnlyBattery!] bat_volt_check_point=%d,BMT_status.SOC=%d\r\n",
						    bat_volt_check_point, BMT_status.SOC);
				}
				/* if(bat_volt_check_point != BMT_status.SOC) */
				/* if(bat_volt_check_point > BMT_status.SOC) */
				if ((bat_volt_check_point > BMT_status.SOC)
					...
					if (Enable_BATDRV_LOG == 1) {
						xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
							    "[Battery_OnlyBattery] bat_volt_check_point=%d,BMT_status.SOC=%d,gFGsyncTimer=%d(on %d)\r\n",
							    bat_volt_check_point, BMT_status.SOC,
							    gFGsyncTimer, DEFAULT_SYNC_TIME_OUT);
					}
					/* reduce after xxs */
					...
					if (bat_volt_check_point <= 0) {
						...
						/* if (Enable_BATDRV_LOG == 1) { */
						xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
							    "[Battery] Use gas gauge : gas gague get 0 first (%d)\r\n",
							    bat_volt_check_point);
						/* } */
					} else {
						gSyncPercentage = 0;
				...
				if (bat_volt_check_point == 100) {
					...
					if (Enable_BATDRV_LOG == 1) {
						xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
							    "[Battery_Only] Set g_bat_full_user_view=KAL_TRUE\r\n");
					}
				}
			}
		}
	...
	if (Enable_BATDRV_LOG >= 1) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:IntegrationFG:point,per_C,per_V,count,vbat_charger,CSDAC_DAT] %d,%d,%d,%d,%d,ADC_Solution=%d\r\n",
			    bat_volt_check_point, BMT_status.SOC,
			    FGADC_Get_BatteryCapacity_VoltageMothod(), BATTERY_AVERAGE_SIZE,
			    BMT_status.bat_vol, gForceADCsolution);
	}
...
static void mt6320_battery_update_power_down(struct mt6320_battery_data *bat_data)
{
	...
	xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
		    "[Battery] mt6320_battery_update_power_down\r\n");
...
static int mt6320_battery_probe(struct platform_device *dev)
{
	...
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
		    "******** MT6320 battery driver probe!! ********\n");

	/* Integrate with Android Battery Service */
	ret = power_supply_register(&(dev->dev), &mt6320_ac_main.psy);
	if (ret) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[MT6320 BAT_probe] power_supply_register AC Fail !!\n");
		return ret;
	}
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
		    "[MT6320 BAT_probe] power_supply_register AC Success !!\n");

	ret = power_supply_register(&(dev->dev), &mt6320_usb_main.psy);
	if (ret) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[MT6320 BAT_probe] power_supply_register USB Fail !!\n");
		return ret;
	}
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
		    "[MT6320 BAT_probe] power_supply_register USB Success !!\n");

	ret = power_supply_register(&(dev->dev), &mt6320_battery_main.psy);
	if (ret) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[MT6320 BAT_probe] power_supply_register Battery Fail !!\n");
		return ret;
	}
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
		    "[MT6320 BAT_probe] power_supply_register Battery Success !!\n");
...
#if 0
	xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
		    "BattThermistorConverTemp() : TBatt_Value = %d\n", TBatt_Value);
...
INT16 BattVoltToTemp(UINT32 dwVolt)
{
	INT32 TRes;
	INT32 dwVCriBat =
	    (TBAT_OVER_CRITICAL_LOW * RBAT_PULL_UP_VOLT) / (TBAT_OVER_CRITICAL_LOW +
							    RBAT_PULL_UP_R);
	INT32 sBaTTMP = -100;
...
#if 0
	xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
		    "BattVoltToTemp() : TBAT_OVER_CRITICAL_LOW = %d\n", TBAT_OVER_CRITICAL_LOW);
	xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
		    "BattVoltToTemp() : RBAT_PULL_UP_VOLT = %d\n", RBAT_PULL_UP_VOLT);
...
void bq24196_set_ac_current(void)
{
	...
	if (Enable_BATDRV_LOG == 1) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:bq24196] bq24196_set_ac_current \r\n");
	}

	if ((g_temp_CC_value > 2484) || (g_temp_CC_value < 500)) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:bq24196] invalid current selected (%d), use 500mA \r\n",
			    g_temp_CC_value);
...
void bq24196_set_low_chg_current(void)
{
	if (Enable_BATDRV_LOG == 1) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:bq24196] bq24196_set_low_chg_current \r\n");
	}
...
void select_charging_curret_bq24196(void)
{
	if (g_ftm_battery_flag) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:bq24196] FTM charging : %d\r\n", charging_level_data[0]);
		...
		if (BMT_status.charger_type == STANDARD_HOST) {
			if (g_Support_USBIF == 1) {
				if (g_usb_state == USB_SUSPEND) {
					...
					if (Enable_BATDRV_LOG == 1) {
						xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
							    "[BATTERY:bq24196] disable charging\r\n");
					}
					...
					if (Enable_BATDRV_LOG == 1) {
						xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
							    "[BATTERY:bq24196] bq24196_set_ac_current(), CC value (%d) \r\n",
							    g_temp_CC_value);
					}
				} else {
					g_temp_CC_value = USB_CHARGER_CURRENT_UNCONFIGURED;
			...
			if (Enable_BATDRV_LOG == 1) {
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BATTERY:bq24196] Unknown charger type\n");
			}
			g_temp_CC_value = 500;
			bq24196_set_iinlim(0x2);	/* IN current limit at 500mA */
			bq24196_set_ac_current();
...
void ChargerHwInit_bq24196(void)
{
	if (Enable_BATDRV_LOG == 1) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:bq24196] ChargerHwInit_bq24196\n");
	}
...
void pchr_turn_off_charging_bq24196(void)
{
	if (Enable_BATDRV_LOG == 1) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:bq24196] pchr_turn_off_charging_bq24196!\r\n");
	}
...
void pchr_turn_on_charging_bq24196(void)
{
	if (BMT_status.bat_charging_state == CHR_ERROR) {
		if (Enable_BATDRV_LOG == 1) {
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:bq24196] Charger Error, turn OFF charging !\r\n");
		}
		...
		if (Enable_BATDRV_LOG == 1) {
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:bq24196] pchr_turn_on_charging_bq24196 !\r\n");
		}
		...
		if (g_temp_CC_value == 0) {
			...
			if ((gFG_booting_counter_I_FLAG == 2) || (g_ocv_lookup_done == 1)) {
				...
				if (Enable_BATDRV_LOG == 1) {
					xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
						    "[BATTERY:bq24196] charger enable !\r\n");
				}
				if (Enable_BATDRV_LOG == 1) {
				...
				pchr_turn_off_charging_bq24196();
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BATTERY:bq24196] wait gFG_booting_counter_I_FLAG==2 (%d)\r\n",
...
int g_Get_I_Charging(void)
{
	...
	if (Enable_BATDRV_LOG == 1) {
		...
		for (i = 0; i < repeat; i++) {
			xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "%d,",
				    ADC_BAT_SENSE_tmp[i]);
		}
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "\r\n");
	}
	...
	if (Enable_BATDRV_LOG == 1) {
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
			    "[g_Get_I_Charging] ADC_BAT_SENSE=%d\r\n", ADC_BAT_SENSE);
	}
	...
	if (Enable_BATDRV_LOG == 1) {
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
			    "[g_Get_I_Charging] ADC_I_SENSE(Before)=%d\r\n", ADC_I_SENSE);
	}
	...
	if (Enable_BATDRV_LOG == 1) {
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
			    "[g_Get_I_Charging] ADC_I_SENSE(After)=%d\r\n", ADC_I_SENSE);
	}
...
UINT32 BattVoltToPercent(UINT16 dwVoltage)
{
	...
	bPercntResult =
	    (((dwVoltage - VBAT1) * bPercnt2) + ((VBAT2 - dwVoltage) * bPercnt1)) / (VBAT2 - VBAT1);
...
int do_jeita_state_machine(void)
{
	...
	if (BMT_status.temperature >= TEMP_POS_60_THRESHOLD) {
		xlog_printk(ANDROID_LOG_WARN, "Power/Battery",
			    "[BATTERY:bq24196] Battery Over high Temperature(%d) !!\n\r",
		...
		if ((g_temp_status == TEMP_ABOVE_POS_60)
			...
			xlog_printk(ANDROID_LOG_WARN, "Power/Battery",
				    "[BATTERY:bq24196] Battery Temperature between %d and %d,not allow charging yet!!\n\r",
				    TEMP_POS_60_THRES_MINUS_X_DEGREE, TEMP_POS_60_THRESHOLD);

			return PMU_STATUS_FAIL;
		} else {
			xlog_printk(ANDROID_LOG_WARN, "Power/Battery",
				    "[BATTERY:bq24196] Battery Temperature between %d and %d !!\n\r",
		...
		if (((g_temp_status == TEMP_POS_45_TO_POS_60)
			...
			xlog_printk(ANDROID_LOG_WARN, "Power/Battery",
				    "[BATTERY:bq24196] Battery Temperature not recovery to normal temperature charging mode yet!!\n\r");
		} else {
			if (Enable_BATDRV_LOG >= 1) {
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BATTERY:bq24196] Battery Normal Temperature between %d and %d !!\n\r",
		...
		xlog_printk(ANDROID_LOG_WARN, "Power/Battery",
			    "[BATTERY:bq24196] Battery below low Temperature(%d) !!\n\r",
...
int BAT_CheckBatteryStatus_bq24196(void)
{
	...
	if (bat_temperature_volt == 0) {
		...
		if (Enable_BATDRV_LOG == 1) {
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY] Warning !! bat_temperature_volt == 0, restore temperature value\n\r");
		}
	} else {
#if defined(CONFIG_POWER_EXT)
		/* by pass */
		...
		if (gForceADCsolution == 1) {
			...
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[tbat_workaround] %d,%d,%d,%d,%d\n", bat_temperature_volt_temp,
				    bat_temperature_volt, fg_current_state, fg_current_temp,
				    fg_r_value);
		}
		/* ----------------------------------------------------------------------------- */
#endif
	...
	if (ret != 1) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:ADC:bq27541] bq27541 i2c access error, use MT6320 measured battery temperature\n");
	} else {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:ADC:bq27541] battery temperature read raw data(%d)0.1K\n",
			    returnData);
		BMT_status.temperature = ((returnData / 10) - 273);
	}
#endif

	if ((g_battery_tt_check_flag == 0) && (BMT_status.temperature < MAX_CHARGE_TEMPERATURE)
	    && (BMT_status.temperature > MIN_CHARGE_TEMPERATURE)) {
		g_battery_thermal_throttling_flag = 1;
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:bq24196] Enable battery TT (%d)\n\r", BMT_status.temperature);
		g_battery_tt_check_flag = 1;
	}

	if (Enable_BATDRV_LOG == 1) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:ADC:bq24196] VCHR:%d BAT_SENSE:%d I_SENSE:%d TBAT:%d TBAT_vol:%d BatTT:%d\n",
			    BMT_status.charger_vol, BMT_status.ADC_BAT_SENSE,
			    BMT_status.ADC_I_SENSE, BMT_status.temperature, bat_temperature_volt,
	...
	if (gForceADCsolution == 1) {
		...
		if (ret != 1) {
			...
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:bq27541] bq27541 i2c access error\n");
		} else {
			BMT_status.SOC = returnData;
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:bq27541] bq27541 read SOC raw data = %d\n",
				    returnData);
		}
#else
		BMT_status.SOC = BattVoltToPercent(BMT_status.bat_vol);
	...
	/*Use gas gauge */
		...
		if (gSyncPercentage == 0) {
			if ((upmu_is_chr_det() == KAL_TRUE) && (!g_Battery_Fail)
			    && (g_Charging_Over_Time == 0) && (usb_is_discharging_det() == 0)) {
				/* SOC only UP when charging */
				if (BMT_status.SOC > bat_volt_check_point) {
					bat_volt_check_point = BMT_status.SOC;
				}
	...
	if (ret != 1) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:AVG:bq27541] bq27541 i2c access error, use MT6320 measured battery temperature\n");
	} else {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:AVG:bq27541] battery temperature read raw data(%d)0.1K\n",
			    returnData);
		BMT_status.temperature = ((returnData / 10) - 273);
	}
#endif
	...
	if ((gFG_booting_counter_I_FLAG < 2) && upmu_is_chr_det()) {
		...
		printk
		    ("[Boot SOC workaround 1] boot_soc_temp=%d, BMT_status.SOC=%d, boot_soc_temp_count=%d\n",
		...
		if (boot_soc_temp_count > 0) {
			...
			if (!(upmu_is_chr_det())) {	/* charger from 1 -> 0 */
				...
				printk
				    ("[Boot SOC workaround 2] boot_soc_temp=%d, BMT_status.SOC=%d, boot_soc_temp_count=%d, gFG_DOD0=%d, gFG_DOD1=%d\n",
				     boot_soc_temp, BMT_status.SOC, boot_soc_temp_count, gFG_DOD0,
				     gFG_DOD1);
			}
		}
	}
	...
	if (Enable_BATDRV_LOG == 1) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:AVG:bq24196] BatTemp:%d Vbat:%d VBatSen:%d SOC:%d ChrDet:%d Vchrin:%d Icharging:%d ChrType:%d USBstate:%d\r\n",
			    BMT_status.temperature, BMT_status.bat_vol, BMT_status.ADC_BAT_SENSE,
			    BMT_status.SOC, upmu_is_chr_det(), BMT_status.charger_vol,
			    BMT_status.ICharging, CHR_Type_num, g_usb_state);
	}

	if (Enable_BATDRV_LOG == 1) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:FG:bq24196] %d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\r\n",
			    BMT_status.temperature, BMT_status.bat_vol, BMT_status.ADC_BAT_SENSE,
			    BMT_status.SOC, upmu_is_chr_det(), BMT_status.charger_vol,
			    BMT_status.ICharging, CHR_Type_num,
			    FGADC_Get_BatteryCapacity_CoulombMothod(),
			    FGADC_Get_BatteryCapacity_VoltageMothod(), BATTERY_AVERAGE_SIZE);
	}
	...
	if (battery_cmd_thermal_test_mode == 1) {
		...
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:bq24196] In thermal_test_mode, Tbat=%d\n",
			    BMT_status.temperature);
	}
#if defined(MTK_JEITA_STANDARD_SUPPORT)
	if (Enable_BATDRV_LOG == 1) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:bq24196] support JEITA, Tbat=%d\n", BMT_status.temperature);
	}
	...
	if (upmu_is_chr_det() == KAL_TRUE) {
#if defined(MTK_JEITA_STANDARD_SUPPORT)
		if ((BMT_status.bat_vol < g_jeita_recharging_voltage) && (BMT_status.bat_full)
		    && (g_HW_Charging_Done == 1) && (!g_Battery_Fail))
#else
...
#endif
		{
			if (Enable_BATDRV_LOG == 1) {
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BATTERY:bq24196] Battery Re-charging !!\n\r");
			}
...
PMU_STATUS BAT_BatteryStatusFailAction(void)
{
	if (Enable_BATDRV_LOG == 1) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:bq24196] BAD Battery status... Charging Stop !!\n\r");
	}
#if defined(MTK_JEITA_STANDARD_SUPPORT)
...
{
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
		    "[BATTERY:bq24196] Charging over %d hr stop !!\n\r", MAX_CHARGING_TIME);
...
PMU_STATUS BAT_BatteryFullAction(void)
{
	if (Enable_BATDRV_LOG == 1) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:bq24196] check Battery full !!\n\r");
	}
	...
	if (gFG_can_reset_flag == 1) {
...
#endif
		{
			...
			if (Enable_BATDRV_LOG >= 1) {
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BATTERY:bq24196] Battery real full. Call FGADC_Reset_SW_Parameter.\n");
			}
			FGADC_Reset_SW_Parameter();
		} else {
			if (Enable_BATDRV_LOG >= 1) {
			...
			if (Enable_BATDRV_LOG >= 1) {
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BATTERY:bq24196] (%d) VBAT<4150mV. Don't FGADC_Reset_SW_Parameter.\n",
					    BMT_status.bat_vol);
			}
		}
	}
...
void mt_battery_notify_check(void)
{
	...
	if (g_BN_TestMode == 0x0000) {
		if (Enable_BATDRV_LOG == 1) {
			xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
				    "[BATTERY:bq24196] mt_battery_notify_check\n");
		}
#if defined(BATTERY_NOTIFY_CASE_0000)
		if (Enable_BATDRV_LOG == 1) {
			xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
				    "[BATTERY:bq24196] BATTERY_NOTIFY_CASE_0000\n");
		}
#endif
			...
			/* if(BMT_status.charger_vol > 3000) //test */
		{
			...
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:bq24196] BMT_status.charger_vol(%d) > %d mV\n",
		...
		if (Enable_BATDRV_LOG == 1) {
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:bq24196] BATTERY_NOTIFY_CASE_0001 (%x)\n",
				    g_BatteryNotifyCode);
		}
#endif
			...
			/* if(BMT_status.temperature > 20) //test */
		{
			...
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:bq24196] bat_temp(%d) > 50'C\n",
				    BMT_status.temperature);
		} else {
			g_BatteryNotifyCode &= ~(0x0002);
		}
		if (Enable_BATDRV_LOG == 1) {
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:bq24196] BATTERY_NOTIFY_CASE_0002 (%x)\n",
				    g_BatteryNotifyCode);
		}
#endif
			...
			/* if(BMT_status.ICharging > 200) //test */
		{
			...
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:bq24196] I_charging(%d) > 1000mA\n",
		...
		if (Enable_BATDRV_LOG == 1) {
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:bq24196] BATTERY_NOTIFY_CASE_0003 (%x)\n",
				    g_BatteryNotifyCode);
		}
#endif
			...
			/* if(BMT_status.bat_vol > 3800) //test */
		{
			...
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:bq24196] bat_vlot(%d) > 4350mV\n",
				    BMT_status.bat_vol);
		} else {
			g_BatteryNotifyCode &= ~(0x0008);
		}
		if (Enable_BATDRV_LOG == 1) {
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:bq24196] BATTERY_NOTIFY_CASE_0004 (%x)\n",
				    g_BatteryNotifyCode);
		}
#endif
		...
		if ((g_battery_thermal_throttling_flag == 2)
				...
				/* if(BMT_status.total_charging_time >= 60) //test */
			{
				...
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BATTERY:bq24196] Charging Over Time\n");
			} else {
				g_BatteryNotifyCode &= ~(0x0010);
			}
		}
		...
		if (Enable_BATDRV_LOG == 1) {
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:bq24196] BATTERY_NOTIFY_CASE_0005 (%x)\n",
				    g_BatteryNotifyCode);
		}
#endif
		...
		g_BatteryNotifyCode = 0x0001;
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
			    "[BATTERY_TestMode] BATTERY_NOTIFY_CASE_0001\n");
	} else if (g_BN_TestMode == 0x0002) {
		g_BatteryNotifyCode = 0x0002;
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
			    "[BATTERY_TestMode] BATTERY_NOTIFY_CASE_0002\n");
	} else if (g_BN_TestMode == 0x0003) {
		g_BatteryNotifyCode = 0x0004;
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
			    "[BATTERY_TestMode] BATTERY_NOTIFY_CASE_0003\n");
	} else if (g_BN_TestMode == 0x0004) {
		g_BatteryNotifyCode = 0x0008;
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
			    "[BATTERY_TestMode] BATTERY_NOTIFY_CASE_0004\n");
	} else if (g_BN_TestMode == 0x0005) {
		g_BatteryNotifyCode = 0x0010;
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
			    "[BATTERY_TestMode] BATTERY_NOTIFY_CASE_0005\n");
	} else {
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
			    "[BATTERY:bq24196] Unknown BN_TestMode Code : %x\n", g_BN_TestMode);
	}
}
...
void BAT_thread_bq24196(void)
{
...
#if defined(MTK_JEITA_STANDARD_SUPPORT)
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
			    "[BAT_thread_bq24196] LOG. %d,%d,%d, %d----------------------------\n",
			    BATTERY_AVERAGE_SIZE, RECHARGING_VOLTAGE, gFG_15_vlot,
			    mtk_jeita_support_flag);
#else
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
			    "[BAT_thread_bq24196] LOG. %d,%d,%d----------------------------\n",
			    BATTERY_AVERAGE_SIZE, RECHARGING_VOLTAGE, gFG_15_vlot);
#endif
	}
...
#if defined(CONFIG_POWER_EXT)
			xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
				    "[BATTERY:bq24196] CONFIG_POWER_EXT, no update mt6577_battery_update_power_down.\n");
#else
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[Battery:bq24196] Tbat(%d)>=60, system need power down.\n",
				    BMT_status.temperature);
	...
	if (upmu_is_chr_det() == KAL_TRUE) {
		...
		if (BMT_status.charger_type == CHARGER_UNKNOWN) {
			...
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BAT_thread:bq24196] CHR_Type_num = %d\r\n", CHR_Type_num);
		...
		/*Use gas gauge */
			...
			if (bat_volt_check_point != 100) {
				...
				if (Enable_BATDRV_LOG == 1) {
					xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
						    "[Battery_Only:bq24196] Set g_bat_full_user_view=KAL_FALSE\r\n");
				}
			}
		}
	...
	if (battery_cmd_thermal_test_mode == 1) {
		BMT_status.temperature = battery_cmd_thermal_test_mode_value;
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:bq24196] In thermal_test_mode 3, Tbat=%d\n",
			    BMT_status.temperature);
	}
...
#if defined(CONFIG_POWER_EXT)
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
		    "[BATTERY:bq24196] CONFIG_POWER_EXT, no update Android.\n");
#else
	if (gFG_booting_counter_I_FLAG == 2) {
		...
		if (gForceADCsolution == 1) {
			...
			mt6320_battery_main.BAT_CAPACITY = 50;
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:bq24196] gFG_booting_counter_I_FLAG is 1, SOC=50.\n");
#endif
		}
	} else {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:bq24196] gFG_booting_counter_I_FLAG!=2 (%d)\r\n",
			    gFG_booting_counter_I_FLAG);
	}
#endif
	...
	if (BAT_status == PMU_STATUS_FAIL || g_Battery_Fail) {
		...
		if (Enable_BATDRV_LOG == 1) {
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:bq24196] PMU_STATUS_FAIL (chr_err_cnt = %d)\n",
				    chr_err_cnt);
		}
	}
	/* Battery Full */
	/* HW charging done, real stop charging */
		...
		if (Enable_BATDRV_LOG == 1) {
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:bq24196] Battery real full.\n");
		}
		BMT_status.bat_full = KAL_TRUE;
		BMT_status.total_charging_time = 0;
		BMT_status.PRE_charging_time = 0;
	...
	/* Battery Not Full and Charger exist : Do Charging */
		...
		if ((BMT_status.bat_vol > RECHARGING_VOLTAGE) && (bq24196_stat == 0x3)) {
			...
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:bq24196] Battery real full and disable charging (%d)\n",
				    bq24196_stat);
			gSyncPercentage = 1;
			return;
			...
			if (Enable_BATDRV_LOG == 1) {
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BATTERY:bq24196] bq24196_status is (%d), but battery voltage not over recharge voltage\n",
					    bq24196_stat);
			}
		}
		...
		if (Enable_BATDRV_LOG == 1) {
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:bq24196] Total charging timer=%d, bq24196_status=%d\n\r",
...
void PrechargeCheckStatus(void)
{
	...
	if (get_charger_detect_status() == KAL_TRUE) {	/* do not check USB device check at precharge state */
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:bq24196] Boot up during charging!\n");
		g_boot_charging = 1;
...
/* pchr_turn_off_charging_bq24196(); */
			...
			bat_vol = get_i_sense_volt(10);
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:bq24196] Powerpath init battery voltage > 3600, battery voltage during system discharging = %d\n",
				    bat_vol);
			return;
		} else {
...
/* bq24196_set_batfet_disable(0x0); // turn on BATFET to continue charging */
		...
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[BATTERY:bq24196] Powerpath init battery voltage < 3600, battery voltage  = %d\n",
			    bat_vol);
	} else {
		return;
	}
	...
	while (1) {
		if (get_charger_detect_status() == KAL_TRUE) {	/* do not check USB device check at precharge state */
			...
			if (BMT_status.charger_type == CHARGER_UNKNOWN) {
				CHR_Type_num = mt_charger_type_detection();
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BATTERY:bq24196] CHR_Type_num = %d\r\n",
					    CHR_Type_num);
				BMT_status.charger_type = CHR_Type_num;
...
/* if(upmu_get_cid() == 0x1020) */
			...
			if ((g_battery_tt_check_flag == 0) && (bat_temp < 60) && (bat_temp > (-20))) {
				g_battery_thermal_throttling_flag = 1;
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BATTERY:bq24196] Powerpath precharge enable battery TT (%d)\n\r",
					    bat_temp);
				g_battery_tt_check_flag = 1;
			}
			...
			if ((bat_temp <= MIN_CHARGE_TEMPERATURE) ||
				...
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BATTERY:bq24196] Powerpath precharge battery Under Temperature or NTC fail !!\n\r");
				BMT_status.bat_charging_state = CHR_ERROR;
			}
#endif
			if (bat_temp >= MAX_CHARGE_TEMPERATURE) {
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BATTERY:bq24196] Powerpath precharge battery Over Temperature !!\n\r");
				BMT_status.bat_charging_state = CHR_ERROR;
			}
#if (V_CHARGER_ENABLE == 1)
			if (charger_vol <= V_CHARGER_MIN) {
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BATTERY:bq24196] Powerpath precharge charger under voltage!!\r\n");
				BMT_status.bat_charging_state = CHR_ERROR;
			}
#endif
			if (charger_vol >= V_CHARGER_MAX) {
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BATTERY:bq24196] Powerpath precharge charger over voltage !!\r\n");
				BMT_status.bat_charging_state = CHR_ERROR;
			}
			...
			if ((g_temp_CC_value == 0) || (chr_err_cnt > 0)) {
				...
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BATTERY:bq24196] Powerpath precharge chr_err_cnt = %d\r\n",
					    chr_err_cnt);
			} else {
				bq24196_set_chg_config(0x1);	/* charger enable */
				if (Enable_BATDRV_LOG == 1) {
					xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
						    "[BATTERY:bq24196] Power path precharging !\r\n");
				}
			}

			if (bat_vol >= SYSTEM_OFF_VOLTAGE + 100) {
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BATTERY:bq24196] Powerpath precharge finish, battery voltage = %d\n",
					    bat_vol);
				BMT_status.bat_charging_state = CHR_CC;
				break;
			} else {
				...
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
					    "[BATTERY:bq24196] Powerpath precharge continue, battery voltage (%d), system voltage (%d) (%d), charger volt (%d)\n",
			...
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:bq24196]VCDT voltage = %d\n", get_charger_volt(10));
			wake_unlock(&battery_suspend_lock);
/* pchr_turn_off_charging_bq24196(); */
			bq24196_set_wdt_rst(0x1);	/* Kick watchdog */
			bq24196_set_chg_config(0x0);
...
#endif
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
				    "[BATTERY:bq24196] Powerpath precharge stopped due to charger plug-out, wait for system shutdown\n");
			msleep(5000);
		}
	}
...
int bat_thread_kthread(void *x)
{
	...
	while (1) {
		...
		if (g_battery_flag_resume == 0) {
			...
			xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
				    "[bat_thread_kthread] g_battery_flag_resume=%d\r\n",
				    g_battery_flag_resume);
		}

		if (Enable_BATDRV_LOG == 1) {
			xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
				    "******** MT6320 battery : bat_thread_kthread : 1 ********\n");
		}
		...
		if (Enable_BATDRV_LOG == 1) {
			xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
				    "******** MT6320 battery : bat_thread_kthread : 2 ********\n");
		}
		...
		if (g_wake_up_bat == 1) {
			...
			if (Enable_BATDRV_LOG == 1) {
				xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
					    "[BATTERY] Call FGADC_Reset_SW_Parameter.\r\n");
			}
		}
...
void bat_thread_wakeup(void)
{
	if (Enable_BATDRV_LOG == 1) {
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
			    "******** MT6320 battery : bat_thread_wakeup : 1 ********\n");
	}
	...
	if (Enable_BATDRV_LOG == 1) {
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
			    "******** MT6320 battery : bat_thread_wakeup : 2 ********\n");
	}
}
...
static long adc_cali_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case SET_ADC_CALI_Slop:
		...
		for (i = 0; i < 14; i++)
			xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "adc_cali_slop[%d] = %d\n",
				    i, *(adc_cali_slop + i));
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
	...
	case SET_ADC_CALI_Cal:
		...
		for (i = 0; i < 1; i++)
			xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "adc_cali_cal[%d] = %d\n",
				    i, *(adc_cali_cal + i));
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
	...
	case ADC_CHANNEL_READ:
		...
		if (adc_in_data[0] == 0) {	/* I_SENSE */
			...
			adc_out_data[0] =
			    PMIC_IMM_GetOneChannelValue(adc_in_data[0], adc_in_data[1],
							1) * adc_in_data[1];
		}
	...
	case Set_Charger_Current:	/* For Factory Mode */
		...
		if (charging_level_data[0] == 0) {
			charging_level_data[0] = 100;
		} else if (charging_level_data[0] == 1) {
			charging_level_data[0] = 400;
		} else if (charging_level_data[0] == 2) {
			charging_level_data[0] = 500;
		} else if (charging_level_data[0] == 3) {
			charging_level_data[0] = 600;
		} else if (charging_level_data[0] == 4) {
			charging_level_data[0] = 700;
		} else if (charging_level_data[0] == 5) {
			charging_level_data[0] = 800;
		} else if (charging_level_data[0] == 6) {
			charging_level_data[0] = 900;
		} else if (charging_level_data[0] == 7) {
			charging_level_data[0] = 1000;
		} else if (charging_level_data[0] == 8) {
			charging_level_data[0] = 1100;
		} else if (charging_level_data[0] == 9) {
			charging_level_data[0] = 1200;
		} else if (charging_level_data[0] == 10) {
			charging_level_data[0] = 1300;
		} else if (charging_level_data[0] == 11) {
			charging_level_data[0] = 1400;
		} else if (charging_level_data[0] == 12) {
			charging_level_data[0] = 1500;
		} else if (charging_level_data[0] == 13) {
			charging_level_data[0] = 1600;
		} else if (charging_level_data[0] == 14) {
			charging_level_data[0] = 1700;
		} else if (charging_level_data[0] == 15) {
			charging_level_data[0] = 1800;
		} else if (charging_level_data[0] == 16) {
			charging_level_data[0] = 1900;
		} else if (charging_level_data[0] == 17) {
			charging_level_data[0] = 2000;
		} else if (charging_level_data[0] == 18) {
			charging_level_data[0] = 2100;
		} else if (charging_level_data[0] == 19) {
			charging_level_data[0] = 2200;
		} else if (charging_level_data[0] == 20) {
			charging_level_data[0] = 2300;
		} else if (charging_level_data[0] == 21) {
			charging_level_data[0] = 2400;
		} else if (charging_level_data[0] == 22) {
			charging_level_data[0] = 2500;
		} else {
			charging_level_data[0] = 500;
		}
		wake_up_bat();
	...
	case BAT_THREAD_CTRL:
		...
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
			    "**** unlocked_ioctl : bat_thread_ctrl:%d\n", bat_thread_ctrl[0]);
		break;
...
void battery_kthread_hrtimer_init(void)
{
	...
	battery_kthread_hrtimer_task =
	    kthread_run(battery_kthread_handler, NULL, "mtk battery_kthread_handler");
	if (IS_ERR(battery_kthread_hrtimer_task)) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "[%s]: failed to create battery_kthread_hrtimer_task thread\n",
			    __func__);
	}
...
static int mt6320_battery_probe(struct platform_device *dev)
{
	...
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
		    "[MT6320 BAT_probe] adc_cali prepare : done !!\n ");
...
/* wake_lock_init(&battery_suspend_lock, WAKE_LOCK_SUSPEND, "battery wakelock"); */
	...
	kthread_run(bat_thread_kthread, NULL, "bat_thread_kthread");
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
		    "[mt6320_battery_probe] bat_thread_kthread Done\n");
	...
	xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
		    "DCT-PMIC-ADC:AUXADC_BATTERY_VOLTAGE_CHANNEL=%d\r\n",
		    AUXADC_BATTERY_VOLTAGE_CHANNEL);
	xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
		    "DCT-PMIC-ADC:AUXADC_REF_CURRENT_CHANNEL=%d\r\n", AUXADC_REF_CURRENT_CHANNEL);
	xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
		    "DCT-PMIC-ADC:AUXADC_CHARGER_VOLTAGE_CHANNEL=%d\r\n",
		    AUXADC_CHARGER_VOLTAGE_CHANNEL);
	xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
		    "DCT-PMIC-ADC:AUXADC_TEMPERATURE_CHANNEL=%d\r\n", AUXADC_TEMPERATURE_CHANNEL);
...
{
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
		    "******** MT6320 battery driver shutdown!! ********\n");
				   ...
				   const char *buf, size_t size)
{
	...
	if (buf != NULL && size != 0) {
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery",
			    "[Battery] buf is %s and size is %d\n", buf, size);
...
static ssize_t battery_cmd_write(struct file *file, const char *buffer, unsigned long count,
				 void *data)
{
...
static int __init mt6320_battery_init(void)
{
	...
	if (ret) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "****[mt6320_battery_driver] Unable to device register(%d)\n", ret);
		return ret;
	}
	...
	if (ret) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
			    "****[mt6320_battery_driver] Unable to register driver (%d)\n", ret);
		return ret;
	}
	/* battery notofy UI */
	...
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery",
		    "****[mt6320_battery_driver] Initialization : DONE !!\n");
