...
static kal_uint32 fg_get_data_ready_status(void)
{
	...
	temp_val =
	    (temp_val & (MT6332_PMIC_FG_LATCHDATA_ST_MASK << MT6332_PMIC_FG_LATCHDATA_ST_SHIFT)) >>
	    MT6332_PMIC_FG_LATCHDATA_ST_SHIFT;
...
/* 1. HW initialization */
	...
	bm_print(BM_LOG_CRTI, "[fgauge_initialization] Reg[0x%x]=0x%x\n", MT6332_FGADC_CON11,
		 upmu_get_reg_value(MT6332_FGADC_CON11));
...
#else
	*(kal_int32 *) (data) =
	    PMIC_IMM_GetOneChannelValue(AUX_BATSNS_AP, *(kal_int32 *) (data), 1);
#endif
...
#else
	*(kal_int32 *) (data) =
	    PMIC_IMM_GetOneChannelValue(AUX_ISENSE_AP, *(kal_int32 *) (data), 1);
#endif
...
static kal_int32(*const bm_func[BATTERY_METER_CMD_NUMBER]) (void *data) = {
	fgauge_initialization	/* hw fuel gague used only */
	    ...
	    , read_adc_v_bat_sense, read_adc_v_i_sense, read_adc_v_bat_temp, read_adc_v_charger, read_hw_ocv, dump_register_fgadc	/* hw fuel gague used only */
};
