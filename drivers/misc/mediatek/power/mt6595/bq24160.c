...
/**********************************************************
  ...
  *
  *********************************************************/
int bq24160_read_byte(kal_uint8 cmd, kal_uint8 *returnData)
{
	int ret;
	...
	if (ret != 2)
		battery_xlog_printk(BAT_LOG_FULL, "[%s]ret=0x%x\n", __func__, ret);

	return ret == 2 ? 1 : 0;
}

int bq24160_write_byte(kal_uint8 cmd, kal_uint8 writeData)
{
	char buf[2];
	int ret;
...
/**********************************************************
  ...
  *
  *********************************************************/
kal_uint32 bq24160_read_interface(kal_uint8 RegNum, kal_uint8 *val, kal_uint8 MASK,
				  kal_uint8 SHIFT)
{
...
kal_uint32 bq24160_config_interface(kal_uint8 RegNum, kal_uint8 val, kal_uint8 MASK,
				    kal_uint8 SHIFT)
{
	kal_uint8 bq24160_reg = 0;
	int ret = 0;
	...
	if (RegNum == bq24160_CON2 && val == 1 && MASK == CON2_RESET_MASK
	    && SHIFT == CON2_RESET_SHIFT) {
		/* RESET bit */
...
kal_uint32 bq24160_get_instat(void)
{
	...
	ret = bq24160_read_interface((kal_uint8) (bq24160_CON1),
				     (&val),
				     (kal_uint8) (CON1_INSTAT_MASK), (kal_uint8) (CON1_INSTAT_SHIFT)
	    );
	return val;
}
...
void bq24160_set_reset(kal_uint32 val)
{
	...
	ret = bq24160_config_interface((kal_uint8) (bq24160_CON2),
				       (kal_uint8) (val),
				       (kal_uint8) (CON2_RESET_MASK), (kal_uint8) (CON2_RESET_SHIFT)
	    );
}
...
void bq24160_set_te(kal_uint32 val)
{
	...
	ret = bq24160_config_interface((kal_uint8) (bq24160_CON2),
				       (kal_uint8) (val),
				       (kal_uint8) (CON2_TE_MASK), (kal_uint8) (CON2_TE_SHIFT)
	    );
}
...
void bq24160_set_ce(kal_uint32 val)
{
	...
	ret = bq24160_config_interface((kal_uint8) (bq24160_CON2),
				       (kal_uint8) (val),
				       (kal_uint8) (CON2_CE_MASK), (kal_uint8) (CON2_CE_SHIFT)
	    );
}
...
void bq24160_set_vbreg(kal_uint32 val)
{
	...
	ret = bq24160_config_interface((kal_uint8) (bq24160_CON3),
				       (kal_uint8) (val),
				       (kal_uint8) (CON3_VBREG_MASK), (kal_uint8) (CON3_VBREG_SHIFT)
	    );
}
...
kal_uint32 bq24160_get_pn(void)
{
	...
	ret = bq24160_read_interface((kal_uint8) (bq24160_CON4),
				     (&val), (kal_uint8) (CON4_PN_MASK), (kal_uint8) (CON4_PN_SHIFT)
	    );
	return val;
}
...
void bq24160_set_ichrg(kal_uint32 val)
{
	...
	ret = bq24160_config_interface((kal_uint8) (bq24160_CON5),
				       (kal_uint8) (val),
				       (kal_uint8) (CON5_ICHRG_MASK), (kal_uint8) (CON5_ICHRG_SHIFT)
	    );
}
...
void bq24160_set_tmr(kal_uint32 val)
{
	...
	ret = bq24160_config_interface((kal_uint8) (bq24160_CON7),
				       (kal_uint8) (val),
				       (kal_uint8) (CON7_TMR_MASK), (kal_uint8) (CON7_TMR_SHIFT)
	    );
}
...
void bq24160_set_ts_en(kal_uint32 val)
{
	...
	ret = bq24160_config_interface((kal_uint8) (bq24160_CON7),
				       (kal_uint8) (val),
				       (kal_uint8) (CON7_TS_EN_MASK), (kal_uint8) (CON7_TS_EN_SHIFT)
	    );
}
...
void bq24160_hw_component_detect(void)
{
	...
	battery_xlog_printk(BAT_LOG_CRTI,
			    "[bq24160_hw_component_detect] exist=%d, Reg[0x04]=0x%x\n",
			    g_bq24160_hw_exist, val);
}

int is_bq24160_exist(void)
{
	battery_xlog_printk(BAT_LOG_CRTI, "[is_bq24160_exist] g_bq24160_hw_exist=%d\n",
			    g_bq24160_hw_exist);
...
{
	int i = 0;
	battery_xlog_printk(BAT_LOG_FULL, "[bq24160] ");
	for (i = 0; i < bq24160_REG_NUM; i++) {
		bq24160_read_byte(i, &bq24160_reg[i]);
		battery_xlog_printk(BAT_LOG_FULL, "[0x%x]=0x%x ", i, bq24160_reg[i]);
	}
	battery_xlog_printk(BAT_LOG_FULL, "\n");
}
...
void bq24160_hw_init(void)
{
	...
	/* set AC input current limit at 1.5A and set battery regulation voltage at 4.2V */
	bq24160_config_interface(bq24160_CON3, 0x8C, 0xFF, 0);
	/* set charge current at 1A (default) and set termination current at 150mA (default) */
	bq24160_config_interface(bq24160_CON5, 0x32, 0xFF, 0);
...
static ssize_t store_bq24160_access(struct device *dev, struct device_attribute *attr,
				    const char *buf, size_t size)
{
	...
	if (buf != NULL && size != 0) {
		battery_xlog_printk(BAT_LOG_CRTI,
				    "[store_bq24160_access] buf is %s and size is %d\n", buf, size);
		reg_address = kstrtoul(buf, &pvalue, 16);

		if (size > 3) {
			reg_value = kstrtoul((pvalue + 1), NULL, 16);
			battery_xlog_printk(BAT_LOG_CRTI,
					    "[store_bq24160_access] write bq24160 reg 0x%x with value 0x%x !\n",
			...
			battery_xlog_printk(BAT_LOG_CRTI,
					    "[store_bq24160_access] read bq24160 reg 0x%x with value 0x%x !\n",
					    reg_address, g_reg_value_bq24160);
			battery_xlog_printk(BAT_LOG_CRTI,
					    "[store_bq24160_access] Please use \"cat bq24160_access\" to get value\r\n");
		}
	}
	return size;
}
...
static struct i2c_board_info i2c_bq24160 __initdata = {
	I2C_BOARD_INFO("bq24160", (bq24160_SLAVE_ADDR_WRITE >> 1))
};
...
static int __init bq24160_init(void)
{
	...
	if (i2c_add_driver(&bq24160_driver) != 0) {
		battery_xlog_printk(BAT_LOG_CRTI,
				    "[bq24160_init] failed to register bq24160 i2c driver.\n");
	} else {
		battery_xlog_printk(BAT_LOG_CRTI,
				    "[bq24160_init] Success to register bq24160 i2c driver.\n");
	}
	...
	if (ret) {
		battery_xlog_printk(BAT_LOG_CRTI,
				    "****[bq24160_init] Unable to device register(%d)\n", ret);
		return ret;
	}
	ret = platform_driver_register(&bq24160_user_space_driver);
	if (ret) {
		battery_xlog_printk(BAT_LOG_CRTI,
				    "****[bq24160_init] Unable to register driver (%d)\n", ret);
		return ret;
	}
