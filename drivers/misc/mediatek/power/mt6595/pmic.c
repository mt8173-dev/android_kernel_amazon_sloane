...
#include <drivers/misc/mediatek/power/mt6595/upmu_common.h>
...
static ssize_t show_MT6331_BUCK_CURRENT_METER(struct device *dev, struct device_attribute *attr,
					      char *buf)
{
	int ret_value = 0;
...
static ssize_t store_MT6331_BUCK_CURRENT_METER(struct device *dev, struct device_attribute *attr,
...
static DEVICE_ATTR(MT6331_BUCK_CURRENT_METER, 0664, show_MT6331_BUCK_CURRENT_METER,
		   store_MT6331_BUCK_CURRENT_METER);

static ssize_t show_MT6332_BUCK_CURRENT_METER(struct device *dev, struct device_attribute *attr,
					      char *buf)
{
	int ret_value = 0;
...
static ssize_t store_MT6332_BUCK_CURRENT_METER(struct device *dev, struct device_attribute *attr,
					       const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static DEVICE_ATTR(MT6332_BUCK_CURRENT_METER, 0664, show_MT6332_BUCK_CURRENT_METER,
		   store_MT6332_BUCK_CURRENT_METER);
...
void low_battery_protect_init(void)
{
	...
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
		    ...
		    upmu_get_reg_value(MT6332_AUXADC_CON18), MT6332_AUXADC_CON17,
		    upmu_get_reg_value(MT6332_AUXADC_CON17), MT6332_INT_CON2,
		    upmu_get_reg_value(MT6332_INT_CON2)
	    );
...
{				/* 0:no limit */
	...
	if (g_battery_oc_stop == 1) {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[exec_battery_oc_callback] g_battery_oc_stop=%d\n", g_battery_oc_stop);
	} else {
...
void battery_oc_protect_init(void)
{
	if (mt6332_upmu_get_swcid() >= PMIC6332_E2_CID_CODE) {
		...
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "Reg[0x%x]=0x%x, Reg[0x%x]=0x%x, Reg[0x%x]=0x%x\n", MT6332_FGADC_CON24,
			    upmu_get_reg_value(MT6332_FGADC_CON24), MT6332_FGADC_CON25,
			    upmu_get_reg_value(MT6332_FGADC_CON25), MT6332_INT_CON2,
			    upmu_get_reg_value(MT6332_INT_CON2)
		    );
		...
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[battery_oc_protect_init] E1 cannot support\n");
	}
}
				     ...
				     BATTERY_PERCENT_PRIO prio_val)
{
	...
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
		    "[register_battery_percent_notify] prio_val=%d\n", prio_val);

	if ((g_battery_percent_stop == 0) && (g_battery_percent_level == 1)) {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[register_battery_percent_notify] level l happen\n");
...
{				/* 0:no limit */
	...
	if (g_battery_percent_stop == 1) {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[exec_battery_percent_callback] g_battery_percent_stop=%d\n",
...
void bat_percent_notify_init(void)
{
	...
	if (IS_ERR(bat_percent_notify_thread))
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "Failed to create bat_percent_notify_thread\n");
	else
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "Create bat_percent_notify_thread : done\n");
}
...
static struct task_struct *pmic_6331_thread_handle;
struct wake_lock pmicThread_lock_mt6331;

static DEFINE_MUTEX(pmic_mutex_mt6332);
static struct task_struct *pmic_6332_thread_handle;
...
void cust_pmic_interrupt_en_setting_mt6331(void)
{
	...
	mt6331_upmu_set_rg_int_en_mad(0);
...
void chrdet_int_handler(void)
{
	...
	if (!upmu_get_rgs_chrdet()) {
		...
		if (boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT
			...
			xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
				    "[chrdet_int_handler] Unplug Charger/USB In Kernel Power Off Charging Mode!  Shutdown OS!\r\n");
			mt_power_off();
		}
	}
#else
...
static void mt6331_int_handler(void)
{
	...
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC_INT] mt6331_int_status_val_0=0x%x\n",
		    mt6331_int_status_val_0);

	if ((((mt6331_int_status_val_0) & (0x0001)) >> 0) == 1)
		pwrkey_int_handler();
	if ((((mt6331_int_status_val_0) & (0x0002)) >> 1) == 1)
		homekey_int_handler();
	if ((((mt6331_int_status_val_0) & (0x0004)) >> 2) == 1)
		chrdet_int_handler();
	if ((((mt6331_int_status_val_0) & (0x0008)) >> 3) == 1)
		mt6331_thr_h_int_handler();
	if ((((mt6331_int_status_val_0) & (0x0010)) >> 4) == 1)
		mt6331_thr_l_int_handler();
	if ((((mt6331_int_status_val_0) & (0x0020)) >> 5) == 1)
		mt6331_bat_h_int_handler();
	if ((((mt6331_int_status_val_0) & (0x0040)) >> 6) == 1)
		mt6331_bat_l_int_handler();
	if ((((mt6331_int_status_val_0) & (0x0080)) >> 7) == 1)
		rtc_int_handler();
	if ((((mt6331_int_status_val_0) & (0x0100)) >> 8) == 1)
		audio_int_handler();
	if ((((mt6331_int_status_val_0) & (0x0200)) >> 9) == 1)
		mad_int_handler();
	if ((((mt6331_int_status_val_0) & (0x0400)) >> 10) == 1)
		accdet_int_handler();
	if ((((mt6331_int_status_val_0) & (0x0800)) >> 11) == 1)
		accdet_eint_int_handler();
	if ((((mt6331_int_status_val_0) & (0x1000)) >> 12) == 1)
		accdet_negv_int_handler();
	/* -------------------------------------------------------------------------------- */
	ret = pmic_read_interface(MT6331_INT_STATUS1, (&mt6331_int_status_val_1), 0xFFFF, 0x0);
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC_INT] mt6331_int_status_val_1=0x%x\n",
		    mt6331_int_status_val_1);

	if ((((mt6331_int_status_val_1) & (0x0001)) >> 0) == 1)
		vdvfs11_oc_int_handler();
	if ((((mt6331_int_status_val_1) & (0x0002)) >> 1) == 1)
		vdvfs12_oc_int_handler();
	if ((((mt6331_int_status_val_1) & (0x0004)) >> 2) == 1)
		vdvfs13_oc_int_handler();
	if ((((mt6331_int_status_val_1) & (0x0008)) >> 3) == 1)
		vdvfs14_oc_int_handler();
	if ((((mt6331_int_status_val_1) & (0x0010)) >> 4) == 1)
		vgpu_oc_int_handler();
	if ((((mt6331_int_status_val_1) & (0x0020)) >> 5) == 1)
		vcore1_oc_int_handler();
	if ((((mt6331_int_status_val_1) & (0x0040)) >> 6) == 1)
		vcore2_oc_int_handler();
	if ((((mt6331_int_status_val_1) & (0x0080)) >> 7) == 1)
		vio18_oc_int_handler();
	if ((((mt6331_int_status_val_1) & (0x0100)) >> 8) == 1)
		mt6331_ldo_oc_int_handler();
}
...
static void mt6332_int_handler(void)
{
	...
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC_INT] mt6332_int_status_val_0=0x%x\n",
		    mt6332_int_status_val_0);

	if ((((mt6332_int_status_val_0) & (0x0001)) >> 0) == 1)
		chr_complete_int_handler();
	if ((((mt6332_int_status_val_0) & (0x0002)) >> 1) == 1)
		thermal_sd_int_handler();
	if ((((mt6332_int_status_val_0) & (0x0004)) >> 2) == 1)
		thermal_reg_in_int_handler();
	if ((((mt6332_int_status_val_0) & (0x0008)) >> 3) == 1)
		thermal_reg_out_int_handler();
	if ((((mt6332_int_status_val_0) & (0x0010)) >> 4) == 1)
		otg_oc_int_handler();
	if ((((mt6332_int_status_val_0) & (0x0020)) >> 5) == 1)
		chr_oc_int_handler();
	if ((((mt6332_int_status_val_0) & (0x0040)) >> 6) == 1)
		otg_thermal_int_handler();
	if ((((mt6332_int_status_val_0) & (0x0080)) >> 7) == 1)
		otg_chrin_short_int_handler();
	if ((((mt6332_int_status_val_0) & (0x0100)) >> 8) == 1)
		otg_drvcdt_short_int_handler();
	if ((((mt6332_int_status_val_0) & (0x0200)) >> 9) == 1)
		chr_plug_in_flash_int_handler();
	if ((((mt6332_int_status_val_0) & (0x0400)) >> 10) == 1)
		chrwdt_flag_int_handler();
	if ((((mt6332_int_status_val_0) & (0x0800)) >> 11) == 1)
		flash_en_timeout_int_handler();
	if ((((mt6332_int_status_val_0) & (0x1000)) >> 12) == 1)
		flash_vled1_short_int_handler();
	if ((((mt6332_int_status_val_0) & (0x2000)) >> 13) == 1)
		flash_vled1_open_int_handler();
	/* -------------------------------------------------------------------------------- */
	ret = pmic_read_interface(MT6332_INT_STATUS1, (&mt6332_int_status_val_1), 0xFFFF, 0x0);
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC_INT] mt6332_int_status_val_1=0x%x\n",
		    mt6332_int_status_val_1);

	if ((((mt6332_int_status_val_1) & (0x0001)) >> 0) == 1)
		ov_int_handler();
	if ((((mt6332_int_status_val_1) & (0x0002)) >> 1) == 1)
		bvalid_det_int_handler();
	if ((((mt6332_int_status_val_1) & (0x0004)) >> 2) == 1)
		vbaton_undet_int_handler();
	if ((((mt6332_int_status_val_1) & (0x0008)) >> 3) == 1)
		chr_plug_in_int_handler();
	if ((((mt6332_int_status_val_1) & (0x0010)) >> 4) == 1)
		chr_plug_out_int_handler();
	if ((((mt6332_int_status_val_1) & (0x0020)) >> 5) == 1)
		bc11_timeout_int_handler();
	if ((((mt6332_int_status_val_1) & (0x0040)) >> 6) == 1)
		flash_vled2_short_int_handler();
	if ((((mt6332_int_status_val_1) & (0x0080)) >> 7) == 1)
		flash_vled2_open_int_handler();
	/* -------------------------------------------------------------------------------- */
	ret = pmic_read_interface(MT6332_INT_STATUS2, (&mt6332_int_status_val_2), 0xFFFF, 0x0);
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC_INT] mt6332_int_status_val_2=0x%x\n",
		    mt6332_int_status_val_2);

	if ((((mt6332_int_status_val_2) & (0x0001)) >> 0) == 1)
		mt6332_thr_h_int_handler();
	if ((((mt6332_int_status_val_2) & (0x0002)) >> 1) == 1)
		mt6332_thr_l_int_handler();
	if ((((mt6332_int_status_val_2) & (0x0004)) >> 2) == 1)
		mt6332_bat_h_int_handler();
	if ((((mt6332_int_status_val_2) & (0x0008)) >> 3) == 1)
		mt6332_bat_l_int_handler();
	if ((((mt6332_int_status_val_2) & (0x0010)) >> 4) == 1)
		fg_bat_h_int_handler();
	if ((((mt6332_int_status_val_2) & (0x0020)) >> 5) == 1)
		fg_bat_l_int_handler();
	if ((((mt6332_int_status_val_2) & (0x0040)) >> 6) == 1)
		spkl_d_int_handler();
	if ((((mt6332_int_status_val_2) & (0x0080)) >> 7) == 1)
		spkl_ab_int_handler();
	if ((((mt6332_int_status_val_2) & (0x0100)) >> 8) == 1)
		bif_int_handler();
	if ((((mt6332_int_status_val_2) & (0x0200)) >> 9) == 1)
		cbus_int_handler();
	if ((((mt6332_int_status_val_2) & (0x0400)) >> 10) == 1)
		vwled_oc_int_handler();
	if ((((mt6332_int_status_val_2) & (0x0800)) >> 11) == 1)
		fg_cur_h_int_handler();
	if ((((mt6332_int_status_val_2) & (0x1000)) >> 12) == 1)
		fg_cur_l_int_handler();
	if ((((mt6332_int_status_val_2) & (0x2000)) >> 13) == 1)
		m3_h_int_handler();
	if ((((mt6332_int_status_val_2) & (0x4000)) >> 14) == 1)
		m3_l_int_handler();
	/* -------------------------------------------------------------------------------- */
	ret = pmic_read_interface(MT6332_INT_STATUS3, (&mt6332_int_status_val_3), 0xFFFF, 0x0);
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC_INT] mt6332_int_status_val_3=0x%x\n",
		    mt6332_int_status_val_3);

	if ((((mt6332_int_status_val_3) & (0x0001)) >> 0) == 1)
		vdram_oc_int_handler();
	if ((((mt6332_int_status_val_3) & (0x0002)) >> 1) == 1)
		vdvfs2_oc_int_handler();
	if ((((mt6332_int_status_val_3) & (0x0004)) >> 2) == 1)
		vrf1_oc_int_handler();
	if ((((mt6332_int_status_val_3) & (0x0008)) >> 3) == 1)
		vrf2_oc_int_handler();
	if ((((mt6332_int_status_val_3) & (0x0010)) >> 4) == 1)
		vpa_oc_int_handler();
	if ((((mt6332_int_status_val_3) & (0x0020)) >> 5) == 1)
		vsbst_oc_int_handler();
	if ((((mt6332_int_status_val_3) & (0x0040)) >> 6) == 1)
		mt6332_ldo_oc_int_handler();
}
...
static int pmic_thread_kthread_mt6331(void *x)
{
	...
	while (1) {
		...
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC31_INT] pwrap_eint_status=0x%x\n",
			    pwrap_eint_status);
		...
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[PMIC31_INT] pmic_wrap_eint_clr(0x0);\n");
		...
		ret =
		    pmic_read_interface(MT6331_INT_STATUS0, (&mt6331_int_status_val_0), 0xFFFF,
					0x0);
		ret =
		    pmic_read_interface(MT6331_INT_STATUS1, (&mt6331_int_status_val_1), 0xFFFF,
					0x0);

		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[PMIC31_INT] after ,mt6331_int_status_val_0=0x%x\n",
			    mt6331_int_status_val_0);
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[PMIC31_INT] after ,mt6331_int_status_val_1=0x%x\n",
			    mt6331_int_status_val_1);
...
static int pmic_thread_kthread_mt6332(void *x)
{
	...
	while (1) {
		...
		pwrap_eint_status = pmic_wrap_eint_status();
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC32_INT] pwrap_eint_status=0x%x\n",
			    pwrap_eint_status);
		...
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[PMIC32_INT] pmic_wrap_eint_clr(2);\n");
		...
		ret =
		    pmic_read_interface(MT6332_INT_STATUS0, (&mt6332_int_status_val_0), 0xFFFF,
					0x0);
		ret =
		    pmic_read_interface(MT6332_INT_STATUS1, (&mt6332_int_status_val_1), 0xFFFF,
					0x0);
		ret =
		    pmic_read_interface(MT6332_INT_STATUS2, (&mt6332_int_status_val_2), 0xFFFF,
					0x0);
		ret =
		    pmic_read_interface(MT6332_INT_STATUS3, (&mt6332_int_status_val_3), 0xFFFF,
					0x0);

		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[PMIC32_INT] after ,mt6332_int_status_val_0=0x%x\n",
			    mt6332_int_status_val_0);
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[PMIC32_INT] after ,mt6332_int_status_val_1=0x%x\n",
			    mt6332_int_status_val_1);
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[PMIC32_INT] after ,mt6332_int_status_val_2=0x%x\n",
			    mt6332_int_status_val_2);
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[PMIC32_INT] after ,mt6332_int_status_val_3=0x%x\n",
			    mt6332_int_status_val_3);
...
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
		    "[mt_pmic_eint_irq_mt6331] receive interrupt\n");
	wake_up_pmic_mt6331();
	return;
}
...
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
		    "[mt_pmic_eint_irq_mt6332] receive interrupt\n");
	wake_up_pmic_mt6332();
	return;
}
...
void PMIC_EINT_SETTING(void)
{
	...
	mt_eint_registration(g_eint_pmit_mt6331_num, g_cust_eint_mt_pmic_type,
			     mt_pmic_eint_irq_mt6331, 0);
	mt_eint_registration(g_eint_pmit_mt6332_num, g_cust_eint_mt_pmic_type,
			     mt_pmic_eint_irq_mt6332, 0);
...
U32 pmic_read_interface_nolock(U32 RegNum, U32 *val, U32 MASK, U32 SHIFT)
{
	...
	if (return_value != 0) {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[pmic_read_interface_nolock] Reg[%x]= pmic_wrap read data fail\n",
			    RegNum);
		return return_value;
	}
...
#else
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
		    "[pmic_read_interface_nolock] Can not access HW PMIC\n");
#endif
...
U32 pmic_config_interface_nolock(U32 RegNum, U32 val, U32 MASK, U32 SHIFT)
{
	...
	if (return_value != 0) {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[pmic_config_interface_nolock] Reg[%x]= pmic_wrap read data fail\n",
			    RegNum);
		return return_value;
	}
...
#else
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
		    "[pmic_config_interface_nolock] Can not access HW PMIC\n");
#endif
...
/* ============================================================================== */
#if 1
static ssize_t show_BUCK_VDVFS11_STATUS(struct device *dev, struct device_attribute *attr,
					char *buf)
{
	kal_uint32 val = 0;
...
static ssize_t store_BUCK_VDVFS11_STATUS(struct device *dev, struct device_attribute *attr,
					 const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t show_BUCK_VDVFS12_STATUS(struct device *dev, struct device_attribute *attr,
					char *buf)
{
	kal_uint32 val = 0;
	kal_uint32 ret_value = 0;
...
static ssize_t store_BUCK_VDVFS12_STATUS(struct device *dev, struct device_attribute *attr,
					 const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t show_BUCK_VDVFS13_STATUS(struct device *dev, struct device_attribute *attr,
					char *buf)
{
	kal_uint32 val = 0;
	kal_uint32 ret_value = 0;
...
static ssize_t store_BUCK_VDVFS13_STATUS(struct device *dev, struct device_attribute *attr,
					 const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t show_BUCK_VDVFS14_STATUS(struct device *dev, struct device_attribute *attr,
					char *buf)
{
	kal_uint32 val = 0;
	kal_uint32 ret_value = 0;
...
static ssize_t store_BUCK_VDVFS14_STATUS(struct device *dev, struct device_attribute *attr,
					 const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_BUCK_VCORE1_STATUS(struct device *dev, struct device_attribute *attr,
					const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_BUCK_VCORE2_STATUS(struct device *dev, struct device_attribute *attr,
					const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_BUCK_VDRAM_STATUS(struct device *dev, struct device_attribute *attr,
				       const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_BUCK_VDVFS2_STATUS(struct device *dev, struct device_attribute *attr,
					const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_BUCK_VRF1_STATUS(struct device *dev, struct device_attribute *attr,
				      const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_BUCK_VRF2_STATUS(struct device *dev, struct device_attribute *attr,
				      const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_BUCK_VPA_STATUS(struct device *dev, struct device_attribute *attr,
				     const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_BUCK_VSBST_STATUS(struct device *dev, struct device_attribute *attr,
				       const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_LDO_VTCXO1_STATUS(struct device *dev, struct device_attribute *attr,
				       const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_LDO_VTCXO2_STATUS(struct device *dev, struct device_attribute *attr,
				       const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_LDO_VAUD32_STATUS(struct device *dev, struct device_attribute *attr,
				       const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_LDO_VAUXA32_STATUS(struct device *dev, struct device_attribute *attr,
					const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_LDO_VEMC33_STATUS(struct device *dev, struct device_attribute *attr,
				       const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_LDO_VCAM_AF_STATUS(struct device *dev, struct device_attribute *attr,
					const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_LDO_VSIM1_STATUS(struct device *dev, struct device_attribute *attr,
				      const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_LDO_VSIM2_STATUS(struct device *dev, struct device_attribute *attr,
				      const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_LDO_VFBB_STATUS(struct device *dev, struct device_attribute *attr,
				     const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_LDO_VMIPI_STATUS(struct device *dev, struct device_attribute *attr,
				      const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t show_LDO_31_VDIG18_STATUS(struct device *dev, struct device_attribute *attr,
					 char *buf)
{
	kal_uint32 ret_value = 0;
...
static ssize_t store_LDO_31_VDIG18_STATUS(struct device *dev, struct device_attribute *attr,
					  const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static DEVICE_ATTR(LDO_31_VDIG18_STATUS, 0664, show_LDO_31_VDIG18_STATUS,
		   store_LDO_31_VDIG18_STATUS);
...
static ssize_t store_LDO_VUSB10_STATUS(struct device *dev, struct device_attribute *attr,
				       const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_LDO_VCAM_IO_STATUS(struct device *dev, struct device_attribute *attr,
					const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t show_LDO_VSRAM_DVFS1_STATUS(struct device *dev, struct device_attribute *attr,
					   char *buf)
{
	kal_uint32 val = 0;
	kal_uint32 ret_value = 0;
...
static ssize_t store_LDO_VSRAM_DVFS1_STATUS(struct device *dev, struct device_attribute *attr,
					    const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static DEVICE_ATTR(LDO_VSRAM_DVFS1_STATUS, 0664, show_LDO_VSRAM_DVFS1_STATUS,
		   store_LDO_VSRAM_DVFS1_STATUS);
...
static ssize_t store_LDO_VBIASN_STATUS(struct device *dev, struct device_attribute *attr,
				       const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_LDO_VBIF28_STATUS(struct device *dev, struct device_attribute *attr,
				       const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_LDO_VAUXB32_STATUS(struct device *dev, struct device_attribute *attr,
					const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_LDO_VUSB33_STATUS(struct device *dev, struct device_attribute *attr,
				       const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t show_LDO_32_VDIG18_STATUS(struct device *dev, struct device_attribute *attr,
					 char *buf)
{
	kal_uint32 ret_value = 0;
...
static ssize_t store_LDO_32_VDIG18_STATUS(struct device *dev, struct device_attribute *attr,
					  const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static DEVICE_ATTR(LDO_32_VDIG18_STATUS, 0664, show_LDO_32_VDIG18_STATUS,
		   store_LDO_32_VDIG18_STATUS);

static ssize_t show_LDO_VSRAM_DVFS2_STATUS(struct device *dev, struct device_attribute *attr,
					   char *buf)
{
	kal_uint32 val = 0;
	kal_uint32 ret_value = 0;
...
static ssize_t store_LDO_VSRAM_DVFS2_STATUS(struct device *dev, struct device_attribute *attr,
					    const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static DEVICE_ATTR(LDO_VSRAM_DVFS2_STATUS, 0664, show_LDO_VSRAM_DVFS2_STATUS,
		   store_LDO_VSRAM_DVFS2_STATUS);
#endif
...
/* voltage */
static ssize_t show_BUCK_VDVFS11_VOLTAGE(struct device *dev, struct device_attribute *attr,
					 char *buf)
{
	kal_uint32 ret_value = 0;
	kal_uint32 reg = 0;
...
static ssize_t store_BUCK_VDVFS11_VOLTAGE(struct device *dev, struct device_attribute *attr,
					  const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static DEVICE_ATTR(BUCK_VDVFS11_VOLTAGE, 0664, show_BUCK_VDVFS11_VOLTAGE,
		   store_BUCK_VDVFS11_VOLTAGE);

static ssize_t show_BUCK_VDVFS12_VOLTAGE(struct device *dev, struct device_attribute *attr,
					 char *buf)
{
	kal_uint32 ret_value = 0;
	kal_uint32 reg = 0;
...
static ssize_t store_BUCK_VDVFS12_VOLTAGE(struct device *dev, struct device_attribute *attr,
					  const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static DEVICE_ATTR(BUCK_VDVFS12_VOLTAGE, 0664, show_BUCK_VDVFS12_VOLTAGE,
		   store_BUCK_VDVFS12_VOLTAGE);

static ssize_t show_BUCK_VDVFS13_VOLTAGE(struct device *dev, struct device_attribute *attr,
					 char *buf)
{
	kal_uint32 ret_value = 0;
	kal_uint32 reg = 0;
...
static ssize_t store_BUCK_VDVFS13_VOLTAGE(struct device *dev, struct device_attribute *attr,
					  const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static DEVICE_ATTR(BUCK_VDVFS13_VOLTAGE, 0664, show_BUCK_VDVFS13_VOLTAGE,
		   store_BUCK_VDVFS13_VOLTAGE);

static ssize_t show_BUCK_VDVFS14_VOLTAGE(struct device *dev, struct device_attribute *attr,
					 char *buf)
{
	kal_uint32 ret_value = 0;
	kal_uint32 reg = 0;
...
static ssize_t store_BUCK_VDVFS14_VOLTAGE(struct device *dev, struct device_attribute *attr,
					  const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static DEVICE_ATTR(BUCK_VDVFS14_VOLTAGE, 0664, show_BUCK_VDVFS14_VOLTAGE,
		   store_BUCK_VDVFS14_VOLTAGE);
...
static ssize_t show_BUCK_VCORE1_VOLTAGE(struct device *dev, struct device_attribute *attr,
					char *buf)
{
	kal_uint32 ret_value = 0;
	kal_uint32 reg = 0;
...
static ssize_t store_BUCK_VCORE1_VOLTAGE(struct device *dev, struct device_attribute *attr,
					 const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t show_BUCK_VCORE2_VOLTAGE(struct device *dev, struct device_attribute *attr,
					char *buf)
{
	kal_uint32 ret_value = 0;
	kal_uint32 reg = 0;
...
static ssize_t store_BUCK_VCORE2_VOLTAGE(struct device *dev, struct device_attribute *attr,
					 const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_BUCK_VDRAM_VOLTAGE(struct device *dev, struct device_attribute *attr,
					const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t show_BUCK_VDVFS2_VOLTAGE(struct device *dev, struct device_attribute *attr,
					char *buf)
{
	kal_uint32 ret_value = 0;
	kal_uint32 reg = 0;
...
static ssize_t store_BUCK_VDVFS2_VOLTAGE(struct device *dev, struct device_attribute *attr,
					 const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_BUCK_VRF1_VOLTAGE(struct device *dev, struct device_attribute *attr,
				       const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_BUCK_VRF2_VOLTAGE(struct device *dev, struct device_attribute *attr,
				       const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_BUCK_VPA_VOLTAGE(struct device *dev, struct device_attribute *attr,
				      const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_BUCK_VSBST_VOLTAGE(struct device *dev, struct device_attribute *attr,
					const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_LDO_VTCXO1_VOLTAGE(struct device *dev, struct device_attribute *attr,
					const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_LDO_VTCXO2_VOLTAGE(struct device *dev, struct device_attribute *attr,
					const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_LDO_VAUD32_VOLTAGE(struct device *dev, struct device_attribute *attr,
					const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t show_LDO_VAUXA32_VOLTAGE(struct device *dev, struct device_attribute *attr,
					char *buf)
{
	kal_uint32 ret_value = 0;
	kal_uint32 ret = 0;
...
static ssize_t store_LDO_VAUXA32_VOLTAGE(struct device *dev, struct device_attribute *attr,
					 const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_LDO_VEMC33_VOLTAGE(struct device *dev, struct device_attribute *attr,
					const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t show_LDO_VCAM_AF_VOLTAGE(struct device *dev, struct device_attribute *attr,
					char *buf)
{
	kal_uint32 ret_value = 0;
	kal_uint32 ret = 0;
...
static ssize_t store_LDO_VCAM_AF_VOLTAGE(struct device *dev, struct device_attribute *attr,
					 const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_LDO_VGP1_VOLTAGE(struct device *dev, struct device_attribute *attr,
				      const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_LDO_VSIM1_VOLTAGE(struct device *dev, struct device_attribute *attr,
				       const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_LDO_VSIM2_VOLTAGE(struct device *dev, struct device_attribute *attr,
				       const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_LDO_VFBB_VOLTAGE(struct device *dev, struct device_attribute *attr,
				      const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_LDO_VMIPI_VOLTAGE(struct device *dev, struct device_attribute *attr,
				       const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t show_LDO_31_VDIG18_VOLTAGE(struct device *dev, struct device_attribute *attr,
					  char *buf)
{
	kal_uint32 ret_value = 0;
	kal_uint32 ret = 0;
...
static ssize_t store_LDO_31_VDIG18_VOLTAGE(struct device *dev, struct device_attribute *attr,
					   const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static DEVICE_ATTR(LDO_31_VDIG18_VOLTAGE, 0664, show_LDO_31_VDIG18_VOLTAGE,
		   store_LDO_31_VDIG18_VOLTAGE);
...
static ssize_t store_LDO_VUSB10_VOLTAGE(struct device *dev, struct device_attribute *attr,
					const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t show_LDO_VCAM_IO_VOLTAGE(struct device *dev, struct device_attribute *attr,
					char *buf)
{
	kal_uint32 ret_value = 0;
	kal_uint32 ret = 0;
...
static ssize_t store_LDO_VCAM_IO_VOLTAGE(struct device *dev, struct device_attribute *attr,
					 const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t show_LDO_VSRAM_DVFS1_VOLTAGE(struct device *dev, struct device_attribute *attr,
					    char *buf)
{
	kal_uint32 ret_value = 0;
	kal_uint32 ret = 0;
...
static ssize_t store_LDO_VSRAM_DVFS1_VOLTAGE(struct device *dev, struct device_attribute *attr,
					     const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static DEVICE_ATTR(LDO_VSRAM_DVFS1_VOLTAGE, 0664, show_LDO_VSRAM_DVFS1_VOLTAGE,
		   store_LDO_VSRAM_DVFS1_VOLTAGE);
...
static ssize_t store_LDO_VGP2_VOLTAGE(struct device *dev, struct device_attribute *attr,
				      const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_LDO_VGP3_VOLTAGE(struct device *dev, struct device_attribute *attr,
				      const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_LDO_VBIASN_VOLTAGE(struct device *dev, struct device_attribute *attr,
					const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_LDO_VBIF28_VOLTAGE(struct device *dev, struct device_attribute *attr,
					const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t show_LDO_VAUXB32_VOLTAGE(struct device *dev, struct device_attribute *attr,
					char *buf)
{
	kal_uint32 ret_value = 0;
	kal_uint32 ret = 0;
...
static ssize_t store_LDO_VAUXB32_VOLTAGE(struct device *dev, struct device_attribute *attr,
					 const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t store_LDO_VUSB33_VOLTAGE(struct device *dev, struct device_attribute *attr,
					const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static ssize_t show_LDO_32_VDIG18_VOLTAGE(struct device *dev, struct device_attribute *attr,
					  char *buf)
{
	kal_uint32 ret_value = 0;
	kal_uint32 ret = 0;
...
static ssize_t store_LDO_32_VDIG18_VOLTAGE(struct device *dev, struct device_attribute *attr,
					   const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static DEVICE_ATTR(LDO_32_VDIG18_VOLTAGE, 0664, show_LDO_32_VDIG18_VOLTAGE,
		   store_LDO_32_VDIG18_VOLTAGE);

static ssize_t show_LDO_VSRAM_DVFS2_VOLTAGE(struct device *dev, struct device_attribute *attr,
					    char *buf)
{
	kal_uint32 ret_value = 0;
	kal_uint32 ret = 0;
...
static ssize_t store_LDO_VSRAM_DVFS2_VOLTAGE(struct device *dev, struct device_attribute *attr,
					     const char *buf, size_t size)
{
	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[EM] Not Support Write Function\n");
	return size;
...
static DEVICE_ATTR(LDO_VSRAM_DVFS2_VOLTAGE, 0664, show_LDO_VSRAM_DVFS2_VOLTAGE,
		   store_LDO_VSRAM_DVFS2_VOLTAGE);
...
static void dct_pmic_VTCXO1_enable(kal_bool dctEnable)
{
...
static void dct_pmic_VTCXO2_enable(kal_bool dctEnable)
{
...
static void dct_pmic_VAUD32_enable(kal_bool dctEnable)
{
...
static void dct_pmic_VAUXA32_enable(kal_bool dctEnable)
{
...
static void dct_pmic_VEMC33_enable(kal_bool dctEnable)
{
...
static void dct_pmic_VCAM_AF_enable(kal_bool dctEnable)
{
...
static void dct_pmic_VSIM1_enable(kal_bool dctEnable)
{
...
static void dct_pmic_VSIM2_enable(kal_bool dctEnable)
{
...
static void dct_pmic_VFBB_enable(kal_bool dctEnable)
{
...
static void dct_pmic_VMIPI_enable(kal_bool dctEnable)
{
...
static void dct_pmic_MT6331_VDIG18_enable(kal_bool dctEnable)
{
...
static void dct_pmic_VUSB10_enable(kal_bool dctEnable)
{
...
static void dct_pmic_VCAM_IO_enable(kal_bool dctEnable)
{
...
static void dct_pmic_VSRAM_DVFS1_enable(kal_bool dctEnable)
{
...
static void dct_pmic_VBIASN_enable(kal_bool dctEnable)
{
...
static void dct_pmic_VBIF28_enable(kal_bool dctEnable)
{
...
static void dct_pmic_VAUXB32_enable(kal_bool dctEnable)
{
...
static void dct_pmic_VUSB33_enable(kal_bool dctEnable)
{
...
static void dct_pmic_MT6332_VDIG18_enable(kal_bool dctEnable)
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[dct_pmic_MT6332_VDIG18_enable] no en bit\n");
}

static void dct_pmic_VSRAM_DVFS2_enable(kal_bool dctEnable)
{
...
static void dct_pmic_VTCXO1_sel(kal_uint32 volt)
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "****[dct_pmic_VTCXO1_sel] value=%d\n", volt);

	if (volt == VOL_DEFAULT)
		mt6331_upmu_set_rg_vtcxo1_vosel(3);
	else if (volt == 2800)
		mt6331_upmu_set_rg_vtcxo1_vosel(3);
	else {
...
static void dct_pmic_VTCXO2_sel(kal_uint32 volt)
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "****[dct_pmic_VTCXO2_sel] value=%d\n", volt);

	if (volt == VOL_DEFAULT)
		mt6331_upmu_set_rg_vtcxo2_vosel(3);
	else if (volt == 2800)
		mt6331_upmu_set_rg_vtcxo2_vosel(3);
	else {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "Error Setting %d. DO nothing.\r\n",
...
static void dct_pmic_VAUD32_sel(kal_uint32 volt)
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "****[dct_pmic_VAUD32_sel] value=%d\n", volt);

	if (volt == VOL_DEFAULT)
		mt6331_upmu_set_rg_vaud32_vosel(3);
	else if (volt == 2800)
		mt6331_upmu_set_rg_vaud32_vosel(0);
	else if (volt == 3000)
		mt6331_upmu_set_rg_vaud32_vosel(1);
	else if (volt == 3200)
		mt6331_upmu_set_rg_vaud32_vosel(3);
	else {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "Error Setting %d. DO nothing.\r\n",
...
static void dct_pmic_VAUXA32_sel(kal_uint32 volt)
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "****[dct_pmic_VAUXA32_sel] value=%d\n", volt);

	if (volt == VOL_DEFAULT)
		mt6331_upmu_set_rg_vauxa32_vosel(3);
	else if (volt == 2800)
		mt6331_upmu_set_rg_vauxa32_vosel(0);
	else if (volt == 3000)
		mt6331_upmu_set_rg_vauxa32_vosel(1);
	else if (volt == 3200)
		mt6331_upmu_set_rg_vauxa32_vosel(3);
	else {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "Error Setting %d. DO nothing.\r\n",
...
static void dct_pmic_VCAMA_sel(kal_uint32 volt)
{
	...
	if (volt == VOL_DEFAULT)
		mt6331_upmu_set_rg_vcama_vosel(3);
	else if (volt == 1500)
		mt6331_upmu_set_rg_vcama_vosel(0);
	else if (volt == 1800)
		mt6331_upmu_set_rg_vcama_vosel(1);
	else if (volt == 2500)
		mt6331_upmu_set_rg_vcama_vosel(2);
	else if (volt == 2800)
		mt6331_upmu_set_rg_vcama_vosel(3);
	else {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "Error Setting %d. DO nothing.\r\n",
...
static void dct_pmic_VMCH_sel(kal_uint32 volt)
{
	...
	if (volt == VOL_DEFAULT)
		...
		mt6331_upmu_set_rg_vmch_vosel(0);
	else if (volt == 3300)
		mt6331_upmu_set_rg_vmch_vosel(1);
	else {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "Error Setting %d. DO nothing.\r\n",
...
static void dct_pmic_VEMC33_sel(kal_uint32 volt)
{
	...
	if (volt == VOL_DEFAULT)
		...
		mt6331_upmu_set_rg_vemc33_vosel(0);
	else if (volt == 3300)
		mt6331_upmu_set_rg_vemc33_vosel(1);
	else {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "Error Setting %d. DO nothing.\r\n",
...
static void dct_pmic_VIO28_sel(kal_uint32 volt)
{
	...
	if (volt == VOL_DEFAULT)
		mt6331_upmu_set_rg_vio28_vosel(0);
	else if (volt == 2800)
		mt6331_upmu_set_rg_vio28_vosel(0);
	else {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "Error Setting %d. DO nothing.\r\n",
...
static void dct_pmic_VCAM_AF_sel(kal_uint32 volt)
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "****[dct_pmic_VCAM_AF_sel] value=%d\n", volt);

	if (volt == VOL_DEFAULT)
		mt6331_upmu_set_rg_vcam_af_vosel(5);
	else if (volt == 1200)
		mt6331_upmu_set_rg_vcam_af_vosel(0);
	else if (volt == 1300)
		mt6331_upmu_set_rg_vcam_af_vosel(1);
	else if (volt == 1500)
		mt6331_upmu_set_rg_vcam_af_vosel(2);
	else if (volt == 1800)
		mt6331_upmu_set_rg_vcam_af_vosel(3);
	else if (volt == 2000)
		mt6331_upmu_set_rg_vcam_af_vosel(4);
	else if (volt == 2800)
		mt6331_upmu_set_rg_vcam_af_vosel(5);
	else if (volt == 3000)
		mt6331_upmu_set_rg_vcam_af_vosel(6);
	else if (volt == 3300)
		mt6331_upmu_set_rg_vcam_af_vosel(7);
	else {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "Error Setting %d. DO nothing.\r\n",
			    volt);
	}
...
static void dct_pmic_VGP1_sel(kal_uint32 volt)
{
	...
	if (volt == VOL_DEFAULT)
		mt6331_upmu_set_rg_vgp1_vosel(5);
	else if (volt == 1200)
		mt6331_upmu_set_rg_vgp1_vosel(0);
	else if (volt == 1300)
		mt6331_upmu_set_rg_vgp1_vosel(1);
	else if (volt == 1500)
		mt6331_upmu_set_rg_vgp1_vosel(2);
	else if (volt == 1800)
		mt6331_upmu_set_rg_vgp1_vosel(3);
	else if (volt == 2000)
		mt6331_upmu_set_rg_vgp1_vosel(4);
	else if (volt == 2800)
		mt6331_upmu_set_rg_vgp1_vosel(5);
	else if (volt == 3000)
		mt6331_upmu_set_rg_vgp1_vosel(6);
	else if (volt == 3300)
		mt6331_upmu_set_rg_vgp1_vosel(7);
	else {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "Error Setting %d. DO nothing.\r\n",
			    volt);
	}
...
static void dct_pmic_VGP4_sel(kal_uint32 volt)
{
	...
	if (get_pmic_mt6331_cid() == PMIC6331_E1_CID_CODE) {
		...
		if (volt == VOL_DEFAULT)
			mt6331_upmu_set_rg_vgp4_vosel(5);
		else if (volt == 1200)
			mt6331_upmu_set_rg_vgp4_vosel(0);
		else if (volt == 1300)
			mt6331_upmu_set_rg_vgp4_vosel(1);
		else if (volt == 1500)
			mt6331_upmu_set_rg_vgp4_vosel(2);
		else if (volt == 1800)
			mt6331_upmu_set_rg_vgp4_vosel(3);
		else if (volt == 2000)
			mt6331_upmu_set_rg_vgp4_vosel(4);
		else if (volt == 2800)
			mt6331_upmu_set_rg_vgp4_vosel(5);
		else if (volt == 3000)
			mt6331_upmu_set_rg_vgp4_vosel(6);
		else if (volt == 3300)
			mt6331_upmu_set_rg_vgp4_vosel(7);
		else
			xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
				    "Error Setting %d. DO nothing.\r\n", volt);
	} else {
...
static void dct_pmic_VSIM1_sel(kal_uint32 volt)
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "****[dct_pmic_VSIM1_sel] value=%d\n", volt);

	if (get_pmic_mt6331_cid() == PMIC6331_E1_CID_CODE) {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "****[dct_pmic_VSIM1_sel] MT6331==E1\n");

		if (volt == VOL_DEFAULT)
			mt6331_upmu_set_rg_vsim1_vosel(0);
		else if (volt == 1800)
			mt6331_upmu_set_rg_vsim1_vosel(0);
		else if (volt == 3000)
			mt6331_upmu_set_rg_vsim1_vosel(1);
		else
			xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
				    "Error Setting %d. DO nothing.\r\n", volt);
	} else {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "****[dct_pmic_VSIM1_sel] MT6331>=E2\n");

		if (volt == VOL_DEFAULT)
			mt6331_upmu_set_rg_vsim1_vosel(0);
		else if (volt == 1700)
			mt6331_upmu_set_rg_vsim1_vosel(2);
		else if (volt == 1800)
			mt6331_upmu_set_rg_vsim1_vosel(3);
		else if (volt == 1860)
			mt6331_upmu_set_rg_vsim1_vosel(4);
		else if (volt == 2760)
			mt6331_upmu_set_rg_vsim1_vosel(5);
		else if (volt == 3000)
			mt6331_upmu_set_rg_vsim1_vosel(6);
		else if (volt == 3100)
			mt6331_upmu_set_rg_vsim1_vosel(7);
		else
			xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
				    "Error Setting %d. DO nothing.\r\n", volt);
	}
...
static void dct_pmic_VSIM2_sel(kal_uint32 volt)
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "****[dct_pmic_VSIM2_sel] value=%d\n", volt);

	if (get_pmic_mt6331_cid() == PMIC6331_E1_CID_CODE) {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "****[dct_pmic_VSIM2_sel] MT6331==E1\n");

		if (volt == VOL_DEFAULT)
			mt6331_upmu_set_rg_vsim2_vosel(0);
		else if (volt == 1800)
			mt6331_upmu_set_rg_vsim2_vosel(0);
		else if (volt == 3000)
			mt6331_upmu_set_rg_vsim2_vosel(1);
		else
			xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
				    "Error Setting %d. DO nothing.\r\n", volt);
	} else {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "****[dct_pmic_VSIM2_sel] MT6331>=E2\n");

		if (volt == VOL_DEFAULT)
			mt6331_upmu_set_rg_vsim2_vosel(0);
		else if (volt == 1700)
			mt6331_upmu_set_rg_vsim2_vosel(2);
		else if (volt == 1800)
			mt6331_upmu_set_rg_vsim2_vosel(3);
		else if (volt == 1860)
			mt6331_upmu_set_rg_vsim2_vosel(4);
		else if (volt == 2760)
			mt6331_upmu_set_rg_vsim2_vosel(5);
		else if (volt == 3000)
			mt6331_upmu_set_rg_vsim2_vosel(6);
		else if (volt == 3100)
			mt6331_upmu_set_rg_vsim2_vosel(7);
		else
			xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
				    "Error Setting %d. DO nothing.\r\n", volt);
	}
...
static void dct_pmic_VFBB_sel(kal_uint32 volt)
{
...
static void dct_pmic_VMIPI_sel(kal_uint32 volt)
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "****[dct_pmic_VMIPI_sel] value=%d\n", volt);

	if (volt == VOL_DEFAULT)
		mt6331_upmu_set_rg_vmipi_vosel(3);
	else if (volt == 1200)
		mt6331_upmu_set_rg_vmipi_vosel(0);
	else if (volt == 1300)
		mt6331_upmu_set_rg_vmipi_vosel(1);
	else if (volt == 1500)
		mt6331_upmu_set_rg_vmipi_vosel(2);
	else if (volt == 1800)
		mt6331_upmu_set_rg_vmipi_vosel(3);
	else if (volt == 2000)
		mt6331_upmu_set_rg_vmipi_vosel(4);
	else if (volt == 2800)
		mt6331_upmu_set_rg_vmipi_vosel(5);
	else if (volt == 3000)
		mt6331_upmu_set_rg_vmipi_vosel(6);
	else if (volt == 3300)
		mt6331_upmu_set_rg_vmipi_vosel(7);
	else {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "Error Setting %d. DO nothing.\r\n",
			    volt);
	}
...
static void dct_pmic_VIBR_sel(kal_uint32 volt)
{
	...
	if (volt == VOL_DEFAULT)
		mt6331_upmu_set_rg_vibr_vosel(5);
	else if (volt == 1200)
		mt6331_upmu_set_rg_vibr_vosel(0);
	else if (volt == 1300)
		mt6331_upmu_set_rg_vibr_vosel(1);
	else if (volt == 1500)
		mt6331_upmu_set_rg_vibr_vosel(2);
	else if (volt == 1800)
		mt6331_upmu_set_rg_vibr_vosel(3);
	else if (volt == 2000)
		mt6331_upmu_set_rg_vibr_vosel(4);
	else if (volt == 2800)
		mt6331_upmu_set_rg_vibr_vosel(5);
	else if (volt == 3000)
		mt6331_upmu_set_rg_vibr_vosel(6);
	else if (volt == 3300)
		mt6331_upmu_set_rg_vibr_vosel(7);
	else {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "Error Setting %d. DO nothing.\r\n",
			    volt);
	}
...
static void dct_pmic_MT6331_VDIG18_sel(kal_uint32 volt)
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "****[dct_pmic_MT6331_VDIG18_sel] value=%d\n",
		    volt);

	if (volt == VOL_DEFAULT)
		mt6331_upmu_set_rg_vdig18_vosel(6);
	else if (volt == 1200)
		mt6331_upmu_set_rg_vdig18_vosel(0);
	else if (volt == 1300)
		mt6331_upmu_set_rg_vdig18_vosel(1);
	else if (volt == 1400)
		mt6331_upmu_set_rg_vdig18_vosel(2);
	else if (volt == 1500)
		mt6331_upmu_set_rg_vdig18_vosel(3);
	else if (volt == 1600)
		mt6331_upmu_set_rg_vdig18_vosel(4);
	else if (volt == 1700)
		mt6331_upmu_set_rg_vdig18_vosel(5);
	else if (volt == 1800)
		mt6331_upmu_set_rg_vdig18_vosel(6);
	else {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "Error Setting %d. DO nothing.\r\n",
...
static void dct_pmic_VCAMD_sel(kal_uint32 volt)
{
	...
	if (volt == VOL_DEFAULT)
		mt6331_upmu_set_rg_vcamd_vosel(4);
	else if (volt == 900)
		mt6331_upmu_set_rg_vcamd_vosel(0);
	else if (volt == 1000)
		mt6331_upmu_set_rg_vcamd_vosel(1);
	else if (volt == 1100)
		mt6331_upmu_set_rg_vcamd_vosel(2);
	else if (volt == 1220)
		mt6331_upmu_set_rg_vcamd_vosel(3);
	else if (volt == 1300)
		mt6331_upmu_set_rg_vcamd_vosel(4);
	else if (volt == 1500)
		mt6331_upmu_set_rg_vcamd_vosel(5);
	else {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "Error Setting %d. DO nothing.\r\n",
			    volt);
	}
...
static void dct_pmic_VUSB10_sel(kal_uint32 volt)
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "****[dct_pmic_VUSB10_sel] value=%d\n", volt);

	if (volt == VOL_DEFAULT)
		mt6331_upmu_set_rg_vusb10_vosel(0);
	else if (volt == 1000)
		mt6331_upmu_set_rg_vusb10_vosel(0);
	else if (volt == 1050)
		mt6331_upmu_set_rg_vusb10_vosel(1);
	else if (volt == 1100)
		mt6331_upmu_set_rg_vusb10_vosel(2);
	else if (volt == 1150)
		mt6331_upmu_set_rg_vusb10_vosel(3);
	else if (volt == 1200)
		mt6331_upmu_set_rg_vusb10_vosel(4);
	else if (volt == 1250)
		mt6331_upmu_set_rg_vusb10_vosel(5);
	else if (volt == 1300)
		mt6331_upmu_set_rg_vusb10_vosel(6);
	else {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "Error Setting %d. DO nothing.\r\n",
			    volt);
	}
...
static void dct_pmic_VCAM_IO_sel(kal_uint32 volt)
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "****[dct_pmic_VCAM_IO_sel] value=%d\n", volt);

	if (volt == VOL_DEFAULT)
		mt6331_upmu_set_rg_vcam_io_vosel(3);
	else if (volt == 1200)
		mt6331_upmu_set_rg_vcam_io_vosel(0);
	else if (volt == 1300)
		mt6331_upmu_set_rg_vcam_io_vosel(1);
	else if (volt == 1500)
		mt6331_upmu_set_rg_vcam_io_vosel(2);
	else if (volt == 1800)
		mt6331_upmu_set_rg_vcam_io_vosel(3);
	else {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "Error Setting %d. DO nothing.\r\n",
...
static void dct_pmic_VSRAM_DVFS1_sel(kal_uint32 volt)
{
	int val = 0;
	...
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
		    "****[dct_pmic_VSRAM_DVFS1_sel] value=%d, val=%d\n", volt, val);
}
...
static void dct_pmic_VGP2_sel(kal_uint32 volt)
{
	...
	if (get_pmic_mt6331_cid() == PMIC6331_E1_CID_CODE) {
		...
		if (volt == VOL_DEFAULT)
			mt6331_upmu_set_rg_vgp2_vosel(3);
		else if (volt == 1200)
			mt6331_upmu_set_rg_vgp2_vosel(0);
		else if (volt == 1300)
			mt6331_upmu_set_rg_vgp2_vosel(1);
		else if (volt == 1500)
			mt6331_upmu_set_rg_vgp2_vosel(2);
		else if (volt == 1800)
			mt6331_upmu_set_rg_vgp2_vosel(3);
		else
			xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
...
static void dct_pmic_VGP3_sel(kal_uint32 volt)
{
	...
	if (volt == VOL_DEFAULT) {
		...
		mt6331_upmu_set_rg_vgp3_vosel(0);
	} else if (volt == 1300) {
		mt6331_upmu_set_rg_vgp3_vosel(1);
	} else if (volt == 1500) {
		mt6331_upmu_set_rg_vgp3_vosel(2);
	} else if (volt == 1800) {
		mt6331_upmu_set_rg_vgp3_vosel(3);
	} else {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "Error Setting %d. DO nothing.\r\n",
...
static void dct_pmic_VBIASN_sel(kal_uint32 volt)
{
	int val = 0;
...
static void dct_pmic_VBIF28_sel(kal_uint32 volt)
{
...
static void dct_pmic_VAUXB32_sel(kal_uint32 volt)
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "****[dct_pmic_VAUXB32_sel] value=%d\n", volt);

	if (volt == VOL_DEFAULT) {
		mt6332_upmu_set_rg_vauxb32_vosel(3);
	} else if (volt == 2800) {
		mt6332_upmu_set_rg_vauxb32_vosel(0);
	} else if (volt == 3000) {
		mt6332_upmu_set_rg_vauxb32_vosel(1);
	} else if (volt == 3200) {
		mt6332_upmu_set_rg_vauxb32_vosel(3);
	} else {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "Error Setting %d. DO nothing.\r\n",
...
static void dct_pmic_VUSB33_sel(kal_uint32 volt)
{
...
static void dct_pmic_MT6332_VDIG18_sel(kal_uint32 volt)
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "****[dct_pmic_MT6332_VDIG18_sel] value=%d\n",
		    volt);

	if (volt == VOL_DEFAULT) {
		mt6332_upmu_set_rg_vdig18_vosel(6);
	} else if (volt == 1200) {
		mt6332_upmu_set_rg_vdig18_vosel(0);
	} else if (volt == 1300) {
		mt6332_upmu_set_rg_vdig18_vosel(1);
	} else if (volt == 1400) {
		mt6332_upmu_set_rg_vdig18_vosel(2);
	} else if (volt == 1500) {
		mt6332_upmu_set_rg_vdig18_vosel(3);
	} else if (volt == 1600) {
		mt6332_upmu_set_rg_vdig18_vosel(4);
	} else if (volt == 1700) {
		mt6332_upmu_set_rg_vdig18_vosel(5);
	} else if (volt == 1800) {
		mt6332_upmu_set_rg_vdig18_vosel(6);
	} else {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "Error Setting %d. DO nothing.\r\n",
...
static void dct_pmic_VSRAM_DVFS2_sel(kal_uint32 volt)
{
	int val = 0;
	...
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
		    "****[dct_pmic_VSRAM_DVFS2_sel] value=%d, val=%d\n", volt, val);
}
#endif
...
struct mt_ldo_ops ldo_ops[] = {
	[MT6331_POWER_LDO_VTCXO1] = {
				     .vosel = dct_pmic_VTCXO1_sel,
				     .vocal = NULL,
				     .enable = dct_pmic_VTCXO1_enable,
				     },
	[MT6331_POWER_LDO_VTCXO2] = {
				     .vosel = dct_pmic_VTCXO2_sel,
				     .vocal = NULL,
				     .enable = dct_pmic_VTCXO2_enable,
				     },
	[MT6331_POWER_LDO_VAUD32] = {
				     .vosel = dct_pmic_VAUD32_sel,
				     .vocal = NULL,
				     .enable = dct_pmic_VAUD32_enable,
				     },
	[MT6331_POWER_LDO_VAUXA32] = {
				      .vosel = dct_pmic_VAUXA32_sel,
				      .vocal = NULL,
				      .enable = dct_pmic_VAUXA32_enable,
				      },
	...
	[MT6331_POWER_LDO_VEMC33] = {
				     .vosel = dct_pmic_VEMC33_sel,
				     .vocal = NULL,
				     .enable = dct_pmic_VEMC33_enable,
				     },
	...
	[MT6331_POWER_LDO_VCAM_AF] = {
				      .vosel = dct_pmic_VCAM_AF_sel,
				      .vocal = NULL,
				      .enable = dct_pmic_VCAM_AF_enable,
				      },
	...
	[MT6331_POWER_LDO_VSIM1] = {
				    .vosel = dct_pmic_VSIM1_sel,
				    .vocal = NULL,
				    .enable = dct_pmic_VSIM1_enable,
				    },
	[MT6331_POWER_LDO_VSIM2] = {
				    .vosel = dct_pmic_VSIM2_sel,
				    .vocal = NULL,
				    .enable = dct_pmic_VSIM2_enable,
				    },
	[MT6331_POWER_LDO_VFBB] = {
				   .vosel = dct_pmic_VFBB_sel,
				   .vocal = NULL,
				   .enable = dct_pmic_VFBB_enable,
				   },
	...
	[MT6331_POWER_LDO_VMIPI] = {
				    .vosel = dct_pmic_VMIPI_sel,
				    .vocal = NULL,
				    .enable = dct_pmic_VMIPI_enable,
				    },
	...
	[MT6331_POWER_LDO_VDIG18] = {
				     .vosel = dct_pmic_MT6331_VDIG18_sel,
				     .vocal = NULL,
				     .enable = dct_pmic_MT6331_VDIG18_enable,
				     },
	...
	[MT6331_POWER_LDO_VUSB10] = {
				     .vosel = dct_pmic_VUSB10_sel,
				     .vocal = NULL,
				     .enable = dct_pmic_VUSB10_enable,
				     },
	[MT6331_POWER_LDO_VCAM_IO] = {
				      .vosel = dct_pmic_VCAM_IO_sel,
				      .vocal = NULL,
				      .enable = dct_pmic_VCAM_IO_enable,
				      },
	[MT6331_POWER_LDO_VSRAM_DVFS1] = {
					  .vosel = dct_pmic_VSRAM_DVFS1_sel,
					  .vocal = NULL,
					  .enable = dct_pmic_VSRAM_DVFS1_enable,
					  },
	...
	[MT6331_POWER_LDO_VBIASN] = {
				     .vosel = dct_pmic_VBIASN_sel,
				     .vocal = NULL,
				     .enable = dct_pmic_VBIASN_enable,
				     },
	[MT6332_POWER_LDO_VBIF28] = {
				     .vosel = dct_pmic_VBIF28_sel,
				     .vocal = NULL,
				     .enable = dct_pmic_VBIF28_enable,
				     },
	[MT6332_POWER_LDO_VAUXB32] = {
				      .vosel = dct_pmic_VAUXB32_sel,
				      .vocal = NULL,
				      .enable = dct_pmic_VAUXB32_enable,
				      },
	[MT6332_POWER_LDO_VUSB33] = {
				     .vosel = dct_pmic_VUSB33_sel,
				     .vocal = NULL,
				     .enable = dct_pmic_VUSB33_enable,
				     },
	[MT6332_POWER_LDO_VDIG18] = {
				     .vosel = dct_pmic_MT6332_VDIG18_sel,
				     .vocal = NULL,
				     .enable = dct_pmic_MT6332_VDIG18_enable,
				     },
	[MT6332_POWER_LDO_VSRAM_DVFS2] = {
					  .vosel = dct_pmic_VSRAM_DVFS2_sel,
					  .vocal = NULL,
					  .enable = dct_pmic_VSRAM_DVFS2_enable,
					  },
};
...
void dump_ldo_status_read_debug(void)
{
	...
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
		    "********** BUCK/LDO status dump [1:ON,0:OFF]**********\n");

	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "DVFS11	  =%d, ",
		    (((val_0) & (0x0001)) >> 0));
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "DVFS12	  =%d, ",
		    (((val_0) & (0x0002)) >> 1));
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "DVFS13	  =%d, ",
		    (((val_0) & (0x0004)) >> 2));
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "DVFS14	  =%d\n",
		    (((val_0) & (0x0008)) >> 3));

	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VGPU		=%d, ",
		    (((val_0) & (0x0010)) >> 4));
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VCORE1	  =%d, ",
		    (((val_0) & (0x0020)) >> 5));
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VCORE2	  =%d, ",
		    (((val_0) & (0x0040)) >> 6));
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VIO18	   =%d\n",
		    (((val_0) & (0x0080)) >> 7));

	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "NA		  =%d, ",
		    (((val_0) & (0x0100)) >> 8));
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VRTC		=%d, ",
		    (((val_0) & (0x0200)) >> 9));
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VTCXO1	  =%d, ",
		    (((val_0) & (0x0400)) >> 10));
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VTCXO2	  =%d\n",
		    (((val_0) & (0x0800)) >> 11));

	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VAUD32	  =%d, ",
		    (((val_0) & (0x1000)) >> 12));
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VAUXA32	 =%d, ",
		    (((val_0) & (0x2000)) >> 13));
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VCAMA	   =%d, ",
		    (((val_0) & (0x4000)) >> 14));
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VIO28	   =%d\n",
		    (((val_0) & (0x8000)) >> 15));
	/* ------------------------------------------------------------------ */
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VCAM_AF	 =%d, ",
		    (((val_1) & (0x0001)) >> 0));
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VMC		 =%d, ",
		    (((val_1) & (0x0002)) >> 1));
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VMCH		=%d, ",
		    (((val_1) & (0x0004)) >> 2));
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VEMC33	  =%d\n",
		    (((val_1) & (0x0008)) >> 3));

	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VGP1		=%d, ",
		    (((val_1) & (0x0010)) >> 4));
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VGP4		=%d, ",
		    (((val_1) & (0x0020)) >> 5));
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VSIM1	   =%d, ",
		    (((val_1) & (0x0040)) >> 6));
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VSIM2	   =%d\n",
		    (((val_1) & (0x0080)) >> 7));

	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VFBB		=%d, ",
		    (((val_1) & (0x0100)) >> 8));
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VMIPI	   =%d, ",
		    (((val_1) & (0x0200)) >> 9));
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VIBR		=%d, ",
		    (((val_1) & (0x0400)) >> 10));
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VCAMD	   =%d\n",
		    (((val_1) & (0x0800)) >> 11));

	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VUSB10	  =%d, ",
		    (((val_1) & (0x1000)) >> 12));
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VCAM_IO	 =%d, ",
		    (((val_1) & (0x2000)) >> 13));
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VSRAM_DVFS1 =%d, ",
		    (((val_1) & (0x4000)) >> 14));
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VGP2		=%d\n",
		    (((val_1) & (0x8000)) >> 15));
	/* ------------------------------------------------------------------ */
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VGP3		=%d, ",
		    (((val_2) & (0x0001)) >> 0));
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VBIASN	  =%d\n",
		    (((val_2) & (0x0002)) >> 1));
	/* ------------------------------------------------------------------ */
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VAUXB32	 =%d, ",
		    (((val_3) & (0x0001)) >> 0));
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VBIF28	  =%d, ",
		    (((val_3) & (0x0002)) >> 1));
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VUSB33	  =%d, ",
		    (((val_3) & (0x0004)) >> 2));
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VSRAM_DVFS2 =%d\n",
		    (((val_3) & (0x0008)) >> 3));

	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VDRAM	   =%d, ",
		    (((val_3) & (0x0040)) >> 6));
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VDVFS2	  =%d\n",
		    (((val_3) & (0x0080)) >> 7));

	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VRF1		=%d, ",
		    (((val_3) & (0x0100)) >> 8));
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VRF2		=%d, ",
		    (((val_3) & (0x0200)) >> 9));
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VPA		 =%d, ",
		    (((val_3) & (0x0400)) >> 10));
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "VSBST	   =%d\n",
		    (((val_3) & (0x0800)) >> 11));
}
...
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
		    "[show_battery_oc_protect_ut] g_battery_oc_level=%d\n", g_battery_oc_level);
	return sprintf(buf, "%u\n", g_battery_oc_level);
}
					   ...
					   const char *buf, size_t size)
{
	...
	if (buf != NULL && size != 0) {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[store_battery_oc_protect_ut] buf is %s and size is %d\n", buf, size);
		val = kstrtoul(buf, &pvalue, 16);
		if (val <= 1) {
			xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
				    "[store_battery_oc_protect_ut] your input is %d\n", val);
			exec_battery_oc_callback(val);
		} else {
			xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
				    "[store_battery_oc_protect_ut] wrong number (%d)\n", val);
		}
	}
	return size;
}
...
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
		    "[show_battery_oc_protect_stop] g_battery_oc_stop=%d\n", g_battery_oc_stop);
	return sprintf(buf, "%u\n", g_battery_oc_stop);
}
					     ...
					     const char *buf, size_t size)
{
	...
	if (buf != NULL && size != 0) {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[store_battery_oc_protect_stop] buf is %s and size is %d\n", buf,
			    size);
		val = kstrtoul(buf, &pvalue, 16);
		if ((val != 0) && (val != 1))
			val = 0;
		...
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[store_battery_oc_protect_stop] g_battery_oc_stop=%d\n",
			    g_battery_oc_stop);
	}
	return size;
}
...
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
		    "[show_battery_oc_protect_level] g_battery_oc_level=%d\n", g_battery_oc_level);
	return sprintf(buf, "%u\n", g_battery_oc_level);
}
...
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
		    "[store_battery_oc_protect_level] g_battery_oc_level=%d\n", g_battery_oc_level);
						...
						const char *buf, size_t size)
{
	...
	if (buf != NULL && size != 0) {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[store_battery_percent_protect_ut] buf is %s and size is %d\n", buf,
			    size);
		val = kstrtoul(buf, &pvalue, 16);
		if (val <= 1) {
			xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
						  ...
						  const char *buf, size_t size)
{
	...
	if (buf != NULL && size != 0) {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[store_battery_percent_protect_stop] buf is %s and size is %d\n", buf,
			    size);
		val = kstrtoul(buf, &pvalue, 16);
		if ((val != 0) && (val != 1))
			val = 0;
		...
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[store_battery_percent_protect_stop] g_battery_percent_stop=%d\n",
			    g_battery_percent_stop);
	}
	return size;
}
...
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[show_pmic_dvt] 0x%x\n", g_reg_value_pmic);
	return sprintf(buf, "%u\n", g_reg_value_pmic);
}
			      ...
			      size_t size)
{
	...
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[store_pmic_dvt]\n");

	if (buf != NULL && size != 0) {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[store_pmic_dvt] buf is %s and size is %d\n", buf, size);
		test_item = kstrtoul(buf, &pvalue, 10);
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[store_pmic_dvt] test_item=%d\n",
			    test_item);
...
#else
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[store_pmic_dvt] no define MTK_PMIC_DVT_SUPPORT\n");
#endif
	}
	return size;
}
...
void PMIC_INIT_SETTING_V1(void)
{
	...
	if (mt6331_chip_version >= PMIC6331_E2_CID_CODE) {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[Kernel_PMIC_INIT_SETTING_V1] 6331 PMIC Chip = 0x%x\n",
			    mt6331_chip_version);
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[Kernel_PMIC_INIT_SETTING_V1] 2014-05-08\n");
		...
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    ...
			    mt6331_chip_version);
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[Kernel_PMIC_INIT_SETTING_V1] 2014-02-06\n");
		...
		ret = pmic_config_interface(0x3A4, 0x1, 0x1, 8);	/* [8:8]: VIO18_VSLEEP_EN; Johnson; SLEEP mode setting */
		ret = pmic_config_interface(0x502, 0x1, 0x1, 11);	/* [11:11]: RG_VTCXO1_ON_CTRL; set to SW if 32K removal but ROME doesn't support 32K removal */
		ret = pmic_config_interface(0x502, 0x0, 0x3, 13);	/* [14:13]: RG_VTCXO1_SRCLK_EN_SEL; YP Niou */
		ret = pmic_config_interface(0x504, 0x1, 0x1, 11);	/* [11:11]: RG_VTCXO2_ON_CTRL; for 6169 peripheral, set to 1'b1 as default */
		ret = pmic_config_interface(0x504, 0x1, 0x3, 13);	/* [14:13]: RG_VTCXO2_SRCLK_EN_SEL; YP Niou */
		ret = pmic_config_interface(0x506, 0x1, 0x1, 0);	/* [0:0]: RG_VAUD32_LP_CTRL; YP Niou, sync with golden setting */
...
#if 1
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x4,
			    upmu_get_reg_value(0x4));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0xA,
			    upmu_get_reg_value(0xA));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0xC,
			    upmu_get_reg_value(0xC));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x10,
			    upmu_get_reg_value(0x10));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x16,
			    upmu_get_reg_value(0x16));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x1E,
			    upmu_get_reg_value(0x1E));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x106,
			    upmu_get_reg_value(0x106));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x124,
			    upmu_get_reg_value(0x124));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x13E,
			    upmu_get_reg_value(0x13E));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x144,
			    upmu_get_reg_value(0x144));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x14A,
			    upmu_get_reg_value(0x14A));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x150,
			    upmu_get_reg_value(0x150));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x184,
			    upmu_get_reg_value(0x184));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x244,
			    upmu_get_reg_value(0x244));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x24A,
			    upmu_get_reg_value(0x24A));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x24C,
			    upmu_get_reg_value(0x24C));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x24E,
			    upmu_get_reg_value(0x24E));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x250,
			    upmu_get_reg_value(0x250));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x25A,
			    upmu_get_reg_value(0x25A));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x262,
			    upmu_get_reg_value(0x262));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x264,
			    upmu_get_reg_value(0x264));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x266,
			    upmu_get_reg_value(0x266));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x26A,
			    upmu_get_reg_value(0x26A));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x282,
			    upmu_get_reg_value(0x282));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x2A6,
			    upmu_get_reg_value(0x2A6));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x2AC,
			    upmu_get_reg_value(0x2AC));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x2B2,
			    upmu_get_reg_value(0x2B2));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x2BC,
			    upmu_get_reg_value(0x2BC));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x2D6,
			    upmu_get_reg_value(0x2D6));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x314,
			    upmu_get_reg_value(0x314));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x33E,
			    upmu_get_reg_value(0x33E));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x362,
			    upmu_get_reg_value(0x362));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x368,
			    upmu_get_reg_value(0x368));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x36A,
			    upmu_get_reg_value(0x36A));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x36C,
			    upmu_get_reg_value(0x36C));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x36E,
			    upmu_get_reg_value(0x36E));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x378,
			    upmu_get_reg_value(0x378));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x394,
			    upmu_get_reg_value(0x394));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x3A4,
			    upmu_get_reg_value(0x3A4));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x502,
			    upmu_get_reg_value(0x502));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x504,
			    upmu_get_reg_value(0x504));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x506,
			    upmu_get_reg_value(0x506));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x508,
			    upmu_get_reg_value(0x508));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x50E,
			    upmu_get_reg_value(0x50E));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x522,
			    upmu_get_reg_value(0x522));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x524,
			    upmu_get_reg_value(0x524));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x528,
			    upmu_get_reg_value(0x528));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x534,
			    upmu_get_reg_value(0x534));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x54A,
			    upmu_get_reg_value(0x54A));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x552,
			    upmu_get_reg_value(0x552));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x73E,
			    upmu_get_reg_value(0x73E));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x740,
			    upmu_get_reg_value(0x740));
#endif
	} else {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[Kernel_PMIC_INIT_SETTING_V1] Unknown PMIC Chip (0x%x)\n",
			    mt6331_chip_version);
	}
	/* -------------------------------------------------------- */
	if (mt6332_chip_version >= PMIC6332_E3_CID_CODE) {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[Kernel_PMIC_INIT_SETTING_V1] 6332 PMIC Chip = 0x%x\n",
			    mt6332_chip_version);
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[Kernel_PMIC_INIT_SETTING_V1] 2014-06-29_1\n");
...
#if 1
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x8004,
			    upmu_get_reg_value(0x8004));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x801A,
			    upmu_get_reg_value(0x801A));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x8094,
			    upmu_get_reg_value(0x8094));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x80A0,
			    upmu_get_reg_value(0x80A0));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x80B2,
			    upmu_get_reg_value(0x80B2));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x8424,
			    upmu_get_reg_value(0x8424));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x8426,
			    upmu_get_reg_value(0x8426));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x8428,
			    upmu_get_reg_value(0x8428));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x843E,
			    upmu_get_reg_value(0x843E));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x8442,
			    upmu_get_reg_value(0x8442));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x844C,
			    upmu_get_reg_value(0x844C));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x845C,
			    upmu_get_reg_value(0x845C));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x846E,
			    upmu_get_reg_value(0x846E));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x8472,
			    upmu_get_reg_value(0x8472));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x8478,
			    upmu_get_reg_value(0x8478));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x847E,
			    upmu_get_reg_value(0x847E));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x8488,
			    upmu_get_reg_value(0x8488));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x8490,
			    upmu_get_reg_value(0x8490));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x8494,
			    upmu_get_reg_value(0x8494));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x8496,
			    upmu_get_reg_value(0x8496));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x8498,
			    upmu_get_reg_value(0x8498));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x84AA,
			    upmu_get_reg_value(0x84AA));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x84B0,
			    upmu_get_reg_value(0x84B0));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x84B2,
			    upmu_get_reg_value(0x84B2));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x84B4,
			    upmu_get_reg_value(0x84B4));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x84D6,
			    upmu_get_reg_value(0x84D6));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x84D8,
			    upmu_get_reg_value(0x84D8));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x84DC,
			    upmu_get_reg_value(0x84DC));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x84DE,
			    upmu_get_reg_value(0x84DE));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x84E0,
			    upmu_get_reg_value(0x84E0));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x84FC,
			    upmu_get_reg_value(0x84FC));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x850A,
			    upmu_get_reg_value(0x850A));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x851C,
			    upmu_get_reg_value(0x851C));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x851E,
			    upmu_get_reg_value(0x851E));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x8526,
			    upmu_get_reg_value(0x8526));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x8540,
			    upmu_get_reg_value(0x8540));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x8868,
			    upmu_get_reg_value(0x8868));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x886E,
			    upmu_get_reg_value(0x886E));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x8870,
			    upmu_get_reg_value(0x8870));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x8C06,
			    upmu_get_reg_value(0x8C06));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x8C0A,
			    upmu_get_reg_value(0x8C0A));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x8C12,
			    upmu_get_reg_value(0x8C12));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x8C16,
			    upmu_get_reg_value(0x8C16));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x8C18,
			    upmu_get_reg_value(0x8C18));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x8C1A,
			    upmu_get_reg_value(0x8C1A));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x8C20,
			    upmu_get_reg_value(0x8C20));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x8C40,
			    upmu_get_reg_value(0x8C40));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x8C44,
			    upmu_get_reg_value(0x8C44));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x8C46,
			    upmu_get_reg_value(0x8C46));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x8C48,
			    upmu_get_reg_value(0x8C48));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x8CB6,
			    upmu_get_reg_value(0x8CB6));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x8CB8,
			    upmu_get_reg_value(0x8CB8));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x8CBA,
			    upmu_get_reg_value(0x8CBA));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x8CBC,
			    upmu_get_reg_value(0x8CBC));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x8CD4,
			    upmu_get_reg_value(0x8CD4));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0x8CBC,
			    upmu_get_reg_value(0x8CBC));
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC INIT] Reg[0x%x]=0x%x\n", 0xE020,
			    upmu_get_reg_value(0xE020));
#endif
...
static long pmic_ftm_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case Get_IS_EXT_BUCK_EXIST:
		...
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[pmic_ftm_ioctl] Get_IS_EXT_BUCK_EXIST:%d\n", adc_out_data[0]);
		break;
		/* #endif */
	...
	case Get_IS_EXT_VBAT_BOOST_EXIST:
		...
		ret = copy_to_user(user_data_addr, adc_out_data, 8);
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[pmic_ftm_ioctl] Get_IS_EXT_VBAT_BOOST_EXIST:%d\n", adc_out_data[0]);
		break;
		/* #endif */
	...
	case Get_IS_EXT_SWCHR_EXIST:
		...
		ret = copy_to_user(user_data_addr, adc_out_data, 8);
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[pmic_ftm_ioctl] Get_IS_EXT_SWCHR_EXIST:%d\n", adc_out_data[0]);
		break;
		/* #endif */
...
void pmic_ftm_init(void)
{
	...
	if (ret)
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[pmic_ftm_init] Error: Can't Get Major number for pmic_ftm\n");
...
static int pmic_mt_probe(struct platform_device *dev)
{
	...
	if (get_pmic_mt6332_cid() >= PMIC6332_E3_CID_CODE) {
		...
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[CONFIG_POWER_EXT] disable MT6332 CHRWDT (0x%x)\n",
...
#if defined(CONFIG_MTK_FPGA)
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
		    "[PMIC_EINT_SETTING] disable when CONFIG_MTK_FPGA\n");
#else
	/* PMIC Interrupt Service */
	pmic_6331_thread_handle =
	    kthread_create(pmic_thread_kthread_mt6331, (void *)NULL, "pmic_6331_thread");
	if (IS_ERR(pmic_6331_thread_handle)) {
		pmic_6331_thread_handle = NULL;
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[pmic_thread_kthread_mt6331] creation fails\n");
	} else {
		wake_up_process(pmic_6331_thread_handle);
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[pmic_thread_kthread_mt6331] kthread_create Done\n");
	}

	pmic_6332_thread_handle =
	    kthread_create(pmic_thread_kthread_mt6332, (void *)NULL, "pmic_6332_thread");
	if (IS_ERR(pmic_6332_thread_handle)) {
		pmic_6332_thread_handle = NULL;
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[pmic_thread_kthread_mt6332] creation fails\n");
	} else {
		wake_up_process(pmic_6332_thread_handle);
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "[pmic_thread_kthread_mt6332] kthread_create Done\n");
	}
...
static int __init pmic_mt_init(void)
{
	...
	wake_lock_init(&pmicThread_lock_mt6331, WAKE_LOCK_SUSPEND,
		       "pmicThread_lock_mt6331 wakelock");
	wake_lock_init(&pmicThread_lock_mt6332, WAKE_LOCK_SUSPEND,
		       "pmicThread_lock_mt6332 wakelock");
	...
	if (ret) {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "****[pmic_mt_init] Unable to device register(%d)\n", ret);
		return ret;
	}
	ret = platform_driver_register(&pmic_mt_driver);
	if (ret) {
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
			    "****[pmic_mt_init] Unable to register driver (%d)\n", ret);
		return ret;
	}
#ifdef CONFIG_HAS_EARLYSUSPEND
	...
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC",
		    "****[pmic_mt_init] Initialization : DONE !!\n");
