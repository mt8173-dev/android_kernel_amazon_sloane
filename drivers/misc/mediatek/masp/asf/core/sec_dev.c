 ...
 *  GLOBAL VARIABLES
 *************************************************************************/
struct MtdPart_s mtd_part_map[MAX_MTD_PARTITIONS];
 ...
 *  READ INFO
 **************************************************************************/
int read_info(int part_index, uint32 part_off, uint32 search_region, char *info_name,
	      uint32 info_name_len, uint32 info_sz, char *info_buf)
{
	...
	struct MtdRCtx_s *ctx = (struct MtdRCtx_s *) osal_kmalloc(sizeof(struct MtdRCtx_s));
	...
	if ((MAX_MTD_PARTITIONS + 1) == part_index) {
		...
		mcpy(part_path + strlen(USER_REGION_PART_PATH_PREFIX), mtd_part_map[part_index].name,
			strlen(mtd_part_map[part_index].name));
...
int sec_dev_read_rom_info(void)
{
	...
	for (search_offset = ROM_INFO_SEARCH_START;
		...
		ret =
		    read_info((MAX_MTD_PARTITIONS + 1), search_offset, ROM_INFO_SEARCH_REGION, RI_NAME,
				      RI_NAME_LEN, sizeof(AND_ROMINFO_T), (uchar *) &rom_info);
		...
		if (mtd_off < mtd_part_map[mtd_num].sz) {
			...
			if (SEC_OK == ret) {
				...
				if (TRUE == sec_secro_ac()) {
					masp_hal_sp_hacc_dec((uchar *) &secroimg.m_andro,
				...
				/* ------------------------ */
				if (TRUE == sec_secro_ac()) {
					masp_hal_sp_hacc_enc((uchar *) &secroimg.m_andro,
...
int sec_dev_read_secroimg_v5(uint32 md_index)
{
	...
	ret =
	    read_info(mtd_num, mtd_off, SECRO_SEARCH_REGION, ROM_SEC_AC_REGION_ID,
		      ROM_SEC_AC_REGION_ID_LEN, img_len, (uchar *) &secroimg_v5);
	...
	if (SEC_OK == ret) {
		...
		if (TRUE == sec_secro_ac()) {
			masp_hal_sp_hacc_dec((uchar *) &secroimg_v5.m_md_info_v5a, cipher_len,
		...
		/* ------------------------ */
		if (TRUE == sec_secro_ac()) {
			masp_hal_sp_hacc_enc((uchar *) &secroimg_v5.m_md_info_v5a, cipher_len,
					     TRUE, FALSE, TRUE);
		}
		dump_buf((uchar *) &secroimg_v5.m_md_info_v5a, 0x4);
