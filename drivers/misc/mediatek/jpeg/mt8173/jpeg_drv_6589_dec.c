...
unsigned int jpeg_drv_dec_get_irqState(void)
{
	...
	IMG_REG_WRITE((u4Value), REG_ADDR_JPGDEC_INTERRUPT_STATUS);	/* REG_JPGDEC_INTERRUPT_STATUS = u4Value ; */
	return u4Value;
...
int jpeg_drv_dec_wait_one_row(JPEG_DEC_DRV_IN *config)
{
	...
	/* for( tri_cnt = 1 ; tri_cnt <= 60 ; tri_cnt++) */
	{
		...
		IMG_REG_WRITE((irq_status), REG_ADDR_JPGDEC_INTERRUPT_STATUS);	/* REG_JPGDEC_INTERRUPT_STATUS = irq_status ; */
...
int jpeg_drv_dec_wait(JPEG_DEC_DRV_IN *config)
{
	...
	JPEG_MSG("JPEG Decode Success, st %x!!\n", irq_status);
	return 1;
}
