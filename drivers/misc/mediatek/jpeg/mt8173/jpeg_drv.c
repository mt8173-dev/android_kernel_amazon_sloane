...
#include <linux/io.h>
#include <linux/clk.h>  /*clock tree*/
static struct clk *clk_venc_larb;	/* =NULL; */
static struct clk *clk_venc_jdec;	/* =NULL; */
static struct clk *clk_disp0_sim_comon;	/* =NULL; */
static struct clk *clk_venc_jenc;	/* =NULL; */
static struct clk *clk_jpg_pwr;	/* =NULL; */
/* struct clk *clk_disp0_mdp_crop =NULL;*/
#endif
...
void jpeg_drv_dec_power_on(void)
{
	...
	#else
	JPEG_MSG("jpeg_drv_enc_power_on \n");
	clk_prepare(clk_jpg_pwr);
	clk_enable(clk_jpg_pwr);
	clk_prepare(clk_disp0_sim_comon);
	clk_enable(clk_disp0_sim_comon);
	clk_prepare(clk_venc_larb);
	clk_enable(clk_venc_larb);
	clk_prepare(clk_venc_jdec);
	clk_enable(clk_venc_jdec);
	#endif
#endif
...
void jpeg_drv_dec_power_off(void)
{
	...
	#else
	clk_disable(clk_disp0_sim_comon);
	clk_unprepare(clk_disp0_sim_comon);
	clk_disable(clk_venc_larb);
	clk_unprepare(clk_venc_larb);
	clk_disable(clk_venc_jdec);
	clk_unprepare(clk_venc_jdec);
	clk_disable(clk_jpg_pwr);
	clk_unprepare(clk_jpg_pwr);
	#endif
#endif
...
void jpeg_drv_enc_power_on(void)
{
	...
	clk_prepare(clk_venc_larb);
	clk_enable(clk_venc_larb);
	clk_prepare(clk_venc_jenc);
	clk_enable(clk_venc_jenc);
	#endif
#endif
...
void jpeg_drv_enc_power_off(void)
{
	...
	clk_disable(clk_venc_larb);
	clk_unprepare(clk_venc_larb);
	clk_disable(clk_venc_jenc);
	clk_unprepare(clk_venc_jenc);
	clk_disable(clk_jpg_pwr);
	clk_unprepare(clk_jpg_pwr);
	#endif
#endif
...
static int jpeg_dec_ioctl(unsigned int cmd, unsigned long arg, struct file *file)
{
	...
	case JPEG_DEC_IOCTL_FLUSH_CMDQ:
		...
		for (i = 0; i < cfg_cmdq_params.goNum; i++) {
			...
			/* wait frame done */
			cmdqRecWait(jpegCMDQ_handle, CMDQ_EVENT_JPEG_DEC_EOF);
			/* cmdqRecPoll(jpegCMDQ_handle, 0x18004274 ,    BIT_INQST_MASK_PAUSE , 0x0010) ; *//*REG_ADDR_JPGDEC_INTERRUPT_STATUS */
		}
					       ...
					       JPEG_DEC_DRV_OUT __user *data)
{
	...
	err |= get_user(ptr, &data->result);
	err |= put_user(ptr_to_compat(ptr), &data32->result);
	return err;
}
						 ...
						 JpegDrvDecResult __user *data)
{
	...
	err |= put_user(ptr_to_compat(pChksum), &data32->pChksum);
	return err;
}
					       ...
					       JPEG_ENC_DRV_OUT __user *data)
{
	...
	err |= put_user(ptr_to_compat(fileSize), &data32->fileSize);
	err |= get_user(result, &data->result);
	err |= put_user(ptr_to_compat(result), &data32->result);
	err |= get_user(cycleCount, &data->cycleCount);
	err |= put_user(ptr_to_compat(cycleCount), &data32->cycleCount);
	return err;
}
...
{
	long ret;
/* JPEG_MSG("compat_jpeg_ioctl --coming\n"); */
...
{
	/*JPEG_MSG("jpeg_unlocked_ioctl --coming\n");*/
	switch (cmd) {
	case JPEG_DEC_IOCTL_INIT:
	case JPEG_DEC_IOCTL_CONFIG:
	case JPEG_DEC_IOCTL_START:
...
static int jpeg_probe(struct platform_device *pdev)
{
...
#endif
	if (_jpeg_driver_probed)	{
		JPEG_MSG("jpeg driver probed\n");
		return 0;
	}
	_jpeg_driver_probed = 1;
#ifdef CONFIG_OF
	...
	JPEG_MSG("jpeg encRegBaseVA [%lx]\n", jpegDev->encRegBaseVA);
	JPEG_MSG("jpeg encIrqId [%x]\n", jpegDev->encIrqId);
	...
	JPEG_MSG("jpeg decRegBaseVA [%lx]\n", jpegDev->decRegBaseVA);
	JPEG_MSG("jpeg decIrqId [%x]\n", jpegDev->decIrqId);

	gJpegqDev = *jpegDev;

	clk_venc_larb	= devm_clk_get(&pdev->dev, "MT_CG_VENC_SMI_LARB3");
	BUG_ON(IS_ERR(clk_venc_larb));
	clk_venc_jdec	= devm_clk_get(&pdev->dev, "MT_CG_VENC_JPGDEC");
	BUG_ON(IS_ERR(clk_venc_jdec));
	clk_venc_jenc	= devm_clk_get(&pdev->dev, "MT_CG_VENC_JPGENC");
	BUG_ON(IS_ERR(clk_venc_jenc));
	clk_disp0_sim_comon = devm_clk_get(&pdev->dev, "MMSYS_CLK_SMI_COMMON");
	BUG_ON(IS_ERR(clk_disp0_sim_comon));
	clk_jpg_pwr = devm_clk_get(&pdev->dev, "MT_CG_JPG_PWR");
	BUG_ON(IS_ERR(clk_jpg_pwr));

	/*disable all jpeg clocks */
	clk_prepare(clk_jpg_pwr);
	clk_enable(clk_jpg_pwr);
	...
	JPEG_MSG("jpeg driver probe JPEG_DEV\n");
...
#else
	JPEG_MSG("jpeg driver probe no JPEG_DEV\n");
	...
	if (request_irq
	    (gJpegqDev.decIrqId, jpeg_drv_dec_isr, IRQF_TRIGGER_FALLING, "jpeg_dec_driver", NULL)) {
