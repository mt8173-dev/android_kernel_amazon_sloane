...
static long jpeg_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
	...
	case JPEG_DEC_IOCTL_FLUSH_CMDQ:
		...
		case JPEG_ENC_IOCTL_INIT:case JPEG_ENC_IOCTL_CONFIG:case JPEG_ENC_IOCTL_WAIT:case
JPEG_ENC_IOCTL_DEINIT:case JPEG_ENC_IOCTL_START:return jpeg_enc_ioctl(cmd, arg,
											  file);
...
static struct file_operations jpeg_fops = {
	.owner = THIS_MODULE,
	    /* .ioctl                = jpeg_ioctl, */
.unlocked_ioctl = jpeg_unlocked_ioctl, .open = jpeg_open, .release = jpeg_release, .flush =
	    jpeg_flush, .read = jpeg_read,};
...
static int jpeg_probe(struct platform_device *pdev) {
	...
	if (request_irq
	    (JPGENC_IRQ_BIT_ID, jpeg_drv_enc_isr, IRQF_TRIGGER_LOW, "jpeg_enc_driver", NULL))
		/* if(request_irq(JPGENC_IRQ_BIT_ID, jpeg_drv_enc_isr, /*IRQF_TRIGGER_RISING */
		IRQF_TRIGGER_HIGH, "jpeg_enc_driver", NULL)) */
	...
	/* if(request_irq(JPGDEC_IRQ_BIT_ID, jpeg_drv_dec_isr, /*IRQF_TRIGGER_RISING */
	    IRQF_TRIGGER_HIGH, "jpeg_dec_driver", NULL)) */
	    /* if(request_irq(JPGDEC_IRQ_BIT_ID, jpeg_drv_dec_isr, IRQF_TRIGGER_RISING , "jpeg_dec_driver" , NULL)) */
	    if (request_irq
		(JPGDEC_IRQ_BIT_ID, jpeg_drv_dec_isr, IRQF_TRIGGER_FALLING, "jpeg_dec_driver",
		 NULL)) {
...
static struct platform_driver jpeg_driver = {
	.probe = jpeg_probe, .remove = jpeg_remove, .shutdown = jpeg_shutdown, .suspend =
	    jpeg_suspend, .resume = jpeg_resume, .driver = {
.name = JPEG_DEVNAME,},};
...
static struct platform_device jpeg_device = {
	.name = JPEG_DEVNAME, .id = 0, .dev = {
.release = jpeg_device_release, .dma_mask = &jpegdec_dmamask, .coherent_dma_mask =
		    0xffffffff,}, .num_resources = 0,};
