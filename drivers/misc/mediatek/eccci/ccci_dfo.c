...
phys_addr_t *__get_modem_start_addr_list(void)
{
	return md_resv_mem_addr;
}

static int check_md_header(struct ccci_modem *md, void *parse_addr, struct ccci_image_info *image)
{
	int ret;
...
static int check_dsp_header(struct ccci_modem *md, void *parse_addr, struct ccci_image_info *image)
{
	return 0;
}
...
static void collect_md_mem_setting(void)
{
	...
	if ((md_usage_case & (MD1_EN | MD2_EN)) == (MD1_EN | MD2_EN)) {	/* Both two MD enabled */
		...
		md_resv_smem_addr[MD_SYS2] =
		    addr[0] + md_resv_mem_size[MD_SYS1] + md_resv_smem_size[MD_SYS1];
	...
	CCCI_INF_COM_MSG("MD1_EN(%d):MD2_EN(%d):MemBase(0x%llX)\n", md1_en, md2_en,
			 (unsigned long long)md_resv_smem_base);

	CCCI_INF_COM_MSG("MemStart(0x%llX:0x%llX):MemSize(0x%08X:0x%08X)\n",
			 (unsigned long long)md_resv_mem_addr[MD_SYS1],
			 (unsigned long long)md_resv_mem_addr[MD_SYS2], md_resv_mem_size[MD_SYS1],
			 md_resv_mem_size[MD_SYS2]);

	CCCI_INF_COM_MSG("SmemStart(0x%llX:0x%llX):SmemSize(0x%08X:0x%08X)\n",
			 (unsigned long long)md_resv_smem_addr[MD_SYS1],
			 (unsigned long long)md_resv_smem_addr[MD_SYS2], md_resv_smem_size[MD_SYS1],
			 md_resv_smem_size[MD_SYS2]);
}
...
static void cal_md_mem_usage(void)
{
	...
	if (md1_en && (md_resv_smem_size[MD_SYS1] > 0) && (md_resv_mem_size[MD_SYS1] > 0)) {
		/* Setting is OK */
		   ...
		   && ((md_resv_smem_size[MD_SYS1] <= 0) || (md_resv_mem_size[MD_SYS1] <= 0))) {
		CCCI_ERR_COM_MSG("DFO Setting for md1 wrong: <%d:0x%08X:0x%08X>\n", md1_en,
				 md_resv_mem_size[MD_SYS1], md_resv_smem_size[MD_SYS1]);
	...
	if (md2_en && (md_resv_smem_size[MD_SYS2] > 0) && (md_resv_mem_size[MD_SYS2] > 0)) {
		/* Setting is OK */
		   ...
		   && ((md_resv_smem_size[MD_SYS2] <= 0) || (md_resv_mem_size[MD_SYS2] <= 0))) {
		CCCI_ERR_COM_MSG("DFO Setting for md2 wrong: <%d:0x%08X:0x%08X>\n", md2_en,
				 md_resv_mem_size[MD_SYS2], md_resv_smem_size[MD_SYS2]);
...
static int load_std_firmware(struct ccci_modem *md, struct file *filp, struct ccci_image_info *img)
{
	void __iomem *start;
	int ret = 0;
	int check_ret = 0;
...
int ccci_load_firmware(struct ccci_modem *md, MD_IMG_TYPE img_type,
