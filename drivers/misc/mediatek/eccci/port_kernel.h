...
extern void aed_md_exception(const int *log, int log_size, const int *phy, int phy_size,
			     const char *detail);
extern __weak void aee_kernel_warning_api(const char *file, const int line, const int db_opt,
...
typedef enum {
	...
	IPC_RPC_GET_RF_CLK_BUF_OP = 0x4006,
...
#define GPIO_ADC_NAME_STR_MAX_LEN 33
enum {
...
struct ccci_rpc_gpio_adc_intput {
	u8 reqMask;
	u8 gpioValidPinMask;
	char gpioPinName[GPIO_MAX_COUNT][GPIO_ADC_NAME_STR_MAX_LEN];
	u32 gpioPinNum[GPIO_MAX_COUNT];
	char adcChName[GPIO_ADC_NAME_STR_MAX_LEN];
	u32 adcChNum;
	u32 adcChMeasCount;
} __attribute__ ((packed));
