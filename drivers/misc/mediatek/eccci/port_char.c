...
static int dev_char_close(struct inode *inode, struct file *file)
{
	...
	if (port->modem->md_state == GATED && dev_char_close_check(port) == 0)
		ccci_send_virtual_md_msg(port->modem, CCCI_MONITOR_CH, CCCI_MD_MSG_READY_TO_RESET,
					 0);
	return 0;
}
			      ...
			      loff_t *ppos)
{
	...
	if (port->modem->md_state == BOOTING && port->tx_ch != CCCI_FS_TX)
		return -ENODEV;
	if (port->modem->md_state == EXCEPTION && port->tx_ch != CCCI_MD_LOG_TX
	...
	if (port->flags & PORT_F_USER_HEADER) {
		if (count > CCCI_MTU + sizeof(struct ccci_header)) {
			CCCI_ERR_MSG(port->modem->index, CHAR,
				     "reject packet(size=%d ), lager than MTU on CH%u\n", count,
				     port->tx_ch);
			return -ENOMEM;
		}
	}
	...
	if (port->flags & PORT_F_USER_HEADER)
		actual_count =
		    count >
		    CCCI_MTU + sizeof(struct ccci_header) ? CCCI_MTU +
		    sizeof(struct ccci_header) : count;
	else
	...
	if (req) {
		...
		if (!(port->flags & PORT_F_USER_HEADER)) {
			ccci_h =
			    (struct ccci_header *)skb_put(req->skb, sizeof(struct ccci_header));
			ccci_h->data[0] = 0;
...
#define CCCI_IOC_CLR_HEADER             _IO(CCCI_IOC_MAGIC,  22)
	...
	case CCCI_IOC_SEND_RUN_TIME_DATA:
		if (ch == CCCI_MONITOR_CH) {
			...
			CCCI_INF_MSG(md->index, CHAR,
				     "Set runtime by invalid user(%u) called by %s\n", ch,
				     current->comm);
			ret = -1;
		}
		break;
	...
	case CCCI_IOC_SEND_STOP_MD_REQUEST:
		...
		if (ret == 0) {
			md->ops->stop(md, 0);
			ret =
			    ccci_send_virtual_md_msg(md, CCCI_MONITOR_CH,
						     CCCI_MD_MSG_STOP_MD_REQUEST, 0);
		}
		break;
	...
	case CCCI_IOC_ENTER_DEEP_FLIGHT:
		...
		if (ret == 0) {
			...
			ret =
			    ccci_send_virtual_md_msg(md, CCCI_MONITOR_CH,
						     CCCI_MD_MSG_ENTER_FLIGHT_MODE, 0);
		}
		break;
	case CCCI_IOC_LEAVE_DEEP_FLIGHT:
		CCCI_INF_MSG(md->index, CHAR, "leave MD flight mode ioctl called by %s\n",
			     current->comm);
		ret =
		    ccci_send_virtual_md_msg(md, CCCI_MONITOR_CH, CCCI_MD_MSG_LEAVE_FLIGHT_MODE, 0);
		break;
	...
	case CCCI_IOC_POWER_ON_MD_REQUEST:
		CCCI_INF_MSG(md->index, CHAR, "Power on MD request ioctl called by %s\n",
			     current->comm);
		ret =
		    ccci_send_virtual_md_msg(md, CCCI_MONITOR_CH, CCCI_MD_MSG_POWER_ON_REQUEST, 0);
		break;
	...
	case CCCI_IOC_POWER_OFF_MD_REQUEST:
		CCCI_INF_MSG(md->index, CHAR, "Power off MD request ioctl called by %s\n",
			     current->comm);
		ret =
		    ccci_send_virtual_md_msg(md, CCCI_MONITOR_CH, CCCI_MD_MSG_POWER_OFF_REQUEST, 0);
		break;
	...
	case CCCI_IOC_GET_SIM_TYPE:
		if (md->sim_type == 0xEEEEEEEE)
			CCCI_ERR_MSG(md->index, KERN, "md has not send sim type yet(%d)",
				     md->sim_type);
	...
	case CCCI_IOC_SEND_BATTERY_INFO:
		ret =
		    ccci_send_msg_to_md(md, CCCI_SYSTEM_TX, MD_GET_BATTERY_INFO,
					(u32) BAT_Get_Battery_Voltage(0));
		break;
	...
	case CCCI_IOC_STORE_MD_TYPE:
		if (copy_from_user
		    (&md->config.load_type_saving, (void __user *)arg, sizeof(unsigned int))) {
			...
			if (md->config.load_type_saving >= 1
			    && md->config.load_type_saving <= MAX_IMG_NUM) {
...
static int port_char_recv_req(struct ccci_port *port, struct ccci_request *req)
{
	...
	if (port->rx_length < port->rx_length_th) {
		...
		if (likely(!(port->flags & PORT_F_CHAR_KERNEL))) {
			wake_up_all(&port->rx_wq);
		} else {
			if (port->rx_ch == CCCI_IPC_RX)
				port_ipc_kernel_read(port);
			else
				CCCI_ERR_MSG(port->modem->index, CHAR,
					     "get a kernel handled packet on %s\n", port->name);
		}
		return 0;
	} else {
		port->flags |= PORT_F_RX_FULLED;
