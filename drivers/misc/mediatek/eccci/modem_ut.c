...
static struct ccci_port md_ut_ports_normal[] = {
	{CCCI_MONITOR_CH, CCCI_MONITOR_CH, 0, 0, 0, 0, 4, &char_port_ops, MD_UT_MAJOR, 0,
	 "ccci_monitor",},

	{CCCI_IPC_UART_TX, CCCI_IPC_UART_RX, 0, 0, 0xFF, 0xFF, 0, &char_port_ops, MD_UT_MAJOR, 1,
	 "ut_ipc_uart",},
	{CCCI_IPC_TX, CCCI_IPC_RX, 0, 0, 0xFF, 0xFF, 0, &char_port_ops, MD_UT_MAJOR, 0, "ut_ipc",},
	{CCCI_IPC_RX_ACK, CCCI_IPC_TX_ACK, 0, 0, 0xFF, 0xFF, 0, &ipc_port_ack_ops, 0, 0,
	 "ut_ipc_ack",},
		...
		if (ret >= 0 || ret == -CCCI_ERR_DROP_PACKET) {
			...
			if (ret == -CCCI_ERR_CHANNEL_NUM_MIS_MATCH
				...
				CCCI_ERR_MSG(md->index, CORE,
					     "received illegle message from ch0x%X on q%d\n",
...
static void md_ut_rx_gen_func(unsigned long data)
{
	...
	case 2:		/* IPC */
		...
		snprintf(skb_put(req->skb, sizeof(pseudo_msg)), sizeof(pseudo_msg),
			 "this is IPC %08X\n", pseudo_bootup);
		break;
	case 3:		/* RPC */
		ccci_h->data[0] = 0;
...
void md_ut_statistic_timer_func(unsigned long data)
{
	...
	if ((sched_clock() - sched_time) >
	    ((unsigned long long)STATISTIC_DUMP_INTERVAL * 1000000000)) {
		sched_time = sched_clock();
		for (i = 0; i < QUEUE_LEN(md_ctrl->txq); i++) {
			if (md_ctrl->txq[i].process_count > 0) {
				temp =
				    100 -
				    md_ctrl->txq[i].not_complet_count * 100 /
				    md_ctrl->txq[i].process_count;
				printk("[CCCI-Q]Txq%d %d/%d %d\n", i,
				       md_ctrl->txq[i].not_complet_count,
				       md_ctrl->txq[i].process_count, temp2 =
				       md_ctrl->txq[i].data_count / 1000 / STATISTIC_DUMP_INTERVAL);
				if (i == 1) {
					txqC = temp;
					txqR = temp2;
				}
		...
		for (i = 0; i < QUEUE_LEN(md_ctrl->rxq); i++) {
			if (md_ctrl->rxq[i].process_count > 0) {
				temp =
				    100 -
				    md_ctrl->rxq[i].not_complet_count * 100 /
				    md_ctrl->rxq[i].process_count;
				printk("[CCCI-Q]Rxq%d %d/%d %d\n", i,
				       md_ctrl->rxq[i].not_complet_count,
				       md_ctrl->rxq[i].process_count, temp2 =
				       md_ctrl->rxq[i].data_count / 1000 / STATISTIC_DUMP_INTERVAL);
				if (i == 1) {
					rxqC = temp;
					rxqR = temp2;
				}
...
static int md_ut_sched_thread(void *arg)
{
	...
	while (1) {
		...
		wait_event_interruptible(md_ctrl->sched_thread_wq, (kthread_should_stop()
...
static int md_ut_init(struct ccci_modem *md)
{
	...
	/* 1. init queue */
	for (i = 0; i < QUEUE_LEN(md_ctrl->txq); i++) {
		md_ut_queue_struct_init(&md_ctrl->txq[i], md, OUT, i, MAX_QUEUE_LENGTH,
					QUEUE_BUDGET);
	}
	for (i = 0; i < QUEUE_LEN(md_ctrl->rxq); i++) {
		md_ut_queue_struct_init(&md_ctrl->rxq[i], md, IN, i, MAX_QUEUE_LENGTH,
					QUEUE_BUDGET);
	}
	/* 2. init port */
	for (i = 0; i < md->port_number; i++) {
...
static int md_ut_send_request(struct ccci_modem *md, unsigned char qno, struct ccci_request *req)
{
 ...
 retry:
		...
		if (md_ctrl->txq[qno].length < md_ctrl->txq[qno].length_th) {	/* tx queue threshold check */
			 ...
			 */
			ret =
			    wait_event_interruptible_exclusive(md_ctrl->txq[qno].req_wq,
							       (md_ctrl->txq[qno].length <
								md_ctrl->txq[qno].length_th));
			if (ret == -ERESTARTSYS) {
				return -EINTR;
			}
			goto retry;
