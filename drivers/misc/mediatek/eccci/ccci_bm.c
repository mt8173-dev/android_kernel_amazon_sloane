 ...
 retry:
	...
	for (i = 0; i < BM_POOL_SIZE; i++) {
		if (req_pool[i].state == IDLE) {
			...
			if (req_pool[i].entry.next == LIST_POISON1
			    && req_pool[i].entry.prev == LIST_POISON2) {
				req = &req_pool[i];
...
void ccci_free_req(struct ccci_request *req)
{
	...
	CCCI_DBG_MSG(-1, BM, "%ps free req=%p, policy=%d, skb=%p, len=%d\n",
		     __builtin_return_address(0), req, req->policy, req->skb, skb_size(req->skb));
