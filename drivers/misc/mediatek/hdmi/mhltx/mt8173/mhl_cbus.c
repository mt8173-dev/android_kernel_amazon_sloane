...
/* /////////////////////////////////////// */
unsigned int mhl_cbus_int_status;
/* /////////////////////////////////////// */
/* for Sink device reg */
...
unsigned int mhl_check_irq(void)
{
	unsigned int irq_status;
	irq_status = u4Read2BCbus(CBUS_LINK_STA_00);
	irq_status = irq_status & (LINK_08_LINKRX_TIMEOUT_INT_CLR_MASK
				   | LINK_08_RBUF_TRIG_INT_CLR_MASK
				   | LINK_08_TX_ARB_FAIL_INT_CLR_MASK
				   | LINK_08_TX_OK_INT_CLR_MASK
				   | LINK_08_TX_RETRY_TO_INT_CLR_MASK
...
void mhl_cbus_get_int_status(void)
{
	mhl_cbus_int_status = u4Read2BCbus(CBUS_LINK_STA_00);
	vWrite2BCbus(CBUS_LINK_08, mhl_cbus_int_status);
...
/* TX process */
	...
	if (((cbus_msc_tx.tx_state == SRC_CBUS_TX_VALID)
		...
		if (cbus_msc_tx.tx_state == SRC_CBUS_TX_VALID) {
			...
			vSetCbusTxOkWaitTmr(40 / MHL_LINK_TIME);
#else
...
/* new cmd process */
	...
	{
		if (fgCbusMscIdle() && fgCbusDdcIdle() && (cbus_msc_tx.tx_state == SRC_CBUS_TX_IDLE)
		    && (cbus_ddc_tx.tx_state == SRC_CBUS_TX_IDLE)) {
			...
			if ((u4CbusRxCount == 0)
				...
				if (stCbusRequester.fgIsDdc == FALSE) {
					for (i = 0; i < stCbusRequester.u4Len; i++)
						cbus_msc_tx.u2TxBuf[i] =
						    stCbusRequester.u2ReqBuf[i];
					cbus_msc_tx.u2Len = stCbusRequester.u4Len;
					cbus_msc_tx.tx_state = SRC_CBUS_TX_SEND;
					MHL_CBUS_LOG("msc req begin\n");
				} else {
					for (i = 0; i < stCbusRequester.u4Len; i++)
						cbus_ddc_tx.u2TxBuf[i] =
						    stCbusRequester.u2ReqBuf[i];
