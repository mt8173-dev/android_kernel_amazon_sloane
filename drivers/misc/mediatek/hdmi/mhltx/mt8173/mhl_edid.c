...
unsigned char _bEdidData[EDID_SIZE];	/* 4 block 512 Bytes */
...
void vShowEdidRawData(void)
{
	...
	for (bTemp = 0; bTemp < (_bEdidData[0x7e] + 1); bTemp++) {
		printk("===================================================================\n");
...
unsigned char fgMhlReadEdid(unsigned char i1noedid)
{
...
void mhl_checedid(unsigned char i1noedid)
{
	unsigned char bTemp;
	unsigned char bRetryCount = 4;
	unsigned char i;
	MHL_EDID_FUNC();
	...
	for (bTemp = 0; bTemp < bRetryCount; bTemp++) {
		if (fgMhlReadEdid(i1noedid) == TRUE) {
			...
			if (bTemp == bRetryCount - 1) {
				if (fgIsCbusConnected() == FALSE)
					vClearEdidInfo();
				else if (fgIsCbusConnected() == TRUE)
					vSetEdidChkError();
...
/* mhl not need1080p */
	_HdmiSinkAvCap.ui4_sink_native_ntsc_resolution &= ~(SINK_1080P50 | SINK_1080P60);
	_HdmiSinkAvCap.ui4_sink_native_pal_resolution &= ~(SINK_1080P50 | SINK_1080P60);
	_HdmiSinkAvCap.ui4_sink_dtd_ntsc_resolution &= ~(SINK_1080P50 | SINK_1080P60);
	_HdmiSinkAvCap.ui4_sink_dtd_pal_resolution &= ~(SINK_1080P50 | SINK_1080P60);
	_HdmiSinkAvCap.ui4_sink_cea_ntsc_resolution &= ~(SINK_1080P50 | SINK_1080P60);
	_HdmiSinkAvCap.ui4_sink_cea_pal_resolution &= ~(SINK_1080P50 | SINK_1080P60);
	_HdmiSinkAvCap.ui4_sink_1st_dtd_ntsc_resolution &= ~(SINK_1080P50 | SINK_1080P60);
	_HdmiSinkAvCap.ui4_sink_1st_dtd_pal_resolution &= ~(SINK_1080P50 | SINK_1080P60);
...
void mhl_AppGetEdidInfo(HDMI_EDID_INFO_T *pv_get_info)
{
	MHL_EDID_FUNC();
