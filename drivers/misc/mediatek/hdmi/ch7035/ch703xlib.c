...
static void hw_SetupReg(uint8 addr)
{
	...
	for (page = 0; page < REG_PAGE_NUM; ++page) {
		if (page == 0x02)
			continue;	/* page 2 is always same ... */
...
static CH703x_DEV_TYPE hw_detectChip(uint8 addr, CH703x_DEV_TYPE start, CH703x_DEV_TYPE end)
{
	...
	printk("Dev %X Rev %d IDBD %X at %X\n", dev, rev & CH703x_RID_MASK, rev & CH703x_IDBD_MASK, addr);	/* MTK-Modify */
...
/* } */
	...
	while (1) {		/* WYC_TEST */
		...
		if (lnsel >= 3) {
			if (pPrefer->true24)
				return ERR_NO_SUPPORT_TRUE24;	/* re-calculate after disable true24 */
			if (pPrefer->true_com)
				return ERR_NO_SUPPORT_TRUECOM;	/* re-calculate after disable true_com */
