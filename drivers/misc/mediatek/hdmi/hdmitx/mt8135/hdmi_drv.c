...
static struct task_struct *hdmi_nlh_task;
wait_queue_head_t hdmi_nlh_wq;	/* NFI, LVDS, HDMI */
atomic_t hdmi_nlh_event = ATOMIC_INIT(0);
...
static int hdmi_nlh_kthread(void *data);
...
{
	if ((u2Reg & 0x01000000) == 0x01000000) {
		pwrap_read(u2Reg - 0xf1000000, p4Data);
	} else {
		*p4Data = (*(volatile unsigned int *)(u2Reg));
	}
	pr_info("Reg read= 0x%08x, data = 0x%08x\n", u2Reg, *p4Data);
}

void hdmi_write(unsigned int u2Reg, unsigned int u4Data)
{
	if ((u2Reg & 0x01000000) == 0x01000000) {
		pwrap_write(u2Reg - 0xf1000000, u4Data);
	} else {
#if (defined(CONFIG_MTK_IN_HOUSE_TEE_SUPPORT) && defined(CONFIG_MTK_HDMI_HDCP_SUPPORT))
...
static int hdmi_internal_init(void)
{
	...
	init_waitqueue_head(&hdmi_nlh_wq);
	hdmi_nlh_task = kthread_create(hdmi_nlh_kthread, NULL, "hdmi_nlh_kthread");
	wake_up_process(hdmi_nlh_task);
...
static int hdmi_nlh_kthread(void *data)
{
	...
	for (;;) {
		wait_event_interruptible(hdmi_nlh_wq, atomic_read(&hdmi_nlh_event));
		atomic_set(&hdmi_nlh_event, 0);
		hdmi_nlh_impl();
		if (kthread_should_stop())
			break;
	}
