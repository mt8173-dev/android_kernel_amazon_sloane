...
#include "../../../../analogix/slimport_tx_drv.h"
#endif
...
void vClearEdidInfo(void)
{
	...
	_HdmiSinkAvCap.ui1_Video_Input_Definitation = 1;
...
#ifdef CONFIG_SLIMPORT_ANX3618
		/* Read EDID data from memeory buffer */
		hdmi_common_read_edid_block(0, &_bEdidData2[0]);
		hdmi_common_read_edid_block(1, &_bEdidData2[128]);
#endif
		for (i = 0; i < 256; i++)
			_bEdidData[i] = _bEdidData2[i];
		bExtBlockNo = _bEdidData2[0];
		return 1;
...
unsigned char fgParserEDID(unsigned char *prbData)
{
	...
	_HdmiSinkAvCap.ui1_Video_Input_Definitation =
		((*(prbData + EDID_VIDEO_INPUT_DEFINITION)) & 0x80) >> 7;
...
void vShowEdidInformation(void)
{
	...
	pr_info("[HDMI]EDID ver:%d/rev:%d\n", _HdmiSinkAvCap.ui1_Edid_Version,
		_HdmiSinkAvCap.ui1_Edid_Revision);
	pr_info("[HDMI]EDID Extend Rev:%d \n", _HdmiSinkAvCap.ui1_ExtEdid_Revision);
	if (_HdmiSinkAvCap.b_sink_support_hdmi_mode) {
		pr_info("[HDMI]SINK Device is HDMI\n");
	} else {
		pr_info("[HDMI]SINK Device is DVI\n");
	}

	if (_HdmiSinkAvCap.b_sink_support_hdmi_mode)
		pr_info("[HDMI]CEC ADDRESS:%x \n", _HdmiSinkAvCap.ui2_sink_cec_address);

	pr_info("[HDMI]max clock limit : %d\n", _HdmiSinkAvCap.ui1_sink_max_tmds_clock);
	...
	if (u4Res & SINK_480I)
		pr_info("[HDMI]SUPPORT 1440x480I 59.94hz\n");
	if (u4Res & SINK_480I_1440)
		pr_info("[HDMI]SUPPORT 2880x480I 59.94hz\n");
	if (u4Res & SINK_480P)
		pr_info("[HDMI]SUPPORT 720x480P 59.94hz\n");
	if (u4Res & SINK_480P_1440)
		pr_info("[HDMI]SUPPORT 1440x480P 59.94hz\n");
	if (u4Res & SINK_480P_2880)
		pr_info("[HDMI]SUPPORT 2880x480P 59.94hz\n");
	if (u4Res & SINK_720P60)
		pr_info("[HDMI]SUPPORT 1280x720P 59.94hz\n");
	if (u4Res & SINK_1080I60)
		pr_info("[HDMI]SUPPORT 1920x1080I 59.94hz\n");
	if (u4Res & SINK_1080P60)
		pr_info("[HDMI]SUPPORT 1920x1080P 59.94hz\n");

	if (u4Res & SINK_576I)
		pr_info("[HDMI]SUPPORT 1440x576I 50hz\n");
	if (u4Res & SINK_576I_1440)
		pr_info("[HDMI]SUPPORT 2880x576I 50hz\n");
	if (u4Res & SINK_576P)
		pr_info("[HDMI]SUPPORT 720x576P 50hz\n");
	if (u4Res & SINK_576P_1440)
		pr_info("[HDMI]SUPPORT 1440x576P 50hz\n");
	if (u4Res & SINK_576P_2880)
		pr_info("[HDMI]SUPPORT 2880x576P 50hz\n");
	if (u4Res & SINK_720P50)
		pr_info("[HDMI]SUPPORT 1280x720P 50hz\n");
	if (u4Res & SINK_1080I50)
		pr_info("[HDMI]SUPPORT 1920x1080I 50hz\n");
	if (u4Res & SINK_1080P50)
		pr_info("[HDMI]SUPPORT 1920x1080P 50hz\n");
	if (u4Res & SINK_1080P30)
		pr_info("[HDMI]SUPPORT 1920x1080P 30hz\n");
	if (u4Res & SINK_1080P24)
		pr_info("[HDMI]SUPPORT 1920x1080P 24hz\n");
	if (u4Res & SINK_1080P25)
		pr_info("[HDMI]SUPPORT 1920x1080P 25hz\n");

	u4Res =
	    (_HdmiSinkAvCap.ui4_sink_native_ntsc_resolution | _HdmiSinkAvCap.
	     ui4_sink_native_pal_resolution);
	pr_info("[HDMI]NTSC Native =%x\n", _HdmiSinkAvCap.ui4_sink_native_ntsc_resolution);
	pr_info("[HDMI]PAL Native =%x\n", _HdmiSinkAvCap.ui4_sink_native_pal_resolution);
	if (u4Res & SINK_480I)
		pr_info("[HDMI]Native resolution is 1440x480I 59.94hz\n");
	if (u4Res & SINK_480I_1440)
		pr_info("[HDMI]Native resolution is 2880x480I 59.94hz\n");
	if (u4Res & SINK_480P)
		pr_info("[HDMI]Native resolution is 720x480P 59.94hz\n");
	if (u4Res & SINK_480P_1440)
		pr_info("[HDMI]Native resolution is 1440x480P 59.94hz\n");
	if (u4Res & SINK_480P_2880)
		pr_info("[HDMI]Native resolution is 2880x480P 59.94hz\n");
	if (u4Res & SINK_720P60)
		pr_info("[HDMI]Native resolution is 1280x720P 59.94hz\n");
	if (u4Res & SINK_1080I60)
		pr_info("[HDMI]Native resolution is 1920x1080I 59.94hz\n");
	if (u4Res & SINK_1080P60)
		pr_info("[HDMI]Native resolution is 1920x1080P 59.94hz\n");
	if (u4Res & SINK_576I)
		pr_info("[HDMI]Native resolution is 1440x576I 50hz\n");
	if (u4Res & SINK_576I_1440)
		pr_info("[HDMI]Native resolution is 2880x576I 50hz\n");
	if (u4Res & SINK_576P)
		pr_info("[HDMI]Native resolution is 720x576P 50hz\n");
	if (u4Res & SINK_576P_1440)
		pr_info("[HDMI]Native resolution is 1440x576P 50hz\n");
	if (u4Res & SINK_576P_2880)
		pr_info("[HDMI]Native resolution is 2880x576P 50hz\n");
	if (u4Res & SINK_720P50)
		pr_info("[HDMI]Native resolution is 1280x720P 50hz\n");
	if (u4Res & SINK_1080I50)
		pr_info("[HDMI]Native resolution is 1920x1080I 50hz\n");
	if (u4Res & SINK_1080P50)
		pr_info("[HDMI]Native resolution is 1920x1080P 50hz\n");
	if (u4Res & SINK_1080P30)
		pr_info("[HDMI]Native resolution is 1920x1080P 30hz\n");
	if (u4Res & SINK_1080P24)
		pr_info("[HDMI]Native resolution is 1920x1080P 24hz\n");
	if (u4Res & SINK_1080P25)
		pr_info("[HDMI]Native resolution is 1920x1080P 25hz\n");


	pr_info("[HDMI]SUPPORT RGB\n");
	if (_HdmiSinkAvCap.ui2_sink_colorimetry & SINK_YCBCR_444) {
		pr_info("[HDMI]SUPPORT YCBCR 444\n");
	}
	if (_HdmiSinkAvCap.ui2_sink_colorimetry & SINK_YCBCR_422) {
		pr_info("[HDMI]SUPPORT YCBCR 422\n");
	}
	if (_HdmiSinkAvCap.ui2_sink_colorimetry & SINK_XV_YCC709) {
		pr_info("[HDMI]SUPPORT xvYCC 709\n");
	}
	if (_HdmiSinkAvCap.ui2_sink_colorimetry & SINK_XV_YCC601) {
		pr_info("[HDMI]SUPPORT xvYCC 601\n");
	}
	if (_HdmiSinkAvCap.ui2_sink_colorimetry & SINK_METADATA0) {
		pr_info("[HDMI]SUPPORT metadata P0\n");
	}
	if (_HdmiSinkAvCap.ui2_sink_colorimetry & SINK_METADATA1) {
		pr_info("[HDMI]SUPPORT metadata P1\n");
	}
	if (_HdmiSinkAvCap.ui2_sink_colorimetry & SINK_METADATA2) {
		pr_info("[HDMI]SUPPORT metadata P2\n");
	}


	if (_HdmiSinkAvCap.e_sink_ycbcr_color_bit & HDMI_SINK_DEEP_COLOR_10_BIT)
		pr_info("[HDMI]SUPPORT YCBCR 30 Bits Deep Color\n");
	if (_HdmiSinkAvCap.e_sink_ycbcr_color_bit & HDMI_SINK_DEEP_COLOR_12_BIT)
		pr_info("[HDMI]SUPPORT YCBCR 36 Bits Deep Color\n");
	if (_HdmiSinkAvCap.e_sink_ycbcr_color_bit & HDMI_SINK_DEEP_COLOR_16_BIT)
		pr_info("[HDMI]SUPPORT YCBCR 48 Bits Deep Color\n");
	if (_HdmiSinkAvCap.e_sink_ycbcr_color_bit == HDMI_SINK_NO_DEEP_COLOR)
		pr_info("[HDMI]Not SUPPORT YCBCR Deep Color\n");

	if (_HdmiSinkAvCap.e_sink_rgb_color_bit & HDMI_SINK_DEEP_COLOR_10_BIT)
		pr_info("[HDMI]SUPPORT RGB 30 Bits Deep Color\n");
	if (_HdmiSinkAvCap.e_sink_rgb_color_bit & HDMI_SINK_DEEP_COLOR_12_BIT)
		pr_info("[HDMI]SUPPORT RGB 36 Bits Deep Color\n");
	if (_HdmiSinkAvCap.e_sink_rgb_color_bit & HDMI_SINK_DEEP_COLOR_16_BIT)
		pr_info("[HDMI]SUPPORT RGB 48 Bits Deep Color\n");
	if (_HdmiSinkAvCap.e_sink_rgb_color_bit == HDMI_SINK_NO_DEEP_COLOR)
		pr_info("[HDMI]Not SUPPORT RGB Deep Color\n");

	if (_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_LPCM)
		pr_info("[HDMI]SUPPORT LPCM\n");
	if (_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_AC3)
		pr_info("[HDMI]SUPPORT AC3 Decode\n");
	if (_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_MPEG1)
		pr_info("[HDMI]SUPPORT MPEG1 Decode\n");
	if (_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_MP3)
		pr_info("[HDMI]SUPPORT AC3 Decode\n");
	if (_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_MPEG2)
		pr_info("[HDMI]SUPPORT MPEG2 Decode\n");
	if (_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_AAC)
		pr_info("[HDMI]SUPPORT AAC Decode\n");
	if (_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_DTS)
		pr_info("[HDMI]SUPPORT DTS Decode\n");
	if (_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_ATRAC)
		pr_info("[HDMI]SUPPORT ATRAC Decode\n");
	if (_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_DSD)
		pr_info("[HDMI]SUPPORT SACD DSD Decode\n");
	if (_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_DOLBY_PLUS)
		pr_info("[HDMI]SUPPORT Dolby Plus Decode\n");
	if (_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_DTS_HD)
		pr_info("[HDMI]SUPPORT DTS HD Decode\n");
	if (_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_MAT_MLP)
		pr_info("[HDMI]SUPPORT MAT MLP Decode\n");
	if (_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_DST)
		pr_info("[HDMI]SUPPORT SACD DST Decode\n");
	if (_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_WMA)
		pr_info("[HDMI]SUPPORT  WMA Decode\n");
	...
	if (_HdmiSinkAvCap.ui1_sink_pcm_ch_sampling[0] != 0) {
		...
		pr_info("[HDMI]SUPPORT PCM Max 2CH, Fs is: %s\n", &cDstStr[0]);
	...
	if (_HdmiSinkAvCap.ui1_sink_pcm_ch_sampling[4] != 0) {
		...
		pr_info("[HDMI]SUPPORT PCM Max 6CH Fs is: %s\n", &cDstStr[0]);
	...
	if (_HdmiSinkAvCap.ui1_sink_pcm_ch_sampling[5] != 0) {
		...
		pr_info("[HDMI]SUPPORT PCM Max 7CH Fs is: %s\n", &cDstStr[0]);
	...
	if (_HdmiSinkAvCap.ui1_sink_pcm_ch_sampling[6] != 0) {
		...
		pr_info("[HDMI]SUPPORT PCM Max 8CH, FS is: %s\n", &cDstStr[0]);

	}

	if (_HdmiSinkAvCap.ui1_sink_spk_allocation & SINK_AUDIO_FL_FR)
		pr_info("[HDMI]Speaker FL/FR allocated\n");
	if (_HdmiSinkAvCap.ui1_sink_spk_allocation & SINK_AUDIO_LFE)
		pr_info("[HDMI]Speaker LFE allocated\n");
	if (_HdmiSinkAvCap.ui1_sink_spk_allocation & SINK_AUDIO_FC)
		pr_info("[HDMI]Speaker FC allocated\n");
	if (_HdmiSinkAvCap.ui1_sink_spk_allocation & SINK_AUDIO_RL_RR)
		pr_info("[HDMI]Speaker RL/RR allocated\n");
	if (_HdmiSinkAvCap.ui1_sink_spk_allocation & SINK_AUDIO_RC)
		pr_info("[HDMI]Speaker RC allocated\n");
	if (_HdmiSinkAvCap.ui1_sink_spk_allocation & SINK_AUDIO_FLC_FRC)
		pr_info("[HDMI]Speaker FLC/FRC allocated\n");
	if (_HdmiSinkAvCap.ui1_sink_spk_allocation & SINK_AUDIO_RLC_RRC)
		pr_info("[HDMI]Speaker RLC/RRC allocated\n");

	pr_info("[HDMI]HDMI edid support content type =%x\n", _HdmiSinkAvCap.ui1_sink_content_cnc);
	pr_info("[HDMI]Lip Sync Progressive audio latency = %d\n",
		_HdmiSinkAvCap.ui1_sink_p_audio_latency);
	pr_info("[HDMI]Lip Sync Progressive video latency = %d\n",
		_HdmiSinkAvCap.ui1_sink_p_video_latency);
	if (_HdmiSinkAvCap.ui1_sink_i_latency_present) {
		pr_info("[HDMI]Lip Sync Interlace audio latency = %d\n",
			_HdmiSinkAvCap.ui1_sink_i_audio_latency);
		pr_info("[HDMI]Lip Sync Interlace video latency = %d\n",
			_HdmiSinkAvCap.ui1_sink_i_video_latency);
	}

	if (_HdmiSinkAvCap.ui1_sink_support_ai == 1)
		pr_info("[HDMI]Support AI\n");
	else
		pr_info("[HDMI]Not Support AI\n");

	if (_HdmiSinkAvCap.ui1_Video_Input_Definitation == 1)
		pr_info("[HDMI]Input is a Digital Video Signal Interface\n");
	else
		pr_info("[HDMI]Input is an Analog Video Signal Interface\n");

	pr_info("[HDMI]Monitor Max horizontal size = %d\n",
		_HdmiSinkAvCap.ui1_Display_Horizontal_Size);
	pr_info("[HDMI]Monitor Max vertical size = %d\n", _HdmiSinkAvCap.ui1_Display_Vertical_Size);


	if (_HdmiSinkAvCap.b_sink_hdmi_video_present == TRUE)
		pr_info("[HDMI]HDMI_Video_Present\n");
	else
		pr_info("[HDMI]No HDMI_Video_Present\n");

	if (_HdmiSinkAvCap.b_sink_3D_present == TRUE)
		pr_info("[HDMI]3D_present\n");
	else
		pr_info("[HDMI]No 3D_present\n");
...
void hdmi_AppGetEdidInfo(HDMI_EDID_INFO_T *pv_get_info)
{
	...
	pv_get_info->ui4_hdmi_pcm_ch_type =
	    ((_HdmiSinkAvCap.ui1_sink_pcm_ch_sampling[0]) | (_HdmiSinkAvCap.
							     ui1_sink_pcm_ch_sampling[4] << 8) |
	     (_HdmiSinkAvCap.ui1_sink_pcm_ch_sampling[6] << 16));
	pv_get_info->ui4_hdmi_pcm_ch3ch4ch5ch7_type =
	    ((_HdmiSinkAvCap.ui1_sink_pcm_ch_sampling[1]) | (_HdmiSinkAvCap.
							     ui1_sink_pcm_ch_sampling[2] << 8) |
	     (_HdmiSinkAvCap.ui1_sink_pcm_ch_sampling[3] << 16) | (_HdmiSinkAvCap.
								   ui1_sink_pcm_ch_sampling[5] <<
								   24));
	...
	pv_get_info->ui1_Video_Input_Definitation =
		_HdmiSinkAvCap.ui1_Video_Input_Definitation;
