...
#define ENABLE_CEC_IRQ()	(hdmi_write(0x1500, 0x20))
#define DISABLE_CEC_IRQ()	(hdmi_write(0x1500, 0x0))
#define CLR_CEC_IRQ()	    (hdmi_write(0x1504, 0x20))
...
{
	unsigned int u4Data;
	unsigned short u2HighData, u2LowData, addr;

	u2LowData = 0;
	u2HighData = 0;
	addr = HDMICEC_BASE + u2Reg;
	pwrap_read((unsigned int)addr, (unsigned int *)(&u2LowData));
	addr = addr + 2;
	pwrap_read((unsigned int)addr, (unsigned int *)(&u2HighData));

	u4Data = ((unsigned int)u2LowData) | ((unsigned int)(u2HighData << 16));
...
{
	unsigned short u2HighData, u2LowData, addr;
	u2LowData = u4Data & 0xFFFF;
	u2HighData = (u4Data >> 16) & 0xFFFF;
	addr = HDMICEC_BASE + u2Reg;
	pwrap_write((unsigned int)addr, (unsigned int)u2LowData);
	addr = addr + 2;
	pwrap_write((unsigned int)addr, (unsigned int)u2HighData);
...
static void _CEC_Receiving(void)
{
	...
	if (rxlen == 0x3) {
		...
		HDMI_CEC_LOG("[CEC] invalid rx length occurs\n");
		/* assert(0); */
	}
	/* for opcode */
	if ((*size) == 1) {
	...
	for (i = 0; i < rxlen; i++) {
		...
		if ((*size) > 15) {
			HDMI_CEC_LOG("[CEC] Receive Data Length is wrong !\n");
			break;
		} else {
			ASSERT((*size) >= 2);
	...
	if (is_d_eom) {
		...
		if (ret == FALSE) {
			...
			HDMI_CEC_LOG("[CEC] cec rx buffer overflow\n");
			/* ASSERT(0); */
		}
	}
}
...
static unsigned char _CEC_SendRemainingDataBlocks(void)
{
	...
	if (IS_TX_FSM_FAIL() | (TX_FAIL_RECORD() > 0)) {
		HDMI_CEC_LOG("[CEC] Detect TX FAIL in %s\n", __func__);
		return 3;
		/* RESET_HW_TX(); */
		/* ASSERT(0); */
	}
	...
	FILL_TX_DATA(data);
	HDMI_CEC_LOG("[CEC] TRIGGER_TX_HW in %s, size: %x\n", __func__, size);
	...
	if (IsCECStatus(STATE_TXING_FRAME)) {
		...
		if (IsCECStatus(STATE_HW_RETX)) {
			...
			if ((TX_FAIL_RECORD() != 0)) {
				...
				if (IS_TX_FSM_FAIL()) {
					HDMI_CEC_LOG("[CEC] TX NO ACK\n");
				} else {
					HDMI_CEC_LOG("[CEC] other TX error\n");
				}
				HDMI_CEC_LOG("[CEC] H ACK: %x, D ACK: %x\n", GET_FOLLOWER_H_ACK(),
					     GET_FOLLOWER_D_ACK());
			...
			if (IS_TX_FSM_IDLE() && IS_TX_FINISH()) {
				...
				ClrCECStatus(STATE_TXING_FRAME);
				HDMI_CEC_LOG("[CEC] TX is COMPLETED with: H ACK: %x and D ACK %x\n",
...
static void PrintFrameDescription(CEC_FRAME_DESCRIPTION *frame)
{
	...
	HDMI_CEC_LOG("[CEC] >>>>>>>>>>>>>>>>>>>>>>>>\n");
	HDMI_CEC_LOG("[CEC] frame description:\n");
	HDMI_CEC_LOG("[CEC] size: 0x%x\n", frame->size);
	HDMI_CEC_LOG("[CEC] sendidx: 0x%x\n", frame->sendidx);
	HDMI_CEC_LOG("[CEC] reTXcnt: 0x%x\n", frame->reTXcnt);
	HDMI_CEC_LOG("[CEC] initiator: 0x%x\n", frame->blocks.header.initiator);
	HDMI_CEC_LOG("[CEC] destination: 0x%x\n", frame->blocks.header.destination);
	if (frame->size > 1) {
		HDMI_CEC_LOG("[CEC] opcode: 0x%x\n", frame->blocks.opcode);
	}
...
static void _CEC_TX_Queue_Loop(void)
{
	...
	if (IsCECStatus(STATE_TXING_FRAME)) {
		...
		if (IsCECStatus(STATE_TX_FRAME_SUCCESS)) {
			HDMI_CEC_LOG("[CEC] This message is successful\n");
...
return;
			...
			HDMI_CEC_LOG("[CEC] This message is failed: %d\n", frame->reTXcnt);
			frame->reTXcnt++;
			frame->sendidx = 0;
			ClrCECStatus(STATE_TX_NOACK);
			...
			if (frame->reTXcnt == RETX_MAX_CNT) {
				_u1TxFailCause = FAIL_NONE;
				HDMI_CEC_LOG("[CEC] ReTX reach MAX\n");
				_CEC_TX_Dequeue();
			}
		}
	} else
		/* if the tx is not active, send the next message */
	{
		...
		HDMI_CEC_LOG("[CEC] Send a new message\n");
...
static unsigned char CEC_frame_validation(CEC_FRAME_DESCRIPTION *frame)
{
	...
	if (i1ret == FALSE) {
		HDMI_CEC_LOG("[CEC] receive invalid frame: %x\n", frame->blocks.opcode);
		PrintFrameDescription(frame);
	}
	return i1ret;
}
...
static unsigned char check_and_init_tx_frame(CEC_FRAME_DESCRIPTION *frame)
{
	...
	if ((frame->size > CEC_MAX_MESG_SIZE) || (frame->size == 0)) {
		HDMI_CEC_LOG("[CEC] Tx fram size is not correct\n");
		ret = 0x01;
	}
	/* valid tx frame */
	if (ret == 0x00) {
...
unsigned char _CEC_TX_Enqueue(CEC_FRAME_DESCRIPTION *frame)
{
	HDMI_CEC_FUNC();
	if (frame->size == 1) {
		HDMI_CEC_LOG("[CEC] Polling LA = 0x%x\n",
			     (unsigned char)((frame->blocks.header.initiator << 4) | frame->blocks.
					     header.destination));
	} else {
		HDMI_CEC_LOG("[CEC] Opcode = 0x%x, size = 0x%x\n", frame->blocks.opcode,
			     frame->size);
	}
	...
	if (IS_TX_Q_FULL()) {
		HDMI_CEC_LOG("[CEC] Tx queue is full\n");
		return 0x01;
	}
...
/* /////////////////////Test//////////////////////////////////// */
		...
		_CEC_TX_Enqueue(&CTSTestFrame);
		HDMI_CEC_LOG("[CEC] CTS Send: OPCODE_FEATURE_ABORT\n");
		...
		_CEC_TX_Enqueue(&CTSTestFrame);
		HDMI_CEC_LOG("[CEC] CTS Send: OPCODE_REPORT_PHYSICAL_ADDRESS\n");
	}
}
...
static void CEC_rx_msg_notify(unsigned char u1rxmode)
{
	...
	while (1) {
		...
		if (CEC_frame_validation(frame)) {
			HDMI_CEC_LOG("[CEC} Receive message\n");
			PrintFrameDescription(frame);
...
unsigned char hdmi_cec_isrprocess(unsigned char u1rxmode)
{
	...
	if (HW_RX_HEADER_ARRIVED()) {
		...
		HDMI_CEC_LOG("[CEC]u1ReceivedDst = 0x%08x\n", u1ReceivedDst);
		if (u1rxmode == CEC_CTS_MODE)
		...
		if ((u1ReceivedDst == _rCECLaAddr.aui1_la[0])
			...
			HDMI_CEC_LOG("[CEC] RX:H\n");
			if (IsCECStatus(STATE_RX_GET_NEW_HEADER)) {
				HDMI_CEC_LOG("[CEC] Lost EOM:1\n");
				SetCECErrorFlag(ERR_RX_LOST_EOM);
			}
			SetCECStatus(STATE_RX_GET_NEW_HEADER);
		} else {
			ClrCECStatus(STATE_RX_GET_NEW_HEADER);
			HDMI_CEC_LOG("[CEC] RX:H False\n");
		}
	}
	if (HW_RX_DATA_ARRIVED()) {
		HDMI_CEC_LOG("[CEC] RX:D\n");
		_CEC_Receiving();
	}
	if (IS_INT_OV()) {
		HDMI_CEC_LOG("[CEC] Overflow\n");
		CLR_INT_OV();
	...
	if (IsCECStatus(STATE_TXING_FRAME)) {
		if (IS_INT_UN()) {
			HDMI_CEC_LOG("[CEC] Underrun\n");
			CLR_INT_UN();
			SetCECErrorFlag(ERR_TX_UNDERRUN);
		}
		if (IS_INT_LOW()) {
			HDMI_CEC_LOG("[CEC] Buffer Low\n");
			CLR_INT_LOW();
			if (!IS_INT_RB_RDY()) {
				HDMI_CEC_LOG("[CEC] FW is slow to trigger the following blocks\n");
				SetCECErrorFlag(ERR_TX_BUFFER_LOW);
			}
		}
		if (IS_INT_RB_ENABLE() && IS_TX_DATA_TAKEN()) {
			HDMI_CEC_LOG("[CEC] TX Data Taken\n");
		...
		if (IS_INT_FAIL_ENABLE() && (TX_FAIL_RECORD() != 0)) {
			...
			if (TX_FAIL_MAX() | IS_TX_FSM_FAIL()) {
				HDMI_CEC_LOG("[CEC] TX Fail: %x\n", TX_FAIL_RECORD());
				HDMI_CEC_LOG("[CEC] TX Fail MAX\n");
			} else {
				HDMI_CEC_LOG("[CEC] TX Fail: %x\n", TX_FAIL_RECORD());
			}
		}
		HDMI_CEC_LOG("[CEC] TX HW FSM: %x\n", TX_FSM_STATUS());
	}
