...
/* Collecting HDCP metric for Sloane */
#ifdef CONFIG_AMAZON_METRICS_LOG
#include <linux/metricslog.h>
#define HDMI_METRIC_TAG "kernel_hdmi"
#define METRICS_HDCP_DATA_LEN 128
#define HDMI_EDID_SUPPORT_4k_RESOLUTION 0x01
#define HDMI_EDID_SUPPORT_1080_RESOLUTION 0x02
#define RESMASK (SINK_2160P_23_976HZ || SINK_2160P_24HZ \
		|| SINK_2160P_25HZ || SINK_2160P_29_97HZ \
		|| SINK_2160P_30HZ || SINK_2161P_24HZ)
extern HDMI_SINK_AV_CAP_T _HdmiSinkAvCap;
static unsigned char resFlag;
#endif
...
unsigned char fgCompareRi(void)
{
	...
	if (bTemp == HDCP_RI_COUNT) {
		...
		/* Log only if TV is powered on */
		if (hdmi_powerenable)
			pr_err("hdmi hdcp Ri compare fail bHDCPBuf[0]=0x%x,bHDCPBuf[1]=0x%x,bHDCPBuf[2]=0x%x,bHDCPBuf[3]=0x%x\n",
				bHDCPBuf[0], bHDCPBuf[1], bHDCPBuf[2], bHDCPBuf[3]);
		return FALSE;
	}
...
void HdcpService(HDCP_CTRL_STATE_T e_hdcp_state)
{
...
#ifdef CONFIG_AMAZON_METRICS_LOG
	char logcat_hdcp_data[METRICS_HDCP_DATA_LEN];
	unsigned int u4Res = _HdmiSinkAvCap.ui4_sink_hdmi_4k2kvic;
#endif
...
#ifdef CONFIG_AMAZON_METRICS_LOG
			memset(logcat_hdcp_data, 0, sizeof(logcat_hdcp_data));
			snprintf(logcat_hdcp_data, METRICS_HDCP_DATA_LEN,
				"HDMIHDCP:def:%s_hdcp_1x=1;CT;1:NR",
				((u4Res & RESMASK) ? "4K" : "1k"));

			log_to_metrics(ANDROID_LOG_INFO, HDMI_METRIC_TAG, logcat_hdcp_data);
#endif
	...
	case HDCP2_AUTHEN_CHECK:
		...
		if (readvalue & HDCP2X_AUTH_DONE_INT_STA) {
			...
			_bReAuthCnt = 0;
		} else if (((readvalue & HDCP2X_AUTH_FAIL_INT_STA) && (_bReAuthCnt != 0)) || (_bReAuthCnt > REPEAT_CHECK_AUTHHDCP_VALUE)) {
			HDMI_HDCP_LOG("hdcp2.2 authentication fail\n");
			vHDCP2InitAuth();

/* Sloane metric collection */
#ifdef CONFIG_AMAZON_METRICS_LOG
			/* HDCP metrics */
			memset(logcat_hdcp_data, 0, sizeof(logcat_hdcp_data));
			snprintf(logcat_hdcp_data, METRICS_HDCP_DATA_LEN,
				"HDMIHDCP:def:auth_retry_count=%d;CT;1:NR", _bReAuthCnt);
	...
	case HDCP2_ENCRYPTION:
		...
		if (i4SharedInfo(SI_HDMI_RECEIVER_STATUS) != HDMI_PLUG_IN_AND_SINK_POWER_ON) {
			...
			memset(logcat_hdcp_data, 0, sizeof(logcat_hdcp_data));
			snprintf(logcat_hdcp_data, METRICS_HDCP_DATA_LEN,
				"HDMIHDCP:def:%s_hdcp_2.2=1;CT;1:NR",
				((u4Res & RESMASK) ? "4K" : "1k"));
...
#ifdef CONFIG_AMAZON_METRICS_LOG
	if ((e_hdcp_state == HDCP_WAIT_RI) || (e_hdcp_state == HDCP2_ENCRYPTION)) {
		bTemp = resFlag;
		resFlag = ((u4Res & RESMASK) ? HDMI_EDID_SUPPORT_4k_RESOLUTION :
				HDMI_EDID_SUPPORT_1080_RESOLUTION);
		if (bTemp != resFlag) {
			memset(logcat_hdcp_data, 0, sizeof(logcat_hdcp_data));
			snprintf(logcat_hdcp_data, METRICS_HDCP_DATA_LEN,
			"HDMIRESOLUTION:def:%s_TV=1;CT;1:NR",
			((u4Res & RESMASK) ? "4K" : "1k"));
