...
{
	/*vWriteIoPadMsk(IO_PAD_PD, 0, IO_PAD_SCL_PD);
	vWriteIoPadMsk(IO_PAD_PD, 0, IO_PAD_SDA_PD);*/
	vWriteIoPadMsk(IO_PAD_PD, 0, IO_PAD_HOT_PLUG_PD);
	/*vWriteIoPadMsk(IO_PAD_EN, IO_PAD_SCL_EN, IO_PAD_SCL_EN);
	vWriteIoPadMsk(IO_PAD_EN, IO_PAD_SDA_EN, IO_PAD_SDA_EN);*/
...
void vSetHDMITxPLL(unsigned char bResIndex, unsigned char bdeepmode)
{
	...
	if ((u4Feq == 2) && (bdeepmode != HDMI_NO_DEEP_COLOR)) {
		...
		vWriteIoPllMsk(HDMI_CON4, (0xa << PRD_IBIAS_CLK_SHIFT), RG_HDMITX_PRD_IBIAS_CLK);
		vWriteIoPllMsk(HDMI_CON4, (0xa << PRD_IBIAS_D2_SHIFT), RG_HDMITX_PRD_IBIAS_D2);
		vWriteIoPllMsk(HDMI_CON4, (0xa << PRD_IBIAS_D1_SHIFT), RG_HDMITX_PRD_IBIAS_D1);
		vWriteIoPllMsk(HDMI_CON4, (0xa << PRD_IBIAS_D0_SHIFT), RG_HDMITX_PRD_IBIAS_D0);
...
void vShowHpdRsenStatus(void)
{
	if (bCheckPordHotPlug(HOTPLUG_MODE) == TRUE)
		HDMI_PLUG_LOG("[HDMI]HPD ON\n");
	else
		HDMI_PLUG_LOG("[HDMI]HPD OFF\n");

	if (bCheckPordHotPlug(PORD_MODE) == TRUE)
		HDMI_PLUG_LOG("[HDMI]RSEN ON\n");
	else
		HDMI_PLUG_LOG("[HDMI]RSEN OFF\n");

	if (i4SharedInfo(SI_HDMI_RECEIVER_STATUS) == HDMI_PLUG_IN_ONLY)
		HDMI_PLUG_LOG("[HDMI]SI_HDMI_RECEIVER_STATUS = HDMI_PLUG_IN_ONLY\n");
	else if (i4SharedInfo(SI_HDMI_RECEIVER_STATUS) == HDMI_PLUG_IN_AND_SINK_POWER_ON)
		HDMI_PLUG_LOG("[HDMI]SI_HDMI_RECEIVER_STATUS = HDMI_PLUG_IN_AND_SINK_POWER_ON\n");
	else if (i4SharedInfo(SI_HDMI_RECEIVER_STATUS) == HDMI_PLUG_OUT)
		HDMI_PLUG_LOG("[HDMI]SI_HDMI_RECEIVER_STATUS = HDMI_PLUG_OUT\n");
	else
		HDMI_PLUG_LOG("[HDMI]SI_HDMI_RECEIVER_STATUS error\n");
	...
	if ((i4SharedInfo(SI_HDMI_RECEIVER_STATUS) == HDMI_PLUG_IN_ONLY) ||
		...
		if (hdcp2_version_flag == TRUE)
			HDMI_PLUG_LOG("[HDMI]support hdcp2.2\n");
		else
			HDMI_PLUG_LOG("[HDMI]support hdcp1.x\n");
	} else {
		HDMI_PLUG_LOG("[HDMI] hdmi not plugged in. hdcp version unknown\n");
	}
	HDMI_PLUG_LOG("[HDMI]hdmi2_force_output=%d\n", hdmi2_force_output);
...
{
	HDMI_PLUG_LOG("[HDMI]HDMI output resolution = %s\n", szHdmiResStr[_stAvdAVInfo.e_resolution]);	/*  */
...
{
	if (vIsDviMode())
		HDMI_PLUG_LOG("[HDMI]DVI Mode\n");
	else
		HDMI_PLUG_LOG("[HDMI]HDMI Mode\n");
...
void vShowDeepColor(void)
{
	...
	if (_stAvdAVInfo.e_deep_color_bit == HDMI_NO_DEEP_COLOR)
		HDMI_PLUG_LOG("[HDMI]HDMI output deepcolor = HDMI_NO_DEEP_COLOR\n");
	else if (_stAvdAVInfo.e_deep_color_bit == HDMI_DEEP_COLOR_10_BIT)
		HDMI_PLUG_LOG("[HDMI]HDMI output deepcolor = HDMI_DEEP_COLOR_10_BIT\n");
	else if (_stAvdAVInfo.e_deep_color_bit == HDMI_DEEP_COLOR_12_BIT)
		HDMI_PLUG_LOG("[HDMI]HDMI output deepcolor = HDMI_DEEP_COLOR_12_BIT\n");
	else if (_stAvdAVInfo.e_deep_color_bit == HDMI_DEEP_COLOR_16_BIT)
		HDMI_PLUG_LOG("[HDMI]HDMI output deepcolor = HDMI_DEEP_COLOR_16_BIT\n");
	else
		HDMI_PLUG_LOG("[HDMI]HDMI output deepcolor error\n");
...
void vShowColorSpace(void)
{
	if (_stAvdAVInfo.e_video_color_space == HDMI_RGB)
		HDMI_PLUG_LOG("[HDMI]HDMI output colorspace = HDMI_RGB\n");
	else if (_stAvdAVInfo.e_video_color_space == HDMI_RGB_FULL)
		HDMI_PLUG_LOG("[HDMI]HDMI output colorspace = HDMI_RGB_FULL\n");
	else if (_stAvdAVInfo.e_video_color_space == HDMI_YCBCR_444)
		HDMI_PLUG_LOG("[HDMI]HDMI output colorspace = HDMI_YCBCR_444\n");
		...
		HDMI_PLUG_LOG("[HDMI]HDMI output colorspace = HDMI_XV_YCC\n");
	else
		HDMI_PLUG_LOG("[HDMI]HDMI output colorspace error\n");

}

void vShowInforFrame(void)
{
	HDMI_PLUG_LOG("====================Audio inforFrame Start ====================================\n");
	...
	HDMI_PLUG_LOG("====================Audio inforFrame End ======================================\n");

	HDMI_PLUG_LOG("====================AVI inforFrame Start ====================================\n");
	...
	HDMI_PLUG_LOG("====================AVI inforFrame End ======================================\n");

	HDMI_PLUG_LOG("====================SPD inforFrame Start ====================================\n");
	...
	HDMI_PLUG_LOG("====================SPD inforFrame End ======================================\n");

	if (fgIsAcpEnable()) {
		HDMI_PLUG_LOG
		    ("====================ACP inforFrame Start ====================================\n");
		HDMI_PLUG_LOG("Acp type =0x%x\n", _bAcpType);

		if (_bAcpType == 0) {
			HDMI_PLUG_LOG("Generic Audio\n");
			HDMI_PLUG_LOG("Data Byte (1~8)= 0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x\n",
			...
			HDMI_PLUG_LOG("IEC 60958-Identified Audio\n");
			HDMI_PLUG_LOG("Data Byte (1~8)= 0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x\n",
			       _bAcpData[0], _bAcpData[1], _bAcpData[2], _bAcpData[3], _bAcpData[4],
			       _bAcpData[5], _bAcpData[6], _bAcpData[7]);
			HDMI_PLUG_LOG("Data Byte (9~16)= 0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x\n",
			...
			HDMI_PLUG_LOG("DVD Audio\n");
			HDMI_PLUG_LOG("DVD-AUdio_TYPE_Dependent Generation = 0x%x\n", _bAcpData[0]);
			HDMI_PLUG_LOG("Copy Permission = 0x%x\n", (_bAcpData[1] >> 6) & 0x03);
			HDMI_PLUG_LOG("Copy Number = 0x%x\n", (_bAcpData[1] >> 3) & 0x07);
			HDMI_PLUG_LOG("Quality = 0x%x\n", (_bAcpData[1] >> 1) & 0x03);
			HDMI_PLUG_LOG("Transaction = 0x%x\n", _bAcpData[1] & 0x01);

		} else if (_bAcpType == 3) {
			HDMI_PLUG_LOG("SuperAudio CD\n");
			HDMI_PLUG_LOG("CCI_1 Byte (1~8)= 0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x\n",
			       _bAcpData[0], _bAcpData[1], _bAcpData[2], _bAcpData[3], _bAcpData[4],
			       _bAcpData[5], _bAcpData[6], _bAcpData[7]);
			HDMI_PLUG_LOG
...
{
	HDMI_PLUG_LOG("[HDMI]HDMI output audio Channel Number =%d\n", _stAvdAVInfo.ui1_aud_out_ch_number);
	HDMI_PLUG_LOG("[HDMI]HDMI output Audio Fs = %s\n", cHdmiAudFsStr[_stAvdAVInfo.e_hdmi_fs]);
	HDMI_PLUG_LOG("[HDMI]HDMI MCLK =%d\n", _stAvdAVInfo.u1HdmiI2sMclk);
	HDMI_PLUG_LOG("[HDMI]HDMI output ACR N= %d, CTS = %d\n", u4ReadNValue(), u4ReadCtsValue());
}

void vShowHdmiDrmHdcpStatus(void)
{
	HDMI_PLUG_LOG("[HDMI]DrmHdcp _bflagvideomute =%d\n", _bflagvideomute);
	HDMI_PLUG_LOG("[HDMI]DrmHdcp _bflagaudiomute =%d\n", _bflagaudiomute);
	HDMI_PLUG_LOG("[HDMI]DrmHdcp _bsvpvideomute =%d\n", _bsvpvideomute);
	HDMI_PLUG_LOG("[HDMI]DrmHdcp _bsvpaudiomute =%d\n", _bsvpaudiomute);
	HDMI_PLUG_LOG("[HDMI]DrmHdcp _bNeedSwHdcp =%d\n", _bHdcpOff);
	HDMI_PLUG_LOG("[HDMI]DrmHdcp _bHdcpOff =%d\n", _bHdcpOff);
}
...
void vCheckHDMICLKPIN(void)
{
	...
	for (i = 0; i < 2; i++) {
		...
		HDMI_PLUG_LOG("[HDMI]sys clk = %d.%dM\n", (u4Data / 1024),
	...
	for (i = 0; i < 2; i++) {
		...
		msleep(60);
		u4Data = ((dReadHdmiTOPCK(0x224) & (0x0000ffff))) * 26;
		HDMI_PLUG_LOG("[HDMI]tvd clk = %d.%dM\n", (u4Data / 1024),
		       (u4Data - ((u4Data / 1024) * 1024)));
	}
	...
	for (i = 0; i < 2; i++) {
		...
		HDMI_PLUG_LOG("[HDMI]hdmi pin clk = %d.%dM\n", (u4Data / 1024),
		       (u4Data - ((u4Data / 1024) * 1024)));
	}
...
void vConfigAudio2Setting(void)
{
	vWriteHdmiTOPCKMsk(CLK_AUDDIV_4, CLK_MUX_POWERUP, CLK_MUX_POWERDOWN);
	if (_stAvdAVInfo.e_hdmi_aud_in == SV_SPDIF) {
		vWriteHdmiTOPCKMsk(CLK_AUDDIV_4, CLK_SEL_INPUT_SPDIF, CLK_SEL_INPUT_SPDIF);
