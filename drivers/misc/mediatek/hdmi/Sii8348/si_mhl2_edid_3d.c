...
/* TODO: FD, TBI */
				...
				MHL_TX_EDID_INFO(mhl_edid_3d_data->dev_context,
						 ...
						 (int)p_desc->
						 range_limits.min_horizontal_rate_in_KHz);
				MHL_TX_EDID_INFO(mhl_edid_3d_data->dev_context,
						 "Max Horizontal Rate in KHz: %d\n",
						 (int)p_desc->
						 range_limits.max_horizontal_rate_in_KHz);
				MHL_TX_EDID_INFO(mhl_edid_3d_data->dev_context,
						 "Max Supported pixel clock rate in MHz/10: %d\n",
						 (int)p_desc->
						 range_limits.max_pixel_clock_in_MHz_div_10);
				MHL_TX_EDID_INFO(mhl_edid_3d_data->dev_context,
		...
		*p_is_timing = 1;
		...
		tmp_word =
		    ...
		    256 *
		    p_desc->dtd.
		    hs_offset_hs_pulse_width_vs_offset_vs_pulse_width.horz_sync_pulse_width_9_8;
		MHL_TX_EDID_INFO(mhl_edid_3d_data->dev_context,
		...
		tmp_word =
		    ...
		    16 *
		    p_desc->dtd.
		    hs_offset_hs_pulse_width_vs_offset_vs_pulse_width.vert_sync_offset_5_4;
		MHL_TX_EDID_INFO(mhl_edid_3d_data->dev_context,
		...
		tmp_word = p_desc->dtd.vert_sync_offset_width.vert_sync_pulse_width_3_0
		    ...
		    16 *
		    p_desc->dtd.
		    hs_offset_hs_pulse_width_vs_offset_vs_pulse_width.vert_sync_pulse_width_5_4;
		MHL_TX_EDID_INFO(mhl_edid_3d_data->dev_context,
...
static void si_mhl_tx_prune_edid(edid_3d_data_p mhl_edid_3d_data)
{
	...
	si_mhl_tx_prune_dtd_list(mhl_edid_3d_data,
				 ...
				 p_EDID_block_0->detailed_timing_descriptors[0], dtd_limit);
	DUMP_EDID_BLOCK(0, p_EDID_block_0, sizeof(*p_EDID_block_0))
	    /* no semicolon needed here */
	    ...
	    dtd_limit =
	    (uint8_t) p_CEA_extension->version_u.version3.
	    misc_support.total_number_detailed_timing_descriptors_in_entire_EDID;
	...
	if (mhl_edid_3d_data->parse_data.p_3d_mask) {
		...
		uint32_t mask32;
		int8_t index =
		    mhl_edid_3d_data->parse_data.p_video_data_blocks_2d[0]->header.
		    fields.length_following_header - 1;
		...
		for (; index >= 8; mask32 >>= 1, lower_mask >>= 1, --index) {
			if (0 ==
			    ...
			    p_video_data_blocks_2d[0]->short_descriptors[index].VIC) {
	...
	if (mhl_edid_3d_data->parse_data.p_three_d) {
		 ...
		 */
		{
			...
			for (i = 0, this_bit = 1; i < limit_2D_VIC; ++i, this_bit <<= 1) {
				...
				VIC =
				    mhl_edid_3d_data->parse_data.
				    p_video_data_blocks_2d[0]->short_descriptors[i].VIC;
				if (0 == VIC) {
		...
		while (p_3D_u.p_as_bytes < mhl_edid_3d_data->parse_data.p_3d_limit) {
			...
			VIC =
			    mhl_edid_3d_data->parse_data.
			    p_video_data_blocks_2d[0]->short_descriptors[_2D_VIC_order].VIC;
			if (0 == VIC) {
	...
	if (mhl_edid_3d_data->parse_data.p_byte_13_through_byte_15) {
		...
		if (0 == length_VIC) {
			...
			if (0 ==
			    mhl_edid_3d_data->parse_data.
			    p_byte_13_through_byte_15->vicList[outer_loop_limit]) {
				num_HDMI_VICs_pruned++;
				inner_loop_limit--;
			}
			...
			mhl_edid_3d_data->parse_data.p_byte_13_through_byte_15->
			    byte14.HDMI_VIC_len = inner_loop_limit;
			p_CEA_extension->byte_offset_to_18_byte_descriptors -= num_HDMI_VICs_pruned;

			MHL_TX_EDID_INFO(, "%p\n", mhl_edid_3d_data->parse_data.p_HDMI_vsdb);
			if (mhl_edid_3d_data->parse_data.p_HDMI_vsdb) {
				mhl_edid_3d_data->parse_data.p_HDMI_vsdb->header.
				    fields.length_following_header -= num_HDMI_VICs_pruned;
	...
	/* Now prune the SVD list and move the CEA 861-D data blocks and DTDs up */
	{
		...
		for (vdb_index = mhl_edid_3d_data->parse_data.num_video_data_blocks - 1;
			...
			if (inner_loop_limit) {
					...
					if (0 ==
						...
						for (prev = i, j = i + 1; j < inner_loop_limit;
							...
							VIC0 =
							    ...
							    [vdb_index]->short_descriptors[prev].
							    VIC;
							VIC1 =
							    mhl_edid_3d_data->
							    parse_data.p_video_data_blocks_2d
							    [vdb_index]->short_descriptors[j].VIC;
							MHL_TX_EDID_INFO
							    (mhl_edid_3d_data->dev_context,
							     ...
							     [vdb_index]->
							     short_descriptors[prev].native ?
							     "Native" : "", VIC0, (uint16_t) prev,
							     mhl_edid_3d_data->parse_data.p_video_data_blocks_2d
							     [vdb_index]->short_descriptors[j].
							     native ? "Native" : "", VIC1,
							     (uint16_t) j);
							mhl_edid_3d_data->
							    parse_data.p_video_data_blocks_2d
				...
				if (0 ==
				    mhl_edid_3d_data->parse_data.
				    p_video_data_blocks_2d[vdb_index]->short_descriptors
				    [outer_loop_limit].VIC) {
					num_CEA_VICs_pruned++;
					inner_loop_limit--;
					mhl_edid_3d_data->
					    parse_data.p_video_data_blocks_2d
					    [vdb_index]->short_descriptors[outer_loop_limit].
					    native = 0;
				}
				...
				{	/* no semicolon needed here */
					...
					uint8_t *pb_dest =
					    (uint8_t *) &mhl_edid_3d_data->
					    parse_data.p_video_data_blocks_2d
					    [vdb_index]->short_descriptors[inner_loop_limit];
					uint8_t *pb_src =
					    (uint8_t *) &mhl_edid_3d_data->
					    parse_data.p_video_data_blocks_2d
					    [vdb_index]->short_descriptors[mhl_edid_3d_data->
									   parse_data.p_video_data_blocks_2d
									   [vdb_index]->header.
									   fields.length_following_header];
				...
				mhl_edid_3d_data->parse_data.
				    ...
				    length_following_header = inner_loop_limit;
	...
	if (mhl_edid_3d_data->parse_data.p_HDMI_vsdb) {
		...
		if (mhl_edid_3d_data->parse_data.p_byte_13_through_byte_15) {
			if (((uint8_t *) &mhl_edid_3d_data->parse_data.
			     p_byte_13_through_byte_15->byte13) < p_next_db) {
				...
				if (((uint8_t *) &mhl_edid_3d_data->
				     parse_data.p_byte_13_through_byte_15->byte14) < p_next_db) {
					uint8_t length_VIC;
					uint8_t index;
					MHL2_video_descriptor_t mhl2_video_descriptor;
					length_VIC =
					    mhl_edid_3d_data->parse_data.
					    p_byte_13_through_byte_15->byte14.HDMI_VIC_len;
						...
						if (!is_qualified_mhl_hdmi_vic
							...
							MHL_TX_EDID_INFO
							    (mhl_edid_3d_data->dev_context,
							     ...
							     (uint16_t) VIC);
							mhl_edid_3d_data->
							    parse_data.p_byte_13_through_byte_15->vicList
							    [index] = 0;
						}
					}
					if (hdmi3D_present) {
						...
						case 0x00:
							...
							p_3D_u.p_3D =
							    &pThree3DSubBlock->hDMI_3D_sub_block_sans_all_AND_mask._3D_structure_and_detail_list
							    [0];
							break;
						case 0x01:
							/*
							 ...
							 */
							{
								...
								mhl2_video_descriptor.left_right =
								    p_3D_structure_all->_3D_structure_all_7_0.side_by_side;
								mhl2_video_descriptor.top_bottom =
								    p_3D_structure_all->_3D_structure_all_15_8.top_bottom;
								mhl2_video_descriptor.frame_sequential
								    =
								    p_3D_structure_all->_3D_structure_all_15_8.frame_packing;
...
;
							...
							p_3D_u.p_3D =
							    &pThree3DSubBlock->HDMI_3D_sub_block_sans_mask._3D_structure_and_detail_list
							    [0];
							break;
						case 0x02:
							/*
							 ...
							 */
							{
								P_3D_structure_all_t
								    ...
								    parse_data.p_byte_13_through_byte_15->vicList
								    [length_VIC];
								P_3D_mask_t p3DMask = (P_3D_mask_t)
								...
								DUMP_EDID_BLOCK(0, (PCEA_extension_t) &mhl_edid_3d_data->EDID_block_data[EDID_BLOCK_SIZE], EDID_BLOCK_SIZE)	/* no semicolon needed here */
									...
									if (mask &
									    p3DMask->_3D_mask_7_0) {
										this_MHL2_video_descriptor
										    =
										    mhl2_video_descriptor;
									} else {
										this_MHL2_video_descriptor.left_right
;
										this_MHL2_video_descriptor.top_bottom
;
										this_MHL2_video_descriptor.frame_sequential;
									}
...
;
							...
							p_3D_u.p_3D =
							    &pThree3DSubBlock->HDMI_3D_sub_block_with_all_AND_mask._3D_structure_and_detail_list
							    [0];
							mhl_edid_3d_data->parse_data.p_3d_mask =
							    &pThree3DSubBlock->HDMI_3D_sub_block_with_all_AND_mask._3D_mask;
							break;
						case 0x03:
							/*
    ...
    )
{
	...
	if (mhl_edid_3d_data->parse_data.flags.parse_3d_in_progress) {
		...
		if (mhl_edid_3d_data->parse_data.vic_2d_index >=
		    ...
		    [mhl_edid_3d_data->parse_data.video_data_block_index]->header.fields.
		...
		if (mhl_edid_3d_data->parse_data.burst_entry_count_3d_vic <
			...
			DUMP_EDID_BLOCK(0, (PCEA_extension_t) &mhl_edid_3d_data->EDID_block_data[EDID_BLOCK_SIZE], EDID_BLOCK_SIZE)	/* no semicolon needed here */
				...
				VIC =
				    mhl_edid_3d_data->
				    parse_data.p_video_data_blocks_2d[mhl_edid_3d_data->
								      parse_data.video_data_block_index]->short_descriptors
				    [mhl_edid_3d_data->parse_data.vic_2d_index].VIC;

				if (VIC) {
					MHL_TX_EDID_INFO(mhl_edid_3d_data->dev_context,
							 "short Descriptor[%d] 3D VIC: %d %s %s %s\n",
							 mhl_edid_3d_data->
							 parse_data.burst_entry_count_3d_vic, VIC,
							 p_mhl2_video_descriptor->left_right ?
							 psz_left_right : psz_space,
							 p_mhl2_video_descriptor->top_bottom ?
					...
					if (!this_mode_doable) {
						...
						mhl_edid_3d_data->
						    parse_data.p_video_data_blocks_2d
						    [mhl_edid_3d_data->parse_data.video_data_block_index]->short_descriptors
						    [mhl_edid_3d_data->parse_data.vic_2d_index].
						    VIC = 0;
					}
				}
	...
	if (mhl_edid_3d_data->parse_data.flags.parse_3d_in_progress) {
		if (!TEST_3D_FLAG(mhl_edid_3d_data, FLAGS_BURST_3D_DTD_VESA_DONE)) {
			...
			for (/* burst_index is set to zero above */
				...
				P_18_byte_descriptor_u p_desc =
				    (P_18_byte_descriptor_u) &
				    p_EDID_block_0->detailed_timing_descriptors[mhl_edid_3d_data->
		...
		}
		{
			...
			for (/* burst_index is set to zero above */
				...
				P_18_byte_descriptor_u p_desc =
				    &p_data_u.p_long_descriptors[mhl_edid_3d_data->
	...
	for (i = 0;
		...
		if ((1 == p_EDID_block_0->standard_timings[i].horz_pix_div_8_minus_31)
			...
			uint16_t horz_active =
			    (uint16_t) ((p_EDID_block_0->
					 standard_timings[i].horz_pix_div_8_minus_31 + 31) * 8);
			uint16_t vert_active = 0;
			uint16_t refresh_rate_in_milliHz =
			    (uint16_t) (p_EDID_block_0->
					standard_timings[i].field_refresh_rate_minus_60 +
					60) * 1000;
	...
	if (EDID_EXTENSION_TAG != p_CEA_extension->tag) {
		...
		if (EDID_REV_THREE != p_CEA_extension->revision) {
			...
			mhl_edid_3d_data->parse_data.underscan =
			    p_CEA_extension_version_3->
			    misc_support.underscan_IT_formats_by_default ? 1 : 0;
			...
			while (p_data_u.puc_data_block < puc_long_descriptors) {
				...
				data_block_length =
				    p_data_u.p_CEA_data_block->header.
				    fields.length_following_header;
				MHL_TX_EDID_INFO(mhl_edid_3d_data->dev_context,
					...
					if (mhl_edid_3d_data->parse_data.num_video_data_blocks <
						...
						while (i < data_block_length) {
							...
							VIC =
							    ...
							    parse_data.p_video_data_blocks_2d
							    [mhl_edid_3d_data->
							     parse_data.num_video_data_blocks]->short_descriptors
							    [i].VIC;
							MHL_TX_EDID_INFO
							    (mhl_edid_3d_data->dev_context,
							     "short desc[%d]: VIC: %d\n", i, VIC);
				...
				case DBTC_AUDIO_DATA_BLOCK:
					{
						...
						while (i <
						       data_block_length /
						       sizeof
						       (p_audio_data_block->short_audio_descriptors
							[0])) {
							mhl_edid_3d_data->
							    parse_data.audio_descriptors
							    [A_descriptor_index]
							    =
							    p_audio_data_block->short_audio_descriptors
							    [i];
				...
				case DBTC_SPEAKER_ALLOCATION_DATA_BLOCK:
					{
						...
						*((Pspeaker_allocation_flags_t) &
						  mhl_edid_3d_data->parse_data.speaker_alloc[i++]) =
						...
						case ETC_VIDEO_CAPABILITY_DATA_BLOCK:
							{
								...
								mhl_edid_3d_data->
								    parse_data.video_capability_flags
								    = *((uint8_t *) p_payload);
								mhl_edid_3d_data->
								    parse_data.p_video_capability_data_block
								    = p_video_capability_data_block;
								MHL_TX_EDID_INFO
								    (mhl_edid_3d_data->dev_context,
						...
						case ETC_COLORIMETRY_DATA_BLOCK:
							{
								...
								mhl_edid_3d_data->
								    parse_data.colorimetry_support_flags
								    = p_payload->ci_data.xvYCC;
								mhl_edid_3d_data->
								    parse_data.meta_data_profile =
								    p_payload->
								    cm_meta_data.meta_data;
							}
					 ...
					 p_HDMI_vendor_specific_payload->D_C;
							...
							if (p_HDMI_vendor_specific_payload->
								...
								if (p_HDMI_vendor_specific_payload->byte8.I_latency_fields_present) {
									mhl_edid_3d_data->parse_data.p_byte_13_through_byte_15
									    =
									    &p_HDMI_vendor_specific_payload->vsdb_fields_byte_9_through_byte_15.vsdb_all_fields_byte_9_through_byte_15.byte_13_through_byte_15;
								} else {
									mhl_edid_3d_data->parse_data.p_byte_13_through_byte_15
									    =
									    &p_HDMI_vendor_specific_payload->vsdb_fields_byte_9_through_byte_15.vsdb_all_fields_byte_9_through_byte_15_sans_interlaced_latency.byte_13_through_byte_15;
								}
							} else {
								if (p_HDMI_vendor_specific_payload->byte8.I_latency_fields_present) {
									mhl_edid_3d_data->parse_data.p_byte_13_through_byte_15
									    =
									    &p_HDMI_vendor_specific_payload->vsdb_fields_byte_9_through_byte_15.vsdb_all_fields_byte_9_through_byte_15_sans_progressive_latency.byte_13_through_byte_15;
								} else {
									mhl_edid_3d_data->parse_data.p_byte_13_through_byte_15
									    =
									    &p_HDMI_vendor_specific_payload->vsdb_fields_byte_9_through_byte_15.vsdb_all_fields_byte_9_through_byte_15_sans_all_latency.byte_13_through_byte_15;
								}
							}
							if (((uint8_t *) &
							     mhl_edid_3d_data->parse_data.
							    ...
							    puc_next_db) {
								mhl_edid_3d_data->
								    parse_data._3D_supported =
								    false;
							} else if (mhl_edid_3d_data->
								   parse_data.p_byte_13_through_byte_15->byte13.
								   _3D_present) {
								mhl_edid_3d_data->
								    parse_data._3D_supported = true;
							} else {
								mhl_edid_3d_data->
								    parse_data._3D_supported =
								    false;
							...
							MHL_TX_EDID_INFO
							    (mhl_edid_3d_data->dev_context,
							     ...
							     mhl_edid_3d_data->parse_data._3D_supported
							     ? "" : "NO ");
						} else {
							MHL_TX_EDID_INFO
							    (mhl_edid_3d_data->dev_context,
