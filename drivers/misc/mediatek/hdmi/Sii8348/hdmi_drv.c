...
int hdmi_drv_power_on(void)
{
	...
	ret = si_mhl_tx_initialize(si_dev_context, false);
#endif

#if 0
	si_mhl_tx_reset_states(si_dev_context);
	ret = si_dev_context->drv_info->mhl_device_initialize((struct drv_hw_context
							       *)(&si_dev_context->drv_context));
