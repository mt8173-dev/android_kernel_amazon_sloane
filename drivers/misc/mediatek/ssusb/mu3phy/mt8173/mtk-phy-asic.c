...
#ifdef CONFIG_PHY0_U2_CURRENT_DETECT
...
static void usb_port1_phy_restore(struct u3phy_info *info, unsigned int clk_on)
{
	...
	printk(KERN_ERR "U3D_USBPHYACR4+: 0x%x\n", readl(sif_base + U3D_USBPHYACR4));
#if RX_SENS
	u3phy_writel(sif_base, U3D_U2PHYDCR0, E60802_RG_SIFSLV_USB20_PLL_FORCE_ON_OFST,
		E60802_RG_SIFSLV_USB20_PLL_FORCE_ON, 1);
	...
	udelay(100);
	printk(KERN_ERR "U3D_U2PHYDTM0+: 0x%x\n", readl(sif_base + U3D_U2PHYDTM0));
#endif
...
static int u2_port0_slew_rate(struct u3phy_info *info)
{
	...
	printk(KERN_INFO "Freq_Valid=(0x%08X)\n", u3phy_readl(sif_base, U3D_U2FREQ_VALID, 0, ~0x0));
	...
	for (i = 0; i < 10; i++) {
		...
		printk(KERN_INFO "FM_OUT value: u4FmOut = %d(0x%08X)\n", u4FmOut, u4FmOut);
	...
	if (u4FmOut == 0) {
		...
		printk(KERN_INFO "SR calibration value u1SrCalVal = %d\n", (PHY_UINT8) u4Tmp);
		u3phy_writel(sif_base, U3D_USBPHYACR5, E60802_RG_USB20_HSTX_SRCTRL_OFST,
			     E60802_RG_USB20_HSTX_SRCTRL, u4Tmp);
	}

	/* driving current match FCC  */
	u3phy_writel(sif_base, U3D_USBPHYACR5, E60802_RG_USB20_HSTX_SRCTRL_OFST,
		     E60802_RG_USB20_HSTX_SRCTRL, 0x7);
