...
static const u3p_d60802_regs g_d60802_regs = {
#ifdef CONFIG_U3_PHY_GPIO_SUPPORT
	.u2phy_regs_d = (struct u2phy_reg_d *)0x0;
	.u3phyd_regs_d = (struct u3phyd_reg_d *)0x100000;
	.u3phyd_bank2_regs_d = (struct u3phyd_bank2_reg_d *)0x200000;
	.u3phya_regs_d = (struct u3phya_reg_d *)0x300000;
	.u3phya_da_regs_d = (struct u3phya_da_reg_d *)0x400000;
	.sifslv_chip_regs_d = (struct sifslv_chip_reg_d *)0x500000;
	.sifslv_fm_regs_d = (struct sifslv_fm_feg_d *)0xf00000;
#else
	.u2phy_regs_d = (struct u2phy_reg_d *)U2_PHY_BASE;
	.u3phyd_regs_d = (struct u3phyd_reg_d *)U3_PHYD_BASE;
	.u3phyd_bank2_regs_d = (struct u3phyd_bank2_reg_d *)U3_PHYD_B2_BASE;
	.u3phya_regs_d = (struct u3phya_reg_d *)U3_PHYA_BASE;
	.u3phya_da_regs_d = (struct u3phya_da_reg_d *)U3_PHYA_DA_BASE;
	.sifslv_chip_regs_d = (struct sifslv_chip_reg_d *)SIFSLV_CHIP_BASE;
	.sifslv_fm_regs_d = (struct sifslv_fm_feg_d *)SIFSLV_FM_FEG_BASE;
#endif
};

void phy_hsrx_set(struct u3phy_info *uinfo)
{
	struct u3p_d60802_regs *info = (struct u3p_d60802_regs *)uinfo->reg_info;
	...
	printk("%s: WRITE HSRX_MMODE_SELE(%d)\n", __func__,
	       U3PhyReadField32(((PHY_UINT32) & info->u2phy_regs_d->usbphyacr6)
				, D60802_RG_USB20_HSRX_MMODE_SELE_OFST,
				D60802_RG_USB20_HSRX_MMODE_SELE));
}

void phy_hsrx_reset(struct u3phy_info *uinfo)
{
	struct u3p_d60802_regs *info = (struct u3p_d60802_regs *)uinfo->reg_info;
...
PHY_INT32 phy_init_d60802(struct u3phy_info *uinfo)
{
	struct u3p_d60802_regs *info = (struct u3p_d60802_regs *)uinfo->reg_info;
	...
	U3PhyWriteReg8(((PHY_UINT32) & info->sifslv_chip_regs_d->gpio_ctla + 3), 0xd4);
...
PHY_INT32 phy_change_pipe_phase_d60802(struct u3phy_info *uinfo, PHY_INT32 phy_drv,
...
PHY_INT32 eyescan_init_d60802(struct u3phy_info *uinfo)
{
	struct u3p_d60802_regs *info = (struct u3p_d60802_regs *)uinfo->reg_info;
...
PHY_INT32 phy_eyescan_d60802(struct u3phy_info *uinfo, PHY_INT32 x_t1, PHY_INT32 y_t1,
...
/* , bExtendCnt, _bXcurr, _bYcurr, cX, cY, pwErrCnt0[bExtendCnt][_bXcurr][_bYcurr], pwErrCnt1[bExtendCnt][_bXcurr][_bYcurr]); */
	...
	printk(KERN_ERR "CurX [0x%x] CurY [0x%x]\n",
	       U3PhyReadField32(((PHY_UINT32) &info->u3phyd_regs_d->eq_eye0),
...
PHY_INT32 u2_connect_d60802(struct u3phy_info *uinfo)
{
	struct u3p_d60802_regs *info = (struct u3p_d60802_regs *)uinfo->reg_info;
...
PHY_INT32 u2_disconnect_d60802(struct u3phy_info *uinfo)
{
	struct u3p_d60802_regs *info = (struct u3p_d60802_regs *)uinfo->reg_info;
...
PHY_INT32 u2_slew_rate_calibration_d60802(struct u3phy_info *uinfo)
{
	struct u3p_d60802_regs *info = (struct u3p_d60802_regs *)uinfo->reg_info;
...
void usb_phy_savecurrent_d60802(unsigned int clk_on)
{
	printk(KERN_INFO "%s\n", __func__);
}

void usb_phy_recover_d60802(unsigned int clk_on)
{
	printk(KERN_INFO "%s\n", __func__);
}


static const struct u3phy_operator u3p_d60802_ops = {
	...
	.usb_phy_savecurrent = usb_phy_savecurrent_d60802,
	.usb_phy_recover = usb_phy_recover_d60802,

};

int u3p_d60802_init(struct u3phy_info *info)
{
	info->reg_info = (void *)&g_d60802_regs;
	info->u3p_ops = &u3p_d60802_ops;
}
