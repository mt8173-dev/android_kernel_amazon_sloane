...
static struct ssusb_gpd *mu3d_get_gpd_from_dma(USB_DIR dir, int num, dma_addr_t gpd_dma_addr)
{
	...
	if (i > MAX_GPD_NUM) {	/* if equal, overflow infact, should not use, only for compare */
		return NULL;
	}
	return gpd_head + i;
...
static dma_addr_t mu3d_gpd_virt_to_dma(USB_DIR dir, int num, struct ssusb_gpd *ptr)
{
	...
	offset = ptr - gpd_head;
	if (offset > MAX_GPD_NUM) {	/* if equal, overflow infact, should not use, only for compare */
		return 0;
	}
						 ...
						 u8 ioc, u8 bps, u8 zlp)
{
	...
	qmu_dbg(K_DEBUG,
		"[TX]"
		"%s gpd=%p, epnum=%d, len=%d, _is_bdp=%d size(struct ssusb_gpd)=%ld zlp=%d\n",
		__func__, gpd, ep_num, data_len, _is_bdp, sizeof(struct ssusb_gpd), zlp);
...
void qmu_done_tx(struct musb *musb, u8 ep_num, unsigned long flags)
{
	...
	if (!req) {
		qmu_dbg(K_DEBUG, "[TXD]" "%s Cannot get next request of %d, "
			"but QMU has done.\n", __func__, ep_num);
		return;
	} else {
		request = &req->request;
	...
	while (gpd != gpd_current && !TGPD_IS_FLAGS_HWO(gpd)) {
		...
		qmu_dbg(K_DEBUG, "[TXD]" "gpd=%p ->HWO=%d, BPD=%d, Next_GPD=%#x, DataBuffer=%#x, "
			"BufferLen=%d request=%p\n", gpd, (u32) TGPD_GET_FLAG(gpd),
			(u32) TGPD_GET_FORMAT(gpd), TGPD_GET_NEXT(gpd),
			TGPD_GET_DATA(gpd), (u32) TGPD_GET_BUF_LEN(gpd), req);
	...
	if (gpd != gpd_current && TGPD_IS_FLAGS_HWO(gpd)) {
		...
		qmu_dbg(K_ERR, "[TXD][ERROR]" "QCR0=%x, QCR1=%x, QCR2=%x, QCR3=%x, "
			"QGCSR=%x\n", mu3d_readl(mbase, U3D_QCR0), mu3d_readl(mbase, U3D_QCR1),
			mu3d_readl(mbase, U3D_QCR2), mu3d_readl(mbase, U3D_QCR3),
			mu3d_readl(mbase, U3D_QGCSR));

		qmu_dbg(K_ERR, "[TXD][ERROR]" "HWO=%d, BPD=%d, Next_GPD=%x, DataBuffer=%x, "
			"BufferLen=%d, Endpoint=%d\n", (u32) TGPD_GET_FLAG(gpd),
			(u32) TGPD_GET_FORMAT(gpd), (u32) TGPD_GET_NEXT(gpd),
			(u32) TGPD_GET_DATA(gpd), (u32) TGPD_GET_BUF_LEN(gpd),
...
void qmu_done_rx(struct musb *musb, u8 ep_num, unsigned long flags)
{
	...
	req = next_request(musb_ep);
	if (!req) {
		qmu_dbg(K_DEBUG, "[RXD]" "%s Cannot get next request of %d, "
			"but QMU has done.\n", __func__, ep_num);
		return;
	} else {
		request = &req->request;
	...
	if (gpd == gpd_current) {
		...
		qmu_dbg(K_ERR, "[RXD][ERROR]" "QCR0=%x, QCR1=%x, QCR2=%x, QCR3=%x, "
			"QGCSR=%x\n", mu3d_readl(mbase, U3D_QCR0), mu3d_readl(mbase, U3D_QCR1),
			mu3d_readl(mbase, U3D_QCR2), mu3d_readl(mbase, U3D_QCR3),
			mu3d_readl(mbase, U3D_QGCSR));

		qmu_dbg(K_INFO, "[RXD][ERROR]" "HWO=%d, Next_GPD=%x ,DataBufLen=%d, "
			"DataBuf=%x, RecvLen=%d, Endpoint=%d\n",
			(u32) TGPD_GET_FLAG(gpd), (u32) TGPD_GET_NEXT(gpd),
			(u32) TGPD_GET_DATA_BUF_LEN(gpd), (u32) TGPD_GET_DATA(gpd),
	...
	if (!gpd || !gpd_current) {
		qmu_dbg(K_ERR, "[RXD][ERROR]" "%s EP%d, gpd=%p, gpd_current=%p, ishwo=%d, \
			rx_gpd_last=%p, RQCPR=0x%x\n", __func__, ep_num, gpd, gpd_current, ((gpd == NULL) ? 999 : TGPD_IS_FLAGS_HWO(gpd)), Rx_gpd_List[ep_num].dequeue, mu3d_readl(mbase, USB_QMU_RQCPR(ep_num)));
		return;
	}
