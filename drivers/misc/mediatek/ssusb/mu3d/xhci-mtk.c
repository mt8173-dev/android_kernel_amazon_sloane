...
static int xhci_port_power_on(struct ssusb_mtk *ssusb, int version, int index)
{
	...
	mu3d_dbg(K_WARNIN, "%s (u%dport:%d), port status:0x%x\n", __func__,
	       version, index, xhci_readl(xhci, addr));
	return 0;
}
...
static int ssusb_port_switch(struct ssusb_mtk *ssusb, bool tohost,
										int version, int index)
{
	...
	/* diable u2 port power down and disable bits --> power on and enable all u2 ports */
	if (USB2_PORT == version) {
		...
		/* diable u3 port power down and disable bits --> power on and enable all u3 ports */
...
int mtk_otg_switch_init(struct ssusb_mtk *ssusb)
{
	...
	if (switch_dev_register(&otg_switch->otg_state)) {
		...
		mu3d_dbg(K_INFO, "switch_dev register success\n");
	}
...
static void wifi_hw_reset(void)
{
	...
	mu3d_dbg(K_ERR, "%s %d gpio:%d (val: %d)\n", __func__, __LINE__, WIFI_HW_RESET, gpio_get_value(WIFI_HW_RESET));
	mdelay(10);
