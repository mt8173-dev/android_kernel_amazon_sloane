...
static struct musb_hdrc_platform_data *mu3d_parse_dts(struct device_node *dn)
{
	...
	if (NULL == pdata) {
		mu3d_dbg(K_ERR, "failed to allocate mu3d platfrom data\n");
		goto pd_err;
	}
	pdata->p0_gpio_num = -1;  /* will upate if use */
	pdata->p1_gpio_num = -1;
	config = kzalloc(sizeof(*config), GFP_KERNEL);
	if (NULL == config) {
		mu3d_dbg(K_ERR, "failed to allocate musb hdrc config\n");
		goto cfg_err;
	}
...
static int mtu3d_probe(struct platform_device *pdev)
{
	...
	if (!ssusb) {
		dev_err(&pdev->dev, "failed to allocate ssusb context\n");
		goto free_pdata;
	}
	...
	mu3d_dbg(K_WARNIN, "%s() done! (time: %s %s)\n", __func__, __DATE__, __TIME__);
