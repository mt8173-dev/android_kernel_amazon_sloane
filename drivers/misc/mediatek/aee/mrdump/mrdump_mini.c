...
#define get_HW_cpuid smp_processor_id
...
static void mrdump_mini_build_elf_misc(void)
{
	...
	unsigned long task_info_pa = MRDUMP_MINI_BUF_PADDR ? (MRDUMP_MINI_BUF_PADDR + MRDUMP_MINI_HEADER_SIZE)
		: __pa(task_info_va);
...
static void __init mrdump_mini_elf_header_init(void)
{
	if (MRDUMP_MINI_BUF_PADDR)
		mrdump_mini_ehdr = remap_lowmem(MRDUMP_MINI_BUF_PADDR, MRDUMP_MINI_HEADER_SIZE
						+ TASK_INFO_SIZE + PSTORE_SIZE);
	else
		mrdump_mini_ehdr = (struct mrdump_mini_elf_header *)kmalloc(MRDUMP_MINI_HEADER_SIZE,
									    GFP_KERNEL);
	if (mrdump_mini_ehdr == NULL) {
		LOGE("mrdump mini reserve buffer fail");
		return;
	}
	LOGE("mirdump: reserved %x+%lx->%p", MRDUMP_MINI_BUF_PADDR, (unsigned long)MRDUMP_MINI_HEADER_SIZE,
	     mrdump_mini_ehdr);
