...
static void ke_gen_userbacktrace_msg(void)
{
	...
	memcpy((data + sizeof(pid_t)+sizeof(int)), (char *)(aed_dev.kerec.lastlog->userthread_stack.Userthread_Stack),
...
static void ke_gen_usermaps_msg(void)
{
	...
	memcpy((data + sizeof(pid_t)+sizeof(int)), (char *)(aed_dev.kerec.lastlog->userthread_maps.Userthread_maps),
			     ...
			     const char *msg)
{
	...
	if (NULL != oops) {
		...
		if (db_opt & DB_OPT_NATIVE_BACKTRACE) {
			...
			LOGE("%s: oops->userthread_stack.Userthread_Stack :0x%08lx,maps:0x%08lx", __func__,
			     (long)oops->userthread_stack.Userthread_Stack,
			     (long)oops->userthread_maps.Userthread_maps);
