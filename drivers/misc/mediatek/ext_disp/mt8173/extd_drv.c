...
extern bool resolution_change;
static unsigned char hdmi_suspend_enable;
...
static struct switch_dev hdmimode_switch_data;

#if defined(CONFIG_MTK_INTERNAL_MHL_SUPPORT)
static unsigned int hdmi_res = HDMI_VIDEO_1280x720p_50Hz;
#endif

extern void TVD_config_pll(unsigned int resolutionmode);
extern void set_dpi_res(unsigned char res);
extern int hdmi_internal_video_config(HDMI_VIDEO_RESOLUTION vformat, HDMI_VIDEO_INPUT_FORMAT vin,
				      HDMI_VIDEO_OUTPUT_FORMAT vout);

#ifdef CONFIG_AMAZON_METRICS_LOG
#include <linux/metricslog.h>
#define LMK_METRIC_TAG "kernel_hdmi"
...
/*
 * 0 : split buffer
 * 1 : use original buffer, don't split buffer
 * in always decouple mode : 4k - don't split buffer non-4k - split buffer
 * in direct-link & decouple dynamic switch : don't need split buffer in decouple mode.
 */
int hdmi_use_original_buffer = 0;
...
int flag_resolution_fps(HDMI_VIDEO_RESOLUTION resolution)
{
	...
	/* The pixel clock for 60Hz modes is configured at 59.94Hz,  */
	/* but for 24Hz modes it's actually 24. So don't set the fps */
	/* to 23976 here until the pixel clock is moved accordingly. */
...
int hdmi_video_config(HDMI_VIDEO_RESOLUTION vformat, HDMI_VIDEO_INPUT_FORMAT vin,
		      HDMI_VIDEO_OUTPUT_FORMAT vout);
static int hdmi_rdma_config_kthread(void *data)
{
	...
	int i = 0;
	int cur_config_fence = 0;
	int subtractor_when_free = 0;
	...
	for (;;) {
		...
		if (get_ext_disp_path_mode() == EXTD_DEBUG_RDMA_DPI_MODE) {
			...
			for (layid = 0; layid < HW_OVERLAY_COUNT; layid++) {
				...
				cur_config_fence = get_cur_config_fence(layid);
				subtractor_when_free = get_subtractor_when_free(layid);
				fence_idx = cur_config_fence - subtractor_when_free;
				if (fence_idx && (-1 != cur_config_fence) && (-1 != subtractor_when_free))
					mtkfb_release_fence(session_id, layid, fence_idx);
				else if ((-1 == cur_config_fence) || (-1 == subtractor_when_free)) {
					DISPMSG("hdmi_rdma_config_kthread : release all layer fence!\n");
					for (i = 0; i < 4; i++)
...
#if HDMI_SUB_PATH_PRESENT_FENCE_SUPPORT
		DISPMSG("disp_register_module_irq_callback ov1/wdma1\n");
...
void config_hdmitx_timing(void)
{
	HDMI_LOG("config_hdmitx_timing\n");
...
/* we think HDMI_STATE_NO_DEVICE and HDMI_STATE_PLUGIN_ONLY are similar event */
static unsigned char is_handle_similar_event;
...
void hdmi_state_callback(HDMI_STATE state)
{
	...
	case HDMI_STATE_PLUGIN_ONLY:
		{
			if (is_handle_similar_event == 1)
				break;
			is_handle_similar_event = 1;
			hdmi_suspend();
			switch_set_state(&hdmi_switch_data, HDMI_STATE_NO_DEVICE);
	...
	case HDMI_STATE_ACTIVE:
		{
			...
			if (atomic_read(&p->state) > HDMI_POWER_STATE_OFF) {
				...
				switch_set_state(&hdmimode_switch_data, 1);
			}
			hdmi_reschange = HDMI_VIDEO_RESOLUTION_NUM;
			break;
		}
	...
	case HDMI_STATE_CHANGE_RESOLUTION:
		{
				HDMI_LOG("[hdmi] HDMI_CHANGE_RESOLUTION\n");
				switch_set_state(&hdmimode_switch_data, 1);
				break;
		}
	...
	case HDMI_STATE_CHANGE_AUDIO_OFF:
		{
			HDMI_LOG("[hdmi] HDMI_STATE_CHANGE_AUDIO_OFF\n");
	...
	case HDMI_STATE_CHANGE_AUDIO_ON:
		{
			HDMI_LOG("[hdmi] HDMI_STATE_CHANGE_AUDIO_ON\n");
...
int _get_ext_disp_info(void *info)
{
	...
	dispif_info->vsyncFPS = hdmi_get_tv_fps();

	hdmi_drv->getedid(&edid);

	/* Convert dimensions from cm to mm */
	dispif_info->physicalHeight = edid.ui1_Display_Vertical_Size * 10;
	dispif_info->physicalWidth = edid.ui1_Display_Horizontal_Size * 10;
...
static void set_flag_for_hdmi_path(unsigned long arg)
{
...
#if HDMI_SUB_PATH_SUPPORT_DL_DC_DYNAMIC_SWITCH
	DISPMSG("support dl/dc dynamic switch, so don't split buffer\n");
	hdmi_use_original_buffer = 1;
	hdmi_ext_disp_mode = EXTD_DIRECT_LINK_MODE; /* use DL mode for all resolution */
	/* if (flag_resolution_4k(arg)) {
		hdmi_ext_disp_mode = EXTD_DIRECT_LINK_MODE;
		DISPMSG("hdmi res is 4k, use EXTD_DIRECT_LINK_MODE\n");
	} else {
		hdmi_ext_disp_mode = EXTD_DECOUPLE_MODE;
		DISPMSG("hdmi res is not 4k, use EXTD_DECOUPLE_MODE\n");
	} */
#else
	if (flag_resolution_4k(arg)) {
		hdmi_use_original_buffer = 1;
		DISPMSG("hdmi res is 4k, so don't split buffer\n");
	} else {
		hdmi_use_original_buffer = 0;
		DISPMSG("hdmi res is not 4k, so need split buffer\n");
	}
#endif
...
static long hdmi_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
...
#ifdef CONFIG_AMAZON_METRICS_LOG
	char logcat_metric_data[METRICS_DATA_LEN];
#endif
	...
	case MTK_HDMI_HDCP_LEVEL:
		{
			if (copy_from_user(&hdcp_level, (void __user *)arg, sizeof(hdcp_level))) {
				HDMI_LOG("copy_from_user failed! line:%d\n", __LINE__);
				r = -EFAULT;
			} else {
				hdmi_drv->hdcp_level(&hdcp_level.current_level, &hdcp_level.max_level);
			}
			if (copy_to_user((void __user *)arg, &hdcp_level, sizeof(hdcp_level))) {
				HDMI_LOG("copy_to_user failed! line:%d\n", __LINE__);
				r = -EFAULT;
			}
			break;
	...
	case MTK_HDMI_ENABLE_SUSPEND:
		{
			if (copy_from_user(&suspend, (void __user *)arg,
						sizeof(HDMI_ENABLE_SUSPEND))) {
				HDMI_LOG("copy_from_user failed! line:%d\n", __LINE__);
				r = -EFAULT;
			} else
				hdmi_suspend_enable = suspend.enable;
			break;
		}
	...
	case MTK_HDMI_VIDEO_CONFIG:
		{
			...
			if (!p->is_enabled) {
				up(&hdmi_update_mutex);
				HDMI_LOG("[HDMI] is_enable is false , return.\n");
				return 0;
			}
			if (IS_HDMI_NOT_ON()) {
				up(&hdmi_update_mutex);
				HDMI_LOG("[HDMI] HDMI is NOT ON , return.\n");
				return 0;
			}
			/* hdmi_video_buffer_info temp; */
			...
			set_flag_for_hdmi_path(arg);

			if (factory_mode == false) {
				if (hdmi_hwc_on)
					switch_set_state(&hdmires_switch_data, hdmi_reschange + 1);
					resolution_change = true;
					switch_set_state(&hdmimode_switch_data, 0);
#ifdef CONFIG_AMAZON_METRICS_LOG
					memset(logcat_metric_data, 0, sizeof(logcat_metric_data));
					snprintf(logcat_metric_data, METRICS_DATA_LEN,
						"HDMI:def:resolution_change=%ld;DV;1:HI",
						hdmi_reschange);

					log_to_metrics(ANDROID_LOG_INFO, LMK_METRIC_TAG, logcat_metric_data);
#endif
			}
			p->is_clock_on = true;
	...
	case MTK_HDMI_GET_DEV_INFO:
		{
			...
			hdmi_info.vsyncFPS = hdmi_get_tv_fps();
...
static int __init hdmi_init(void)
{
	...
	hdmimode_switch_data.name = "mode_hdmi";
	hdmimode_switch_data.index = 0;
	hdmimode_switch_data.state = 0;

	ret = switch_dev_register(&hdmimode_switch_data);
