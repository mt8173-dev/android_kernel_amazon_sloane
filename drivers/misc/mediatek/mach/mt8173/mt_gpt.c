...
#ifndef CONFIG_ARM64
	/*arm64 support : move from "alps/kernel/arch/arm/kernel/sched_clock.c" to "arch\arm64\kernel\time.c" */
	/* sched_clock() used jiffies in "kernel/sched/clock.c" */
	setup_sched_clock((void *)mt_read_sched_clock, 32, freq);
#endif
...
static void __init mt_gpt_init(struct device_node *node)
{
	...
	pr_alert("mt_gpt_init: tmr_regs=0x%p, tmr_irq=%d, freq=0x%x\n", xgpt_timers.tmr_regs,
