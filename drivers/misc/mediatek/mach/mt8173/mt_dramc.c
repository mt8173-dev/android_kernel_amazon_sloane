...
void store_vcore_dvfs_setting(void)
{
	...
	if (of_chosen) {
		...
		if (node) {
			...
			printk("get EMI_DVFS_SRAM_ADDR @ %p\n", EMI_DVFS_SRAM_ADDR);
			...
			printk("[vcore dvfs][kernel]pll_setting_num = %x\n", pll_setting_num);
			printk("[vcore dvfs][kernel]freq_setting_num = %x\n", freq_setting_num);
		} else {
...
#endif
	{
		...
		printk("[vcore dvfs]pll_high_addr = 0x%lx, pll_low_addr = 0x%lx, num = %d\n", high,
		       low, pll_num);

		get_freq_table_info(&high_cha, &high_chb, &low_cha, &low_chb, &num);
		printk
		    ("[vcore dvfs]high_cha_addr = 0x%lx, high_chb_addr = 0x%lx, low_cha_addr = 0x%lx, low_chb_addr = 0x%lx, num = %d\n",
...
void vcore_dvfs_ipi_handler(int id, void *data, unsigned int len)
{
	...
	*(unsigned int *)(MD32_DTCM + md32_dvfs_info->freq_setting_num) = freq_setting_num;
	...
	printk("[vcore dvfs][md32]MD32_DTCM=0x%p\n", MD32_DTCM);
	printk("[vcore dvfs][md32]src_ptr=0x%p\n", src_ptr);
	/* printk("[vcore dvfs][md32]md32_dvfs_info->low_freq_pll_setting_addr=0x%p\n", md32_dvfs_info->low_freq_pll_setting_addr ); */

	for (i = 0; i < pll_setting_num; i++) {
		*dst_ptr = *src_ptr;
		/* printk("[vcore dvfs][md32]pll_low: dst_ptr = 0x%p, src_ptr=0x%p, value=0x%x\n", dst_ptr, src_ptr, *dst_ptr); */
		dst_ptr++;
		src_ptr++;
	}
	...
	for (i = 0; i < freq_setting_num; i++) {
		*dst_ptr = *src_ptr;
		/* printk("[vcore dvfs][md32]cha_low: dst_ptr = 0x%p, src_ptr=0x%p, value=0x%x\n", dst_ptr, src_ptr, *dst_ptr); */
		dst_ptr++;
		src_ptr++;
	}
	...
	for (i = 0; i < freq_setting_num; i++) {
		*dst_ptr = *src_ptr;
		/* printk("[vcore dvfs][md32]chb_low: dst_ptr = 0x%p, src_ptr=0x%p, value=0x%x\n", dst_ptr, src_ptr, *dst_ptr); */
		dst_ptr++;
		src_ptr++;
	}
	...
	for (i = 0; i < pll_setting_num; i++) {
		*dst_ptr = *src_ptr;
		/* printk("[vcore dvfs][md32]pll_high: dst_ptr = 0x%p, src_ptr=0x%p, value=0x%x\n", dst_ptr, src_ptr, *dst_ptr); */
		dst_ptr++;
		src_ptr++;
	}
	...
	for (i = 0; i < freq_setting_num; i++) {
		*dst_ptr = *src_ptr;
		/* printk("[vcore dvfs][md32]cha_high: dst_ptr = 0x%p, src_ptr=0x%p, value=0x%x\n", dst_ptr, src_ptr, *dst_ptr); */
		dst_ptr++;
		src_ptr++;
	}
	...
	for (i = 0; i < freq_setting_num; i++) {
		*dst_ptr = *src_ptr;
		/* printk("[vcore dvfs][md32]chb_high: dst_ptr = 0x%p, src_ptr=0x%p, value=0x%x\n", dst_ptr, src_ptr, *dst_ptr); */
		dst_ptr++;
		src_ptr++;
	}
}
...
static int __init dram_test_init(void)
{
	...
	if (node) {
		...
		printk("[DRAMC] get APMIXED_BASE_ADDR @ %p\n", APMIXED_BASE_ADDR);
	} else {
	...
	if (node) {
		...
		printk("[DRAMC]can't find CQDMA_BASE_ADDR compatible node\n");
		/* return -1; */
	}
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,DDRPHY0");
	if (node) {
...
void DFS_Reserved_Memory(void)
{
	...
	/* phys_addr_t max_dram_size = get_max_DRAM_size(); */
	phys_addr_t max_dram_size = 0x8000000;
