...
__weak unsigned int mt_get_chip_sw_ver(void)
{
	return 0;
}
...
unsigned long *mt_save_generic_timer(unsigned long *container, int sw)
{

	__asm__ __volatile__(" mrs   x2, CNTKCTL_EL1\n\t"
...
void mt_restore_generic_timer(unsigned long *container, int sw)
{

	__asm__ __volatile__(" ldp   x2, x3, [%0, #0]\n\t"
			     " msr   CNTKCTL_EL1, x2\n\t"
...
static void save_gic_distributor_private(u32 *pointer, unsigned long gic_distributor_address)
{
	...
	*pointer = id->enable.set[0];
	...
	pointer = copy_words(pointer, (unsigned int *)id->priority, 8);
	/*  Save SGI,PPI target status */
	pointer = copy_words(pointer, (unsigned int *)id->target, 8);
	...
	*pointer = id->pending.set[0];
	 ...
	 */
	ptr = pointer;
	copy_words(pointer, (unsigned int *)id->sgi_set_pending, 4);
	pointer += 8;
	 ...
	 */
	copy_words((unsigned int *)id->sgi_clr_pending, ptr, 4);
...
static void save_gic_distributor_shared(u32 *pointer, unsigned long gic_distributor_address)
{
	...
	if (num_spis) {
		pointer = copy_words(pointer, (unsigned int *)id->enable.set + 1, num_spis / 32);
		pointer = copy_words(pointer, (unsigned int *)id->priority + 8, num_spis / 4);
		pointer = copy_words(pointer, (unsigned int *)id->target + 8, num_spis / 4);
		pointer = copy_words(pointer, (unsigned int *)id->configuration + 2, num_spis / 16);
		pointer = copy_words(pointer, (unsigned int *)id->security + 1, num_spis / 32);
		saved_pending = pointer;
		pointer = copy_words(pointer, (unsigned int *)id->pending.set + 1, num_spis / 32);

		pointer = copy_words(pointer, (unsigned int *)id->non_security_access_control + 1, num_spis / 16);
	}
...
static void restore_gic_distributor_private(u32 *pointer, unsigned long gic_distributor_address)
{
	...
	copy_words((unsigned int *)id->priority, pointer, 8);
	pointer += 8;
	/* Restore SGI,PPI target status */
	copy_words((unsigned int *)id->target, pointer, 8);
	pointer += 8;
	 ...
	 */
	copy_words((unsigned int *)id->sgi_set_pending, pointer, 4);
	pointer += 4;
...
static void restore_gic_distributor_shared(u32 *pointer, unsigned long gic_distributor_address)
{
	...
	if (num_spis) {
		copy_words((unsigned int *)id->enable.set + 1, pointer, num_spis / 32);
		pointer += num_spis / 32;
		copy_words((unsigned int *)id->priority + 8, pointer, num_spis / 4);
		pointer += num_spis / 4;
		copy_words((unsigned int *)id->target + 8, pointer, num_spis / 4);
		pointer += num_spis / 4;
		copy_words((unsigned int *)id->configuration + 2, pointer, num_spis / 16);
		pointer += num_spis / 16;
		copy_words((unsigned int *)id->security + 1, pointer, num_spis / 32);
		pointer += num_spis / 32;
		copy_words((unsigned int *)id->pending.set + 1, pointer, num_spis / 32);
		pointer += num_spis / 32;

		copy_words((unsigned int *)id->non_security_access_control + 1, pointer, num_spis / 16);
		pointer += num_spis / 16;
...
void mt_cpu_save(void)
{
	...
	cluster_context *cluster;
	unsigned int *ret;
	unsigned long dbg_base, *ret_timer;
	...
	ret_timer = mt_save_generic_timer(core->timer_data, 0x0);
	stop_generic_timer();	/* disable timer irq, and upper layer should enable again. */

	SENTINEL_CHECK(core->timer_data, ret_timer);
...
/* ret = mt_save_dbg_regs(cluster->dbg_data, dbg_base + cpuid*DBGAPB_CORE_OFFSET); */
	...
	if (mt_get_chip_sw_ver() == CHIP_SW_VER_01) {
		for (i = 0; i < CA57_MCU_CONFIG_SIZE; i = i + 1)
			ca15_mcusys[i] = spm_read(CA57_MCU_CONFIG + i * 4);
	}
}
...
void mt_cpu_restore(void)
{
	...
	if (mt_get_chip_sw_ver() == CHIP_SW_VER_01) {
		for (i = 0; i < CA57_MCU_CONFIG_SIZE; i = i + 1)
			spm_write(CA57_MCU_CONFIG + i * 4, ca15_mcusys[i]);
	}
...
/* __asm__ __volatile__ ("1: b 1b\n\t"); */
	...
	/* start_resume: */
	/* jump to cpu_resume() */
	__asm__ __volatile__("adr x0, 1f \n\t"
			     "ldr x2, [x0]\n\t"
			     "sub x2, %0, x2\n\t"
			     "add x3, x0, x2\n\t"
			     "ldr x3, [x3]\n\t"
	...
	if (/* (_IS_DORMANT_SET(flags, DORMANT_CPUSYS_OFF)) */
		...
		CPU_DORMANT_INFO
		    ("dormant(flags:%x) (ret:%x) (core:%d/%d) cnt:%d, rst:%d, abt:%d, brk:%d\n",
		     flags, ret, clusterid, cpuid, core->count, core->rst, core->abt, core->brk);
		CPU_DORMANT_INFO("dormant timing: %llu, %llu, %llu, %llu, %llu\n",
				 core->timestamp[0], core->timestamp[1], core->timestamp[2],
				 core->timestamp[3], core->timestamp[4]);
	}
#endif
...
static int mt_dormant_dts_map(void)
{
	...
	gic_base = (unsigned long)of_iomap(node, 0);
	if (!gic_base) {
...
int mt_cpu_dormant_init(void)
{
	...
	dormant_data[0].poc.cpu_dormant_aee_rr_rec = aee_rr_rec_cpu_dormant();
	sleep_aee_rec_cpu_dormant_va = (phys_addr_t)dormant_data[0].poc.cpu_dormant_aee_rr_rec;

	sleep_aee_rec_cpu_dormant =
	    (phys_addr_t)virt_to_phys((void *)sleep_aee_rec_cpu_dormant_va);
	...
	if (1) {
		__asm__ __volatile__("isb; dsb sy\n\t"
				     ...
				     "isb\n\t"
				     "dsb sy\n\t":"=r"(dormant_data[0].poc.l2ectlr),
				     "=r"(dormant_data[0].poc.l2actlr)
 :  : "memory");

		zlog("dormant init (cluster/cpu:%d/%d), l2ectlr(%lx) l2actlr(%lx)!\n",
		     clusterid, cpuid, dormant_data[0].poc.l2ectlr, dormant_data[0].poc.l2actlr);
...
#if defined(MT_DORMANT_UT)
volatile int mt_cpu_dormant_test_mode = (CPU_SODI_MODE | 0);
