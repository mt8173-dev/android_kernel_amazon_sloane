...
static int set_volt_gpu(struct ptp_det *det)
{
	...
	return mt_gpufreq_voltage_set_by_ptpod(det->volt_tbl_pmic,
			det->num_freq_tbl);
}
...
static void get_freq_table_gpu(struct ptp_det *det)
{
	...
	for (i = 0; i < NR_FREQ; i++) {
		/* TODO: FIXME */
		det->freq_tbl[i] = PERCENT(mt_gpufreq_get_frequency_by_level(i),
						det->max_freq_khz);
		if (0 == det->freq_tbl[i])
			break;
	}
...
static int ptp_probe(struct platform_device *pdev)
{
	...
	clk_axi = devm_clk_get(&pdev->dev, "ptp_mfg_axi");
	BUG_ON(IS_ERR(clk_axi));
#ifdef CLK_208MHZ
	if (ptpod_id <= 1) {
	...
	/* set PTP IRQ */
	ret = request_irq(ptpod_irq_number, ptp_isr, IRQF_TRIGGER_LOW, "ptp",
						NULL);
...
#ifdef CLK_208MHZ
	if (ptpod_id <= 1) {
		clk_set_parent(clk_axi_sel, parent);
		ptp_error("clk_therm = %ld\n", __clk_get_rate(clk_therm));
		ptp_error("ptp_mfg_axi = %ld\n", __clk_get_rate(clk_axi));
	}
#endif
	clk_disable_unprepare(clk_axi);
						...
						unsigned int *volt)
{
	...
	/* TODO: FIXME */
	*temp = tscpu_get_bL_temp((id == PTP_DET_LITTLE) ?
			THERMAL_BANK0 : THERMAL_BANK1);
