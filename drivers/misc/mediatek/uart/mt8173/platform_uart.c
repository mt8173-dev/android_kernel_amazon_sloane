...
#include "arch/arm/mach-mt8173/include/mach/mt_idle.h"
#include "mach/irqs.h"
...
/*---------------------------------------------------------------------------*/
struct mtk_uart_setting mtk_uart_default_settings[] = {
	{
...
void set_uart_default_settings(int idx)
{
	...
	if (of_property_read_u32_index
	    (node, "reg", 0, &mtk_uart_default_settings[idx].uart_phys_base)) {
	...
	/*printk("[UART%d] phys_regs=0x%lx, regs=0x%lx, irq=%d, irq_flags=0x%x\n", idx,
...
void *get_apdma_uart0_base(void)
{
	...
	/*printk("[UART] apdma uart0 base=0x%llx\n", base);*/
...
void mtk_uart_reset_dma(struct mtk_uart_dma *dma)
{
	...
	if (dma->mode == UART_RX_VFIFO_DMA || dma->mode == UART_TX_VFIFO_DMA) {
		...
		while (UART_READ32(VFF_EN(base))) {
				polling_cnt++;
			if (polling_cnt > 10000) {
				MSG(ERR, "mtk_uart_reset_dma: polling VFF_EN failed\n");
				break;
			}
		}
...
#ifdef POWER_FEATURE
		clk_prepare(setting->pll_id);
		if (0 != clk_enable(setting->pll_id))
			MSG(ERR, "power on fail!!\n");
		if ((uart != console_port) &&
				(uart->tx_mode == UART_TX_VFIFO_DMA || uart->rx_mode == UART_RX_VFIFO_DMA)) {
			clk_prepare(PDN_FOR_UART_DMA);
		if (0 != clk_enable(PDN_FOR_UART_DMA))
...
#ifndef CONFIG_FPGA_EARLY_PORTING
#ifdef POWER_FEATURE
		clk_disable(setting->pll_id);
		clk_unprepare(setting->pll_id);
		if ((uart != console_port)
		    && (uart->tx_mode == UART_TX_VFIFO_DMA || uart->rx_mode == UART_RX_VFIFO_DMA)) {
					clk_disable(PDN_FOR_UART_DMA);
					clk_unprepare(PDN_FOR_UART_DMA);
		}
		uart->poweron_count--;
#endif
#endif
