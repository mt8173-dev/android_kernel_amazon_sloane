...
static unsigned int uart_clock_inited;
...
void dump_uart_reg(void)
{
	...
	for (i = 0; i < UART_NR; i++) {
		...
		if (uart->poweron_count > 0) {
			...
			pr_info("[UART%d] LSR=0x%x   ESCAPE_EN=0x%x\n", uart->nport, lsr,
				escape_en);
		} else
			pr_info("[UART%d] clock is off\n", uart->nport);
	}
...
static void mtk_uart_dma_vfifo_tx_tasklet_byte(unsigned long arg)
{
	...
	while (len--) {
		...
		while (UART_READ32(VFF_FLUSH(vfifo->base))) {
			...
			if ((b.tv_sec - a.tv_sec) > 1
				...
				pr_info("[UART%d] Polling flush timeout\n", port->line);
				return;
			}
		}
	...
	if (UART_DEBUG_EVT(DBG_EVT_DAT) && UART_DEBUG_EVT(DBG_EVT_BUF)) {
		int i;
		pr_info("[UART%d_TX] %4d bytes:", uart->nport, vfifo->cur->idx);
		for (i = 0; i < vfifo->cur->idx; i++) {
			if (i % 16 == 0)
				pr_info("\n");
			pr_info("%.2x ", (unsigned char)vfifo->cur->dat[i]);
		}
		pr_info("\n");
	}
#endif
...
/*static void mtk_uart_dma_vfifo_rx_tasklet_byte(unsigned long arg)
{
    ...
    if (UART_DEBUG_EVT(DBG_EVT_DAT) && UART_DEBUG_EVT(DBG_EVT_BUF)) {
	int i;
	pr_info("[UART%d_RX] %4d bytes:", uart->nport, vfifo->cur->idx);
...
static void mtk_uart_cal_baud(struct mtk_uart *uart, int baudrate, int highspeed)
{
	...
	if (highspeed == 0) {
		 ...
		 * In order to improve compatibilty, the guard time register is enabled which is used to extend
		 * the stop bit.
		 */
		if (baudrate >= 3000000)
...
void mtk_uart_power_up(struct mtk_uart *uart)
{
	...
	/*set uart clock initial state as power off except for debug uart */
	if (!uart_clock_inited) {
		int i = 0;
		for (; i < sizeof(mtk_uart_default_settings) / sizeof(mtk_uart_default_settings[0]);
		     i++) {
			if ((mtk_uart_default_settings[i].pll_id) != console_port->setting->pll_id) {
				if (clock_is_on(mtk_uart_default_settings[i].pll_id)) {
					pr_info("disable uart%d clock\n", i + 1);
					disable_clock(mtk_uart_default_settings[i].pll_id, "UART");
				}
			}
		}
		uart_clock_inited = 1;
...
void dump_uart_history(void)
{
	...
	for (i = 0; i < RECORD_NUMBER; i++) {
		...
		for (j = 0; j < uart_history_cnt[curr]; j++) {
			if ((j % 0xF) == 0)
				pr_info("\n");
			pr_info("%02x ", uart_history[curr][j]);
		}
		msleep(20);
		curr++;
		if (curr >= RECORD_NUMBER)
	...
	for (i = 0; i < RECORD_NUMBER; i++) {
		...
		for (j = 0; j < uart_rx_history_cnt[rx_curr]; j++) {
			if ((j % 0xF) == 0)
				pr_info("\n");
			pr_info("%02x ", uart_rx_history[rx_curr][j]);
		}
		msleep(20);
		rx_curr++;
		if (rx_curr >= RECORD_NUMBER)
