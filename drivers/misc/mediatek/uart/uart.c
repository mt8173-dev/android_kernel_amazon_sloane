...
struct clk *PDN_FOR_UART_DMA = NULL;
/*---------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------*/
inline static unsigned short mtk_uart_vfifo_get_trig(struct mtk_uart *uart,
						     struct mtk_uart_vfifo *vfifo)
{
	return vfifo->trig;
}
...
static int mtk_uart_probe(struct platform_device *pdev)
{
	...
	clk = devm_clk_get(&pdev->dev, "MT_CG_PERI_dma");
	if (!IS_ERR(clk))
		PDN_FOR_UART_DMA = clk;

	clk = devm_clk_get(&pdev->dev, "MT_CG_PERI_UART0");
	if (!IS_ERR(clk))
		mtk_uart_default_settings[0].pll_id = clk;

	clk = devm_clk_get(&pdev->dev, "MT_CG_PERI_UART1");
	if (!IS_ERR(clk))
		mtk_uart_default_settings[1].pll_id = clk;

	clk = devm_clk_get(&pdev->dev, "MT_CG_PERI_UART2");
	if (!IS_ERR(clk))
		mtk_uart_default_settings[2].pll_id = clk;

	clk = devm_clk_get(&pdev->dev, "MT_CG_PERI_UART3");
	if (!IS_ERR(clk))
		mtk_uart_default_settings[3].pll_id = clk;
...
#endif
	uart->pinctrl = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR(uart->pinctrl)) {
		err = PTR_ERR(uart->pinctrl);
		goto err_pinctrl;
	}

	uart->rx_default = pinctrl_lookup_state(uart->pinctrl, "rx_default");
	if (IS_ERR(uart->rx_default)) {
		dev_err(&pdev->dev, "could not get rx_default pinstate\n");
	} else {
		ret = pinctrl_select_state(uart->pinctrl,
									uart->rx_default);
		if (ret)
			dev_dbg(&pdev->dev, "could not set default pinstate\n");
	}

	uart->rx_sleep = pinctrl_lookup_state(uart->pinctrl, "rx_sleep");
	if (IS_ERR(uart->rx_sleep))
		dev_err(&pdev->dev, "could not get rx_sleep pinstate\n");
	uart->tx_default = pinctrl_lookup_state(uart->pinctrl, "tx_default");
	if (IS_ERR(uart->tx_default)) {
		dev_err(&pdev->dev, "could not get tx_default pinstate\n");
	} else {
		ret = pinctrl_select_state(uart->pinctrl,
									uart->tx_default);
		if (ret)
	...
	uart->tx_sleep = pinctrl_lookup_state(uart->pinctrl, "tx_sleep");
	if (IS_ERR(uart->tx_sleep))
		dev_err(&pdev->dev, "could not get tx_sleep pinstate\n");
...
static int mtk_uart_syscore_suspend(void)
{
	...
	if (!IS_ERR(uart->tx_sleep)) {
			ret = pinctrl_select_state(uart->pinctrl,
										uart->tx_sleep);
			if (ret)
				pr_debug("could not set pins to tx_sleep state\n");
		}
	...
	if (bt_port) {
		...
		if (!IS_ERR(uart->tx_default)) {
			ret = pinctrl_select_state(uart->pinctrl,
									uart->tx_default);
	    if (ret)
				pr_debug("could not set pins to tx_default state\n");
		}
	...
	if (uart && (uart->nport < UART_NR) && (uart != bt_port)) {
		...
		if (!IS_ERR(uart->rx_sleep)) {
			ret = pinctrl_select_state(uart->pinctrl,
									uart->rx_sleep);
			if (ret)
				dev_err(&pdev->dev, "could not set pins to rx_sleep state\n");
		}
	}
	return ret;
}
...
static int mtk_uart_resume(struct platform_device *pdev)
{
	...
	if (uart && (uart->nport < UART_NR) && (uart != bt_port)) {
		...
		if (!IS_ERR(uart->rx_default)) {
			ret = pinctrl_select_state(uart->pinctrl, uart->rx_default);
	    if (ret)
				dev_err(&pdev->dev, "could not set pins to rx_default state\n");
	}
		ret = uart_resume_port(&mtk_uart_drv, &uart->port);
		printk(KERN_NOTICE "[UART%d] Resume(%d)!\n", uart->nport, ret);
	}
...
#if defined(ENABLE_VFIFO)
		for (idx = i*2; idx < i*2 + 2 && idx < ARRAY_SIZE(mtk_uart_vfifo_port); idx++) {
			vfifo = &mtk_uart_vfifo_port[idx];
