...
/* ---------------------------------------------------------------------------- */
/* #define LOG_MSG(fmt, arg...)    pr_err("[ISP][%s]" fmt,__FUNCTION__, ##arg) */
/* #define LOG_DBG(fmt, arg...)    pr_err("[ISP][%s]" fmt,__FUNCTION__, ##arg) */
/* #define LOG_WRN(fmt, arg...)    pr_err("[ISP][%s]Warning" fmt,__FUNCTION__, ##arg) */
/* #define LOG_ERR(fmt, arg...)    pr_err("[ISP][%s]Err(%5d):" fmt, __FUNCTION__,__LINE__, ##arg) */
...
#define LOG_ERR(format, args...)    xlog_printk(ANDROID_LOG_ERROR  , "ISP", \
"[%s, line%04d] ERROR: " format, __func__, __LINE__, ##args)
#define LOG_AST(format, args...)    xlog_printk(ANDROID_LOG_ASSERT , "ISP", \
"[%s, line%04d] ASSERT: " format, __func__, __LINE__, ##args)
...
#define ISP_REG_MASK_INTX_STATUS_ERR    (ISP_IRQ_INTX_STATUS_TG1_ERR_ST |\
					    ...
					    /* DMA_ERR rootcause not clear.
					       it caused resume problem with busy log print out. */
					    /* ISP_IRQ_INTX_STATUS_DMA_ERR_ST) */
					    /* Vent@20121025: Remove ISP_IRQ_INTX_STATUS_BPC_ERR_ST.
					    From TH Wu's explanation, this bit is not used as an error state anymore. */
...
/*  */
static ISP_RT_BUF_STRUCT *pstRTBuf;
...
static void ISP_EnableClock(MBOOL En)
{
	...
	if (En) {		/* Enable clock. */
		...
		enable_subsys(SYS_ISP, "CAMERA");
		clkmux_sel(MT_CLKMUX_MSDC30_4_SEL, MT_CG_V_AD_MSDCPLL_CK, "CAMERA");
		clkmux_sel(MT_CLKMUX_CAM_SEL, MT_CG_V_SYSPLL_D3, "CAMERA");
		clkmux_sel(MT_CLKMUX_SMI_SEL, MT_CG_V_CLKPH_MCK, "CAMERA");

		enable_clock(MT_CG_ISP_CAM_SMI_CKPDN, "CAMERA");
		enable_clock(MT_CG_ISP_CAM_CAM_CKPDN, "CAMERA");
		enable_clock(MT_CG_ISP_SEN_TG_CKPDN, "CAMERA");
		enable_clock(MT_CG_ISP_SEN_CAM_CKPDN, "CAMERA");
		enable_clock(MT_CG_ISP_LARB3_SMI_CKPDN, "CAMERA");
		enable_clock(MT_CG_ISP_LARB4_SMI_CKPDN, "CAMERA");
		enable_clock(MT_CG_ISP_COMM_SMI_CKPDN, "CAMERA");
		enable_clock(MT_CG_MIX_UNIVPLL_48M_EN, "CAMERA");
		enable_clock(MT_CG_V_UNIVPLL_D26, "CAMERA");
		enable_clock(MT_CG_MIX_UNIVPLL_USB48M_EN, "CAMERA");
		enable_clock(MT_CG_V_AD_MSDCPLL_CK, "CAMERA");

		G_u4EnableClockCount++;
		LOG_DBG("Camera clock enbled. G_u4EnableClockCount: %d.", G_u4EnableClockCount);
	} else {		/* Disable clock. */

		disable_subsys(SYS_ISP, "CAMERA");

		disable_clock(MT_CG_ISP_CAM_SMI_CKPDN, "CAMERA");
		disable_clock(MT_CG_ISP_CAM_CAM_CKPDN, "CAMERA");
		disable_clock(MT_CG_ISP_SEN_TG_CKPDN, "CAMERA");
		disable_clock(MT_CG_ISP_SEN_CAM_CKPDN, "CAMERA");
		disable_clock(MT_CG_ISP_LARB3_SMI_CKPDN, "CAMERA");
		disable_clock(MT_CG_ISP_LARB4_SMI_CKPDN, "CAMERA");
		disable_clock(MT_CG_ISP_COMM_SMI_CKPDN, "CAMERA");
		disable_clock(MT_CG_MIX_UNIVPLL_48M_EN, "CAMERA");
		disable_clock(MT_CG_V_UNIVPLL_D26, "CAMERA");
		disable_clock(MT_CG_MIX_UNIVPLL_USB48M_EN, "CAMERA");
		disable_clock(MT_CG_V_AD_MSDCPLL_CK, "CAMERA");
...
#define SMI_LARB_NUMBER 5
	...
	LOG_DBG
	    ("+LARB in reset,BWL(0x%08X)/(0x%08X)/(0x%08X)/(0x%08X),220(0x%08X)/(0x%08X)",
	...
	LOG_DBG
	    ("+LARB in reset,0x14(0x%08X)/(0x%08X)/(0x%08X)/(0x%08X)/(0x%08X)",
...
static MUINT32 prv_tstamp_s;
static MUINT32 prv_tstamp_us;


static MUINT32 sof_count;
static MUINT32 start_time;
static MUINT32 avg_frame_time;

/* allan */
static int sof_pass1done;
	...
	if (1 == pstRTBuf->ring_buf[rt_dma].empty_count) {
		...
		LOG_DBG
		    ("[rtbc][ENQUE]:dma(%d),base_pAddr(0x%x)/imgo(0x%x)/img2o(0x%x)/empty_count(%d) ",
		     ...
		     ISP_RD32(ISP_REG_ADDR_IMGO_BASE_ADDR), ISP_RD32(ISP_REG_ADDR_IMG2O_BASE_ADDR),
		     pstRTBuf->ring_buf[rt_dma].empty_count);
		/* } */
...
static MINT32 ISP_RTBC_DEQUE(MINT32 dma)
{
	...
	if (0 == deque_buf.count) {
		...
		LOG_DBG("[rtbc][DEQUE]:dma(%d),start(%d),total(%d),empty(%d), deque_buf.count(%d) ",
			...
			pstRTBuf->ring_buf[rt_dma].total_count,
			pstRTBuf->ring_buf[rt_dma].empty_count, deque_buf.count);
		/*  */
			...
			if (IspInfo.DebugMask & ISP_DBG_BUF_CTRL) {
				LOG_DBG
				    ("[rtbc][DEQUE]:index(%d)/PA(0x%x)/memID(%d)/size(0x%x)/VA(0x%x)",
				     index + i, deque_buf.data[i].base_pAddr,
				     deque_buf.data[i].memID, deque_buf.data[i].size,
				     deque_buf.data[i].base_vAddr);
			}
	...
	if (copy_from_user(&rt_buf_ctrl, (void *)Param, sizeof(ISP_BUFFER_CTRL_STRUCT)) == 0) {
		...
		if (IspInfo.DebugMask & ISP_DBG_INT) {
			LOG_DBG("[rtbc]ctrl(0x%x)/buf_id(0x%x)/data_ptr(0x%x)/ex_data_ptr(0x%x)",
				rt_buf_ctrl.ctrl,
				rt_buf_ctrl.buf_id, rt_buf_ctrl.data_ptr, rt_buf_ctrl.ex_data_ptr);
		}
		/*  */
		...
		case ISP_RT_BUF_CTRL_EXCHANGE_ENQUE:
			...
			if (copy_from_user
				...
				if ((reg_val & 0x01) || (reg_val2 & 0x01)) {
					...
					/*  */
					if (IspInfo.DebugMask & ISP_DBG_BUF_CTRL) {
						LOG_DBG("[rtbc][ENQUE]:ex_data_ptr(0x%x)",
						...
						if (copy_from_user
							...
							for (i = 0; i < ISP_RT_BUF_SIZE; i++) {
								...
								if (pstRTBuf->ring_buf[rt_dma].
								    ...
								    rt_buf_info.base_pAddr) {
									LOG_DBG
									    ("[rtbc]dma(%d),old(%d) PA(0x%x) VA(0x%x)",
									     rt_dma, i,
									     pstRTBuf->
									     ring_buf[rt_dma].
									     data[i].base_pAddr,
									...
									pstRTBuf->ring_buf[rt_dma].
									    ...
									    base_vAddr;
									LOG_DBG
									    ("[rtbc]dma(%d),new(%d) PA(0x%x) VA(0x%x)",
									     rt_dma, i,
									     pstRTBuf->
									     ring_buf[rt_dma].
									     data[i].base_pAddr,
					...
					/* if(IspInfo.DebugMask & ISP_DBG_BUF_CTRL) { */
					LOG_DBG
					    ("[rtbc][ENQUE]:dma(%d),PA(0x%x),O(0x%x),2O(0x%x),fps(%d)us",
					...
					bEnqBuf = 1;
					Timeout =
					    wait_event_interruptible_timeout(IspInfo.WaitQueueHead,
									     (0 == bEnqBuf),
									     ISP_MsToJiffies
									     (Timeout));
					/*  */
					if (Timeout == 0) {
						LOG_ERR("[rtbc][ENQUE]:timeout(%d)", bEnqBuf);
						Ret = -EFAULT;
					}
					/*  */
					if (IspInfo.DebugMask & ISP_DBG_BUF_CTRL) {
...
LOG_DBG("[rtbc][DEQUE]:TStamp\"%d.%06d\",co(0x%08x),c2o(0x%08x),i(%d),VA(0x%x),PA(0x%x),O(0x%x),2O(0x%x)",
					deque_buf.data[i].timeStampS,
					deque_buf.data[i].timeStampUs, imgo_fbc, img2o_fbc,
...
#if 0
				LOG_DBG
				    ("+LARB in DEQUE,BWL(0x%08X)/(0x%08X)/(0x%08X)/(0x%08X),220(0x%08X)/(0x%08X)",
				     ISP_RD32(0xF0202000 + 0x204), ISP_RD32(0xF0202000 + 0x20c),
				     ISP_RD32(0xF0202000 + 0x210), ISP_RD32(0xF0202000 + 0x214),
				     ISP_RD32(0xF0202000 + 0x220), ISP_RD32(0xF0202000 + 0x230));
				LOG_DBG
				    ("+LARB in DEQUE,BWL 0x14(0x%08X)/(0x%08X)/(0x%08X)/(0x%08X)/(0x%08X)",
				     ISP_RD32(SMI_LARB0 + 0x10), ISP_RD32(SMI_LARB1 + 0x10),
				     ISP_RD32(SMI_LARB2 + 0x10), ISP_RD32(SMI_LARB3 + 0x10),
				     ISP_RD32(SMI_LARB4 + 0x10));
#endif
				...
				/*  */
				if (Timeout == 0) {
					LOG_ERR("[rtbc][DEQUE]:timeout(%d)", bDeqBuf);
					LOG_ERR("ISP_IRQ_TYPE_INT:IrqStatus(0x%08X)",
						IspInfo.IrqInfo.Status[ISP_IRQ_TYPE_INT]);
					ISP_DumpReg();
					Ret = -EFAULT;
				}
...
static MINT32 ISP_SOF_Buf_Get(unsigned long long sec, unsigned long usec)
{
	...
	if (0 == pstRTBuf->dropCnt) {
		...
		("[rtbc]TStamp(%d.%06d),curr(%d),pa(0x%x/0x%x),o2o(0x%x/0x%x),fbc_o(0x%08x),fbc_2o(0x%08x),cq0c(0x%x)",
			     ...
			     pstRTBuf->ring_buf[_imgo_].data[curr_imgo].timeStampUs, curr_imgo,
			     pstRTBuf->ring_buf[_imgo_].data[curr_imgo].base_pAddr,
			     pstRTBuf->ring_buf[_img2o_].data[curr_imgo].base_pAddr,
			     ISP_RD32(ISP_REG_ADDR_IMGO_BASE_ADDR),
			     ISP_RD32(ISP_REG_ADDR_IMG2O_BASE_ADDR), imgo_fbc.Reg_val,
			     img2o_fbc.Reg_val, ISP_RD32(ISP_ADDR + 0xBC));
		}
		/*  */
	}
	...
	if (IspInfo.DebugMask & ISP_DBG_INT) {
		LOG_DBG("[rtbc]E:fbc(0x%x/0x%x)/aaa(%d)", ISP_RD32(ISP_REG_ADDR_IMGO_FBC),
			ISP_RD32(ISP_REG_ADDR_IMG2O_FBC), aaa++);
	}
	/*  */
	/* spin_lock_irqsave(&(IspInfo.SpinLockRTBC),g_Flash_SpinLock); */
	/*  */
	...
	for (i = 0; i <= 1; i++) {
		...
		if (0 == pstRTBuf->ring_buf[i_dma].empty_count) {
			...
			if (2 <= pstRTBuf->ring_buf[i_dma].empty_count) {
				...
				next =
				    (pstRTBuf->ring_buf[i_dma].start +
				     1) % pstRTBuf->ring_buf[i_dma].total_count;
				...
				if (IspInfo.DebugMask & ISP_DBG_BUF_CTRL) {
					LOG_DBG
					    ("[rtbc][SOF]:dma(%d),start(%d),empty(%d),next(%d),next_PA(0x%x) ",
					     i_dma, pstRTBuf->ring_buf[i_dma].start,
					     pstRTBuf->ring_buf[i_dma].empty_count, next, pAddr);
				}
			} else {
				if (IspInfo.DebugMask & ISP_DBG_INT) {
					LOG_DBG
...
/* int k; */
	...
	if (IspInfo.DebugMask & ISP_DBG_BUF_CTRL) {
		LOG_DBG("[rtbc]E:fbc(0x%x/0x%x)", ISP_RD32(ISP_REG_ADDR_IMGO_FBC),
			ISP_RD32(ISP_REG_ADDR_IMG2O_FBC));
	}
	/*  */
#if 0
		...
		while (1) {
			...
			if (IspInfo.DebugMask & ISP_DBG_INT) {
				LOG_DBG("i_dma(%d),curr(%d),bFilled(%d)",
					i_dma, curr, pstRTBuf->ring_buf[i_dma].data[curr].bFilled);
			}
...
static MINT32 ISP_WaitIrq(ISP_WAIT_IRQ_STRUCT WaitIrq)
{
	...
	while (1) {
		IrqStatus[ISP_IRQ_TYPE_INT] =
		    (ISP_RD32(ISP_REG_ADDR_INT_STATUS) &
		     (IspInfo.IrqInfo.Mask[ISP_IRQ_TYPE_INT] | IspInfo.IrqInfo.
		      ErrMask[ISP_IRQ_TYPE_INT]));
		IrqStatus[ISP_IRQ_TYPE_DMA] =
		    (ISP_RD32(ISP_REG_ADDR_DMA_INT) &
		     (IspInfo.IrqInfo.Mask[ISP_IRQ_TYPE_DMA] | IspInfo.IrqInfo.
		      ErrMask[ISP_IRQ_TYPE_DMA]));
		IrqStatus[ISP_IRQ_TYPE_INTB] =
		    (ISP_RD32(ISP_REG_ADDR_INTB_STATUS) &
		     (IspInfo.IrqInfo.Mask[ISP_IRQ_TYPE_INTB] | IspInfo.IrqInfo.
		      ErrMask[ISP_IRQ_TYPE_INTB]));
		IrqStatus[ISP_IRQ_TYPE_DMAB] =
		    (ISP_RD32(ISP_REG_ADDR_DMAB_INT) &
		     (IspInfo.IrqInfo.Mask[ISP_IRQ_TYPE_DMAB] | IspInfo.IrqInfo.
		      ErrMask[ISP_IRQ_TYPE_DMAB]));
		IrqStatus[ISP_IRQ_TYPE_INTC] =
		    (ISP_RD32(ISP_REG_ADDR_INTC_STATUS) &
		     (IspInfo.IrqInfo.Mask[ISP_IRQ_TYPE_INTC] | IspInfo.IrqInfo.
		      ErrMask[ISP_IRQ_TYPE_INTC]));
		IrqStatus[ISP_IRQ_TYPE_DMAC] =
		    (ISP_RD32(ISP_REG_ADDR_DMAC_INT) &
		     (IspInfo.IrqInfo.Mask[ISP_IRQ_TYPE_DMAC] | IspInfo.IrqInfo.
		      ErrMask[ISP_IRQ_TYPE_DMAC]));
		IrqStatus[ISP_IRQ_TYPE_INTX] =
		    (ISP_RD32(ISP_REG_ADDR_INT_STATUSX) &
		     (IspInfo.IrqInfo.Mask[ISP_IRQ_TYPE_INTX] | IspInfo.IrqInfo.
		      ErrMask[ISP_IRQ_TYPE_INTX]));
		IrqStatus[ISP_IRQ_TYPE_DMAX] =
		    (ISP_RD32(ISP_REG_ADDR_DMA_INTX) &
		     (IspInfo.IrqInfo.Mask[ISP_IRQ_TYPE_DMAX] | IspInfo.IrqInfo.
		      ErrMask[ISP_IRQ_TYPE_DMAX]));
		/*  */
		for (i = 0; i < ISP_IRQ_TYPE_AMOUNT; i++) {
			if (IspInfo.IrqInfo.ErrMask[i] & IrqStatus[i]) {
			/* LOG_ERR("Error IRQ, Type(%d), Status(0x%08X)",
					i,IspInfo.IrqInfo.ErrMask[i] & IrqStatus[i]); */
				/* TODO: Add error handler... */
			}
			/*  */
			if (IspInfo.DebugMask & ISP_DBG_INT) {
				LOG_DBG("Type(%d), IrqStatus(0x%08X | 0x%08X)", i,
					IspInfo.IrqInfo.Status[i], IrqStatus[i]);
			/* LOG_DBG("Mask(0x%08X), ErrMask(0x%08X)",
					IspInfo.IrqInfo.Mask[i], IspInfo.IrqInfo.ErrMask[i]); */
			}
	...
	if (Timeout == 0) {
		LOG_ERR("Clear(%d), Type(%d), IrqStatus(0x%08X), WaitStatus(0x%08X), Timeout(%d)",
			WaitIrq.Clear,
			WaitIrq.Type,
			IspInfo.IrqInfo.Status[WaitIrq.Type], WaitIrq.Status, WaitIrq.Timeout);
...
*
********************************************************************************/
static __tcmfunc irqreturn_t ISP_Irq(MINT32 Irq, void *DeviceId)
{
/* LOG_DBG("- E."); */
	MUINT32 i;
	...
	IrqStatus[ISP_IRQ_TYPE_INT] =
	    (ISP_RD32(ISP_REG_ADDR_INT_STATUS) &
	     (IspInfo.IrqInfo.Mask[ISP_IRQ_TYPE_INT] | IspInfo.IrqInfo.ErrMask[ISP_IRQ_TYPE_INT]));
	IrqStatus[ISP_IRQ_TYPE_DMA] =
	    (ISP_RD32(ISP_REG_ADDR_DMA_INT) &
	     (IspInfo.IrqInfo.Mask[ISP_IRQ_TYPE_DMA] | IspInfo.IrqInfo.ErrMask[ISP_IRQ_TYPE_DMA]));
	IrqStatus[ISP_IRQ_TYPE_INTB] =
	    (ISP_RD32(ISP_REG_ADDR_INTB_STATUS) &
	     (IspInfo.IrqInfo.Mask[ISP_IRQ_TYPE_INTB] | IspInfo.IrqInfo.
	      ErrMask[ISP_IRQ_TYPE_INTB]));
	...
	for (i = 0; i < ISP_IRQ_TYPE_AMOUNT; i++) {
		if (IspInfo.IrqInfo.ErrMask[i] & IrqStatus[i]) {
			...
			if (gChipVer == 0x00) {	/* E1 */
				if (((i ==
				      ISP_IRQ_TYPE_INTX) & (IspInfo.IrqInfo.
					...
					LOG_DBG("Error IRQ, Type(%d), Status(0x%08X)", i,
						IspInfo.IrqInfo.ErrMask[i] & IrqStatus[i]);
					/* TODO: Add error handler... */
				}
			} else {
	...
	if (IrqStatus[ISP_IRQ_TYPE_INTX] & ISP_IRQ_INTX_STATUS_DMA_ERR_ST) {
		...
		LOG_DBG("ERR (8018 0x%08x)(8024 0x%08x)(802C 0x%08x)(4440 0x%08x) (810C 0x%08x)",
			ISP_RD32(0xF5008018),
			ISP_RD32(0xF5008024),
			ISP_RD32(0xF500802C), ISP_RD32(0xF5004440), ISP_RD32(0xF500810C));
#endif
...
static long ISP_ioctl(struct file *pFile, unsigned int Cmd, unsigned long Param)
{
	...
	case ISP_GET_CHIP_VER:
		{
			if (copy_from_user(&chipVer, (void *)Param, sizeof(int)) == 0) {
				chipVer = gChipVer;
				/* LOG_DBG("chip ver:0x%x\n",chipVer); */
				/*  */
				if (copy_to_user((void *)Param, &chipVer, sizeof(int)) != 0) {
	...
	case ISP_READ_IRQ:
		{
			if (copy_from_user(&ReadIrq, (void *)Param, sizeof(ISP_READ_IRQ_STRUCT)) ==
				...
				/*  */
				for (i = 0; i < ISP_IRQ_TYPE_AMOUNT; i++) {
					IspInfo.IrqInfo.Status[i] |=
					    (IrqStatus[i] & IspInfo.IrqInfo.Mask[i]);
				}
				/*  */
				spin_unlock_irqrestore(&(IspInfo.SpinLockIrq), flags);
#endif
	...
	case ISP_SENSOR_FREQ_CTRL:
		{
			...
			if (copy_from_user(&senFreq, (void *)Param, sizeof(MUINT32)) == 0) {
				...
				clksrc =
				    senFreq == 0 ? MT_CG_V_CKSQ_MUX_CK : (senFreq ==
									  1 ? MT_CG_V_UNIVPLL_D26 :
									  MT_CG_V_UNIVPLL1_D6);
				clkmux_sel(MT_CLKMUX_CAMTG_SEL, clksrc, "CAMERA_SENSOR");
			} else {
...
/* IspInfo.DebugMask = ISP_DBG_BUF_CTRL; */
 ...
 EXIT:
	...
	/* #warning for MT8135, temperally remove spm_disable_sodi */
	/* spm_disable_sodi(); */
...
static MINT32 ISP_release(struct inode *pInode, struct file *pFile)
{
 ...
 EXIT:
	...
	/* #warning for MT8135, temperally remove spm_enable_sodi */
	/* spm_enable_sodi(); */
...
static MINT32 ISP_probe(struct platform_device *pDev)
{
	...
	for (i = 0; i < 1; i++) {
		/* NEED_TUNING_BY_CHIP.
		1: Only one IORESOURCE_MEM type resource in kernel\mt_devs.c\mt_resource_isp[]. */
				 ...
				 loff_t *ppos)
{
	...
	MINT32 ret = 0;
	/*  */
	LOG_DBG("- E. pPage: 0x%08x. off: %d. Count: %d.", (unsigned int)pPage, (unsigned int)off,
		Count);
	/*  */
			    ...
			    loff_t *ppos)
{
	...
	if (sscanf(RegBuf, "%x %x", &Addr, &Data) == 2) {
		...
		LOG_ERR("Read => Addr: 0x%08X, Read Data: 0x%08X.", ISP_ADDR_CAMINF + Addr,
			      ...
			      loff_t *ppos)
{
	...
	if (sscanf(RegBuf, "%x %x", &Addr, &Data) == 2) {
		...
		LOG_ERR("Write => Addr: 0x%08X, Write Data: 0x%08X. Read Data: 0x%08X.",
			GPIO_BASE + Addr, Data, ISP_RD32(GPIO_BASE + Addr));
	} else if (sscanf(RegBuf, "%x", &Addr) == 1) {
		proc_regOfst = Addr;
		LOG_ERR("Read => Addr: 0x%08X, Read Data: 0x%08X.", GPIO_BASE + Addr,
			ISP_RD32(GPIO_BASE + Addr));
	}
	/*  */
	LOG_DBG("- X. Count: %d.", (int)Count);
	return Count;
...
static MINT32 __init ISP_Init(void)
{
	...
	/* the clock is opened at default, we should close it first when we do init */
	disable_clock(MT_CG_ISP_COMM_SMI_CKPDN, "CAMERA");
	/* get chip version */
	gChipVer = mt_get_chip_sw_ver();
	LOG_DBG("gChipVer(0x%x)", gChipVer);
	/*  */
	LOG_DBG("- X. Ret: %d.", Ret);
	return Ret;
}
