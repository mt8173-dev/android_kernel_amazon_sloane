	...
	for (i = 0; i < CAM_PIPE_MGR_PIPE_AMOUNT; i++) {
		if ((1 << i) & PipeMask) {
			if (CamPipeMgr.PipeInfo[i].Pid == 0 && CamPipeMgr.PipeInfo[i].Tgid == 0) {
				...
				LOG_ERR
				    ("PipeMask(0x%08lX),Pipe(%ld,%s),Pid(%d),Tgid(%d),Time(%ld.%06ld)",
...
static long CamPipeMgr_Ioctl_Lock(CAM_PIPE_MGR_PROC_STRUCT *pProc, CAM_PIPE_MGR_LOCK_STRUCT *pLock)
{
	MINT32 Ret = 0;
	CAM_PIPE_MGR_LOCK_STRUCT Lock = *pLock;
	...
	if ((Lock.PipeMask & CamPipeMgr.PipeLockTable[CamPipeMgr.Mode.ScenHw]) != Lock.PipeMask) {
		if ((Lock.PipeMask & CAM_PIPE_MGR_PIPE_MASK_CDP_CONCUR)
			...
			LOG_WRN
				("LOCK:Sw(%d),Hw(%d),LPM(0x%08lX),PLT(0x%08lX) fail",
		...
		if (Status == CAM_PIPE_MGR_STATUS_OK) {
			...
			if (pProc->Tgid == 0) {
				...
				if (CamPipeMgr.LogMask & Lock.PipeMask) {
					LOG_MSG
						("LOCK:Sw(%d),Hw(%d),LPM(0x%08lX),PLT(0x%08lX) OK",
						 ...
						 PipeLockTable[CamPipeMgr.Mode.
							   ScenHw]);
					LOG_MSG
						("LOCK:Proc:Name(%s),Pid(%d),Tgid(%d),PipeMask(0x%08lX)",
			...
			if ((CamPipeMgr.LogMask & Lock.PipeMask) ||
				...
				if ((Lock.
					 PipeMask &
					 CAM_PIPE_MGR_PIPE_MASK_CDP_CONCUR)
					|| (Lock.
					PipeMask &
					CAM_PIPE_MGR_PIPE_MASK_CDP_LINK)) {
					LOG_WRN
						("LOCK:Sw(%d),Hw(%d),LPM(0x%08lX),PLT(0x%08lX) fail,Status(%d)",
						 CamPipeMgr.Mode.ScenSw,
						 CamPipeMgr.Mode.ScenHw,
						 Lock.PipeMask,
						 CamPipeMgr.
	...
	case CAM_PIPE_MGR_LOCK:
		{
			if (copy_from_user(&Lock, (void *)Param, sizeof(CAM_PIPE_MGR_LOCK_STRUCT))
			    == 0) {
				CamPipeMgr_Ioctl_Lock(pProc, &Lock);
			} else {
			...
			if (copy_from_user
				...
				if (pProc->PipeMask & Unlock.PipeMask) {
					...
					if (CamPipeMgr.LogMask & Unlock.PipeMask) {
						LOG_MSG
						    ("UNLOCK:Sw(%d),Hw(%d),UPM(0x%08lX),PLT(0x%08lX) OK",
						     CamPipeMgr.Mode.ScenSw, CamPipeMgr.Mode.ScenHw,
						     Unlock.PipeMask,
						     CamPipeMgr.PipeLockTable[CamPipeMgr.Mode.
									      ScenHw]);
						LOG_MSG
						    ("UNLOCK:Proc:Name(%s),Pid(%d),Tgid(%d),PipeMask(0x%08lX)",
					...
					if ((CamPipeMgr.LogMask & Unlock.PipeMask) ||
						...
						LOG_ERR
						    ("UNLOCK:Sw(%d),Hw(%d),UPM(0x%08lX),PLT(0x%08lX) fail, it was not locked before",
						     CamPipeMgr.Mode.ScenSw, CamPipeMgr.Mode.ScenHw,
						     Unlock.PipeMask,
						     CamPipeMgr.PipeLockTable[CamPipeMgr.Mode.
									      ScenHw]);
	...
	case CAM_PIPE_MGR_SET_MODE:
		{
			if (copy_from_user(&Mode, (void *)Param, sizeof(CAM_PIPE_MGR_MODE_STRUCT))
				...
				if ((CamPipeMgr.PipeMask | CamPipeMgr.PipeLockTable[Mode.ScenHw]) ^
	...
	case CAM_PIPE_MGR_ENABLE_PIPE:
		{
			if (copy_from_user
				...
				LOG_MSG("ENABLE_PIPE:Sw(%d),Hw(%d):EPM(0x%08lX),PLT(0x%08lX)",
					CamPipeMgr.Mode.ScenSw, CamPipeMgr.Mode.ScenHw,
					Enable.PipeMask,
					CamPipeMgr_GtePipeLockTable(CamPipeMgr.Mode.ScenSw,
								    CamPipeMgr.Mode.ScenHw));
				if ((Enable.
 ...
 EXIT:
	if (Ret != 0) {
		if ((CamPipeMgr.LogMask & Lock.PipeMask) ||
			...
			if (pFile->private_data != NULL) {
				LOG_ERR
				    ("Proc:Name(%s),Pid(%d),Tgid(%d),PipeMask(0x%08lX),Time(%ld.%06ld)",
...
static int CamPipeMgr_Probe(struct platform_device *pDev)
{
	...
	strcpy(CamPipeMgr.PipeName[CAM_PIPE_MGR_PIPE_CDP_CAM], CAM_PIPE_MGR_PIPE_NAME_CDP_CAM);
	strcpy(CamPipeMgr.PipeName[CAM_PIPE_MGR_PIPE_CDP_CONCUR],
