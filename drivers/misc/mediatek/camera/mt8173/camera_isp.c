...
struct clk *g_ispclk_scp_sys_isp;
...
struct clk *g_ispclk_scp_sys_dis;
...
static inline void Get_ccf_clock(struct platform_device *pDev)
{
	...
	g_ispclk_scp_sys_isp = devm_clk_get(&pDev->dev, "SCP_SYS_ISP");
	BUG_ON(IS_ERR(g_ispclk_scp_sys_isp));
	...
	g_ispclk_scp_sys_dis = devm_clk_get(&pDev->dev, "SCP_SYS_DIS");
	BUG_ON(IS_ERR(g_ispclk_scp_sys_dis));
...
{
	BUG_ON(IS_ERR(g_ispclk_scp_sys_isp));
	clk_prepare(g_ispclk_scp_sys_isp);
	BUG_ON(IS_ERR(g_ispclk_larb2_smi));
	...
	/*BUG_ON(IS_ERR(g_ispclk_fd     ));     clk_prepare(g_ispclk_fd); */
	BUG_ON(IS_ERR(g_ispclk_scp_sys_dis));
	clk_prepare(g_ispclk_scp_sys_dis);
	BUG_ON(IS_ERR(g_ispclk_smi_common));
...
{
	BUG_ON(IS_ERR(g_ispclk_scp_sys_isp));
	clk_enable(g_ispclk_scp_sys_isp);
	BUG_ON(IS_ERR(g_ispclk_larb2_smi));
	...
	/*BUG_ON(IS_ERR(g_ispclk_fd     ));     clk_enable(g_ispclk_fd); */
	BUG_ON(IS_ERR(g_ispclk_scp_sys_dis));
	clk_enable(g_ispclk_scp_sys_dis);
	BUG_ON(IS_ERR(g_ispclk_smi_common));
...
static inline void Disable_ccf_clock(void)
{
	...
	/*BUG_ON(IS_ERR(g_ispclk_fd     ));     clk_disable(g_ispclk_fd); */
	BUG_ON(IS_ERR(g_ispclk_scp_sys_isp));
	clk_disable(g_ispclk_scp_sys_isp);
	BUG_ON(IS_ERR(g_ispclk_smi_common));
	clk_disable(g_ispclk_smi_common);
	BUG_ON(IS_ERR(g_ispclk_scp_sys_dis));
	clk_disable(g_ispclk_scp_sys_dis);
	return;
}
...
static inline void Unprepare_ccf_clock(void)
{
	...
	/*BUG_ON(IS_ERR(g_ispclk_fd     ));     clk_unprepare(g_ispclk_fd); */
	BUG_ON(IS_ERR(g_ispclk_scp_sys_isp));
	clk_unprepare(g_ispclk_scp_sys_isp);
	BUG_ON(IS_ERR(g_ispclk_smi_common));
	clk_unprepare(g_ispclk_smi_common);
	BUG_ON(IS_ERR(g_ispclk_scp_sys_dis));
	clk_unprepare(g_ispclk_scp_sys_dis);
	return;
}
				  ...
				  loff_t *ppos)
{
	...
	if (sscanf(RegBuf, "%x %x", &Addr, &Data) == 2) {
		proc_regOfst = Addr;
		ISP_WR32(compat_ptr(GPIO_BASE + Addr), Data);
