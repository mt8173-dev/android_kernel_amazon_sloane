...
struct clk *g_fdvtclk_scp_sys_isp;
struct clk *g_fdvtclk_fd;
struct clk *g_fdvtclk_scp_sys_dis;
...
static inline void Get_ccf_clock(struct platform_device *pDev)
{
	...
	g_fdvtclk_scp_sys_isp = devm_clk_get(&pDev->dev, "SCP_SYS_ISP");
	BUG_ON(IS_ERR(g_fdvtclk_scp_sys_isp));
	g_fdvtclk_fd = devm_clk_get(&pDev->dev, "IMG_FD");
	BUG_ON(IS_ERR(g_fdvtclk_fd));
	g_fdvtclk_scp_sys_dis = devm_clk_get(&pDev->dev, "SCP_SYS_DIS");
	BUG_ON(IS_ERR(g_fdvtclk_scp_sys_dis));
...
{
	BUG_ON(IS_ERR(g_fdvtclk_scp_sys_isp));
	clk_prepare_enable(g_fdvtclk_scp_sys_isp);
	BUG_ON(IS_ERR(g_fdvtclk_fd));
	clk_prepare_enable(g_fdvtclk_fd);
	BUG_ON(IS_ERR(g_fdvtclk_scp_sys_dis));
	clk_prepare_enable(g_fdvtclk_scp_sys_dis);
	BUG_ON(IS_ERR(g_fdvtclk_smi_common));
...
static inline void DisableUnprepare_ccf_clock(void)
{
	...
	BUG_ON(IS_ERR(g_fdvtclk_scp_sys_isp));
	clk_disable_unprepare(g_fdvtclk_scp_sys_isp);
	BUG_ON(IS_ERR(g_fdvtclk_smi_common));
	clk_disable_unprepare(g_fdvtclk_smi_common);
	BUG_ON(IS_ERR(g_fdvtclk_scp_sys_dis));
	clk_disable_unprepare(g_fdvtclk_scp_sys_dis);
	return;
}
