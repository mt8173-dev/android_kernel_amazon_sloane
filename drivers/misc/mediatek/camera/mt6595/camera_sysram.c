		...
		if (pProc->Pid != 0 || pProc->Tgid != 0 || pProc->Table != 0) {
			...
			if (pProc->Tgid == 0 && pProc->Table != 0) {
				...
				/*
				   ...
				   pProc->TimeUS);
				 */
			} else if ((pProc->Tgid == current->tgid) || ((pProc->Tgid != current->tgid)
								      &&
								      (strcmp
								       (current->comm,
									"binder") == 0))) {
...
static int SYSRAM_mmap(struct file *pFile, struct vm_area_struct *pVma)
{
	...
	if ((length > ISP_VALID_REG_RANGE) || (pfn < IMGSYS_BASE_ADDR)
		...
		LOG_ERR("mmap range error : vm_start(0x%x),vm_end(0x%x),length(0x%x),pfn(0x%x)!",
...
/* ------------------------------------------------------------------------------ */
static int SYSRAM_DumpLayoutToProc(struct file *pFile, char *pBuffer, size_t Count, loff_t *Off)
{
	char *p = (char *)pFile;
	MUINT32 len = 0;
	MUINT32 Index = 0;
...
/* ------------------------------------------------------------------------------ */
static int SYSRAM_ReadFlag(struct file *pFile, char *pBuffer, size_t Count, loff_t *Off)
{
	char *p = (char *)pFile;
	char **ppStart = NULL;
	MUINT32 len = 0;
...
/* ------------------------------------------------------------------------------ */
static int SYSRAM_WriteFlag(struct file *pFile, const char *pBuffer, size_t Count, loff_t *pData)
{
	char acBuf[32];
