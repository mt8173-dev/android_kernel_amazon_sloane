			  ...
			  size_t count)
{
	...
	/* if ( sscanf(buf, "%d %d %d %d %d %d %d %d %d %d %d %d %s",
	   &bus_id, &address, &operation, &trans_mode, &trans_stop, &speed_mode,
	   &pushpull_mode, &query_mode, &timing, &trans_num, &trans_auxlen,&dir, data_buffer) ) { */
	scanf_ret = sscanf
	    (buf, "%d %x %d %d %d %d %d %d %d %d %d %d %1023s", &bus_id, &address, &operation, &trans_mode,
	     &trans_stop, &speed_mode, &pushpull_mode, &query_mode, &timing, &trans_num,
	     &trans_auxlen, &data_len, data_buffer);
	if (scanf_ret) {
		if ((address != 0) && (operation <= 2)) {
			...
			if (trans_mode == 0) {
				...
				I2CLOG("invalid trans_mod fifo/dma\n");
				goto err;
			}
			...
			if (query_mode == 0) {
				...
				I2CLOG("invalid query mode interrupt/polling\n");
				goto err;
			}

			if (trans_mode == 1) {	/*DMA MODE */
				vir_addr =
				    dma_alloc_coherent(NULL, length >> 1, &dma_addr, GFP_KERNEL);
				if (vir_addr == NULL) {

					I2CLOG("alloc dma memory failed\n");
					goto err;
				}
			} else {
				...
				if (vir_addr == NULL) {
					...
					I2CLOG("alloc virtual memory failed\n");
					goto err;
				}
			}

			get_hexbuffer(data_buffer, vir_addr, length);
			I2CLOG(KERN_ALERT "bus_id:%d,address:%x,count:%x,ext_flag:0x%x,timing:%d\n",
			       bus_id, address, number, ext_flag, timing);
			I2CLOG(KERN_ALERT "data_buffer:%s\n", data_buffer);

			if (trans_mode == 1)
				/*DMA*/
				    ret = i2c_trans_data(bus_id, address, (void *)dma_addr, number,
							 ext_flag, timing);
			else
				ret = i2c_trans_data(bus_id, address, vir_addr, number, ext_flag,
						     timing);
			/* dealing */
			...
			if (ret >= 0) {
				...
				I2CLOG("Actual return Value:%d %p\n", ret, vir_addr);
			} else if (ret < 0) {
				...
				if (ret == -EINVAL)
					...
					snprintf(data_buffer, sizeof(data_buffer), "0 %s", "unknow error");
				I2CLOG("Actual return Value:%d %p\n", ret, vir_addr);
			}
...
static int i2c_common_probe(struct platform_device *pdev)
{
	...
	I2CLOG(KERN_ALERT "i2c_common device probe\n");
...
{
	I2CLOG(KERN_ALERT "i2c_common device init\n");
