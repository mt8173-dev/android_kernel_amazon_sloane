...
void i2c_writel(struct mt_i2c *i2c, U8 offset, U16 value)
{
	...
	mt_reg_sync_writel((u32) value, (i2c->base) + (offset));
}

U32 i2c_readl(struct mt_i2c *i2c, U8 offset)
{
	...
	if (FALSE == i2c->dma_en) {	/*non-DMA mode */
		if (I2C_MASTER_WRRD != i2c->op) {
			...
			if (!trans_len || !trans_num || trans_len * trans_num > 8) {
				I2CERR
				    (" non-WRRD transfer length wrong.trans_len=%x, tans_num=%x, trans_auxlen=%x\n",
		...
		if (I2C_MASTER_WRRD != i2c->op) {
			...
			if (!trans_len || !trans_num || trans_len > MAX_DMA_TRANS_SIZE
				...
				I2CERR
				    ("DMA non-WRRD trans length wrong.trans_len=%x, tans_num=%x, trans_auxlen=%x\n",
				     trans_len, trans_num, trans_auxlen);
			...
			if (!trans_len || !trans_auxlen || trans_len > MAX_DMA_TRANS_SIZE
				...
				I2CERR
				    ("DMA WRRD transfer length wrong trans_len=%x, tans_num=%x, trans_auxlen=%x\n",
				     trans_len, trans_num, trans_auxlen);
...
static S32 mt_i2c_start_xfer(struct mt_i2c *i2c, struct i2c_msg *msg)
{
	S32 return_value = 0;
	S32 ret = msg->len;
...
static S32 mt_i2c_do_transfer(struct mt_i2c *i2c, struct i2c_msg *msgs, S32 num)
{
	S32 ret = 0;
	S32 left_num = num;
...
#ifdef I2C_DRIVER_IN_KERNEL
static S32 _i2c_deal_result_3dcamera(struct mt_i2c *i2c, struct i2c_msg *msg)
{
	U16 addr = msg->addr;
...
#ifdef CONFIG_OF
static void mt_i2c_parse_dt(struct device_node *np, struct mt_i2c *i2c)
{
	if (!np)
		return;

	of_property_read_u32(np, "id", (u32 *) &i2c->id);
	of_property_read_u32(np, "pdn", &i2c->pdn);
	of_property_read_u32(np, "dma_pdn", &i2c->dma_pdn);
}
#endif
...
static S32 mt_i2c_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_OF
	mt_i2c_parse_dt(pdev->dev.of_node, i2c);
	/* initialize mt_i2c structure */
	i2c->base = of_iomap(pdev->dev.of_node, 0);
	if (i2c->base == NULL) {
		dev_err(&pdev->dev, "i2c->base == NULL\n");
		ret = -ENOMEM;
		goto io_map_err;
	}

	i2c->irqnr = irq_of_parse_and_map(pdev->dev.of_node, 0);
	if (i2c->irqnr) {
		ret = request_irq(i2c->irqnr, mt_i2c_irq, IRQF_TRIGGER_LOW, I2C_DRV_NAME, i2c);
		if (ret < 0) {
	...
	i2c->pdmabase = of_iomap(pdev->dev.of_node, 1);
	if (i2c->pdmabase == NULL) {
		dev_err(&pdev->dev, "i2c->pdmabase is NULL\n");
		ret = -ENOMEM;
		goto dma_io_map_err;
	}
#else
