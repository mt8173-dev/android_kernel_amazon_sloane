...
static long cmdq_ioctl(struct file *pFile, unsigned int code, unsigned long param)
{
	...
	case CMDQ_IOCTL_SYNC_BUF_HDCP_VERSION:
		do {
			...
			if (copy_from_user(&syncHandle, (void *)param, sizeof(syncHandle))) {
				CMDQ_ERR("CMDQ_IOCTL_SYNC_BUF_HDCP_VERSION copy_from_user failed\n");
				return -EFAULT;
			}
	...
	default:
		...
		CMDQ_ERR("sizeof(X) is a operator, the return value will be const value when compile this code,");
...
static int cmdq_probe(struct platform_device *pDevice)
{
	...
	if (NULL == proc_create(CMDQ_DRIVER_DEVICE_NAME, 0640, NULL, &cmdqOP)) {
...
static int __init cmdq_init(void)
{
	...
	cmdqCoreRegisterCB(CMDQ_GROUP_MDP, cmdqMdpClockOn, cmdqMdpDumpInfo, cmdqMdpResetEng, cmdqMdpClockOff);
