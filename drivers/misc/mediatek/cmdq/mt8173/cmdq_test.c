...
static void testcase_dram_access(void)
{
	...
	*pCmdEnd = CMDQ_DATA_REG_DEBUG;
	pCmdEnd += 1;
	/*1 1 0 */
	*pCmdEnd = (CMDQ_CODE_READ << 24) | (0 & 0xffff) | ((CMDQ_DATA_REG_DEBUG_DST & 0x1f) << 16) | (6 << 21);
	pCmdEnd += 1;
	...
	*pCmdEnd = CMDQ_DATA_REG_DEBUG;
	pCmdEnd += 1;
	/*1 1 0 */
	*pCmdEnd = (CMDQ_CODE_WRITE << 24) | (0 & 0xffff) | ((CMDQ_DATA_REG_DEBUG_DST & 0x1f) << 16) | (6 << 21);
	pCmdEnd += 1;
...
static void testcase_write_from_data_reg(void)
{
	...
	const uint32_t dstRegVA = CMDQ_TEST_DISP_PWM0_DUMMY_VA;
...
int32_t callback_func(unsigned long data)
{
	...
	case WAIT_MUTEX0_EOF_NORMAL:
		CMDQ_LOG
		    ("testcase_wait_mutex0_stream_eof execute done in normal world\n");
		break;
	case WAIT_MUTEX0_EOF_SECURE:
		CMDQ_LOG
		    ("testcase_wait_mutex0_stream_eof execute done in secure world\n");
		break;
	default:
...
{
	cmdqRecHandle handle;
	const uint32_t MMSYS_DUMMY_REG = MMSYS_CONFIG_BASE_VA + 0x0890;
	...
	for (i = 0; i < REP_COUNT; ++i)
		cmdqRecWrite(handle, __pa(testReg), PATTERN, ~0);
