...
void cmdq_core_run_print_log(struct work_struct *workItem)
{
	...
	if (pPrint->enablePrint) {
		if (!cmdq_print_log_task) {
			cmdq_print_log_task =
			    kthread_create(cmdq_core_print_log_kthread, NULL, "cmdq_core_print_log_kthread");
...
ssize_t cmdqCorePrintStatus(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	static const char *const engineNames[] = { CMDQ_FOREACH_STATUS_MODULE_PRINT(GENERATE_STRING) };
...
DEBUG_STATIC int32_t cmdq_core_task_realloc_buffer_size(struct TaskStruct *pTask, uint32_t size)
{
...
DEBUG_STATIC struct TaskStruct *cmdq_core_task_create(void)
{
	struct TaskStruct *pTask = NULL;
	int32_t status = 0;
...
DEBUG_STATIC void cmdq_core_release_task_unlocked(struct TaskStruct *pTask)
{
	pTask->taskState = TASK_STATE_IDLE;
	pTask->thread = CMDQ_INVALID_THREAD;
...
DEBUG_STATIC void cmdq_core_release_task(struct TaskStruct *pTask)
{
...
DEBUG_STATIC struct TaskStruct *cmdq_core_find_free_task(void)
{
	struct TaskStruct *pTask = NULL;
...
static void cmdq_core_reorder_task_array(struct ThreadStruct *pThread, int32_t prevID)
{
	...
	for (loop = 1; loop < (CMDQ_MAX_TASK_IN_THREAD - 1); loop++, nextID++) {
		...
		if (((CMDQ_CODE_JUMP << 24) == pThread->pCurTask[nextID]->pCMDEnd[0]) &&
		    (0x00000008 == pThread->pCurTask[nextID]->pCMDEnd[-1])) { /* jump 8 byte */
			/* We reached the last task's command end */
			break;
		}
	}
...
DEBUG_STATIC int32_t cmdq_core_insert_read_reg_command(struct TaskStruct *pTask,
...
/* #define CMDQ_PROFILE_COMMAND */
	...
	if (cmdq_core_is_request_from_user_space(pTask->scenario)) {
		...
		for (i = 0; i < pTask->regCount; i++) {
			...
			bool is_mdp_reg = (backup_reg >= 0x14001000) && (backup_reg < 0x1400B000);
			bool is_cmdq_reg = (backup_reg >= 0x10212080) && (backup_reg < 0x102120C0);
...
DEBUG_STATIC struct TaskStruct *cmdq_core_acquire_task(struct cmdqCommandStruct *pCommandDesc,
...
/*
	...
	false for can not
*/
DEBUG_STATIC const int32_t cmdq_core_can_start_to_acquire_HW_thread_unlocked(const uint64_t
									     engineFlag,
									     const bool isSecure)
{
	...
	do {
		...
		if (preferSecurePath == isSecure) {
			...
			CMDQ_MSG(
				"THREAD: is sec(%d, eng:0x%llx) as first waiting task(%p, eng:0x%llx), start thread dispatch.\n",
			     preferSecurePath, engineFlag, pFirstWaitingTask,
			     pFirstWaitingTask->engineFlag);
			break;
		}

		CMDQ_VERBOSE("THREAD: is not the first waiting task(%p), yield.\n", pFirstWaitingTask);
		status = -EFAULT;
	} while (0);
...
DEBUG_STATIC bool cmdq_core_check_engine_conflict_for_thread_dispatch_locked(const uint64_t
									     engineFlag,
									     bool forceLog,
									     const bool isSecure,
									     int32_t *pThreadOut)
...
DEBUG_STATIC uint32_t *cmdq_core_get_pc(const struct TaskStruct *pTask, uint32_t thread,
					uint32_t insts[4])
{
	long currPC = 0L;
	uint8_t *pInst = NULL;
					  ...
					  const bool fullTatskDump, const bool dumpCookie)
{
	...
	for (index = 0; index < CMDQ_MAX_TASK_IN_THREAD; index++) {
		...
		CMDQ_ERR(
		 "Slot %d, Task: 0x%p, VABase: 0x%p, MVABase 0x%pa, Size: %d, Last Inst 0x%08x:0x%08x, 0x%08x:0x%08x, priority:%d\n",
...
void cmdq_core_dump_instructions_to_file(uint64_t *pInstrBuffer, uint32_t bufferSize, const char *fileName)
{
	uint64_t *pBuffer = pInstrBuffer;
	char textBuffer[100];
	int i = 0;
...
static void cmdq_core_attach_error_task(const struct TaskStruct *pTask, int32_t thread)
{
	...
	if (NULL != pTask) {
		...
		CMDQ_ERR("=============== [CMDQ] Error Task Instructions ===============\n");
...
DEBUG_STATIC int32_t cmdq_core_insert_task_from_thread_array_by_cookie(struct TaskStruct *pTask,
								       struct ThreadStruct *pThread,
...
DEBUG_STATIC int32_t cmdq_core_remove_task_from_thread_array_by_cookie(struct ThreadStruct *
...
DEBUG_STATIC int32_t cmdq_core_force_remove_task_from_thread(struct TaskStruct *pTask,
							     uint32_t thread)
{
	int32_t status = 0;
	int32_t cookie = 0;
 ...
 */
DEBUG_STATIC int32_t cmdq_core_wait_task_done_with_timeout_impl(struct TaskStruct *pTask,
								int32_t thread)
{
	int32_t waitQ;
	unsigned long flags;
	...
	while (0 == waitQ && retryCount < CMDQ_PREDUMP_RETRY_COUNT) {
		...
		CMDQ_ERR("pTask[0x%p], trigger[%lld]\n", pTask, pTask->trigger);
		if (0 == retryCount) {
...
DEBUG_STATIC int32_t cmdq_core_handle_wait_task_result_secure_impl(struct TaskStruct *pTask,
								   int32_t thread,
								   const int32_t waitQ)
{
...
DEBUG_STATIC int32_t cmdq_core_handle_wait_task_result_impl(struct TaskStruct *pTask,
							    int32_t thread, const int32_t waitQ)
{
	...
	do {
		...
		if ((((pTask->pCMDEnd[0] & 0xFF000000) >> 24) == CMDQ_CODE_JUMP) &&
			((pTask->pCMDEnd[0] & ~0xFF000000) == 0x000001)) { /* to absolute addr */
			pNextTask =
...
DEBUG_STATIC int32_t cmdq_core_exec_task_async_secure_impl(struct TaskStruct *pTask,
							   int32_t thread)
{
#ifndef CMDQ_SECURE_PATH_SUPPORT
	...
	unsigned long flags;
	CMDQ_MSG(
	 "-->EXEC: task 0x%p on thread %d begin, VABase: 0x%p, MVABase: %pa, Size: %d, bufferSize: %d, scenario:%d, flag:0x%llx\n",
...
DEBUG_STATIC int32_t cmdq_core_exec_task_async_impl(struct TaskStruct *pTask, int32_t thread)
{
	int32_t status;
	struct ThreadStruct *pThread;
	unsigned long flags;
	...
	if (pThread->taskCount <= 0) {
		...
		if ((CMDQ_AREG_TO_PHYS(CMDQ_REG_GET32(CMDQ_THR_CURR_ADDR(thread))) ==
			...
			CMDQ_LOG(
			 "EXEC: Set HW thread(%d) pc from 0x%08x(end:0x%08x) to %pa, oriNextCookie:%d, oriTaskCount:%d\n",
			 thread, CMDQ_REG_GET32(CMDQ_THR_CURR_ADDR(thread)),
			 CMDQ_REG_GET32(CMDQ_THR_END_ADDR(thread)), &pTask->MVABase, cookie,
			 pThread->taskCount);
			CMDQ_REG_SET32(CMDQ_THR_CURR_ADDR(thread),
			...
			for (prev = (cookie % CMDQ_MAX_TASK_IN_THREAD); loop > 0; loop--) {
				...
				if ((loop > 1)
				    && (pPrev->priority < pTask->priority)) {
					CMDQ_MSG
					    ("Switch prev(%d) and current(%d) order\n",
					     prev, index);
					...
					if (pLast == pTask)
					   ...
					   && (pPrev->priority >=
					       pTask->priority)) {
					CMDQ_MSG
					    ("Set current(%d) order for the new task\n",
					     index);

					CMDQ_MSG
					    ("Original PC %pa, end %pa\n",
					     &pPrev->MVABase,
					     &pPrev->MVABase + pPrev->commandSize);
					CMDQ_MSG
					    ("Original instruction 0x%08x, 0x%08x\n",
...
DEBUG_STATIC void cmdq_core_handle_secure_thread_done_impl(const int32_t thread,
...
DEBUG_STATIC void cmdq_core_handle_secure_paths_exec_done_notify(const int32_t notifyThread,
								 const int32_t value,
								 CMDQ_TIME *pGotIRQ)
{
	uint32_t i;
...
DEBUG_STATIC int32_t cmdq_core_exec_task_async_with_retry(struct TaskStruct *pTask, int32_t thread)
{
	int32_t retry;
	int32_t status;
		...
		if (CMDQ_INVALID_THREAD == thread || NULL == pThread) {
			...
			if (needLog) {
				...
				cmdqCoreLongString(needPrintLog, longMsg, &msgOffset, &msgMAXSize,
						   ...
						   pTask->scenario, pTask->submit, pTask->taskState);
				if (msgOffset > 0) {
					/* print message */
					/* for Error Handle */
					CMDQ_MSG("%s", longMsg);
...
int32_t cmdq_core_alloc_sec_metadata(struct cmdqCommandStruct *pCommandDesc, struct TaskStruct *pTask)
{
	int32_t status = 0;
	void *pNewBuf = NULL;
	if (pCommandDesc->secData.isSecure && pCommandDesc->secData.addrMetadataCount) {
		uint32_t size = sizeof(cmdqSecAddrMetadataStruct) * pCommandDesc->secData.addrMetadataCount;
