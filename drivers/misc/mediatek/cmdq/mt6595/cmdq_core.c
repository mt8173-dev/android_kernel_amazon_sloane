...
static bool cmdq_core_uninit_emergency_buffer(void)
{
	...
	for (i = 0; i < CMDQ_EMERGENCY_BLOCK_COUNT; ++i) {
		struct EmergencyBufferStruct *buf = &gCmdqEmergencyBuffer[i];
		dma_free_coherent(cmdq_dev_get(), CMDQ_EMERGENCY_BLOCK_SIZE, buf->va, buf->pa);
		if (buf->used) {
...
ssize_t cmdqCorePrintError(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < gCmdqContext.errNum && i < CMDQ_MAX_ERROR_COUNT; ++i) {
		...
		length +=
		    snprintf(buf + length, PAGE_SIZE - length, "[%5lu.%06lu] ", (unsigned long)ts,
			     rem_nsec / 1000);
		length +=
		    cmdq_core_print_record(&pError->errorRec, i, buf + length, PAGE_SIZE - length);
		if (length >= PAGE_SIZE)
			break;
...
void cmdq_task_free_task_command_buffer(struct TaskStruct *pTask)
{
	if (pTask->pVABase) {
		if (pTask->useEmergencyBuf) {
			...
			dma_free_coherent(cmdq_dev_get(), pTask->bufferSize, pTask->pVABase,
					  pTask->MVABase);
		}
...
DEBUG_STATIC int32_t cmdq_core_task_realloc_buffer_size(struct TaskStruct *pTask, uint32_t size)
{
	...
	do {
		...
		pNewBuffer =
		    dma_alloc_coherent(cmdq_dev_get(), size, &newMVABase,
...
DEBUG_STATIC void cmdq_core_release_task_unlocked(struct TaskStruct *pTask)
{
	...
	if (pTask->regResults) {
		CMDQ_MSG("COMMAND: Free result buf VA:0x%p, PA:%pa\n", pTask->regResults,
			 &pTask->regResultsMVA);
		dma_free_coherent(NULL, pTask->regCount * sizeof(pTask->regResults[0]),
...
#define CMDQ_PROFILE_COMMAND
	...
	status =
	    cmdq_core_task_realloc_buffer_size(pTask,
	...
	if (CMDQ_SCENARIO_USER_SPACE != pTask->scenario) {
		...
		memcpy(pTask->pCMDEnd + 1,
		       ...
		       (copiedBeforeProfile / sizeof(pCommandDesc->pVABase[0])),
	...
	do {
			...
			if (engine & (0x1LL << index)) {
				if (CMDQ_INVALID_THREAD == pEngine[index].currOwner) {
					...
					if (forceLog) {
						...
						CMDQ_LOG("THREAD: try locate on thread %d ",
							 thread);
						CMDQ_LOG
						    ("but engine %d also occupied by thread %d\n",
						     index, pEngine[index].currOwner);
					} else {
						/*line over 120 characters,so devide to two statments */
						CMDQ_VERBOSE
						    ("THREAD: try locate on thread %d ", thread);
						CMDQ_VERBOSE
						    ("but engine %d also occupied by thread %d\n",
						     index, pEngine[index].currOwner);
					}
		...
		if ((0xFFFFFFFF == free) && (CMDQ_INVALID_THREAD == thread)) {
			...
			if (CMDQ_INVALID_THREAD == thread) {
				...
				/* preserved for DISPSYS for high-prio threads */
				const bool isDisplayThread =
				    CMDQ_THR_PRIO_DISPLAY_TRIGGER < thread_prio;
				const bool isTriggerThread =
				    (CMDQ_SCENARIO_TRIGGER_LOOP == scenario);
				...
				for (index = startIndex; index < endIndex; ++index) {
					...
					if ((0 == pThread[index].taskCount)
						...
						/* line over 120 characters,so devide to two statments */
						CMDQ_VERBOSE
						    ("THREAD: dispatch to thread %d,", index);
						CMDQ_VERBOSE
					...
					uint32_t insts[4])
{
	...
	/* CMDQ_ERR("[PC]thr:%d, paPC:0x%08x, pVABase:0x%08x, */
...
int32_t cmdq_core_parse_instruction(const uint32_t *pCmd, char *textBuf, int bufLen)
{
	...
	case CMDQ_CODE_JUMP:
		if (argA) {
			...
			if ((int32_t) argB >= 0) {
				reqLen =
				    snprintf(textBuf, bufLen, "JUMP(REL): +%d\n", (int32_t) argB);
			} else {
				reqLen =
				    snprintf(textBuf, bufLen, "JUMP(REL): %d\n", (int32_t) argB);
			}
		...
		if (0x80008001 == argB) {
			...
			reqLen =
			    snprintf(textBuf, bufLen,
				     ...
				     cmdq_core_get_event_name(argA), (argB >> 31) & 0x1,
				     (argB >> 28) & 0x7, (argB >> 16) & 0xFFF, (argB >> 15) & 0x1,
	...
	case CMDQ_CODE_EOC:
		if (argA == 0 && argB == 0x00000001) {
			...
			if (cmdq_core_support_sync_non_suspendable()) {
				reqLen =
				    snprintf(textBuf, bufLen, "MARKER: sync_no_suspnd=%d",
					     (argA & (1 << 20)) > 0);
			} else {
			...
			reqLen =
			    snprintf(textBuf, bufLen,
				     ...
				     (argA & (1 << 21)) > 0, (argA & (1 << 16)) > 0,
				     (argB & (1 << 20)) > 0, (argB & (1 << 17)) > 0,
...
static void cmdq_core_attach_error_task(const struct TaskStruct *pTask, int32_t thread)
{
	...
	static const char *const engineGroupName[] = { CMDQ_FOREACH_GROUP(GENERATE_STRING) };
	...
	/* isSMIHang = smi_debug_bus_hanging_detect(SMI_DBG_DISPSYS |
	...
	do {
		...
		for (index = 0; index < CMDQ_MAX_TASK_IN_THREAD; index++) {
			...
			if (NULL != pDumpTask) {
				...
				CMDQ_ERR
				    ("Last Inst 0x%08x:0x%08x, 0x%08x:0x%08x\n",
				     pDumpTask->pCMDEnd[-3], pDumpTask->pCMDEnd[-2],
				     pDumpTask->pCMDEnd[-1], pDumpTask->pCMDEnd[0]);
			}
		}
	} while (0);
...
DEBUG_STATIC int32_t cmdq_core_remove_task_from_thread_array_by_cookie(struct ThreadStruct *pThread,
								       int32_t index,
...
DEBUG_STATIC int32_t cmdq_core_exec_task_async_impl(struct TaskStruct *pTask, int32_t thread)
{
	...
	/*end of if (pThread->taskCount <= 0) */
	/*enter else if (pThread->taskCount > 0) */
	...
	if ((CMDQ_AREG_TO_PHYS(CMDQ_REG_GET32(CMDQ_THR_CURR_ADDR(thread))) ==
		...
		for (index = (cookie % CMDQ_MAX_TASK_IN_THREAD); loop > 0; loop--, index--) {
			...
			if (NULL != pPrev) {
				if ((loop > 1) && (pPrev->priority < pTask->priority)) {
					CMDQ_MSG
				...
				} else if ((loop > 1) && (pPrev->priority >= pTask->priority)) {
	...
	list_for_each_safe(p, n, &gCmdqContext.taskWaitList) {
		...
		if (CMDQ_INVALID_THREAD == thread || NULL == pThread) {
			...
			if (needLog) {
				...
				/*line over 120 characters,so devide to two statments */
				CMDQ_ERR
				    ("acquire thread pre-dump for task: 0x%p, thread_prio: %d,",
				     pTask, thread_prio);
				CMDQ_ERR
				    (" task_prio: %d, flag: 0x%llx\n",
				     pTask->priority, pTask->engineFlag);
			}
			continue;
		}
...
int cmdqCoreAllocWriteAddress(uint32_t count, dma_addr_t *paStart)
{
	...
	do {
		...
		*paStart = 0;
		...
		pWriteAddr->va =
		    dma_alloc_coherent(cmdq_dev_get(), count * sizeof(uint32_t), &(pWriteAddr->pa),
				       GFP_KERNEL);
		if (current)
