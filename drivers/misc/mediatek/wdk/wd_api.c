...
int wd_api_init(void)
{
	...
	long  i = 0;
	long *check_p = NULL;
	unsigned long api_size = 0;

	api_size = (sizeof(g_wd_api_obj) /sizeof(long *));  //152 / 8 = 19
	/*printk("wd api_size=%zu /%zu = %zu\n", sizeof(g_wd_api_obj) , sizeof(long *), api_size);*/
	/* check wd api */
	check_p = (long  *)&g_wd_api_obj;
	for (i = 1; i < api_size; i++) {
		pr_info("p[%ld] = %ld\n", i, *(check_p + i));
		if (0 == check_p[i]) {
			pr_info("wd_api init fail the %ld api not init\n", i);
