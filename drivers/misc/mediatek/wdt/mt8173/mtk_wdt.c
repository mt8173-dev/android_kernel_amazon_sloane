 ...
 */
#define NO_DEBUG 1
#define warnmsg(msg...) pr_warn(msg);
...
/*
    ...
    value unit: second
*/
void mtk_wdt_set_time_out_value(unsigned int timeout)
{
	unsigned int ticks;

	spin_lock(&rgu_reg_operation_spinlock);
#ifdef CONFIG_KICK_SPM_WDT
	spm_wdt_set_timeout(timeout);
#else
	ticks = (unsigned int)(timeout * MTK_WDT_LENGTH_SECOND_TO_TICKS);
	ticks = ticks << MTK_WDT_LENGTH_FIELD_OFFSET;
	DRV_WriteReg32(MTK_WDT_LENGTH, (ticks | MTK_WDT_LENGTH_KEY));
#endif
...
void mtk_wdt_mode_config(BOOL dual_mode_en, BOOL irq, BOOL ext_en, BOOL ext_pol, BOOL wdt_en)
{
	...
	/* mdelay(100); */
	warnmsg("mtk_wdt_mode_config  mode value=%x, tmp:%x, pid=%d\n",
...
#else
		/* *(volatile u64 *)(MTK_WDT_RESTART) = MTK_WDT_RESTART_KEY; */
		DRV_WriteReg32(MTK_WDT_RESTART, MTK_WDT_RESTART_KEY);
#endif
	} else {
		warnmsg("WDT:[mtk_wdt_restart] type error: type = %d,pid = %d\n", type, current->pid);
	}
}
...
int mtk_wdt_request_en_set(int mark_bit, WD_REQ_CTL en)
{
	...
	if (NULL == toprgu_base)
		toprgu_base = (void *)get_base_by_name("mediatek,TOPRGU");
...
static void wdt_fiq(void *arg, void *regs, void *svc_sp)
{
	...
	aee_wdt_printf("kick=0x%08x,check=0x%08x,STA=%x,pmic_0x14A=%x\n",
...
#else
	ret = request_irq(wdt_irq_id, (irq_handler_t) mtk_wdt_isr, IRQF_TRIGGER_FALLING,
			  "mtk_watchdog", NULL);
#endif /* CONFIG_KICK_SPM_WDT */
#else  /* #ifndef CONFIG_FIQ_GLUE */
	printk("******** MTK WDT register fiq ********\n");
#ifdef CONFIG_KICK_SPM_WDT
	ret = spm_wdt_register_fiq(wdt_fiq);
#else
	ret = request_fiq(wdt_irq_id, wdt_fiq, IRQF_TRIGGER_FALLING, NULL);
#endif				/* CONFIG_KICK_SPM_WDT */
#endif				/* CONFIG_FIQ_GLUE */
...
#endif
/*#ifdef CONFIG_OF
       deglitch_base = (void*)get_base_by_name("mediatek,TOPCKGEN");
	mtk_wdt_deglitch_en();
#endif*/
	udelay(100);
...
MODULE_AUTHOR("MTK");
MODULE_DESCRIPTION("MT8173 Watchdog Device Driver");
MODULE_LICENSE("GPL");
