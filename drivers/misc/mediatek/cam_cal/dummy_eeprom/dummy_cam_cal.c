...
static struct i2c_board_info kd_cam_cal_dev __initdata = { I2C_BOARD_INFO("dummy_cam_cal", 0xAB >> 1) };	/* make dummy_eeprom co-exist */
...
static int iWriteData(unsigned int ui4_offset, unsigned int ui4_length, unsigned char *pinputdata)
{
	...
	do {
		if (i4ResidueDataLength >= 6) {
			...
			i4RetValue =
			    iWriteCAM_CAL((u16) u4CurrentOffset, i4ResidueDataLength, pBuff);
			if (i4RetValue != 0) {
				CAM_CALDB("[CAM_CAL] I2C iWriteData failed!!\n");
				return -1;
			}
...
static int iReadData(unsigned int ui4_offset, unsigned int ui4_length, unsigned char *pinputdata)
{
	...
	do {
		if (i4ResidueDataLength >= 8) {
			...
			i4RetValue =
			    iReadCAM_CAL((u16) u4CurrentOffset, i4ResidueDataLength, pBuff);
			if (i4RetValue != 0) {
				CAM_CALDB("[CAM_CAL] I2C iReadData failed!!\n");
				return -1;
			}
...
static unsigned short force[] =
    { CAM_CAL_I2C_GROUP_ID, S24CS64A_DEVICE_ID, I2C_CLIENT_END, I2C_CLIENT_END };
#else
