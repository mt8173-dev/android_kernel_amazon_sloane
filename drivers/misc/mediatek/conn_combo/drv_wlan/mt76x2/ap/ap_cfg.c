	...
	while ((this_char = strsep((char **)&pIoctlCmdStr->u.data.pointer, ",")) != NULL) {
		...
		for (PRTMP_PRIVATE_SET_PROC = RTMP_PRIVATE_SUPPORT_PROC;
			...
			if (!strcmp(this_char, PRTMP_PRIVATE_SET_PROC->name)) {
				if (!PRTMP_PRIVATE_SET_PROC->set_proc(pAd, value)) {	/*FALSE:Set private failed then return Invalid argument */
					Status = -EINVAL;
				}
				break;	/*Exit for loop. */
			}
	...
	while ((this_char = strsep((char **)&pIoctlCmdStr->u.data.pointer, ",")) != NULL) {
		...
		for (PRTMP_PRIVATE_SHOW_PROC = RTMP_PRIVATE_SHOW_SUPPORT_PROC;
			...
			if (!strcmp(this_char, PRTMP_PRIVATE_SHOW_PROC->name)) {
				if (!PRTMP_PRIVATE_SHOW_PROC->set_proc(pAd, value)) {	/*FALSE:Set private failed then return Invalid argument */
					Status = -EINVAL;
				}
				break;	/*Exit for loop. */
			}
	...
	case HOSTAPD_OID_SET_KEY:
		{
			...
			if ((Key.ik_type == CIPHER_WEP64) || (Key.ik_type == CIPHER_WEP128)) {	/*dynamic wep with 1x */
				if (pEntry) {	/*pairwise key */
					...
					RTMPAddWcidAttributeEntry(pAd, pEntry->apidx, KeyIdx,	/* The value may be not zero */
...
INT Set_AP_Key1_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (retVal == TRUE) {
		/* Set keys (into ASIC) */
		if (pAd->ApCfg.MBSSID[apidx].wdev.AuthMode >= Ndis802_11AuthModeWPA);	/* not support */
		else {		/* Old WEP stuff */
...
INT Show_MATTable_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	printk("Default BroadCast Address=%02x:%02x:%02x:%02x:%02x:%02x!\n", BROADCAST_ADDR[0],
	       BROADCAST_ADDR[1], BROADCAST_ADDR[2], BROADCAST_ADDR[3], BROADCAST_ADDR[4],
	       BROADCAST_ADDR[5]);
	return TRUE;
}
#endif /* MAT_SUPPORT */
