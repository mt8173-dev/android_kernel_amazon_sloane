...
INT set_ed_ap_scaned_count_proc(RTMP_ADAPTER *pAd, PSTRING arg)
{
	...
	ret = os_strtol(arg, 10, &ed_ap_scaned);
	if (ret < 0)
		return FALSE;

	DBGPRINT(RT_DEBUG_OFF, ("%s(): ed_ap_scaned=%ld\n", __func__, ed_ap_scaned));
...
#ifdef CONFIG_STA_SUPPORT
	DBGPRINT(RT_DEBUG_OFF,
		 ("Dump ChannelBusy Counts, ed_chk=%u, ed_ap_scaned=%u, ed_current_ch_aps=%u, ed_rssi_threshold=%d, ",
		  pAd->ed_chk, pAd->ed_ap_scaned, pAd->ed_current_ch_aps, pAd->ed_rssi_threshold));
	DBGPRINT(RT_DEBUG_OFF,
		 ("false_cca_threshold=%u, ChkPeriod=%dms, ED_Threshold=%d%%,HitCntForBlockTx=%d\n",
		  pAd->false_cca_threshold, pAd->ed_chk_period, pAd->ed_threshold, pAd->ed_block_tx_threshold));
#endif /* endif */
