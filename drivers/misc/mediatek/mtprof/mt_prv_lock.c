...
static void sem_down(void)
{
	...
	pr_info("down mtprof sem static...\n");
	down(&mtprof_sem_static);
	pr_info("down mtprof sem dyn..\n");
	down(mtprof_sem_dyn);
}

static void sem_up(void)
{
	pr_info("up mtprof sem dyn..\n");
	up(mtprof_sem_dyn);
	pr_info("up mtprof sem static...\n");
...
static ssize_t mt_pvlk_write(struct file *filp, const char *ubuf, size_t cnt, loff_t *data)
{
	...
	pr_info("[MT prove locking] debug_locks = %d\n", debug_locks);
	return cnt;
}
