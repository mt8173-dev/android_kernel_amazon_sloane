...
static struct device_node *io_cfg_left_node;
static struct device_node *io_cfg_top_node;
static struct device_node *io_cfg_bottom_node;
static struct device_node *io_cfg_right_node;
...
#define MTL_METRIC_TAG "kernel_sdcard"
#define METRICS_SDCARD_DATA_LEN  256
static char sdcard_metrics_buf[METRICS_SDCARD_DATA_LEN] = {0};
static struct work_struct sdcard_metrics_work;
#endif
...
#ifdef CONFIG_AMAZON_METRICS_LOG
static void sdcard_metrics(struct work_struct *work)
{
	/* Log suspend state failure or success */
	log_to_metrics(ANDROID_LOG_INFO, MTL_METRIC_TAG, sdcard_metrics_buf);
}

void sdcard_log_metrics(char *input_msg)
{
	struct timespec ts = current_kernel_time();

	snprintf(sdcard_metrics_buf, sizeof(sdcard_metrics_buf),
		"sdcard_status:def:%s=1;CT;1,timestamp=%lu;TI;1:NR",
		input_msg,
		ts.tv_sec * 1000 + ts.tv_nsec / NSEC_PER_MSEC);

	schedule_work(&sdcard_metrics_work);
}
#endif

/* init gpio reg base */
void msdc_gpio_reg_base_init(struct msdc_host *host)
{
	if (host->id >= HOST_MAX_NUM) {
		pr_err("%s -> host id is out of range (%d)\n", __func__, host->id);
		return;
	}
	switch (host->id) {
	case 0:
		host->gpio_base[GPIO_CLK_CTRL] = MSDC0_GPIO_CLK_BASE;
		host->gpio_base[GPIO_CMD_CTRL] = MSDC0_GPIO_CMD_BASE;
		host->gpio_base[GPIO_DAT_CTRL] = MSDC0_GPIO_DAT_BASE;
		host->gpio_base[GPIO_DAT1_CTRL] = 0;
		host->gpio_base[GPIO_RST_CTRL] = MSDC0_GPIO_RST_BASE;
		host->gpio_base[GPIO_DS_CTRL] = MSDC0_GPIO_DS_BASE;
		host->gpio_base[GPIO_PAD_CTRL] = MSDC0_GPIO_PAD_BASE;
		break;
	case 1:
		host->gpio_base[GPIO_CLK_CTRL] = MSDC1_GPIO_CLK_BASE;
		host->gpio_base[GPIO_CMD_CTRL] = MSDC1_GPIO_CMD_BASE;
		host->gpio_base[GPIO_DAT_CTRL] = MSDC1_GPIO_DAT_BASE;
		host->gpio_base[GPIO_DAT1_CTRL] = MSDC1_GPIO_DAT1_BASE;
		host->gpio_base[GPIO_RST_CTRL] = 0;
		host->gpio_base[GPIO_DS_CTRL] = 0;
		host->gpio_base[GPIO_PAD_CTRL] = MSDC1_GPIO_PAD_BASE;
		break;
	case 2:
		host->gpio_base[GPIO_CLK_CTRL] = MSDC2_GPIO_CLK_BASE;
		host->gpio_base[GPIO_CMD_CTRL] = MSDC2_GPIO_CMD_BASE;
		host->gpio_base[GPIO_DAT_CTRL] = MSDC2_GPIO_DAT_BASE;
		host->gpio_base[GPIO_DAT1_CTRL] = MSDC2_GPIO_DAT1_BASE;
		host->gpio_base[GPIO_RST_CTRL] = 0;
		host->gpio_base[GPIO_DS_CTRL] = 0;
		host->gpio_base[GPIO_PAD_CTRL] = MSDC2_GPIO_PAD_BASE;
		break;
	case 3:
		host->gpio_base[GPIO_CLK_CTRL] = MSDC3_GPIO_CLK_BASE;
		host->gpio_base[GPIO_CMD_CTRL] = MSDC3_GPIO_CMD_BASE;
		host->gpio_base[GPIO_DAT_CTRL] = MSDC3_GPIO_DAT_BASE;
		host->gpio_base[GPIO_DAT1_CTRL] = MSDC3_GPIO_DAT1_BASE;
		host->gpio_base[GPIO_RST_CTRL] = 0;
		host->gpio_base[GPIO_DS_CTRL] = MSDC3_GPIO_DS_BASE;
		host->gpio_base[GPIO_PAD_CTRL] = MSDC3_GPIO_PAD_BASE;
		break;
	default:
		break;
	}
...
struct msdc_host *mtk_msdc_host[HOST_MAX_NUM] = { NULL };
...
void msdc_dump_padctl(struct msdc_host *host, u32 pin)
{
	switch (pin) {
	case GPIO_CLK_CTRL:
		INIT_MSG("  CLK_PAD_CTRL[0x%p]        = 0x%.8x", host->gpio_base[GPIO_CLK_CTRL],
			 sdr_read32(host->gpio_base[GPIO_CLK_CTRL]));
		break;
	case GPIO_CMD_CTRL:
		INIT_MSG("  CMD_PAD_CTRL[0x%p]        = 0x%.8x", host->gpio_base[GPIO_CMD_CTRL],
			 sdr_read32(host->gpio_base[GPIO_CMD_CTRL]));
		break;
	case GPIO_DAT_CTRL:
		if (host->id == 0) {
			INIT_MSG("  DAT_PAD_CTRL[0x%p]        = 0x%.8x",
				 host->gpio_base[GPIO_DAT_CTRL],
				 sdr_read32(host->gpio_base[GPIO_DAT_CTRL]));
		} else {
			INIT_MSG("  DAT_PAD_CTRL[0x%p]        = 0x%.8x",
				 host->gpio_base[GPIO_DAT_CTRL],
				 sdr_read32(host->gpio_base[GPIO_DAT_CTRL]));
			INIT_MSG("  DAT1_PAD_CTRL[0x%p]       = 0x%.8x",
				 host->gpio_base[GPIO_DAT1_CTRL],
				 sdr_read32(host->gpio_base[GPIO_DAT1_CTRL]));
		}
		break;
	case GPIO_RST_CTRL:
		if (host->id == 0) {
			INIT_MSG("  RST_PAD_CTRL[0x%p]        = 0x%.8x",
				 host->gpio_base[GPIO_RST_CTRL],
				 sdr_read32(host->gpio_base[GPIO_RST_CTRL]));
		}
		break;
	case GPIO_DS_CTRL:
		if (host->id == 0) {
			INIT_MSG("  DS_PAD_CTRL[0x%p]         = 0x%.8x",
				 host->gpio_base[GPIO_DS_CTRL],
				 sdr_read32(host->gpio_base[GPIO_DS_CTRL]));
		}
		break;
	case GPIO_PAD_CTRL:
		INIT_MSG("  TDRD_PAD_CTRL[0x%p]           = 0x%.8x", host->gpio_base[GPIO_PAD_CTRL],
			 sdr_read32(host->gpio_base[GPIO_PAD_CTRL]));
		break;
	case GPIO_MODE_CTRL:
		if (host->id == 0) {
			INIT_MSG("  PAD_MODE_CTRL[0x%p]         = 0x%.8x", MSDC0_GPIO_MODE0_BASE,
				 sdr_read32(MSDC0_GPIO_MODE0_BASE));
			INIT_MSG("  PAD_MODE_CTRL[0x%p]         = 0x%.8x", MSDC0_GPIO_MODE1_BASE,
				 sdr_read32(MSDC0_GPIO_MODE1_BASE));
			INIT_MSG("  PAD_MODE_CTRL[0x%p]         = 0x%.8x", MSDC0_GPIO_MODE2_BASE,
				 sdr_read32(MSDC0_GPIO_MODE2_BASE));
		} else if (host->id == 1) {
			INIT_MSG("  PAD_MODE_CTRL[0x%p]         = 0x%.8x", MSDC1_GPIO_MODE0_BASE,
				 sdr_read32(MSDC1_GPIO_MODE0_BASE));
			INIT_MSG("  PAD_MODE_CTRL[0x%p]         = 0x%.8x", MSDC1_GPIO_MODE1_BASE,
				 sdr_read32(MSDC1_GPIO_MODE1_BASE));
		} else if (host->id == 2) {
			INIT_MSG("  PAD_MODE_CTRL[0x%p]         = 0x%.8x", MSDC2_GPIO_MODE0_BASE,
				 sdr_read32(MSDC2_GPIO_MODE0_BASE));
			INIT_MSG("  PAD_MODE_CTRL[0x%p]         = 0x%.8x", MSDC2_GPIO_MODE1_BASE,
				 sdr_read32(MSDC2_GPIO_MODE1_BASE));
		} else if (host->id == 3) {
			INIT_MSG("  PAD_MODE_CTRL[0x%p]         = 0x%.8x", MSDC3_GPIO_MODE0_BASE,
				 sdr_read32(MSDC3_GPIO_MODE0_BASE));
			INIT_MSG("  PAD_MODE_CTRL[0x%p]         = 0x%.8x", MSDC3_GPIO_MODE1_BASE,
				 sdr_read32(MSDC3_GPIO_MODE1_BASE));
		}
		break;
	default:
		break;
...
static void msdc_top_reset(struct msdc_host *host)
{
	...
	/* printk("[%s]: back up\n",__func__); */
	sdr_get_field((base + OFFSET_SDC_DCRC_STS), (SDC_DCRC_STS_NEG | SDC_DCRC_STS_POS),
		      emmc_sdc_data_crc_value);
	for (i = 0; i < TOP_RESET_BACKUP_REG_NUM; i++) {
		sdr_get_field(base + top_reset_backup_reg_list[i].offset,
		...
		if (top_reset_backup_reg_list[i].restore_func) {
			...
			if (err) {
				ERR_MSG
				    ("failed to backup reg[0x%x][0x%x], expected value[0x%x], actual value[0x%x] err=0x%x",
				     base + top_reset_backup_reg_list[i].offset,
				     top_reset_backup_reg_list[i].mask,
				     top_reset_backup_reg_list[i].value,
				     sdr_read32(base + top_reset_backup_reg_list[i].offset), err);
			}
		}
	}
	...
	/* printk("[%s]: restore\n",__func__); */
	for (i = 0; i < TOP_RESET_BACKUP_REG_NUM; i++) {
		sdr_set_field(base + top_reset_backup_reg_list[i].offset,
			      top_reset_backup_reg_list[i].mask,
			      top_reset_backup_reg_list[i].value);
		if (top_reset_backup_reg_list[i].restore_func) {
			...
			if (err) {
				ERR_MSG
				    ("failed to restore reg[0x%x][0x%x], expected value[0x%x], actual value[0x%x] err=0x%x",
				     base + top_reset_backup_reg_list[i].offset,
				     top_reset_backup_reg_list[i].mask,
				     top_reset_backup_reg_list[i].value,
				     sdr_read32(base + top_reset_backup_reg_list[i].offset), err);
...
{
	sdr_set_field(host->gpio_base[GPIO_CLK_CTRL], GPIO_MSDC_SR_MASK, clk);
	sdr_set_field(host->gpio_base[GPIO_CMD_CTRL], GPIO_MSDC_SR_MASK, cmd);
	sdr_set_field(host->gpio_base[GPIO_DAT_CTRL], GPIO_MSDC_SR_MASK, dat);
	if (host->id == 0) {
		sdr_set_field(host->gpio_base[GPIO_RST_CTRL], GPIO_MSDC_SR_MASK, rst);
		sdr_set_field(host->gpio_base[GPIO_DS_CTRL], GPIO_MSDC_SR_MASK, ds);
	}
}
...
void msdc_set_rdsel(struct msdc_host *host, bool sd_18)
{
	switch (host->id) {
	case 0:
		sdr_set_field(host->gpio_base[GPIO_PAD_CTRL], GPIO_PAD_RDSEL_MASK, 0x0);
		break;
	case 1:
		if (sd_18) {
			sdr_set_field(host->gpio_base[GPIO_PAD_CTRL], GPIO_PAD_RDSEL_MASK, 0x0);
		} else {
			sdr_set_field(host->gpio_base[GPIO_PAD_CTRL], GPIO_PAD_RDSEL_MASK, 0xc);
		}
		break;
	case 2:
		if (sd_18) {
...
void msdc_set_tdsel(struct msdc_host *host, bool sleep)
{
	switch (host->id) {
	case 0:
		sdr_set_field(host->gpio_base[GPIO_PAD_CTRL], GPIO_PAD_TDSEL_MASK, 0x0);
		break;
	case 1:
		if (sleep) {
			sdr_set_field(host->gpio_base[GPIO_PAD_CTRL], GPIO_PAD_TDSEL_MASK, 0xf);
		} else {
			sdr_set_field(host->gpio_base[GPIO_PAD_CTRL], GPIO_PAD_TDSEL_MASK, 0xa);
		}
		break;
	case 2:
		if (sleep) {
...
static void msdc_pin_pud(struct msdc_host *host, u32 pin, u32 mode, u32 val)
{
	switch (pin) {
	case GPIO_CLK_CTRL:
		sdr_set_field(host->gpio_base[GPIO_CLK_CTRL], GPIO_MSDC_R1R0_MASK, val);
		sdr_set_field(host->gpio_base[GPIO_CLK_CTRL], GPIO_MSDC_PUPD_MASK, mode);
		break;
	case GPIO_CMD_CTRL:
		sdr_set_field(host->gpio_base[GPIO_CMD_CTRL], GPIO_MSDC_R1R0_MASK, val);
		sdr_set_field(host->gpio_base[GPIO_CMD_CTRL], GPIO_MSDC_PUPD_MASK, mode);
		break;
	case GPIO_DAT_CTRL:
		if (host->id == 0) {
			sdr_set_field(host->gpio_base[GPIO_DAT_CTRL], GPIO_MSDC_R1R0_MASK, val);
			sdr_set_field(host->gpio_base[GPIO_DAT_CTRL], GPIO_MSDC_PUPD_MASK, mode);
		} else {
			sdr_set_field(host->gpio_base[GPIO_DAT1_CTRL], GPIO_MSDC_DAT0_R1R0_MASK,
				      val);
			sdr_set_field(host->gpio_base[GPIO_DAT1_CTRL], GPIO_MSDC_DAT0_PUPD_MASK,
				      mode);

			sdr_set_field(host->gpio_base[GPIO_DAT1_CTRL], GPIO_MSDC_DAT1_R1R0_MASK,
				      val);
			sdr_set_field(host->gpio_base[GPIO_DAT1_CTRL], GPIO_MSDC_DAT1_PUPD_MASK,
				      mode);

			sdr_set_field(host->gpio_base[GPIO_DAT1_CTRL], GPIO_MSDC_DAT2_R1R0_MASK,
				      val);
			sdr_set_field(host->gpio_base[GPIO_DAT1_CTRL], GPIO_MSDC_DAT2_PUPD_MASK,
				      mode);

			sdr_set_field(host->gpio_base[GPIO_DAT1_CTRL], GPIO_MSDC_DAT3_R1R0_MASK,
				      val);
			sdr_set_field(host->gpio_base[GPIO_DAT1_CTRL], GPIO_MSDC_DAT3_PUPD_MASK,
				      mode);
		}
		break;
	case GPIO_RST_CTRL:
		if (host->id == 0) {
			sdr_set_field(host->gpio_base[GPIO_RST_CTRL], GPIO_MSDC_R1R0_MASK, val);
			sdr_set_field(host->gpio_base[GPIO_RST_CTRL], GPIO_MSDC_PUPD_MASK, mode);
		}
		break;
	case GPIO_DS_CTRL:
		if (host->id == 0) {
			sdr_set_field(host->gpio_base[GPIO_DS_CTRL], GPIO_MSDC_R1R0_MASK, val);
			sdr_set_field(host->gpio_base[GPIO_DS_CTRL], GPIO_MSDC_PUPD_MASK, mode);
		}
		break;
	default:
		break;
...
void msdc_set_smt(struct msdc_host *host, u32 pin, u32 val)
{
	switch (pin) {
	case GPIO_CLK_CTRL:
		sdr_set_field(host->gpio_base[GPIO_CLK_CTRL], GPIO_MSDC_SMT_MASK, val);
		break;
	case GPIO_CMD_CTRL:
		sdr_set_field(host->gpio_base[GPIO_CMD_CTRL], GPIO_MSDC_SMT_MASK, val);
		break;
	case GPIO_DAT_CTRL:
		sdr_set_field(host->gpio_base[GPIO_DAT_CTRL], GPIO_MSDC_SMT_MASK, val);
		break;
	case GPIO_RST_CTRL:
		if (host->id == 0)
			sdr_set_field(host->gpio_base[GPIO_RST_CTRL], GPIO_MSDC_SMT_MASK, val);
		break;
	case GPIO_DS_CTRL:
		if (host->id == 0)
			sdr_set_field(host->gpio_base[GPIO_DS_CTRL], GPIO_MSDC_SMT_MASK, val);
		break;
	default:
		break;
	}
...
void msdc_set_driving(struct msdc_host *host, struct msdc_hw *hw, bool sd_18)
{
	if (sd_18 && ((host->id == 1) || (host->id == 2))) {
		sdr_set_field(host->gpio_base[GPIO_CLK_CTRL], GPIO_MSDC_DRV_MASK,
			      hw->clk_drv_sd_18);
		sdr_set_field(host->gpio_base[GPIO_CMD_CTRL], GPIO_MSDC_DRV_MASK,
			      hw->cmd_drv_sd_18);
		sdr_set_field(host->gpio_base[GPIO_DAT_CTRL], GPIO_MSDC_DRV_MASK,
			      hw->dat_drv_sd_18);
	} else {
		sdr_set_field(host->gpio_base[GPIO_CLK_CTRL], GPIO_MSDC_DRV_MASK, hw->clk_drv);
		sdr_set_field(host->gpio_base[GPIO_CMD_CTRL], GPIO_MSDC_DRV_MASK, hw->cmd_drv);
		sdr_set_field(host->gpio_base[GPIO_DAT_CTRL], GPIO_MSDC_DRV_MASK, hw->dat_drv);
		if (host->id == 0) {
			sdr_set_field(host->gpio_base[GPIO_RST_CTRL], GPIO_MSDC_DRV_MASK,
				      hw->rst_drv);
			sdr_set_field(host->gpio_base[GPIO_DS_CTRL], GPIO_MSDC_DRV_MASK,
				      hw->ds_drv);
		}
	}
}
...
{
	sdr_get_field(host->gpio_base[GPIO_CLK_CTRL], GPIO_MSDC_DRV_MASK, msdc_ctl->clk_pu_driving);
	sdr_get_field(host->gpio_base[GPIO_CMD_CTRL], GPIO_MSDC_DRV_MASK, msdc_ctl->cmd_pu_driving);
	sdr_get_field(host->gpio_base[GPIO_DAT_CTRL], GPIO_MSDC_DRV_MASK, msdc_ctl->dat_pu_driving);
#if defined(CONFIG_EMMC_50_FEATURE) && defined(CONFIG_MTK_EMMC_SUPPORT)
	if (host->id == 0) {
		sdr_get_field(host->gpio_base[GPIO_RST_CTRL], GPIO_MSDC_DRV_MASK,
			      msdc_ctl->rst_pu_driving);
		sdr_get_field(host->gpio_base[GPIO_DS_CTRL], GPIO_MSDC_DRV_MASK,
...
#ifdef MTK_MSDC_USE_REGULATOR
static void msdc_init_regulator(struct platform_device *pdev, struct msdc_host *host)
{
#define CHECK_INIT_REGULATOR(ID, NAME) \
	host->regulator[ID] = devm_regulator_get(&pdev->dev, NAME); \
	if (IS_ERR(host->regulator[ID])) { \
		pr_err("[%s] Cannot find regulator err(%ld).\n", \
				pdev->name, (unsigned long)host->regulator[ID]); \
		host->regulator[ID] = NULL; \
		break; \
	}

	switch (pdev->id) {
	case 0:
		CHECK_INIT_REGULATOR(0, "msdc0-vemc");
		if (IS_ERR_VALUE(regulator_enable(host->regulator[0])))
			pr_err("[MSDC%d-REGULATOR] enable regulator VEMC failed!\n", pdev->id);
		regulator_disable(host->regulator[0]);
		break;
	case 1:
		CHECK_INIT_REGULATOR(0, "msdc1-vmc");
		CHECK_INIT_REGULATOR(1, "msdc1-vmch");
		if (IS_ERR_VALUE(regulator_enable(host->regulator[0])))
			pr_err("[MSDC%d-REGULATOR] enable regulator VMC failed!\n", pdev->id);
		if (IS_ERR_VALUE(regulator_enable(host->regulator[1])))
			pr_err("[MSDC%d-REGULATOR] enable regulator VMCH failed!\n", pdev->id);
		regulator_disable(host->regulator[0]);
		regulator_disable(host->regulator[1]);
		break;
	case 2:
		CHECK_INIT_REGULATOR(0, "msdc2-vmc");
		CHECK_INIT_REGULATOR(1, "msdc2-vmch");
		if (IS_ERR_VALUE(regulator_enable(host->regulator[0])))
			pr_err("[MSDC%d-REGULATOR] enable regulator VMC failed!\n", pdev->id);
		if (IS_ERR_VALUE(regulator_enable(host->regulator[1])))
			pr_err("[MSDC%d-REGULATOR] enable regulator VMCH failed!\n", pdev->id);
	...
	case 3:
		...
		break;
	default:
		printk("[msdc_get_regulator] Invalid msdc device name => %s.\n", pdev->name);
		break;
	}
#undef CHECK_INIT_REGULATOR
}

static void msdc_regulator_config_on(struct regulator *regulator, int voltage, const char *name)
{
	int ret;

	if (regulator == NULL) {
		pr_err("[MSDC-REGULATOR] FATAL ERR: no regulator found!\n");
		return;
	}

	ret = regulator_get_voltage(regulator);
	if (ret != voltage) {
		if (IS_ERR_VALUE(ret = regulator_set_voltage(regulator, voltage, voltage))) {
			if (name)
				pr_err("[MSDC-REGULATOR] %s set voltage %d uV failed! err(%d)\n",
				       name, voltage, ret);
			else
				pr_err("[MSDC-REGULATOR] set voltage %d uV failed! err(%d)\n",
				       voltage, ret);
		}
	} else if (regulator_is_enabled(regulator)) {
		pr_err("[MSDC-REGULATOR] %s is already enabled. Do nothing.\n",
				name);
		return;
	}

	/* 0-disable, 1-enable, others-errno */
	if (IS_ERR_VALUE(ret = regulator_enable(regulator))) {
		if (name)
			pr_err("[MSDC-REGULATOR] enable regulator %s failed! err(%d)\n",
			       name, ret);
		else
			pr_err("[MSDC-REGULATOR] enable regulator failed! err(%d)\n", ret);
	}
}

static void msdc_regulator_config_off(struct regulator *regulator, const char *name)
{
	int ret;
	...
	if (!regulator_is_enabled(regulator)) {
		pr_err("[MSDC-REGULATOR] %s is already disabled. Do nothing.\n",
				name);
		return;
	}

	if (IS_ERR_VALUE(ret = regulator_disable(regulator))) {
		if (name)
			pr_err("[MSDC-REGULATOR] disable regulator %s failed! err(%d)\n",
			       name, ret);
		else
			pr_err("[MSDC-REGULATOR] disable regulator failed! err(%d)\n",
			       ret);
	}
}
...
static void msdc_emmc_power(struct msdc_host *host, u32 on)
{
	...
	pr_err("msdc%d power %s!\n", host->id, on ? "ON" : "OFF");

	if (on)
		msdc_regulator_config_on(host->regulator[0], 3000000, "VEMC");
	else
		msdc_regulator_config_off(host->regulator[0], "VEMC");
}

static void msdc_sd_power(struct msdc_host *host, u32 on)
{
	if (on) {
		msdc_regulator_config_on(host->regulator[0], 3300000, "VMC");
		msdc_regulator_config_on(host->regulator[1], 3300000, "VMCH");
		pr_err("[MSDC%d] power on - 3300\n", host->id);
	} else {
		msdc_regulator_config_off(host->regulator[0], "VMC");
		msdc_regulator_config_off(host->regulator[1], "VMCH");
		pr_err("[MSDC%d] power off\n", host->id);
	}
}
...
{
	if (on) {
		pr_err("[MSDC%d] power switch to 1800\n", host->id);
		msdc_regulator_config_on(host->regulator[0], 1800000, "VMC");
	} else {
		msdc_regulator_config_off(host->regulator[0], "VMC");
	}
}
...
{
	/* Nothing to do with 1.8V power(VCC18IO_0_MSDC3) which is always on */
}
#else				/* #ifdef MTK_MSDC_USE_REGULATOR */
static void msdc_emmc_power(struct msdc_host *host, u32 on)
{
	unsigned long tmo = 0;
	void __iomem *base = host->base;
	...
	pwrap_read(0x552, &ldo_en);
	pwrap_read(0x56a, &ldo_vol);
	printk("[%s]: msdc%d, VEMC_EN[0x552][bit10=1]=0x%d, VEMC_VOL[0x56a][bit6=1]=0x%x\n",
...
static void msdc_sd_power(struct msdc_host *host, u32 on)
{
	...
	case 1:
		...
		if (host->hw->flags & MSDC_SD_NEED_POWER) {
			msdc_ldo_power(1, MT65XX_POWER_LDO_VMC, VOL_3300, &g_msdc1_io);
	...
	case 2:
		msdc_set_driving(host, host->hw, 0);
		msdc_set_rdsel(host, 0);
		msdc_ldo_power(on, MT65XX_POWER_LDO_VMC, VOL_3300, &g_msdc2_io);
		msdc_ldo_power(on, MT65XX_POWER_LDO_VMCH, VOL_3300, &g_msdc2_flash);
		break;
	default:
		break;
	}
	...
	pwrap_read(0x566, &ldo_vol);
	pr_err(
	       "[%s]: msdc%d, VMC_EN[0x54e][bit10=1]=0x%d, VMC_VOL[0x566][bit4~6=1]=0x%x\n",
	       __func__, host->id, ldo_en, ldo_vol);
	pwrap_read(0x550, &ldo_en);
	pwrap_read(0x568, &ldo_vol);
	pr_err(
	       "[%s]: msdc:%d, VMCH_EN[0x550][bit10=1]=0x%d, VMCH_VOL[0x568][bit6=1]=0x%x\n",
	       __func__, host->id, ldo_en, ldo_vol);
...
static void msdc_sd_power_switch(struct msdc_host *host, u32 on)
{
	...
	case 1:
		...
		break;
	case 2:
		msdc_ldo_power(on, MT65XX_POWER_LDO_VMC, VOL_1800, &g_msdc2_io);
		msdc_set_rdsel(host, 1);
		msdc_set_driving(host, host->hw, 1);
		break;
	default:
...
#endif				/* #if !defined(CONFIG_REGULATOR) */
...
static void msdc_set_bad_card_and_remove(struct msdc_host *host)
{
	...
	if (host->mmc->card && host->card_inserted) {
		...
		if (!(host->hw->flags & MSDC_CD_PIN_EN)) {
			...
			/* reset irq level to trigger irq */
			if (IRQ_TYPE_LEVEL_LOW == host->sd_cd_irq_level)
				host->sd_cd_irq_level = IRQ_TYPE_LEVEL_HIGH;
			else
				host->sd_cd_irq_level = IRQ_TYPE_LEVEL_LOW;

			irq_set_irq_type(host->irq, host->sd_cd_irq_level);
		}
	#if 0
...
static void msdc_set_timeout(struct msdc_host *host, u32 ns, u32 clks)
{
	...
	if (host->sclk == 0) {
		...
		/* 1. change the max clk freq from 12000000 to 100000000
		   2. double the ddr timeout value
		   3. record the float data to a bigger integer*/
...
static void msdc_tasklet_card(unsigned long arg)
{
	...
	if (hw->flags & MSDC_UHS1) {
		host->mmc->caps |= MMC_CAP_UHS_SDR12 | MMC_CAP_UHS_SDR25 | MMC_CAP_UHS_SDR50 | MMC_CAP_UHS_SDR104;	/* | MMC_CAP_SET_XPC_180; */
	...
	if ((hw->flags & MSDC_CD_PIN_EN) && host->card_inserted) {
...
static int ot_thread_func(void *data)
{
	...
	if (chg_volt == 0) {
		...
		if ((cur_temperature != -127000) && (host->pre_temper != -127000)
			...
			err = msdc_online_tuning(host, 1, 0x00B0)
			    if (err != 0) {
...
static void msdc_init_clk(struct platform_device *pdev, struct msdc_host *host)
{
	struct clk *clk;
	int ret;
...
#endif
	host->clk = devm_clk_get(&pdev->dev, "source");
	BUG_ON(IS_ERR(host->clk));
	...
	if (host->id == 0) {
#ifdef CONFIG_EMMC_50_FEATURE
		/* Set eMMC 5.0 clock source */
		struct clk *clk_mux;
		struct clk *parent;
		struct clk *msdcpll2;

		clk_mux = clk_get_parent(host->clk); /* MSDC50_0_SEL */
		BUG_ON(IS_ERR(clk_mux));

		parent = devm_clk_get(&pdev->dev, "mux_parent");
		BUG_ON(IS_ERR(parent));

		ret = clk_set_parent(clk_mux, parent);
		if (ret < 0) {
			pr_err("Failed to set MSDC50_0_SEL's parent!\n");
			pr_err("Please check your dts setting!\n");
			BUG();
		}

		host->hclk = clk_get_rate(clk_mux);

		msdcpll2 = devm_clk_get(&pdev->dev, "msdcpll2");
		BUG_ON(IS_ERR(msdcpll2));
		clk_prepare_enable(msdcpll2);
		clk_disable_unprepare(msdcpll2);
#endif
	}
}

static void msdc_exit_clk(struct msdc_host *host)
{
...
static u32 msdc_power_tuning(struct msdc_host *host)
{
	...
	if (mmc_card_sd(card) && (host->hw->host_function == MSDC_SD)) {
		if ((host->power_cycle < MSDC_MAX_POWER_CYCLE) && (host->power_cycle_enable)) {
			...
			if (((host->sclk <= 100000000)
			     && ((host->sclk > 50000000) || (host->state == MSDC_STATE_DDR)))
			    && (host->power_cycle >= 1)) {
				mmc->caps &=
			...
			ERR_MSG
			    ("the %d time, exceed the max power cycle time %d, go to remove the bad card, power_cycle_enable=%d",
...
static int msdc_cache_ctrl(struct msdc_host *host, unsigned int enable, u32 *status)
{
	...
	cmd.arg =
	    (MMC_SWITCH_MODE_WRITE_BYTE << 24) | (EXT_CSD_CACHE_CTRL << 16) | (!!enable << 8) |
...
int msdc_get_reserve(void)
{
	...
	printk("total reserve: l_otp_reserve = 0x%x blocks, \
		l_mtk_reserve = 0x%x blocks, l_offset = 0x%x blocks\n", l_otp_reserve, l_mtk_reserve, l_offset);
#endif
				       ...
				       unsigned long timeout)
{
	...
	if (opcode == MMC_READ_MULTIPLE_BLOCK) {
		...
		else if ((host->autocmd & MSDC_AUTOCMD23) && mrq->sbc)
			rawcmd |= (1 << 29);
		...
		if ((host->autocmd & MSDC_AUTOCMD12) && mrq->stop)
			...
			rawcmd |= (1 << 29);
#ifdef MTK_MSDC_USE_CACHE
			if (sbc && host->mmc->card && mmc_card_mmc(host->mmc->card)
				...
				if (sdr_read32(SDC_BLK_NUM) != (sbc->arg & 0xFFFF))	/* if the block number is bigger than 0xFFFF, then CMD23 arg will be failed to set it. */
					printk
					    ("msdc%d: acmd23 arg(0x%x) failed to match the block num(0x%x), SDC_BLK_NUM(0x%x)\n",
		...
		rawcmd |= (1 << 14);
		rawcmd &= ~(0x0FFF << 16);
	} else if (opcode == MMC_GEN_CMD) {
		pr_info("MMC_GEN_CMD (56) arg->arg = 0x%x\n", cmd->arg);
		rawcmd |= (1 << 11);    /* signal block transaction */
		if ((cmd->arg & 0x01) == 0)
			rawcmd |= (1 << 13);
	}
					      ...
					      int tune, unsigned long timeout)
{
	...
	if (intsts & cmdsts) {
		...
		if ((intsts & MSDC_INT_CMDRDY) || (intsts & MSDC_INT_ACMDRDY) ||
			...
			IRQ_MSG("[%s]: msdc%d XXX CMD<%d> MSDC_INT_RSPCRCERR Arg<0x%.8x>", __func__,
				host->id, cmd->opcode, cmd->arg);
			/* msdc_dump_info(host->id); */
			msdc_reset_hw(host->id);
		} else if (intsts & MSDC_INT_CMDTMO) {
			cmd->error = (unsigned int)-ETIMEDOUT;
			IRQ_MSG("[%s]: msdc%d XXX CMD<%d> MSDC_INT_CMDTMO Arg<0x%.8x>", __func__,
				host->id, cmd->opcode, cmd->arg);
	...
	while (1) {
		...
		if (ptr == NULL || (PageHighMem((struct page *)(sg->page_link & ~0x3)))) {
			...
			if (subpage != 0 || (sg->offset != 0))
				N_MSG(OPS,
				      "msdc0: This read size or start not align %x,%x, hmpage %p",
				      subpage, left, hmpage);
			...
			if (flag == 1) {
				/* High memory and more than 1 va address va and  not continous */
				for (i = 0; i < totalpages; i++) {
					left = PAGE_SIZE;
					ptr = (u32 *) kaddr[i];
...
int msdc_pio_write(struct msdc_host *host, struct mmc_data *data)
{
	...
	while (1) {
		...
		if (ptr == NULL || (PageHighMem((struct page *)(sg->page_link & ~0x3)))) {
			...
			if ((subpage != 0) || (sg->offset != 0))
				N_MSG(OPS,
				      "msdc0: This write size or start not align %d,%d, hmpage %p,sg offset %x\n",
				      subpage, left, hmpage, sg->offset);
...
static int msdc_dma_config(struct msdc_host *host, struct msdc_dma *dma)
{
	...
	case MSDC_MODE_DMA_BASIC:
		...
		N_MSG(DMA, "DMA BASIC mode dma_len<%x> dma_address<%llx>", dma_len,
		      (u64) dma_address);
	...
	case MSDC_MODE_DMA_DESC:
		...
		for (j = 0; j < bdlen; j++) {
			...
			N_MSG(DMA, "DMA DESC mode dma_len<%x> dma_address<%llx>", dma_len,
			      (u64) dma_address);
...
#ifdef MSDC_DMA_VIOLATION_DEBUG
		{
			...
			for (i = 0; i < 1; i++) {
				...
				N_MSG(DMA, "[SD%d] GD[%d](0x%llxh): %xh %xh %xh %xh %xh %xh %xh\n",
				      host->id, i, (u64) ptr, *ptr, *(ptr + 1), *(ptr + 2),
...
static unsigned int msdc_online_tuning(struct msdc_host *host, unsigned fn, unsigned addr)
{
 ...
 _retry:
	...
	if (otData.cmddlypass == 0) {
		...
		if (otData.datrddlypass == 0) {
			...
			ret =
			    ot_adjust_tunewnd(host, &otData.fDatTestedGear, &tune_wind_size_dat,
					      &CRCPass);
		...
		if (otData.dat0rddlypass == 0) {
			...
			ret =
			    ot_adjust_tunewnd(host, &otData.fDat0TestedGear, &tune_wind_size_dat0,
					      &CRCPass);
			...
			ret =
			    ot_adjust_tunewnd(host, &otData.fDat1TestedGear, &tune_wind_size_dat1,
					      &CRCPass);
			...
			ret =
			    ot_adjust_tunewnd(host, &otData.fDat2TestedGear, &tune_wind_size_dat2,
					      &CRCPass);
			...
			ret =
			    ot_adjust_tunewnd(host, &otData.fDat3TestedGear, &tune_wind_size_dat3,
					      &CRCPass);
 ...
 out:
	...
	if (ret < 0) {
		...
		if (otData.eco_ver >= 4) {
			rxdly0 =
			    (otData.orig_dat0rddly << 24) | (otData.
							     orig_dat1rddly << 16) |
			    (otData.orig_dat2rddly << 8) | (otData.orig_dat3rddly << 0);
		} else {
			rxdly0 =
			    (otData.orig_dat0rddly << 0) | (otData.
							    orig_dat1rddly << 8) |
			    (otData.orig_dat2rddly << 16) | (otData.orig_dat3rddly << 24);
		}
		sdr_write32(MSDC_DAT_RDDLY0, rxdly0);

		printk("[SDIO_OnlineTuning]ret = %d, vcore_uv = 0x%x, cmd delay = 0x%x\n", ret,
		       vcore_uv, otData.orig_cmdrdly);
	} else if (otData.orig_ddlsel == 0) {
...
#ifdef MTK_MSDC_USE_CACHE
	if ((host->hw->host_function == MSDC_EMMC) && host->mmc->card
	    && (host->mmc->card->ext_csd.cache_ctrl & 0x1) && (cmd->opcode == MMC_WRITE_BLOCK
							       || cmd->opcode ==
...
static int msdc_lower_freq(struct msdc_host *host)
{
	...
	if (div >= MSDC_MAX_FREQ_DIV) {
		...
		/*when HS400 low freq, you cannot change to mode 2 (DDR mode), \
		   else read data will be latched by clk, but not ds pin when card speed mode is still HS400. */
		if (hs400_src == 1) {
			return 0;
#if 0
		...
		host->sclk =
		    (div ==
		     0) ? hclks[host->hw->clk_src] / 2 : hclks[host->hw->clk_src] / (4 * div);
		...
		host->sclk =
		    (mode ==
		     2) ? hclks[host->hw->clk_src] / (2 * 4 * (div +
							       1)) : hclks[host->hw->clk_src] / (4 *
												 (div
												  +
												  1));
		ERR_MSG("new div<%d>, mode<%d> new freq.<%dKHz>", div + 1, mode, host->sclk / 1000);
...
#ifdef CONFIG_EMMC_50_FEATURE
#define MAX_HS400_TUNE_COUNT (1024)	/* (32*32) */
...
static void emmc_hs400_backup(void)
{
	...
	for (i = 0; i < HS400_BACKUP_REG_NUM; i++) {
		sdr_get_field(base + hs400_backup_reg_list[i].offset, hs400_backup_reg_list[i].mask,
		...
		if (hs400_backup_reg_list[i].restore_func) {
			...
			if (err) {
				pr_err(
				       ...
				       __func__, base + hs400_backup_reg_list[i].offset,
				       hs400_backup_reg_list[i].mask,
				       hs400_backup_reg_list[i].default_value,
				       sdr_read32(base + hs400_backup_reg_list[i].offset), err);
			}
		}
	}
...
static void emmc_hs400_restore(void)
{
	...
	for (i = 0; i < HS400_BACKUP_REG_NUM; i++) {
		sdr_set_field(base + hs400_backup_reg_list[i].offset, hs400_backup_reg_list[i].mask,
			      hs400_backup_reg_list[i].value);
		if (hs400_backup_reg_list[i].restore_func) {
		...
		printk("[%s]:i:%d, reg=0x%p, value=0x%x\n", __func__, i,
		       base + hs400_backup_reg_list[i].offset,
		       sdr_read32(base + hs400_backup_reg_list[i].offset));
	}
...
static int msdc_tune_read(struct msdc_host *host)
{
	...
	if ((sel == 1 && clkmode == 1
	     && host->t_counter.time_read == READ_TUNE_UHS_CLKMOD1_MAX_TIME) || (sel == 1
										 && (clkmode == 0
										     || clkmode ==
										     2)
										 && host->
				  ...
				  struct mmc_command *stop, struct mmc_command *sbc)
{
	...
	if (sbc) {
		ERR_MSG("XXX SBC<%d><0x%x> Error<%d> Resp<0x%x>", sbc->opcode, sbc->arg, sbc->error,
			sbc->resp[0]);
	}
#ifdef MTK_SDIO30_ONLINE_TUNING_SUPPORT
...
static void msdc_ops_request_legacy(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	msdc_reset_crc_tune_counter(host, all_counter);
	if (host->mrq) {
		ERR_MSG("XXX host->mrq<0x%.8x> cmd<%d>arg<0x%x>", (int)host->mrq,
	...
	while (msdc_do_request(mmc, mrq)) {	/* there is some error */
		...
		if ((cmd->error == (unsigned int)-EIO)
		    || (stop && (stop->error == (unsigned int)-EIO)) || (sbc
									 && (sbc->error ==
									     (unsigned int)-EIO))) {
#else
		if ((cmd->error == (unsigned int)-EIO)
...
static void msdc_tune_async_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	if (host->mrq) {
		...
		ERR_MSG("XXX current request <0x%.8x> cmd<%d>arg<0x%x>", (int)mrq, mrq->cmd->opcode,
			mrq->cmd->arg);
		if (mrq->data)
...
static void msdc_apply_ett_settings(struct msdc_host *host, int mode)
{
	...
	for (i = 0; i < host->hw->ett_count; i++) {
		ett =
		    (struct msdc_ett_settings *)(host->hw->ett_settings +
						 sizeof(struct msdc_ett_settings) * i);
...
static void msdc_ops_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if ((msdc_host_mode[host->id] != mmc->caps) || (msdc_host_mode2[host->id] != mmc->caps2)) {
		...
		if (1 == g_emmc_mode_switch) {
...
#endif
			{
				...
				if (mmc->card && mmc->card->ext_csd.hs_max_dtr > 52000000) {
					...
					if ((mmc->card->
					     ext_csd.raw_card_type & EXT_CSD_CARD_TYPE_MASK) &
			...
			pr_err("[%s]: msdc%d, ext_csd.card_type=0x%x\n", __func__,
			       host->id, mmc->card->ext_csd.card_type);
		}
#endif
		...
		sdr_set_field(MSDC_IOCON, MSDC_IOCON_DDLSEL, 1);
		cur_rxdly0 =
		    ((host->hw->dat0rddly & 0x1F) << 24) | ((host->
							     hw->dat1rddly & 0x1F) << 16) | ((host->
											      hw->dat2rddly
											      &
											      0x1F)
											     << 8) |
		...
		cur_rxdly1 =
		    ((host->hw->dat4rddly & 0x1F) << 24) | ((host->
							     hw->dat5rddly & 0x1F) << 16) | ((host->
											      hw->dat6rddly
											      &
											      0x1F)
											     << 8) |
	...
	if (host->mclk != ios->clock || host->state != state) {	/* not change when clock Freq. not changed state need set clock */
		if (ios->clock >= 25000000) {
			...
			if ((host->hw->host_function == MSDC_EMMC)
			    || (host->hw->host_function == MSDC_SD)) {
				sdr_set_field(MSDC_PATCH_BIT1, MSDC_PB1_BUSY_MARGIN,
					      host->saved_para.write_busy_margin);
...
static int msdc_ops_get_cd(struct mmc_host *mmc)
{
	...
	if ((!(host->hw->flags & MSDC_REMOVABLE) || (!(host->hw->flags & MSDC_CD_PIN_EN)))) {
		host->card_inserted = 1;
		goto end;
	}
...
#ifndef FPGA_PLATFORM
static irqreturn_t msdc1_eint_handler(int irq, void *dev)
{
	struct msdc_host *host = (struct msdc_host *)dev;

	host->card_inserted = (IRQ_TYPE_LEVEL_LOW == host->sd_cd_irq_level);
	pr_err("msdc1_eint_handler -> SD card %s\n",
			host->card_inserted ? "insert" : "remove");
	if (IRQ_TYPE_LEVEL_LOW == host->sd_cd_irq_level)
		host->sd_cd_irq_level = IRQ_TYPE_LEVEL_HIGH;
	else
		host->sd_cd_irq_level = IRQ_TYPE_LEVEL_LOW;

	irq_set_irq_type(irq, host->sd_cd_irq_level);

	tasklet_hi_schedule(&host->card_tasklet);

#ifdef CONFIG_AMAZON_METRICS_LOG
	if (host->card_inserted)
		sdcard_log_metrics("inserted");
	else
		sdcard_log_metrics("removed");
#endif
...
static irqreturn_t msdc_irq(int irq, void *dev_id)
{
	...
	if (intsts & MSDC_INT_MMCIRQ) {
		/* pr_info("msdc[%d] MMCIRQ: SDC_CSTS=0x%.8x\r\n", host->id, sdr_read32(SDC_CSTS)); */
	}
...
static void msdc_enable_cd_irq(struct msdc_host *host, int enable)
{
	...
	if (eint_node) {
		unsigned int gpio_pullen[2] = {0};
		unsigned int gpio_pullsel[2] = {0};
		void __iomem *reg_gpio_pullen = NULL, *reg_gpio_pullsel = NULL;

		pr_err("[MSDC%d] find MSDC1_INS-eint node!!\n", host->id);

		/* get gpio pull enable & sel */
		of_property_read_u32_array(eint_node, "gpio-pullen", gpio_pullen, ARRAY_SIZE(gpio_pullen));
		reg_gpio_pullen = ioremap(gpio_pullen[0], 4);	/* here remap gpio register to get virtual address; */
		if (NULL == reg_gpio_pullen) {
			pr_err("[MSDC%d] msdc1 cd gpio reg mapping fail (PULLEN 0x%x)!!\n",
				host->id, gpio_pullen[0]);
			return;
		}

		of_property_read_u32_array(eint_node, "gpio-pullsel", gpio_pullsel, ARRAY_SIZE(gpio_pullsel));
		reg_gpio_pullsel = ioremap(gpio_pullsel[0], 4);	/* here remap gpio register to get virtual address; */
		if (NULL == reg_gpio_pullsel) {
			pr_err("[MSDC%d] msdc1 cd gpio reg mapping fail (PULLSEL 0x%x)!!\n",
				host->id, gpio_pullsel[0]);
			return;
		}

		/* get irq */
		host->irq = irq_of_parse_and_map(eint_node, 0);
		if (!host->irq) {
			pr_err("[MSDC%d] can't irq_of_parse_and_map for card detect eint!!\n", host->id);
			return;
		} else {
			int interrupts[2];
			pr_err("[MSDC%d] msdc1 EINT get irq # %d\n", host->id, host->irq);
			of_property_read_u32_array(eint_node, "interrupts", interrupts, ARRAY_SIZE(interrupts));
			host->sd_cd_irq_level = interrupts[1];
		}

		if (enable) {
			if ((host->id == 1) && (MSDC_SD == host->hw->host_function)
			    && (host->hw->flags & MSDC_CD_PIN_EN)) {
				unsigned int intr_debounce[2] = {0};

				/* gpio pull en&sel */
				sdr_set_field(reg_gpio_pullen, gpio_pullen[1], 1);
				sdr_set_field(reg_gpio_pullsel, gpio_pullsel[1], 1);

				/* get gpio pin & debounce time */
				of_property_read_u32_array(eint_node, "debounce", intr_debounce,
											ARRAY_SIZE(intr_debounce));

				/* set debounce */
				gpiopin = intr_debounce[0];
				debounce = intr_debounce[1];
				gpio_set_debounce(gpiopin, debounce);

				if (request_irq(host->irq, msdc1_eint_handler, host->sd_cd_irq_level, DRV_NAME, host)) {
					pr_err("[MSDC%d] MSDC1 IRQ LINE NOT AVAILABLE!!\n", host->id);
				}

				/* Wakeup mode lets this IRQ wake the system from sleep state */
				enable_irq_wake(host->irq);

				pr_err("[MSDC%d] SD card detection eint resigter, sd_cd_irq_level=%d\n",
						host->id, host->sd_cd_irq_level);
			}

			if ((host->id == 2) && (MSDC_SD == host->hw->host_function)
			    && (host->hw->flags & MSDC_CD_PIN_EN)) {
				/* sdio need handle eint11 */
			}
		} else {
			/* gpio pull en&sel */
			sdr_set_field(reg_gpio_pullen, gpio_pullen[1], 0);
			sdr_set_field(reg_gpio_pullsel, gpio_pullsel[1], 0);
			disable_irq(host->irq);
		}
	} else {
		pr_err("[MSDC%d] can't find MSDC1_INS-eint compatible node\n", host->id);
		return;
	}
	...
	if (enable) {
		 ...
		 * should not be turned off. Here adds a reference count to keep
...
static void msdc_init_hw(struct msdc_host *host)
{
	...
	{
		unsigned int msdc_cfg;
		msdc_cfg = sdr_read32(SDC_CFG);
		pr_err("%s -> msdc_cfg=0x%x\n", __func__, msdc_cfg);
	}
...
static int msdc_drv_probe(struct platform_device *pdev)
{
	...
	if (gpio_node == NULL) {
		...
		if (gpio_node)
			msdc_gpio_base = of_iomap(gpio_node, 0);
	...
	status = of_find_property(pdev->dev.of_node, "status", NULL);
	pr_info("[%s]: %s status=%s\n", __func__, pdev->name, (char *)status->value);
	if (strncmp(status->value, "okay", status->length) == 0) {
...
#ifdef MTK_MSDC_USE_REGULATOR
	msdc_init_regulator(pdev, host);
#endif

	/* Initialize msdc related gpio register base table */
	msdc_gpio_reg_base_init(host);

	/* Initialise msdc clock */
	msdc_init_clk(pdev, host);
	...
	printk("[%s]: msdc%d, mmc->caps=0x%x, mmc->caps2=0x%x\n", __func__, host->id, mmc->caps,
	       mmc->caps2);
	pr_err(
	       "[%s]: msdc%d, msdcpll en[0xF000C250][bit0 should be 0x1]=0x%x, msdcpll on[0xF000C25C][bit0~1 should be 0x1]=0x%x, mux[0xF0000070][bit23, 16~19 for msdc0, bit31, 16~19 for msdc1]=0x%x\n",
	       __func__, host->id, sdr_read32(apmixed_base1 + 0x250),
	       sdr_read32(apmixed_base1 + 0x25C), sdr_read32(topckgen_base + 0x070));
#endif

/*
 * Please mark this code section defaultly,
 * cuz it probably broke the valid data,
 * then system crash...
 */
#if 0				/* def FPGA_PLATFORM */
#ifdef CONFIG_MTK_EMMC_SUPPORT
	printk("[%s]: waiting emmc init complete\n", __func__);
	host->mmc->card_init_wait(host->mmc);
	if (host->mmc->card && (mmc_card_mmc(host->mmc->card) || mmc_card_sd(host->mmc->card))) {
		printk("[%s]: start msdc%d read write compare test\n", __func__, host->id);
...
/* collect the SDCARD init info */
#ifdef CONFIG_AMAZON_METRICS_LOG
	sdcard_log_metrics("init");
#endif
	return 0;
...
#ifdef CONFIG_AMAZON_METRICS_LOG
	sdcard_log_metrics("init_failed");
#endif
...
static int __init mt_msdc_init(void)
{
	...
	if (!proc_emmc) {
		printk("proc_emmc Can't proc_create\n");
		return ret;
	}
#endif				/* CONFIG_MTK_EMMC_SUPPORT && CONFIG_PROC_FS */

	pr_err(DRV_NAME ": MediaTek MSDC Driver Init\n");
#ifdef ONLINE_TUNING_DVTTEST
	...
	INIT_WORK(&sdcard_metrics_work, sdcard_metrics);
...
{
	pr_err(DRV_NAME ": MediaTek MSDC Driver Exit\n");

#if defined(CONFIG_AMAZON_METRICS_LOG)
	cancel_work_sync(&sdcard_metrics_work);
#endif /* CONFIG_AMAZON_METRICS_LOG	*/
