...
char test_kdump[] = { 8, 1, 7, 3, 'k', 'd', 'u', 'm', 'p', 't', 'e', 's', 't' };
...
static struct msdc_hw *p_msdc_hw[HOST_MAX_NUM] = { NULL };
...
static void simp_msdc_dump_register(struct simp_msdc_host *host)
{
	...
	pr_emerg("R[00]=0x%x  R[04]=0x%x  R[08]=0x%x  R[0C]=0x%x  R[10]=0x%x  R[14]=0x%x\n",
	...
	pr_emerg("R[30]=0x%x  R[34]=0x%x  R[38]=0x%x  R[3C]=0x%x  R[40]=0x%x  R[44]=0x%x\n",
	...
	pr_emerg("R[48]=0x%x  R[4C]=0x%x  R[50]=0x%x  R[58]=0x%x  R[5C]=0x%x  R[60]=0x%x\n",
	...
	pr_emerg("R[70]=0x%x  R[74]=0x%x  R[78]=0x%x  R[7C]=0x%x  R[80]=0x%x  R[84]=0x%x\n",
	...
	pr_emerg("R[88]=0x%x  R[90]=0x%x  R[94]=0x%x  R[98]=0x%x  R[9C]=0x%x  R[A0]=0x%x\n",
	...
	pr_emerg("R[A4]=0x%x  R[A8]=0x%x  R[B0]=0x%x  R[B4]=0x%x  R[E0]=0x%x  R[E4]=0x%x\n",
	...
	pr_emerg("R[E8]=0x%x  R[EC]=0x%x  R[F0]=0x%x  R[F4]=0x%x  R[F8]=0x%x  R[100]=0x%x  R[104]=0x%x\n",
	...
	if (host->id == 0) {
		pr_emerg("R[%x]=0x%x  R[%x]=0x%x  R[%x]=0x%x  R[%x]=0x%x  R[%x]=0x%x  R[%x]=0x%x\n",
		...
		pr_emerg("R[%x]=0x%x  R[%x]=0x%x  R[%x]=0x%x  R[%x]=0x%x  R[%x]=0x%x  R[%p]=0x%x\n",
		...
		pr_emerg("R[%p]=0x%x  R[%p]=0x%x  R[%p]=0x%x  R[%p]=0x%x  R[%p]=0x%x  R[%p]=0x%x\n",
		...
		pr_emerg("R[%p]=0x%x  R[%p]=0x%x  R[%p]=0x%x  R[%p]=0x%x  R[%p]=0x%x  R[%p]=0x%x\n",
		       MSDC1_GPIO_CLK_BASE, sdr_read32(MSDC1_GPIO_CLK_BASE), MSDC1_GPIO_CMD_BASE,
		       sdr_read32(MSDC1_GPIO_CMD_BASE), MSDC1_GPIO_DAT_BASE,
		       sdr_read32(MSDC1_GPIO_DAT_BASE), MSDC1_GPIO_DAT1_BASE,
		       sdr_read32(MSDC1_GPIO_DAT1_BASE), MSDC1_GPIO_MODE0_BASE,
		       sdr_read32(MSDC1_GPIO_MODE0_BASE), MSDC1_GPIO_MODE1_BASE,
	...
	while (i <= 25) {
		...
		*(u32 *) (base + 0xa0) = i + 3;
		...
		pr_emerg("R[a0]=0x%x  R[a4]=0x%x  R[a0]=0x%x  R[a4]=0x%x  R[a0]=0x%x  R[a4]=0x%x  R[a0]=0x%x  R[a4]=0x%x\n",
...
static void simp_mmc_regulator_on(struct regulator *regulator, int voltage, const char *name)
{
	int ret;
	...
	if (ret != voltage) {
		...
		pr_info("[MSDC-REGULATOR] %s is already enabled. Do nothing.\n",
				name);
		return;
	}
...
static void simp_mmc_regulator_off(struct regulator *regulator, const char *name)
{
	int ret;
...
static unsigned int simp_mmc_power_up(struct simp_mmc_host *host, bool on)
{
	...
	case 0:
		if (mtk_msdc_host[0]->regulator[0]) {
			if (on)
				simp_mmc_regulator_on(mtk_msdc_host[0]->regulator[0], 3000000, "VEMC");
			else
				simp_mmc_regulator_off(mtk_msdc_host[0]->regulator[0], "VEMC");
		}
		break;

	case 1:
		if (mtk_msdc_host[1]->regulator[0] && mtk_msdc_host[1]->regulator[1]) {
			if (on) {
				simp_mmc_regulator_on(mtk_msdc_host[1]->regulator[0], 3300000, "VMC");
				simp_mmc_regulator_on(mtk_msdc_host[1]->regulator[1], 3300000, "VMCH");
				pr_info("[MSDC%d] power on - 3300\n", host->mtk_host->id);
			} else {
				simp_mmc_regulator_off(mtk_msdc_host[1]->regulator[0], "VMC");
				simp_mmc_regulator_off(mtk_msdc_host[1]->regulator[1], "VMCH");
				pr_info("[MSDC%d] power off\n", host->mtk_host->id);
			}
		}
		break;
...
{
	clk_prepare_enable(host->mtk_host->clk);
...
static unsigned int simp_msdc_init(struct simp_mmc_host *mmc_host)
{
	...
	case 0:
		...
		/*pr_emerg("host0 clk= %d, cmd= %d, dat= %d, rst=%d, ds=%d.\r\n",\
	...
	case 1:
		...
		/*pr_emerg("host1 clk= %d, cmd= %d, dat= %d.\r\n",\
...
#if MTK_MMC_DUMP_DBG
		pr_emerg("msdc cmd<%d> arg<0x%x> resp<0x%x>Ready \r\n", cmd, arg, *resp);
#endif
	} else {
		error = 4;
		goto end;
...
#if MTK_MMC_DUMP_DBG
		pr_emerg("msdc cmd<%d> done \r\n", cmd);
#endif
	}

 end:
	if (error) {
		pr_emerg("cmd:%d,arg:0x%x,error=%d,intsts=0x%x\n", cmd, arg, error,
		       intsts);
		simp_msdc_dump_info(host->id);
	}
	return error;
...
static int simp_mmc_all_send_cid(struct simp_mmc_host *host, unsigned int *cid)
{
	...
	if (err) {
		pr_emerg("cmd2: error(0x%d)\n", err);
	}
#if MTK_MMC_DUMP_DBG
	pr_emerg("resp: 0x%x 0x%x 0x%x 0x%x\n", resp[0], resp[1], resp[2], resp[3]);
#endif
...
static int simp_mmc_decode_csd(struct simp_mmc_card *card)
{
	...
	if (csd->structure == 0) {
		pr_emerg("unrecognised CSD structure version %d\n", csd->structure);
		return -EINVAL;
	}
...
static int simp_mmc_select_card(struct simp_mmc_host *host, struct simp_mmc_card *card)
{
	...
	if (err) {
		pr_emerg("cmd7: select card error(0x%d)\n", err);
	}
...
static u64 simp_msdc_get_user_capacity(struct simp_mmc_card *card)
{
	...
	if (card->csd.read_blkbits)
		...
		/* pr_emerg("XXX read_blkbits = 0 XXX\n"); */
	}
	device_capacity =
...
static void simp_emmc_cal_offset(struct simp_mmc_card *card)
{
	...
	if (simp_offset < 0) {
		pr_emerg("cal offset error(0x%d)\n", simp_offset);
	}
...
static int simp_mmc_read_ext_csd(struct simp_mmc_host *host, struct simp_mmc_card *card)
{
	...
	if (card->csd.mmca_vsn < CSD_SPEC_VER_4) {
		pr_emerg("MSDC MMCA_VSN: %d. Skip EXT_CSD\n", card->csd.mmca_vsn);
		return 0;
	}
	...
	if (err) {
		pr_emerg("cmd8: send cmd to read ext csd error(0x%d)\n", err);
		goto out;
	}

	err = simp_msdc_pio_read(phost, (unsigned int *)(simp_ext_csd), 512);
	if (err) {
		pr_emerg("pio read ext csd error(0x%d)\n", err);
		goto out;
	}
...
static int simp_mmc_sd_init(struct simp_mmc_host *host)
{
	...
	if (simp_msdc_cmd(phost, CMD8, CMD8_RAW, CMD8_ARG, RESP_R7, &resp)) {
		...
		/* pr_err("SD v1.0, clr HCS bit\n"); */
		ACMD41_ARG = ACMD41_ARG_10;
	} else if (resp == CMD8_ARG) {
		/* pr_err("SD v2.0, set HCS bit\n"); */
	...
	while (retry--) {
		...
		if (resp & R3_OCR_POWER_UP_BIT) {
			...
			if (phost->card->card_cap == standard_capacity) {
				/* pr_err("just standard_capacity card!!\r\n"); */
			}
			break;
		}
	...
	/* pr_emerg("cmd7 raw: 0x%x, cmd 7 arg: 0x%x, reps type: 0x%x\n", CMD7_RAW,   CMD7_ARG, RESP_R1); */
...
static unsigned int simple_mmc_attach_mmc(struct simp_mmc_host *host)
{
	...
	if (ocr & 0x7F) {
#if MTK_MMC_DUMP_DBG
		pr_emerg("msdc0: card claims to support voltages "
	 ...
	 */
	if (!host->ocr) {
		pr_emerg("msdc0: card voltage not support\n");
		err = -EINVAL;
		goto err;
	}
	...
	if (err == SIMP_FAILED) {
		pr_emerg("init eMMC failed\n");
		goto err;
	}
	/* power up host */
#if MTK_MMC_DUMP_DBG
	pr_emerg("init eMMC success\n");
#endif
	...
	for (i = 0; i < ARRAY_SIZE(g_freqs); i++) {
		if (SIMP_SUCCESS ==
				simp_mmc_rescan_try_freq(pmmc_boot_host,
					(unsigned)max(g_freqs[i], (unsigned)HOST_MIN_MCLK)))
			break;
...
unsigned int reset_boot_up_device(int type)
{
	...
	if (type == MMC_TYPE_MMC)
		...
		pr_err("%s: do not support SD card dump!!!\n", __func__);
		/* ret = simp_init_sd(); */
	else {
				 ...
				 unsigned int size)
{
	...
	if (size != 512) {
		pr_emerg("emmc: write para error!\n");
		return -1;
	}
				...
				unsigned int size)
{
	...
	if (err) {
		pr_emerg("cmd17: error(0x%d)\n", err);
	}

	/* read the data out */
	err = simp_msdc_pio_read(phost, (unsigned int *)buf, 512);
	if (err) {
		pr_emerg("read data: error(%d)\n", err);
	}
	/* check and clear interrupt */
	/* while( (intsts & MSDC_INT_XFER_COMPL) == 0 ){ */
	/* intsts = sdr_read32(MSDC_INT); */
...
static int simp_mmc_init(int card_type, bool boot)
{
	...
	/*
	 * mtk_msdc_host is still available that when kernel PANIC happed
	 * this structure variable should not been freed
	 */
	if (boot) {
		/* init some struct */
		...
		pr_err("of_iomap for msdc @ 0x%p, id:%d\n", pmmc_boot_host->mtk_host->base,
		       pmmc_boot_host->mtk_host->id);

		pmmc_boot_host->mtk_host->clk = mtk_msdc_host[pmmc_boot_host->mtk_host->id]->clk;
		pmmc_boot_host->mtk_host->clksrc = MSDC_CLKSRC;
		pmmc_boot_host->mtk_host->hclk = clks[MSDC_CLKSRC];
		...
		/* pr_emerg("g_msdc_card[SD_MSDC_ID] addr is 0x%x\n", &g_msdc_card[EXTEND_STORAGE_ID]); */
		/* pr_emerg("g_msdc_card +1 addr is 0x%x\n", g_msdc_card + 1); */
		/* pr_emerg("pmsdc_sd_host->card addr is 0x%x\n", pmsdc_extend_host->card); */
...
#if MTK_MMC_DUMP_DBG
	pr_emerg("write start address=%llu\n",
...
#endif	/* end of CONFIG_MTK_EMMC_SUPPORT */
#endif	/* end of CONFIG_MTK_GPT_SCHEME_SUPPORT */
...
#if MTK_MMC_DUMP_DBG
	pr_err("l_start_offset = 0x%x len = %d buf<0x%x>\n", l_addr, len, l_buf);
#endif
...
/* unsigned int i; */
	...
	int err = 0;
#if MTK_MMC_DUMP_DBG
	pr_err("1 l_start_offset = 0x%x len =0x\n", l_addr, len);
#endif
	if (0 != len % 512) {
		pr_err("debug: parameter error!\n");
		return ret;
...
#if MTK_MMC_DUMP_DBG
	pr_emerg("card_dump_func_write len<%d> addr<%lld> type<%d>\n", len, offset, dev);
#endif
	if (offset % 512) {
...
#if MTK_MMC_DUMP_DBG
	pr_emerg("read start sector = %llu, part size = %llu\n", lp_start_sect,
	       lp_nr_sects);
#endif

	l_start_offset = (u64) offset + (u64) (lp_start_sect << 9);
#if MTK_MMC_DUMP_DBG
	pr_emerg("read start address = %llu\n", l_start_offset);
#endif
...
/* unsigned int l_slot; */
	...
	unsigned int sec_offset = 0;
#if MTK_MMC_DUMP_DBG
	pr_emerg("card_dump_func_read len<%d> addr<%lld> type<%d>\n", len, offset, dev);
#endif
	if (offset % 512) {
		pr_err("Address isn't 512 alignment!\n");
		return SIMP_FAILED;
