			 ...
			 void *pBuffer, unsigned int u4Len, bool write)
{
	...
	if ((pBuffer == NULL) || (host == NULL)) {
		printk("[%s] [ERR] pBuffer = 0x%p, host = 0x%p\n", __func__, pBuffer, host);
		return -1;
	}
		    ...
		    unsigned int u4Len, unsigned int u4Cmd)
{
	...
	if ((pBuffer == NULL) || (host == NULL)) {
		printk("[%s] pBuffer = 0x%p, host = 0x%p\n", __func__, pBuffer, host);
		return -1;
	}
			...
			case RD_SCAN_PAD_BOUND_E:
				if (filter) {
					if (/*(prAutok_raw_scan->Reg1Cnt <= AUTOK_TUNING_INACCURACY) && */
						...
						AUTOK_PRINT
						    ("[WARN] Try to filter the holes on raw data when CKGEN=%d\r\n",
			...
			case RD_SCAN_PAD_BOUND_E_2:
				...
				if (filter) {
					...
					/* In case of the hole locates near region1 */
						...
						AUTOK_PRINT
						    ("[ERR] Find holes on raw data when CKGEN=%d, but can NOT filter! \r\n",
						     prAutok_raw_scan->CurCKGEN);
					...
					AUTOK_PRINT
					    ("[WARN] Find too much fail regions when CKGEN=%d(Invalid)\r\n",
					     prAutok_raw_scan->CurCKGEN);
				...
				for (m = 0; m < AUTOK_RDAT_TIMES; m++) {
					...
					if (res != E_RESULT_PASS) {
						...
						if (res == E_RESULT_CMD_CRC) {
							AUTOK_PRINT
							    ("[WARN] CMD CRC error in tuning read[%d %d], need to tune command again!!\r\n",
							     x, m);
							reTuneCmd = 1;
						}
						break;
	...
	while (k <= range_max) {
		...
		if (k && !fCMDIntDlyFound && !fCMDMatrixStress) {
			if (ABS_DIFF(sel, autok_ckg_data[k - 1].interDelaySel) > 8) {
				...
				AUTOK_PRINT
				    ("[WARN]Restart to scan CKGEN due to invalid CMD matrix!IntDly Gap(%d)\r\n",
 ...
 ReTuneCMDPad:
	...
	if (stg == TUNING_STG1) {
		...
		if ((CMDIntMargin == AUTOK_SKIP_CMDTUNE_ON_STG2)) {
			AUTOK_PRINT
			    ("[WARN] The internal delay is not stable enough, skip the CMD tune!\r\n");
		...
		if (CMDIntMargin) {
			...
			AUTOK_PRINT
			    ("[WARN] The internal delay may NOT be stable enough! Reduce the range.\r\n");
		} else
	...
	if (stg == TUNING_STG1) {
		 ...
		 */
			...
			AUTOK_PRINT
			    ("[WARN] Can NOT find the best CMD pad delay, the timing between CMD & RDAT differs too much!\r\n");
			/*
		...
		/* If still can NOT find fail region after full range scan */
			...
			AUTOK_PRINT
			    ("[WARN]Still can NOT find the best CMD pad delay! Just select the middle score!\r\n");
			CMDEdgeSel = 1;
			CMDDlySel = 15;
		} else if (CMDPadScan.Reg2Cnt
			   && ((CMDPadScan.BoundReg2_E - CMDPadScan.BoundReg1_S) <= 6)) {
			AUTOK_PRINT
			    ("[WARN] Find holes in fail region when scan CMD pad delay!\r\n");
		 ...
		 */
			...
			AUTOK_PRINT
			    ("[WARN] Temperature shift on stage2 exceeds our expectation! Scan rising edge again!\r\n");
		...
		/* The scan can NOT cover the boundary */
						...
						((CMDDlyScanE - PadDlyCMDRef) <
						 CMDScanRangeStg2))) {
			AUTOK_PRINT
			    ("[WARN] Can NOT find the suitable CMD pad delay! the temp shift is too much!!!\r\n");
			CMDDlySel = PadDlyCMDRef;
			rCmdTuneRes.fTimingShiftLarge = 1;
		} else {
			AUTOK_PRINT("[ERR] Undefined scenario when scan CMD pad delay!!!\r\n");
	      ...
	      P_AUTOK_CYC_SCAN_RES_T pPadDlyRes, P_AUTOK_CMD_TUNE_RES_T pCmdTuneRes)
{
 ...
 findLBound:
			...
			if ((LeftBoundCnt >= 2) && (x < (LeftBoundCnt - 1))) {
				...
				if (AUTOK_TUNING_INACCURACY < ABS_DIFF(bound_diff,
					...
					AUTOK_PRINT
					    ("[WARN] The left Pad boundary NOT correct! Find again!\r\n");
					x++;
					goto findLBound;
				}
			} else {
		...
		if (FBoundFound) {
			...
			if (FBoundFound == 2) {
				...
				if (3 < ABS_DIFF(bound_diff, FBoundCnt))
					AUTOK_PRINT
					    ("[WARN] The full Pad boundary may NOT correct!\r\n");
			}
		...
		if (!FBoundCnt /* && !LBoundPad */) {
			...
			if (freq_mhz <= 100) {
				...
				if (pPadDlyRes->fFBoundRefCMD) {
					...
					AUTOK_PRINT
					    ("Refered from CMD: FBoundCKGEN:%d, FBoundCnt:%d, FBoundLMargin:%d, IntBound:%d\r\n",
 ...
 ChngBaseCK:
		 ...
		 */
			...
			AUTOK_PRINT
			    ("[WARN] Timing is compressed by too large output delay(CK=%d)\r\n",
			     baseCKGEN);
			baseCKGEN++;
		...
		/* In case of Right boundary at CKGEN = 0 */
			...
			/* In case of un-enough margin */
				...
				AUTOK_PRINT
				    ("[WARN] Un-enough margin to right boundary! Check layout!\r\n");
		...
		if (autok_rdata_scan[RealCKGEN].Reg1Cnt && !autok_rdata_scan[RealCKGEN].BoundReg1_S) {
			...
			if (gfIOSS && gfCoreTT && (IntBound < 32)) {
				...
				if (TinyMar > AUTOK_TINY_MAR_TH_BTM) {
					...
					AUTOK_PRINT
					    ("[WARN] Tiny margin(%d), shift the pad delay(%d) to right\r\n",
		...
		/* In case of right boundary by shifting CKGEN */
			...
			if ((IntBound < 32)
				...
				AUTOK_PRINT
				    ("[WARN] The margin may NOT up to T/2 due to internal boundary\r\n");
				...
				if (IntBound > RealPadDelay) {
					...
					AUTOK_PRINT
					    ("[WARN] The internal boundary is too large!\r\n");
		...
		while (x < SCALE_PAD_TUNE_DATWRDLY && reTuneCmd == 0) {
			...
			for (m = 0; m < (AUTOK_WDAT_TIMES); m++) {
				...
				if (res != E_RESULT_PASS) {
					...
					if (res == E_RESULT_CMD_CRC) {
						AUTOK_PRINT
						    ("[WARN] CMD CRC error in tuning write[%d %d], need to tune command again!!\r\n",
						     x, m);
						reTuneCmd = 1;
					}
					break;
