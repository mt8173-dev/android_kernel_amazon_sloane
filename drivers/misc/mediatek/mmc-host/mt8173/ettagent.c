...
static int msdc_ettagent_write(struct msdc_host *host, unsigned int u4Addr, unsigned int u4Func,
			       void *pBuffer, unsigned int u4Len, unsigned int u4Cmd);
static int msdc_ettagent_read(struct msdc_host *host, unsigned int u4Addr, unsigned int u4Func,
			      void *pBuffer, unsigned int u4Len, unsigned int u4Cmd);
...
char args_type[][DEVNAME_SIZE] = { "CMD_EDGE", "RDATA_EDGE", "WDATA_EDGE",
...
static int check_recovery(struct msdc_host *host)
{
	...
	while (1) {
		ret =
		    msdc_ettagent_read(host, SDIO_CCCR_ABORT, 0, &data, sizeof(unsigned char),
				       CMD_52);
		if (0 == ret)
			break;
		if (++cnt > 1000) {
...
static ssize_t test_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	if (select == DATA_READ) {
		...
		for (i = 0; i < (TUNING_TEST_TIME / 4); i++) {
			for (j = 0; j < 4; j++) {
				len =
				    snprintf(data_buf + count, 32, "%8x ",
					     g_test_read_pattern[i * 4 + j]);
				count += len;
			}
		...
		len =
		    snprintf(buf, 2048,
			     ...
			     select, cur_name, result_type[result], pre_pattern, post_pattern,
			     data_buf);


	} else if (select == DATA_WRITE)
		len =
		    snprintf(buf, 3 * DEVNAME_SIZE, "res:[%d:%d:%s:%s]\n[PreData:%8x][Status:%8x]",
			     id, select, cur_name, result_type[result], pre_pattern, post_pattern);
	else if (select == CMD)
		len =
		    snprintf(buf, 3 * DEVNAME_SIZE, "res:[%d:%d:%s:%s]\n[Send %8x to WTMDPCR0]", id,
			     select, cur_name, result_type[result], pre_pattern);
#endif
...
static int create_test_nodes(int size, struct kobject *parent_kobj)
{
	...
	for (i = 0; i < size; i++) {
		...
		for (j = 0; j < TEST_MODE_COUNT; j++) {
			test_attrs[i * TEST_MODE_COUNT + j] =
			    kzalloc(sizeof(struct attribute), GFP_KERNEL);
			test_kattr[i * TEST_MODE_COUNT + j] =
			...
			retval =
			    sysfs_create_file(tmode_kobj[i],
					      &test_kattr[i * TEST_MODE_COUNT + j]->attr);
			if (retval)
...
static ssize_t reg_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	if (msdc_reg_offset != -1L) {
		reg_addr = base + msdc_reg_offset;
...
#endif
{
	...
	retval = create_test_nodes(HOST_MAX_NUM, test_kobj);
	if (retval != 0)
		return retval;

	retval = create_reg_node(HOST_MAX_NUM, reg_kobj);
	if (retval != 0)
		return retval;

	retval = create_freq_node(HOST_MAX_NUM, freq_kobj);
	if (retval != 0)
		return retval;
...
static E_RESULT_TYPE autok_write_test(struct msdc_host *host, unsigned int test_data[],
				      int test_len)
{
	int i;
	E_RESULT_TYPE res = E_RESULT_PASS;
	...
	for (i = 0; i < test_len; i++) {
		...
		if (msdc_ettagent_read(host, SDIO_IP_WTMCR, LTE_MODEM_FUNC, (void *)data, 1, CMD_52)
		    != 0) {
			res = E_RESULT_CMD_CRC;
			goto end;
		}
		if (msdc_ettagent_read
			      ...
			      void *pBuffer, unsigned int u4Len, unsigned int u4Cmd)
{
	...
	if (((u4Cmd == 53) && (u4Len < 4)) || ((u4Cmd == 52) && (u4Len > 1))) {
