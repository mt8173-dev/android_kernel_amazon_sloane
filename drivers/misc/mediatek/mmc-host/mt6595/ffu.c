...
static struct mtk_emmc_ffu_ext_csd *get_ffu_ext_csd(struct mtk_emmc_ffu_ext_csd *ext_csd)
{
	...
	ext_csd->ffu_arg =
	    ((l_buf[EXT_CSD_FW_FFU_ARG_03] << 24) | (l_buf[EXT_CSD_FW_FFU_ARG_02] << 16)
...
static int is_mode_operation_codes_supported(void)
{
	...
	if (get_ffu_ext_csd(&ffu_ext_csd))
		return ffu_ext_csd.
		    ffu_features & EXT_CSD_FFU_FEATURES_SUPPORTED_MODE_OPERATION_CODES;
...
static int install_firmware(void)
{
	...
	err = mmc_switch(card, EXT_CSD_CMD_SET_NORMAL,
			 EXT_CSD_MODE_OPERATION_CODES, EXT_CSD_MODE_OPERATION_CODES_FFU_INSATALL,
			 1000);
	mmc_release_host(mmc);
	if (err) {
...
static int check_ffu_status(void)
{
	...
	if (get_ffu_ext_csd(&ffu_ext_csd)) {
		if (ffu_ext_csd.ffu_status == EXT_CSD_FFU_STATUS_SUCCESS) {
			...
			pr_err
			    ("EMMC FFU: Field fw update failed! not finish FFU flow and power cycle occurs!\n");
			return 0;
		} else if (ffu_ext_csd.ffu_status == EXT_CSD_FFU_STATUS_DOWNLOAD_ERROR) {
			pr_err
			    ("EMMC FFU: Field firmware update failed! header information is incorrect!\n");
			return 0;
		} else {
...
static ssize_t emmc_ffu_read(struct file *filp, char __user *ubuf, size_t cnt, loff_t *ppos)
{
	char buf[2] = { 0 };
	if (is_ffu_supported())
		buf[0] = '1';
...
static int emmc_ffu_write(struct file *file, const char __user *buffer, size_t size, loff_t *ppos)
{
	char tmp[4];
	int err = 0;
