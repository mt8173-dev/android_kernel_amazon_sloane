...
static void msdc_dump_reg(unsigned int base)
{
...
#ifdef MTK_MSDC_BRINGUP_DEBUG
	/* dump pad control */
	printk(KERN_ERR "=========================msdc%d pin mux dump=============================",
	       id);
	...
	/* dump clock status */
	printk(KERN_ERR "=========================msdc%d clock dump=============================",
	       id);
	printk(KERN_ERR
	...
	/* dump ldo status */
	printk(KERN_ERR "=========================msdc%d power dump=============================",
	       id);
	switch (id) {
	case 0:
...
static void msdc_set_field(unsigned int address, unsigned int start_bit, unsigned int len,
			   unsigned int value)
{
	unsigned long field;
...
static void msdc_get_field(unsigned int address, unsigned int start_bit, unsigned int len,
			   unsigned int value)
{
	unsigned long field;
	if (start_bit > 31 || start_bit < 0 || len > 32 || len <= 0)
...
static int msdc_debug_proc_write(struct file *file, const char *buf, size_t count, loff_t *data)
{
	...
	if (cmd == SD_TOOL_ZONE) {
		...
		if (id >= HOST_MAX_NUM || id < 0)
			...
			if (p1 == 0) {
				...
				if (offset == 0xE0 || offset == 0xE4 || offset == 0xE8) {
					...
					printk
					    ("[****SD_Debug****][MSDC Reg]Original:0x%x+0x%x (0x%x)\n",
					     base, offset, sdr_read32(base + offset));
					sdr_write32(base + offset, reg_value);
					printk
					    ("[****SD_Debug****][MSDC Reg]Modified:0x%x+0x%x (0x%x)\n",
					     base, offset, sdr_read32(base + offset));
				}
			} else if (p1 == 1)
				if (offset == 0xE0 || offset == 0xE4 || offset == 0xE8) {
...
#ifndef FPGA_PLATFORM
				if (p2 == 0) {
					sdr_set_field(msdc_tdrdsel_reg[id], GPIO_PAD_RDSEL_MASK,
						      p3);
					printk("[****SD_Debug****]msdc%d, set rd=%d\n", id, p3);
				} else if (p2 == 1) {
					sdr_set_field(msdc_tdrdsel_reg[id], GPIO_PAD_TDSEL_MASK,
						      p3);
					...
					sdr_get_field(msdc_tdrdsel_reg[id], GPIO_PAD_RDSEL_MASK,
						      p3);
					printk
					    ("[****SD_Debug****]msdc%d, reg[0x%x]=0x%x, rd mask:0x%x, td mask:0x%x\n",
					     id, msdc_tdrdsel_reg[id],
					     sdr_read32(msdc_tdrdsel_reg[id]), gpioPadRdselMask,
					     gpioPadTdselMask);
				}
#endif
			}
		...
		if (mode == 0) {
			...
			rw_thread =
			    kthread_create(rwThread, (void *)((id & 0x3) | ((mode & 0x3) << 4)),
					   "msdc_rw_thread");
...
static int msdc_debug_proc_write_FT(struct file *file, const char __user *buf, size_t count,
				    loff_t *data)
{
	int ret;
...
static int msdc_debug_proc_write_DVT(struct file *file, const char __user *buf, size_t count,
				     loff_t *data)
{
	int ret;
	int i_msdc_id = 0;
...
static int msdc_tune_proc_write(struct file *file, const char __user *buf, size_t count,
				loff_t *data)
{
	int ret;
	int cmd, p1, p2;
...
static int msdc_voltage_proc_write(struct file *file, const char __user *buf, size_t count,
				   loff_t *data)
{
#if 0
	int ret;
