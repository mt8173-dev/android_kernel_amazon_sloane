...
/* 0 for read; 1 for write; 2 for not hing */
...
u32 msdc_clk_ctrl_reg[4] =
    { MSDC0_GPIO_CLK_BASE, MSDC1_GPIO_CLK_BASE, MSDC2_GPIO_CLK_BASE, MSDC3_GPIO_CLK_BASE };
u32 msdc_cmd_ctrl_reg[4] =
    { MSDC0_GPIO_CMD_BASE, MSDC1_GPIO_CMD_BASE, MSDC2_GPIO_CMD_BASE, MSDC3_GPIO_CMD_BASE };
u32 msdc_dat_ctrl_reg[4] =
    { MSDC0_GPIO_DAT_BASE, MSDC1_GPIO_DAT_BASE, MSDC2_GPIO_DAT_BASE, MSDC3_GPIO_DAT_BASE };
u32 msdc_pad_ctrl_reg[4] =
    { MSDC0_GPIO_PAD_BASE, MSDC1_GPIO_PAD_BASE, MSDC2_GPIO_PAD_BASE, MSDC3_GPIO_PAD_BASE };
u32 msdc_dat1_ctrl_reg[4] =
    { MSDC0_GPIO_DAT_BASE, MSDC1_GPIO_DAT1_BASE, MSDC2_GPIO_DAT1_BASE, MSDC3_GPIO_DAT1_BASE };

void msdc_dump_padctl(struct msdc_host *host, u32 pin)
{
	switch (pin) {
	case GPIO_CLK_CTRL:
		INIT_MSG("  CLK_PAD_CTRL[0x%x]        = 0x%.8x", msdc_clk_ctrl_reg[host->id],
			 sdr_read32(msdc_clk_ctrl_reg[host->id]));
		break;
	case GPIO_CMD_CTRL:
		INIT_MSG("  CMD_PAD_CTRL[0x%x]        = 0x%.8x", msdc_cmd_ctrl_reg[host->id],
			 sdr_read32(msdc_cmd_ctrl_reg[host->id]));
		break;
	case GPIO_DAT_CTRL:
		if (host->id == 0) {
			INIT_MSG("  DAT_PAD_CTRL[0x%x]        = 0x%.8x",
				 msdc_dat_ctrl_reg[host->id],
				 sdr_read32(msdc_dat_ctrl_reg[host->id]));
		} else {
			INIT_MSG("  DAT_PAD_CTRL[0x%x]        = 0x%.8x",
				 msdc_dat_ctrl_reg[host->id],
				 sdr_read32(msdc_dat_ctrl_reg[host->id]));
			INIT_MSG("  DAT1_PAD_CTRL[0x%x]       = 0x%.8x",
				 msdc_dat1_ctrl_reg[host->id],
				 sdr_read32(msdc_dat1_ctrl_reg[host->id]));
		}
		break;
	case GPIO_RST_CTRL:
		if (host->id == 0) {
			INIT_MSG("  RST_PAD_CTRL[0x%x]        = 0x%.8x", MSDC0_GPIO_RST_BASE,
				 sdr_read32(MSDC0_GPIO_RST_BASE));
		}
		break;
	case GPIO_DS_CTRL:
		if (host->id == 0) {
			INIT_MSG("  DS_PAD_CTRL[0x%x]         = 0x%.8x", MSDC0_GPIO_DS_BASE,
				 sdr_read32(MSDC0_GPIO_DS_BASE));
		}
		break;
	case GPIO_PAD_CTRL:
		INIT_MSG("  TDRD_PAD_CTRL[0x%x]           = 0x%.8x", msdc_pad_ctrl_reg[host->id],
			 sdr_read32(msdc_pad_ctrl_reg[host->id]));
		break;
	case GPIO_MODE_CTRL:
		if (host->id == 0) {
			INIT_MSG("  PAD_MODE_CTRL[0x%x]         = 0x%.8x", MSDC0_GPIO_MODE0_BASE,
				 sdr_read32(MSDC0_GPIO_MODE0_BASE));
			INIT_MSG("  PAD_MODE_CTRL[0x%x]         = 0x%.8x", MSDC0_GPIO_MODE1_BASE,
				 sdr_read32(MSDC0_GPIO_MODE1_BASE));
			INIT_MSG("  PAD_MODE_CTRL[0x%x]         = 0x%.8x", MSDC0_GPIO_MODE2_BASE,
				 sdr_read32(MSDC0_GPIO_MODE2_BASE));
			INIT_MSG("  PAD_MODE_CTRL[0x%x]         = 0x%.8x", MSDC0_GPIO_MODE3_BASE,
				 sdr_read32(MSDC0_GPIO_MODE3_BASE));
		} else if (host->id == 1) {
			INIT_MSG("  PAD_MODE_CTRL[0x%x]         = 0x%.8x", MSDC1_GPIO_MODE0_BASE,
				 sdr_read32(MSDC1_GPIO_MODE0_BASE));
			INIT_MSG("  PAD_MODE_CTRL[0x%x]         = 0x%.8x", MSDC1_GPIO_MODE1_BASE,
				 sdr_read32(MSDC1_GPIO_MODE1_BASE));
		} else if (host->id == 2) {
			INIT_MSG("  PAD_MODE_CTRL[0x%x]         = 0x%.8x", MSDC2_GPIO_MODE0_BASE,
				 sdr_read32(MSDC2_GPIO_MODE0_BASE));
			INIT_MSG("  PAD_MODE_CTRL[0x%x]         = 0x%.8x", MSDC2_GPIO_MODE1_BASE,
				 sdr_read32(MSDC2_GPIO_MODE1_BASE));
		} else if (host->id == 3) {
			INIT_MSG("  PAD_MODE_CTRL[0x%x]         = 0x%.8x", MSDC3_GPIO_MODE0_BASE,
				 sdr_read32(MSDC3_GPIO_MODE0_BASE));
			INIT_MSG("  PAD_MODE_CTRL[0x%x]         = 0x%.8x", MSDC3_GPIO_MODE1_BASE,
...
int top_reset_restore_msdc_cfg(int restore)
{
	...
	if (restore) {
		msdc_retry(!(sdr_read32(MSDC0_BASE + OFFSET_MSDC_CFG) & MSDC_CFG_CKSTB),
			   retry, count, 0);
		if ((retry == 0) && (count == 0)) {
			return 1;
		}
...
static void msdc_top_reset(struct msdc_host *host)
{
	...
	/*printk("[%s]: back up\n",__func__); */
	sdr_get_field((MSDC0_BASE + OFFSET_SDC_DCRC_STS), (SDC_DCRC_STS_NEG | SDC_DCRC_STS_POS),
		      emmc_sdc_data_crc_value);
	for (i = 0; i < TOP_RESET_BACKUP_REG_NUM; i++) {
		sdr_get_field(top_reset_backup_reg_list[i].addr, top_reset_backup_reg_list[i].mask,
			...
			if (err) {
				ERR_MSG
				    ("failed to backup reg[0x%x][0x%x], expected value[0x%x], actual value[0x%x] err=0x%x",
				     top_reset_backup_reg_list[i].addr,
				     top_reset_backup_reg_list[i].mask,
				     top_reset_backup_reg_list[i].value,
				     sdr_read32(top_reset_backup_reg_list[i].addr), err);
			}
		}
	}
	...
	/*printk("[%s]: restore\n",__func__); */
	for (i = 0; i < TOP_RESET_BACKUP_REG_NUM; i++) {
		sdr_set_field(top_reset_backup_reg_list[i].addr, top_reset_backup_reg_list[i].mask,
			      top_reset_backup_reg_list[i].value);
		if (top_reset_backup_reg_list[i].restore_func) {
...
static void msdc_set_timeout(struct msdc_host *host, u32 ns, u32 clks)
{
	...
	N_MSG(OPS, "Set read data timeout: %dns %dclks -> %d x 1048576  cycles", ns, clks,
	      timeout + 1);
}
...
{
	printk(KERN_ERR "[%s]: msdc%d change clk_src from %dKHz to %dKHz\n", __func__, host->id,
	       (host->hclk / 1000), (hclks[clksrc] / 1000));

#if 1
	if (host->hw->host_function == MSDC_EMMC) {
		if (clksrc == MSDC_CLKSRC_800MHZ)
			clkmux_sel(MT_MUX_MSDC50_0, 1, "emmc");
		else if (clksrc == MSDC_CLKSRC_400MHZ)
			clkmux_sel(MT_MUX_MSDC50_0, 2, "emmc");
		else if (clksrc == MSDC_CLKSRC_200MHZ)
			clkmux_sel(MT_MUX_MSDC50_0, 6, "emmc");
	}
#else
	if (host->hw->host_function == MSDC_EMMC) {
		if (clksrc == MSDC_CLKSRC_800MHZ)
...
static void msdc_pm(pm_message_t state, void *data)
{
	...
	if (evt == PM_EVENT_SUSPEND || evt == PM_EVENT_USER_SUSPEND) {
		...
		printk(KERN_ERR "msdc%d -> %s Suspend", host->id,
		...
		printk(KERN_ERR "msdc%d -> %s Resume", host->id,
	...
	while (1) {
		...
		if (ptr == NULL || (PageHighMem((struct page *)(sg->page_link & ~0x3)))) {
			...
			if (subpage != 0 || (sg->offset != 0))
				N_MSG(OPS,
				      "msdc0: This read size or start not align %x,%x, hmpage %x",
...
int msdc_pio_write(struct msdc_host *host, struct mmc_data *data)
{
	...
	while (1) {
		...
		if (ptr == NULL || (PageHighMem((struct page *)(sg->page_link & ~0x3)))) {
			...
			if ((subpage != 0) || (sg->offset != 0))
				N_MSG(OPS,
				      "msdc0: This write size or start not align %d,%d, hmpage %x,sg offset %x\n",
...
#define SND_CMD 1
	...
	if (host->hw->flags & MSDC_SDIO_IRQ) {
		...
		/*ERR_MSG("Ahsin u_sdio_irq_counter=%d, u_msdc_irq_counter=%d  int_sdio_irq_enable=%d\
		   SDC_CFG=%x MSDC_INTEN=%x MSDC_INT=%x MSDC_PATCH_BIT0=%x",\
		   u_sdio_irq_counter, u_msdc_irq_counter, int_sdio_irq_enable, sdr_read32(SDC_CFG),\
		   sdr_read32(MSDC_INTEN),sdr_read32(MSDC_INT),sdr_read32(MSDC_PATCH_BIT0)); */
...
static void emmc_hs400_backup(void)
{
	...
	for (i = 0; i < HS400_BACKUP_REG_NUM; i++) {
		sdr_get_field(hs400_backup_reg_list[i].addr, hs400_backup_reg_list[i].mask,
			      hs400_backup_reg_list[i].value);
		if (hs400_backup_reg_list[i].restore_func) {
			err = hs400_backup_reg_list[i].restore_func(0);
			if (err) {
				printk(KERN_ERR
				       "[%s]: failed to restore reg[0x%x][0x%x], expected value[0x%x], actual value[0x%x] err=0x%x",
...
static void emmc_hs400_restore(void)
{
	...
	for (i = 0; i < HS400_BACKUP_REG_NUM; i++) {
		sdr_set_field(hs400_backup_reg_list[i].addr, hs400_backup_reg_list[i].mask,
			      hs400_backup_reg_list[i].value);
		if (hs400_backup_reg_list[i].restore_func) {
		...
		printk("[%s]:i:%d, reg=0x%x, value=0x%x\n", __func__, i,
		       hs400_backup_reg_list[i].addr, sdr_read32(hs400_backup_reg_list[i].addr));
	}
...
static void msdc_apply_ett_settings(struct msdc_host *host, int mode)
{
	...
	for (i = 0; i < host->hw->ett_count; i++) {
		ett =
		    (struct msdc_ett_settings *)((u32) host->hw->ett_settings +
...
static irqreturn_t msdc_irq(int irq, void *dev_id)
{
	...
	if (intsts & MSDC_INT_SDIOIRQ) {
		...
		if (host->hw->flags & MSDC_SDIO_IRQ) {
			...
			/* ERR_MSG("Ahsin u_msdc_irq_counter=%d SDC_CFG=%x MSDC_INTEN=%x MSDC_INT=%x MSDC_PATCH_BIT0=%x",\
			   u_msdc_irq_counter,sdr_read32(SDC_CFG),sdr_read32(MSDC_INTEN),sdr_read32(MSDC_INT),sdr_read32(MSDC_PATCH_BIT0)); */
...
/*
	...
	printk("[%s]: msdc%d, mmc->caps=0x%x, mmc->caps2=0x%x\n", __func__, host->id, mmc->caps,
	       mmc->caps2);
	printk(KERN_ERR
	       "[%s]: msdc%d, pll[0xf020925c][bit0~1=0x1]=0x%x, pll[0xf0209250][bit0=0x1]=0x%x, mux[0xf0000070][bit15,23,31=0x1]=0x%x, clk[0xf0003018][bit13=msdc0, bit14=msdc1]=0x%x\n",
	       __func__, host->id, sdr_read32(0xf020925c), sdr_read32(0xf0209250),
	       sdr_read32(0xf0000070), sdr_read32(0xf0003018));
#endif
