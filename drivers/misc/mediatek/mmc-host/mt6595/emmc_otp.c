...
static struct hd_struct *get_part(char *name)
{
	dev_t devt;
	int partno;
...
static void put_part(struct hd_struct *part)
{
	disk_put_part(part);
}
...
unsigned int emmc_get_wp_size(void)
{
	...
	if (0 == sg_wp_size) {
		...
		/*printk("[%s:%d]claim host done! claim_status = %d, claim_cnt = %d,\
		   claimer = 0x%x, current = 0x%x\n", __func__, __LINE__,\
...
unsigned int emmc_otp_start(void)
{
	...
	if (mmc_card_blockaddr(host_ctl->mmc->card)) {
		...
		l_size =
		    host_ctl->mmc->card->csd.capacity << (host_ctl->mmc->card->csd.read_blkbits -
							  9);
