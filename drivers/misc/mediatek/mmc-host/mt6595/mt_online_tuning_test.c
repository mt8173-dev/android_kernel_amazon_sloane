static int mt_ot_stress_test(struct msdc_host *host, u32 rawcmd, u32 rawarg, u8 *error, u8 rw)
{
	...
	for (i = 0; i < 2000; i++) {
		...
		if (cmd->resp[0] != goldenCRC |
		    ...
		    cmd->resp[2] != goldenCRC | cmd->resp[3] != goldenCRC) {
...
static int mt_ot_tmo_test(struct msdc_host *host, u32 rawcmd, u32 rawarg, u8 *error, u8 rw)
{
	...
	if (ACMDTMO | DATTMO) {
		...
		if (ACMD53Done == 1) {
			printk
			    ("[SDIO_DVT][%d] AUTOCMD53_DONE status error, AUTOCMD53_DONE should NOT be set to 1 when TMO happened\n",
			     rw);
			*error = 1;
		}
	}
...
static int mt_ot_gearRegister_test(struct msdc_host *host, u8 rw, u32 eco_ver, u8 *error)
{
	...
	for (cmdrdly = 0; cmdrdly < 32; cmdrdly++) {
		...
		if (cmdrdly < 32) {
			...
			if (tmpdly != cmdrdly) {
				printk
				    ("[SDIO_DVT][%d] PAD_CMD_RXDLY write fail, write = %d, read = %d\n",
				     rw, cmdrdly, tmpdly);
				*error = 1;
			}
		} else {
	...
	for (; dat0rddly < 32; dat0rddly++) {
		if (eco_ver >= 4) {
			rxdly0 =
			    (dat0rddly << 24) | (dat1rddly << 16) | (dat2rddly << 8) | (dat3rddly <<
											0);
		} else {
			rxdly0 =
			    (dat0rddly << 0) | (dat1rddly << 8) | (dat2rddly << 16) | (dat3rddly <<
										       24);
		}
		...
		if (dat0rddly < 32) {
			...
			if (tmpdly != rxdly0) {
				printk
				    ("[SDIO_DVT][%d] DAT_RD_DLY0 write fail, write = %d, read = %d, cmddly = %d\n",
	...
	for (gear_window_size = 0; gear_window_size < 16; gear_window_size++) {
		...
		if (tmpdly != gear_window_size) {
			printk
			    ("[SDIO_DVT][%d] SDIO_TUNE_WIND write fail, write = %d, read = %d, cmddly = %d, datdly = %d\n",
...
static int mt_ot_outbound_test(struct msdc_host *host, u32 rawcmd, u32 rawarg, u8 i_rw, u32 eco_ver,
			       u8 *error)
{
	...
	u32 cmdrdly[20] =
	    { 31, 30, 15, 0, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15 };
	u32 rxdly0;
	u32 tmpdly;
	u32 dat0rddly[20] =
	    { 15, 15, 15, 15, 31, 30, 15, 0, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15 };
	u32 dat1rddly[20] =
	    { 15, 15, 15, 15, 15, 15, 15, 15, 31, 30, 15, 0, 15, 15, 15, 15, 15, 15, 15, 15 };
	u32 dat2rddly[20] =
	    { 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 31, 30, 15, 0, 15, 15, 15, 15 };
	u32 dat3rddly[20] =
	    { 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 31, 30, 15, 0 };
	...
	for (i = 0; i < 20; i++) {
		...
		if (eco_ver >= 4) {
			rxdly0 =
			    (dat0rddly[i] << 24) | (dat1rddly[i] << 16) | (dat2rddly[i] << 8) |
			    (dat3rddly[i] << 0);
		} else {
			rxdly0 =
			    (dat0rddly[i] << 0) | (dat1rddly[i] << 8) | (dat2rddly[i] << 16) |
			    (dat3rddly[i] << 24);
		}
		sdr_write32(MSDC_DAT_RDDLY0, rxdly0);	/* Set dat delay */
		...
		if (ot_do_command(host, cmd, rawcmd, rawarg, &intsts)) {
			printk
			    ("[SDIO_DVT][%d] cmdrdly = %d, dat0rddly = %d, dat1rddly = %d, dat2rddly = %d, dat3rddly = %d, gear_window_size = %d\n\n",
			     i_rw, cmdrdly[i], dat0rddly[i], dat1rddly[i], dat2rddly[i],
			     dat3rddly[i], gear_window_size);
			*error = 1;
			return -1;
		}
		...
		if (cmdrdly[i] > 31 || dat0rddly[i] > 31 || dat1rddly[i] > 31 || dat2rddly[i] > 31
		    || dat3rddly[i] > 31) {
			if (GearOutBound == 0) {
				printk("[SDIO_DVT]intsts = 0x%x, GearOutBound = %d\n", intsts,
				       GearOutBound);
				printk
				    ("[SDIO_DVT][%d] GEAR_OUT_BOUND status error, GEAR_OUT_BOUND should be set to 1 when gear is out of bound\n",
				     i_rw);
				printk
				    ("[SDIO_DVT][%d] cmdrdly = %d, dat0rddly = %d, dat1rddly = %d, dat2rddly = %d, dat3rddly = %d, gear_window_size = %d\n\n",
				     i_rw, cmdrdly[i], dat0rddly[i], dat1rddly[i], dat2rddly[i],
		...
		} else if (cmdrdly[i] + gear_window_size / 2 > 31
			   || cmdrdly[i] < gear_window_size / 2
			   || dat0rddly[i] + gear_window_size / 2 > 31
			   || dat0rddly[i] < gear_window_size / 2
			   || dat1rddly[i] + gear_window_size / 2 > 31
			   || dat1rddly[i] < gear_window_size / 2
			   || dat2rddly[i] + gear_window_size / 2 > 31
			   || dat2rddly[i] < gear_window_size / 2
			   || dat3rddly[i] + gear_window_size / 2 > 31
			   || dat3rddly[i] < gear_window_size / 2) {
			if (GearOutBound == 0) {
				printk("[SDIO_DVT]intsts = 0x%x, GearOutBound = %d\n", intsts,
				       GearOutBound);
				printk
				    ("[SDIO_DVT][%d] GEAR_OUT_BOUND status error, GEAR_OUT_BOUND should be set to 1 when gear +/- gear window is out of bound\n",
				     i_rw);
				printk
				    ("[SDIO_DVT][%d] cmdrdly = %d, dat0rddly = %d, dat1rddly = %d, dat2rddly = %d, dat3rddly = %d, gear_window_size = %d\n\n",
				     i_rw, cmdrdly[i], dat0rddly[i], dat1rddly[i], dat2rddly[i],
			...
			if (GearOutBound == 1) {
				printk("[SDIO_DVT]intsts = 0x%x, GearOutBound = %d\n", intsts,
				       GearOutBound);
				printk
				    ("[SDIO_DVT][%d] GEAR_OUT_BOUND status error, GEAR_OUT_BOUND should be set to 0 when gear is in bound\n",
				     i_rw);
				printk
				    ("[SDIO_DVT][%d] cmdrdly = %d, dat0rddly = %d, dat1rddly = %d, dat2rddly = %d, dat3rddly = %d, gear_window_size = %d\n\n",
				     i_rw, cmdrdly[i], dat0rddly[i], dat1rddly[i], dat2rddly[i],
...
static int mt_ot_crcerr_acmd53_test(struct msdc_host *host, u32 rawcmd, u32 rawarg, u8 rw,
				    u32 eco_ver, u8 *error)
{
	...
	for (i = 0; i < 20; i++) {
		...
		if (CRCStatus_CMD != goldenCRC) {
			...
			if (CRCErr == 0) {
				printk
				    ("[SDIO_DVT][%d] SD_AUTOCMD_RESP_CRCERR status error, SD_AUTOCMD_RESP_CRCERR should be set to 1 when CRC error happened\n",
				     rw);
				printk
				    ("[SDIO_DVT][%d] cmd crc status = 0x%x, gear window size = %d, cmddly = %d, datdly = %d\n",
			...
			if (ACMD53Err == 0) {
				printk
				    ("[SDIO_DVT][%d] AUTOCMD53_FAIL status error, AUTOCMD53_FAIL should be set to 1 when CRC error happened\n",
				     rw);
				printk
				    ("[SDIO_DVT][%d] cmd crc status = 0x%x, gear window size = %d, cmddly = %d, datdly = %d\n",
				     rw, CRCStatus_CMD, gear_window_size, cmdrdly[i], rxdly0);
			...
			if (CRCErr) {
				printk
				    ("[SDIO_DVT][%d] SD_AUTOCMD_RESP_CRCERR status error, SD_AUTOCMD_RESP_CRCERR should be set to 0 when N0 CRC error\n",
				     rw);
				printk
				    ("[SDIO_DVT][%d] cmd crc status = 0x%x, gear window size = %d, cmddly = %d, datdly = %d\n",
				     rw, CRCStatus_CMD, gear_window_size, cmdrdly[i], rxdly0);
			...
			if (ACMD53Err) {
				printk
				    ("[SDIO_DVT][%d] AUTOCMD53_FAIL status error, AUTOCMD53_FAIL should be set to 0 when No CRC error\n",
				     rw);
				printk
				    ("[SDIO_DVT][%d] cmd crc status = 0x%x, gear window size = %d, cmddly = %d, datdly = %d\n",
				     rw, CRCStatus_CMD, gear_window_size, cmdrdly[i], rxdly0);
		...
		if (cmd->resp[0] != goldenCRC |
			...
			if (CRCErr == 0) {
				printk
				    ("[SDIO_DVT][%d] SD_DATA_CRCERR status error, SD_DATA_CRCERR should be set to 1 when CRC error happened\n",
				     rw);
				printk
				    ("[SDIO_DVT][%d] dat0 crc status = 0x%x, dat1 crc status = 0x%x, dat2 crc status = 0x%x, dat3 crc status = 0x%x\n",
				     rw, cmd->resp[0], cmd->resp[1], cmd->resp[2], cmd->resp[3]);
				printk
				    ("[SDIO_DVT][%d] gear window size = %d, cmddly = %d, datdly = %d\n",
			...
			if (CRCErr) {
				printk
				    ("[SDIO_DVT][%d] SD_DATA_CRCERR status error, SD_DATA_CRCERR should be set to 0 when No CRC error\n",
				     rw);
				printk
				    ("[SDIO_DVT][%d] dat0 crc status = 0x%x, dat1 crc status = 0x%x, dat2 crc status = 0x%x, dat3 crc status = 0x%x\n",
				     rw, cmd->resp[0], cmd->resp[1], cmd->resp[2], cmd->resp[3]);
		...
		if (ACMD53Done == 0) {
			printk
			    ("[SDIO_DVT][%d] AUTOCMD53_DONE status error, AUTOCMD53_DONE should be set to 1 at the end of training blocks\n",
			     rw);
...
static int mt_ot_composite_test(struct msdc_host *host, u32 rawcmd, u32 rawarg, u8 *error,
				char *xferMode, u8 rw, u32 eco_ver)
{
	u8 nRet = 0;
#if 1
	/*======TMO test======*/
...
static int msdc_ottest_proc_read(char *page, char **start, off_t off, int count, int *eof,
				 void *data)
{
	return count;
}
...
static int msdc_ottest_proc_write(struct file *file, const char *buf, unsigned long count,
				  void *data)
{
