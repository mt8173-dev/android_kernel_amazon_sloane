...
int msdc_get_dma_status(int host_id)
{
	...
	if (host_id < 0 || host_id >= HOST_MAX_NUM) {
		pr_info("[%s] failed to get dma status, bad host_id %d\n", __func__, host_id);
		return result;
	}
...
struct dma_addr *msdc_get_dma_address(int host_id)
{
	...
	if (!mtk_msdc_host[host_id]) {
		pr_info("[%s] failed to get dma status, msdc%d is not exist\n", __func__, host_id);
		return NULL;
	}
	...
	if (on) {		/* want to power on */
		if (*status == 0) {	/* can power on */
			...
			*status = powerVolt;
			...
			pr_err("LDO<%d><%d> power on again!\n", powerId, powerVolt);
		} else {	/* for sd3.0 later */
			pr_err("LDO<%d> change<%d> to <%d>\n", powerId, *status, powerVolt);
...
EXPORT_SYMBOL(msdc_set_driving);
...
static void msdc_sd_power(struct msdc_host *host, u32 on)
{
	...
	case 2:
		...
		/* msdc_ldo_power(on, MT65XX_POWER_LDO_VGP6, VOL_3300, &g_msdc2_flash); */
...
static void msdc_set_mclk(struct msdc_host *host, int ddr, u32 hz)
{
	...
	if (!hz) {		/* set mmc system clock to 0 */
		pr_err("msdc%d -> set mclk to 0", host->id);	/* fix me: need to set to 0 */
	...
	if (hz > 100000000) {
		...
		/* sdr_write32(0xF020927C, 0x800F2000);    // switch clk source to 196.625Mhz */
		/* sdr_write32(0xF020927C, 0x800F4000);  // switch clk source to 198.25Mhz */
		/* sdr_write32(0xF020927C, 0x800F6000);  // switch clk source to 199.875Mhz */

		/* set driving for DDR50/HS200 */
		sdr_set_field(MSDC0_DAT_DRVING_BASE, MSDC0_DAT_DRVING, 1);
...
static void msdc_remove_card(struct work_struct *work)
{
	...
	if (host->mmc->card) {
		...
		/* ERR_MSG("2.host->mmc<0x%x>",host->mmc); */
...
int msdc_reinit(struct msdc_host *host)
{
	...
	if (host->hw->host_function == MSDC_SD) {
		if ((!(host->hw->flags & MSDC_CD_PIN_EN)) && (host->block_bad_card == 0)) {
			...
			if (host->app_cmd_arg) {
				while ((err = msdc_get_card_status(mmc, host, &status))) {
					...
					if (err == (unsigned int)-EIO && msdc_tune_cmdrsp(host)) {
		...
		if ((host->hw->flags & MSDC_CD_PIN_EN) && (host->mmc->card)
		    && mmc_card_present(host->mmc->card) && (host->block_bad_card == 0))
			ret = 0;
	}
	return ret;
}
EXPORT_SYMBOL(msdc_reinit);
...
u32 msdc_polling_idle(struct msdc_host *host)
{
	struct mmc_host *mmc = host->mmc;
	u32 status = 0;
	u32 state = 0;
...
static void msdc_pin_config(struct msdc_host *host, int mode)
{
	...
	u32 base = host->base;
#endif
	int pull = (mode == MSDC_PIN_PULL_UP) ? MSDC_PIN_PULL_UP : MSDC_PIN_PULL_DOWN;
...
static void msdc_config_emmc_pad(int padEmmc)
{
	...
	if (padEmmc == 0) {
		sg_gpio164_mode = mt_pin_get_mode(164);
		sg_gpio165_mode = mt_pin_get_mode(165);
		sg_gpio166_mode = mt_pin_get_mode(166);
		sg_gpio168_mode = mt_pin_get_mode(168);
		sg_gpio169_mode = mt_pin_get_mode(169);
		sg_gpio170_mode = mt_pin_get_mode(170);
		sg_gpio171_mode = mt_pin_get_mode(171);
		sg_gpio173_mode = mt_pin_get_mode(173);
		sg_gpio175_mode = mt_pin_get_mode(175);

		mt_pin_set_mode_gpio(164);
		mt_pin_set_mode_gpio(165);
		mt_pin_set_mode_gpio(166);
		mt_pin_set_mode_gpio(168);
		mt_pin_set_mode_gpio(169);
		mt_pin_set_mode_gpio(170);
		mt_pin_set_mode_gpio(171);
		mt_pin_set_mode_gpio(173);
		mt_pin_set_mode_gpio(175);

		gpio_direction_input(164);
		gpio_direction_input(165);
		gpio_direction_input(166);
		gpio_direction_input(168);
		gpio_direction_input(169);
		gpio_direction_input(170);
		gpio_direction_input(171);
		gpio_direction_input(173);
		gpio_direction_input(175);

		mt_pin_set_pull(164, MT_PIN_PULL_DISABLE);
		mt_pin_set_pull(165, MT_PIN_PULL_DISABLE);
		mt_pin_set_pull(166, MT_PIN_PULL_DISABLE);
		mt_pin_set_pull(168, MT_PIN_PULL_DISABLE);
		mt_pin_set_pull(169, MT_PIN_PULL_DISABLE);
		mt_pin_set_pull(170, MT_PIN_PULL_DISABLE);
		mt_pin_set_pull(171, MT_PIN_PULL_DISABLE);
		mt_pin_set_pull(173, MT_PIN_PULL_DISABLE);
		mt_pin_set_pull(175, MT_PIN_PULL_DISABLE);
	} else {
		mt_pin_set_mode_enum(164, sg_gpio164_mode);
		mt_pin_set_mode_enum(165, sg_gpio165_mode);
		mt_pin_set_mode_enum(166, sg_gpio166_mode);
		mt_pin_set_mode_enum(168, sg_gpio168_mode);
		mt_pin_set_mode_enum(169, sg_gpio169_mode);
		mt_pin_set_mode_enum(170, sg_gpio170_mode);
		mt_pin_set_mode_enum(171, sg_gpio171_mode);
		mt_pin_set_mode_enum(173, sg_gpio173_mode);
		mt_pin_set_mode_enum(175, sg_gpio175_mode);
	}
}
...
/* #endif */
			...
			if (CHIP_SW_VER_01 != mt_get_chip_sw_ver()) {	/* only for MT8135 E2 and later */
#ifdef CONFIG_MTK_EMMC_SUPPORT
...
/* #endif */
			if (CHIP_SW_VER_01 == mt_get_chip_sw_ver()) {	/* only for MT8135 E1 */
				if (mmc_card_is_sleep(host->mmc->card)) {
					/* /                pr_err("!!!!!! hack the str, clean the sleep mode\n"); */
...
int msdc_get_reserve(void)
{
	...
	l_mtk_reserve = 0; //(unsigned int)(0x00a8) << 8;	/* unit is 512B */
#ifdef CONFIG_MTK_EMMC_SUPPORT_OTP
	/* lp_excel_info = msdc_reserve_part_info("otp"); */
	l_otp_reserve = (unsigned int)(0x0158) << 8;	/* unit is 512B */
					      ...
					      int tune, unsigned long timeout)
{
	...
	if (intsts & cmdsts) {
		if ((intsts & MSDC_INT_CMDRDY) || (intsts & MSDC_INT_ACMDRDY) ||
			...
			IRQ_MSG("XXX CMD<%d> MSDC_INT_RSPCRCERR Arg<0x%.8x>", cmd->opcode,
				cmd->arg);
			msdc_reset_hw(host->id);
...
{
	u32 base = host->base;
	u32 wints = MSDC_INTEN_XFER_COMPL | MSDC_INTEN_DATTMO | MSDC_INTEN_DATCRCERR | MSDC_INTEN_DMA_BDCSERR | MSDC_INTEN_DMA_GPDCSERR | MSDC_INTEN_DMA_PROTECT;	/* CHECK ME! 5J */
	if (host->autocmd == MSDC_AUTOCMD12)
		wints |= MSDC_INT_ACMDCRCERR | MSDC_INT_ACMDTMO | MSDC_INT_ACMDRDY | MSDC_INT_DMA_BDCSERR | MSDC_INT_DMA_GPDCSERR | MSDC_INT_DMA_PROTECT;	/* CHECK ME! 5J */
	sdr_set_bits(MSDC_INTEN, wints);
	mb();
...
static void msdc_dma_dump(struct msdc_host *host, struct msdc_dma *dma)
{
	...
	/* dump bd */
	ERR_MSG(".bd<0x%.8x> bd_phy<0x%.8x> gpd_ptr<0x%.8x>", (int)bd, (int)dma->bd_addr,
		(int)gpd->ptr);
	ptr = bd;
	...
	case MSDC_MODE_DMA_DESC:
		...
		for (j = 0; j < bdlen; j++) {
			...
			if (g_dma_debug[host->id]
				...
				pr_info("[%s] msdc%d do write 0x10000\n", __func__, host->id);
				dma_address = 0x10000;
			} else {
				dma_address = sg_dma_address(sg);
			}
...
#define SND_CMD 1
	...
	if (msdc_txfifocnt() || msdc_rxfifocnt()) {
		pr_info("[SD%d] register abnormal,please check!\n", host->id);
		msdc_reset_hw(host->id);
	}
...
static unsigned int msdc_command_start_simple(struct msdc_host *host, struct mmc_command *cmd, int tune,	/* not used */
					      unsigned long timeout)
{
	u32 base = host->base;
	u32 opcode = cmd->opcode;
...
u32 msdc_intr_wait(struct msdc_host *host, struct mmc_command *cmd, u32 intrs,
		   unsigned long timeout)
{
	u32 base = host->base;
	u32 sts;
	...
	/* command interrupts */
	if ((sts & MSDC_INT_CMDRDY) || (sts & MSDC_INT_ACMDRDY) || (sts & MSDC_INT_ACMD19_DONE)) {
		u32 *rsp = &cmd->resp[0];
		...
		if (sts & MSDC_INT_ACMDCRCERR) {
			pr_info("XXX CMD<%d> MSDC_INT_ACMDCRCERR", cmd->opcode);
		} else {
			pr_info("XXX CMD<%d> MSDC_INT_RSPCRCERR Arg<0x%x>", cmd->opcode, cmd->arg);
		}
		cmd->error = (unsigned int)-EIO;
		msdc_reset_hw(host->id);
	} else if ((sts & MSDC_INT_CMDTMO) || (sts & MSDC_INT_ACMDTMO)) {
		if (sts & MSDC_INT_ACMDTMO) {
			pr_info("XXX CMD<%d> MSDC_INT_ACMDTMO", cmd->opcode);
		} else {
			pr_info("XXX CMD<%d> MSDC_INT_CMDTMO Arg<0x%x>", cmd->opcode, cmd->arg);
		}
		cmd->error = (unsigned int)-ETIMEDOUT;
		msdc_reset_hw(host->id);
	}
					     ...
					     int tune, unsigned long timeout)
{
	...
	u32 wints = MSDC_INT_CMDRDY | MSDC_INT_RSPCRCERR | MSDC_INT_CMDTMO |
	    MSDC_INT_ACMDRDY | MSDC_INT_ACMDCRCERR | MSDC_INT_ACMDTMO | MSDC_INT_ACMD19_DONE;
...
static int msdc_tune_write(struct msdc_host *host)
{
	...
	sdr_get_field(MSDC_PATCH_BIT1, MSDC_PATCH_BIT1_WRDAT_CRCS, orig_d_cntr);
	cur_rxdly0 = sdr_read32(MSDC_DAT_RDDLY0);
	INIT_MSG("TUNE_WRITE: dsmpl<%d> wrrdly<%d> rxdly0<0x%x> d_cntr<%d> sfreq.<%d>", orig_dsmpl,
		 orig_wrrdly, cur_rxdly0, orig_d_cntr, host->sclk);
		...
		if (data && msdc_abort_data(host)) {
			...
			if (host->hw->host_function == MSDC_SD) {
				...
				if (host->mmc->card) {
					...
					spin_unlock_irqrestore(&host->remove_bad_card,
							flags);
		...
		if (data && (data->error == (unsigned int)-ETIMEDOUT)) {
			if (data->flags & MMC_DATA_READ) {
				if ((host->sw_timeout)
					...
					ERR_MSG
					    ("msdc%d exceed max read timeout tune times(%d) or SW timeout(%d),Power cycle\n",
				...
				if ((host->sw_timeout)
					...
					ERR_MSG
					    ("msdc%d exceed max write timeout tune times(%d) or SW timeout(%d),Power cycle\n",
...
static void msdc2_eint_handler(void)
{
	...
	spin_unlock_irqrestore(&host->remove_bad_card, flags);
	host->block_bad_card = 0;
	mt_eint_set_polarity(EINT_MSDC2_INS_NUM, host->sd_cd_polarity);
	tasklet_hi_schedule(&host->card_tasklet);
	ERR_MSG("SD card %s", (host->hw->cd_level ^ host->sd_cd_polarity) ? "insert" : "remove");
}
#endif
static irqreturn_t msdc_irq(int irq, void *dev_id)
{
	...
	if (data != NULL) {
		...
		if ((stop != NULL) && (host->autocmd == MSDC_AUTOCMD12) && (intsts & cmdsts)) {
			...
			if (((intsts & MSDC_INT_ACMDCRCERR) || (intsts & MSDC_INT_ACMDTMO)) && host->dma_xfer) {
				if ((data->host_cookie) && (host->tune == 0)) {
					msdc_dma_stop(host);
					msdc_clr_fifo(host->id);
					msdc_clr_int();
...
static void msdc_enable_cd_irq(struct msdc_host *host, int enable)
{
	...
	if (enable) {
		if (host->id == 1 && (MSDC_SD == hw->host_function) && (hw->flags & MSDC_CD_PIN_EN)) {
			/* set pinmux/pupd */
			sdr_set_field_discrete(MSDC1_DETECT_BASE, MSDC1_DETECT_SEL, 1);
			sdr_set_field_discrete(MSDC1_PUPD_ENA_DET_BASE, MSDC1_PUPD_ENA_DET, 1);
			sdr_set_field_discrete(MSDC1_PUPD_DET_BASE, MSDC1_PUPD_DET, 1);

			if (hw->cd_level) {
				host->sd_cd_polarity = (~EINT_MSDC1_INS_POLARITY) & 0x1;
				trig_flag = EINTF_TRIGGER_HIGH;
			} else {
				host->sd_cd_polarity = EINT_MSDC1_INS_POLARITY;
				trig_flag = EINTF_TRIGGER_LOW;
			}
...
#if 1
			mt_eint_registration(EINT_MSDC1_INS_NUM, trig_flag,
					     /* host->sd_cd_polarity, */
					     msdc1_eint_handler, 1);
#else
			mt_eint_registration(EINT_MSDC1_INS_NUM, EINT_MSDC1_INS_DEBOUNCE_EN,
					     /* host->sd_cd_polarity, */
					     msdc1_eint_handler, 1);
#endif
		...
		if (host->id == 2 && (MSDC_SD == hw->host_function) && (hw->flags & MSDC_CD_PIN_EN)) {
			...
			mt_eint_set_sens(EINT_MSDC2_INS_NUM, EINT_MSDC2_INS_SENSITIVE);
			mt_eint_set_hw_debounce(EINT_MSDC2_INS_NUM, EINT_MSDC2_INS_DEBOUNCE_CN);
			mt_eint_registration(EINT_MSDC2_INS_NUM, EINT_MSDC2_INS_DEBOUNCE_EN,
					     /* host->sd_cd_polarity, */
					     msdc2_eint_handler, 1);

			/* mt_eint_unmask(CUST_EINT_MSDC2_INS_NUM); */
			ERR_MSG("SD card detection eint resigter.");
		...
		if (host->id == 2) {
			mt_eint_mask(EINT_MSDC2_INS_NUM);
		}
	}
#else
	struct msdc_hw *hw = host->hw;
	...
	if ((hw->flags & MSDC_CD_PIN_EN) == 0) {
		...
		/*
		   ...
		   hw->config_gpio_pin(MSDC_CD_PIN, MSDC_PIN_PULL_DOWN);
		 */
		sdr_clr_bits(MSDC_PS, MSDC_PS_CDEN);
		sdr_clr_bits(MSDC_INTEN, MSDC_INTEN_CDSC);
		sdr_clr_bits(SDC_CFG, SDC_CFG_INSWKUP);
	...
	if (enable) {
		if (hw->enable_cd_eirq) {	/* not set, never enter */
			 ...
			 */
			if (hw->config_gpio_pin)	/* NULL */
				hw->config_gpio_pin(MSDC_CD_PIN, MSDC_PIN_PULL_UP);
...
EXPORT_SYMBOL(mmc_get_disk);


#if 0				/* defined(CONFIG_MTK_EMMC_SUPPORT) */
...
void emmc_create_sys_symlink(struct mmc_card *card)
{
	...
	if (host != (struct msdc_host *)card->host->private) {
		pr_info(DRV_NAME "%s: NOT emmc card, \n", __func__);
		return;
	}
	...
	while ((part = disk_part_iter_next(&piter))) {
		for (i = 0; i < PART_NUM; i++) {
			if (PartInfo[i].partition_idx != 0
				...
				pr_info(DRV_NAME "%s: target=%s, name=%s \n", __func__,
...
static void msdc_timer_pm(unsigned long data)
{
	...
	if (host->clk_gate_count == 0) {
		...
		pr_info("[%s]: msdc%d, time out, but clk_gate_count=%d\n", __func__, host->id,
			host->clk_gate_count);
	}
	spin_unlock_irqrestore(&host->clk_gate_lock, flags);
}
...
static void msdc_set_host_power_control(struct msdc_host *host)
{
	...
	case 0:
		...
		break;
	case 1:
		if ((MSDC_VEMC_3V3 == host->power_domain) && (MSDC_SD == host->hw->host_function)) {
...
static void msdc_check_mpu_voilation(u32 addr, int wr_vio)
{
	...
	pr_crit("MSDC check EMI MPU violation.\n");
	pr_crit("addr = 0x%x,%s violation.\n", addr,
		wr_vio ? "Write memory<Read frome eMMC/SD>" : "Read memory<Write to eMMC/SD>");
	pr_crit("MSDC DMA running status:\n");
	pr_crit("**************************************\n");
	for (index = 0; index < HOST_MAX_NUM; index++) {
		status = msdc_get_dma_status(index);
		pr_crit("MSDC%d: ", index);
		if (status == 0) {
			pr_crit("DMA mode is disabled Now\n");
			continue;
		} else if (status == 1) {
			pr_crit("Read data from eMMC/SD to DRAM with DMA mode\n");
		} else if (status == 2) {
			pr_crit("Write data from DRAM to eMMC/SD with DMA mode\n");
		} else if (status == -1) {
			pr_crit("No data transaction or the device is not present until now\n");
			continue;
		}
...
static void msdc_register_emi_mpu_callback(int id)
{
	...
	case 4:
		...
		pr_crit("msdc%d register emi_mpu_notifier\n", id);
		break;
	case 2:
...
#ifdef CONFIG_MT8135_FPGA
	sdr_write16(PWR_GPIO_EO, l_val | 0xFF00);
#else
	sdr_write16(PWR_GPIO_EO,
		    (l_val | PWR_GPIO_L4_DIR | PWR_MASK_EN | PWR_MASK_VOL_33 | PWR_MASK_VOL_18));
#endif
	l_val = sdr_read16(PWR_GPIO_EO);
	pr_info("[%s]: pwr gpio dir = 0x%x\n", __func__, l_val);
#endif
	if (first_probe == 0) {
		first_probe++;
		...
		enable_clock(PERI_MSDC4_PDN, "SD");
#endif
		 ...
		 */
		pr_err("[5J] Disable all msdc clock in the first entry of probe()!\n");
...
/*
	...
	/* mt65xx_irq_set_polarity(irq, MT_POLARITY_LOW); */
