...
struct proc_dir_entry *mtk_thermal_get_proc_drv_cooler_dir_entry(void);
...
static int __init mtkthermal_init(void)
{
	...
	if (!entry) {
		THRML_ERROR_LOG
		    ("[mtkthermal_init] Can not create mtm_indicator\n");
	}
	...
	if (!entry) {
		THRML_ERROR_LOG("[mtkthermal_init] Can not create mtm_extctrl\n");
	}
...
struct proc_dir_entry *mtk_thermal_get_proc_drv_cooler_dir_entry(void)
{
	return _get_proc_cooler_dir_entry();
}
...
EXPORT_SYMBOL(mtk_thermal_get_proc_drv_cooler_dir_entry);
