...
#if 0
	xlog_printk(ANDROID_LOG_INFO, "Power/AP_Thermal",
		    "APtThermistorConverTemp() : TAP_Value = %d\n", TAP_Value);
...
#if 0
	{
		...
		for (i = 0; i < (sizeof(AP_Temperature_Table) / sizeof(AP_TEMPERATURE)); i++) {
			mtkts_AP_dprintk("AP_Temperature_Table[%d].APteryTemp =%d\n", i,
					 AP_Temperature_Table[i].APTemp);
			mtkts_AP_dprintk("AP_Temperature_Table[%d].TemperatureR=%d\n", i,
					 AP_Temperature_Table[i].TemperatureR);
		}
	}
#endif
}
...
static int __init mtkts_AP_init(void)
{
	...
	if (!mtkts_AP_dir) {
		...
		entry =
		    proc_create("mtktsAP", S_IRUGO | S_IWUSR | S_IWGRP, mtkts_AP_dir,
				&mtkts_AP_fops);
		if (entry) {
		...
		entry =
		    proc_create("mtkts_AP_param", S_IRUGO | S_IWUSR | S_IWGRP, mtkts_AP_dir,
				&mtkts_AP_param_fops);
		if (entry) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0)
			proc_set_user(entry, 0, 1000);
