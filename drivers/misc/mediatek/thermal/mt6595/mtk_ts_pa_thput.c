...
static int __init mtk_mdm_txpwr_init(void)
{
	...
	if (!mdtxpwr_dir) {
		...
		entry =
		    proc_create("txpwr_sw", S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP, mdtxpwr_dir,
				&mtk_mdm_sw_fops);
		entry =
		    proc_create("txpwr_value", S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP, mdtxpwr_dir,
				&mtk_mdm_value_fops);
		entry =
		    proc_create("timeout", S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP, mdtxpwr_dir,
				&mtk_mdm_proc_timeout_fops);

#if MTK_TS_PA_THPUT_VIA_ATCMD == 1
		entry =
		    proc_create("mdinfo", S_IRUGO | S_IWUSR | S_IWGRP, mdtxpwr_dir,
		...
		entry =
		    proc_create("mdinfoex", S_IRUGO | S_IWUSR | S_IWGRP, mdtxpwr_dir,
		...
		entry =
		    proc_create("mdinfoex_thre", S_IRUGO, mdtxpwr_dir,
