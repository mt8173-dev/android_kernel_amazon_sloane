	...
	if (curr_temp != 0) {	/* not the first temp read after resume from suspension */
		if ((curr_temp > 150000) || (curr_temp < -20000)) {	/* invalid range */
			...
			if ((curr_temp - last_abb_read_temp > 20000) || (last_abb_read_temp - curr_temp > 20000)) {	/* delta 20C, invalid change */
				printk
				    ("[Power/ALLTS_Thermal] TS1 temp float hugely temp=%d, lasttemp=%d\n",
...
static int tsallts_get_temp2(struct thermal_zone_device *thermal, unsigned long *t)
{
#if MTK_ALLTS_SW_FILTER == 1
	int curr_temp;
	int temp_temp;
	...
	if (curr_temp != 0) {	/* not the first temp read after resume from suspension */
		if ((curr_temp > 150000) || (curr_temp < -20000)) {	/* invalid range */
			...
			if ((curr_temp - last_abb_read_temp > 20000) || (last_abb_read_temp - curr_temp > 20000)) {	/* delta 20C, invalid change */
				printk
				    ("[Power/ALLTS_Thermal] TS2 temp float hugely temp=%d, lasttemp=%d\n",
				     curr_temp, last_abb_read_temp);
				temp_temp = 50000;
				ret = -1;
			}
...
static int tsallts_get_temp3(struct thermal_zone_device *thermal, unsigned long *t)
{
	...
	if (curr_temp != 0) {	/* not the first temp read after resume from suspension */
		if ((curr_temp > 150000) || (curr_temp < -20000)) {	/* invalid range */
			...
			if ((curr_temp - last_abb_read_temp > 20000) || (last_abb_read_temp - curr_temp > 20000)) {	/* delta 20C, invalid change */
				printk
				    ("[Power/ALLTS_Thermal] TS3 temp float hugely temp=%d, lasttemp=%d\n",
				     curr_temp, last_abb_read_temp);
				temp_temp = 50000;
				ret = -1;
			}
...
static int tsallts_get_temp4(struct thermal_zone_device *thermal, unsigned long *t)
{
	...
	if (curr_temp != 0) {	/* not the first temp read after resume from suspension */
		if ((curr_temp > 150000) || (curr_temp < -20000)) {	/* invalid range */
			...
			if ((curr_temp - last_abb_read_temp > 20000) || (last_abb_read_temp - curr_temp > 20000)) {	/* delta 20C, invalid change */
				printk
				    ("[Power/ALLTS_Thermal] TS4 temp float hugely temp=%d, lasttemp=%d\n",
				     curr_temp, last_abb_read_temp);
				temp_temp = 50000;
				ret = -1;
			}
			      ...
			      loff_t *data)
{
	...
	if (sscanf
		...
		tsallts_dprintk
		    ("[tsallts_write1] g_THERMAL_TRIP_0=%d,g_THERMAL_TRIP_1=%d,g_THERMAL_TRIP_2=%d,g_THERMAL_TRIP_3=%d,g_THERMAL_TRIP_4=%d,\
		g_THERMAL_TRIP_5=%d,g_THERMAL_TRIP_6=%d,g_THERMAL_TRIP_7=%d,g_THERMAL_TRIP_8=%d,g_THERMAL_TRIP_9=%d,\n",
		...
		cooldev5=%s,cooldev6=%s,cooldev7=%s,cooldev8=%s,cooldev9=%s\n", g_bind0, g_bind1,
				g_bind2, g_bind3, g_bind4, g_bind5, g_bind6, g_bind7, g_bind8, g_bind9);
		...
		tsallts_dprintk
		    ("[tsallts_write] trip_0_temp=%d,trip_1_temp=%d,trip_2_temp=%d,trip_3_temp=%d,trip_4_temp=%d,\
		trip_5_temp=%d,trip_6_temp=%d,trip_7_temp=%d,trip_8_temp=%d,trip_9_temp=%d,time_ms=%d\n",
		     trip_temp[0], trip_temp[1], trip_temp[2], trip_temp[3], trip_temp[4], trip_temp[5], trip_temp[6], trip_temp[7], trip_temp[8], trip_temp[9],
		     interval);
...
static ssize_t tsallts_write2(struct file *file, const char __user *buffer, size_t count,
			      loff_t *data)
{
	int len = 0, time_msec = 0;
	int trip[10] = { 0 };
	...
	if (sscanf
		...
		tsallts_dprintk
		    ("[tsallts_write2] g_THERMAL_TRIP_0=%d,g_THERMAL_TRIP_1=%d,g_THERMAL_TRIP_2=%d,g_THERMAL_TRIP_3=%d,g_THERMAL_TRIP_4=%d,\
		g_THERMAL_TRIP_5=%d,g_THERMAL_TRIP_6=%d,g_THERMAL_TRIP_7=%d,g_THERMAL_TRIP_8=%d,g_THERMAL_TRIP_9=%d,\n",
		     g_THERMAL_TRIP[0], g_THERMAL_TRIP[1], g_THERMAL_TRIP[2], g_THERMAL_TRIP[3], g_THERMAL_TRIP[4], g_THERMAL_TRIP[5], g_THERMAL_TRIP[6], g_THERMAL_TRIP[7], g_THERMAL_TRIP[8],
		     g_THERMAL_TRIP[9]);
		...
		tsallts_dprintk
		    ("[tsallts_write2] trip_0_temp=%d,trip_1_temp=%d,trip_2_temp=%d,trip_3_temp=%d,trip_4_temp=%d,\
		trip_5_temp=%d,trip_6_temp=%d,trip_7_temp=%d,trip_8_temp=%d,trip_9_temp=%d,time_ms=%d\n",
		     trip_temp[0], trip_temp[1], trip_temp[2], trip_temp[3], trip_temp[4], trip_temp[5], trip_temp[6], trip_temp[7], trip_temp[8], trip_temp[9],
		     interval);
			      ...
			      loff_t *data)
{
	...
	if (sscanf
		...
		tsallts_dprintk
		    ("[tsallts_write3] g_THERMAL_TRIP_0=%d,g_THERMAL_TRIP_1=%d,g_THERMAL_TRIP_2=%d,g_THERMAL_TRIP_3=%d,g_THERMAL_TRIP_4=%d,\
		g_THERMAL_TRIP_5=%d,g_THERMAL_TRIP_6=%d,g_THERMAL_TRIP_7=%d,g_THERMAL_TRIP_8=%d,g_THERMAL_TRIP_9=%d,\n",
		     g_THERMAL_TRIP[0], g_THERMAL_TRIP[1], g_THERMAL_TRIP[2], g_THERMAL_TRIP[3], g_THERMAL_TRIP[4], g_THERMAL_TRIP[5], g_THERMAL_TRIP[6], g_THERMAL_TRIP[7], g_THERMAL_TRIP[8],
		     g_THERMAL_TRIP[9]);
		...
		tsallts_dprintk
		    ("[tsallts_write3] trip_0_temp=%d,trip_1_temp=%d,trip_2_temp=%d,trip_3_temp=%d,trip_4_temp=%d,\
		trip_5_temp=%d,trip_6_temp=%d,trip_7_temp=%d,trip_8_temp=%d,trip_9_temp=%d,time_ms=%d\n",
		     trip_temp[0], trip_temp[1], trip_temp[2], trip_temp[3], trip_temp[4], trip_temp[5], trip_temp[6], trip_temp[7], trip_temp[8], trip_temp[9],
		     interval);
			      ...
			      loff_t *data)
{
	...
	if (sscanf
		...
		tsallts_dprintk
		    ("[tsallts_write4] g_THERMAL_TRIP_0=%d,g_THERMAL_TRIP_1=%d,g_THERMAL_TRIP_2=%d,g_THERMAL_TRIP_3=%d,g_THERMAL_TRIP_4=%d,\
		g_THERMAL_TRIP_5=%d,g_THERMAL_TRIP_6=%d,g_THERMAL_TRIP_7=%d,g_THERMAL_TRIP_8=%d,g_THERMAL_TRIP_9=%d,\n",
		     g_THERMAL_TRIP[0], g_THERMAL_TRIP[1], g_THERMAL_TRIP[2], g_THERMAL_TRIP[3], g_THERMAL_TRIP[4], g_THERMAL_TRIP[5], g_THERMAL_TRIP[6], g_THERMAL_TRIP[7], g_THERMAL_TRIP[8],
		     g_THERMAL_TRIP[9]);
		tsallts_dprintk("[tsallts_write] cooldev0=%s,cooldev1=%s,cooldev2=%s,cooldev3=%s,cooldev4=%s,\
		...
		tsallts_dprintk
		    ("[tsallts_write4] trip_0_temp=%d,trip_1_temp=%d,trip_2_temp=%d,trip_3_temp=%d,trip_4_temp=%d,\
		trip_5_temp=%d,trip_6_temp=%d,trip_7_temp=%d,trip_8_temp=%d,trip_9_temp=%d,time_ms=%d\n",
		     trip_temp[0], trip_temp[1], trip_temp[2], trip_temp[3], trip_temp[4], trip_temp[5], trip_temp[6], trip_temp[7], trip_temp[8], trip_temp[9],
		     interval);
...
/* err = tsallts_register_cooler(); */
/* printk(KERN_CRIT "tsallts_register_cooler:err=%x\n",err); */
/* if(err) */
/* return err; */
	...
	tsallts_dir = proc_mkdir("mtktsallts", NULL);
	if (!tsallts_dir) {
		tsallts_dprintk("[tsallts_init]: mkdir /proc/mtktsallts failed\n");
	} else {
		entry =
		    proc_create("mtkts1", S_IRUGO | S_IWUSR | S_IWGRP, tsallts_dir, &tsallts_fops1);
		if (entry) {
		...
		entry =
		    proc_create("mtkts2", S_IRUGO | S_IWUSR | S_IWGRP, tsallts_dir, &tsallts_fops2);
		if (entry) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0)
			proc_set_user(entry, 0, 1000);
#else
		...
		entry =
		    proc_create("mtkts3", S_IRUGO | S_IWUSR | S_IWGRP, tsallts_dir, &tsallts_fops3);
		if (entry) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0)
			proc_set_user(entry, 0, 1000);
#else
		...
		entry =
		    proc_create("mtkts4", S_IRUGO | S_IWUSR | S_IWGRP, tsallts_dir, &tsallts_fops4);
		if (entry) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0)
			proc_set_user(entry, 0, 1000);
#else
