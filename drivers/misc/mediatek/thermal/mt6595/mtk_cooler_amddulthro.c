...
static unsigned int deepest_step;
...
ssize_t amddulthro_param_write(struct file *filp, const char __user *buf, unsigned long len,
			       void *data)
{
	char desc[MAX_LEN] = { 0 };
...
ssize_t amddulthro_pid_write(struct file *filp, const char __user *buf, unsigned long len,
			     void *data)
{
	int ret = 0;
	char tmp[MAX_LEN] = { 0 };
...
ssize_t amddulthro_dbg_write(struct file *filp, const char __user *buf, unsigned long len,
			     void *data)
{
	char desc[MAX_LEN] = { 0 };
...
static int amddulthro_proc_register(void)
{
	...
	if (!amddulthro_proc_dir) {
		...
		entry =
		    create_proc_entry("amddulthro_param", S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP,
				      amddulthro_proc_dir);
		if (entry) {
