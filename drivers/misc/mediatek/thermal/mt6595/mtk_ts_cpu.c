...
*/
static int CA7_TS1_T = 0, CA7_TS2_T;
static int CA15_TS1_T = 0, CA15_TS3_T;
static int GPU_TS3_T = 0, GPU_TS4_T;
static int CORE_TS2_T = 0, CORE_TS4_T = 0, CORE_TSABB_T;

static int CA7_TS1_R = 0, CA7_TS2_R;
static int CA15_TS1_R = 0, CA15_TS3_R;
static int GPU_TS3_R = 0, GPU_TS4_R;
static int CORE_TS2_R = 0, CORE_TS4_R = 0, CORE_TSABB_R;
...
static void set_adaptive_cpu_power_limit(unsigned int limit)
{
	...
	if (prv_adp_cpu_pwr_lim != adaptive_cpu_power_limit) {
		tscpu_printk("set_adaptive_cpu_power_limit %d, T=%d,%d,%d,%d,%d,%d,%d,%d,%d\n",
			     (final_limit != 0x7FFFFFFF) ? final_limit : 0, CA7_TS1_T, CA15_TS1_T,
			     CA7_TS2_T, CORE_TS2_T, CA15_TS3_T, GPU_TS3_T, GPU_TS4_T, CORE_TS4_T,
			     CORE_TSABB_T);
...
static void tscpu_set_GPIO_toggle_for_monitor(void)
{
	...
	if (g_GPIO_out_enable == 1) {
		if (g_max_temp > g_trigger_temp) {
			...
			tscpu_printk
			    ("tscpu_set_GPIO_toggle_for_monitor:lv_GPIO118_MODE=0x%x,lv_GPIO118_DIR=0x%x,lv_GPIO118_DOUT=0x%x,\n",
				       ...
				       loff_t *data)
{
	...
	if (9 <=
	    sscanf(desc, "%d %d %d %d %d %d %d %d %d", &i_id, &i_first_step, &i_theta_r, &i_theta_f,
			   ...
			   loff_t *data)
{
	...
	if (sscanf
		...
		tscpu_dprintk
		    ("tscpu_write g_THERMAL_TRIP_0=%d,g_THERMAL_TRIP_1=%d,g_THERMAL_TRIP_2=%d,g_THERMAL_TRIP_3=%d,g_THERMAL_TRIP_4=%d,\
...
static int tscpu_thermal_probe(struct platform_device *pdev)
{
	 ...
	 */
	temp = DRV_Reg32(TS_CON1);
	tscpu_printk("tscpu_thermal_probe :TS_CON1=0x%x\n", temp);
	temp &= ~(0x00000030);	/* TS_CON1[5:4]=2'b00,   00: Buffer on, TSMCU to AUXADC */
	THERMAL_WRAP_WR32(temp, TS_CON1);	/* read abb need */
	udelay(200);		/* RG_TS2AUXADC < set from 2'b11 to 2'b00 when resume.wait 100uS than turn on thermal controller. */
	...
	err =
	    request_irq(THERM_CTRL_IRQ_BIT_ID, thermal_all_bank_interrupt_handler, IRQF_TRIGGER_LOW,
			THERMAL_NAME, NULL);
	if (err)
		tscpu_printk("tscpu_thermal_probe IRQ register fail\n");
	...
	if (!mtktscpu_dir) {
		tscpu_printk("tscpu_thermal_probe mkdir /proc/mtktscpu failed\n");
	} else {

		entry =
		    proc_create("mtktscpu", S_IRUGO | S_IWUSR | S_IWGRP, mtktscpu_dir,
				&mtktscpu_fops);
		if (entry) {
		...
		entry =
		    proc_create("mtktscpu_read_temperature", S_IRUGO, mtktscpu_dir,
				&mtktscpu_read_temperature_fops);

		entry =
		    proc_create("mtktscpu_set_temperature", S_IRUGO | S_IWUSR, mtktscpu_dir,
				&mtktscpu_set_temperature_fops);

		entry =
		    proc_create("mtktscpu_talking_flag", S_IRUGO | S_IWUSR, mtktscpu_dir,
				&mtktscpu_talking_flag_fops);
#if CPT_ADAPTIVE_AP_COOLER
		entry =
		    proc_create("mtktscpu_dtm_setting", S_IRUGO | S_IWUSR | S_IWGRP, mtktscpu_dir,
		...
		entry =
		    proc_create("mtktscpu_gpu_threshold", S_IRUGO | S_IWUSR | S_IWGRP, mtktscpu_dir,
...
#if MTKTSCPU_FAST_POLLING
		entry =
		    proc_create("mtktscpu_fastpoll", S_IRUGO | S_IWUSR | S_IWGRP, mtktscpu_dir,
		...
		entry =
		    proc_create("mtktscpu_Tj_out_via_HW_pin", S_IRUGO | S_IWUSR, mtktscpu_dir,
...
#if THERMAL_GPIO_OUT_TOGGLE
		entry =
		    proc_create("mtktscpu_GPIO_out_monitor", S_IRUGO | S_IWUSR, mtktscpu_dir,
		...
		/* +ASC+ */
		entry =
		    proc_create("mtktscpu_atm", S_IRUGO | S_IWUSR, mtktscpu_dir,
				&mtktscpu_atm_fops);
		if (entry) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0)
			proc_set_user(entry, 0, 1000);
