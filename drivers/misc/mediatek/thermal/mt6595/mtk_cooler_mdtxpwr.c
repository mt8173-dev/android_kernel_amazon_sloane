...
static void mtk_cl_mdtxpwr_set_mdtxpwr_limit(void)
{
	...
	if (min_limit != cl_mdtxpwr_cur_limit) {
		...
		if (65535 <= cl_mdtxpwr_cur_limit) {
			int ret = eemcs_notify_md_by_sys_msg(MD_SYS5, EXT_MD_TX_PWR_REDU_REQ, 30);	/* TODO: 30db as unlimit... */
...
static int mtk_cooler_mdtxpwr_register_ltf(void)
{
	...
	for (i = MAX_NUM_INSTANCE_MTK_COOLER_MDTXPWR; i-- > 0;) {
		...
		cl_mdtxpwr_dev[i] = mtk_thermal_cooling_device_register(temp, (void *)&cl_mdtxpwr_state[i],	/* put mdtxpwr state to cooler devdata */
									&mtk_cl_mdtxpwr_ops);
	}
