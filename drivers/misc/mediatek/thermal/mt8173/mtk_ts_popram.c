...
static int mtktspopram_debug_log;
...
static int mtktspopram_get_temp(struct thermal_zone_device *thermal, unsigned long *t)
{
#if MTK_TS_POPRAM_SW_FILTER == 1
	int curr_temp;
	int temp_temp;
	int ret = 0;
	static int last_popram_read_temp;
	...
	if (curr_temp != 0) {	/* not the first temp read after resume from suspension */
		if ((curr_temp > 150000) || (curr_temp < -20000)) {	/* invalid range */
			...
			if ((curr_temp - last_popram_read_temp > 20000) || (last_popram_read_temp - curr_temp > 20000)) {	/* delta 20C, invalid change */
				printk
				    ("[Power/POPRAM_Thermal] POPRAM temp float hugely temp=%d, lasttemp=%d\n",
...
static int mtktspopram_get_mode(struct thermal_zone_device *thermal, enum thermal_device_mode *mode)
{
	*mode = (kernelmode) ? THERMAL_DEVICE_ENABLED : THERMAL_DEVICE_DISABLED;
	return 0;
}

static int mtktspopram_set_mode(struct thermal_zone_device *thermal, enum thermal_device_mode mode)
{
	kernelmode = mode;
	return 0;
}
...
#if 0
static int tspopram_sysrst_get_max_state(struct thermal_cooling_device *cdev, unsigned long *state)
{
	*state = 1;
	return 0;
}

static int tspopram_sysrst_get_cur_state(struct thermal_cooling_device *cdev, unsigned long *state)
{
	*state = cl_dev_sysrst_state;
	return 0;
}

static int tspopram_sysrst_set_cur_state(struct thermal_cooling_device *cdev, unsigned long state)
{
...
static int mtktspopram_register_thermal(void);
static void mtktspopram_unregister_thermal(void);

static ssize_t mtktspopram_write(struct file *file, const char __user *buffer, size_t count,
				 loff_t *data)
{
	...
	if (sscanf
		...
		mtktspopram_dprintk
		    ("[mtktspopram_write] g_THERMAL_TRIP_0=%d,g_THERMAL_TRIP_1=%d,g_THERMAL_TRIP_2=%d,g_THERMAL_TRIP_3=%d,g_THERMAL_TRIP_4=%d,\
g_THERMAL_TRIP_5=%d,g_THERMAL_TRIP_6=%d,g_THERMAL_TRIP_7=%d,g_THERMAL_TRIP_8=%d,g_THERMAL_TRIP_9=%d,\n",
...
static int __init mtktspopram_init(void)
{
	...
	if (!mtktspopram_dir) {
		...
		entry =
		    proc_create("mtktspopram", S_IRUGO | S_IWUSR | S_IWGRP, mtktspopram_dir,
				&mtktspopram_fops);
		if (entry) {
