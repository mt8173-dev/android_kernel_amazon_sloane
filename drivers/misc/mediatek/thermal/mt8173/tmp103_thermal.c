...
static int tmp103_thermal_get_temp(struct thermal_zone_device *thermal, unsigned long *t)
{
...
static int temp_show(struct device *dev, struct device_attribute *devattr, char *buf)
{
...
static int config_show(struct device *dev, struct device_attribute *devattr, char *buf)
{
	int i = 0;
	int ret = 0;
	int d, d0, d1, d2;
	...
	for (i = 0; i < NUM_SENSORS; i++) {
		...
		d1 = (d < 10) ? 0 : d / 10;
		d2 = (d % 10);
		ret += sprintf(pbuf + ret, "%d.%d%d%d ", vs_data[i].offset / DMF, d0, d1, d2);
	}
	...
	for (i = 0; i < NUM_SENSORS; i++) {
		...
		d1 = (d < 10) ? 0 : d / 10;
		d2 = (d % 10);
		ret += sprintf(pbuf + ret, "%d.%d%d%d ", vs_data[i].alpha / DMF, d0, d1, d2);
	}
	ret += sprintf(pbuf + ret, "\nweights ");
	for (i = 0; i < NUM_SENSORS; i++) {
		...
		d1 = (d < 10) ? 0 : d / 10;
		d2 = (d % 10);
		ret += sprintf(pbuf + ret, "%d.%d%d%d ", vs_data[i].weight / DMF, d0, d1, d2);
	}

	return sprintf(buf, "%s\n", pbuf);
}

static int thermal_zone_show(struct device *dev, struct device_attribute *devattr, char *buf)
{
	int len = 0;
	char pbuf[BUF_SIZE];
	int i;
	...
	for (i = 0; i < tzone->num_trips; i++)
		len += sprintf((pbuf + len), "temp=%d type=%d cdev=%s ",
			       (int)tzone->trip[i].temp, tzone->trip[i].type, tzone->trip[i].name);
...
static ssize_t config_set(struct device *dev,
			  struct device_attribute *devattr, const char *pbuf, size_t count)
{
	int ret, index;
	int i = 0;
	int d[3];
				...
				struct device_attribute *devattr, const char *pbuf, size_t count)
{
	...
	if (sscanf(pbuf, "%d%n", &(tzone->num_trips), &off) == 1) {
		pbuf += off;
		for (i = 0; i < tzone->num_trips; i++) {
			if (sscanf(pbuf, "%d %d %19s%n", &temp, &type, name, &off) != 3) {
...
static DEVICE_ATTR(thermal_zone, S_IWUSR | S_IRUGO, thermal_zone_show, thermal_zone_set);
...
static int tmp103_thermal_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < NUM_COOLERS; i++) {
		...
		strncpy(tzone->trip[i].name, coolers[i], THERMAL_NAME_LENGTH);
	}
	tzone->tdev = thermal_zone_device_register(THERMAL_NAME,
						   NUM_COOLERS,
						   0, tzone, &tmp103_tz_dev_ops, NULL, 0, 0);
...
module_init(tmp103_thermal_init);
