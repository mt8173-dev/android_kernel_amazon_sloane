...
static int mtkts_AP_read(struct seq_file *m, void *v)
{
	...
	seq_printf(m,
		   "[mtkts_AP_read] trip_0_temp=%d,trip_1_temp=%d,trip_2_temp=%d,trip_3_temp=%d,trip_4_temp=%d,\n\
			      ...
			      loff_t *data)
{
	...
	if (sscanf
		...
		mtkts_AP_dprintk
		    ("[mtkts_AP_write] g_THERMAL_TRIP_0=%d,g_THERMAL_TRIP_1=%d,g_THERMAL_TRIP_2=%d,g_THERMAL_TRIP_3=%d,g_THERMAL_TRIP_4=%d,\
					g_THERMAL_TRIP_5=%d,g_THERMAL_TRIP_6=%d,g_THERMAL_TRIP_7=%d,g_THERMAL_TRIP_8=%d,g_THERMAL_TRIP_9=%d,\n",
		     ...
		     g_THERMAL_TRIP[9]);
		mtkts_AP_dprintk
		    ("[mtkts_AP_write] cooldev0=%s,cooldev1=%s,cooldev2=%s,cooldev3=%s,cooldev4=%s,\
		...
		mtkts_AP_dprintk
		    ("[mtkts_AP_write] trip_0_temp=%d,trip_1_temp=%d,trip_2_temp=%d,trip_3_temp=%d,trip_4_temp=%d,\
...
static int cooler_get_max_state(struct thermal_cooling_device *cdev,
			   unsigned long *state)
{
	*state = 1;
	return 0;
...
static int cooler_get_cur_state(struct thermal_cooling_device *cdev,
			   unsigned long *state)
{
...
static int cooler_set_cur_state(struct thermal_cooling_device *cdev,
			   unsigned long state)
{
	struct tmp103_cooler_ctx *cctx = cdev->devdata;
	mtkts_AP_dprintk("[cooler_set_cur_state] %ld\n", state);

	if (!cctx) {
		mtkts_AP_dprintk("%s: NULL %s device data\n", __func__, cdev->type);
		return -EINVAL;
	}
...
int mtkts_AP_register_dvfs_cooler(void)
{
	...
	mtkts_AP_dprintk("[mtkts_AP_register_dvfs_cooler]\n");
	...
	cooler_dir = mtk_thermal_get_proc_drv_cooler_dir_entry();
	if (!cooler_dir) {
		mtkts_AP_dprintk(" mkdir /proc/mtkcooler failed\n");
		return -EINVAL;
	}
	entry = proc_create("tmp103-cooler", S_IRUGO | S_IWUSR | S_IWGRP, cooler_dir,
				&cooler_fops);
	if (entry) {
...
static int __init mtkts_AP_init(void)
{
	...
	if (!mtkts_AP_dir) {
		mtkts_AP_dprintk("[mtkts_AP_init]: mkdir /proc/driver/thermal failed\n");
	} else {

		entry =
		    proc_create("tzbts", S_IRUGO | S_IWUSR | S_IWGRP, mtkts_AP_dir,
				&mtkts_AP_fops);
		if (entry) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0)
			proc_set_user(entry, 0, 1000);
