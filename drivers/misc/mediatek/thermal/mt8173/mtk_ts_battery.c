...
/* static ssize_t mtktsbattery_write(struct file *file, const char *buffer, unsigned long count, void *data) */
{
	...
	if (sscanf
		...
		mtktsbattery_dprintk
		    ("[mtktsbattery_write] g_THERMAL_TRIP_0=%d,g_THERMAL_TRIP_1=%d,g_THERMAL_TRIP_2=%d,g_THERMAL_TRIP_3=%d,g_THERMAL_TRIP_4=%d,\
g_THERMAL_TRIP_5=%d,g_THERMAL_TRIP_6=%d,g_THERMAL_TRIP_7=%d,g_THERMAL_TRIP_8=%d,g_THERMAL_TRIP_9=%d,\n",
...
static int __init mtktsbattery_init(void)
{
	...
	if (!mtktsbattery_dir) {
		mtktsbattery_dprintk("[mtktsbattery_init]: mkdir /proc/driver/thermal failed\n");
	} else {
#if 1
		entry =
		    proc_create("tzbattery", S_IRUGO | S_IWUSR, mtktsbattery_dir,
				&mtkts_battery_fops);
		if (entry)
