...
static int cl_mdulthro_klog_on;
#if 0
	...
	if (mail_box[0] >= 0 && mail_box[1] >= 0) {
		...
		case HIGH_STAT:
			if (throttling_pre_stat < HIGH_STAT || throttling_pre_stat == WFD_STAT) {
				...
				mtk_cooler_mdulthro_dprintk_always
				    ("LOW/MID-->HIGH:%lu <- (%d / %d) %lu", thro_constraint,
				...
				if ((over_up_time * interval) >= up_duration) {
					...
					mtk_cooler_mdulthro_dprintk_always
					    ("HIGH-->HIGH:%lu <- (%d / %d) %lu", thro_constraint,
					     up_numerator, up_denominator, cur_thro);
		...
		case LOW_STAT:
			if (throttling_pre_stat == WFD_STAT) {
				...
				mtk_cooler_mdulthro_dprintk_always
				    ("MID/HIGH-->LOW:%lu <- (%d / %d) %lu", thro_constraint,
...
#if 0
static int mtk_cl_mdulthro_get_max_state(struct thermal_cooling_device *cdev, unsigned long *state)
{
	*state = 1;
...
static int mtk_cl_mdulthro_get_cur_state(struct thermal_cooling_device *cdev, unsigned long *state)
{
...
static int mtk_cl_mdulthro_set_cur_state(struct thermal_cooling_device *cdev, unsigned long state)
{
...
static int mtk_cooler_mdulthro_register_ltf(void)
{
	...
	for (i = MAX_NUM_INSTANCE_MTK_COOLER_MDULTHRO; i-- > 0;) {
		...
		cl_mdulthro_dev[i] = mtk_thermal_cooling_device_register(temp, (void *)&cl_mdulthro_state[i],	/* put mdulthro state to cooler devdata */
									 &mtk_cl_mdulthro_ops);
	}
#endif
...
#if 0
static int _mtk_cl_mdulthro_proc_read(char *buf, char **start, off_t off, int count, int *eof,
				      void *data)
{
	int len = 0;
	char *p = buf;
...
static ssize_t _mtk_cl_mdulthro_proc_write(struct file *file, const char *buffer,
					   unsigned long count, void *data)
{
	...
	if (1 <= sscanf(desc, "%d %d %d %d", &klog_on, &limit0, &limit1, &limit2)) {
		...
		if (limit0 >= -1)
			MTK_CL_MDULTHRO_SET_LIMIT(limit0, cl_mdulthro_state[0]);
		if (limit1 >= -1)
			MTK_CL_MDULTHRO_SET_LIMIT(limit1, cl_mdulthro_state[1]);
		if (limit2 >= -1)
			MTK_CL_MDULTHRO_SET_LIMIT(limit2, cl_mdulthro_state[2]);
...
int mdulthro_param_read(char *buf, char **start, off_t offset, int count, int *eof, void *data)
{
	...
	sprintf(tmp, "[up]\t%3d(sec)\t%2d/%2d\n[low]\t%3d(sec)\t%2d/%2d\nrst=%2d\ninterval=%d\n",
		...
		low_denominator, low_rst_max, polling_interval);
	ret = strlen(tmp);

	memcpy(buf, tmp, ret * sizeof(char));

	mtk_cooler_mdulthro_dprintk_always
	    ("[%s] [up]%d %d/%d, [low]%d %d/%d, rst=%d, interval=%d\n", __func__, up_duration,
...
ssize_t mdulthro_param_write(struct file *filp, const char __user *buf, unsigned long len,
			     void *data)
{
	char desc[MAX_LEN] = { 0 };
	...
	if (sscanf
	    (desc, "%d %d/%d %d %d/%d %d %d", &tmp_up_dur, &tmp_up_num, &tmp_up_den, &tmp_low_dur,
		...
		mtk_cooler_mdulthro_dprintk_always
		    ("[%s] %s [up]%d %d/%d, [low]%d %d/%d, rst=%d, interval\n", __func__, desc,
		     up_duration, up_numerator, up_denominator, low_duration, low_numerator,
		     low_denominator, low_rst_max, polling_interval);
...
ssize_t mdulthro_pid_write(struct file *filp, const char __user *buf, unsigned long len,
			   void *data)
{
	int ret = 0;
	char tmp[MAX_LEN] = { 0 };
...
static int mdulthro_proc_register(void)
{
	...
	if (!mdulthro_proc_dir) {
...
#if 0
		entry =
		    create_proc_entry("mdulthro", S_IRUGO | S_IWUSR | S_IWGRP, mdulthro_proc_dir);
		if (NULL != entry) {
		...
		entry =
		    create_proc_entry("mdulthro_param", S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP,
				      mdulthro_proc_dir);
		if (entry) {
