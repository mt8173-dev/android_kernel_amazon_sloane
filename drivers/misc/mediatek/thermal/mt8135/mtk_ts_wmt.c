...
ssize_t wmt_wifi_tx_thro_read(struct file *file, char *buf, size_t count, loff_t *data)
{
...
/*New Wifi throttling Algo+*/
ssize_t wmt_wifi_algo_write(struct file *file, const char *buf, size_t len, loff_t *data)
{
	char desc[MAX_LEN] = { 0 };
...
ssize_t wmt_wifi_algo_read(struct file *file, char *buf, size_t count, loff_t *data)
{
	static unsigned char con;
	char *p = buf;
	...
	if (con) {
		...
		wmt_tm_printk("[%s] [up]%d %d/%d, [low]%d %d/%d, rst=%d\n", __func__, up_duration,
			      up_numerator, up_denominator, low_duration, low_numerator,
			      low_denominator, low_rst_max);
	}
...
ssize_t wmt_tm_wfd_write(struct file *file, const char *buf, size_t len, loff_t *data)
{
	int ret = 0;
	char tmp[MAX_LEN] = { 0 };
...
ssize_t wmt_tm_wfd_read(struct file *file, char *buf, size_t count, loff_t *data)
{
	static unsigned char con;
	char *p = buf;
	...
	if (con) {
		...
		wmt_tm_printk("[%s] %s = %d\n", __func__, buf, tm_wfd_stat);
	}

	return p - buf;
}

ssize_t wmt_tm_pid_write(struct file *file, const char *buf, size_t len, loff_t *data)
{
	int ret = 0;
	char tmp[MAX_LEN] = { 0 };
...
ssize_t wmt_tm_pid_read(struct file *file, char *buf, size_t count, loff_t *data)
{
	static unsigned char con;
	char *p = buf;
...
static ssize_t wmt_tm_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	...
	if (sscanf
		...
		if (g_num_trip < 1 || g_num_trip > 10) {
			wmt_tm_dprintk("[wmt_tm_write] bad argument: g_num_trip=%d\n", g_num_trip);
			return -EINVAL;
		}
...
static int wmt_tm_proc_register(void)
{
	...
	if (!wmt_tm_proc_dir) {
		...
		entry =
		    proc_create("wmt_tm", S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP, wmt_tm_proc_dir,
				&wmt_tm_fops);
		if (!entry) {
			wmt_tm_dprintk
			    ("[wmt_tm_proc_register]: create /proc/wmt_tm/wmt_tm failed\n");
		}

		entry =
		    proc_create("tm_pid", S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP, wmt_tm_proc_dir,
				&wmt_tm_pid_fops);
		if (!entry) {
			wmt_tm_dprintk
			    ("[wmt_tm_proc_register]: create /proc/wmt_tm/tm_pid failed\n");
		}

		entry =
		    proc_create("wmt_val", S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP, wmt_tm_proc_dir,
				&wmt_wifi_algo_fops);
		if (!entry) {
			wmt_tm_dprintk
			    ("[wmt_tm_proc_register]: create /proc/wmt_tm/wmt_val failed\n");
		}

		entry =
		    proc_create("tx_thro", S_IRUGO | S_IWUSR, wmt_tm_proc_dir,
				&wmt_wifi_tx_thro_fops);
		if (!entry) {
			wmt_tm_dprintk
			    ("[wmt_tm_proc_register]: create /proc/wmt_tm/tx_thro failed\n");
		}

		entry =
		    proc_create("wfd_stat", S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP, wmt_tm_proc_dir,
				&wmt_tm_wfd_fops);
		if (!entry) {
			wmt_tm_dprintk
			    ("[wmt_tm_proc_register]: create /proc/wmt_tm/wfd_stat failed\n");
		}
	}
	return 0;
}
...
static void __exit wmt_tm_deinit(void)
{
	...
	del_timer_sync(&wmt_stats_timer);
