...
static int sysrst_set_cur_state(struct thermal_cooling_device *cdev, unsigned long state)
{
	...
	if (cl_dev_sysrst_state == 1) {
		pr_info("Power/PA_Thermal: reset, reset, reset!!!");
		pr_info("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
		pr_info("*****************************************");
		pr_info("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
...
static ssize_t mtktspa_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	...
	if (sscanf
		...
		mtktspa_dprintk
		    ("[mtktspa_write] g_THERMAL_TRIP_0=%d,g_THERMAL_TRIP_1=%d,g_THERMAL_TRIP_2=%d,g_THERMAL_TRIP_3=%d,g_THERMAL_TRIP_4=%d,\
g_THERMAL_TRIP_5=%d,g_THERMAL_TRIP_6=%d,g_THERMAL_TRIP_7=%d,g_THERMAL_TRIP_8=%d,g_THERMAL_TRIP_9=%d,\n",
