...
static void pmic_cali_prepare(void)
{
	...
	pr_info
	    ("Power/PMIC_Thermal: g_adc_ge = 0x%x, g_adc_oe = 0x%x, g_o_vts = 0x%x, g_degc_cali = 0x%x, g_adc_cali_en = 0x%x, g_o_slope = 0x%x, g_o_slope_sign = 0x%x, g_id = 0x%x\n",
	     g_adc_ge, g_adc_oe, g_o_vts, g_degc_cali, g_adc_cali_en, g_o_slope, g_o_slope_sign,
	     g_id);
...
static ssize_t mtktspmic_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	...
	if (sscanf
		...
		mtktspmic_dprintk
		    ("[mtktspmic_write] g_THERMAL_TRIP_0=%d,g_THERMAL_TRIP_1=%d,g_THERMAL_TRIP_2=%d,g_THERMAL_TRIP_3=%d,g_THERMAL_TRIP_4=%d,\
g_THERMAL_TRIP_5=%d,g_THERMAL_TRIP_6=%d,g_THERMAL_TRIP_7=%d,g_THERMAL_TRIP_8=%d,g_THERMAL_TRIP_9=%d,\n",
