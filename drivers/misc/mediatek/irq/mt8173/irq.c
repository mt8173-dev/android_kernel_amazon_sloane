...
void __iomem *gic_cpubase;
void __iomem *ca9base;
void __iomem *mcucfgbase;
...
void mt_irq_set_pending_for_sleep(unsigned int irq)
{
	...
	*/
	writel(mask, (void *)(GIC_DIST_BASE + GIC_DIST_PENDING_SET + irq / 32 * 4));
	dsb();
}
...
static int __init mt_init_irq(void)
{
	struct irq_chip *gic_chip;
	...
	if (node) {
		ca9base = of_iomap(node, 0);
		gic_cpubase = of_iomap(node, 1);
		mcucfgbase = of_iomap(node, 2);
	} else
		pr_err("cannot find_arm,cortex-a15-gic _node\n");
#endif
