...
static u32 tz_service_call(u32 handle, u32 op, u32 arg1, unsigned long arg2)
{
	...
	register u32 r0 asm("x0") = 0x32000008; /* trusted os call function id for mtee */
...
{
	return (TZ_RESULT) tz_service_call(handle, command, paramTypes, (unsigned long) param);
}
...
error:
	...
	for (i = 0; tmpTypes; i++) {
		...
		case TZPT_MEM_INOUT:
			...
			if (param[i].mem.buffer && (param[i].mem.buffer != oparam[i].mem.buffer)) {
