...
if (file) {
	FILE_Read(file, (void *) &secure_time, sizeof(struct TZ_JG_SECURECLOCK_INFO), &u8Offset);
...
if (file) {
	FILE_Write(file, (void *) &secure_time, sizeof(struct TZ_JG_SECURECLOCK_INFO), &u8Offset);
	filp_close(file, NULL);
} else {
