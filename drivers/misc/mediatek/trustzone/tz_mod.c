...
static long _map_user_pages(struct MTIOMMU_PIN_RANGE_T *pinRange, unsigned long uaddr, uint32_t size,
			    uint32_t control)
{
	int nr_pages;
...
static long tz_client_reg_sharedmem(struct file *file, unsigned long arg)
{
	...
	/* register it ...
	 */
	ret = kree_register_sharedmem(session, &mem_handle, pin->start, pin->size,
				      (void *)map_p);
	if (ret != TZ_RESULT_SUCCESS) {
