				  ...
				  void *start, uint32_t size, void *map_p)
{
	...
	ret = KREE_TeeServiceCall(session, TZCMD_MEM_SHAREDMEM_REG,
				  TZ_ParamTypes4(TZPT_VALUE_INPUT, TZPT_VALUE_INPUT,
						 TZPT_MEM_INPUT, TZPT_VALUE_OUTPUT), p);
	if (ret != TZ_RESULT_SUCCESS) {
		*mem_handle = 0;
		return ret;
	}
				 ...
				 KREE_SHAREDMEM_HANDLE *shm_handle, KREE_SHAREDMEM_PARAM *param)
{
	...
	if ((param->buffer >= (void *)PAGE_OFFSET) &&
		...
		ret = kree_register_sharedmem(session, shm_handle,
						param->buffer, param->size, 0); /* set 0 for no remap... */
		if (ret != TZ_RESULT_SUCCESS) {
#ifdef DBG_KREE_MEM
...
#ifdef DBG_KREE_MEM
	/* if count is increasing spuriously which it should not, this check and print
...
TZ_RESULT KREE_StartSecurechunkmemSvc(KREE_SESSION_HANDLE session, unsigned long start_pa, uint32_t size)
{
	MTEEC_PARAM p[4];
	TZ_RESULT ret;
...
TZ_RESULT KREE_StopSecurechunkmemSvc(KREE_SESSION_HANDLE session, unsigned long *cm_pa, uint32_t *size)
{
	MTEEC_PARAM p[4];
	TZ_RESULT ret;
...
TZ_RESULT KREE_QuerySecurechunkmem(KREE_SESSION_HANDLE session, unsigned long *cm_pa, uint32_t *size)
{
	MTEEC_PARAM p[4];
	TZ_RESULT ret;
