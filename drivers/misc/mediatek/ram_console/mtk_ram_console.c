...
/* emmc based ram console doesn't work */
#if 0
...
static int ram_console_lastk_show(struct ram_console_buffer *buffer, struct seq_file *m, void *v)
{
	...
	if (buffer->off_console != 0
	    && buffer->off_linux + ALIGN(sizeof(struct last_reboot_reason), 64) == buffer->off_console
...
static int __init ram_console_init(struct ram_console_buffer *buffer, size_t buffer_size)
{
	...
	if (likely(buffer->sig == REBOOT_REASON_SIG)) {
		/* Save old log for a warm reboot */
		if (!ram_console_check_header(buffer) && (0 != buffer->sz_buffer))
			ram_console_save_old(buffer, buffer_size);
		else
			pr_err("ram_console: corrupted kernel header. Usually mean a cold boot.\n");
	} else {
		/* this routine should rarely be entered due to signature is initialized in preloader */
		pr_err("ram_console: signature is unexpectedly changed, reset buffer and signature!\n");
	...
	pr_err("ram_console: init: off_linux(0x%p):0x%x, off_console(0x%p):0x%x, sz_console(0x%p):0x%x\n",
		&(buffer->off_linux), buffer->off_linux,
		&(buffer->off_console), buffer->off_console,
		&(buffer->sz_console), buffer->sz_console);
