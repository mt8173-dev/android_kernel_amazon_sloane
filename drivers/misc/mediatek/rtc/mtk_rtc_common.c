...
static DEFINE_MUTEX(rtc_lock1);
#define rtc_get_lock() mutex_lock(&rtc_lock1)
#define rtc_put_lock()  mutex_unlock(&rtc_lock1)

static unsigned long rtc_flags = 0L;
void rtc_acquire_lock(void)
{
	rtc_get_lock();
	spin_lock_irqsave(&rtc_lock, rtc_flags);
}
EXPORT_SYMBOL(rtc_acquire_lock);

void rtc_release_lock(void)
{
	spin_unlock_irqrestore(&rtc_lock, rtc_flags);
	rtc_put_lock();
}
