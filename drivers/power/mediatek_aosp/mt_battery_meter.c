...
#include <linux/power/mt_battery_meter_hal.h>
#include <linux/power/mt_battery_meter_custom_wrap.h>
#include <linux/power/mt_default_battery_data.h>
/* ============================================================ // */
/* define */
/* ============================================================ // */
...
kal_bool gFG_Is_Init = KAL_FALSE;
...
unsigned long g_rtc_time_before_sleep = 0;
...
/* EM */
kal_int32 g_fg_dbg_bat_hwocv = 0;
...
kal_int32 get_battery_aging_factor(kal_int32 cycle)
{
	...
	kal_int32 saddles;
	kal_int32 factor;
	if (p_bat_meter_data && battery_aging_table)
...
#if defined(CONFIG_SOC_BY_HW_FG)
	/* decrease rtc soc by 1 if swocv is less by threshold 5% */
	if (g_rtc_fg_soc > 1 && g_rtc_fg_soc >= gFG_capacity_by_v_init + 5) {
		g_rtc_fg_soc -= 1;
...
kal_int32 battery_meter_get_battery_voltage(void)
{
	...
	if (battery_meter_ctrl == NULL || gFG_Is_Init == KAL_FALSE)
		return 0;
...
kal_int32 battery_meter_get_batteryR(void)
{
#if defined(CONFIG_POWER_EXT)
	return 0;
#else
	return gFG_resistance_bat + R_FG_VALUE + FG_METER_RESISTANCE;
#endif
}
...
static int battery_meter_probe(struct platform_device *dev)
{
	...
	if (!p_bat_meter_data) {
		p_bat_meter_data = &default_bat_meter_data;
		pr_info("no platform data. init by default setting.\n");
...
static void battery_aging_check(void)
{
	...
	unsigned long time_after_sleep;
	kal_int32 vbat;
	kal_int32 DOD_hwocv;
	kal_int32 DOD_now;
