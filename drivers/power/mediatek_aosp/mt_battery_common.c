...
#include <linux/power/mt_battery_charging_custom_wrap.h>
...
static mt_battery_charging_custom_data default_charging_data = {
...
#if defined(CONFIG_AMAZON_METRICS_LOG) && !defined(CONFIG_POWER_EXT)
static void battery_critical_voltage_check(void)
{
	...
	if (BMT_status.bat_vol <= SYSTEM_OFF_VOLTAGE) {
		char buf[128];
		snprintf(buf, sizeof(buf), "bq24297:def:critical_shutdown=1;CT;1:HI");
...
#if defined(CONFIG_EARLYSUSPEND)
static void bq_log_metrics(struct battery_info *info, char *msg, char *metricsmsg)
{
...
static void battery_metrics_locked(struct battery_info *info)
{
	...
	if (info->suspend_capacity != -1) {
		...
		snprintf(buf, sizeof(buf), "batt:def:cap=%d;CT;1,mv=%d;CT;1,current_avg=%d;CT;1," "temp_g=%d;CT;1,charge=%d;CT;1,charge_design=%d;CT;1:NR", BMT_status.UI_SOC, BMT_status.bat_vol, BMT_status.ICharging, BMT_status.temperature, gFG_BATT_CAPACITY_aging,	/*battery_remaining_charge,? */
...
static void mt_battery_Sync_UI_Percentage_to_Real(void)
{
	...
	if ((BMT_status.UI_SOC > BMT_status.SOC) && ((BMT_status.UI_SOC != 1))) {
		/* reduce after xxs */
		if (g_refresh_ui_soc
		    || timer_counter == (SYNC_TO_REAL_TRACKING_TIME / BAT_TASK_PERIOD)) {
			BMT_status.UI_SOC--;
			timer_counter = 0;
...
static void do_chrdet_int_task(void)
{
	...
	if (g_bat_init_flag == KAL_TRUE) {
		if (upmu_is_chr_det() == KAL_TRUE) {
...
#ifdef CONFIG_AMAZON_METRICS_LOG
			snprintf(buf, sizeof(buf), "%s:bq24297:vbus_on=1;CT;1:NR", __func__);
			...
			snprintf(buf, sizeof(buf), "%s:bq24297:vbus_off=1;CT;1;DV;1:NR", __func__);
			log_to_metrics(ANDROID_LOG_INFO, "USBCableEvent", buf);
#endif
...
static int slimport_charging_notifier_callback(struct notifier_block *nb, unsigned long val,
					       void *data)
{
	mutex_lock(&charger_type_mutex);
	BMT_status.charger_type = bat_charger_get_charger_type();
...
static int battery_probe(struct platform_device *pdev)
{
	...
	if (!p_bat_charging_data) {
		pr_err("%s: platform data is missing. replace with default settings.\n", __func__);
