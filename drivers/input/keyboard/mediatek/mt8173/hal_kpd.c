...
#if KPD_PWRKEY_USE_EINT
static u8 kpd_pwrkey_state;
static u8 kpd_pwrkey_ori_polarity;
static unsigned int kpd_pwrkey_irq;
#endif

#if KPD_HAS_SLIDE_QWERTY
static u8 kpd_slide_irq;
#endif
...
void kpd_slide_qwerty_init(void)
{
	...
	if (node) {
		...
		of_property_read_u32_array(node, "debounce", debouncetime, ARRAY_SIZE(debouncetime));
		mt_eint_set_hw_debounce(debouncetime[0], debouncetime[1]);

		of_property_read_u32_array(node, "interrupts", intflag, ARRAY_SIZE(intflag));

		ret = request_irq(kpd_slide_irq, kpd_slide_eint_handler, intflag[1],
				"KPD_SLIDE-eint", NULL);
		if (ret > 0)
			printk(KPD_SAY "failed to request KPD_SLIDE-eint irq!\n");
	} else {
		printk(KPD_SAY "failed to find kpd slide eint device node!\n");
		return;
	}
...
{
	int i = 0;
	memcpy(kpd_keymap, mtk_kpd_hw.kpd_init_keymap, sizeof(kpd_keymap)/sizeof(kpd_keymap[0]));
...
void kpd_pwrkey_handler_hal(unsigned long data)
{
	...
	pressed = (kpd_pwrkey_state == !!kpd_pwrkey_ori_polarity);
	if (kpd_show_hw_keycode) {
	...
	irq_set_irq_type(kpd_pwrkey_irq, (old_state & 0xe));
	enable_irq(kpd_pwrkey_irq);
#endif
}
...
void mt_eint_register(void)
{
	...
	if (node) {
		...
		kpd_pwrkey_irq = irq_of_parse_and_map(node, 0);
		...
		if (intflag[1] & (IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING)) {
			kpd_pwrkey_ori_polarity = (intflag[1] & IRQF_TRIGGER_RISING) ? 0 : 1;
		} else if (intflag[1] & (IRQF_TRIGGER_HIGH | IRQF_TRIGGER_LOW)) {
			kpd_pwrkey_ori_polarity = (intflag[1] & IRQF_TRIGGER_LOW) ? 0 : 1;
		} else {
			printk(KPD_SAY "[EINT]: Wrong EINT Pol/Sens Setting 0x%x\n", intflag[1]);
			return;
		}

		kpd_pwrkey_state = !kpd_pwrkey_ori_polarity;

		ret = request_irq(kpd_pwrkey_irq, kpd_pwrkey_eint_handler, intflag[1],
				"KPD_PWRKEY-eint", NULL);

		if (ret > 0)
			printk(KPD_SAY "failed to request KPD_PWRKEY-eint!\n");
	} else {
		printk(KPD_SAY "failed to find kpd pwrkey eint device node!\n");
		return;
	}
