...
/* Timeout for workdog work in ms  */
#define SYNA_WATCHDOG_TIMEOUT 5000
...
static ssize_t synaptics_esd_reset_show(struct device *dev,
		struct device_attribute *attr, char *buf);
...
static struct device_attribute attrs[] = {
	...
	__ATTR(esd_reset, S_IRUSR,
			synaptics_esd_reset_show, NULL),
};
...
static ssize_t synaptics_esd_reset_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "f01_err_rst=%d, watchdog_rst=%d\n",
			f01_err_rst, watchdog_reset);
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	retval = synaptics_rmi4_reset_device_pm(rmi4_data);
	if (retval < 0) {
		dev_err(dev,
...
static void synaptics_reset_work(struct work_struct *work)
{
	struct synaptics_rmi4_data *rmi4_data =
	...
	if (!pdata->rst_power) {
		dev_err(&rmi4_data->input_dev->dev,
			"%s: rst_power is NULL in board file\n",
			__func__);
		return;
	}
	...
	dev_info(&rmi4_data->input_dev->dev,
		"%s: Reset Power\n", __func__);
	pdata->rst_power(pdata->reset_gpio);
	return;
}

static void synaptics_rmi4_f01_report(struct synaptics_rmi4_data *rmi4_data,
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
			"%s: Failed to read device status, retval=%d\n",
			__func__, retval);
		return;
	}

	error_code = device_status&0x0f;
	dev_dbg(&rmi4_data->i2c_client->dev,
				"%s: device status : 0x%x, error code: 0x%x\n",
				__func__, device_status, error_code);

	if (error_code == 0x03) {
		f01_err_rst++;
		dev_err(&rmi4_data->i2c_client->dev,
			"%s: Will power cycle touch controller!\n",
			__func__);
		schedule_work(&rmi4_data->reset_work);
	}
		...
		unsigned char *touch_count)
{
	...
	case SYNAPTICS_RMI4_F01:
		synaptics_rmi4_f01_report(rmi4_data, fhandler);
		break;
	default:
		break;
	}
...
static void synaptics_start_wd_work(struct synaptics_rmi4_data *rmi4_data)
{
	queue_delayed_work(rmi4_data->wd_workqueue,
			&rmi4_data->wd_work,
			msecs_to_jiffies(SYNA_WATCHDOG_TIMEOUT));
}

static void synaptics_watchdog_work(struct work_struct *work)
{
	struct synaptics_rmi4_data *rmi4_data =
			container_of(work, struct synaptics_rmi4_data,
			wd_work.work);
	...
	dev_dbg(&rmi4_data->input_dev->dev,
			"%s: Watchdog Work Triggered\n", __func__);
	if (rmi4_data == NULL) {
		pr_err("%s: NULL context pointer\n", __func__);
		return;
	}

	retval = synaptics_rmi4_i2c_read(rmi4_data, SYNAPTICS_RMI4_ID,
			&data, sizeof(data));
	if (retval >= 0)
		dev_dbg(&rmi4_data->i2c_client->dev,
			"data=0x%x\n", data);

	if (unlikely(retval < 0 || data == 0x0)) {
		dev_err(&rmi4_data->i2c_client->dev,
			"%s: Failed read ID for watchdog, reset touch\n",
				__func__);
		if (pdata->rst_power) {
			watchdog_reset++;
			pdata->rst_power(pdata->reset_gpio);
	...
	synaptics_start_wd_work(rmi4_data);
}
...
static int synaptics_rmi4_reset_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	cancel_delayed_work_sync(&rmi4_data->wd_work);
...
*/
	if (platform_data->init_power) {
		retval = platform_data->init_power(platform_data, 1, &client->dev);
	}
	...
	INIT_WORK(&rmi4_data->reset_work, synaptics_reset_work);
	rmi4_data->wd_workqueue =
			create_singlethread_workqueue("rmi_wd_workqueue");
	INIT_DELAYED_WORK(&rmi4_data->wd_work,
			synaptics_watchdog_work);
	...
	rmi4_data->irq = platform_data->gpio;

	gpio_direction_input(rmi4_data->irq);
...
err_query_device:
	...
	if (platform_data->power) {
		retval = platform_data->power(platform_data, 0);
	}
...
static int synaptics_rmi4_remove(struct i2c_client *client)
{
	...
	cancel_delayed_work_sync(&rmi4_data->wd_work);
	flush_workqueue(rmi4_data->wd_workqueue);
	destroy_workqueue(rmi4_data->wd_workqueue);
	...
	if (platform_data->power) {
		platform_data->power(platform_data, 0);
	}
...
static int synaptics_rmi4_suspend(struct device *dev)
{
	...
	if (rmi4_data) {
		platform_data =
		(struct synaptics_rmi4_platform_data *)rmi4_data->board;

		if (!rmi4_data && !rmi4_data->sensor_sleep) {
			rmi4_data->touch_stopped = true;
			wake_up(&rmi4_data->wait);
			synaptics_rmi4_irq_enable(rmi4_data, false);
			synaptics_rmi4_sensor_sleep(rmi4_data);
		...
		if (platform_data && platform_data->power) {
			retval = platform_data->power(platform_data,
					false);
		}
	}
