...
static int tpd_power_on(struct i2c_client *client)
{
 ...
 reset_proc:
	...
	core_reg = devm_regulator_get(&client->dev, "reg-vgp6");
	ret = regulator_set_voltage(core_reg, 3300000, 3300000);
	if (ret != 0) {
...
static int tpd_irq_registration(void)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mtk-tpd,gt9xx");
	if (node) {
		...
		{
			ret =
			    request_irq(touch_irq, tpd_interrupt_handler, EINTF_TRIGGER_RISING,
					TPD_DEVICE, NULL);
			...
			ret =
			    request_irq(touch_irq, tpd_interrupt_handler,
					EINTF_TRIGGER_FALLING, TPD_DEVICE, NULL);
