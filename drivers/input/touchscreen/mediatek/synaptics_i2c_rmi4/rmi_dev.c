...
static ssize_t rmidev_sysfs_data_show(struct device *dev, struct device_attribute *attr, char *buf);

static ssize_t rmidev_sysfs_data_store(struct device *dev,
				       struct device_attribute *attr, const char *buf,
				       size_t count);

static ssize_t rmidev_sysfs_attn_show(struct device *dev, struct device_attribute *attr, char *buf);
					  ...
					  size_t count)
{
	...
	dev_dbg(&rmidev->rmi4_data->i2c_client->dev, "%s: Attention interrupt enabled\n", __func__);
...
static ssize_t rmidev_sysfs_attn_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int attn = gpio_get_value(tpd_int_gpio_number);
			    ...
			    size_t count, loff_t *f_pos)
{
	...
	retval = rmidev->fn_ptr->write(rmidev->rmi4_data, *f_pos, tmpbuf, count);
	if (retval >= 0)
		*f_pos += retval;
...
static int rmidev_init_device(struct i2c_client *client)
{
	...
	if (!rmidev) {
		dev_err(&client->dev, "%s: Failed to alloc mem for rmidev\n", __func__);
		retval = -ENOMEM;
		goto err_rmidev;
	}
	...
	if (!dev_data) {
		dev_err(&client->dev, "%s: Failed to alloc mem for dev_data\n", __func__);
		retval = -ENOMEM;
		goto err_dev_data;
	}
	...
	if (retval < 0) {
		dev_err(&client->dev, "%s: Failed to add rmi char device\n", __func__);
	...
	if (IS_ERR(device_ptr)) {
		dev_err(&client->dev, "%s: Failed to create rmi char device\n", __func__);
		retval = -ENODEV;
		goto err_char_device;
	}
...
{
	synaptics_rmi4_new_function(RMI_DEV, true, rmidev_init_device, rmidev_remove_device, NULL);
	return 0;
}
...
static void __exit rmidev_module_exit(void)
{
	...
	synaptics_rmi4_new_function(RMI_DEV, false, rmidev_init_device, rmidev_remove_device, NULL);
