...
#ifdef CONFIG_OF
static int of_get_synaptics_platform_data(struct device *dev)
{
	/*int ret, num;*/
...
static int tpd_irq_registration(void)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mtk-tpd,synaptics");
...
static int tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	TPD_DMESG("%s:enter\n", __func__);
	of_get_synaptics_platform_data(&client->dev);
 ...
 TPD_RESET_PROBE:
#ifndef USE_REGULATOR_FRAMEWORK
	hwPowerOn(MT65XX_POWER_LDO_VGP6, VOL_3300, "TP");
	hwPowerOn(MT65XX_POWER_LDO_VGP4, VOL_1800, "TP");
	msleep(10);
	/*mt_set_gpio_mode(tpd_rst_gpio_number, GPIO_CTP_RST_PIN_M_GPIO);
	mt_set_gpio_dir(tpd_rst_gpio_number, GPIO_DIR_OUT);*/
...
#ifdef USE_REGULATOR_FRAMEWORK
	ts->core_reg = devm_regulator_get(&client->dev, "reg-vgp6");
	ts->io_reg = devm_regulator_get(&client->dev, "reg-vgp4");
	retval = regulator_set_voltage(ts->core_reg, 3300000, 3300000);
	if (retval != 0) {
		TPD_DMESG("Failed to set reg-rgp6 voltage: %d\n", retval);
		goto err_query_device;
	}
	retval = regulator_set_voltage(ts->io_reg, 1800000, 1800000);
	if (retval != 0) {
		TPD_DMESG("Failed to set reg-rgp4 voltage: %d\n", retval);
		goto err_query_device;
	}
	retval = regulator_enable(ts->core_reg);
	if (retval != 0) {
		TPD_DMESG("Failed to enable reg-rgp6: %d\n", retval);
		goto err_query_device;
	}
	retval = regulator_enable(ts->io_reg);
	if (retval != 0) {
...
/*	mt_set_gpio_mode(tpd_int_gpio_number, GPIO_CTP_EINT_PIN_M_EINT);
...
#else
	/* Set EINT PIN to low */
/*	mt_set_gpio_mode(tpd_int_gpio_number, GPIO_CTP_EINT_PIN_M_GPIO);
	mt_set_gpio_dir(tpd_int_gpio_number, GPIO_DIR_OUT);*/
...
#ifdef USE_REGULATOR_FRAMEWORK
	retval = regulator_disable(ts->io_reg);
	if (retval != 0)
		TPD_DMESG("Failed to disable reg-rgp6: %d\n", retval);

	retval = regulator_disable(ts->core_reg);
	if (retval != 0)
		TPD_DMESG("Failed to disable reg-rgp6: %d\n", retval);
