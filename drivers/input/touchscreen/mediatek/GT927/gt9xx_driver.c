...
struct gt9xx_platform_data *p_gt9xx_platform_data;
...
int i2c_read_bytes(struct i2c_client *client, u16 addr, u8 *rxbuf, int len)
{
	...
	while (left > 0) {
		...
		if (mt_i2c_transfer(client->adapter, &msg[0], 2) != 2) {
...
int i2c_write_bytes(struct i2c_client *client, u16 addr, u8 *txbuf, int len)
{
	...
	while (left > 0) {
		...
		if (mt_i2c_transfer(client->adapter, &msg, 1) != 1) {
...
static s32 gtp_init_panel(struct i2c_client *client)
{
	...
	memcpy(cfg_info_group1, p_gt9xx_platform_data->ctp_cfg_group1, cfg_len);
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	p_gt9xx_platform_data = client->dev.platform_data;
	...
	err =
	    request_irq(EINT_IRQ(client->irq), tpd_eint_interrupt_handler,
			(int_type) ? IRQ_TYPE_EDGE_FALLING : IRQ_TYPE_EDGE_RISING, "GT927", NULL);

	if (err) {
		GTP_ERROR("Could not register GT927, err=%d\n", err);
	}
...
static int tpd_local_init(void)
{
	...
	if (FACTORY_BOOT == get_boot_mode()) {
		...
		memcpy(tpd_def_calmat_local_factory,
		       p_gt9xx_platform_data->tpd_calibration_matrix_rotation_factory, 8 * 4);
		...
		memcpy(tpd_def_calmat_local_normal,
		       p_gt9xx_platform_data->tpd_calibration_matrix_rotation_normal, 8 * 4);
...
static void tpd_suspend(struct early_suspend *h)
{
	...
	disable_irq(EINT_IRQ(i2c_client_point->irq));
}
...
static void tpd_resume(struct early_suspend *h)
{
	...
	enable_irq(EINT_IRQ(i2c_client_point->irq));
	tpd_halt = 0;
