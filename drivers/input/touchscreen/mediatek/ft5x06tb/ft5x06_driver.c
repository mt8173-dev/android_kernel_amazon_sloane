...
#define TPD_INFO(fmt, arg...)  pr_info("[tpd info:5x06]" "[%s]" fmt "\r\n", __func__ , ##arg)
/* #define TP_DEBUG */
#if defined(TP_DEBUG)
#define TPD_DEBUG(fmt, arg...)  pr_info("[tpd debug:5x06]" "[%s]" fmt "\r\n", __func__ , ##arg)
#define TPD_DMESG(fmt, arg...)  pr_info("[tpd dmesg:5x06]" "[%s]" fmt "\r\n", __func__ , ##arg)
#else
#undef TPD_DEBUG
...
static irqreturn_t tpd_eint_interrupt_handler(int irq, void *dev);
...
static struct i2c_board_info i2c_tpd __initdata = { I2C_BOARD_INFO("mtk-tpd", (0x72 >> 1)) };
#else
unsigned short force[] = { 0, 0x70, I2C_CLIENT_END, I2C_CLIENT_END };
static const unsigned short *const forces[] = { force, NULL };

/* static struct i2c_client_address_data addr_data = { .forces = forces, }; */
static struct i2c_board_info i2c_tpd __initdata = { I2C_BOARD_INFO("mtk-tpd", (0x70 >> 1)) };
#endif

static struct kobject *touchdebug_kobj;
static struct kobject *touchdebug_kobj_info;
static int sensitivity_level = 1;
static int EnableWakeUp;
...
static u32 gpDMABuf_pa = (u32) NULL;

int fts_dma_i2c_read(struct i2c_client *client, u16 addr, int len, char *rxbuf)
{
	int ret;
	u8 buffer[1];
	...
	ret = mt_i2c_transfer(client->adapter, &msg[0], 2);
	if (ret < 0)
...
static ssize_t fts_i2cdev_write(struct file *file, const char __user *buf, size_t count,
				loff_t *offset)
{
	int ret;
	char *tmp;
...
static noinline int i2cdev_ioctl_rdrw(struct i2c_client *client, unsigned long arg)
{
	...
	struct mt_i2c_msg *rdwr_pa, *rdwr_pa_tmp;
	...
	rdwr_pa = kmalloc(rdwr_arg.nmsgs * sizeof(struct mt_i2c_msg), GFP_KERNEL);
	if (!rdwr_pa)
		return -ENOMEM;
...
*/
	...
	res = mt_i2c_transfer(client->adapter, rdwr_pa, rdwr_arg.nmsgs);
	while (i-- > 0) {
...
static noinline int i2cdev_ioctl_smbus(struct i2c_client *client, unsigned long arg)
{
	...
	if ((data_arg.read_write != I2C_SMBUS_READ) && (data_arg.read_write != I2C_SMBUS_WRITE)) {
		dev_dbg(&client->adapter->dev,
	...
	if ((data_arg.size == I2C_SMBUS_QUICK) ||
	    ((data_arg.size == I2C_SMBUS_BYTE) && (data_arg.read_write == I2C_SMBUS_WRITE)))
	...
	else if ((data_arg.size == I2C_SMBUS_WORD_DATA) || (data_arg.size == I2C_SMBUS_PROC_CALL))
...
static int fts_i2cdev_open(struct inode *inode, struct file *file)
{
	...
	if (ts_dev.ts) {
		...
		pr_info("[i2c-8] [fts_i2cdev_open][success]\n");
	} else {
		pr_info("[i2c-8] [fts_i2cdev_open][fail]\n");
		return -ENOMEM;
	}
...
static struct kobj_attribute firmware_attr = {.attr = {.name = __stringify(firmware),
						       .mode = 0644,
						       },
.show = firmware_show,
};

static struct kobj_attribute wakeup_attr = {.attr = {.name = __stringify(wakeup),
						     .mode = 0644,
						     },
.show = wakeup_show,
.store = wakeup_store,
...
static struct kobj_attribute sensitivity_attr = {.attr = {.name = __stringify(sensitivity),
							  .mode = 0644,
							  },
...
static void tpd_down(int x, int y, int p)
{
	...
	if (0 == strncmp(CONFIG_MTK_LCM_PHYSICAL_ROTATION, "270", 3)
	    || 0 == strncmp(CONFIG_MTK_LCM_PHYSICAL_ROTATION, "90", 2)) {
...
#if defined(TP_CFG_FOR_E1910_SMT) || defined(ROTATION_FOR_E1910_CQ)
	if (boot_mode != NORMAL_BOOT && (0 == strncmp(CONFIG_MTK_LCM_PHYSICAL_ROTATION, "270", 3)
					 || 0 == strncmp(CONFIG_MTK_LCM_PHYSICAL_ROTATION, "90",
							 2))) {
		int temp;
...
static int tpd_touchinfo(struct touch_info *cinfo, struct touch_info *pinfo)
{
	...
	fts_dma_i2c_read(i2c_client, 0x00, 64, (char *)gpDMABuf_pa);
...
static void tpd_calibrate_driver(int *x, int *y)
{
	...
	tx = ((tpd_calmat_driver[0] * (*x)) + (tpd_calmat_driver[1] * (*y)) +
	      (tpd_calmat_driver[2])) >> 12;
	*y = ((tpd_calmat_driver[3] * (*x)) + (tpd_calmat_driver[4] * (*y)) +
	      (tpd_calmat_driver[5])) >> 12;
	*x = tx;
}
...
static int touch_event_handler(void *unused)
{
	...
	do {
		enable_irq(EINT_IRQ(CUST_EINT_TOUCH_PANEL_NUM));
...
static irqreturn_t tpd_eint_interrupt_handler(int irq, void *dev)
{
	...
	tpd_flag = 1;
	disable_irq(EINT_IRQ(CUST_EINT_TOUCH_PANEL_NUM));
...
static int tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
 ...
 i2c_transfer_sucess:
	...
	err =
	    request_irq(EINT_IRQ(CUST_EINT_TOUCH_PANEL_NUM), tpd_eint_interrupt_handler,
			CUST_EINT_TOUCH_PANEL_TYPE, "ft5x06tb", NULL);

	if (err) {
		TPD_INFO("Could not register ft5x06tb, err=%d\n", err);
	}
	/* enable_irq(EINT_IRQ(CUST_EINT_TOUCH_PANEL_NUM)); */
	msleep(50);
...
static void tpd_resume(struct early_suspend *h)
{
	...
	if (!EnableWakeUp) {
		...
		do {
			...
			if ((ret =
			     i2c_smbus_read_i2c_block_data(i2c_client, FW_ID_ADDR, 1, &data)) < 0) {
				...
				TPD_DEBUG
				    ("i2c transfer success after reset :ret=%d,retry_num == %d\n",
				     ret, retry_num);
				break;
			}
			retry_num++;
		...
		if ((ret = i2c_smbus_read_i2c_block_data(i2c_client, FW_ID_ADDR, 1, &data)) < 0) {
			...
			if ((ret =
			     i2c_smbus_read_i2c_block_data(i2c_client, FW_ID_ADDR, 1, &data)) < 0) {
				TPD_DEBUG
				    ("i2c transf error after reset :ret = %d,retry_num == %d\n",
				     ret, retry_num);
			} else {
				TPD_DEBUG
				    ("i2c transfer success after reset :ret = %d,retry_num == %d\n",
	...
	mod_timer(&sensitivity_write_timer, jiffies + msecs_to_jiffies(TOUCH_RESUME_INTERVAL));
