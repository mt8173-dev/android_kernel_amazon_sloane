...
static struct i2c_board_info i2c_mpu6050 __initdata = {I2C_BOARD_INFO(MPU6050_DEV_NAME, (MPU6050_I2C_SLAVE_ADDR >> 1))};
					...
					if (atomic_read(&priv->trace) & MPU6050_TRC_FILTER) {
						GSE_LOG
						    ("add [%2d] [%5d %5d %5d] => [%5d %5d %5d] : [%5d %5d %5d]\n",
						     idx, priv->fir.raw[idx][MPU6050_AXIS_X],
						     priv->fir.raw[idx][MPU6050_AXIS_Y],
						     priv->fir.raw[idx][MPU6050_AXIS_Z],
						     priv->fir.sum[MPU6050_AXIS_X],
						     priv->fir.sum[MPU6050_AXIS_Y],
...
static int MPU6050_ReadCalibration(struct i2c_client *client, int dat[MPU6050_AXES_NUM])
{
	...
	dat[obj->cvt.map[MPU6050_AXIS_X]] =
	    obj->cvt.sign[MPU6050_AXIS_X] * (obj->offset[MPU6050_AXIS_X] * mul * GRAVITY_EARTH_1000/(obj->reso->sensitivity) +
					     obj->cali_sw[MPU6050_AXIS_X]);
	dat[obj->cvt.map[MPU6050_AXIS_Y]] =
	    obj->cvt.sign[MPU6050_AXIS_Y] * (obj->offset[MPU6050_AXIS_Y] * mul * GRAVITY_EARTH_1000/(obj->reso->sensitivity) +
					     obj->cali_sw[MPU6050_AXIS_Y]);
	dat[obj->cvt.map[MPU6050_AXIS_Z]] =
	    obj->cvt.sign[MPU6050_AXIS_Z] * (obj->offset[MPU6050_AXIS_Z] * mul * GRAVITY_EARTH_1000/(obj->reso->sensitivity) +
...
#ifdef SW_CALIBRATION
	obj->cali_sw[MPU6050_AXIS_X] =
	    obj->cvt.sign[MPU6050_AXIS_X] * (cali[obj->cvt.map[MPU6050_AXIS_X]]);
	obj->cali_sw[MPU6050_AXIS_Y] =
	    obj->cvt.sign[MPU6050_AXIS_Y] * (cali[obj->cvt.map[MPU6050_AXIS_Y]]);
	obj->cali_sw[MPU6050_AXIS_Z] =
	    obj->cvt.sign[MPU6050_AXIS_Z] * (cali[obj->cvt.map[MPU6050_AXIS_Z]]);
#else

	obj->offset[MPU6050_AXIS_X] =
	    (s8) (obj->cvt.sign[MPU6050_AXIS_X] * (cali[obj->cvt.map[MPU6050_AXIS_X]]) * (obj->reso->sensitivity)/GRAVITY_EARTH_1000 / (divisor));
	obj->offset[MPU6050_AXIS_Y] =
	    (s8) (obj->cvt.sign[MPU6050_AXIS_Y] * (cali[obj->cvt.map[MPU6050_AXIS_Y]]) * (obj->reso->sensitivity)/GRAVITY_EARTH_1000 / (divisor));
	obj->offset[MPU6050_AXIS_Z] =
	    (s8) (obj->cvt.sign[MPU6050_AXIS_Z] * (cali[obj->cvt.map[MPU6050_AXIS_Z]]) * (obj->reso->sensitivity)/GRAVITY_EARTH_1000 / (divisor));

	/*convert software calibration using standard calibration */
	obj->cali_sw[MPU6050_AXIS_X] =
	    obj->cvt.sign[MPU6050_AXIS_X] * (cali[obj->cvt.map[MPU6050_AXIS_X]]) % (divisor);
	obj->cali_sw[MPU6050_AXIS_Y] =
	    obj->cvt.sign[MPU6050_AXIS_Y] * (cali[obj->cvt.map[MPU6050_AXIS_Y]]) % (divisor);
	obj->cali_sw[MPU6050_AXIS_Z] =
	    obj->cvt.sign[MPU6050_AXIS_Z] * (cali[obj->cvt.map[MPU6050_AXIS_Z]]) % (divisor);
...
static int MPU6050_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	if (res) {
		...
		#if 0
		acc[MPU6050_AXIS_X] =
		    acc[MPU6050_AXIS_X] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
		acc[MPU6050_AXIS_Y] =
		    acc[MPU6050_AXIS_Y] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
		acc[MPU6050_AXIS_Z] =
		    acc[MPU6050_AXIS_Z] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
		#endif
		sprintf(buf, "%04x %04x %04x", acc[MPU6050_AXIS_X], acc[MPU6050_AXIS_Y],
			acc[MPU6050_AXIS_Z]);
...
/*----------------------------------------------------------------------------*/
#if !MPU6XXX_HWSELFTEST
static int MPU6050_JudgeTestResult(struct i2c_client *client, s32 prv[MPU6050_AXES_NUM],
				   s32 nxt[MPU6050_AXES_NUM])
{
	struct criteria {
		int min;
		int max;
...
static ssize_t show_cali_value(struct device_driver *ddri, char *buf)
{
	...
	err = MPU6050_ReadOffset(client, obj->offset);
	err1 = MPU6050_ReadCalibration(client, tmp);
	if (err)
		return -EINVAL;
	else if (err1)
		return -EINVAL;
		...
		len +=
		    snprintf(buf + len, PAGE_SIZE - len,
			     ...
			     obj->offset[MPU6050_AXIS_X], obj->offset[MPU6050_AXIS_Y],
			     obj->offset[MPU6050_AXIS_Z], obj->offset[MPU6050_AXIS_X],
...
static ssize_t show_self_value(struct device_driver *ddri, char *buf)
{
	...
	if (selftestret & 0x1)
		len +=
		    snprintf(buf + len, PAGE_SIZE - len, "Self-Test result=%d- Gyro = PASS \t",
			     selftestret);
	else
		len +=
		    snprintf(buf + len, PAGE_SIZE - len, "Self-Test result=%d- Gyro = FAIL \t",
			     selftestret);
	if (selftestret & 0x2)
	...
	len +=
	    snprintf(buf + len, PAGE_SIZE - len, "[gyro_bias](%+3d, %+3d, %+3d)\n",
		     obj->mpu_selftest_device.gyro_bias[MPU6050_AXIS_X],
		     obj->mpu_selftest_device.gyro_bias[MPU6050_AXIS_Y],
		     obj->mpu_selftest_device.gyro_bias[MPU6050_AXIS_Z]);
	len +=
	    snprintf(buf + len, PAGE_SIZE - len, "[accel_bias](%+3d, %+3d, %+3d)\n",
		     obj->mpu_selftest_device.accel_bias[MPU6050_AXIS_X],
		     obj->mpu_selftest_device.accel_bias[MPU6050_AXIS_Y],
		     obj->mpu_selftest_device.accel_bias[MPU6050_AXIS_Z]);
	len +=
	    snprintf(buf + len, PAGE_SIZE - len, "[gyro_bias_st](%+3d, %+3d, %+3d)\n",
		     obj->mpu_selftest_device.gyro_bias_st[MPU6050_AXIS_X],
		     obj->mpu_selftest_device.gyro_bias_st[MPU6050_AXIS_Y],
		     obj->mpu_selftest_device.gyro_bias_st[MPU6050_AXIS_Z]);
	len +=
	    snprintf(buf + len, PAGE_SIZE - len, "[accel_bias_st](%+3d, %+3d, %+3d)\n",
		     obj->mpu_selftest_device.accel_bias_st[MPU6050_AXIS_X],
		     obj->mpu_selftest_device.accel_bias_st[MPU6050_AXIS_Y],
		     obj->mpu_selftest_device.accel_bias_st[MPU6050_AXIS_Z]);
	return len;
}
		    ...
		    void *buff_out, int size_out, int *actualout)
{
	...
	case SENSOR_GET_DATA:
		if ((buff_out == NULL) || (size_out < sizeof(hwm_sensor_data))) {
			...
			if (!err) {
				if (3 != sscanf(buff, "%x %x %x", &gsensor_data->values[0],
...
static int mpu6050_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	err = mpu6050_create_attr(&mpu6050_gsensor_driver.driver);
	if (err) {
...
static int mpu6050_i2c_remove(struct i2c_client *client)
{
	...
	err = mpu6050_delete_attr(&mpu6050_gsensor_driver.driver);
	if (err)
...
static int __init mpu6050gse_init(void)
{
	...
	GSE_LOG("%s: @@@i2c_register_board_info\n", __func__);
	if (platform_driver_register(&mpu6050_gsensor_driver)) {
		GSE_ERR("failed to register driver");
		return -ENODEV;
	}
	GSE_LOG("%s: @@@platform_driver_register\n", __func__);
	return 0;
}
