...
#define AKMFUNC(func) pr_info("AKM8963 " func " is called\n")
#else
#define AKMFUNC(func)
#endif
...
static long akm8963_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case MSENSOR_IOCTL_SENSOR_ENABLE:
		...
		if (copy_from_user(&enable, argp, sizeof(enable))) {
			...
			pr_info("MSENSOR_IOCTL_SENSOR_ENABLE enable=%d!\r\n", enable);
			factory_mode = 1;
			if (1 == enable) {
				atomic_set(&o_flag, 1);
...
static int akm8963_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	pr_info(" AKM8963 akm8963_probe: befor init prob\n");
	/* Check connection */
