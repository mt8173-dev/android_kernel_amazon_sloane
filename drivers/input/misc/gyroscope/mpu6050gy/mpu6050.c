...
static struct i2c_board_info i2c_mpu6050 __initdata = {
	I2C_BOARD_INFO(MPU6050_DEV_NAME, (MPU6050_I2C_SLAVE_ADDR >> 1)) };
...
#define GYRO_FUN(f)               pr_err(KERN_ERR GYRO_TAG"%s\n", __func__)
#define GYRO_ERR(fmt, args...)    pr_err(KERN_ERR GYRO_TAG"%s %d : "fmt, __func__, __LINE__, ##args)
#define GYRO_LOG(fmt, args...)    pr_err(KERN_ERR GYRO_TAG fmt, ##args)
/*----------------------------------------------------------------------------*/
...
static DEVICE_ATTR(inv_gyro_data_ready, S_IRUGO | S_IWUSR, inv_gyro_data_ready_show,
		   inv_gyro_data_ready_store);
static DEVICE_ATTR(inv_gyro_power_state, S_IRUGO | S_IWUSR, inv_gyro_power_state_show,
		   inv_gyro_power_state_store);
...
static int MPU6050_write_rel_calibration(struct mpu6050_i2c_data *obj, int dat[MPU6050_AXES_NUM])
{
	obj->cali_sw[MPU6050_AXIS_X] =
	    obj->cvt.sign[MPU6050_AXIS_X] * dat[obj->cvt.map[MPU6050_AXIS_X]];
	obj->cali_sw[MPU6050_AXIS_Y] =
	    obj->cvt.sign[MPU6050_AXIS_Y] * dat[obj->cvt.map[MPU6050_AXIS_Y]];
	obj->cali_sw[MPU6050_AXIS_Z] =
	    obj->cvt.sign[MPU6050_AXIS_Z] * dat[obj->cvt.map[MPU6050_AXIS_Z]];
#if DEBUG
	if (atomic_read(&obj->trace) & GYRO_TRC_CALI) {
		GYRO_LOG("test  (%5d, %5d, %5d) ->(%5d, %5d, %5d)->(%5d, %5d, %5d))\n",
			 obj->cvt.sign[MPU6050_AXIS_X], obj->cvt.sign[MPU6050_AXIS_Y],
			 obj->cvt.sign[MPU6050_AXIS_Z], dat[MPU6050_AXIS_X], dat[MPU6050_AXIS_Y],
			 dat[MPU6050_AXIS_Z], obj->cvt.map[MPU6050_AXIS_X],
			 obj->cvt.map[MPU6050_AXIS_Y], obj->cvt.map[MPU6050_AXIS_Z]);
...
static int MPU6050_ReadCalibration(struct i2c_client *client, int dat[MPU6050_AXES_NUM])
{
	...
	dat[obj->cvt.map[MPU6050_AXIS_X]] =
	    obj->cvt.sign[MPU6050_AXIS_X] * obj->cali_sw[MPU6050_AXIS_X];
	dat[obj->cvt.map[MPU6050_AXIS_Y]] =
	    obj->cvt.sign[MPU6050_AXIS_Y] * obj->cali_sw[MPU6050_AXIS_Y];
	dat[obj->cvt.map[MPU6050_AXIS_Z]] =
	    obj->cvt.sign[MPU6050_AXIS_Z] * obj->cali_sw[MPU6050_AXIS_Z];
...
static int MPU6050_ReadStart(struct i2c_client *client, bool enable)
{
	...
	if (enable) {
		...
		databuf[1] =
		    (MPU6050_FIFO_GYROX_EN | MPU6050_FIFO_GYROY_EN | MPU6050_FIFO_GYROZ_EN);
	} else {
	...
	*datalen = packet_cnt;
	...
	if (packet_cnt && (abs(packet_thresh - packet_cnt) < 4)) {
		...
		for (i = 0; i < packet_cnt; i++) {
			...
			tmp1[MPU6050_AXIS_X] =
			    (s16) ((buf[MPU6050_AXIS_X * 2 + 1]) | (buf[MPU6050_AXIS_X * 2] << 8));
			tmp1[MPU6050_AXIS_Y] =
			    (s16) ((buf[MPU6050_AXIS_Y * 2 + 1]) | (buf[MPU6050_AXIS_Y * 2] << 8));
			tmp1[MPU6050_AXIS_Z] =
			    (s16) ((buf[MPU6050_AXIS_Z * 2 + 1]) | (buf[MPU6050_AXIS_Z * 2] << 8));

			/* remap coordinate// */
			tmp2[obj->cvt.map[MPU6050_AXIS_X]] =
			    obj->cvt.sign[MPU6050_AXIS_X] * tmp1[MPU6050_AXIS_X];
			tmp2[obj->cvt.map[MPU6050_AXIS_Y]] =
			    obj->cvt.sign[MPU6050_AXIS_Y] * tmp1[MPU6050_AXIS_Y];
			tmp2[obj->cvt.map[MPU6050_AXIS_Z]] =
			    obj->cvt.sign[MPU6050_AXIS_Z] * tmp1[MPU6050_AXIS_Z];
			...
			GYRO_LOG("gyro FIFO packet[%d]:[%04X %04X %04X] => [%5d %5d %5d]\n", i,
				 data[3 * i + MPU6050_AXIS_X], data[3 * i + MPU6050_AXIS_Y],
				 data[3 * i + MPU6050_AXIS_Z], data[3 * i + MPU6050_AXIS_X],
				 data[3 * i + MPU6050_AXIS_Y], data[3 * i + MPU6050_AXIS_Z]);
		}
...
if (hwmsen_read_block(client, MPU6050_REG_GYRO_XH, databuf, 6))
#endif
	{
		...
		obj->data[MPU6050_AXIS_X] =
		    ((s16)
		     ((databuf[MPU6050_AXIS_X * 2 + 1]) | (databuf[MPU6050_AXIS_X * 2] << 8)));
		obj->data[MPU6050_AXIS_Y] =
		    ((s16)
		     ((databuf[MPU6050_AXIS_Y * 2 + 1]) | (databuf[MPU6050_AXIS_Y * 2] << 8)));
		obj->data[MPU6050_AXIS_Z] =
		    ((s16)
		     ((databuf[MPU6050_AXIS_Z * 2 + 1]) | (databuf[MPU6050_AXIS_Z * 2] << 8)));
#if DEBUG
		if (atomic_read(&obj->trace) & GYRO_TRC_RAWDATA) {
			...
			GYRO_LOG("get gyro raw data (0x%08X, 0x%08X, 0x%08X) -> (%5d, %5d, %5d)\n",
				 obj->data[MPU6050_AXIS_X], obj->data[MPU6050_AXIS_Y],
				 obj->data[MPU6050_AXIS_Z], obj->data[MPU6050_AXIS_X],
		...
		data[MPU6050_AXIS_X] =
		    data[MPU6050_AXIS_X] * MPU6050_FS_MAX_LSB / MPU6050_DEFAULT_LSB;
		data[MPU6050_AXIS_Y] =
		    data[MPU6050_AXIS_Y] * MPU6050_FS_MAX_LSB / MPU6050_DEFAULT_LSB;
		data[MPU6050_AXIS_Z] =
		    data[MPU6050_AXIS_Z] * MPU6050_FS_MAX_LSB / MPU6050_DEFAULT_LSB;


	}

	sprintf(buf, "%04x %04x %04x", data[MPU6050_AXIS_X], data[MPU6050_AXIS_Y],
		data[MPU6050_AXIS_Z]);
...
static int MPU6050_PROCESS_SMT_DATA(struct i2c_client *client, short *data)
{
	...
	for (i = 0; i < total_num; i++) {
		...
		/*
		   FLPLOGD("read gyro data OK: packet_num:%d, [X:%5d, Y:%5d, Z:%5d]\n", i,
			   data[MPU6050_AXES_NUM*i + MPU6050_AXIS_X +2],
			   data[MPU6050_AXES_NUM*i + MPU6050_AXIS_Y +2],
	...
	for (i = 0; i < total_num; i++) {
		xRMS +=
		    (data[MPU6050_AXES_NUM * i + MPU6050_AXIS_X + 2] -
		     xAvg) * (data[MPU6050_AXES_NUM * i + MPU6050_AXIS_X + 2] - xAvg);
		yRMS +=
		    (data[MPU6050_AXES_NUM * i + MPU6050_AXIS_Y + 2] -
		     yAvg) * (data[MPU6050_AXES_NUM * i + MPU6050_AXIS_Y + 2] - yAvg);
		zRMS +=
		    (data[MPU6050_AXES_NUM * i + MPU6050_AXIS_Z + 2] -
		     zAvg) * (data[MPU6050_AXES_NUM * i + MPU6050_AXIS_Z + 2] - zAvg);
	}
		    ...
		    void *buff_out, int size_out, int *actualout)
{
	...
	case SENSOR_GET_DATA:
		if ((buff_out == NULL) || (size_out < sizeof(hwm_sensor_data))) {
			...
			if (!err) {
				if (3 != sscanf(buff, "%x %x %x", &gyro_data->values[0],
...
static long mpu6050_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case GYROSCOPE_IOCTL_SET_CALI:
		...
		if (copy_from_user(&sensor_data, data, sizeof(sensor_data))) {
			...
			cali[MPU6050_AXIS_X] =
			    sensor_data.x * MPU6050_DEFAULT_LSB / MPU6050_FS_MAX_LSB;
			cali[MPU6050_AXIS_Y] =
			    sensor_data.y * MPU6050_DEFAULT_LSB / MPU6050_FS_MAX_LSB;
			cali[MPU6050_AXIS_Z] =
			    sensor_data.z * MPU6050_DEFAULT_LSB / MPU6050_FS_MAX_LSB;
