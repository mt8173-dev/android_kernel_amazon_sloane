		...
		while (QUEUE_IS_NOT_EMPTY(prTxQueue)) {
			...
			while (QUEUE_IS_NOT_EMPTY(prTempQue)) {
				...
				/* Handle normal frame */
					...
					u4Status =
					    wlanEnqueueTxPacket(prGlueInfo->prAdapter,
	...
	while (TRUE) {
		...
		do {
			ret = wait_event_interruptible(prGlueInfo->waitq_hif,
						       ((prGlueInfo->
							 ulFlag & GLUE_FLAG_HIF_PROCESS) != 0));
		} while (ret != 0);
...
VOID kalIndicateRxMgmtFrame(IN P_GLUE_INFO_T prGlueInfo, IN P_SW_RFB_T prSwRfb)
{
	...
	do {
		...
		cfg80211_rx_mgmt(
					...
					i4Freq,	/* in MHz */
					RCPI_TO_dBm((UINT_8)
						    HAL_RX_STATUS_GET_RCPI
						    (prSwRfb->prRxStatusGroup3)), prSwRfb->pvHeader,
...
unsigned long __read_mostly tracing_mark_write_addr = 0;
...
static const struct file_operations rMetProcFops = {
...
UINT_64 kalGetBootTime(void)
{
	...
	bootTime = ts.tv_nsec;
	do_div(bootTime, NSEC_PER_USEC);
	bootTime += ts.tv_sec * USEC_PER_SEC;
	return bootTime;
}
