...
*/
static void
wext_support_ioctl_SIOCSIWGENIE(IN P_GLUE_INFO_T prGlueInfo, IN char *prExtraBuf,
				IN UINT_32 u4ExtraSize);

static void
wext_support_ioctl_SIOCSIWPMKSA_Action(IN struct net_device *prDev, IN char *prExtraBuf,
		    ...
		    IN struct iw_point *prEnc, IN char *pcExtra)
{
...
#endif
	{
		...
		case IW_ENCODE_ALG_AES_CMAC:
			...
			prKey->u4Length =
			    ((ULONG) & (((P_PARAM_KEY_T) 0)->aucKeyMaterial)) + prKey->u4KeyLength;
			...
			if (1 /* wlanQueryWapiMode(prGlueInfo->prAdapter) */) {
				...
				if (u4ExtraSize) {
					...
					if (copy_from_user(prExtraBuf,
						...
						wext_support_ioctl_SIOCSIWGENIE(prGlueInfo,
										prExtraBuf,
										u4ExtraSize);
					}
					kalMemFree(prExtraBuf, VIR_MEM_TYPE, u4ExtraSize);
		...
		if (iwr->u.data.pointer) {
			...
			if (copy_from_user(prExtraBuf,
				...
				case IW_PMKSA_ADD:
					 ...
					 */
					{
						wext_support_ioctl_SIOCSIWPMKSA_Action(prDev,
										       prExtraBuf,
										       IW_PMKSA_ADD,
										       &ret);
					}
				...
				IN UINT_32 u4ExtraSize)
{
	...
	rStatus = kalIoctl(prGlueInfo,
			   ...
			   prExtraBuf, u4ExtraSize, FALSE, FALSE, TRUE, &u4BufLen);
	if (rStatus != WLAN_STATUS_SUCCESS) {
#endif
#if CFG_SUPPORT_WPS2
				       ...
				       IN int ioMode, OUT int *ret)
{
	...
	case IW_PMKSA_ADD:
		prPmkid =
		    (P_PARAM_PMKID_T) kalMemAlloc(8 + sizeof(PARAM_BSSID_INFO_T), VIR_MEM_TYPE);
		if (!prPmkid) {
		...
		rStatus = kalIoctl(prGlueInfo,
				   wlanoidSetPmkid,
				   prPmkid, sizeof(PARAM_PMKID_T), FALSE, FALSE, TRUE, &u4BufLen);
