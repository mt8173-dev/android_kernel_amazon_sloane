		     ...
		     struct net_device *ndev, struct cfg80211_connect_params *sme)
{
	...
	if (sme->crypto.n_ciphers_pairwise) {
		...
		prGlueInfo->prAdapter->rWifiVar.rConnSettings.
		    rRsnInfo.au4PairwiseKeyCipherSuite[0] = sme->crypto.ciphers_pairwise[0];
...
#define CHN_DIRTY_WEIGHT_UPPERBOUND 4
	...
	PARAM_PREFER_CHN_INFO PreferChannels[2],
	    ar2_4G_ChannelLoadingWeightScore[MAXMUN_2_4G_CHA_NUM];
...
#endif
	{
		...
		prGetChnLoad =
		    (P_PARAM_GET_CHN_LOAD) &(prGlueInfo->prAdapter->rWifiVar.rChnLoadInfo);
		...
		for (ucIdx = ucDefaultIdx; ucIdx < ucMax_24G_Chn_List; ucIdx++) {
...
#if 1
			u2APNumScore =
			    prGetChnLoad->rEachChnLoad[ucIdx].u2APNum * CHN_DIRTY_WEIGHT_UPPERBOUND;
			...
			if ((ucIdx > 0) && (ucIdx < (MAXMUN_2_4G_CHA_NUM - 1))) {
				...
				DBGLOG(P2P, INFO,
					...
					ucIdx, ar2_4G_ChannelLoadingWeightScore[ucIdx].u2APNum,
					prGetChnLoad->rEachChnLoad[ucIdx].u2APNum,
					prGetChnLoad->rEachChnLoad[ucIdx + 1].u2APNum,
					prGetChnLoad->rEachChnLoad[ucIdx - 1].u2APNum));
			}
		...
		for (ucIdx = ucDefaultIdx; ucIdx < ucMax_24G_Chn_List; ucIdx++) {
			...
			if (PreferChannels[0].u2APNum >=
				...
				PreferChannels[0].u2APNum =
				    ar2_4G_ChannelLoadingWeightScore[ucIdx].u2APNum;
			} else {
				if ((PreferChannels[1].u2APNum >=
				     ar2_4G_ChannelLoadingWeightScore[ucIdx].u2APNum)
				    || (fgIsFistRecord == 1)) {
					fgIsFistRecord = FALSE;
					PreferChannels[1].ucChannel = ucIdx;
					PreferChannels[1].u2APNum =
					    ar2_4G_ChannelLoadingWeightScore[ucIdx].u2APNum;
				}
			}
			ucChValidCnt++;
