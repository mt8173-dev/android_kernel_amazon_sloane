			...
			if (prStaRec->ucStaState > STA_STATE_1) {
				...
				if (authProcessRxDeauthFrame(prSwRfb,
					...
					if (prAisSpecBssInfo->fgMgmtProtection
						...
						saaChkDeauthfrmParamHandler(prAdapter, prSwRfb,
									    prStaRec);
						return WLAN_STATUS_SUCCESS;
					}
#endif
					saaSendDisconnectMsgHandler(prAdapter, prStaRec,
								    prAisBssInfo, FRM_DEAUTH);
				}
			}
		}
#if CFG_ENABLE_WIFI_DIRECT
...
VOID
saaChkDeauthfrmParamHandler(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb,
			    IN P_STA_RECORD_T prStaRec)
{
	P_WLAN_DEAUTH_FRAME_T prDeauthFrame;
	do {
...
/*----------------------------------------------------------------------------*/
VOID
saaSendDisconnectMsgHandler(IN P_ADAPTER_T prAdapter, IN P_STA_RECORD_T prStaRec,
...
#endif
					saaSendDisconnectMsgHandler(prAdapter, prStaRec, NULL,
								    FRM_DISASSOC);
				}
			}
		}
