	...
	if (pucPairSuite) {
		...
		for (i = 0; i < (UINT_32) u2PairSuiteCount; i++) {
			...
			DBGLOG(SEC, LOUD,
				...
				(UCHAR) ((prWapiInfo->au4PairwiseKeyCipherSuite[i] >> 8) &
					 0x000000FF),
				(UCHAR) ((prWapiInfo->au4PairwiseKeyCipherSuite[i] >> 16) &
					 0x000000FF),
				(UCHAR) ((prWapiInfo->au4PairwiseKeyCipherSuite[i] >> 24) &
					 0x000000FF)));
		}
	} else {
	...
	if (pucAuthSuite) {
		...
		for (i = 0; i < (UINT_32) u2AuthSuiteCount; i++) {
			...
			DBGLOG(SEC, LOUD, ("WAPI: AKM suite [%d]: %02x-%02x-%02x-%02x\n",
					   ...
					   (UCHAR) ((prWapiInfo->au4AuthKeyMgtSuite[i] >> 8) &
						    0x000000FF),
					   (UCHAR) ((prWapiInfo->au4AuthKeyMgtSuite[i] >> 16) &
						    0x000000FF),
					   (UCHAR) ((prWapiInfo->au4AuthKeyMgtSuite[i] >> 24) &
						    0x000000FF)));
		}
	} else {
		/* The information about the authentication and key management suites
		...
		DBGLOG(SEC, LOUD, ("WAPI: AKM suite: %02x-%02x-%02x-%02x (default)\n",
				   ...
				   (UCHAR) ((prWapiInfo->au4AuthKeyMgtSuite[0] >> 24) &
					    0x000000FF)));
	}
