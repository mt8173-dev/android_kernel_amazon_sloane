	...
	do {
		...
		case AIS_STATE_SEARCH:
			...
			/* 4 <3> We are under Roaming Condition. */
				 ...
				 */
				if (prAisBssInfo->ucReasonOfDisconnect != DISCONNECT_REASON_CODE_REASSOCIATION && ((!prBssDesc) ||	/* CASE I */
	...
	do {
		...
		/* 4 <2> JOIN was not successful */
			...
			if (aisFsmStateInit_RetryJOIN(prAdapter, prStaRec) == FALSE) {
				...
				prBssDesc =
				    scanSearchBssDescByBssid(prAdapter, prStaRec->aucMacAddr);
		...
		case AIS_STATE_IBSS_MERGE:
			{
				...
				prBssDesc =
				    scanSearchBssDescByBssid(prAdapter, prAisBssInfo->aucBSSID);
				if (prBssDesc != NULL) {
					prBssDesc->fgIsConnecting = FALSE;
					prBssDesc->fgIsConnected = FALSE;
				}
...
VOID aisUpdateBssInfoForCreateIBSS(IN P_ADAPTER_T prAdapter)
{
	...
	if (prAisBssInfo->eBand == BAND_2G4) {
		...
		/* Depend on eBand */
		prAisBssInfo->ucPhyTypeSet =
		    prAdapter->rWifiVar.ucAvailablePhyTypeSet & PHY_TYPE_SET_802_11ANAC;
		/* Depend on eCurrentOPMode and ucPhyTypeSet */
...
static VOID aisFsmRunEventScanDoneTimeOut(IN P_ADAPTER_T prAdapter, ULONG ulParam)
{
	...
	DBGLOG(AIS, STATE,
	       ("aisFsmRunEventScanDoneTimeOut Current[%d]\n", prAisFsmInfo->eCurrentState));
		   ...
		   IN P_MSDU_INFO_T prMgmtTxMsdu, IN UINT_64 u8Cookie)
{
	...
	do {
		...
		TX_SET_MMPDU(prAdapter,
			     prMgmtTxMsdu,
			     (prStaRec !=
			      NULL) ? (prStaRec->ucBssIndex) : (prAdapter->
								prAisBssInfo->ucBssIndex),
