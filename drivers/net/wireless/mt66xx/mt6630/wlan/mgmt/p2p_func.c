...
p2pFuncGetAttriListAction(IN P_ADAPTER_T prAdapter,
			  ...
			  OUT BOOLEAN * fgIsAllocMem, OUT BOOLEAN * fgBackupAttributes,
			  OUT UINT_16 * u2BufferSize);
...
p2pFuncProcessP2pProbeRspAction(IN P_ADAPTER_T prAdapter,
				...
				OUT BOOLEAN * fgIsWSCIE, OUT BOOLEAN * fgIsP2PIE,
				OUT BOOLEAN * fgIsWFDIE);
static VOID p2pFuncGetSpecAttriAction(IN P_IE_P2P_T prP2pIE, IN UINT_8 ucOuiType,
				      IN UINT_8 ucAttriID, OUT P_ATTRIBUTE_HDR_T * prTargetAttri);
/*----------------------------------------------------------------------------*/
/*!
		...
		case SCAN_CHANNEL_P2P_SOCIAL:
			{
				...
				for (prScanReqV2->ucSSIDNum = 0;
					...
					COPY_SSID(prParamSsid->aucSsid,
						  ...
						  prScanReqInfo->
						  arSsidStruct[prScanReqV2->ucSSIDNum].aucSsid,
						  prScanReqInfo->
						  arSsidStruct[prScanReqV2->ucSSIDNum].ucSsidLen);
		   ...
		   IN UINT_8 ucBssIndex, IN P_MSDU_INFO_T prMgmtTxMsdu, IN BOOLEAN fgNonCckRate)
{
	...
	do {
		...
		prWlanHdr =
		    (P_WLAN_MAC_HEADER_T) ((ULONG) prMgmtTxMsdu->prPacket + MAC_TX_RESERVED_FIELD);
		    ...
		    IN UINT_32 u4BodyLen, IN UINT_32 u4DtimPeriod, IN UINT_32 u4BcnInterval)
{
	...
	do {
		...
		do {
			...
			/* Body not change. */
			u2OldBodyLen =
			    (UINT_16) ((UINT_32) pucTIMBody - (UINT_32) prBcnFrame->aucInfoElem);
			/* Move body. */
		...
		rResultStatus = nicUpdateBeaconIETemplate(prAdapter,
							  ...
							  OFFSET_OF
							  (WLAN_BEACON_FRAME_T, aucInfoElem));
		if (prP2pBssInfo->eCurrentOPMode == OP_MODE_ACCESS_POINT) {
			/* AP is created, Beacon Update. */
...
#endif
		prBcnFrame =
		    (P_WLAN_BEACON_FRAME_T) ((ULONG) prBcnMsduInfo->prPacket +
		...
		case OP_MODE_INFRASTRUCTURE:
			...
			if (prP2pBssInfo->prStaRecOfAP) {
				kalP2PGCIndicateConnectionStatus(prAdapter->prGlueInfo,
								 (UINT_8)
								 prP2pBssInfo->u4PrivateData, NULL,
								 NULL, 0,
								 REASON_CODE_DEAUTH_LEAVING_BSS);
	...
	do {
		...
		IE_FOR_EACH(pucIE, u4IELen, u2Offset) {
			...
			case ELEM_ID_HT_CAP:	/* 45 *//* V */
				{
					...
					if ((HT_CAP_IE(pucIE)->u2HtCapInfo &
					    ...
					    == 0) {
						prAdapter->rWifiVar.
						    rConnSettings.fgRxShortGIDisabled = TRUE;
					} else {
						prAdapter->rWifiVar.
						    rConnSettings.fgRxShortGIDisabled = FALSE;
					}
			...
			case ELEM_ID_VENDOR:	/* 221 *//* V */
				...
				{
					p2pFuncParseBeaconVenderId(prAdapter, pucIE,
		...
		if (rsnParseCheckForWFAInfoElem(prAdapter, pucIE, &ucOuiType, &u2SubTypeVersion)) {
			...
			if (ucOuiType == VENDOR_OUI_TYPE_P2P) {
				kalMemCopy(&prP2pSpecificBssInfo->
					   aucAttributesCache[prP2pSpecificBssInfo->u2AttributeLen],
					   pucIE, IE_SIZE(pucIE));
			...
			kalMemCopy(&prP2pSpecificBssInfo->
				   aucAttributesCache[prP2pSpecificBssInfo->u2AttributeLen], pucIE,
				   IE_SIZE(pucIE));
			...
			if (u2CopyLen) {
				kalMemCopy((PUINT_8)
					   ((ULONG) (*pucAttriListStart) +
					    (ULONG) (*u2AttriListLen)), &prIe->aucP2PAttributes[0],
					   u2CopyLen);
			  ...
			  IN UINT_8 ucBssIdx, IN P_MSDU_INFO_T prMgmtTxMsdu)
{
	...
	do {
		...
		IE_FOR_EACH(pucIEBuf, u2IELength, u2Offset) {
			...
			case ELEM_ID_SSID:
				{
					p2pFuncProcessP2pProbeRspAction(prAdapter, pucIEBuf,
									ELEM_ID_SSID, &ucBssIdx,
									&prP2pBssInfo, &fgIsWSCIE,
									&fgIsP2PIE, &fgIsWFDIE);
				}
				break;
			case ELEM_ID_VENDOR:
		...
		if (fgIsP2PIE) {
			...
			u2EstimatedExtraIELen +=
			    p2pFuncCalculateP2P_IE_NoA(prAdapter, ucBssIdx, NULL);
		}
#if CFG_SUPPORT_WFD
					...
					if ((sizeof(prAdapter->prGlueInfo->prP2PInfo->aucWFDIE) >=
						...
						*fgIsWFDIE = TRUE;
						kalMemCopy(prAdapter->prGlueInfo->prP2PInfo->
							   aucWFDIE, pucIEBuf, IE_SIZE(pucIEBuf));
						prAdapter->prGlueInfo->prP2PInfo->u2WFDIELen +=
						    IE_SIZE(pucIEBuf);
					}
				}	/*  VENDOR_OUI_TYPE_WFD */
	...
	do {
		...
		do {
			...
			if (prP2pIE) {
				...
				u2BufferLenLeft =
				    u2BufferLen - (UINT_16) (((ULONG) prP2pIE) -
							     ((ULONG) pucIEBuf));
				...
				if (IE_LEN(prP2pIE) > P2P_OUI_TYPE_LEN) {
					p2pFuncGetSpecAttriAction(prP2pIE, ucOuiType, ucAttriID,
								  &prTargetAttri);
				}
				/* P2P_OUI_TYPE_LEN */
			  ...
			  OUT P_ATTRIBUTE_HDR_T *prTargetAttri)
{
	...
	if (prP2pIE->ucOuiType == ucOuiType) {
		...
		if ((prP2pIE->aucOui[0] == aucWfaOui[0]) &&
			...
			if (ucOuiType == VENDOR_OUI_TYPE_WPS) {
				WSC_ATTRI_FOR_EACH(pucAttri, (IE_LEN(prP2pIE) - P2P_IE_OUI_HDR),
						   u2OffsetAttri) {
				...
				P2P_ATTRI_FOR_EACH(pucAttri, (IE_LEN(prP2pIE) - P2P_IE_OUI_HDR),
						   u2OffsetAttri) {
				...
				WFD_ATTRI_FOR_EACH(pucAttri, (IE_LEN(prP2pIE) - P2P_IE_OUI_HDR),
						   u2OffsetAttri) {
...
UINT_32
p2pFuncCalculateP2P_IE_NoA(IN P_ADAPTER_T prAdapter, IN UINT_8 ucBssIdx, IN P_STA_RECORD_T prStaRec)
{
	P_P2P_SPECIFIC_BSS_INFO_T prP2pSpecificBssInfo = NULL;
	UINT_8 ucIdx;
	UINT_32 u4NumOfNoaDesc = 0;
...
VOID p2pFuncGenerateP2P_IE_NoA(IN P_ADAPTER_T prAdapter, IN P_MSDU_INFO_T prMsduInfo)
{
	...
	p2pFuncComposeNoaAttribute(prAdapter, prMsduInfo->ucBssIndex, prIeP2P->aucP2PAttributes,
				   &u4AttributeLen);
