...
static void wlanP2PEarlySuspend(void)
{
	...
	pr_info("*********p2pEarlySuspend************\n");

	if (!wlanExportGlueInfo(&prGlueInfo)) {
		pr_info("*********p2pEarlySuspend ignored************\n");
		return;
	}
...
/* prParamNetAddr = (P_PARAM_NETWORK_ADDRESS)((UINT_32)prParamNetAddr + sizeof(ip6)); */
		...
		if (rStatus != WLAN_STATUS_SUCCESS) {
			pr_info(DRV_NAME "set HW pattern filter fail 0x%lx\n", rStatus);
		}
	}
}
...
static void wlanP2PLateResume(void)
{
	...
	pr_info("*********wlanP2PLateResume************\n");
	if (!wlanExportGlueInfo(&prGlueInfo)) {
		pr_info("*********p2pLateResume ignored************\n");
		return;
	}
...
{
	pr_info("*********wlanP2P_early_suspend************\n");
	wlanP2PEarlySuspend();
}

static void p2p_late_resume(struct early_suspend *h)
{
	pr_info("*********wlanP2P_late_resume************\n");
	wlanP2PLateResume();
}
#endif
...
static int initP2P(void)
{
	...
	pr_info(KERN_INFO DRV_NAME "InitP2P, Ifname: %s, Mode: %s\n", ifname, mode ? "AP" : "P2P");
...
static VOID __exit exitP2P(void)
{
	...
	pr_info(KERN_INFO DRV_NAME "ExitP2P\n");
