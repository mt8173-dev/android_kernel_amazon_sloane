...
void debug_gpio_init(void)
{
	...
	for (i = 0; i < sizeof(dbgPinSTP) / sizeof(dbgPinSTP[0]); ++i) {
		...
		mt_pin_set_pull(dbgPinSTP[i], MT_PIN_PULL_DISABLE);	/* disable pull */
		gpio_direction_output(dbgPinSTP[i], 0);	/* set output */
		mt_pin_set_mode_gpio(dbgPinSTP[i]);	/* set gpio mode */

		/* toggle twice to check if ok: */
		gpio_set_value(dbgPinSTP[i], 0);	/* tie low */
		gpio_set_value(dbgPinSTP[i], 1);	/* tie high */
		gpio_set_value(dbgPinSTP[i], 0);	/* tie low */
		gpio_set_value(dbgPinSTP[i], 1);	/* tie high */
	}
...
void debug_gpio_deinit(void)
{
	...
	for (i = 0; i < sizeof(dbgPinSTP) / sizeof(dbgPinSTP[0]); ++i) {
		...
		/* pr_info("[%s] %ld\n", __FUNCTION__, dbgPinSTP[i]); */
		gpio_direction_input(dbgPinSTP[i]);
	}
...
void mtk_wcn_stp_debug_gpio_assert(UINT_32 dwIndex, UINT_32 dwMethod)
{
	...
	if (dwMethod & DBG_TIE_DIR) {
		if (dwMethod & DBG_HIGH) {
			gpio_set_value(dbgPinSTP[dwIndex], 1);
		} else {
			gpio_set_value(dbgPinSTP[dwIndex], 0);
		}
		return;
	}
...
static INT32 mtk_sdio_interrupt(MTK_WCN_HIF_SDIO_CLTCTX cltCtx)
{
	...
	if (!prGlueInfo) {
		/* pr_info(DRV_NAME"No glue info in mtk_sdio_interrupt()\n"); */
		return (-HIF_SDIO_ERR_FAIL);
	}

	if (prGlueInfo->u4Flag & GLUE_FLAG_HALT) {
		/* pr_info(DRV_NAME"GLUE_FLAG_HALT skip INT\n"); */
...
static void mtk_sdio_interrupt(struct sdio_func *func)
{
	...
	if (prGlueInfo->u4Flag & GLUE_FLAG_HALT) {
		...
		/* pr_info(DRV_NAME"ignore pending interrupt\n"); */
	} else {
...
mtk_sdio_probe(MTK_WCN_HIF_SDIO_CLTCTX cltCtx, const MTK_WCN_HIF_SDIO_FUNCINFO *prFuncInfo)
{
	...
	if (pfWlanProbe((PVOID) & cltCtx) != WLAN_STATUS_SUCCESS) {
		/* pr_warn(DRV_NAME"pfWlanProbe fail!call pfWlanRemove()\n"); */
		pfWlanRemove();
		ret = -(HIF_SDIO_ERR_FAIL);
	} else {
		/* pr_info(DRV_NAME"mtk_wifi_sdio_probe() done(%d)\n", ret); */
	}
	return ret;
}
#else
static int mtk_sdio_probe(struct sdio_func *func, const struct sdio_device_id *id)
{
	...
	/* pr_info(DRV_NAME "mtk_sdio_probe()\n"); */

	ASSERT(func);
	ASSERT(id);

	/* pr_info(DRV_NAME "Basic struct size checking...\n"); */
	/* pr_info(DRV_NAME "sizeof(struct device) = %d\n", sizeof(struct device)); */
	/* pr_info(DRV_NAME "sizeof(struct mmc_host) = %d\n", sizeof(struct mmc_host)); */
	/* pr_info(DRV_NAME "sizeof(struct mmc_card) = %d\n", sizeof(struct mmc_card)); */
	/* pr_info(DRV_NAME "sizeof(struct mmc_driver) = %d\n", sizeof(struct mmc_driver)); */
	/* pr_info(DRV_NAME "sizeof(struct mmc_data) = %d\n", sizeof(struct mmc_data)); */
	/* pr_info(DRV_NAME "sizeof(struct mmc_command) = %d\n", sizeof(struct mmc_command)); */
	/* pr_info(DRV_NAME "sizeof(struct mmc_request) = %d\n", sizeof(struct mmc_request)); */
	/* pr_info(DRV_NAME "sizeof(struct sdio_func) = %d\n", sizeof(struct sdio_func)); */

	/* pr_info(DRV_NAME "Card information checking...\n"); */
	/* pr_info(DRV_NAME "func = 0x%p\n", func); */
	/* pr_info(DRV_NAME "Number of info = %d:\n", func->card->num_info); */

	for (i = 0; i < func->card->num_info; i++) {
		/* pr_info(DRV_NAME "info[%d]: %s\n", i, func->card->info[i]); */
	}
	...
	if (ret) {
		/* pr_info(DRV_NAME"sdio_enable_func failed!\n"); */
		goto out;
	}
	/* pr_info(DRV_NAME"sdio_enable_func done!\n"); */
...
#if CFG_DBG_GPIO_PINS
		/* pr_info("[%s] init debug gpio, 20100815\n", __FUNCTION__); */
 ...
 out:
	/* pr_info(DRV_NAME"mtk_sdio_probe() done(%d)\n", ret); */
	return ret;
}
#endif
...
{
	INT32 ret = HIF_SDIO_ERR_SUCCESS;
	/* pr_info(DRV_NAME"pfWlanRemove done\n"); */
	pfWlanRemove();
...
{
	/* pr_info(DRV_NAME"mtk_sdio_remove()\n"); */

#if CFG_DBG_GPIO_PINS
	/* pr_info("[%s] deinit debug gpio\n", __FUNCTION__); */
	debug_gpio_deinit();
#endif
	...
	/* pr_info(DRV_NAME"sdio_disable_func() done\n"); */
	sdio_release_host(func);

	/* pr_info(DRV_NAME"mtk_sdio_remove() done\n"); */
}
#endif
...
{
	/* pr_info("mtk_sdio: mtk_sdio_suspend dev(0x%p)\n", pDev); */
	/* pr_info("mtk_sdio: MediaTek SDIO WLAN driver\n"); */
...
{
	/* pr_info("mtk_sdio: mtk_sdio_resume dev(0x%p)\n", pDev); */
...
VOID glSetHifInfo(P_GLUE_INFO_T prGlueInfo, UINT_32 u4Cookie)
{
	...
	/* pr_info(DRV_NAME"prHif->func->dev = 0x%p\n", &prHif->func->dev); */
	/* pr_info(DRV_NAME"prHif->func->vendor = 0x%04X\n", prHif->func->vendor); */
	/* pr_info(DRV_NAME"prHif->func->device = 0x%04X\n", prHif->func->device); */
	/* pr_info(DRV_NAME"prHif->func->func = 0x%04X\n", prHif->func->num); */
...
BOOL glBusInit(PVOID pvData)
{
	...
	if (ret) {
		/* pr_info(DRV_NAME"sdio_set_block_size 512 failed!\n"); */
	} else {
		/* pr_info(DRV_NAME"sdio_set_block_size 512 done!\n"); */
	}

	/* pr_info(DRV_NAME"param: func->cur_blksize(%d)\n", func->cur_blksize); */
	/* pr_info(DRV_NAME"param: func->max_blksize(%d)\n", func->max_blksize); */
	/* pr_info(DRV_NAME"param: func->card->host->max_blk_size(%d)\n", func->card->host->max_blk_size); */
	/* pr_info(DRV_NAME"param: func->card->host->max_blk_count(%d)\n", func->card->host->max_blk_count); */
#endif
	return TRUE;
}				/* end of glBusInit() */
...
VOID glBusFreeIrq(PVOID pvData, PVOID pvCookie)
{
	...
	ASSERT(pvData);
	if (!pvData) {
		/* pr_info(DRV_NAME"%s null pvData\n", __FUNCTION__); */
		return;
	}
	prNetDevice = (struct net_device *)pvData;
	prGlueInfo = (P_GLUE_INFO_T) pvCookie;
	ASSERT(prGlueInfo);
	if (!prGlueInfo) {
		/* pr_info(DRV_NAME"%s no glue info\n", __FUNCTION__); */
		return;
	}
...
#if DBG
	/* pr_info(DRV_NAME"++kalDevPortRead++ buf:0x%p, port:0x%x, length:%d\n", pucBuf, u2Port, u2Len); */
#endif
...
#if DBG
	/* pr_info(DRV_NAME"++kalDevPortWrite++ buf:0x%p, port:0x%x, length:%d\n", pucBuf, u2Port, u2Len); */
#endif
