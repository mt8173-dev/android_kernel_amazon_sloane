			...
			OUT PPVOID ppvMapFileBuf, OUT PUINT_32 pu4FileLength)
{
	...
	do {
		...
		if (i4Ret) {
			pr_info(DRV_NAME "fw %s:request failed %d\n", CFG_FW_FILENAME, i4Ret);
			break;
		} else {
...
VOID kalQueryTxChksumOffloadParam(IN PVOID pvPacket, OUT PUINT_8 pucFlag)
{
...
#endif
	{
		...
		if (skb->protocol != __constant_htons(ETH_P_IP)) {
			/* pr_info("Wrong skb->protocol( = %08x) for TX Checksum Offload.\n", skb->protocol); */
		} else
#endif
	...
	case WLAN_STATUS_MEDIA_SPECIFIC_INDICATION:
		if (pStatus) {
			...
			case ENUM_STATUS_TYPE_AUTHENTICATION:
				/*
				   pr_notice("ENUM_STATUS_TYPE_AUTHENTICATION: L(%ld) [" MACSTR "] F:%lx\n",
			...
			case ENUM_STATUS_TYPE_CANDIDATE_LIST:
				/*
				   pr_notice("Param_StatusType_PMKID_CandidateList: Ver(%ld) Num(%ld)\n",
				   pPmkid->u2Version,
				   pPmkid->u4NumCandidates);
				   if (pPmkid->u4NumCandidates > 0) {
				   pr_notice("candidate[" MACSTR "] preAuth Flag:%lx\n",
			...
			default:
				...
				/*
				   pr_notice("unknown media specific indication type:%x\n",
				   pStatus->StatusType);
				 */
				break;
			}
			...
			/*
			   pr_warn("media specific indication buffer NULL\n");
			 */
		}
		break;
			...
			IN UINT_32 u4FrameBodyLen, IN BOOLEAN fgReassocRequest)
{
	...
	if (fgReassocRequest) {
		if (u4FrameBodyLen < 15) {
			/*
			   pr_warn("frameBodyLen too short:%ld\n", frameBodyLen);
			 */
			return;
		}
	} else {
...
#if 0
		if (prGlueInfo->u4Flag & GLUE_FLAG_HALT) {
			pr_info(DRV_NAME "<2>tx_thread should stop now...\n");
			break;
		}
#endif
...
#if 0
		if (prGlueInfo->u4Flag & GLUE_FLAG_HALT) {
			pr_info(DRV_NAME "<3>tx_thread should stop now...\n");
			break;
		}
#endif
...
VOID kalEnqueueCommand(IN P_GLUE_INFO_T prGlueInfo, IN P_QUE_ENTRY_T prQueueEntry)
{
	...
	if (prCmdInfo->prPacket &&
		...
		prMsduInfo->u4InqueTime = prCmdInfo->u4InqueTime;
	}
	/*end*/
	...
	)
{
	...
	do {
		if ((prGlueInfo == NULL) ||
			(pucFrameBuf == NULL) ||
			(u4FrameLen == 0)) {
			DBGLOG(AIS, TRACE, ("Unexpected pointer PARAM. %p, %p, %lu.",
...
#define DBG_MGMT_FRAME_INDICATION 1
	...
	do {
		...
		case MAC_FRAME_ACTION:
			pr_info("RX Action frame at channel %d ", ucChnlNum);
			break;
		default:
