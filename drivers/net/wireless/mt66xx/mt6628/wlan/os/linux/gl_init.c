...
** $Log: gl_init.c $
**
** 12 19 2013 yuche.tsai
** [ALPS01246661] [wifi] It is too slow to search the second ap when add two hidded ssid into wifi list
** Enable combo scan feature.
**
** Review: http://mtksap20:8080/go?page=NewReview&reviewid=85659
 *
...
BOOLEAN wlanSubModInit(P_GLUE_INFO_T prGlueInfo)
{
	...
	set_bit(GLUE_FLAG_SUB_MOD_INIT_BIT, &prGlueInfo->u4Flag);
	/* wake up main thread */
...
BOOLEAN wlanSubModExit(P_GLUE_INFO_T prGlueInfo)
{
	...
	/* 4  Mark HALT, notify main thread to finish current job */
	set_bit(GLUE_FLAG_SUB_MOD_EXIT_BIT, &prGlueInfo->u4Flag);
	/* wake up main thread */
	wake_up_interruptible(&prGlueInfo->waitq);
...
static struct wireless_dev *wlanNetCreate(PVOID pvData)
{
...
#endif
	if (!prDev) {
		pr_alert(DRV_NAME "unable to get struct dev for wlan\n");
	}
 ...
 netcreate_err:
	...
	if ((NULL != prGlueInfo) && (NULL != prGlueInfo->prDevHandler)) {
...
static int set_p2p_mode_handler(struct net_device *netdev, PARAM_CUSTOM_P2P_SET_STRUC_T p2pmode)
{
	...
	pr_info("set_p2p_mode_handler ret = %lu\n", rWlanStatus);
...
static int __init initWlan(void)
{
	...
	pr_info("[wlan] -- patch for ALPS01428223\n");
	...
	aucDebugModule[DBG_MEM_IDX] = DBG_CLASS_ERROR|DBG_CLASS_WARN | DBG_CLASS_LOUD;
#endif				/* DBG */
