		 ...
		 struct ieee80211_channel *mtk_channels, UINT_8 mtk_channel_sz)
{
	...
	while (TRUE) {
		...
		if (*pucChIdx == ucCurChIdx) {
			pr_alert(DRV_NAME "Orphan channel [%d]\n", ucChannelNum);
			break;
		}
	}
}
...
BOOLEAN p2pNetRegister(P_GLUE_INFO_T prGlueInfo, BOOLEAN fgIsRtnlLockAcquired)
{
	...
	if (register_netdev(prGlueInfo->prP2PInfo->prDevHandler) < 0) {
		pr_alert(DRV_NAME "unable to register netdevice for p2p\n");
...
VOID p2pUpdateChannelTableByDomain(P_GLUE_INFO_T prGlueInfo)
{
	...
	P_RF_CHANNEL_INFO_T aucChannelList = NULL;

	uc2gChIdx = uc5gChIdx = 0;
	aucChannelList = kalMemAlloc((sizeof(struct _RF_CHANNEL_INFO_T)*ucMaxChannelNum), VIR_MEM_TYPE);
	...
	for (i = 0; i < ucNumOfChannel; i++) {
		...
		default:
			pr_alert(DRV_NAME "Unknow band.\n");
			break;

		}
	}
	kalMemFree(aucChannelList, VIR_MEM_TYPE, (sizeof(struct _RF_CHANNEL_INFO_T)*ucMaxChannelNum));
}
...
BOOLEAN glRegisterP2P(P_GLUE_INFO_T prGlueInfo, const char *prDevName, BOOLEAN fgIsApMode)
{
	...
	pr_info("glRegisterP2P\n");

	/*0. allocate p2pinfo */
	if (!p2PAllocInfo(prGlueInfo)) {
		pr_alert(DRV_NAME "Allocate memory for p2p FAILED\n");
		ASSERT(0);
		return FALSE;
	}
#if CFG_ENABLE_WIFI_DIRECT_CFG_80211
	...
	if (!prGlueInfo->prP2PInfo->wdev.wiphy) {
		pr_alert(DRV_NAME "unable to allocate wiphy for p2p\n");
	...
	if (!prDev) {
		pr_alert(DRV_NAME "unable to get struct dev for p2p\n");
	}
#else
	...
	if (wiphy_register(prGlueInfo->prP2PInfo->wdev.wiphy) < 0) {
		pr_alert(DRV_NAME "Couldn't register wiphy device for p2p\n");
	...
	*((P_GLUE_INFO_T *) wiphy_priv(prGlueInfo->prP2PInfo->wdev.wiphy)) = prGlueInfo;
	...
	if (!prGlueInfo->prP2PInfo->prDevHandler) {
		pr_alert(DRV_NAME "unable to allocate netdevice for p2p\n");

		pr_info("unable to allocate netdevice for p2p\n");
...
BOOLEAN glUnregisterP2P(P_GLUE_INFO_T prGlueInfo)
{
	...
	if (!p2PFreeInfo(prGlueInfo)) {
		pr_alert(DRV_NAME "Free memory for p2p FAILED\n");
		ASSERT(0);
		return FALSE;
	}
#endif
...
/* ASSERT(prAdapter); */
	...
	if (!prFuncSwitch) {
		...
		pr_alert(DRV_NAME "Allocate for p2p mesasage FAILED\n");
		/* return -ENOMEM; */
	}
...
static void p2pSetMulticastList(IN struct net_device *prDev)
{
	...
	ASSERT(prGlueInfo);
	if (!prDev || !prGlueInfo) {
		pr_warn(DRV_NAME " abnormal dev or skb: prDev(0x%p), prGlueInfo(0x%p)\n", prDev,
			prGlueInfo);
		return;
	}
...
int p2pHardStartXmit(IN struct sk_buff *prSkb, IN struct net_device *prDev)
{
	...
	if (test_bit(GLUE_FLAG_HALT_BIT, &prGlueInfo->u4Flag)) {
		pr_info(DRV_NAME "GLUE_FLAG_HALT skip tx\n");
		dev_kfree_skb(prSkb);
		return NETDEV_TX_OK;
	}
	/* mark as P2P packets */
	...
	if (wlanProcessSecurityFrame(prGlueInfo->prAdapter, (P_NATIVE_PACKET) prSkb) == FALSE) {
		...
		if (u2QueueIdx >= CFG_MAX_TXQ_NUM) {
			pr_info(DRV_NAME "Incorrect queue index, skip this frame\n");
			dev_kfree_skb(prSkb);
			return NETDEV_TX_OK;
		}
			   ...
			   IN OUT union iwreq_data *wrqu, IN OUT char *extra)
{
	...
	/* pr_info("wext_set_power value(%d) disabled(%d) flag(0x%x)\n", */
	...
	if (prPower->disabled) {
		...
		if (i4PowerValue == 0) {
			...
			pr_debug("%s(): unsupported power management mode value = %d.\n",
	...
	if (rStatus != WLAN_STATUS_SUCCESS) {
		/* pr_info(DRV_NAME"wlanoidSet802dot11PowerSaveProfile fail 0x%lx\n", rStatus); */
		return -EFAULT;
	}
#endif
...
mtk_p2p_wext_get_powermode(IN struct net_device *prNetDev,
			   ...
			   IN OUT union iwreq_data *wrqu, IN OUT char *extra)
{
	/* pr_info("mtk_p2p_wext_get_powermode\n"); */
				    ...
				    IN OUT union iwreq_data *wrqu, IN OUT char *extra)
{
	...
	pr_info("mtk_p2p_wext_get_sec_check_response\n");
	rStatus = kalIoctl(prGlueInfo,
			   ...
			   IN OUT union iwreq_data *wrqu, IN OUT char *extra)
{
	...
	pr_info("mtk_p2p_wext_set_noa_param\n");
...
/* P_IW_P2P_TRANSPORT_STRUCT   prP2PReq = (P_IW_P2P_TRANSPORT_STRUCT)extra; */
	...
	pr_info("mtk_p2p_wext_set_oppps_param\n");
