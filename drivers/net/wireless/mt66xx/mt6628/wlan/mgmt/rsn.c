...
BOOLEAN rsnCheckSecurityModeChanged(P_ADAPTER_T prAdapter, P_BSS_INFO_T prBssInfo, P_BSS_DESC_T prBssDesc)
{
	...
	/* privacy has changed, means security mode changed */
	if ((prBssDesc->u2CapInfo & CAP_INFO_PRIVACY) != (prBssInfo->u2CapInfo & CAP_INFO_PRIVACY))
		return TRUE;
	/* privacy bit is the same but both are not set, means security mode not changed */
	if (0 == (prBssDesc->u2CapInfo & CAP_INFO_PRIVACY))
		return FALSE;

	/* if auth mode is wpa, check if new beacon/probe response has WPA IE,
		and check the Group Cipher, Pariwise Cipher, and AKM */
	if (eAuthMode == AUTH_MODE_WPA || eAuthMode == AUTH_MODE_WPA_PSK || eAuthMode == AUTH_MODE_WPA_NONE)
		return prBssDesc->fgIEWPA ? rsnCheckWpaRsnInfo(prBssInfo, &prBssDesc->rWPAInfo):TRUE;
	else if (eAuthMode == AUTH_MODE_WPA2 || eAuthMode == AUTH_MODE_WPA2_PSK)
		return prBssDesc->fgIERSN ? rsnCheckWpaRsnInfo(prBssInfo, &prBssDesc->rRSNInfo):TRUE;

	/* eAuthMode is not wpa/wpa2, and privacy bit are both set, so security mode is not changed */
	return FALSE;
}
#endif
