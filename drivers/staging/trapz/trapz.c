	...
	extra4, struct trapz_info __user *, ti)
{
	...
	ret_val = systrapz(ctrl, extra1, extra2, extra3, extra4, &t);
	if (ret_val != 0)
		return ret_val;

	if (ti != NULL) {
		if (copy_to_user(ti, &t, sizeof(t)))
			return -EFAULT;
	}
	return 0;
}
...
#ifdef CONFIG_COMPAT
COMPAT_SYSCALL_DEFINE6(trapz, unsigned int, ctrl, unsigned int, extra1, unsigned int,
	extra2, unsigned int, extra3, unsigned int,
	extra4, struct compat_trapz_info __user *, ti)
{
	struct trapz_info t;
	long ret_val;
	...
	if (ti != NULL) {
		if (!access_ok(VERIFY_WRITE, ti, sizeof(struct compat_trapz_info)) ||
			put_compat_timespec(&(t.ts), &(ti->ts)) ||
			__put_user(t.counter, &ti->counter))
			return -EFAULT;
	}
	return 0;
}
