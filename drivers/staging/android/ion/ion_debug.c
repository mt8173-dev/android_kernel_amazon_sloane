...
void ion_debug_show_address_usage_record(struct ion_address_usage_record *address_usage_record)
{
	if (address_usage_record != NULL) {
		...
		printk(ION_DEBUG_INFO "===address_usage_record.address_type : %d\n",
		       address_usage_record->address_type);
		printk(ION_DEBUG_INFO "===address_usage_record.mapping_address : 0x%d\n",
	...
	if (backtrace_info == USER_BACKTRACE) {
		...
		if (tmpMapping != NULL) {
			...
			if (backtrace_symbol != NULL) {
				snprintf(backtrace_string, backtrace_string_len,
					 "KERNEL[%d] addr: 0x%u symbol: %p\n", backtrace_index,
					 backtrace[backtrace_index],
					 (backtrace_symbol + backtrace_index));
				/* snprintf(backtrace_string,backtrace_string_len,
...
void *ion_get_list(unsigned int record_type, void *record, unsigned int list_type)
{
	...
	if (record_type == LIST_BUFFER) {
		...
		case BUFFER_ALLOCATION_LIST:
			{
				printk(ION_DEBUG_INFO "[ion_get_list return]buffer using list %p\n",
				       buffer_record->buffer_using_list);
				ion_debug_show_buffer_usage_record
				    (buffer_record->buffer_using_list);
		...
		case ADDRESS_ALLOCATION_LIST:
			{
				printk(ION_DEBUG_INFO
				       "[ion_get_list return] address using list %p\n",
				       buffer_record->address_using_list);
				ion_debug_show_address_usage_record
				    (buffer_record->address_using_list);
		...
		case ADDRESS_FREE_LIST:
			{
				printk(ION_DEBUG_INFO
				       "[ion_get_list return] address freed list %p\n",
				       buffer_record->address_freed_list);
				ion_debug_show_address_usage_record
				    (buffer_record->address_freed_list);
		...
		case FD_ALLOCATION_LIST:
			{
				printk(ION_DEBUG_INFO
				       "[ion_get_list return]process fd using list %p\n",
				       process_record->fd_using_list);
		...
		case FD_FREE_LIST:
			{
				printk(ION_DEBUG_INFO
				       "[ion_get_list return]process fd free list %p\n",
				       process_record->fd_freed_list);
		...
		case ADDRESS_ALLOCATION_LIST:
			{
				printk(ION_DEBUG_INFO
				       "[ion_get_list return]process address using list %p\n",
				       process_record->address_using_list);
				ion_debug_show_address_usage_record
				    (process_record->address_using_list);
		...
		case ADDRESS_FREE_LIST:
			{
				printk(ION_DEBUG_INFO
				       "[ion_get_list return]process  address freed list %p\n",
				       process_record->address_freed_list);
				ion_debug_show_address_usage_record
				    (process_record->address_freed_list);
...
struct ion_process_record *ion_get_inuse_process_usage_record2(void)
{
	...
	if (tmp_process != NULL) {
		printk(ION_DEBUG_INFO
		       "[ion_get_inuse_process_record]return process_record  %p pid %d\n",
		...
		printk(ION_DEBUG_WARN
		       "[ion_get_inuse_process_recoed] tmp_process is null process_created_list is %p\n",
...
struct ion_client_usage_record *ion_get_inuse_client_record(void)
{
	...
	pr_info("[ion_get_inuse_client_record]return client_record  %p\n", tmp_client);
	if (tmp_client != NULL) {
...
struct ion_client_usage_record *ion_get_freed_client_record(void)
{
	...
	pr_info(ION_DEBUG_INFO "[ion_get_inuse_client_record]return client_record  %p\n",
		tmp_client);
	if (tmp_client != NULL) {
		printk(ION_DEBUG_INFO "[ion_get_inuse_client_record]return client %p\n",
		       tmp_client);
	...
	if (buffer_created_list != NULL) {
		while (tmp_buffer_record != NULL) {
			if (tmp_buffer_record->buffer_address == buffer) {
				printk(ION_DEBUG_INFO
				       "               found record tmp_buffer_record: 0x%p\n",
				       tmp_buffer_record);
		...
		printk(ION_DEBUG_INFO
		       "		[search_record_in_list]can't get corresponding buffer %p in buffer list %p\n",
		       buffer, list);
	} else {
		printk(ION_DEBUG_INFO
...
struct ion_process_record *search_process_in_list(pid_t pid, struct ion_process_record *list)
{
	...
	if (process_created_list != NULL) {
		while (tmp_process_record != NULL) {
			printk(ION_DEBUG_INFO
			       "               tmp_process_record: 0x%p pid %d count %d\n",
			       tmp_process_record, tmp_process_record->pid,
			       tmp_process_record->count);
			if (tmp_process_record->pid == pid) {
				printk(ION_DEBUG_INFO
				       "               found record tmp_process_record: 0x%p\n",
				       tmp_process_record);
		...
		printk(ION_DEBUG_INFO
		       "        [search_process_in_list]can't get corresponding process %x in process list %p\n",
		       ...
		       unsigned int *kernel_symbol)
{
	...
	for (i = 0; i < numEntries; i++) {
		...
		*(kernel_symbol + i) =
		    (unsigned long)get_kernelString_from_hashTable(symbol, strlen(symbol));
...
void *find_node_in_list(pid_t pid, unsigned int client_address, unsigned int data,
			unsigned int **previous_node, unsigned int *list,
			unsigned int search_type, unsigned int node_type)
{
	...
	while (current_node != NULL) {
		if ((search_type == SEARCH_PROCESS_PID) && (node_type == LIST_PROCESS)
			...
			printk(ION_DEBUG_INFO
			       "            [find_node_in_list]curent_node is %p process_tmp->pid is %d process_tmp->count is %d\n",
			...
			printk(ION_DEBUG_INFO
			       "            [find_node_in_list]curent_node is %lu buffer_tmp->buffer_address is 0x%p data 0x%d\n",
			...
			printk(ION_DEBUG_INFO
			       "            [find_node_in_list]current_node is %p record_ID_tmp %p record_ID_tmp->pid %d record_ID_tmp->client_address %x\n",
			       current_node, record_ID_tmp,
	...
	printk(ION_DEBUG_INFO
	       "              [find_node_in_list]can't find node in list. search_type %d node_type %d pid %d client_address %d\n",
			    ...
			    unsigned int node_type)
{
	...
	printk(ION_DEBUG_INFO
	       "          [move_node_to_freelist]pid %d client_address %x from %p to %p search_type %d node_type %d\n",
	...
	printk(ION_DEBUG_INFO
	       "          [move_node_to_freelist] found_node %p previous_node is %p *to %p\n",
	...
	if (found_node != NULL) {
		...
		*to = found_node;
		printk(ION_DEBUG_INFO
		       "          [move_node_to_freelist]from list is %p to list is %p found_node is %p\n",
		       *from, *to, found_node);
		return found_node;
	} else {
		printk(ION_DEBUG_ERROR
		       "          [move_node_to_freelist]can't found node in list %p: node info pid %d client address %x\n",
	...
	case LIST_BUFFER:
		{
			...
			if (ion_list_buffer_cachep != NULL) {
				...
				new_buffer_record = (struct ion_buffer_record *)
				...
				if (new_buffer_record != NULL) {
					...
					pr_alert(KERN_ALERT
						 ...
						 list_buffer_size, buffer_node_size,
						 client_node_size, fd_node_size, mmap_node_size,
						 process_node_size, total_size);
	...
	case LIST_PROCESS:
		{
			...
			if (ion_list_process_cachep != NULL) {
				...
				process_record = (struct ion_process_record *)
...
void *get_record(unsigned int type, ion_sys_record_t *param)
{
	...
	if (tmp != NULL) {
		...
		case HASH_NODE_BUFFER:
			{
				hash = (unsigned int)
				    hash_32(((unsigned long)param->buffer + param->buffer->size +
		...
		default:
			{
				printk(ION_DEBUG_ERROR
				       "[get_record][ERROR] get_record error type %d", type);
				break;
			}
		}
	}
				     ...
				     unsigned int node_type, unsigned int from_kernel)
{
	...
	printk
	    ("[%d][%d][create_new_record_into_process(%d)] process_record 0x%p node_type %d new node %p\n",
	     record_param->pid, record_param->group_id, from_kernel, process_record, node_type,
	     new_node);
		...
		case ION_FUNCTION_DESTROY_CLIENT:
			{
				...
				printk(ION_DEBUG_TRACE
				       "[%d][%d][ION_FUNCTION_CLOSE(%d)]  client_freed_list %p\n",
				...
				found_node =
				    move_node_to_freelist(record_param->pid,
							  (unsigned long)record_param->client, 0,
				...
				if (found_node != NULL) {
					...
					printk(ION_DEBUG_ERROR
					       "[%d][%d][ION_FUNCTION_CLOSE(%d)]ERROR can't find client: 0x%p in client_using_list %p\n",
					       record_param->pid, record_param->group_id,
					       from_kernel, record_param->client,
					       client_using_list);
					break;
				}
				if (from_kernel == 0) {
					...
					printk(ION_DEBUG_TRACE
					       "[%d][%d][ION_FUNCTION_CLOSE(%d)]  DONE prepare to move process %d into destroyed list %p\n",
					       record_param->pid, record_param->group_id,
					...
					printk(ION_DEBUG_TRACE
					       "[%d][%d][ION_FUNCTION_CLOSE(%d)]  DONE process_destroyed_list %p\n",
					       record_param->pid, record_param->group_id,
					       from_kernel, process_destroyed_list);
				}
				printk(ION_DEBUG_TRACE
				       "[%d][%d][ION_FUNCTION_CLOSE(%d)]  DONE client_freed_list %p\n",
				       record_param->pid, record_param->group_id, from_kernel,
				       client_freed_list);
		...
		case ION_FUNCTION_ALLOC_CONT:
			{
				...
				printk(ION_DEBUG_TRACE
				       "[%d][%d][ION_FUNCTION_ALLOC(%d)_%d]buffer: 0x%p\n",
				       record_param->pid, record_param->group_id, from_kernel,
				       record_param->action, record_param->buffer);
				if (record_param->buffer == NULL) {
					printk(ION_DEBUG_ERROR
					       "[%d][%d][ION_FUNCTION_ALLOC(%d)_%d]ERROR!!! buffer: 0x%p is NULL\n",
					       record_param->pid, record_param->group_id,
				...
				if (new_buffer_record != NULL) {
					/* assign data into buffer usage record */
					new_node = (struct ion_buffer_usage_record *)
					...
					printk(ION_DEBUG_ERROR
					       "[%d][%d][ION_FUNCTION_ALLOC(%d)_%d]ERROR!!! buffer: 0x%p can't get  enough memory for create LIST_BUFFER structure\n",
					       record_param->pid, record_param->group_id,
					       from_kernel, record_param->action,
					       record_param->buffer);
					break;
				...
				printk(ION_DEBUG_TRACE
				       "[%d][%d][ION_FUNCTION_ALLOC(%d)_%d] DONE create buffer_usage_record : 0x%p buffer_created_list: 0x%p new buffer record: 0x%p buffer_using_list: 0x%p\n",
				       record_param->pid, record_param->group_id, from_kernel,
				       record_param->action, new_node, buffer_created_list,
				       new_buffer_record, new_buffer_record->buffer_using_list);
				break;
			}
		case ION_FUNCTION_IMPORT:
			{
				...
				struct ion_buffer_usage_record *new_node = NULL;
				printk(ION_DEBUG_TRACE
				       "[%d][%d][ION_FUNCTION_IMPORT(%d)]input buffer: 0x%p fd %d\n",
				       record_param->pid, record_param->group_id, from_kernel,
				       record_param->buffer, (unsigned int)record_param->fd);
				if (record_param->buffer == NULL) {
					printk(ION_DEBUG_ERROR
					       "[%d][%d][ION_FUNCTION_IMPORT(%d)]ERROR!!! buffer: 0x%p is NULL\n",
					       record_param->pid, record_param->group_id,
					       from_kernel, record_param->buffer);
				}
				mutex_lock(&buffer_lifecycle_mutex);
				buffer_record =
				...
				if (buffer_record == NULL) {
					printk(ION_DEBUG_ERROR
					       "[%d][%d][ION_FUNCTION_IMPORT(%d)]can't found corresponding buffer 0x%p in buffer created list %p",
					       record_param->pid, record_param->group_id,
					       from_kernel, record_param->buffer,
					       buffer_created_list);
					break;
				...
				printk(ION_DEBUG_TRACE
				       "[%d][%d][ION_FUNCTION_IMPORT(%d)]DONE new_node %p buffer_using_list%p\n",
				       record_param->pid, record_param->group_id, from_kernel,
				       new_node, buffer_record->buffer_using_list);
				break;
			}
		...
		case ION_FUNCTION_FREE:
			{
				...
				struct ion_buffer_usage_record *found_node = NULL;
				printk(ION_DEBUG_TRACE
				       "[%d][%d][ION_FUNCTION_FREE(%d)]input buffer: 0x%p\n",
				       record_param->pid, record_param->group_id, from_kernel,
				       record_param->buffer);
				if (record_param->buffer == NULL) {
					printk(ION_DEBUG_ERROR
					       "[%d][%d][ION_FUNCTION_FREE(%d)]ERROR!!! buffer: 0x%p is NULL\n",
					       record_param->pid, record_param->group_id,
					       from_kernel, record_param->buffer);
				}
				...
				if (buffer_record == NULL) {
					printk(ION_DEBUG_ERROR
					       "[%d][%d][ION_FUNCTION_FREE(%d)]can't found corresponding buffer 0x%p in buffer created list %p",
					       record_param->pid, record_param->group_id,
					       from_kernel, record_param->buffer,
					       buffer_created_list);
					break;
				...
				if (found_node != NULL) {
					...
					printk(ION_DEBUG_ERROR
					       "[%d][%d][ION_FUNCTION_FREE(%d)]can't found corresponding buffer usage record client 0x%p in buffer using list 0x%p",
					       record_param->pid, record_param->group_id,
					       from_kernel, record_param->client,
					       buffer_record->buffer_using_list);
					break;
				...
				if (buffer_record->buffer_using_list == NULL) {
					...
					move_node_to_freelist(record_param->pid, 0,
							      (unsigned long)record_param->buffer,
					...
					mutex_unlock(&buffer_lifecycle_mutex);
					printk(ION_DEBUG_TRACE
					       "[%d][%d][ION_FUNCTION_FREE(%d)]move buffer record %p from buffer_created_list %p to buffer_destroyed_list %p\n",
					       record_param->pid, record_param->group_id,
					       from_kernel, buffer_record, buffer_created_list,
					       buffer_destroyed_list);
				...
				printk(ION_DEBUG_TRACE
				       "[%d][%d][ION_FUNCTION_FREE(%d)] DONE buffer_using_list %p buffer_freed_list %p\n",
				       record_param->pid, record_param->group_id, from_kernel,
				       buffer_record->buffer_using_list,
				       buffer_record->buffer_freed_list);
		...
		case ION_FUNCTION_MMAP:
			{
				...
				printk(ION_DEBUG_TRACE
				       "[%d][%d][ION_FUNCTION_MMAP(%d)]input buffer: 0x%p mapping_address %x length %d\n",
				       record_param->pid, record_param->group_id, from_kernel,
				       record_param->buffer, record_param->address,
				       record_param->length);
				if ((record_param->buffer == NULL) && (record_param->fd != 0)
					...
					printk(ION_DEBUG_TRACE
					       "[%d][%d][ION_FUNCTION_MMAP(%d)]get buffer from fd %d input buffer: 0x%p\n",
					       record_param->pid, record_param->group_id,
					       from_kernel, record_param->fd, record_param->buffer);
				}
				if (record_param->buffer == NULL) {
					printk(ION_DEBUG_ERROR
					       "[%d][%d][ION_FUNCTION_MMAP(%d)]ERROR!!! buffer: 0x%p is NULL\n",
					       record_param->pid, record_param->group_id,
					       from_kernel, record_param->buffer);
					break;
				}
				if (from_kernel) {
					...
					if (buffer_record == NULL) {
						printk(ION_DEBUG_ERROR
						       "[%d][%d][ION_FUNCTION_MMAP(%d)]ERROR !!! can't found corresponding buffer 0x%p in buffer created list %p",
						       record_param->pid, record_param->group_id,
						       from_kernel, record_param->buffer,
						       buffer_created_list);
						break;
					...
					new_node = (struct ion_address_usage_record *)
					    create_new_record_into_list(record_param, buffer_record,
									NODE_MMAP, from_kernel);
					printk(ION_DEBUG_TRACE
					       "[%d][%d][ION_FUNCTION_MMAP(%d)] DONE new node:  0x%p in buffer record address mapping 0x%x size %d address using list: 0x%p\n",
					       record_param->pid, record_param->group_id,
					       from_kernel, new_node,
					...
					if (process_record == NULL) {
						printk(ION_DEBUG_ERROR
						       "[%d][%d][ION_FUNCTION_MMAP(%d)]ERROR !!! can't found corresponding process %d in process created list %p",
						       record_param->pid, record_param->group_id,
						       from_kernel, record_param->pid,
						       process_created_list);
						break;
					}
					new_node2 = (struct ion_address_usage_record *)
					    create_new_record_into_process(record_param,
									   process_record,
									   NODE_MMAP, from_kernel);
					printk(ION_DEBUG_TRACE
					       "[%d][%d][ION_FUNCTION_MMAP(%d)] DONE new node 0x%p in process record address mapping 0x%x size %d address using list 0x%p\n",
					       record_param->pid, record_param->group_id,
					       from_kernel, new_node2,
					       new_node2->mapping_address,
					       new_node2->size, process_record->address_using_list);
				}
				break;
			}
		case ION_FUNCTION_MUNMAP:
			{
				...
				printk(ION_DEBUG_TRACE
				       "[%d][%d][ION_FUNCTION_MUNMAP(%d)]input buffer: 0x%p address 0x%x size %d\n",
				       record_param->pid, record_param->group_id, from_kernel,
				       record_param->buffer, record_param->address,
				       record_param->length);
				if (from_kernel) {
					...
					if (buffer_record == NULL) {
						printk(ION_DEBUG_ERROR
						       "[%d][%d][ION_FUNCTION_MUNMAP(%d)]ERROR !!! can't found corresponding buffer 0x%p in buffer created list %p\n",
						       record_param->pid, record_param->group_id,
						       from_kernel, record_param->buffer,
						       buffer_created_list);
						break;
					...
					if (found_node != NULL) {
						...
						printk(ION_DEBUG_ERROR
						       "[%d][%d][ION_FUNCTION_MUNMAP(%d)]can't found corresponding buffer usage record client 0x%p in address using list 0x%p\n",
						       record_param->pid, record_param->group_id,
						       from_kernel,
						       record_param->client,
						       buffer_record->address_using_list);
					...
					if (process_record == NULL) {
						printk(ION_DEBUG_ERROR
						       "[%d][%d][ION_FUNCTION_MUNMAP(%d)]ERROR !!! can't found corresponding process pid %d in process created list %p\n",
						       record_param->pid, record_param->group_id,
						       from_kernel, record_param->pid,
						       process_created_list);
						break;
					...
					if (found_node2 != NULL) {
						record_release_backtrace(record_param,
									 &
									 (found_node2->tracking_info),
									 from_kernel);
					} else {
						printk(ION_DEBUG_ERROR
						       "[%d][%d][ION_FUNCTION_MUNMAP(%d)]can't found corresponding address usage record process 0x%d in address using list 0x%p\n",
						       record_param->pid, record_param->group_id,
						       from_kernel, record_param->pid,
						       process_record->address_using_list);
						break;
					...
					printk(ION_DEBUG_TRACE
					       "[%d][%d][ION_FUNCTION_MUNMAP(%d)] DONE client %p process->address_using_list %p process->address_free_list %p\n",
					       record_param->pid, record_param->group_id,
					       from_kernel, record_param->client,
					       process_record->address_using_list,
					       process_record->address_freed_list);
		...
		case ION_FUNCTION_SHARE_CLOSE:
			{
				...
				printk(ION_DEBUG_TRACE
				       "[%d][%d][ION_FUNCTION_SHARE_CLOSE(%d)]input buffer: 0x%p fd %d\n",
				       record_param->pid, record_param->group_id, from_kernel,
				       record_param->buffer, record_param->fd);
				mutex_lock(&process_lifecycle_mutex);
				process_record =
				...
				if (process_record == NULL) {
					printk(ION_DEBUG_ERROR
					       "[%d][%d][ION_FUNCTION_SHARE_CLOSE(%d)]ERROR !!!can't found corresponding process %d in process created list %p\n",
					       record_param->pid, record_param->group_id,
				...
				found_node =
				    move_node_to_freelist(record_param->pid,
							  (unsigned long)record_param->client,
							  record_param->fd,
							  (unsigned int **)
							  &(process_record->fd_using_list),
							  (unsigned int **)
				...
				if (found_node != NULL) {
					...
					printk(ION_DEBUG_ERROR
					       "[%d][%d][ION_FUNCTION_SHARE_CLOSE(%d)]can't found corresponding process  record client 0x%p in fd using list 0x%p\n",
					       record_param->pid, record_param->group_id,
					       from_kernel, record_param->client,
					       process_record->fd_using_list);
					break;
				...
				printk(ION_DEBUG_TRACE
				       "[%d][%d][ION_FUNCTION_SHARE_CLOSE(%d)] client %p fd_using_list %p fd_freed_list %p\n",
				       record_param->pid, record_param->group_id, from_kernel,
				       record_param->client,
				       process_record->fd_using_list,
				       process_record->fd_freed_list);
