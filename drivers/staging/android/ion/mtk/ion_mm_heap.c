			    ...
			    struct ion_buffer *buffer, ion_phys_addr_t *addr, size_t *len)
{
	...
	if (pBufferInfo->MVA == 0) {
		int ret = m4u_alloc_mva_sg(pBufferInfo->eModuleID, buffer->sg_table, buffer->size,
...
static int ion_mm_heap_debug_show(struct ion_heap *heap, struct seq_file *s, void *unused)
{
...

	{
		...
		for (n = rb_first(&dev->buffers); n; n = rb_next(n)) {
			...
			ION_PRINT_LOG_OR_SEQ(s, "%p %8zu %3d %3d %3d %3d %8x %3u %3lu %3d %s "
	...
	for (n = rb_first(&dev->clients); n; n = rb_next(n)) {
...

		{
			...
			for (m = rb_first(&client->handles); m; m = rb_next(m)) {
				...
				ION_PRINT_LOG_OR_SEQ(s,
						     "\thandle=%p, buffer=%p, heap=%d, backtrace is:\n",
						     handle,
						     handle->buffer, handle->buffer->heap->id);
			...
			if ((1 << buffer->heap->id) & ION_HEAP_MULTIMEDIA_MASK) {
				...
				if (!buffer->handle_count) {
					lmk_add_to_buffer("%16.s(%16.s) %16u %16zu %d %d\n",
							buffer->task_comm, "orphan", buffer->pid,
							buffer->size, buffer->kmap_cnt,
							atomic_read(&buffer->ref.refcount));
					total_orphaned_size += buffer->size;
				}
...
struct ion_heap *ion_mm_heap_create(struct ion_platform_heap *unused)
{
	...
	heap->info_cache = kmem_cache_create("ion_page_infos",
					     sizeof(struct page_info), 0, SLAB_HWCACHE_ALIGN, NULL);
	if (!heap->info_cache) {
			...
			if (Param.buf_debug_info_param.handle) {
				...
				kernel_handle =
				    ion_drv_get_handle(client, Param.buf_debug_info_param.handle,
						       ...
						       from_kernel);
				if (IS_ERR(kernel_handle)) {
					IONMSG("ion set debuf info fail! kernel_handle=0x%p.\n",
					       kernel_handle);
					ret = -EINVAL;
					break;
				}
	...
	case ION_MM_GET_DEBUG_INFO:
		{
			...
			if (Param.buf_debug_info_param.handle) {
				...
				if ((int)buffer->heap->type == ION_HEAP_TYPE_MULTIMEDIA) {
					...
					IONMSG
					    ("[ion_mm_heap]: Error. Cannot get dbg buffer that is not from multimedia heap.\n");
					ret = -EFAULT;
				}
				ion_drv_put_kernel_handle(kernel_handle);
			} else {
				IONMSG
				    ("[ion_mm_heap]: Error. get dbg buffer with invalid handle.\n");
				ret = -EFAULT;
			}
		}
		break;
	...
	case ION_MM_GET_SF_BUF_INFO:
		{
			...
			if (Param.sf_buf_info_param.handle) {
				...
				if ((int)buffer->heap->type == ION_HEAP_TYPE_MULTIMEDIA) {
					...
					IONMSG
					    ("[ion_mm_heap]: Error. Cannot get sf_buf_info buffer that is not from multimedia heap.\n");
					ret = -EFAULT;
				}
				ion_drv_put_kernel_handle(kernel_handle);
			} else {
				IONMSG
				    ("[ion_mm_heap]: Error. get sf_buf_info buffer with invalid handle.\n");
				ret = -EFAULT;
			}
		}
		break;
