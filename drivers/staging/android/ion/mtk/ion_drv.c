	...
	case ION_SYS_RECORD:
		{
			...
			if (!from_kernel && (Param.record_param.backtrace_num > 0)) {
				for (i = 0; i < Param.record_param.backtrace_num; i++) {
					...
					string_len =
					    strlen_user((void __user *)
							Param.record_param.mapping_record[i].name);
					if (string_len <= 0) {
					...
					ret = copy_from_user(tmp_string, (void __user *)
							     Param.record_param.mapping_record
							     [i].name, string_len);
					if (tmp_string != NULL) {
						Param.record_param.mapping_record[i].name =
...
int ion_drv_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR_OR_NULL(g_ion_device)) {
		pr_info("ion_device_create() error! device=%p\n", g_ion_device);
