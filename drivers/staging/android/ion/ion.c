...
static struct ion_handle *__ion_alloc(struct ion_client *client, size_t len,
			       size_t align, unsigned int heap_id_mask, unsigned int flags, bool grab_handle)
{
	struct ion_handle *handle;
	     ...
	     ion_phys_addr_t *addr, size_t *len)
{
	struct ion_buffer *buffer;
	int ret;
	MMProfileLogEx(ION_MMP_Events[PROFILE_GET_PHYS], MMProfileFlagStart, (unsigned long)client,
...
void ion_unmap_kernel(struct ion_client *client, struct ion_handle *handle)
{
	...
	ion_debug_kern_rec(client, handle->buffer, handle, ION_FUNCTION_MUNMAP,
			   ADDRESS_KERNEL_VIRTUAL, (unsigned long)handle->buffer->vaddr,
	...
	while ((n = rb_first(&client->handles))) {
		...
		IONMSG
		    ("warning: release handle @ client destory: handle=%p, buf=%p, ref=%d, size=%ld, kmap=%d\n",
		     handle, handle->buffer, atomic_read(&handle->buffer->ref.refcount), handle->buffer->size,
	...
	case ION_IOC_ALLOC:
		{
			...
			handle = __ion_alloc(client, data.allocation.len,
					     ...
					     data.allocation.heap_id_mask, data.allocation.flags, true);
			if (IS_ERR(handle))
				...
				for (hn = rb_first(&client->handles); hn; hn = rb_next(hn)) {
					...
					while (!!usg_rec) {
						seq_printf(s,
							   "%s [0x%x] %10s [%d] (%s [%d])\n",
							   "    client",
							   usg_rec->tracking_info.
							   recordID.client_address, "Process",
						...
						usg_rec = (struct ion_buffer_usage_record *)
	...
	case DBCL_MMAP:
		...
		pr_info(KERN_INFO "DBCL_MMAP\n");
		{
			...
			for (cn = rb_first(&dev->clients); cn; cn = rb_next(cn)) {
				...
				for (hn = rb_first(&client->handles); hn; hn = rb_next(hn)) {
					...
					while (!!adr_rec) {
						seq_printf(s,
							   ...
							   "Process",
							   adr_rec->tracking_info.recordID.pid,
							   "Address", adr_rec->mapping_address,
							   (adr_rec->mapping_address +
							    adr_rec->size), "Size", adr_rec->size);
						/* Show address allocation backtrace */
						ion_debugdb_show_backtrace(s,
									   &adr_rec->tracking_info,
									   ALLOCATE_BACKTRACE_INFO);
						/* Next address record */
						adr_rec = (struct ion_address_usage_record *)
						    ion_get_data_from_record((void *)
									     adr_rec, RECORD_NEXT);
					}

					adr_rec =
					    ion_get_list_from_buffer(buffer, ADDRESS_FREE_LIST);
					if (adr_rec != NULL) {
						seq_printf(s, "%10s\n",
				...
				client_rec = (struct ion_client_usage_record *)
				    ion_get_client_record(client);
				if (client_rec != NULL) {
					adr_rec =
					    ion_get_list_from_process
				...
				if (client_rec != NULL) {
					...
					while (!!fd_rec) {
						seq_printf(s, "%10s [%d] %10s [%d]\n",
							   "Process",
							   fd_rec->tracking_info.recordID.pid,
							   "inused fd", fd_rec->fd);
						/* Show address allocation backtrace */
						ion_debugdb_show_backtrace(s,
									   &fd_rec->tracking_info,
									   ALLOCATE_BACKTRACE_INFO);
						/* Next address record */
						fd_rec = (struct ion_fd_usage_record *)
					...
					fd_rec =
					    ion_get_list_from_process
					    (client_rec->tracking_info.recordID.pid, FD_FREE_LIST);
					if (fd_rec != NULL)
						seq_printf(s, "%10s\n", "<FD_FREE_LIST>");
					while (!!fd_rec) {
						seq_printf(s, "%10s [%d] %10s [%d]\n",
						...
						ion_debugdb_show_backtrace(s,
									   ...
									   RELEASE_BACKTRACE_INFO);
						/* Next address record */
						fd_rec = (struct ion_address_usage_record *)
						    ion_get_data_from_record((void *)fd_rec,
									     RECORD_NEXT);
					}
#endif
			...
			while (process_rec != NULL) {
				...
				while (!!adr_rec_user) {
					...
					ion_debugdb_show_backtrace(s, &adr_rec_user->tracking_info,
								   ALLOCATE_BACKTRACE_INFO);
					/* Next fd record */
					adr_rec_user = (struct ion_address_usage_record *)
				...
				while (!!adr_rec_user_free) {
					...
					ion_debugdb_show_backtrace(s,
								   &adr_rec_user_free->tracking_info,
								   RELEASE_BACKTRACE_INFO);
					/* Next fd record */
					adr_rec_user_free = (struct ion_address_usage_record *)
			...
			while (buf_rec != NULL) {
				...
				while (!!adr_rec_free) {
					...
					ion_debugdb_show_backtrace(s, &adr_rec_free->tracking_info,
								   RELEASE_BACKTRACE_INFO);
					/* Next address record */
					adr_rec_free = (struct ion_address_usage_record *)
			...
			while (process_rec != NULL) {
				...
				while (!!fd_rec2) {
					...
					ion_debugdb_show_backtrace(s, &fd_rec2->tracking_info,
								   RELEASE_BACKTRACE_INFO);
					/* Next fd record */
					fd_rec2 = (struct ion_fd_usage_record *)
		...
		if (dbis_child_attr[index].attr == DBIS_FILE) {
			...
			for (his_index = 0; his_index < _TOTAL_DBIS; ++his_index) {
				debug_db_root.dbis.history_record[his_index]
				    =
				    debugfs_create_file(dbis_child_attr[index + his_index + 1].name,
							0444, debug_db_root.dbis.child[index],
