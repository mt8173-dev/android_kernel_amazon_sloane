...
/*static struct task_struct *lowmem_deathpending;*/ /* ACOS_MOD_ONELINE {fwk_crash_log_collection} */
...
extern void gpu_mem_debug_lmk(void);
...
static int lowmem_shrink(struct shrinker *s, struct shrink_control *sc)
{
	...
	/* ACOS_MOD_BEGIN {fwk_crash_log_collection} */
	int print_extra_info = 0; /* move print_extra_info outside of CONFIG_MT_ENG_BUILD */
	/* ACOS_MOD_END {fwk_crash_log_collection} */
#ifdef CONFIG_MT_ENG_BUILD
...
/* ACOS_MOD_BEGIN {fwk_crash_log_collection} */
	...
	if (lmk_log_buffer && selected && selected_oom_score_adj == 0 && !print_extra_info) {
		...
		if (kill_msg_index && previous_crash)
			memcpy(previous_crash, kill_msg_index, ELEMENT_SIZE);
		lowmem_print(1, "======low memory killer=====\n");
		lowmem_print(1, "Free memory other_free: %d, other_file:%d pages\n", other_free, other_file);
...
#ifdef CONFIG_ZRAM
			lowmem_print(1, "Candidate %d (%s), adj %d, score_adj %d, rss %lu, rswap %lu, to kill\n",
				p2->pid, p2->comm, lowmem_oom_score_adj_to_oom_adj(oom_score_adj2), oom_score_adj2, get_mm_rss(p2->mm),
...
/* ACOS_MOD_END {fwk_crash_log_collection} */
	...
	if (selected) {
		...
		if (print_extra_info || foreground_kill) {
...
#define MSG_SIZE_TO_AEE 70
	    ...
	    mlog(1);
#endif
		/* send_sig(SIGKILL, selected, 0); */ /* ACOS_MOD_ONELINE {fwk_crash_log_collection} */
...
static int __init lowmem_init(void)
{
	...
	vm_swappiness = 100;
#endif
	/* task_free_register(&task_nb); */ /* ACOS_MOD_ONELINE {fwk_crash_log_collection} */
...
static void __exit lowmem_exit(void)
{
	...
	/* task_free_unregister(&task_nb); */ /* ACOS_MOD_ONELINE {fwk_crash_log_collection} */
}
