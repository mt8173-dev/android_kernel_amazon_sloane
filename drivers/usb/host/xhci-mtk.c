...
int xhci_mtk_init_quirk(struct xhci_hcd *xhci)
{
	...
	xhci->sch_ports = sch_array;

	return 0;
}
EXPORT_SYMBOL_GPL(xhci_mtk_init_quirk);

void xhci_mtk_exit_quirk(struct xhci_hcd *xhci)
{
	kfree(xhci->sch_ports);
}
EXPORT_SYMBOL_GPL(xhci_mtk_exit_quirk);
		...
		struct usb_host_endpoint *ep)
{
	...
	sch_array = xhci->sch_ports;

	xhci_err(xhci, "%s() type:%d, speed:%d, mpkt:%d, dir:%d, ep:%p\n",
		...
		struct usb_host_endpoint *ep)
{
	...
	xhci_err(xhci, "%s() type:%d, speed:%d, mpks:%d, dir:%d, ep:%p\n",
		__func__, usb_endpoint_type(&ep->desc), udev->speed,
		GET_MAX_PACKET(usb_endpoint_maxp(&ep->desc)),
		usb_endpoint_dir_in(&ep->desc), ep);
